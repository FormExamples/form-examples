...
- `bin/publish-github-pages-subtree [--dry-run]` (also `make github-pages`, a thin `git subtree push --prefix=formexamples.github.io github-pages main` for when the `github-pages` remote is already configured) — per [`spec/monorepo-github-pages/`](spec/monorepo-github-pages/): export `formexamples.github.io/`'s tree via `git subtree push` to the sibling, top-level, read-only `github.com/FormExamples/formexamples.github.io` repo (the GitHub-convention `<org>.github.io` repo, auto-served at the domain root — unlike this monorepo's own Pages, which nests under `/form-examples/`). `formexamples.github.io/.github/workflows/deploy.yml` is inert here (GitHub only reads `.github/workflows/` at a repo's root) but becomes the sibling's real, root-level deploy workflow once exported, so the sibling builds and redeploys itself on every push this tool makes. Never work directly in the sibling clone — always re-publish from here. `--dry-run` splits without pushing; unlike `make github-pages`, `bin/publish-github-pages-subtree` also adds the `github-pages` remote if it's missing (e.g. on a fresh clone)
