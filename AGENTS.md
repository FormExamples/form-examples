...
- `bin/verify-personas [<slug>…]` — UI-level complement to `bin/test-personas`: per `personas.json` entry, drives the real wizard on a fresh headless page (`setState()`, autofill any still-empty required field, click `#submit-btn`), asserting no page error and a real rendered report — the submit/render path `test-personas` never touches. A post-autofill validation block is informational, not a failure (some personas are deliberately incomplete). 265/265 forms clean as of 2026-09-12. Slow — occasional gate, not routine CI
...
bin/verify-personas                            # personas.json vs the real wizard's submit/render path (slow — occasional gate, not routine CI)
