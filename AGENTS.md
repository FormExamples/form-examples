...
- `bin/test-vendored-uniformity [--verbose]` — verify the vendored theme catalogues (both stacks) and the one still-vendored Lily Svelte helper component, `DateTimePicker.svelte`, are byte-identical across every form. (The other four Svelte helpers — `ThemePicker`/`LocalePicker`/`TextSizePicker`/`SharePicker` — and `locales.ts` stopped being vendored fleet-wide when `bin/svelte-lily-pnpm-migrate` moved them to real pnpm dependencies; nothing left to hash is a stronger uniformity guarantee than a byte-compare ever was.) The CI-checkable half of the checkout-reading sync tools' invariant: fleet uniformity runs everywhere, upstream currency stays a maintainer-run check against the pinned checkout
...
[`AGENTS/back-end-with-loco.md`](AGENTS/back-end-with-loco.md) §Fleet-wide
maintenance tools, to keep this file under the repo's own 40KB target.
Each has a `--check` CI drift detector, listed in the Verify block below.
...
- `bin/svelte-autosave-restore-fix [--check] [--dry-run] [--all|<slug>…]` — fixes a real, fleet-wide bug found live: `id = $state('new')` defeats the page's own hydration guard on `/new`, so a saved draft never restores; a second defect (the persist effect's pre-hydration first run) also clobbers it with blanks. Fixes both, wires `RestoreBanner.svelte`. 305/355 forms, 2026-09-12. `--check` is the CI drift detector
- `bin/svelte-date-time-picker-vendor [--check|--apply]` — one-shot (superseded): used to vendor `DateTimePicker.svelte`, the fifth Lily Svelte helper, into every form's `front-end-with-svelte/src/lib/components/ui/`; vendor-only, not wired into any layout/route — the existing `DateInput.svelte` (native `<input type="date">`) stays the actual date field in every form. Superseded by `bin/svelte-lily-picker-defaults` below, which deleted the vendored copy fleet-wide as unneeded (unused by every form); do not re-run this tool to re-vendor it
...
- `bin/generate-export-samples [--check] [<slug>…]` — generates `examples/export-sample.csv`/`.tsv` per form matching its typical persona (`personas.json`'s first entry), by actually driving the real wizard in a headless browser (thin wrapper around `e2e/generate-export-samples.mjs`, same Playwright deps as `bin/test-e2e`): inject the persona via `window.__FORM_STATE__.setState()`, click the real "Download CSV"/"Download TSV" buttons, save the real download — guarantees fidelity rather than reimplementing the serialisation. Scoped to forms with both `js/form-export.js` wired and a `personas.json`: 265/356 done 2026-09-12 (91 SKIP, matching `bin/form-export-import-refactor`'s unmigrated set). Found + fixed a real bug along the way: `advance-statement-about-care`'s "Remove person" button queried a `.btn-remove` class its template never set, crashing the wizard's re-render whenever that section had a row. `--check` is the CI drift detector, but slow (drives a real browser per form) — an occasional gate, not a routine one
- `bin/verify-personas [<slug>…]` — UI complement to `bin/test-personas`: per `personas.json` entry, drives the real wizard (`setState()`, autofill, click `#submit-btn`) on a fresh page, asserting no page error and a real report — the submit/render path `test-personas` never touches. 265/265 clean; slow
- `bin/verify-blank-submit [<slug>…]` — sibling: loads each wizard's blank state, clicks `#submit-btn`, asserts no crash. Cheap stand-in for hand-curating an `example-invalid.json` + expected-error list per form. 348/348 clean
...
- `bin/svelte-helpers-picker-rename [--check|--apply]` — one-shot (superseded): renamed the four Svelte helper controls from `*-chooser` to `*-picker` names, matching upstream's `*-chooser` → `*-picker` package rename ("to harmonize with Adobe"; read fresh component source from the pinned checkout at apply time). Superseded by `bin/svelte-lily-pnpm-migrate` below, which replaced the vendored copies these renames operated on with real pnpm dependencies — almost no form has a `src/lib/components/ui/{Theme,Locale,TextSize,Share}Picker.svelte` left to rename, so `--check` now finds nothing fleet-wide (harmless, but do not run it as a drift detector)
- `bin/html-helpers-picker-rename [--check|--apply]` — the HTML-side half of the same rename: `text-size-chooser`→`text-size-picker`, `share-chooser`→`share-picker`. `#theme-select`/`#locale-select` are untouched (they mirror the unrelated, never-renamed catalog components); `--check` is the CI drift detector. Unaffected by the pnpm migration below — the HTML front-end still vendors these controls by design (no build step, no package manager)
- `bin/svelte-lily-pnpm-migrate [--check] [--dry-run] [--all|<slug>…]` — migrate every form's `front-end-with-svelte/` off vendored Lily Svelte components and onto the real, published pnpm packages: `@lilydesignsystem/svelte-headless` (added as a dependency; every local `components/ui/<Name>.svelte` that is a byte-for-byte mirror — modulo comments/quote style, verified against the published package, not just name — of a headless-catalogue component is deleted and every importer rewritten to `import { Name } from "@lilydesignsystem/svelte-headless"`) and the four picker packages, consolidated behind `@lilydesignsystem/svelte-picker-bar` in every form whose header wiring matches the fleet's standard four-picker block (355/356; the one exception, `medical-language-speaking-assessment-for-cymraeg`, drives the pickers through its own i18n store and keeps four separate `<ThemePicker>`/etc. tags — now importing from the real packages rather than a vendored copy, just not consolidated). Vendors one new file per PickerBar-consolidated form, `src/lib/config/share-targets.ts`, with the fleet's six share destinations (copy link, email, LinkedIn, Reddit, Bluesky, Mastodon via mastodonshare.com). A component whose local copy has diverged from upstream for a real reason (`Form.svelte`'s `novalidate`, `NumberInput.svelte`'s `null`-not-`undefined` default, `Field.svelte`/`Fieldset.svelte`'s extra CSS class hooks) is left vendored rather than silently overwritten — reported, not skipped silently. Reverses the "no runtime dependency" model in `forms/lily-svelte-version.md` / `forms/lily-svelte-helpers-version.md`; see those files for the new pnpm-dependency contract. `--check` is the CI drift detector
- `bin/svelte-lily-scope-rename [--check] [--dry-run] [--all|<slug>…]` — one-shot: rename the six Lily Svelte pnpm dependencies above from their original unscoped npm names (`lily-design-system-svelte-*`) to their `@lilydesignsystem/svelte-*` scoped names, matching upstream's package move the day after the pnpm migration above landed. Confirmed content-identical before renaming (a pure package-name move, not a behavioural change) — plain string substitution across every `package.json` dependency key and every `import ... from "<old-name>"` in any `.svelte`/`.ts` file; also applied to `formexamples.github.io/` (its own separate npm dependency on three of the six packages). `--check` is the CI drift detector
- `bin/svelte-lily-picker-defaults [--check] [--dry-run] [--all|<slug>…]` — apply [`spec/lily-design-system-svelte-with-picker-bar/`](spec/lily-design-system-svelte-with-picker-bar/) fleet-wide: `src/lib/config/themes.ts`'s `THEME_OPTIONS` is derived from `@lilydesignsystem/svelte-picker-bar`'s own `DEFAULT_THEMES` instead of a hand-maintained list — a real fix, not just a refactor, since every form's old list sorted the UK/US themes alphabetically among the rest rather than at the very end as the spec (and `DEFAULT_THEMES` itself) requires; `text-sizes.ts`'s `TEXT_SIZE_OPTIONS` is derived from the same package's `DEFAULT_SIZES` (labelled via `@lilydesignsystem/svelte-text-size-picker`'s `sizeName()`) instead of a hand-maintained, non-default four-value catalogue (`small`/`medium`/`large`/`x-large`) — the real Lily default is seven steps (`largest`/`larger`/`large`/`normal`/`small`/`smaller`/`smallest`), `DEFAULT_TEXT_SIZE` becomes `"normal"`, and `app.css`'s `:root[data-text-size="…"]` block is rewritten for the new scale; and the vendored, universally-unused `src/lib/components/ui/DateTimePicker.svelte` is deleted fleet-wide (356/356 forms had it vendored, 0/356 ever wired it into a route/layout/step). `--check` is the CI drift detector
...
- `bin/oxford-spelling-sweep [--check] [--dry-run]` — sweep hand-authored Markdown prose (every form's `index.md`/`AGENTS.md`/`CHANGELOG.md`/`plan.md`/`tasks.md`/`doc/**`/`spec/**`, plus the same root-level docs and `docs/`/`arc42/`/`spec/`/`AGENTS/`) from British `-ise`/`-isation` to Oxford `-ize`/`-ization` spelling, per [`spec/oxford-spelling/oxford-spelling.md`](spec/oxford-spelling/oxford-spelling.md). Converts only the Greek-derived verbs that spec lists (organise → organize, standardise → standardize, …); leaves English/French `-ise` roots alone (exercise, promise, surprise, comprise, practise, precise, disable, …) via a hand-classified convert list, not a blind suffix regex — the earlier blind-regex draft of this sweep (an abandoned branch) is exactly the failure mode this avoids. Protects fenced code blocks, inline code spans, Markdown link targets, *italic*/_italic_ spans (this repo's citation-title convention — including spans that hard-wrap across a line break, which needed a specific fix after the first pass silently missed one and "fixed" a citation into the wrong spelling), and an explicit `PROTECTED_PHRASES` list for exact proper nouns that appear as plain unmarked text (e.g. the *Human Fertilisation and Embryology Act 1990*, a UK statute always cited in its enacted British spelling regardless of Oxford's general rule). Excludes `spec/oxford-spelling/oxford-spelling.md` itself, whose own "Use (Oxford) | Not" table deliberately puts the wrong spelling in its right-hand column as a worked example — sweeping it would "fix" the example into two identical columns. `--check` is the CI drift detector
...
- `bin/sync-form-tasks-checkboxes [--check] [--all|<slug>…]` — check off stale per-form `tasks.md`/`plan.md` items whose named artefact already exists on disk: every form's checklist began as `bin/create-form`'s scaffold template, and fleet-wide tooling since has built almost all of it directly without ever going back to tick the original per-form list (1856 stale `tasks.md` + 69 stale `plan.md` lines found fleet-wide 2026-09-12). Deliberately conservative — a line only flips to `[x]` when *every* backtick-quoted token on it resolves to a real, non-empty file or directory relative to that form's own directory; a line with no backtick-quoted path, or any backtick span that's a command invocation rather than a path (`bin/test-form <slug>`, `cargo test` — unverifiable here without a live Postgres per crate, ~45s/crate observed), is left untouched rather than guessed at. 450/1856 `tasks.md` + 36/69 `plan.md` eligible and closed under this rule; the rest are prose-only scaffold items with no path claim to verify, correctly left open. The root `tasks.md` remains the actually-maintained fleet backlog; this only keeps the per-form snapshots from misleading a reader into thinking finished work is outstanding
...
bin/svelte-autosave-restore-fix --check        # Svelte autosave-restore bug fix + restore-banner drift detector
# bin/svelte-date-time-picker-vendor --check   # one-shot, superseded by svelte-lily-picker-defaults — vendored copy deleted fleet-wide as unneeded; do not run as a CI gate
...
bin/generate-export-samples --check            # per-form export-sample.csv/.tsv drift detector (265/356 forms; slow — drives a real browser)
bin/verify-personas                            # personas.json vs real wizard submit/render (slow)
bin/verify-blank-submit                        # blank-state submit never crashes
...
bin/svelte-lily-pnpm-migrate --check --all    # Svelte Lily vendoring -> real pnpm dependency drift detector (headless primitives + picker-bar consolidation)
bin/svelte-lily-scope-rename --check --all    # Svelte Lily unscoped -> @lilydesignsystem npm-scope rename drift detector
bin/svelte-lily-picker-defaults --check --all # Svelte Lily default theme-order/text-size-scale + DateTimePicker-retirement drift detector
...
# bin/svelte-helpers-picker-rename --check     # one-shot, superseded by svelte-lily-pnpm-migrate — the vendored copies it renamed are gone fleet-wide; always reports "nothing to do"; do not run as a CI gate
...
bin/oxford-spelling-sweep --check     # British -ise/-isation -> Oxford -ize/-ization prose drift detector
...
bin/test-vendored-uniformity          # vendored themes + DateTimePicker byte-identical fleet-wide
