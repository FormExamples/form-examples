import type { MicrobiologyCultureResult, Flag, FlagPriority } from './types';
import { hasCriticalOrganism, hasResistanceMarker, isPositiveCulture } from './utils';

/**
 * Detects safety-critical flags independently of the four axes. Flag
 * categories mirror sql/07_create_table_microbiology_culture_test_result_grade_flag.sql.
 * Flags are returned sorted high → medium → low priority.
 */
export function detectFlags(r: MicrobiologyCultureResult): Flag[] {
	const flags: Flag[] = [];
