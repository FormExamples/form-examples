# Microbiology Culture Test Result — agent instructions
...
structured microbiology report.

This is the **result/report counterpart** to `microbiology-culture-test-request`
(a referral that vets and triages an incoming MC&S request). It mirrors the
`ct-scan-test-result` gold template for layout, SQL conventions, and the
four-axis grading contract.

## Result semantics

A result form records *what the test found and what it means* — it is **not** a
referral. The source-of-truth record is `microbiology_culture_test_result`; the
clinician is the report **author/signer**, not a requester. The engine
...
- **Axis B — severity & structured reporting:** abnormalitySeverity
  ...
  (e.g. an infection-significance category).
- **Axis C — report completeness:** 0–100 % of mandatory report sections
  (history, specimen, microscopy/culture, sensitivities, impression).
- **Axis D — follow-up urgency:** routine / recommended / urgent / critical-alert,
  with a target timeframe and a recommended action.

A **critical organism** (positive blood culture, CSF isolate, CPE
(`resistance_cpe`), or any record flagged `critical_organism`)
**auto-escalates** Axis D to *critical-alert* and raises the
`critical-result-alert` flag regardless of the other axes (RCPath critical-result
communication). Choose the least-urgent band only when no rule fires.
