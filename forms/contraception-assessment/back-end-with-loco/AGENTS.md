# Contraception Assessment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Contraception Assessment form, built with axum + Loco +
...
- [`contraception_assessment/`](./contraception_assessment/) — the Loco crate: `src/contraception_assessment/` holds `app.rs`
...
`src/contraception_assessment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd contraception_assessment && cargo check --all-targets && cargo test
```
