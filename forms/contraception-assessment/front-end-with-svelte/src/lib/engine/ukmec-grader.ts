import type { AssessmentData, UKMECMethodResult, FiredRule, UKMECCategory, GradingResult } from './types';
import { ukmecRules, allMethods, methodLabels } from './ukmec-rules';
import { detectAdditionalFlags } from './flagged-issues';
import { calculateAge } from './utils';

/**
 * Pure function: produce the full {@link GradingResult} for an assessment.
 * Evaluates the UKMEC category per contraceptive method, derives the overall
 * highest category and the preferred-method category, and collects the
 * additional clinician flags. Shared by the wizard, the report, and the
 * clinician dashboard so every surface stays aligned.
 */
export function gradeContraception(data: AssessmentData): GradingResult {
	const { ukmecResults, firedRules } = evaluateUKMEC(data);
	const additionalFlags = detectAdditionalFlags(data, ukmecResults);

	const overallHighestCategory = Math.max(
		...ukmecResults.map((r) => r.category)
	) as UKMECCategory;

	const preferredMethod = data.preferencesPriorities.preferredMethod;
