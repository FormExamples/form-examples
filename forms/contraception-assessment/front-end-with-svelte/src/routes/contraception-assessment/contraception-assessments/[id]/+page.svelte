<script lang="ts">
	...
	function submit() {
		...
		assessment.result = gradeContraception(assessment.data);
		goto(`/contraception-assessment/contraception-assessments/${id}/report`);
	}
...
<main class="mx-16 px-4 py-6">
	<h1 class="text-2xl font-bold text-base-content">
		{isNew ? 'New contraception assessment' : `Contraception assessment ${id}`}
	</h1>
	<p class="mt-1 text-sm text-base-content/70">
		Complete the ten sections; the UKMEC categories per method and the overall category are computed
		on submit.
	</p>
	...
	<Form label="Contraception assessment" onsubmit={submit}>
		<Step1Demographics />
