...
function detectAdditionalFlags(data, ukmecResults) {
  ...
  if (data.preferencesPriorities.preferredMethod) {
    ...
    if (preferredResult && preferredResult.category >= 3) {
      flags.push({
        ...
        message:
          `Patient's preferred method (${preferredResult.methodLabel}) has UKMEC category ` +
          `${preferredResult.category} - ${preferredResult.reasons.join('; ')}`,
        priority: 'high'
      });
    }
  }
  ...
  if (data.medicalHistory.migraineWithAura === 'yes') {
    flags.push({
      ...
      message:
        'Migraine with aura - all combined hormonal contraception (COC, patch, ring) is UKMEC 4 due to increased stroke risk',
      priority: 'high'
    });
  }
  ...
  if (
    ...
    flags.push({
      ...
      message:
        `Smoker aged ${age} (>= 35) - combined hormonal contraception carries increased cardiovascular risk`,
      priority: 'high'
    });
  }
      ...
      if (!isNaN(delivery.getTime())) {
        ...
        if (weeksSinceDelivery < 6) {
          flags.push({
            ...
            category: 'Postpartum',
            message:
              `Breastfeeding and ${Math.round(weeksSinceDelivery)} weeks postpartum (< 6 weeks) - CHC is UKMEC 4`,
            priority: 'high'
          });
        }
      }
  ...
  if (
    ...
    data.cardiovascularRisk.bloodPressureSystolic !== undefined &&
    ...
    flags.push({
      ...
      message:
        `Uncontrolled hypertension (systolic ${data.cardiovascularRisk.bloodPressureSystolic} mmHg) - CHC is UKMEC 4`,
      priority: 'high'
    });
  }
  ...
  if (data.medicalHistory.breastCancer === 'yes') {
    flags.push({
      ...
      category: 'Oncology',
      message:
        'Current or recent breast cancer - all hormonal contraception is UKMEC 4. Consider copper IUD or barrier methods.',
      priority: 'high'
    });
  }
  ...
  if (data.medicalHistory.hiv === 'yes') {
    flags.push({
      ...
      message:
        'HIV positive - consider drug interactions with antiretroviral therapy and ensure barrier method counselling for STI prevention',
      priority: 'medium'
    });
  }
  ...
  if (data.medicalHistory.stiHistory === 'yes') {
    flags.push({
      ...
      message:
        'History of STI - ensure barrier method counselling alongside chosen contraception',
      priority: 'medium'
    });
  }
  ...
  if (
    ...
    flags.push({
      ...
      category: 'Obesity',
      message:
        `BMI ${data.cardiovascularRisk.bmi} (>= 35) - CHC is UKMEC 3, consider progestogen-only or non-hormonal methods`,
      priority: 'medium'
    });
  }
  ...
  if (data.medicalHistory.liverDisease === 'yes') {
    flags.push({
      ...
      category: 'Hepatic',
      message:
        'Active liver disease - CHC and POP carry increased risk. Consider copper IUD or barrier methods.',
      priority: 'medium'
    });
  }

  // Cervical screening overdue (> 3 years)
  if (data.breastCervicalScreening.lastCervicalScreening) {
    ...
    if (!isNaN(lastScreen.getTime())) {
      const today = new Date();
      const yearsSinceScreen =
        (today.getTime() - lastScreen.getTime()) / (1000 * 60 * 60 * 24 * 365.25);
      if (yearsSinceScreen > 3) {
        flags.push({
          id: 'FLAG-CERVICAL-OVERDUE',
          category: 'Screening',
          message:
            `Cervical screening may be overdue (last: ${data.breastCervicalScreening.lastCervicalScreening}) - advise scheduling`,
          priority: 'low'
        });
      }
    }
