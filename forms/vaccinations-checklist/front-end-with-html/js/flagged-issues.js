// Flagged-issue detection for the Vaccinations Checklist. Independent of
// the compliance/risk classification produced by the grader, this module
// raises clinician-facing safety alerts (anaphylaxis history, immune
// status, pregnancy, allergy/contraindication, exposure incident, OH
// clearance, etc.). Ported 1:1 from the SvelteKit reference engine
// `flagged-issues.ts`.
...
function detectAdditionalFlags(data) {
  ...
  if (
    data.contraindicationsAllergies.eggAllergy === 'yes' &&
    data.contraindicationsAllergies.eggAllergySeverity === 'anaphylaxis'
  ) {
    flags.push({
      id: 'FLAG-EGG-001',
      category: 'Allergy',
