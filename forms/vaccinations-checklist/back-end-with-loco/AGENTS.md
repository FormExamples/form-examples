# Vaccinations Checklist — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Vaccinations Checklist form, built with axum + Loco +
...
- [`vaccinations_checklist/`](./vaccinations_checklist/) — the Loco crate: `src/vaccinations_checklist/` holds `app.rs`
...
`src/vaccinations_checklist/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd vaccinations_checklist && cargo check --all-targets && cargo test
```
