...
use crate::engine::{
    types::AssessmentData, utils::compliance_status_label, vaccination_grader,
};
...
///
/// Form field naming convention: `section.field` where both `section`
/// and `field` are already in camelCase to match the JSON keys exactly.
/// This avoids snake↔camel ambiguity around acronym-heavy field names
/// such as `mmrDose1`, `tbIGRAResult`, `historyOfGBS`.
#[debug_handler]
    ...
    if let Some(obj) = data_value.as_object_mut() {
        if let Some(form_obj) = form_data.as_object() {
            for (key, value) in form_obj {
                // section.field -- dotted camelCase keys.
                ...
                if parts.len() == 2 {
                    ...
                    {
                        section_obj.insert(field.to_string(), parse_form_value(value));
                    }
                }
            }
        }
...
async fn show_report(
    ...
    let grade = vaccination_grader::grade(&assessment_data);
    ...
    context.insert("compliance_status", &grade.compliance_status);
    context.insert(
        "compliance_label",
        &compliance_status_label(&grade.compliance_status),
    );
    context.insert("overall_risk", &grade.overall_risk);
    context.insert("childhood_complete", &grade.childhood_complete);
    context.insert("occupational_complete", &grade.occupational_complete);
    context.insert("covid_complete", &grade.covid_complete);
    context.insert("flu_current", &grade.flu_current);
    context.insert("fired_rules", &grade.fired_rules);
    context.insert("additional_flags", &grade.additional_flags);
    context.insert("missing_vaccinations", &grade.missing_vaccinations);
