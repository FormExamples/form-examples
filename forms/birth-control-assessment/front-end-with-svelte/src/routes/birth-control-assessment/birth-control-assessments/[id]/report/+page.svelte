<script lang="ts">
	...
	$effect(() => {
		if (!assessment.result) {
			goto(`/birth-control-assessment/birth-control-assessments/${id}`);
		}
	});
	...
	async function downloadPDF() {
		pdfError = '';
		try {
			const res = await fetch(`/birth-control-assessments/${id}/report/pdf`, {
				method: 'POST',
			...
			if (res.ok) {
				...
				a.href = url;
				a.download = `birth-control-assessment-${data.demographics.lastName || id}.pdf`;
				a.click();
...
{#if result}
	<header class="border-b border-base-300 bg-base-100 shadow-sm no-print">
		<div class="mx-auto flex max-w-4xl items-center justify-between px-4 py-4">
			<h1 class="text-lg font-bold text-base-content">Birth control assessment report</h1>
			<div class="flex items-center gap-3">
				...
				<Button data-variant="secondary" onclick={() => goto(`/birth-control-assessment/birth-control-assessments/${id}`)}>Edit</Button>
			</div>
		</div>
	</header>
	...
	<main class="mx-16 px-4 py-6">
		...
		<!-- Per-method UK MEC categories -->
		<div class="mb-6 rounded-xl border border-base-300 bg-base-100 p-6">
			<h2 class="mb-4 text-lg font-bold text-base-content">UK MEC category by method</h2>
			<div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
				{#each methods as m (m)}
					<div class="flex items-center justify-between gap-3 rounded-lg border border-base-300 p-3">
						<span class="text-sm font-medium text-base-content">{methodDisplayName(m)}</span>
						<Badge grade={result.methodMEC[m]} />
					</div>
				{/each}
			</div>
			<p class="mt-3 text-xs text-base-content/60">
				{mecCategoryLabel(1)}. {mecCategoryLabel(4)}.
			</p>
		</div>
					...
					<tbody>
						{#each result.firedRules as rule (rule.id)}
							<tr class="border-b border-base-200">
								...
								<td class="py-2"><Badge grade={rule.mecCategory} /></td>
							</tr>
						{/each}
					</tbody>
				</table>
