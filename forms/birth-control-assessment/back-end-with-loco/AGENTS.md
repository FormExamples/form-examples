# Birth Control Assessment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Birth Control Assessment form, built with axum + Loco +
...
- [`birth_control_assessment/`](./birth_control_assessment/) — the Loco crate: `src/birth_control_assessment/` holds `app.rs`
...
`src/birth_control_assessment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd birth_control_assessment && cargo check --all-targets && cargo test
```
