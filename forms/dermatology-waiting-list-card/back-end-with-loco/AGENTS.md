# Dermatology Waiting List Card — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Dermatology Waiting List Card form, built with axum + Loco +
...
- [`dermatology_waiting_list_card/`](./dermatology_waiting_list_card/) — the Loco crate: `src/dermatology_waiting_list_card/` holds `app.rs`
...
`/metrics`. The registered domain controllers are:
  ...
  - `dermatology_waiting_list_card`
  - `dermatology_waiting_list_card_appointment`
  - `dermatology_waiting_list_card_grade`
  - `dermatology_waiting_list_card_grade_flag`
  - `dermatology_waiting_list_card_grade_rule`
  - `patient`
  - `practitioner`

## Engine

`src/dermatology_waiting_list_card/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd dermatology_waiting_list_card && cargo check --all-targets && cargo test
```
