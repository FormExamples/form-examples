# Clinical references

Authoritative guidance underpinning the four-axis grading engine.

## Appropriateness (ACR Appropriateness Criteria)

Ordinal 1–9 scale in three bands: usually-appropriate (7–9),
may-be-appropriate (4–6), usually-not-appropriate (1–3). Mapped per
(indication × body region). On this scale 1 = least appropriate (risks
outweigh benefit), 9 = most appropriate (benefit outweighs risk).

Source: ACR Appropriateness Criteria, <https://acsearch.acr.org/list>; RCR
*iRefer: Making the best use of clinical radiology*.

## Radiation and contrast safety (RCR iRefer / ESUR + IR(ME)R)

### Contrast (ESUR Contrast Media Safety Committee)

| eGFR (mL/min/1.73m²) | IV iodinated contrast | Renal risk |
| --- | --- | --- |
| ≥ 45 | proceed (standard hydration) | low |
| 30–44 | caution; minimize volume, hydrate | moderate |
| < 30 / AKI | contraindicated relative; weigh benefit vs risk | high |

- **Metformin:** continue normally at eGFR ≥ 30; stop from the time of contrast
  administration and re-check renal function if eGFR < 30 or AKI.
- **Allergy / previous reaction:** severe previous reaction → contraindicated
  unless premedicated and benefit clearly outweighs risk; moderate → caution.

Source: ESUR Contrast Media Safety Committee guidelines,
<https://www.esur.org/guidelines/>; ACR Manual on Contrast Media.

### Radiation dose bands (illustrative effective dose)

| Study | Dose band |
| --- | --- |
| CT head | low |
| CT neck / cervical spine | low–moderate |
| CT chest / CTPA | moderate |
| CT abdomen–pelvis | moderate–high |
| CT colonography | moderate–high |
| Whole-body / multi-phase | high |

### IR(ME)R justification

Every medical exposure must be justified: the benefit to the individual must
outweigh the radiation detriment. Pregnancy (or possible pregnancy) requires
explicit justification and dose optimization, and may prompt redirection to a
non-ionizing modality.

Source: UK Ionizing Radiation (Medical Exposure) Regulations 2017,
<https://www.legislation.gov.uk/uksi/2017/1322/contents/made>.

## Triage tiers

| Tier | Target | Triggers |
| --- | --- | --- |
| Emergency | same hour | major trauma, suspected stroke within window, suspected massive PE, instability |
| Urgent | same day / 24–72 h | acute abdominal pain, renal colic, suspected infection/abscess, urgent staging |
| Routine | standard booking | surveillance, pre-surgical planning, stable follow-up |

## Sources

- ACR Appropriateness Criteria; ACR Manual on Contrast Media.
- RCR *iRefer: Making the best use of clinical radiology*.
- ESUR Contrast Media Safety Committee guidelines (iodinated contrast + eGFR).
- UK Ionizing Radiation (Medical Exposure) Regulations 2017 (IR(ME)R).
