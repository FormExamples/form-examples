...
// CT Scan Test Request — vetting dashboard (vanilla classic-script app).
//
...
// contrast-safety dropdown + safety-flags dropdown).
//
// Sibling modules loaded as plain `<script>` tags (in dependency order)
// attach their exports to `window.CtScanTestRequestDashboard`. The whole file
...
const columns = [
  ...
  { key: 'contrastSafetyBand',  label: 'Contrast Safety' },
  { key: 'estimatedDoseBand',   label: 'Dose' },
...
function contrastClass(band) {
  return band ? 'contrast-' + String(band) : '';
}

function doseClass(band) {
  return band ? 'dose-' + String(band) : '';
}
...
function matchesFilters(row) {
  if (filters.search) {
    ...
    const matches =
      ...
      String(row.bodyRegion || '').toLowerCase().includes(term) ||
  ...
  if (filters.contrast && row.contrastSafetyBand !== filters.contrast) return false;
...
function compareRows(a, b) {
  ...
  if (key === 'contrastSafetyBand') {
    return ((contrastRank[av] ?? -1) - (contrastRank[bv] ?? -1)) * dir;
  }
  if (key === 'estimatedDoseBand') {
    return ((doseRank[av] ?? -1) - (doseRank[bv] ?? -1)) * dir;
  }
  if (key === 'triageTier') {
  ...
  // Default: string compare (nhs, patient, bodyRegion, indication, clinician,
  // referralDate).
...
function renderTableBody() {
  ...
  for (const row of rows) {
    ...
    if (row.triageTier === 'emergency' || row.contrastSafetyBand === 'contraindicated') {
    ...
    tr.innerHTML = `
      ...
      <td class="data-table-td">${esc(titleCase(row.bodyRegion))}</td>
      ...
      <td class="data-table-td"><span class="contrast-badge ${contrastClass(row.contrastSafetyBand)}">${esc(titleCase(row.contrastSafetyBand))}</span></td>
      <td class="data-table-td"><span class="dose-badge ${doseClass(row.estimatedDoseBand)}">${esc(titleCase(row.estimatedDoseBand))}</span></td>
