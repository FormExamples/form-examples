<script lang="ts">
	...
	import Step5ContrastAndRenalSafety from '#lib/components/steps/Step5ContrastAndRenalSafety.svelte';
	import Step6RadiationSafety from '#lib/components/steps/Step6RadiationSafety.svelte';
	import Step7TriageAndSetting from '#lib/components/steps/Step7TriageAndSetting.svelte';
	...
	function validate(): boolean {
		...
		if (d.request.bodyRegion === '') {
			found.push({ id: 'bodyRegion', message: 'Requested body region is required.' });
		}
	...
	function submit() {
		...
		goto(`/ct-scan-test-request/ct-scan-test-requests/${id}/report`);
	}
...
<main class="mx-16 px-4 py-6">
	<h1 class="text-2xl font-bold text-base-content">
		{isNew ? 'New CT scan request' : `CT scan request ${id}`}
	</h1>
	...
	<Form label="CT scan test request" onsubmit={submit}>
		<Step1RequestingClinician />
		<Step2PatientIdentification />
		<Step3RequestedExamination />
		<Step4ClinicalContext />
		<Step5ContrastAndRenalSafety />
		<Step6RadiationSafety />
		<Step7TriageAndSetting />
