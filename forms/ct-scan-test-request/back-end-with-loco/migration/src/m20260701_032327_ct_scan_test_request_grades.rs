...
impl MigrationTrait for Migration {
    async fn up(&self, m: &SchemaManager) -> Result<(), DbErr> {
        create_table(m, "ct_scan_test_request_grades",
            &[
            ...
            ("contrast_safety_band", ColType::StringWithDefault(String::new())),
            ("estimated_dose_band", ColType::StringWithDefault(String::new())),
            ("renal_risk", ColType::BooleanWithDefault(false)),
            ...
            &[
            ("ct_scan_test_request", ""),
            ]
        ).await
    }

    async fn down(&self, m: &SchemaManager) -> Result<(), DbErr> {
        drop_table(m, "ct_scan_test_request_grades").await
    }
}
