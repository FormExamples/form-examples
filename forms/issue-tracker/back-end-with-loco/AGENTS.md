...
- [`issue_tracker/`](./issue_tracker/) — the Loco crate: `src/issue_tracker/` holds `app.rs`
...
`/metrics`. The registered domain controllers are:
  ...
  - `issue_tracker_grade_rule`
  - `participant`
  - `reporter`

## Engine

`src/issue_tracker/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd issue_tracker && cargo check --all-targets && cargo test
```
