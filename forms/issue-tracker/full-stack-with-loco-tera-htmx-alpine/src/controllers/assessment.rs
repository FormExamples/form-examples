...
/// POST /assessment/new -- create a new draft issue, redirect to the wizard.
#[debug_handler]
async fn create_new(State(ctx): State<AppContext>) -> Result<Response> {
    let item = ActiveModel::new_draft()
        .map_err(|e| Error::BadRequest(format!("Failed to create issue: {e}")))?;
    let item = item.insert(&ctx.db).await?;
    let id = item.id;
    Ok(Redirect::to(&format!("/assessment/{id}")).into_response())
}

/// GET /assessment/{id} -- render the single-page issue-tracker wizard.
#[debug_handler]
...
/// Convert form section name (snake_case) to JSON key. Every section key
/// in the issue-tracker is a single-token lowercase identifier
/// (`reporter`, `cc`, `pt`, `sx`, `fx`, `hx`, `ix`, `dx`, `txpx`,
/// `scores`) so no remapping is required.
...
async fn show_report(
    ...
    let assessment_data: AssessmentData = item
        .assessment_data()
        .map_err(|e| Error::BadRequest(format!("Invalid issue data: {e}")))?;
    ...
    context.insert("composite_priority", &grade.composite_priority);
    context.insert(
        "composite_label",
        &crate::engine::utils::composite_label(&grade.composite_priority),
    );
    ...
    context.insert("score_by_priority_rank", &grade.score_by_priority_rank);
    context.insert(
        "score_by_severity_of_impact",
        &grade.score_by_severity_of_impact,
    );
    context.insert(
        "score_by_magnitude_of_damage",
        &grade.score_by_magnitude_of_damage,
    );
    context.insert("score_by_harm_grade", &grade.score_by_harm_grade);
    context.insert(
        "score_by_failure_condition",
        &grade.score_by_failure_condition,
    );
    context.insert(
        "score_by_moscow_requirement",
        &grade.score_by_moscow_requirement,
    );
    context.insert(
        "score_by_frequency_percent",
        &grade.score_by_frequency_percent,
    );
