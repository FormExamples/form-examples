# Venereology Waiting List Card — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Venereology Waiting List Card form, built with axum + Loco +
...
- [`venereology_waiting_list_card/`](./venereology_waiting_list_card/) — the Loco crate: `src/venereology_waiting_list_card/` holds `app.rs`
...
`/metrics`. The registered domain controllers are:
  ...
  - `venereology_waiting_list_card`
  - `venereology_waiting_list_card_appointment`
  - `venereology_waiting_list_card_grade`
  - `venereology_waiting_list_card_grade_flag`
  - `venereology_waiting_list_card_grade_rule`
  - `patient`
  - `practitioner`

## Engine

`src/venereology_waiting_list_card/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd venereology_waiting_list_card && cargo check --all-targets && cargo test
```
