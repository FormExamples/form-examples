...
<Fieldset title="Current Medications" description="List all current medications and allergies">
	<h3 class="mb-2 text-sm font-semibold text-base-content/80">Current Medications</h3>
	<MedicationEntry bind:medications={med.medications} />
	{#if med.medications.length === 0}
		<p class="mt-2 mb-4 text-sm text-base-content/60">No medications added.</p>
	{/if}
	...
	<h3 class="mb-2 mt-6 text-sm font-semibold text-base-content/80">Known Allergies</h3>
	<AllergyEntry bind:allergies={med.allergies} />
	{#if med.allergies.length === 0}
		<p class="mt-2 mb-4 text-sm text-base-content/60">No allergies added.</p>
	{/if}
</Fieldset>
