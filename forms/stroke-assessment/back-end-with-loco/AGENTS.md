# Stroke Assessment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Stroke Assessment form, built with axum + Loco +
...
- [`stroke_assessment/`](./stroke_assessment/) — the Loco crate: `src/stroke_assessment/` holds `app.rs`
...
`src/stroke_assessment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd stroke_assessment && cargo check --all-targets && cargo test
```
