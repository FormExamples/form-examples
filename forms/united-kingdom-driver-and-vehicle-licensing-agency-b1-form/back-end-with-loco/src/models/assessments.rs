...
impl Model {
    ...
    /// Deserialize the JSONB `result` column into serde_json::Value.
    pub fn report_result(&self) -> Result<Option<serde_json::Value>, serde_json::Error> {
        match &self.result {
...
/// List all completed assessments (those with a result).
