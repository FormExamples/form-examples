<script lang="ts">
	...
	import Step3Shock from '#lib/components/steps/Step3Shock.svelte';
	import Step4Comorbidity from '#lib/components/steps/Step4Comorbidity.svelte';
	import Step5Endoscopy from '#lib/components/steps/Step5Endoscopy.svelte';
	...
	function submit() {
		...
		assessment.result = calculateRockallGrade(assessment.data);
		goto(`/rockall-score-for-upper-gastrointestinal-bleeding/${plural}/${id}/report`);
	}
...
<main class="mx-16 px-4 py-6">
	<h1 class="text-2xl font-bold text-base-content">
		{isNew ? 'New Rockall assessment' : `Rockall assessment ${id}`}
	</h1>
	<p class="mt-1 text-sm text-base-content/70">
		Complete the six sections; the clinical and full Rockall scores are computed on submit.
	</p>
	...
	<Form label="Rockall assessment" onsubmit={submit}>
		<Step1Context />
		<Step2Identification />
		<Step3Shock />
		<Step4Comorbidity />
