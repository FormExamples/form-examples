<script lang="ts">
	...
	// SVAR DataGrid columns. Scores and band render through the shared engine
	...
	const columns = [
		...
		{ id: 'clinicalRockallScore', header: 'Clinical', width: 90, sort: true },
		{
			id: 'fullRockallScore',
			header: 'Full',
			width: 90,
	...
	function init(api: any) {
		...
		api.on('select-row', (ev: { id?: string | number }) => {
			if (ev?.id != null) goto(`/rockall-score-for-upper-gastrointestinal-bleeding/${plural}/${ev.id}`);
		});
	}
</script>
...
<main class="mx-16 px-4 py-6">
	<div class="mb-6 flex flex-wrap items-end justify-between gap-4">
		<div>
			<h1 class="text-2xl font-bold text-base-content">Rockall dashboard</h1>
			<p class="mt-1 text-sm text-base-content/70">
				Clinical and full Rockall scores and risk band for assessed patients, computed by the shared
				engine. Select a row to open the assessment.
			</p>
		</div>
		<a href="/rockall-score-for-upper-gastrointestinal-bleeding/{plural}/new" class="button" data-variant="primary">New assessment</a>
	</div>
	...
	<div class="mb-4 flex flex-wrap gap-4">
		<label class="text-sm">
			...
			<select class="select inline-block w-auto" bind:value={settingFilter}>
				...
				<option value="ward">Acute / gastroenterology ward</option>
		...
		<label class="text-sm">
			...
			<select class="select inline-block w-auto" bind:value={bandFilter}>
				...
				<option value="clinical-only">Clinical score only</option>
			</select>
		</label>
	</div>
