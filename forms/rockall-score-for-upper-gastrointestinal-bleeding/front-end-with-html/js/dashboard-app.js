...
// Rockall Score — clinician dashboard (vanilla classic-script app).
//
...
// box + care-setting dropdown + risk-band dropdown + escalation dropdown).
//
// Sibling modules loaded as plain `<script>` tags (in dependency order) attach
// their exports to `window.RockallScoreForUpperGastrointestinalBleedingDashboard`.
...
const filters = {
  search: '',
  setting: '',   // '' | 'emergency-department' | 'ward' | 'endoscopy-unit' | 'other'
  band: '',      // '' | 'low' | 'intermediate' | 'high' | 'clinical-only'
  escalate: ''   // '' | 'yes' | 'no'
};
...
const columns = [
  ...
  { key: 'clinicalRockallScore', label: 'Clinical' },
  { key: 'fullRockallScore',     label: 'Full' },
  { key: 'riskBand',             label: 'Risk Band' },
  { key: 'escalationFlag',       label: 'Escalate' }
];

// Rank used when sorting the riskBand column so bands order low < intermediate
// < high, with clinical-only (pre-endoscopy) grouped after them.
const bandRank = {
  'low': 0,
  'intermediate': 1,
  'high': 2,
...
function scoreLabel(score, max) {
  return (score === null || score === undefined) ? 'N/A' : `${score} / ${max}`;
}
 ...
 * rank table; the nullable full score sorts nulls last; the escalation boolean
...
function compareRows(a, b) {
  ...
  if (key === 'clinicalRockallScore') {
    return ((av ?? 0) - (bv ?? 0)) * dir;
  }

  if (key === 'fullRockallScore') {
...
function renderTableBody() {
  ...
  for (const row of rows) {
    ...
    const fullClassName = row.fullRockallScore === null
    ...
    tr.innerHTML = `
      ...
      <td><span class="class-cell">${esc(scoreLabel(row.clinicalRockallScore, 7))}</span></td>
      <td><span class="${fullClassName}">${esc(scoreLabel(row.fullRockallScore, 11))}</span></td>
