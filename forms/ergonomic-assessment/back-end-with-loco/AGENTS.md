# Ergonomic Assessment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Ergonomic Assessment form, built with axum + Loco +
...
- [`ergonomic_assessment/`](./ergonomic_assessment/) — the Loco crate: `src/ergonomic_assessment/` holds `app.rs`
...
`src/ergonomic_assessment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd ergonomic_assessment && cargo check --all-targets && cargo test
```
