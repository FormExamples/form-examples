// Flagged-issue detection. Independent of REBA score (which the grader
// computes), this module raises clinician-facing flags for safety-critical
// or actionable conditions: extreme posture, existing musculoskeletal
// injury, heavy manual handling, lack of breaks, vibration exposure,
// poor workstation setup, severe symptoms, multiple pain sites, and
// psychosocial risk factors.
//
// Mirrors the SvelteKit `flagged-issues.ts` reference engine.
...
function detectAdditionalFlags(data) {
  ...
  if (
    data.workstationSetup.monitorHeight !== '' &&
    data.workstationSetup.monitorHeight !== 'at-eye-level'
  ) {
    wsIssues.push('monitor height');
  }
  if (
    data.workstationSetup.keyboardPlacement !== '' &&
    data.workstationSetup.keyboardPlacement !== 'correct'
  ) {
