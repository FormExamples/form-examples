<script lang="ts">
	...
	$effect(() => {
		if (!assessment.result) {
			goto(`/pulmonary-embolism-rule-out-criteria/pulmonary-embolism-rule-out-criterias/${id}`);
		}
	});
		...
		<div class="mx-auto flex max-w-4xl items-center justify-between px-4 py-4">
			...
			<div class="flex items-center gap-3">
				...
				<Button
					...
					onclick={() => goto(`/pulmonary-embolism-rule-out-criteria/pulmonary-embolism-rule-out-criterias/${id}`)}>Edit</Button
				>
			</div>
		</div>
	</header>
