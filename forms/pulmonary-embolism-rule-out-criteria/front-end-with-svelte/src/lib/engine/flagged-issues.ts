 ...
 * Missing inputs are treated as failed, so the result defaults toward
 * PERC-positive.
 */
export function hasMissingInputs(data: AssessmentData): boolean {
	return (
 ...
 * the perc-negative / perc-positive classification (spec §5):
 *
 ...
 *                                   probability missing
 *
 * Rows mirror the `pulmonary_embolism_rule_out_criteria_grade_flag` SQL table
