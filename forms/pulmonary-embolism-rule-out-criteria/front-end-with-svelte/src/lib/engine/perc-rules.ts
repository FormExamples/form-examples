import type { PercRule } from './types';

/**
 * Declarative PERC criterion rules.
 *
 * The Pulmonary Embolism Rule-out Criteria has exactly eight objective
 * criteria. Each rule evaluates the patient data and returns true when its
 * criterion is SATISFIED (the reassuring, low-risk state); the grader
 * (`perc-grader.ts`) combines them with a boolean conjunction and the
 * pre-test-probability gate into a binary classification. Unlike a
 * numeric-score form there are no points: a criterion is simply satisfied or
 * failed, and a single failure is decisive. Rows mirror the
 * `pulmonary_embolism_rule_out_criteria_grade_rule` SQL table.
 *
 ...
 * documented) — an unset ('') or 'yes' answer is failed.
 */
export const percRules: PercRule[] = [
	...
	// ─── CRITERION 6: NO RECENT SURGERY OR TRAUMA ─────────────────────
	{
		...
		category: 'history',
		description:
			'No surgery or trauma requiring general anaesthesia within the past 4 weeks',
