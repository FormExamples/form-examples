<script lang="ts">
	...
	import Step3Functional from '$lib/components/steps/Step3Functional.svelte';
	import Step4Fluid from '$lib/components/steps/Step4Fluid.svelte';
	...
	import Step7Devices from '$lib/components/steps/Step7Devices.svelte';
	...
	function validate(): boolean {
		...
		if (d.diagnosis.heartFailureType === '') {
			found.push({ id: 'diagnosis-heartFailureType', message: 'Heart-failure type is required.' });
		}
		if (d.functional.nyhaClass === null) {
			found.push({ id: 'functional-nyhaClass', message: 'NYHA functional class is required.' });
		}
		errors = found;
	...
	function submit() {
		...
		assessment.result = gradeReview(assessment.data);
		goto(`/heart-failure-review/heart-failure-reviews/${id}/report`);
	}
...
<main class="mx-16 px-4 py-6">
	<h1 class="text-2xl font-bold text-base-content">
		{isNew ? 'New heart-failure annual review' : `Heart-failure review ${id}`}
	</h1>
	<p class="mt-1 text-sm text-base-content/70">
		Complete the nine sections; the NYHA functional status, four-pillar medication-optimisation
		status, and review completeness are computed on submit. This is a documentation and
		status-classification instrument — there is no numeric score.
	</p>
