...
- Ported the pure classification engine to TypeScript: `types.ts`,
  ...
  four-pillar medication optimisation, documented-domains ÷ 6 completeness),
  `flagged-issues.ts` (urgent review, optimisation gap, hyperkalaemia,
...
- Engine unit tests (`heart-failure-review-grader.test.ts`) with a local
  ...
  the HFrEF four-pillar optimisation transitions (optimised / partial /
...
- Nine wizard step components (context, patient & diagnosis, functional status,
  fluid status, investigations, medication optimisation with all four pillars,
...
- `sample-reports.ts` — four sample records spanning NYHA status, optimisation
