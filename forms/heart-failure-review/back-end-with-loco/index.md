# Heart Failure Annual Review: Back-end with Rust Axum Loco (JSON API)

Server-side Rust JSON API for the Heart Failure Annual Review form. Pure back-end: no HTML
rendering, no Tera templates, no HTMX, no Alpine.js, no CSS, no Lily
Design System.

The Loco crate lives in [`heart_failure_review/`](heart_failure_review/) — one migration, SeaORM
