...
| 05 | `05_create_table_heart_failure_review_grade.sql` | `heart_failure_review_grade` | Computed grade (NYHA functional status, medication-optimisation status, review status, completeness score, pillar breakdown), 1:1 with review |
...
(`stable` / `symptomatic` / `advanced` / `unknown`), the medication-optimisation
