...
- [ ] SQL migrations in `sql/` (referral table, UUIDv4 PK, timestamps).
- [ ] Generate XML, FHIR R5, protobuf, OpenAPI, Loco setup.
- [ ] Documentation engine (`types.ts`, `utils.ts`, `validation-rules.ts`,
      `referral-validator.ts`, `flagged-issues.ts`) + Vitest tests.
