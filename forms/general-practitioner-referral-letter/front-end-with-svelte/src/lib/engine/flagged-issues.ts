...
import { MANDATORY_ALWAYS, nonEmpty } from './gp-referral-rules';

/**
 * Names the always-mandatory fields that are blank, mirroring MANDATORY_ALWAYS
 * in `gp-referral-rules.ts`. Used by the mandatory-information-missing flag.
 */
function missingMandatoryFields(r: AssessmentData): string[] {
	const missing: string[] = [];
	for (const field of MANDATORY_ALWAYS) {
...
/**
 * Detect referrer-facing flags. Independent of the completeness status (which
 * the grader produces), this module raises flags per spec §5:
 *
 ...
 *
 * Rows mirror the `general_practitioner_referral_letter_grade_flag` SQL table
...
export function detectFlaggedIssues(referral: AssessmentData): FlaggedIssue[] {
	...
	if (missing.length > 0) {
		flags.push({
			...
			priority: 'high',
			description: `One or more mandatory fields are missing: ${missing.join(
				', '
			)}. The receiving service may reject or bounce the referral.`,
			suggestedAction:
