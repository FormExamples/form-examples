import type { AssessmentData, MandatoryField } from './types';
 ...
 *
 * The grader (`gp-referral-grader.ts`) marks the referral `Complete` only when
 * every applicable mandatory field is present, otherwise `Incomplete`, and
 * computes a completeness percentage from the same set. Each mandatory slot
 * below carries a stable id, category, and human-readable description that
 * mirror the `general_practitioner_referral_letter_grade_rule` SQL table.
 */

export const nonEmpty = (s: string): boolean => typeof s === 'string' && s.trim() !== '';

/**
 * The always-mandatory field set — required for every urgency. Order matches the
 * spec §4 `MANDATORY_ALWAYS` list.
 */
export const MANDATORY_ALWAYS: MandatoryField[] = [
	{
	...
	},
	{
		...
		present: (r) => nonEmpty(r.patient.patientDateOfBirth)
	},
	{
 ...
 */
export const MANDATORY_URGENCY_REASON: MandatoryField = {
...
export const MANDATORY_SUSPECTED_CANCER_CRITERION: MandatoryField = {
	id: 'R-MANDATORY-SUSPECTED-CANCER-CRITERION',
	rule: 'suspectedCancerCriterion',
	category: 'conditional-requirement',
	label: 'suspected-cancer criterion',
	description:
		'A named NICE NG12 suspected-cancer criterion is recorded (required for two-week-wait)',
	present: (r) => nonEmpty(r.urgencyInfo.suspectedCancerCriterion)
};

export const MANDATORY_SUSPECTED_CANCER_PATHWAY: MandatoryField = {
 ...
 */
export function mandatoryFor(r: AssessmentData): MandatoryField[] {
