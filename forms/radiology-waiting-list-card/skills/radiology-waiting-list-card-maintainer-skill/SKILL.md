---
name: radiology-waiting-list-card-maintainer-skill
description: "Implementation workflow for maintaining and extending the Radiology Waiting List Card form (forms/radiology-waiting-list-card/) — editing its spec, schema, or engine, regenerating derived artefacts, and running its verify gates. Use when implementing a change to this form's spec, SQL schema, front-end, back-end, or personas. For the repo-wide workflow, use form-examples-maintainer-skill instead."
---

# Radiology Waiting List Card — Maintainer Skill

Implementation-facing companion to `radiology-waiting-list-card-skill` (this form's end-user concepts skill) and to `form-examples-maintainer-skill` (the repo-wide maintainer skill — read that one first for the golden rule, generators, and the full verify-gate catalogue). This skill is the one-form-scoped map.

## Directory layout

`forms/radiology-waiting-list-card/` contains:
...
```sh
bin/test-form radiology-waiting-list-card
bin/test-sql-apply radiology-waiting-list-card
bin/test-personas radiology-waiting-list-card
bin/test-e2e --html radiology-waiting-list-card
```
