# Psychiatry Assessment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Psychiatry Assessment form, built with axum + Loco +
...
- [`psychiatry_assessment/`](./psychiatry_assessment/) — the Loco crate: `src/psychiatry_assessment/` holds `app.rs`
...
`src/psychiatry_assessment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd psychiatry_assessment && cargo check --all-targets && cargo test
```
