<script lang="ts">
	...
	const columns = [
		...
		{ id: 'legalStatus', header: 'Legal status', width: 120, sort: true },
		{
			id: 'suicideFlag',
			header: 'Suicidal',
			width: 100,
	...
	function init(api: any) {
		...
		api.on('select-row', (ev: { id?: string | number }) => {
			if (ev?.id != null) goto(`/psychiatry-assessments/${ev.id}`);
		});
	}
</script>
...
<main class="mx-auto max-w-6xl px-4 py-6">
	<div class="mb-6 flex flex-wrap items-end justify-between gap-4">
		...
		<a href="/psychiatry-assessments/new" class="button" data-variant="primary">New assessment</a>
	</div>
