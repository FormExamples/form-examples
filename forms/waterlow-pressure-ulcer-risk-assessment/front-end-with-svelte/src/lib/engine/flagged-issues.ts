...
const CORE_INPUTS: [keyof AssessmentData, string][] = [
 ...
 * Waterlow total (which the grader produces), per spec §5:
 *
 *   - Very high risk (high)                  — waterlowScore >= 20
 *   - High risk (high)                       — 15 <= waterlowScore < 20
 *   - At risk (medium)                       — 10 <= waterlowScore < 15
 *   - Existing pressure damage (high)        — existingPressureDamage == 'yes'
 *                                              or skinType is discoloured / broken
 *   - Multiple special risk factors (medium) — two or more of the four
 *                                              special-risk groups contribute points
 *   - Incomplete assessment (low)            — any core category input missing
 *
 * Rows mirror the `waterlow_pressure_ulcer_risk_assessment_grade_flag` SQL
...
export function detectFlaggedIssues(data: AssessmentData, grade: GradingResult): FlaggedIssue[] {
	...
	if (missing > 0) {
		flags.push({
			...
			suggestedAction: 'Complete the outstanding core categories and re-score.'
		});
	}
