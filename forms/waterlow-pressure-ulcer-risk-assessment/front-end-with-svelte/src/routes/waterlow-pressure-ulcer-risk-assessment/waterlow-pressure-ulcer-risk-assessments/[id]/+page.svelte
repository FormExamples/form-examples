<script lang="ts">
	...
	import Step3CoreCategories from '$lib/components/steps/Step3CoreCategories.svelte';
	import Step4SpecialRisk from '$lib/components/steps/Step4SpecialRisk.svelte';
	...
	function validate(): boolean {
		...
		if (d.context.nurseName.trim() === '') {
			found.push({ id: 'context-nurseName', message: 'Assessing nurse name is required.' });
		}
	...
	function submit() {
		...
		assessment.result = calculateWaterlowGrade(assessment.data);
		goto(`/waterlow-pressure-ulcer-risk-assessment/${plural}/${id}/report`);
	}
...
<main class="mx-16 px-4 py-6">
	<h1 class="text-2xl font-bold text-base-content">
		{isNew ? 'New Waterlow assessment' : `Waterlow assessment ${id}`}
	</h1>
	<p class="mt-1 text-sm text-base-content/70">
		Complete the five sections; the Waterlow score and risk band are computed on submit.
	</p>
	...
	<Form label="Waterlow assessment" onsubmit={submit}>
		<Step1Context />
		<Step2Identification />
		<Step3CoreCategories />
		<Step4SpecialRisk />
