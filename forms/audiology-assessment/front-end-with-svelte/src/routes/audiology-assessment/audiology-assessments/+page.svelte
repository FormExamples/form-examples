<script lang="ts">
	...
	import { hearingGradeLabel } from '#lib/engine/utils.js';
	...
	function init(api: any) {
		...
		api.on('select-row', (ev: { id?: string | number }) => {
			if (ev?.id != null) goto(`/audiology-assessment/audiology-assessments/${ev.id}`);
		});
	}
</script>
...
<main class="mx-16 px-4 py-6">
	<div class="mb-6 flex flex-wrap items-end justify-between gap-4">
		...
		<a href="/audiology-assessment/audiology-assessments/new" class="button" data-variant="primary">New assessment</a>
	</div>
