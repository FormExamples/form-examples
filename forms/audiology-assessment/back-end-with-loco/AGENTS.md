# Audiology Assessment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Audiology Assessment form, built with axum + Loco +
...
- [`audiology_assessment/`](./audiology_assessment/) — the Loco crate: `src/audiology_assessment/` holds `app.rs`
...
`src/audiology_assessment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd audiology_assessment && cargo check --all-targets && cargo test
```
