...
function detectAdditionalFlags(data) {
  ...
  if (
    ...
    if (diff > 15) {
      flags.push({
        ...
        message:
          'Asymmetric hearing loss (' + diff + ' dB difference) - suspect acoustic neuroma, consider MRI',
        priority: 'high'
      });
    }
  }
  ...
  // Mirrors the SvelteKit logic exactly: any vertigo report combined with
  // either ear above 25 dB triggers this flag.
  if (
  ...
  if (data.medicalHistory.ototoxicMedications === 'yes') {
    flags.push({
      ...
      message:
        ...
        (data.medicalHistory.ototoxicMedicationDetails || 'details not specified') +
        ' - monitor audiometry',
      priority: 'medium'
    });
  }
  ...
  if (data.medicalHistory.menieres === 'yes') {
    flags.push({
      ...
      category: 'Medical',
      message:
        "Meniere's disease - fluctuating hearing loss expected, salt restriction advised",
      priority: 'medium'
    });
  }
  ...
  // ─── Tympanic membrane abnormality (right) ──────────────
  if (
  ...
  // ─── Tympanic membrane abnormality (left) ───────────────
  if (
  ...
  if (data.medicalHistory.autoimmune === 'yes') {
    flags.push({
      ...
      message:
        'Autoimmune condition: ' +
        (data.medicalHistory.autoimmuneDetails || 'details not specified') +
  ...
  if (
    ...
    flags.push({
      ...
      category: 'Tinnitus',
      message:
        'Severe tinnitus handicap (THI score: ' +
        data.tinnitusAssessment.tinnitusHandicapInventoryScore +
  ...
  if (
    ...
    flags.push({
      ...
      category: 'Otoscopic',
      message:
        'Ear wax present - may need removal before accurate audiometry or hearing aid fitting',
      priority: 'low'
    });
  }
  ...
  if (data.otoscopicFindings.previousSurgery === 'yes') {
    flags.push({
      ...
      message:
        'Previous ear surgery: ' +
        (data.otoscopicFindings.previousSurgeryDetails || 'details not specified'),
      priority: 'low'
    });
  }
