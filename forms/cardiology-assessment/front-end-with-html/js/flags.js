// Flagged-issue detection. Independent of CCS/NYHA classification (which the
// grader produces), this module raises clinician-facing safety flags for
// recent MI, unstable angina, severe heart failure, severely reduced LVEF,
// abnormal stress test, syncope, pacemaker/ICD, anticoagulant/antiplatelet
// use, contrast allergy, anaphylaxis history, atrial fibrillation,
// uncontrolled hypertension, poor functional capacity, and cardiomyopathy.
...
function detectAdditionalFlags(data) {
  ...
  for (let i = 0; i < data.allergies.allergies.length; i++) {
    const allergy = data.allergies.allergies[i];
  ...
  // Sort: urgent > high > medium > low (urgent reserved for future use;
  // matches the asthma reference ordering and remains forward-compatible).
