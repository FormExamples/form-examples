<script lang="ts">
	...
	$effect(() => {
		if (!assessment.result) {
			goto(`/cardiology-assessment/cardiology-assessments/${id}`);
		}
	});
	...
	async function downloadPDF() {
		pdfError = '';
		try {
			const res = await fetch(`/cardiology-assessments/${id}/report/pdf`, {
				method: 'POST',
			...
			if (res.ok) {
				...
				a.href = url;
				a.download = `cardiology-assessment-${data.demographics.lastName || id}.pdf`;
				a.click();
...
{#if result}
	<header class="border-b border-base-300 bg-base-100 shadow-sm no-print">
		<div class="mx-auto flex max-w-4xl items-center justify-between px-4 py-4">
			<h1 class="text-lg font-bold text-base-content">Cardiology assessment report</h1>
			<div class="flex items-center gap-3">
				...
				<Button data-variant="secondary" onclick={() => goto(`/cardiology-assessment/cardiology-assessments/${id}`)}>Edit</Button>
			</div>
		</div>
	</header>
	...
	<main class="mx-16 px-4 py-6">
		...
		<div class="mb-6 rounded-xl border-2 p-6 text-center {riskLevelColor(result.overallRisk)}">
			...
			<div class="mt-2 flex justify-center gap-6 text-sm">
				{#if result.ccsClass}<span>{ccsClassLabel(result.ccsClass)}</span>{/if}
				{#if result.nyhaClass}<span>{nyhaClassLabel(result.nyhaClass)}</span>{/if}
			</div>
		...
		{#if result.additionalFlags.length > 0}
			<div class="mb-6 rounded-xl border border-error/40 bg-base-100 p-6">
				<h2 class="mb-4 text-lg font-bold text-error">Flagged issues for cardiologist</h2>
				<div class="space-y-2">
		...
		{#if data.allergies.allergies.length > 0}
			<div class="mb-6 rounded-xl border border-base-300 bg-base-100 p-6">
				...
				<ul class="list-disc space-y-1 pl-5 text-sm text-base-content/80">
					{#each data.allergies.allergies as allergy (allergy.allergen)}
						<li>
