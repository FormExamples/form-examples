# Geriatrics Waiting List Card — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Geriatrics Waiting List Card form, built with axum + Loco +
...
- [`geriatrics_waiting_list_card/`](./geriatrics_waiting_list_card/) — the Loco crate: `src/geriatrics_waiting_list_card/` holds `app.rs`
...
`/metrics`. The registered domain controllers are:
  ...
  - `geriatrics_waiting_list_card`
  - `geriatrics_waiting_list_card_appointment`
  - `geriatrics_waiting_list_card_grade`
  - `geriatrics_waiting_list_card_grade_flag`
  - `geriatrics_waiting_list_card_grade_rule`
  - `patient`
  - `practitioner`

## Engine

`src/geriatrics_waiting_list_card/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd geriatrics_waiting_list_card && cargo check --all-targets && cargo test
```
