# Audio-Vestibular Assessment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Audio-Vestibular Assessment form, built with axum + Loco +
...
- [`audio_vestibular_assessment/`](./audio_vestibular_assessment/) — the Loco crate: `src/audio_vestibular_assessment/` holds `app.rs`
...
`src/audio_vestibular_assessment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd audio_vestibular_assessment && cargo check --all-targets && cargo test
```
