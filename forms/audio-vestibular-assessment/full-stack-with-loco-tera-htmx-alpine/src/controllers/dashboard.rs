...
struct DashboardParams {
    ...
    hearing_loss_grade: Option<String>,
    dhi_handicap_level: Option<String>,
    has_flags: Option<String>,
}

/// GET /dashboard -- render the audiology / ENT dashboard with filters.
#[debug_handler]
async fn dashboard(
    ...
    // Server-side search across patient name and provisional diagnosis.
    if let Some(ref term) = params.search {
        let term = term.to_lowercase();
        items.retain(|row| {
            row.patient_name.to_lowercase().contains(&term)
                || row.provisional_diagnosis.to_lowercase().contains(&term)
        });
    }

    if let Some(ref grade) = params.hearing_loss_grade {
        if !grade.is_empty() {
            items.retain(|row| row.hearing_loss_grade == *grade);
        }
    }

    if let Some(ref level) = params.dhi_handicap_level {
        if !level.is_empty() {
            items.retain(|row| row.dhi_handicap_level == *level);
        }
    }
    ...
    context.insert(
        "hearing_loss_grade",
        &params.hearing_loss_grade.unwrap_or_default(),
    );
    context.insert(
        "dhi_handicap_level",
        &params.dhi_handicap_level.unwrap_or_default(),
    );
