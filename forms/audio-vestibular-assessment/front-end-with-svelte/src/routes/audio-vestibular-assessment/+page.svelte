<script lang="ts">
	// The app root is a welcome page: it explains the work and offers prominent
	// links to the two working surfaces — the assessment form and the clinician
	// dashboard. The collection routes (`/audio-vestibular-assessments` and
	// `/audio-vestibular-assessments/new`) hold the actual functionality.
	const plural = 'audio-vestibular-assessments';
</script>

<main class="mx-auto max-w-4xl px-4 py-12">
	<header class="mb-10">
		<p class="text-sm font-semibold uppercase tracking-wide text-primary">Audio-Vestibular Assessment</p>
		<h1 class="mt-2 text-3xl font-bold text-base-content">Combined audiology and balance evaluation</h1>
		<p class="mt-4 text-base leading-relaxed text-base-content/70">
			A combined audiology and vestibular (balance) assessment completed as a single continuous
			wizard, whose shared engine classifies hearing loss with the WHO pure-tone audiometry grades,
			quantifies the Dizziness Handicap Inventory (DHI) handicap level, and flags issues for the
			audiologist or ENT clinician.
		</p>
	</header>

	<div class="grid gap-4 sm:grid-cols-2">
		<a
			href="/audio-vestibular-assessment/{plural}/new"
			class="group rounded-lg border border-primary bg-primary p-6 text-primary-content shadow-sm transition hover:opacity-90"
		>
			<h2 class="text-lg font-semibold">Start a new assessment</h2>
			<p class="mt-2 text-sm text-primary-content/80">
				Open the step-by-step assessment form. One continuous single-page wizard that grades the
				patient as you complete it.
			</p>
			<span class="mt-4 inline-block text-sm font-semibold">Open the form →</span>
		</a>
		<a
			href="/audio-vestibular-assessment/{plural}"
			class="group rounded-lg border border-base-300 bg-base-100 p-6 text-base-content shadow-sm transition hover:border-primary hover:shadow"
		>
			<h2 class="text-lg font-semibold">Clinician dashboard</h2>
			<p class="mt-2 text-sm text-base-content/70">
				Browse assessed patients with their better-ear PTA, WHO hearing-loss grade, DHI total and
				handicap level, and filter by hearing grade and DHI handicap.
			</p>
			<span class="mt-4 inline-block text-sm font-semibold text-primary">Open the dashboard →</span>
		</a>
	</div>

	<section class="mt-12">
		<h2 class="text-lg font-semibold text-base-content">About this work</h2>
		<dl class="mt-4 space-y-4 text-sm">
			<div>
				<dt class="font-semibold text-base-content">Purpose</dt>
				<dd class="mt-1 text-base-content/70">
					Capture a structured audio-vestibular profile — demographics, presenting symptoms,
					otoscopic examination, pure-tone and speech audiometry, tympanometry and acoustic
					reflexes, bedside vestibular screening, the 25-item Dizziness Handicap Inventory (DHI),
					and the clinical impression and referral plan — and produce a graded report (WHO
					hearing-loss grade, DHI handicap level, and flagged issues) to support audiology and ENT
					review.
				</dd>
			</div>
			<div>
				<dt class="font-semibold text-base-content">Specification</dt>
				<dd class="mt-1 text-base-content/70">
					Spec-driven: the living domain spec defines the data model, the WHO grading and DHI scoring
					engine, and the flag rules. The same engine grades both the form and the dashboard.
				</dd>
			</div>
			<div>
				<dt class="font-semibold text-base-content">Documentation</dt>
				<dd class="mt-1 text-base-content/70">
					Uses WHO pure-tone audiometry hearing-loss grades (better-ear four-frequency average) and
					the Jacobson &amp; Newman Dizziness Handicap Inventory (DHI).
				</dd>
			</div>
		</dl>
	</section>
</main>
