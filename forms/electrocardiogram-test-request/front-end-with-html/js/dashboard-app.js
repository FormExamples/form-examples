...
function matchesFilters(row) {
  if (filters.search) {
    ...
    const matches =
      ...
      String(row.ecgType || '').toLowerCase().includes(term) ||
...
function compareRows(a, b) {
  ...
  // Default: string compare (nhs, patient, ecgType, indication, clinician,
  // referralDate).
...
function renderTableBody() {
  ...
  for (const row of rows) {
    ...
    tr.innerHTML = `
      ...
      <td class="data-table-td">${esc(titleCase(row.ecgType))}</td>
