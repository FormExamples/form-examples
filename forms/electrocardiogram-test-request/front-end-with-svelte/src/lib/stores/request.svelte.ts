...
// Re-export the default factory so call sites can import it from the store.
export { createDefaultRequest };

/** localStorage draft key for a given request id (defaults to `new`). */
function storageKey(id: string): string {
	return `electrocardiogram-test-request.front-end-with-svelte.${id || 'new'}.v1`;
}

/**
 * Svelte 5 reactive store for the ECG test request, with localStorage
 ...
 */
class RequestStore {
	data = $state<EcgRequest>(createDefaultRequest());
	 ...
	 * `const d = request.data.symptoms`) stay bound to live state.
	 */
	loadForId(id: string, seed?: EcgRequest) {
		const key = id || 'new';
		this.id = key;
		this.result = null;
		this.currentStep = 1;

		let draft: EcgRequest | null = null;
		if (browser) {
			const raw = localStorage.getItem(storageKey(key));
			if (raw) {
				try {
					draft = JSON.parse(raw) as EcgRequest;
				} catch {
