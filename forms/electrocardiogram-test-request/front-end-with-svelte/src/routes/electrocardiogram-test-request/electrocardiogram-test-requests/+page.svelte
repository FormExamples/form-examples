<script lang="ts">
	...
	const columns = [
		...
		{ id: 'patientName', header: 'Patient', flexgrow: 2, sort: true },
		{
			...
			header: 'ECG type',
			flexgrow: 2,
			template: (v: string) => ecgTypeLabel(v as never)
		},
		{
		...
		{ id: 'referralDate', header: 'Requested', width: 110, sort: true },
		{
		...
		},
		{
			...
			width: 110,
			sort: true,
			template: (v: string) => priorityLabel(v as never)
		},
		{
			id: 'recommendation',
	...
	function init(api: any) {
		...
		api.on('select-row', (ev: { id?: string | number }) => {
			if (ev?.id != null) goto(`/electrocardiogram-test-request/electrocardiogram-test-requests/${ev.id}`);
		});
	}
</script>
...
<main class="mx-auto max-w-6xl px-4 py-6">
	<div class="mb-6 flex flex-wrap items-end justify-between gap-4">
		<div>
			<h1 class="text-2xl font-bold text-base-content">ECG vetting dashboard</h1>
			<p class="mt-1 text-sm text-base-content/70">
				Four-axis vetting grades for incoming ECG test requests, computed by the shared engine.
				Select a row to open the request.
			</p>
		</div>
		<a href="/electrocardiogram-test-request/electrocardiogram-test-requests/new" class="button" data-variant="primary">New request</a>
	</div>
