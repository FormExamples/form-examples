<script lang="ts">
	...
	function validate(): boolean {
		...
		if (d.request.ecgType === '') {
			found.push({ id: 'ecgType', message: 'Requested ECG type is required.' });
		}
	...
	function submit() {
		...
		goto(`/electrocardiogram-test-request/electrocardiogram-test-requests/${id}/report`);
	}
...
<main class="mx-16 px-4 py-6">
	<h1 class="text-2xl font-bold text-base-content">
		{isNew ? 'New ECG test request' : `ECG test request ${id}`}
	</h1>
	...
	<Form label="ECG test request" onsubmit={submit}>
