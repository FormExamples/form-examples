...
function storageKey(id: string): string {
	return `sleep-study-test-request.front-end-with-svelte.${id || 'new'}.v1`;
}

/**
 * Svelte 5 reactive store for the sleep-study request, with localStorage
 ...
 */
class RequestStore {
	data = $state<SleepStudyRequest>(createDefaultRequest());
	 ...
	 */
	loadForId(id: string, seed?: SleepStudyRequest) {
		const key = id || 'new';
		this.id = key;
		this.result = null;
		this.currentStep = 1;

		let draft: SleepStudyRequest | null = null;
		if (browser) {
			const raw = localStorage.getItem(storageKey(key));
			if (raw) {
				try {
					draft = JSON.parse(raw) as SleepStudyRequest;
				} catch {
