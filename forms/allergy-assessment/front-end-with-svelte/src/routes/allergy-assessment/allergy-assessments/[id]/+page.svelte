<script lang="ts">
	...
	function submit() {
		...
		assessment.result = {
			severityLevel,
			allergyBurdenScore: calculateAllergyBurden(assessment.data),
			firedRules,
			additionalFlags: detectAdditionalFlags(assessment.data),
			timestamp: new Date().toISOString()
		};
		goto(`/allergy-assessment/allergy-assessments/${id}/report`);
	}
...
<main class="mx-16 px-4 py-6">
	<h1 class="text-2xl font-bold text-base-content">
		{isNew ? 'New allergy assessment' : `Allergy assessment ${id}`}
	</h1>
	<p class="mt-1 text-sm text-base-content/70">
		Complete the ten sections; the severity grade and burden score are computed on submit.
	</p>
	...
	<Form label="Allergy assessment" onsubmit={submit}>
		<Step1Demographics />
		<Step2AllergyHistory />
		<Step3DrugAllergies />
		<Step4FoodAllergies />
