...
import type { AssessmentData, Subscores, FlaggedIssue } from './types';

export function detectFlaggedIssues(
	data: AssessmentData,
	grade: { subscores: Subscores; aggregateScore: number; maxParameterScore: number }
): FlaggedIssue[] {
	...
	if (data.concern.nurseConcern === 'yes') {
		flags.push({
			...
			suggestedAction: 'Escalate on the documented concern regardless of the aggregate total.'
		});
	}
	...
	if (missing.length > 0) {
		flags.push({
			...
			suggestedAction: 'Record the missing observation(s) and the age band, then re-score.'
		});
	}
