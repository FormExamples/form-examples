...
// parameter via the band tables in `pews-rules.ts` (the two rate parameters
// against the selected age band), sum the aggregate, take the maximum single-
// parameter sub-score, and derive the escalation band with the recommended
...
const BAND_SEVERITY: Record<EscalationBand, number> = {
	routine: 0,
	low: 1,
	medium: 2,
	high: 3
...
const PARAMETER_KEYS: (keyof Subscores)[] = [
	'respiratoryRate',
	'respiratoryEffort',
	'oxygenSaturation',
	'supplementalOxygen',
...
export function aggregateBand(aggregate: number): EscalationBand {
...
export function worstBand(a: EscalationBand, b: EscalationBand): EscalationBand {
	return BAND_SEVERITY[a] >= BAND_SEVERITY[b] ? a : b;
}

/** Monitoring frequency + recommendation for the final escalation band. */
function response(band: EscalationBand): {
...
function collectFiredRules(
	...
	if (maxParameter === 3) {
		fired.push({
			...
			description:
				'A single parameter scored 3 — escalate to at least medium regardless of the aggregate total.'
		});
	}
...
function collectFiredTriggers(data: AssessmentData, maxParameter: number): FiredTrigger[] {
	...
	if (maxParameter === 3) {
		triggers.push({
			...
			category: 'single-parameter',
			description:
				'Single parameter scored 3 — urgent review (at least medium escalation) regardless of the total.'
		});
	}
	if (data.concern.nurseConcern === 'yes') {
		triggers.push({
...
export function gradePews(data: AssessmentData): GradingResult {
	...
	const aggregateScore = PARAMETER_KEYS.reduce<number>(
		(sum, k) => sum + (subscores[k] ?? 0),
		0
	);

	// Maximum single-parameter sub-score (nulls treated as 0).
	const maxParameterScore = PARAMETER_KEYS.reduce<number>(
	...
	const triggerBand: EscalationBand = singleParameterTrigger ? 'medium' : 'routine';
