...
// of age. Each pure scoring function below maps a measured value to its subscore
// per the bands in `index.md` / `spec/index.md` §4. The grader
// (`pews-grader.ts`) sums the subscores into the aggregate and derives the
// escalation band. These band tables mirror the
...
/** A rate parameter scored against the age band. */
type RateParameter = 'respiratoryRate' | 'heartRate';

/** One [upperBoundInclusive, score] pair from a rate scoring table. */
type BandRow = [number, 0 | 1 | 2 | 3];

/**
 * Age-band scoring tables for the two rate parameters. Each parameter is an
 * ordered list of [upperBoundInclusive, score] pairs covering the whole numeric
 * domain. `scoreAgainstBand` returns the score of the first pair whose upper
 * bound is >= the measured value. The score-0 span is the age-band NORMAL range
 * from index.md; increasing deviation above or below scores 1, 2, then 3
 ...
 */
export const AGE_BAND_TABLE: Record<
	Exclude<AgeBand, ''>,
	Record<RateParameter, BandRow[]>
> = {
	neonate: {
 ...
 * Returns null when the value is unrecorded or the age band is unset.
 */
export function scoreAgainstBand(
	...
	if (ageBand === '' || !(ageBand in AGE_BAND_TABLE)) return null;
...
export function scoreRespiratoryRate(
	rr: number | null,
	ageBand: AgeBand
): 0 | 1 | 2 | 3 | null {
...
export function scoreHeartRate(hr: number | null, ageBand: AgeBand): 0 | 1 | 2 | 3 | null {
	return scoreAgainstBand(hr, ageBand, 'heartRate');
}

/** Respiratory effort / recession: none 0, mild 1, moderate 2, severe/grunting 3. */
export function scoreRespiratoryEffort(effort: RespiratoryEffort): 0 | 1 | 2 | 3 | null {
	switch (effort) {
		case 'none':
			return 0;
		case 'mild':
...
/** Oxygen saturation (SpO2, %): >= 96 -> 0, 94-95 -> 1, 92-93 -> 2, < 92 -> 3. */
export function scoreOxygenSaturation(spo2: number | null): 0 | 1 | 2 | 3 | null {
...
export function scoreSupplementalOxygen(value: SupplementalOxygen): 0 | 1 | 3 | null {
	switch (value) {
		case 'room-air':
			return 0;
		case 'low-flow':
...
export function scoreCapillaryRefill(value: CapillaryRefill): 0 | 1 | 2 | 3 | null {
	switch (value) {
		case 'under-2s':
			return 0;
		case '2-3s':
...
export function scoreConsciousness(value: Consciousness): 0 | 1 | 2 | 3 | null {
	switch (value) {
		case 'alert':
			return 0;
		case 'voice':
