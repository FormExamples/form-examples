<script lang="ts">
	...
	function validate(): boolean {
		...
		if (d.identification.ageBand === '') {
			found.push({
				id: 'identification-ageBand',
				message: 'Age band is required — it sets the normal ranges for the rate parameters.'
			});
		}
		errors = found;
	...
	function submit() {
		...
		assessment.result = gradePews(assessment.data);
		goto(`/paediatric-early-warning-score/${plural}/${id}/report`);
	}
...
<main class="mx-16 px-4 py-6">
	<h1 class="text-2xl font-bold text-base-content">
		{isNew ? 'New PEWS assessment' : `PEWS assessment ${id}`}
	</h1>
	<p class="mt-1 text-sm text-base-content/70">
		Complete the seven sections; the aggregate PEWS score and escalation band are computed on
		submit.
	</p>
	...
	<Form label="PEWS assessment" onsubmit={submit}>
		<Step1Context />
		<Step2Identification />
		<Step3Respiratory />
		<Step4Cardiovascular />
