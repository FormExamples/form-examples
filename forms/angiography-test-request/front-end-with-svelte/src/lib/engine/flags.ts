...
// metformin-contrast, missing-indication, missing-clinical-question, other.
//
// Each flag is { flagId, category, priority, description, suggestedAction }.
// Flag IDs are stable and identical across every front-end and the back-end.

import type { Flag, RequestData } from './types';
import { usesIonisingRadiation } from './utils';

/** Detect safety flags for a vascular angiography request. */
export function detectFlags(data: RequestData): Flag[] {
	const flags: Flag[] = [];
	const usesContrast =
	...
	if (
		...
		flags.push({
			...
			suggestedAction:
				'Discuss with the radiologist; individualise per ACR–NKF / ESUR, arrange renal-protective hydration, and consider a non-iodinated alternative.'
		});
	} else if (
		data.contrast.contrastRequired === 'iodinated' &&
		egfr !== null &&
		...
		flags.push({
			...
			suggestedAction:
				'Use renal-protective hydration, minimise contrast volume, and recheck renal function after the examination.'
		});
	}
	...
	if (
		...
		flags.push({
			...
			suggestedAction:
				'Review peri-procedural anticoagulation, check coagulation, and plan bridging / hold per local catheter-angiography policy.'
		});
	}
	...
	if (
		...
		flags.push({
			...
			suggestedAction:
				'Justify under IR(ME)R, consider MR angiography or ultrasound, and minimise / shield the dose if the examination proceeds.'
		});
	} else if (
		(data.pregnancy.pregnancyStatus === 'possible' ||
			data.pregnancy.pregnancyStatus === 'unknown') &&
		usesIonisingRadiation(data.request.angiographyType)
	) {
		flags.push({
			...
			suggestedAction:
				'Confirm pregnancy status before exposure; apply the IR(ME)R 28-day / 10-day rule.'
		});
	}

	// --- Metformin + contrast ------------------------------------------
	if (data.contrast.metformin === true && data.contrast.contrastRequired === 'iodinated') {
		flags.push({
			...
			suggestedAction:
				'Review metformin per ESUR guidance based on eGFR; withhold and recheck renal function where indicated.'
		});
	}
