<script lang="ts">
	...
	import Step4ContrastRenalSafety from '$lib/components/steps/Step4ContrastRenalSafety.svelte';
	import Step5BleedingAnticoagulation from '$lib/components/steps/Step5BleedingAnticoagulation.svelte';
	import Step6PregnancyRadiation from '$lib/components/steps/Step6PregnancyRadiation.svelte';
	...
	function validate(): boolean {
		...
		if (d.request.angiographyType === '') {
			found.push({ id: 'angiographyType', message: 'Angiography type is required.' });
		}
	...
	function submit() {
		...
		goto(`/angiography-test-request/angiography-test-requests/${id}/report`);
	}
...
<main class="mx-16 px-4 py-6">
	<h1 class="text-2xl font-bold text-base-content">
		{isNew ? 'New angiography request' : `Angiography request ${id}`}
	</h1>
	<p class="mt-1 text-sm text-base-content/70">
		Complete the seven sections; the four-axis grade, recommendation, and flags are computed on
		submit.
	</p>
	...
	<Form label="Angiography test request" onsubmit={submit}>
		<Step1RequestingClinician />
		<Step2PatientIdentification />
		<Step3RequestedExamination />
		<Step4ContrastRenalSafety />
		<Step5BleedingAnticoagulation />
		<Step6PregnancyRadiation />
		<Step7TriageSubmit />
