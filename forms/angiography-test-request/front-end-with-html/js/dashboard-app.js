...
// Angiography Test Request — vetting dashboard
...
// attach their exports to `window.AngiographyTestRequestDashboard`.
...
const columns = [
  ...
  { key: 'angiographyType',    label: 'Type' },
...
function safetyClass(b) {
  return b ? 'safety-' + String(b) : '';
}
...
function matchesFilters(row) {
  if (filters.search) {
    ...
    const matches =
      ...
      String(row.angiographyType || '').toLowerCase().includes(term) ||
...
function compareRows(a, b) {
  ...
  // Default: string compare (nhs, patient, angiographyType, bodyRegion,
...
function renderTableBody() {
  ...
  for (const row of rows) {
    ...
    tr.innerHTML = `
      ...
      <td class="data-table-td">${esc(titleCase(row.angiographyType))}</td>
...
function bindFilterInputs() {
  ...
  const safetySel = document.getElementById('filter-safety');
  ...
  if (safetySel) {
    safetySel.addEventListener('change', () => {
      filters.safety = safetySel.value;
      renderAll();
    });
  }
  if (flags) {
