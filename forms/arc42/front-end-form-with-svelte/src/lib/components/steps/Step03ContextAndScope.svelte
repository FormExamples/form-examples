...
<section>
  ...
  <label class="block mb-4">
    <span class="text-sm text-slate-700">Business context description</span>
  ...
  <div class="mb-6">
    <header class="flex items-center justify-between mb-2">
      <h3 class="text-md font-semibold text-slate-700">Business context partners (max {MAX_BUSINESS})</h3>
      <button type="button"
    ...
    {#each businessPartners() as item, i (i)}
      <div class="border border-slate-200 rounded p-3 mb-2">
        ...
        <button type="button" class="text-xs text-red-600 mt-2" onclick={() => removePartnerWhere('business', i)}>Remove</button>
      </div>
    {/each}
  </div>

  <label class="block mb-4">
    <span class="text-sm text-slate-700">Technical context description</span>
  ...
  <div class="mb-6">
    <header class="flex items-center justify-between mb-2">
      <h3 class="text-md font-semibold text-slate-700">Technical interfaces (max {MAX_TECHNICAL})</h3>
      <button type="button"
              class="text-sm px-2 py-1 rounded bg-slate-200 hover:bg-slate-300 disabled:opacity-50"
    ...
    {#each technicalInterfaces() as item, i (i)}
      <div class="border border-slate-200 rounded p-3 mb-2">
        ...
        <button type="button" class="text-xs text-red-600 mt-2" onclick={() => removePartnerWhere('technical', i)}>Remove</button>
      </div>
    {/each}
  </div>
</section>
