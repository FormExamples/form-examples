...
<section>
  ...
  <label class="block mb-4">
    <span class="text-sm text-slate-700">Solution strategy summary</span>
  ...
  <label class="block mb-4">
    <span class="text-sm text-slate-700">Top-level decomposition summary</span>
  ...
  <div class="mb-6">
    <header class="flex items-center justify-between mb-2">
      <h3 class="text-md font-semibold text-slate-700">Quality strategies (max {MAX_QUALITY_STRATEGIES})</h3>
      <button type="button"
    ...
    {#each store.data.qualityStrategies as _s, i (i)}
      <div class="flex gap-2 mb-2 items-start">
        ...
        <button type="button" class="text-xs text-red-600 mt-1" onclick={() => removeQualityStrategy(i)}>Remove</button>
      </div>
    {/each}
  </div>
</section>
