...
<section>
  ...
  <!-- Maturity preview -->
  <div class="bg-base-200 border border-base-300 rounded p-4 mb-6">
    <h3 class="text-md font-semibold text-base-content/80 mb-3">Maturity assessment</h3>

    <div class="grid grid-cols-2 gap-4 mb-3">
      <div>
        <span class="text-xs text-base-content/60 uppercase tracking-wide">Computed maturity</span>
        <p class="font-semibold text-lg capitalize">{result.computedMaturity || '—'}</p>
      </div>
      <div>
        <span class="text-xs text-base-content/60 uppercase tracking-wide">Final maturity</span>
        <p class="font-semibold text-lg capitalize">{result.finalMaturity || '—'}</p>
      </div>
    </div>

    <h4 class="text-sm font-semibold text-base-content/70 mb-2">Completeness by section</h4>
    <ul class="grid grid-cols-2 md:grid-cols-3 gap-1 mb-3 text-sm">
      {#each Object.entries(result.completenessBySection) as [n, c]}
        <li class="flex gap-1">
          ...
          <span class="capitalize font-medium {c === 'complete' ? 'text-success' : c === 'partial' ? 'text-warning' : 'text-error'}">{c}</span>
        </li>
      {/each}
    </ul>

    {#if result.firedRules.length > 0}
      <h4 class="text-sm font-semibold text-base-content/70 mb-2">Fired rules</h4>
      <ul class="text-sm mb-3 space-y-1">
        {#each result.firedRules as rule}
          <li><span class="font-mono text-xs text-base-content/60">{rule.ruleId}</span> — {rule.description}</li>
        {/each}
      </ul>
    {/if}

    {#if result.additionalFlags.length > 0}
      <h4 class="text-sm font-semibold text-base-content/70 mb-2">Flags</h4>
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
    <label class="block">
      <span class="text-sm text-base-content/80">Maturity override</span>
    ...
    <label class="block">
      <span class="text-sm text-base-content/80">Recommendation</span>
