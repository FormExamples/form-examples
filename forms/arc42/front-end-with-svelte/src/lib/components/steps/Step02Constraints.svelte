...
<section>
  ...
  <div class="mb-6">
    <header class="flex items-center justify-between mb-2">
      <h3 class="text-md font-semibold text-base-content/80">Technical constraints (max {MAX_TECHNICAL})</h3>
      <button type="button"
    ...
    {#each technicalItems() as item, i (i)}
      <div class="border border-base-300 rounded p-3 mb-2">
        ...
        <button type="button" class="text-xs text-error mt-2" onclick={() => removeConstraintWhere('technical', i)}>Remove</button>
      </div>
    {/each}
  </div>
  ...
  <div class="mb-6">
    <header class="flex items-center justify-between mb-2">
      <h3 class="text-md font-semibold text-base-content/80">Organizational constraints (max {MAX_ORGANIZATIONAL})</h3>
      <button type="button"
              class="text-sm px-2 py-1 rounded bg-base-300 hover:bg-base-300 disabled:opacity-50"
    ...
    {#each organizationalItems() as item, i (i)}
      <div class="border border-base-300 rounded p-3 mb-2">
        <input type="text" placeholder="Name" class="w-full border rounded px-2 py-1 mb-1" bind:value={item.name} />
        <textarea rows="2" placeholder="Description" class="w-full border rounded px-2 py-1" bind:value={item.description}></textarea>
        <button type="button" class="text-xs text-error mt-2" onclick={() => removeConstraintWhere('organizational', i)}>Remove</button>
      </div>
    {/each}
  </div>
  ...
  <div class="mb-6">
    <header class="flex items-center justify-between mb-2">
      <h3 class="text-md font-semibold text-base-content/80">Conventions (max {MAX_CONVENTION})</h3>
      <button type="button"
              class="text-sm px-2 py-1 rounded bg-base-300 hover:bg-base-300 disabled:opacity-50"
    ...
    {#each conventionItems() as item, i (i)}
      <div class="border border-base-300 rounded p-3 mb-2">
        ...
        <textarea rows="2" placeholder="Description" class="w-full border rounded px-2 py-1" bind:value={item.description}></textarea>
        <button type="button" class="text-xs text-error mt-2" onclick={() => removeConstraintWhere('convention', i)}>Remove</button>
      </div>
    {/each}
  </div>
</section>
