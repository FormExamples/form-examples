# arc42 — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the arc42 form, built with axum + Loco +
...
- [`arc42/`](./arc42/) — the Loco crate: `src/arc42/` holds `app.rs`
...
`src/arc42/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd arc42 && cargo check --all-targets && cargo test
```
