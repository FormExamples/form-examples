# Pathology Waiting List Card — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Pathology Waiting List Card form, built with axum + Loco +
...
- [`pathology_waiting_list_card/`](./pathology_waiting_list_card/) — the Loco crate: `src/pathology_waiting_list_card/` holds `app.rs`
...
`/metrics`. The registered domain controllers are:
  ...
  - `pathology_waiting_list_card`
  - `pathology_waiting_list_card_appointment`
  - `pathology_waiting_list_card_grade`
  - `pathology_waiting_list_card_grade_flag`
  - `pathology_waiting_list_card_grade_rule`
  - `patient`
  - `practitioner`

## Engine

`src/pathology_waiting_list_card/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd pathology_waiting_list_card && cargo check --all-targets && cargo test
```
