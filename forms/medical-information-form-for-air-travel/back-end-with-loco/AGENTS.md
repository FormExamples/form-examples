# Medical Information Form for Air Travel (MEDIF) — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Medical Information Form for Air Travel (MEDIF) form, built with axum + Loco +
...
- [`medical_information_form_for_air_travel/`](./medical_information_form_for_air_travel/) — the Loco crate: `src/medical_information_form_for_air_travel/` holds `app.rs`
...
`src/medical_information_form_for_air_travel/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd medical_information_form_for_air_travel && cargo check --all-targets && cargo test
```
