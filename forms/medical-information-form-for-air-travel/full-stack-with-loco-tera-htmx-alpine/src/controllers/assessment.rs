...
use crate::engine::{fitness_grader, types::AssessmentData, utils::band_label};
...
/// GET /assessment/{id} -- render the single-page MEDIF wizard.
#[debug_handler]
...
/// GET /assessment/{id}/report -- run the fitness engine, persist the
...
async fn show_report(
    ...
    let grade = fitness_grader::grade(&assessment_data);
    ...
    context.insert("fitness_band", &grade.fitness_band);
    context.insert("fitness_label", &band_label(&grade.fitness_band));
    context.insert("desk_recommendation", &grade.desk_recommendation);
    context.insert("valid_until", &grade.valid_until);
    context.insert("fired_rules", &grade.fired_rules);
    context.insert("safety_flags", &grade.safety_flags);
