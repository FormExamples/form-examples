 ...
 * produces), each flag carries a priority (spec §5):
 *
 ...
 *
 * @param status the completeness status; the `incomplete` flag fires when
 *   `status === 'incomplete'`.
 */
export function detectFlaggedIssues(
	...
	if (plan.healthActionPlanProduced !== 'yes') {
		flags.push({
			...
			priority: 'high',
			description: 'The check recorded findings but no Health Action Plan was produced',
			suggestedAction:
