import type { AssessmentData, ComponentRule } from './types';
...
//
// The grader (`ld-health-check-grader.ts`) evaluates every component rule,
// counts the completed ones, derives the completeness percentage and the
// complete/incomplete status, and appends an audit trail. Rows here mirror the
 ...
 */
export const NOT_RECORDED = new Set<string>([
	'',
	'not-recorded',
	'not-assessed',
	'not-reviewed',
...
export const componentRules: ComponentRule[] = [
	...
	},
	{
		...
		completed: (d) =>
			isRecorded(d.screening.cancerScreeningStatus) && isRecorded(d.screening.immunisationStatus)
	},
	{
	...
	},
	{
		...
		completed: (d) =>
			isRecorded(d.mental.mentalHealthStatus) && isRecorded(d.mental.behaviourStatus)
	},
	{
	...
	},
	{
		...
		completed: (d: AssessmentData) => isRecorded(d.carer.carerNeedsStatus)
	}
];
