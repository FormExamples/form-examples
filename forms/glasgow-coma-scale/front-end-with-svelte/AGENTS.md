# Glasgow Coma Scale — SvelteKit front-end (form + dashboard)
...
for the GCS specification and scoring engine. Lily Svelte headless
conventions: [`../../AGENTS-front-end-svelte.md`](../../AGENTS-front-end-svelte.md).

## Layout

- `src/lib/engine/` — pure scoring engine (`types.ts`, `utils.ts`,
  `gcs-rules.ts`, `gcs-grader.ts`, `flagged-issues.ts`) + `gcs-grader.test.ts`.
...
- `src/routes/glasgow-coma-scales/` — RESTful routes:
  `/<plural>/` (dashboard, `ssr = false`) + `/<plural>/[id]` (wizard) +
  `/<plural>/[id]/report` (+ `report/pdf` server endpoint).

## Engine

The GCS rates three components — Eye (E, 1-4), Verbal (V, 1-5), Motor (M, 1-6) —
each from a fixed descriptor list plus a "not testable" (NT) option. The total
(3-15) is defined only when all three components are numeric; any NT leaves the
total and severity band null. Severity bands: 13-15 mild, 9-12 moderate, 3-8
severe (coma). GCS-Pupils subtracts the pupil reactivity score (0-2 unreactive
pupils) from the total. The intubated verbal-NT case reports the "T" convention
(e.g. `8T`).
