...
//
// Rows here mirror the `glasgow_coma_scale_grade_flag` SQL table
 ...
 *           eyeScore: number|null, verbalScore: number|null,
 *           pupilReactivityScore: number|null }} grade  - from the grader
...
function detectFlaggedIssues(data, grade) {
  ...
  const totalFall2 =
    total !== null && prevTotal !== null && (prevTotal - total) >= 2;
  const motorFall =
    motor !== null && prevMotor !== null && motor < prevMotor;
  ...
  if (totalFall2 || motorFall) {
    const detail = totalFall2
      ? `Total GCS fell from ${prevTotal} to ${total} (drop of ${prevTotal - total})`
    ...
    flags.push({
      ...
      suggestedAction:
        'Urgent senior and neurosurgical review; consider CT head imaging.'
    });
  }
  ...
  if (asymmetric || eitherUnreactive) {
    ...
    flags.push({
      ...
      suggestedAction:
        'Urgent CT head and neurosurgical referral.'
    });
  }
