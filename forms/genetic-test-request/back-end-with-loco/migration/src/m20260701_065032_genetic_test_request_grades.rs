...
impl MigrationTrait for Migration {
    async fn up(&self, m: &SchemaManager) -> Result<(), DbErr> {
        create_table(m, "genetic_test_request_grades",
            &[
            ...
            ("consent_counselling_band", ColType::StringWithDefault(String::new())),
            ...
            &[
            ("genetic_test_request", ""),
            ]
        ).await
    }

    async fn down(&self, m: &SchemaManager) -> Result<(), DbErr> {
        drop_table(m, "genetic_test_request_grades").await
    }
}
