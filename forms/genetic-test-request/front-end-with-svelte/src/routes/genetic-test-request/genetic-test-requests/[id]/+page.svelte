<script lang="ts">
	...
	import Step4ClinicalDetails from '#lib/components/steps/Step4ClinicalDetails.svelte';
	import Step5ConsentAndCounselling from '#lib/components/steps/Step5ConsentAndCounselling.svelte';
	import Step6SpecimenAndTriage from '#lib/components/steps/Step6SpecimenAndTriage.svelte';
	...
	function submit() {
		...
		goto(`/genetic-test-request/genetic-test-requests/${id}/report`);
	}
...
<main class="mx-16 px-4 py-6">
	<h1 class="text-2xl font-bold text-base-content">
		{isNew ? 'New genetic test request' : `Genetic test request ${id}`}
	</h1>
	...
	<Form label="Genetic test request" onsubmit={submit}>
		<Step1RequestingClinician />
		<Step2PatientIdentification />
		<Step3RequestedTest />
		<Step4ClinicalDetails />
		<Step5ConsentAndCounselling />
		<Step6SpecimenAndTriage />
