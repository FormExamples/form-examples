...
import type { GeneticTestRequest, GradingResult } from '$lib/engine/types';
import { createDefaultRequest } from '$lib/engine/defaults';

// Re-export the canonical empty-shape factory so consumers can import it from
// the store as well as from the engine.
export { createDefaultRequest };

/** localStorage draft key for a given request id (defaults to `new`). */
function storageKey(id: string): string {
	return `genetic-test-request.front-end-with-svelte.${id || 'new'}.v1`;
}

/**
 * Svelte 5 reactive store for the genetic test request, with localStorage
 ...
 */
class RequestStore {
	data = $state<GeneticTestRequest>(createDefaultRequest());
	 ...
	 * (e.g. `const d = request.data`) stay bound to live state.
	 */
	loadForId(id: string, seed?: GeneticTestRequest) {
		const key = id || 'new';
		this.id = key;
		this.result = null;
		this.currentStep = 1;

		let draft: GeneticTestRequest | null = null;
		if (browser) {
			const raw = localStorage.getItem(storageKey(key));
			if (raw) {
				try {
					draft = JSON.parse(raw) as GeneticTestRequest;
				} catch {
