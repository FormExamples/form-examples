...
function applyInstructionRules(app) {
  const fired = [];
  ...
  if (app.lstChoice === '') {
    fired.push({
      ...
      suggestedCorrection: 'Choose Option A or Option B on LP1H s.5.',
    });
  ...
  app.instructions.forEach((ins, idx) => {
    ...
    if (ins.contradictsAdrt === 'yes') {
      fired.push({
        ...
        suggestedCorrection: 'Reconcile the LPA instruction with the existing ADRT.',
      });
    }
    if (ins.statement.trim() && PROHIBITED_PATTERNS.some((p) => p.test(ins.statement))) {
      fired.push({
        ...
        description: `Instruction #${idx + 1} appears to authorise an unlawful act.`,
