...
export function applyDonorRules(app: LpaApplication): FiredRule[] {
  const fired: FiredRule[] = [];
  const { donor } = app;
  const donorSig = findDonorSignature(app);
  const signingDate = donorSig?.signedAt || donor.capacityDeclaredAt || new Date().toISOString();
  ...
  if (donor.jurisdiction !== 'england' && donor.jurisdiction !== 'wales') {
    fired.push({
      ...
      suggestedCorrection: 'Select England or Wales as the donor jurisdiction. Scotland and Northern Ireland use different schemes.',
    });
  }
