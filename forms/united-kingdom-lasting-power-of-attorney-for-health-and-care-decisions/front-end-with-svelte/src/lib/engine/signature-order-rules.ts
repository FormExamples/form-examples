...
export function applySignatureOrderRules(app: LpaApplication): FiredRule[] {
  ...
  if (!cpSig || !signatureSignedAt(cpSig)) {
    fired.push({
      ...
      suggestedCorrection: 'The certificate provider must sign LP1H s.10 before the attorneys sign.',
    });
  }
  ...
  if (attSigs.length === 0 || attSigs.some((s) => !signatureSignedAt(s))) {
    fired.push({
      ...
      suggestedCorrection: 'Every attorney must sign LP1H s.11 after the donor and the certificate provider.',
    });
  }
  ...
  if (donorSig) {
    const witnessIsDonor =
      donorSig.witnessName.trim().toLowerCase() ===
      `${app.donor.givenNames} ${app.donor.familyName}`.trim().toLowerCase();
    if (witnessIsDonor && donorSig.witnessName.trim() !== '') {
      fired.push({
