# Organ Donation Assessment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Organ Donation Assessment form, built with axum + Loco +
...
- [`organ_donation_assessment/`](./organ_donation_assessment/) — the Loco crate: `src/organ_donation_assessment/` holds `app.rs`
...
`src/organ_donation_assessment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd organ_donation_assessment && cargo check --all-targets && cargo test
```
