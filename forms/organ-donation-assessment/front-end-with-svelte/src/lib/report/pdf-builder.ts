...
export function buildPdfDocument(
	...
	return {
		...
		header: {
			text: 'ORGAN DONATION ASSESSMENT REPORT',
			alignment: 'center',
			margin: [0, 20, 0, 0],
			fontSize: 10,
			color: '#6b7280',
		...
		content: [
			...
			{
				text: `${riskLevelLabel(result.riskLevel)} · ${donorTypeLabel(data.donorTypeRegistration.donorType)}`,
				fontSize: 12,
				alignment: 'center',
				color: '#4b5563',
				margin: [0, 0, 0, 4]
			},
			...(result.eligibility !== result.suggestedEligibility
				? [
						{
							text: `Engine-suggested eligibility: ${eligibilityLabel(result.suggestedEligibility)} (assessor decision shown above).`,
							fontSize: 9,
			...
			sectionHeader('Donor Details'),
			{
				table: {
					...
					body: [
						...
						[
							field('Donor type', donorTypeLabel(data.donorTypeRegistration.donorType)),
							field('Intended organ(s)', data.donorTypeRegistration.intendedOrgans || 'N/A')
						]
					]
				},
			...
			...(result.additionalFlags.length > 0
				? [
						sectionHeader('Flagged Issues for the Donor Assessment Team'),
						{
			...
			...(result.firedRules.length > 0
				? [
						sectionHeader('Donor Rule Audit'),
						{
							table: {
								headerRows: 1,
								widths: [60, 90, '*', 50],
