import type { AssessmentData, AdditionalFlag, InfectiousDiseaseScreening } from './types';
import { calculateAge } from './utils';

/**
 * Detects additional flags that should be highlighted for the donor assessment
 * team, independent of the rule-firing grader. These raise clinician-facing
 * alerts for safety-critical findings (positive infection screens, ABO
 * incompatibility, malignancy, coercion, missing consent, etc.).
 *
 * Priorities: high (absolute contraindications / safety-critical),
 * medium (expanded-criteria markers, modest organ dysfunction),
 * low (advanced age, controlled hypertension). Sorted high → medium → low.
 */
export function detectAdditionalFlags(data: AssessmentData): AdditionalFlag[] {
	...
	const positiveLabels: Partial<Record<keyof InfectiousDiseaseScreening, string>> = {
		hivStatus: 'HIV',
	...
	for (const key of Object.keys(positiveLabels) as (keyof InfectiousDiseaseScreening)[]) {
		if (id[key] === 'positive') {
			flags.push({
				id: `FLAG-ID-${key}`,
	...
	// CMV / EBV: surface any positive donor result as medium, since recipient
	// status is not tracked in this form.
