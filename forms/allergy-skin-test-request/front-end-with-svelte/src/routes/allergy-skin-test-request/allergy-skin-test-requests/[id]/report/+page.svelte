<script lang="ts">
	...
	$effect(() => {
		if (!request.result) {
			goto(`/allergy-skin-test-request/allergy-skin-test-requests/${id}`);
		}
	});

	const panels = $derived(selectedPanelCategories(data.test).map((c) => titleCase(c)).join(', '));
	...
	async function downloadPDF() {
		pdfError = '';
		try {
			const res = await fetch(`/allergy-skin-test-requests/${id}/report/pdf`, {
				method: 'POST',
			...
			if (res.ok) {
				...
				a.href = url;
				a.download = `allergy-skin-test-request-${data.patient.lastName || id}.pdf`;
				a.click();
...
{#if result}
	<header class="border-b border-base-300 bg-base-100 shadow-sm no-print">
		<div class="mx-auto flex max-w-4xl items-center justify-between px-4 py-4">
			<h1 class="text-lg font-bold text-base-content">Allergy test request vetting report</h1>
			<div class="flex items-center gap-3">
				...
				<Button data-variant="secondary" onclick={() => goto(`/allergy-skin-test-request/allergy-skin-test-requests/${id}`)}>Edit</Button>
			</div>
		</div>
	</header>
	...
	<main class="mx-16 px-4 py-6">
		...
		<div class="mb-6 rounded-xl border-2 p-6 text-center {recommendationColor(result.recommendation)}">
			...
			<div class="mt-2 flex flex-wrap justify-center gap-6 text-sm">
				<span>{testTypeLabel(data.test.testType) || '—'}</span>
				{#if panels}<span>Panels: {panels}</span>{/if}
		...
		<div class="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-2">
			...
			<div class="rounded-xl border border-base-300 bg-base-100 p-5">
				<div class="text-sm font-medium text-base-content/70">B · Validity &amp; safety</div>
				<div class="mt-2">
					<span class="rounded-full border px-3 py-1 text-sm font-bold {validityBandColor(result.validitySafetyBand)}">
						{validityBandLabel(result.validitySafetyBand)}
					</span>
				</div>
			</div>
			<div class="rounded-xl border border-base-300 bg-base-100 p-5">
				...
				<div class="mt-2 h-2 overflow-hidden rounded-full bg-base-300">
					<div class="h-full {completenessColor(result.completenessPercent)}" style="width: {result.completenessPercent}%"></div>
				</div>
			</div>
			<div class="rounded-xl border border-base-300 bg-base-100 p-5">
				<div class="text-sm font-medium text-base-content/70">D · Triage priority</div>
				<div class="mt-2">
					<span class="rounded-full border px-3 py-1 text-sm font-bold {triageTierColor(result.triageTier)}">
						{triageTierLabel(result.triageTier)}
					</span>
					{#if result.targetTimeframe}
						<span class="ml-2 text-sm text-base-content/70">{result.targetTimeframe}</span>
					{/if}
				</div>
			</div>
		</div>
		...
		<div class="mb-6 rounded-xl border border-base-300 bg-base-100 p-6">
			...
			<div class="grid grid-cols-1 gap-4 text-sm sm:grid-cols-2">
				...
				<div><span class="font-medium text-base-content/70">Test type:</span> {testTypeLabel(data.test.testType) || 'N/A'}</div>
			...
			{#if data.indication.clinicalQuestion}
				<p class="mt-4 text-sm"><span class="font-medium text-base-content/70">Clinical question:</span> {data.indication.clinicalQuestion}</p>
			{/if}
		</div>
	</main>
{/if}
