...
function storageKey(id: string): string {
	return `allergy-skin-test-request.front-end-with-svelte.${id || 'new'}.v1`;
}

/**
 * Svelte 5 reactive store for the allergy skin test request, with localStorage
 ...
 */
class RequestStore {
	data = $state<RequestData>(createDefaultRequest());
	 ...
	 */
	loadForId(id: string, seed?: RequestData) {
		const key = id || 'new';
		this.id = key;
		this.result = null;
		this.currentStep = 1;

		let draft: RequestData | null = null;
		if (browser) {
			const raw = localStorage.getItem(storageKey(key));
			if (raw) {
				try {
					draft = JSON.parse(raw) as RequestData;
				} catch {
