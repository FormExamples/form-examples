...
//
// Ported from `front-end-form-with-html/js/flags.js`. Pure function returning
// safety flags using the grade_flag categories from the SQL source of truth:
...
// missing-clinical-details, no-allergen-selected, other. Flag IDs are stable
// and identical across every front-end and the back-end.

import type { RequestData, SafetyFlag } from './types';
import { countSelectedPanels, SKIN_TEST_TYPES } from './rules';

/** Detect safety flags for an allergy test request. */
export function detectFlags(data: RequestData): SafetyFlag[] {
