...
// validity-band dropdown + safety-flags dropdown).
...
const columns = [
  ...
  { key: 'validitySafetyBand',  label: 'Validity' },
...
function validityClass(band) {
  return band ? 'validity-' + String(band) : '';
}
...
function matchesFilters(row) {
  if (filters.search) {
    ...
    const allergens = Array.isArray(row.allergens) ? row.allergens.join(' ') : '';
    const matches =
      ...
      allergens.toLowerCase().includes(term) ||
  ...
  if (filters.validity && row.validitySafetyBand !== filters.validity) return false;
...
function compareRows(a, b) {
  ...
  if (key === 'validitySafetyBand') {
    return ((validityRank[av] ?? -1) - (validityRank[bv] ?? -1)) * dir;
  }
  if (key === 'triageTier') {
...
function renderAllergensCell(allergens) {
  if (!Array.isArray(allergens) || allergens.length === 0) {
    return '<span class="flag-empty">—</span>';
  }
  const chips = allergens
    .map((a) => `<span class="allergen-chip">${esc(titleCase(a))}</span>`)
    .join('');
...
function renderTableBody() {
  ...
  for (const row of rows) {
    ...
    if (row.validitySafetyBand === 'contraindicated') {
    ...
    tr.innerHTML = `
      ...
      <td class="data-table-td">${renderAllergensCell(row.allergens)}</td>
      ...
      <td class="data-table-td"><span class="validity-badge ${validityClass(row.validitySafetyBand)}">${esc(titleCase(row.validitySafetyBand))}</span></td>
