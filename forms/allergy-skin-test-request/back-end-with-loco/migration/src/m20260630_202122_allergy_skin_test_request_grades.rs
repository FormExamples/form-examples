...
impl MigrationTrait for Migration {
    async fn up(&self, m: &SchemaManager) -> Result<(), DbErr> {
        create_table(m, "allergy_skin_test_request_grades",
            &[
            ...
            ("validity_safety_band", ColType::StringWithDefault(String::new())),
            ...
            &[
            ("allergy_skin_test_request", ""),
            ]
        ).await
    }

    async fn down(&self, m: &SchemaManager) -> Result<(), DbErr> {
        drop_table(m, "allergy_skin_test_request_grades").await
    }
}
