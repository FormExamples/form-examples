...
import type { CytologyResult, GradingResult } from '$lib/engine/types';
import {
...
/** Builds the pdfmake document definition for the cytopathology report. */
export function buildPdfDocument(
	...
	return {
		...
		footer: (currentPage: number, pageCount: number) => ({
			text: `Page ${currentPage} of ${pageCount} | Generated ${new Date(result.gradedAt).toLocaleString()} | NHS Cervical Screening / RCPath`,
			alignment: 'center',
			margin: [0, 20, 0, 0],
			fontSize: 8,
			color: '#9ca3af'
		...
		content: [
			...
			{
				ul: [
					`Result category: ${data.cytologyResultCategory || 'N/A'}`,
					`HPV result: ${hpvResultLabel(data.hpvResult)}`,
					`Malignancy present: ${data.malignancyPresent ? 'Yes' : 'No'}`,
					`Dysplasia present: ${data.dysplasiaPresent ? 'Yes' : 'No'}`
				],
				margin: [0, 0, 0, 8] as Margin
			},
			{ text: data.microscopicDescription || 'No microscopic description.', margin: [0, 0, 0, 16] as Margin },

			// Diagnosis and category
			sectionHeader('Diagnosis & category'),
			{ text: data.diagnosis || 'Not specified', margin: [0, 0, 0, 4] as Margin },
			{
