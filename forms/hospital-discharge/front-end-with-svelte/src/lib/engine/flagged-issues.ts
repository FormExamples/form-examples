...
/** Detect safety-critical and quality flags for the discharge summary. */
export function detectAdditionalFlags(data: AssessmentData): AdditionalFlag[] {
	...
	if (data.dischargeMedications.reconciliationCompleted === 'no') {
		flags.push({
			...
			message:
				'Medication reconciliation NOT completed - high risk of medication error on discharge.',
			priority: 'urgent'
		});
	}
	...
	if (
		...
		data.dischargeMedications.medications.some((m) => !m.name || !m.dose || !m.frequency)
	) {
		flags.push({
			id: 'FLAG-MEDS-003',
			category: 'Discharge Medications',
	...
	if (data.diagnoses.diagnoses.length > 0 && data.diagnoses.diagnoses.every((x) => !x.icd10)) {
		flags.push({
			id: 'FLAG-DX-003',
			category: 'Diagnoses',
	...
	if (
		...
		if (
			...
			flags.push({
				...
				category: 'Community Care',
				message:
					'Patient self-caring at home with active community referrals but no package of care recorded.',
				priority: 'medium'
			});
		}
	}
