# References

## Record standard

- Professional Record Standards Body. *eDischarge summary standard.*
  <https://theprsb.org/standards/edischargesummary/>
- NHS Digital. *DCB1577 Transfer of Care: Inpatient Discharge.*
  <https://digital.nhs.uk/data-and-information/information-standards/information-standards-and-data-collections-including-extractions/publications-and-notifications/standards-and-collections/dcb1577-transfer-of-care-inpatient-discharge>
- NHS England. *UK Core FHIR Implementation Guide.*
  <https://digital.nhs.uk/services/fhir-uk-core>
- HL7. *International Patient Summary IG.*
  <http://hl7.org/fhir/uv/ips/>
- LOINC. *18842-5 Discharge summary.* <https://loinc.org/18842-5/>

## Clinical guidance

- NICE NG27. *Transition between inpatient hospital settings and
  community or care home settings for adults with social care needs.*
  <https://www.nice.org.uk/guidance/ng27>
- NICE NG5. *Medicines optimization.*
  <https://www.nice.org.uk/guidance/ng5>
- NICE NG94. *Emergency and acute medical care in over 16s.*
  <https://www.nice.org.uk/guidance/ng94>
- NICE QS136. *Transition between inpatient hospital settings and
  community or care home settings.*
  <https://www.nice.org.uk/guidance/qs136>

## Patient flow and discharge model

- NHS England. *SAFER patient flow bundle.*
  <https://www.england.nhs.uk/sustainableimprovement/safer-patient-flow-bundle/>
- UK Government. *Hospital discharge and community support: policy
  and operating model.*
  <https://www.gov.uk/government/publications/hospital-discharge-and-community-support-policy-and-operating-model>
- NHS England. *SBAR communication tool.*
  <https://www.england.nhs.uk/wp-content/uploads/2021/03/qsir-sbar-communication-tool.pdf>

## Coding

- WHO. *ICD-10.* <https://icd.who.int/browse10/2019/en>
- NHS Digital. *OPCS-4.*
  <https://isd.digital.nhs.uk/trud/users/guest/filters/0/categories/10/items/119/releases>
- SNOMED International. *SNOMED CT.* <https://www.snomed.org/>

## Medication safety

- Royal Pharmaceutical Society. *Keeping patients safe when they
  transfer between care providers.*
  <https://www.rpharms.com/resources/reports/getting-the-medicines-right>
- WHO. *Medication Without Harm.*
  <https://www.who.int/initiatives/medication-without-harm>
- NHS Digital. *Electronic Prescription Service.*
  <https://digital.nhs.uk/services/electronic-prescription-service>

## Safety, safeguarding, data

- NHS England. *Learn from patient safety events service.*
  <https://www.england.nhs.uk/patient-safety/learn-from-patient-safety-events-service/>
- WHO. *Conceptual framework for the international classification for
  patient safety.*
  <https://www.who.int/publications/i/item/conceptual-framework-for-the-international-classification-for-patient-safety>
- Care Act 2014. <https://www.legislation.gov.uk/ukpga/2014/23/contents>
- Mental Capacity Act 2005.
  <https://www.legislation.gov.uk/ukpga/2005/9/contents>
- NHS England. *Accessible Information Standard.*
  <https://www.england.nhs.uk/about/equality/equality-hub/patient-equalities-programme/equality-frameworks-and-information-standards/accessibleinfo/>
- *NHS Records Management Code of Practice.*
  <https://www.nhsx.nhs.uk/information-governance/guidance/records-management-code/>
