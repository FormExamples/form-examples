...
Pure JSON API back-end for the Hospital Discharge form, built with axum + Loco +
...
- [`hospital_discharge/`](./hospital_discharge/) — the Loco crate: `src/hospital_discharge/` holds `app.rs`
...
`src/hospital_discharge/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd hospital_discharge && cargo check --all-targets && cargo test
```
