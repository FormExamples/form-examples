...
- [`dental_assessment/`](./dental_assessment/) — the Loco crate: `src/dental_assessment/` holds `app.rs`
...
`src/dental_assessment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd dental_assessment && cargo check --all-targets && cargo test
```
