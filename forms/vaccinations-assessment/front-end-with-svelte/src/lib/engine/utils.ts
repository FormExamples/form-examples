...
export function vaccinationLevelColor(level: string): string {
	switch (level) {
		case 'upToDate': return 'bg-success text-success-content border-success';
		case 'partiallyComplete': return 'bg-warning text-warning-content border-warning';
		case 'overdue': return 'bg-error text-error-content border-error';
		case 'contraindicated': return 'bg-secondary text-secondary-content border-secondary';
