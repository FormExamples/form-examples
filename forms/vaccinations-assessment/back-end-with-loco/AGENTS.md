# Vaccinations Assessment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Vaccinations Assessment form, built with axum + Loco +
...
- [`vaccinations_assessment/`](./vaccinations_assessment/) — the Loco crate: `src/vaccinations_assessment/` holds `app.rs`
...
`src/vaccinations_assessment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd vaccinations_assessment && cargo check --all-targets && cargo test
```
