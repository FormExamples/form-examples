...
// urgency (most urgent wins), evaluated by the grader (`grader.js`). Rows
// here mirror the `diabetes_podiatry_assessment_grade_rule` SQL table
 ...
 * @typedef {import('./types.js').FootExam} FootExam
 ...
 *
 * @typedef {Object} OverallContext
 * @property {Risk} rightFootRisk
 * @property {Risk} leftFootRisk
 * @property {Risk} worstFootRisk
 * @property {boolean} onRenalReplacementTherapy
 * @property {boolean} tighterHighRiskInterval
 *
 ...
 * @property {1 | 3 | 6 | 12 | null} intervalMonths
 * @property {string} description
 * @property {(ctx: OverallContext) => boolean} evaluate
 */
 ...
 * @returns {number}
 */
function perFootRiskFactorCount(foot) {
  let count = 0;
 ...
 * @returns {boolean}
 */
function hasHistory(foot) {
  return foot.previousUlcer === 'yes' ||
    foot.previousAmputation === 'minor' ||
    foot.previousAmputation === 'major';
}

/**
 * Derive the overall classification context the review-pathway cascade
 * evaluates.
 * @param {AssessmentData} d
 * @returns {OverallContext}
 */
function deriveContext(d) {
  const rightFootRisk = perFootRisk(d.rightFoot);
  const leftFootRisk = perFootRisk(d.leftFoot);
  const worstFootRisk = RISK_SEVERITY[rightFootRisk] >= RISK_SEVERITY[leftFootRisk]
    ? rightFootRisk
    : leftFootRisk;
  const worstFoot = RISK_SEVERITY[rightFootRisk] >= RISK_SEVERITY[leftFootRisk]
    ? d.rightFoot
    : d.leftFoot;
  return {
    rightFootRisk,
...
const reviewRules = [
  ...
  // ─── 2. HIGH RISK — patient-wide or per-foot ───────────────────
  {
    ...
    evaluate: (ctx) =>
      (ctx.worstFootRisk === 'high' || ctx.onRenalReplacementTherapy) &&
      ctx.tighterHighRiskInterval
  },
  {
  ...
  // ─── 3. MODERATE RISK — one risk factor ────────────────────────
  {
    ...
    intervalMonths: 6,
    description:
      'Moderate risk (one risk factor on the worse foot) — 6-monthly review by the foot protection service.',
  ...
  // ─── 4. LOW RISK — annual review ───────────────────────────────
  {
    ...
    intervalMonths: 12,
    description:
      'Low risk (no risk factors on either foot) — annual review.',
    evaluate: () => true
  }
];

export { reviewRules, deriveContext, perFootRisk, perFootBaseRisk, perFootRiskFactorCount, hasHistory, RISK_SEVERITY };
