...
import type { AssessmentData, FootExam, GradingResult } from '#lib/engine/types.js';

/** localStorage draft key for a given assessment id (defaults to `new`). */
function storageKey(id: string): string {
	return `diabetes-podiatry-assessment.front-end-with-svelte.${id || 'new'}.v1`;
}

/** A blank per-foot examination block with all fields at their unanswered defaults. */
...
/** A blank diabetes-podiatry-assessment record with all fields at their unanswered defaults. */
...
/**
 * Svelte 5 reactive store for the diabetes-podiatry-assessment record, with
...
class AssessmentStore {
	...
	/**
	 * Load the assessment for `id` into the store. A saved draft for that id
	 * (in localStorage) takes precedence; otherwise the `seed` record is used
	 * (e.g. a sample for an existing id), falling back to a blank draft.
	 *
	 * The data is merged in place (nested object identities preserved) rather
	 * than reassigned, so step components that captured a section reference
	 * (e.g. `const d = assessment.data.rightFoot`) stay bound to live state.
	 */
