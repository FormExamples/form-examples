import type { AssessmentData, FlaggedIssue, Risk, Status } from './types';
import { perFootRiskFactorCount, RISK_SEVERITY } from './podiatry-rules';
...
export function detectFlaggedIssues(data: AssessmentData, grade: GradeSummary): FlaggedIssue[] {
	...
	if (data.rightFoot.activeUlcer === 'yes' || data.leftFoot.activeUlcer === 'yes') {
		flags.push({
			...
			priority: 'high',
			description: 'Active ulceration present on at least one foot.',
			suggestedAction: 'Urgent referral to the multidisciplinary foot team (same or next working day).'
		});
	}
	...
	if (data.rightFoot.suspectedCharcot === 'yes' || data.leftFoot.suspectedCharcot === 'yes') {
		flags.push({
			...
			suggestedAction: 'Urgent same/next-working-day referral to the multidisciplinary foot team; do not wait for imaging.'
		});
	}
	...
	if (data.rightFoot.ulcerSeverity === 'critical-ischaemia' || data.leftFoot.ulcerSeverity === 'critical-ischaemia') {
		flags.push({
			...
			priority: 'high',
			description: 'Signs of critical limb ischaemia recorded on at least one foot ulcer.',
			suggestedAction: 'Urgent vascular referral.'
		});
	}
	...
	if (data.rightFoot.previousAmputation === 'major' || data.leftFoot.previousAmputation === 'major') {
		flags.push({
			...
			suggestedAction: 'Manage under the multidisciplinary foot team; review the contralateral / residual limb closely.'
		});
	}
	...
	if (data.riskFactors.onRenalReplacementTherapy === 'yes') {
		flags.push({
			...
			priority: 'high',
			description: 'Patient is on renal replacement therapy (dialysis) — an automatic high-risk factor per NICE NG19.',
			suggestedAction: 'Manage as high risk regardless of current foot examination findings.'
		});
	}
	...
	if (
		...
		flags.push({
			...
			suggestedAction: 'Review referral timing to the multidisciplinary foot team.'
		});
	}

	// ─── Footwear (MEDIUM) ───────────────────────────────────────
	if (data.riskFactors.footwearAppropriate === 'no' && RISK_SEVERITY[grade.overallRisk] >= RISK_SEVERITY['moderate']) {
		flags.push({
			id: 'F-FOOTWEAR-001',
			category: 'footwear',
			priority: 'medium',
			description: "Current footwear is not appropriate for the patient's risk level.",
			suggestedAction: 'Refer to orthotics / footwear assessment.'
		});
	}

	// ─── Self-care (MEDIUM) ──────────────────────────────────────
	const selfCareImpaired =
		data.riskFactors.selfCareAbility !== 'independent' || data.riskFactors.visualAcuityImpairment === 'yes';
	...
	if (selfCareImpaired && !supportNoted) {
		flags.push({
			...
			category: 'self-care',
			priority: 'medium',
			description: "Patient's ability to self-inspect and self-care for their feet is impaired, with no support recorded.",
			suggestedAction: 'Arrange support (carer, family, or service) for regular foot self-inspection.'
		});
	}
	...
	if (grade.status === 'incomplete') {
		flags.push({
			...
			category: 'incomplete',
			priority: 'low',
			description: 'A foot is missing a neuropathy or pulses test result — the risk classification may understate risk.',
			suggestedAction: 'Complete the sensory neuropathy and pedal pulses test for each foot.'
		});
	}
