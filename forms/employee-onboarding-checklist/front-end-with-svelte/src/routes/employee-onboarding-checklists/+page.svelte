<script lang="ts">
	...
	import { completionStatusLabel, riskLevelLabel } from '$lib/engine/utils';
	...
	const columns = [
		...
		{ id: 'dbsCleared', header: 'DBS', width: 90, template: (v: boolean) => (v ? 'Cleared' : 'No') },
	...
	function init(api: any) {
		...
		api.on('select-row', (ev: { id?: string | number }) => {
			if (ev?.id != null) goto(`/employee-onboarding-checklists/${ev.id}`);
		});
	}
</script>
...
<main class="mx-auto max-w-6xl px-4 py-6">
	<div class="mb-6 flex flex-wrap items-end justify-between gap-4">
		...
		<a href="/employee-onboarding-checklists/new" class="button" data-variant="primary">New checklist</a>
	</div>
