...
// professional registration, mandatory training, ID badge, manager sign-
// off, and confidentiality agreement.
//
// Ported 1-to-1 from `src/lib/engine/flagged-issues.ts`.
...
function detectAdditionalFlags(data) {
  ...
  if (data.preEmploymentChecks.rightToWorkExpiryDate) {
    ...
    if (!isNaN(expiry.getTime()) && expiry < threeMonths) {
      flags.push({
        id: 'FLAG-RTW-002',
        category: 'Pre-Employment',
