# Workplace Safety Assessment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Workplace Safety Assessment form, built with axum + Loco +
...
- [`workplace_safety_assessment/`](./workplace_safety_assessment/) — the Loco crate: `src/workplace_safety_assessment/` holds `app.rs`
...
`src/workplace_safety_assessment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd workplace_safety_assessment && cargo check --all-targets && cargo test
```
