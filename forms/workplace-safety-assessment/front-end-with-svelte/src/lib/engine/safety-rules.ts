import type { AssessmentData, SafetyRule, YesNoNA, SeverityGrade } from './types';

// Workplace Safety Assessment — declarative scoring rules.
//
// Each rule maps a single audit checklist item to a severity grade when the
...
//   - 1 when the audited control is in place (yes / N/A in the safe direction);
...
function gradeNoIsBad(answer: YesNoNA, severity: SeverityGrade): number {
	if (answer === '') return 0;
...
function gradeYesIsBad(answer: YesNoNA, severity: SeverityGrade): number {
	if (answer === '') return 0;
...
/** Look up a rule by id (keeps checklist rendering in sync with the registry). */
export function ruleById(id: string): SafetyRule | undefined {
	return safetyRules.find((r) => r.id === id);
}

/** Re-exported for callers that wish to evaluate a single answer directly. */
