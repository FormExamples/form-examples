import type { AssessmentData, AdditionalFlag, FlagPriority } from './types';

// Flagged-issue detection. Independent of the grade aggregation (which the
// safety-grader computes), this module raises auditor-facing flags prioritised
// as high / medium / low for the kinds of finding that warrant targeted
// follow-up beyond the headline grade.

/**
 * Detect additional flags that should be highlighted for the auditor,
 * independent of the headline outcome. These are safety-critical alerts.
 */
export function detectAdditionalFlags(data: AssessmentData): AdditionalFlag[] {
	...
	if (data.fireSafety.fireExtinguishersServiced === 'no') {
		flags.push({
			...
			message:
				'Fire extinguishers have no current annual service record — immediate servicing required.',
			priority: 'high'
		});
	}
	...
	if (data.chemicalBiologicalHazards.untreatedSpillsObserved === 'yes') {
		flags.push({
			...
			message:
				'Untreated chemical or biological spill observed — clean up using spill kit and PPE.',
			priority: 'high'
		});
	}
	...
	if (data.chemicalBiologicalHazards.coshhRegisterPresent === 'no') {
		flags.push({
			...
			category: 'Chemical & Biological',
			message:
				'COSHH register is missing — required by the Control of Substances Hazardous to Health Regulations 2002.',
			priority: 'high'
		});
	}
	...
	if (data.incidentReporting.riddorReportableIncidentsReported === 'no') {
		flags.push({
			...
			message:
				'RIDDOR-reportable incidents have not been reported to HSE — legal duty under RIDDOR 2013.',
			priority: 'high'
		});
	}
	...
	if (incidents !== null && nearMisses !== null && incidents > 0 && nearMisses < incidents) {
		flags.push({
			id: 'FLAG-INC-003',
			category: 'Incident Reporting',
	...
	const priorityOrder: Record<FlagPriority, number> = { urgent: 0, high: 1, medium: 2, low: 3 };
