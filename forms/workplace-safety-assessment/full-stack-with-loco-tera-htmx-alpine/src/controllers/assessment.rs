...
use crate::engine::{safety_grader, types::AssessmentData, utils};
...
/// POST /assessment/new -- create a new draft audit, redirect to the wizard.
#[debug_handler]
...
/// GET /assessment/{id} -- render the single-page audit wizard.
#[debug_handler]
...
async fn submit_assessment(
    ...
    if let Some(obj) = data_value.as_object_mut() {
        if let Some(form_obj) = form_data.as_object() {
            // Collect dynamic signoff_action_plan.action_items[idx].field rows.
            let mut action_items: std::collections::BTreeMap<
                usize,
                serde_json::Map<String, serde_json::Value>,
            > = std::collections::BTreeMap::new();

            for (key, value) in form_obj {
                // action_items[0].description -- dynamic action-item rows.
                if key.starts_with("action_items[") {
            ...
            // Materialize action_items into signoff_action_plan.actionItems.
            if !action_items.is_empty() {
                let items: Vec<serde_json::Value> = action_items
                    .values()
                    .filter(|entry| {
                        entry
                            .get("description")
                    ...
                    .collect();
                if let Some(sp) = obj
                    .get_mut("signoffActionPlan")
                    .and_then(|s| s.as_object_mut())
                {
                    sp.insert("actionItems".to_string(), serde_json::Value::Array(items));
                }
            }
        }
    }
...
async fn show_report(
    ...
    let grade = safety_grader::grade(&assessment_data);
    ...
    context.insert("outcome", &grade.outcome);
    context.insert("outcome_label", &utils::outcome_label(&grade.outcome));
    context.insert("outcome_class", &utils::outcome_class(&grade.outcome));
    context.insert(
        "action_timeframe",
        &utils::action_timeframe(&grade.outcome),
    );
    context.insert("findings_by_category", &grade.findings_by_category);
