...
impl MigrationTrait for Migration {
    async fn up(&self, m: &SchemaManager) -> Result<(), DbErr> {
        create_table(m, "blood_cross_match_test_request_grades",
            &[
            ...
            ("identity_safety_band", ColType::StringWithDefault(String::new())),
            ...
            &[
            ("blood_cross_match_test_request", ""),
            ]
        ).await
    }

    async fn down(&self, m: &SchemaManager) -> Result<(), DbErr> {
        drop_table(m, "blood_cross_match_test_request_grades").await
    }
}
