...
// Blood Cross-Match Test Request — vetting dashboard
...
// identity-safety dropdown + safety-flags dropdown).
//
// Sibling modules loaded as plain `<script>` tags (in dependency order)
// attach their exports to `window.BloodCrossMatchTestRequestDashboard`.
...
const columns = [
  ...
  { key: 'identitySafetyBand', label: 'Identity Safety' },
...
function identityClass(band) {
  return band ? 'identity-' + String(band) : '';
}
...
function matchesFilters(row) {
  if (filters.search) {
    ...
    const matches =
      ...
      String(row.requestType || '').toLowerCase().includes(term) ||
  ...
  if (filters.identity && row.identitySafetyBand !== filters.identity) return false;
...
function compareRows(a, b) {
  ...
  if (key === 'identitySafetyBand') {
    return ((identityRank[av] ?? -1) - (identityRank[bv] ?? -1)) * dir;
  }
  if (key === 'triageTier') {
  ...
  // Default: string compare (nhs, patient, requestType, component,
...
function renderTableBody() {
  ...
  for (const row of rows) {
    ...
    if (row.triageTier === 'stat' || row.identitySafetyBand === 'reject-risk') {
    ...
    tr.innerHTML = `
      ...
      <td class="data-table-td">${esc(titleCase(row.requestType))}</td>
      <td class="data-table-td">${esc(titleCase(row.component))}</td>
      ...
      <td class="data-table-td"><span class="identity-badge ${identityClass(row.identitySafetyBand)}">${esc(titleCase(row.identitySafetyBand))}</span></td>
...
function bindFilterInputs() {
  ...
  const identity = document.getElementById('filter-identity');
  ...
  if (identity) {
    identity.addEventListener('change', () => {
      filters.identity = identity.value;
      renderAll();
    });
  }
  if (flags) {
