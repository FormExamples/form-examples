<script lang="ts">
	...
	// SVAR DataGrid columns. The four-axis grade and recommendation render through
	...
	const columns = [
...

		{
			...
			width: 150,
			sort: true,
			template: (v: RequestType) => requestTypeLabel(v) || '—'
		},
		{
			...
			template: (v: number, row: any) =>
				`${v}/9 · ${appropriatenessBandLabel(row.appropriatenessBand as AppropriatenessBand)}`
		},
		{
			id: 'identitySafetyBand',
			header: 'Identity safety',
			width: 130,
			sort: true,
			template: (v: IdentitySafetyBand) => identitySafetyBandLabel(v)
		},
		{
		...
		},
		{
			...
			width: 110,
			sort: true,
			template: (v: TriageTier) => titleCase(v)
		},
		{
			id: 'recommendation',
			...
			width: 160,
			sort: true,
			template: (v: Recommendation) => recommendationLabel(v)
		},
	...
	function init(api: any) {
		gridApi = api;
		api.exec('sort-rows', { key: 'requestedDate', order: 'asc' });
		api.on('select-row', (ev: { id?: string | number }) => {
			if (ev?.id != null) goto(`/blood-cross-match-test-request/blood-cross-match-test-requests/${ev.id}`);
		});
	}
</script>
...
<main class="mx-16 px-4 py-6">
	<div class="mb-6 flex flex-wrap items-end justify-between gap-4">
		<div>
			<h1 class="text-2xl font-bold text-base-content">Transfusion-laboratory vetting dashboard</h1>
			<p class="mt-1 text-sm text-base-content/70">
				Four-axis grade and recommendation for incoming cross-match requests, computed by the shared
				engine. Select a row to open the request.
			</p>
		</div>
		<a href="/blood-cross-match-test-request/blood-cross-match-test-requests/new" class="button" data-variant="primary">New request</a>
	</div>
	...
	<div class="mb-4 flex flex-wrap gap-4">
		...
		<label class="text-sm">
			...
			<select class="select inline-block w-auto" bind:value={recommendationFilter}>
				<option value="">All</option>
				<option value="accept">Accept and process</option>
