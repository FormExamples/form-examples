...
>
	{#if reassessments.length === 0}
		<p class="mb-4 text-sm text-gray-600">No reassessments recorded yet.</p>
	{/if}

	{#each reassessments as r, idx (idx)}
		<div class="mb-4 rounded-lg border border-gray-200 bg-gray-50 p-4">
			<div class="mb-2 flex items-center justify-between">
				<h3 class="text-base font-semibold text-gray-800">Reassessment #{idx + 1}</h3>
				<button
					type="button"
					onclick={() => remove(idx)}
					class="rounded-md border border-gray-300 bg-white px-3 py-1 text-xs font-medium text-gray-700 hover:bg-gray-100"
				>
					Remove
				</button>
			</div>
	...
	<button
		...
		onclick={add}
		disabled={!canAdd}
		class="rounded-lg border border-primary bg-white px-4 py-2 text-sm font-medium text-primary transition-colors hover:bg-blue-50 disabled:cursor-not-allowed disabled:border-gray-300 disabled:text-gray-400 disabled:hover:bg-white"
	>
