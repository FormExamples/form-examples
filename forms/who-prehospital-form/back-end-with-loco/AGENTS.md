# WHO Prehospital Form — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the WHO Prehospital Form form, built with axum + Loco +
...
- [`who_prehospital_form/`](./who_prehospital_form/) — the Loco crate: `src/who_prehospital_form/` holds `app.rs`
...
`src/who_prehospital_form/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd who_prehospital_form && cargo check --all-targets && cargo test
```
