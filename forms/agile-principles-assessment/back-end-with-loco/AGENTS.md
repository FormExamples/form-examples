# Agile Principles Assessment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Agile Principles Assessment form, built with axum + Loco +
...
- [`agile_principles_assessment/`](./agile_principles_assessment/) — the Loco crate: `src/agile_principles_assessment/` holds `app.rs`
...
`/metrics`. The registered domain controllers are:
  ...
  - `agile_principles_assessment_grade_flag`
  - `agile_principles_assessment_grade_rule`
  - `respondent`

## Engine

`src/agile_principles_assessment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd agile_principles_assessment && cargo check --all-targets && cargo test
```
