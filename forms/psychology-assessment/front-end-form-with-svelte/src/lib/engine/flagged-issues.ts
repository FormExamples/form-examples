...
export function detectAdditionalFlags(
	...
	// ─── Risk-screen escalations (urgent) ────────────────────
	...
	if (data.supportAndHistory.socialSupport === 'isolated') {
		flags.push({
			...
			message: 'Patient reports social isolation - consider connection-focused interventions.',
			priority: 'medium'
		});
	}
	...
	if (data.supportAndHistory.substanceUseConcern === 'yes') {
		flags.push({
			id: 'FLAG-SUPP-002',
			category: 'Support and History',
			message: 'Substance-use concern reported - consider co-occurring disorders screening.',
			priority: 'medium'
		});
	}
