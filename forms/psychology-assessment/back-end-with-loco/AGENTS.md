# Psychology Assessment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Psychology Assessment form, built with axum + Loco +
...
- [`psychology_assessment/`](./psychology_assessment/) — the Loco crate: `src/psychology_assessment/` holds `app.rs`
...
`src/psychology_assessment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd psychology_assessment && cargo check --all-targets && cargo test
```
