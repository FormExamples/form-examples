...
use crate::engine::{grader, types::AssessmentData};
...
/// POST /assessment/new -- create a new draft fit note, redirect to the wizard.
#[debug_handler]
async fn create_new(State(ctx): State<AppContext>) -> Result<Response> {
    let item = ActiveModel::new_draft()
        .map_err(|e| Error::BadRequest(format!("Failed to create fit note: {e}")))?;
    let item = item.insert(&ctx.db).await?;
    let id = item.id;
    Ok(Redirect::to(&format!("/assessment/{id}")).into_response())
}

/// GET /assessment/{id} -- render the single-page fit-note wizard.
#[debug_handler]
    ...
    if let Some(obj) = data_value.as_object_mut() {
        if let Some(form_obj) = form_data.as_object() {
            for (key, value) in form_obj {
                // Dotted name `section.field` -- nested fields (clinician,
                // medicalPractice, patient).
                if let Some((section_raw, field_raw)) = key.split_once('.') {
                    let section = section_to_json(section_raw);
                    let field = field_to_json(field_raw);
                ...
                // Top-level (flat) field -- snake_case to camelCase.
                let field = field_to_json(key);
                obj.insert(field, parse_form_value(value));
            }
        }
    }
...
async fn show_report(
    ...
    let assessment_data: AssessmentData = item
        .assessment_data()
        .map_err(|e| Error::BadRequest(format!("Invalid fit-note data: {e}")))?;

    let grade = grader::grade_fit_note(&assessment_data);
    ...
    context.insert(
        ...
        &crate::engine::utils::recommendation_label(&grade.recommendation),
    );
