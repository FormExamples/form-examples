...
impl ActiveModel {
    /// Create a new in-progress fit-note draft with empty data.
...
impl Model {
    ...
    /// Deserialize the JSONB `result` column into Grade.
    pub fn grade(&self) -> Result<Option<Grade>, serde_json::Error> {
        match &self.result {
...
/// List fit notes with pagination (page is 0-indexed, per_page items each).
