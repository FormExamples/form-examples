...
import { emptyFitNote, type FitNote } from './types';
import { gradeFitNote } from './grader';
...
function validBase(): FitNote {
	...
	fn.medicalPractice.postalAddressAsFullText = '12 Pondside Lane, London SW1A 1AA';
...
describe('validity rules', () => {
	it('fires invalid_no_name when clinician name is missing', () => {
		...
		expect(r.safetyFlags.some((f) => f.category === 'invalid_no_name')).toBe(true);
		expect(r.firedRules.some((rule) => rule.ruleId === 'R-VALID-NAME-001')).toBe(true);
	});
	...
	it('fires invalid_no_profession when profession is missing', () => {
		...
		const r = gradeFitNote(fn);
		expect(r.safetyFlags.some((f) => f.category === 'invalid_no_profession')).toBe(true);
	});
	...
	it('fires invalid_no_practice_address when practice address is missing', () => {
		...
		const r = gradeFitNote(fn);
		expect(r.safetyFlags.some((f) => f.category === 'invalid_no_practice_address')).toBe(true);
	});

	it('passes validity when all three required fields are present', () => {
		const r = gradeFitNote(validBase());
		expect(r.isValid).toBe('yes');
		expect(r.safetyFlags.some((f) => f.category === 'invalid_no_name')).toBe(false);
	});
});
...
describe('adaptation rules', () => {
	...
	it('classifies 0 adaptations as none and fires may_be_fit_no_adaptations', () => {
		...
		expect(r.safetyFlags.some((f) => f.category === 'may_be_fit_no_adaptations')).toBe(true);
	});
...
describe('period rules', () => {
	it('classifies < 7 days as self_cert_range', () => {
		...
		expect(r.safetyFlags.some((f) => f.category === 'self_cert_range')).toBe(true);
	});
	...
	it('classifies > 4 weeks as long_term', () => {
		...
		expect(r.safetyFlags.some((f) => f.category === 'long_absence_four_weeks')).toBe(true);
	});
	...
	it('classifies > 3 months in first 6 months as exceeds_initial_max', () => {
		...
		expect(
			r.safetyFlags.some((f) => f.category === 'duration_exceeds_3_months_in_first_6_months')
		).toBe(true);
	});
...
describe('safety flags', () => {
	it('fires automatic_disability when diagnosis text contains "HIV"', () => {
		...
		const r = gradeFitNote(fn);
		expect(r.safetyFlags.some((f) => f.category === 'automatic_disability')).toBe(true);
	});
	...
	it('fires mental_health_condition when category is mental_health', () => {
		...
		const r = gradeFitNote(fn);
		expect(r.safetyFlags.some((f) => f.category === 'mental_health_condition')).toBe(true);
	});
	...
	it('fires driving_restriction_recommended when comments mention "should not drive"', () => {
		...
		fn.adaptationPhasedReturn = 'yes';
		const r = gradeFitNote(fn);
		expect(r.safetyFlags.some((f) => f.category === 'driving_restriction_recommended')).toBe(true);
	});
	...
	it('fires new_authority_hcp for nurse / OT / pharmacist / physio', () => {
		...
		const r = gradeFitNote(fn);
		expect(r.safetyFlags.some((f) => f.category === 'new_authority_hcp')).toBe(true);
	});
	...
	it('fires safeguarding_concern when flagged', () => {
		...
		const r = gradeFitNote(fn);
		expect(r.safetyFlags.some((f) => f.category === 'safeguarding_concern')).toBe(true);
	});
});
