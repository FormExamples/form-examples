 ...
 *
 * Rule IDs (R-VALID-*, R-ADAPT-*, R-PERIOD-*, R-SAFE-*) and flag categories
 * are identical across the static HTML, the SvelteKit front-end, and the Rust
 * backend so dashboards can group rows by flag category.
 */
...
function classifyPeriod(days: number | null, withinFirst6Months: boolean): PeriodCompliance {
...
/** Grade a fit note against the DWP policy matrix. */
export function gradeFitNote(fitNote: FitNote): GradingResult {
	...
	if (!fitNote.clinician.name) {
		...
		flags.push({
			...
			suggestedAction: 'Issue a corrected fit note with the clinician name printed.'
		});
	}
	if (!fitNote.clinician.profession) {
		...
		flags.push({
			...
			suggestedAction: 'Re-issue with the profession (doctor / nurse / OT / pharmacist / physio).'
		});
	}
	...
	if (fitNote.isNonMedical === 'yes') {
		...
		flags.push({
			...
			suggestedAction: 'Discuss alternative support (ACAS, careers advice, debt advice).'
		});
	}
	...
	if (fitNote.fitnessForWork === 'may_be_fit') {
		...
		firedRules.push({
			...
			description:
				'Adaptation count = ' + adaptationCount + ' → intensity ' + adaptationIntensity + '.'
		});
		...
		if (!fitNote.comments) {
			firedRules.push({
				...
				ruleSet: 'adaptation',
				severity: 'medium',
				description: 'Comments box empty when "may be fit" selected (DWP policy 5.6).'
			});
			flags.push({
				...
				suggestedAction: 'Provide practical advice about what the patient can do at work.'
			});
		}
	}
	...
	if (fitNote.conditionFirstRecordedDate && fitNote.assessmentDate) {
		...
		if (!Number.isNaN(condition.getTime()) && !Number.isNaN(assessment.getTime())) {
			const diffDays = (assessment.getTime() - condition.getTime()) / (1000 * 60 * 60 * 24);
	...
	if (periodCompliance === 'exceeds_initial_max') {
		flags.push({
			...
			suggestedAction: 'Reduce period to ≤ 3 months or document why the rule does not apply.'
		});
	}
	if (periodCompliance === 'self_cert_range') {
		flags.push({
			...
			suggestedAction: 'Recommend self-certification (employee statement of sickness).'
		});
	}
	if (periodDays !== null && periodDays > 28) {
		flags.push({
			...
			suggestedAction: 'Signpost the patient to Access to Work and the Health Adjustment Passport.'
		});
	}
	if (periodDays !== null && periodDays > 84) {
		flags.push({
			...
			suggestedAction: 'Discuss SSP, ESA, or Universal Credit eligibility with the patient.'
		});
	}
	...
	if (
		...
		firedRules.push({
			...
			ruleSet: 'safety',
			severity: 'medium',
			description: 'Diagnosis matches automatic disability (Equality Act 2010 s.6).'
		});
		flags.push({
			flagId: 'F-AUTO-DISABILITY',
			category: 'automatic_disability',
			priority: 'medium',
			description: 'Condition is automatically classed as a disability (HIV / cancer / MS).',
			suggestedAction:
	...
	if (fitNote.diagnosisCategory === 'mental_health') {
		...
		flags.push({
			...
			priority: 'medium',
			description: 'Mental-health diagnosis — additional support pathways available.',
			suggestedAction:
	...
	if (fitNote.safeguardingConcern === 'yes') {
		...
		flags.push({
			...
			suggestedAction: 'Follow local safeguarding policy; do not print details on the fit note.'
		});
	}
	if (fitNote.clinician.isPrivatePractice === 'yes') {
		...
		flags.push({
			...
			suggestedAction: 'Inform the patient that the employer may request additional evidence.'
		});
	}
	...
	if (
		...
		flags.push({
			...
			priority: 'low',
			description: 'Issued by a 2022-authorised profession (' + fitNote.clinician.profession + ').',
	...
	} else if (periodDays !== null && periodDays >= 84 && fitNote.fitnessForWork === 'not_fit') {
