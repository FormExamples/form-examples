# Nerve Conduction Study Test Result — agent instructions

Nerve conduction study / EMG (electrodiagnostic) result (report). A reporting
clinician completes a single-page wizard recording the performed study's
...
and produces a structured electrodiagnostic report.

This is the **result/report counterpart** to `nerve-conduction-study-test-request`
(a referral). It mirrors the gold template `ct-scan-test-result`; keep it clean
and faithful to the conventions below.

## Result semantics

A result form records *what the study found and what it means* — it is **not** a
referral. The source-of-truth record is `nerve_conduction_study_test_result`; the
clinician is the report **author/signer** (neurologist, clinical
neurophysiologist, other), not a referrer. The engine **interprets findings**
...
- **Axis B — severity & structured reporting:** abnormalitySeverity
  ...
  (e.g. an AANEM CTS severity grade or polyneuropathy descriptor).
...
a severe acute neuropathy such as a Guillain-Barré syndrome pattern —
