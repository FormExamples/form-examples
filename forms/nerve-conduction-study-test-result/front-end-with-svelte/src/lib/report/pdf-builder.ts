...
import type { NerveConductionStudyResult, GradingResult } from '$lib/engine/types';
import {
...
/** Builds the pdfmake document definition for the NCS / EMG electrodiagnostic report. */
export function buildPdfDocument(
	data: NerveConductionStudyResult,
	...
	return {
		...
		header: {
			text: 'NERVE CONDUCTION STUDY / EMG REPORT',
			alignment: 'center',
			margin: [0, 20, 0, 0],
			fontSize: 10,
			color: '#6b7280',
		...
		footer: (currentPage: number, pageCount: number) => ({
			text: `Page ${currentPage} of ${pageCount} | Generated ${new Date(result.gradedAt).toLocaleString()} | AANEM electrodiagnostic standards`,
			alignment: 'center',
			margin: [0, 20, 0, 0],
			fontSize: 8,
			color: '#9ca3af'
		...
		content: [
			{
				text: 'NERVE CONDUCTION STUDY TEST RESULT',
				fontSize: 18,
				bold: true,
				alignment: 'center',
			...
			{
				text: `${studyTypeLabel(data.studyType)} (${regionLabel(data.region)}) — ${reportStatusLabel(data.reportStatus)}`,
				fontSize: 12,
				alignment: 'center',
				color: '#4b5563',
				margin: [0, 0, 0, 4] as Margin
			...
			{ text: `Nerve conduction: ${data.nerveConductionFindings || 'Not specified'}`, margin: [0, 0, 0, 4] as Margin },
			{ text: `EMG: ${data.emgFindings || 'Not specified'}`, margin: [0, 0, 0, 8] as Margin },
			{
			...
			// Characterisation
			sectionHeader('Characterisation'),
			{
				ul: [
			...
			{ text: `Reporting category: ${data.reportingCategory || 'N/A'}`, margin: [0, 0, 0, 4] as Margin },
			{
...
function structuredFindingList(data: NerveConductionStudyResult): string[] {
	const list: string[] = [];
