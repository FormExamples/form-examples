...
impl MigrationTrait for Migration {
    async fn up(&self, m: &SchemaManager) -> Result<(), DbErr> {
        create_table(m, "nerve_conduction_study_test_result_grades",
            &[
            ...
            &[
            ("nerve_conduction_study_test_result", ""),
            ]
        ).await
    }

    async fn down(&self, m: &SchemaManager) -> Result<(), DbErr> {
        drop_table(m, "nerve_conduction_study_test_result_grades").await
    }
}
