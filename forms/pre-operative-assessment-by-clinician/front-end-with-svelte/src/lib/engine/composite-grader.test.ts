...
describe("calculateASA", () => {
  ...
  it("fires difficult-airway flag at Mallampati III", () => {
    ...
    const r = calculateASA(data);
    expect(
      r.additionalFlags.some((f) => f.category === "difficult-airway"),
    ).toBe(true);
  ...
  it("fires severe-cardiac flag at EF < 40%", () => {
    ...
    const r = calculateASA(data);
    expect(r.additionalFlags.some((f) => f.category === "severe-cardiac")).toBe(
      true,
    );
  });
  ...
  it("fires coagulopathy flag for INR > 1.5 off anticoagulants", () => {
    ...
    const r = calculateASA(data);
    expect(r.additionalFlags.some((f) => f.category === "coagulopathy")).toBe(
      true,
    );
  });
  ...
  it("fires severe-respiratory flag at SpO2 < 92 on room air", () => {
    ...
    const r = calculateASA(data);
    expect(
      r.additionalFlags.some((f) => f.category === "severe-respiratory"),
    ).toBe(true);
  });
  ...
  it("fires severe-frailty flag at CFS 8", () => {
    ...
    const r = calculateASA(data);
    expect(r.computedAsaGrade).toBe("IV");
    expect(r.additionalFlags.some((f) => f.category === "severe-frailty")).toBe(
      true,
    );
  });
  ...
  it("fires recent-covid-19 flag within 7 weeks", () => {
    ...
    const r = calculateASA(data);
    expect(
      r.additionalFlags.some((f) => f.category === "recent-covid-19"),
    ).toBe(true);
  });
  ...
  it("fires glp1-aspiration-risk when a GLP-1 agonist was not held and GI symptoms are present", () => {
    ...
    const r = calculateASA(data);
    expect(
      r.additionalFlags.some((f) => f.category === "glp1-aspiration-risk"),
    ).toBe(true);
  });
  ...
  it("fires sarcopenia-risk for a frail patient on a GLP-1 agonist", () => {
    ...
    const r = calculateASA(data);
    expect(
      r.additionalFlags.some((f) => f.category === "sarcopenia-risk"),
    ).toBe(true);
  });
  ...
  it("fires dehydration-aki-risk for a frail patient on a GLP-1 agonist with GI symptoms", () => {
    ...
    const r = calculateASA(data);
    expect(
      r.additionalFlags.some((f) => f.category === "dehydration-aki-risk"),
    ).toBe(true);
  });
  ...
  it("fires rebound-glycaemic-risk when a held GLP-1 agonist meets insulin-requiring diabetes", () => {
    ...
    const r = calculateASA(data);
    expect(
      r.additionalFlags.some((f) => f.category === "rebound-glycaemic-risk"),
    ).toBe(true);
  });
