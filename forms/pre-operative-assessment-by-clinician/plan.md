...
report, routes nested under `src/routes/pre-operative-assessment-by-clinician/`),
and `back-end-with-loco/` (Rust edition 2024, Loco 0.16 + axum 0.8 + SeaORM,
relational per-table schema). `bin/test-form pre-operative-assessment-by-clinician`
...
- Acceptance: every path or filename named in `index.md`, `AGENTS.md`,
  ...
  `bin/test-form pre-operative-assessment-by-clinician` still passes.
...
- Acceptance: golden vectors pass in Svelte TS, HTML JS, and Rust; `cargo
  ...
  pre-operative-assessment-by-clinician` green after the new migration.
...
bin/test-sql-apply pre-operative-assessment-by-clinician   # if sql/ changed
bin/lily-html-refactor --check pre-operative-assessment-by-clinician
bin/lily-svelte-refactor --check pre-operative-assessment-by-clinician
bin/generate-llms-txt.py --check pre-operative-assessment-by-clinician
bin/generate-changelog-and-examples.py --check pre-operative-assessment-by-clinician
```
