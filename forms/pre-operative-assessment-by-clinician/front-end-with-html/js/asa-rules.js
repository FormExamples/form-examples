...
const ASA_RULES = [
  // ASA II
  {
    ...
    fires: (d) => {
      ...
      return sbp >= 140 && sbp <= 160 && dbp >= 90 && dbp <= 100;
    }
  },
  {
    id: 'R-ASA-II-02',
  ...
  // ASA III
  {
    ...
    fires: (d) =>
      (d.vitals.systolicBp ?? 0) > 160 || (d.vitals.diastolicBp ?? 0) > 100
  },
  {
    id: 'R-ASA-III-02',
    grade: 'III',
    category: 'endocrine',
    description:
      'Poorly controlled diabetes (HbA1c > 53 mmol/mol or fasting glucose > 10 mmol/L)',
    fires: (d) =>
  ...
  },
  {
    ...
    fires: (d) =>
      d.renalHepatic.dialysisStatus === 'peritoneal' ||
      d.renalHepatic.dialysisStatus === 'haemodialysis'
  },
  {
    id: 'R-ASA-III-10',
    grade: 'III',
  ...
  },
  {
    ...
    fires: (d) =>
      ...
      // grade, not silently fall through to ASA I. See doc/asa-grading-
      // rules.md and examples/personas.json's note for the verified gap
      // this closes.
      (d.neurological.daysSinceStrokeTia ?? 0) <= 90
  },
  {
...
/** Apply all ASA rules; return the FiredRule[] in catalogue order. */
function applyAsaRules(data) {
  const fired = [];
