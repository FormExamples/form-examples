// Flagged-issue detection for HR / line-managers. Independent of the
// rule engine and the headline outcome, this module raises priority
// alerts about safety-, security-, and contract-critical offboarding
// items.
//
// Priority bands:
//   high   — Access not revoked, equipment not returned, no NDA
//            reaffirmation when confidentiality required, unauthorised
//            data download detected.
...
function detectAdditionalFlags(data) {
  ...
  if (data.accessRevocation.unauthorisedDownloadDetected === 'yes') {
    flags.push({
      ...
      message: 'Unauthorised data download detected during offboarding audit - escalate to data protection officer',
      priority: 'high'
    });
  }
  ...
  if (
    ...
    flags.push({
      ...
      message: 'Forwarding address not recorded - P60 / future correspondence may not reach employee',
      priority: 'low'
    });
  }
  ...
  if (data.referencesRecommendations.referenceConsentGiven === '') {
    flags.push({
      ...
      message: 'Employee position on references not declared (consent neither given nor declined)',
      priority: 'low'
    });
  }
