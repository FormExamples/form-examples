...
// non-Complete outcome). Mandatory rules contribute to the headline outcome
// calculation; blocker rules force an Incomplete outcome and require
// escalation. Non-mandatory items, when outstanding, downgrade the outcome
...
//                These are the IT/HR critical items: NDA reaffirmation, core
//                access revocation, equipment return, payroll, and final
//                HR/IT/manager sign-off.

import type { ValidationRule } from './types';

/** "Outstanding" helper: yes-no field is anything other than 'yes'. */
function notYes(v: string): boolean {
	return v !== 'yes';
}

/** "Outstanding" helper: yes-no-na field is anything other than 'yes' or 'na'. */
function notYesOrNa(v: string): boolean {
	return v !== 'yes' && v !== 'na';
}
...
export const validationRules: ValidationRule[] = [
	// ─── EMPLOYEE DETAILS (mandatory non-blocking) ──────────────
	{
		...
		blocker: false,
		evaluate: (d) =>
			!d.employeeDetails.firstName.trim() || !d.employeeDetails.lastName.trim()
	},
	{
		id: 'EMP-002',
		category: 'Employee Details',
