...
use crate::engine::{checklist_validator, types::AssessmentData, utils};
...
/// GET /assessment/{id} -- render the single-page offboarding wizard.
#[debug_handler]
...
/// Parse a form value string. Empty strings stay as `""`; everything else
/// is preserved verbatim (this checklist has no numeric or date fields
/// that need JSON-number coercion).
...
/// GET /assessment/{id}/report -- run the validation engine, persist the
...
async fn show_report(
    ...
    let result = checklist_validator::validate(&assessment_data);
    ...
    context.insert("outcome", &result.outcome);
    context.insert("outcome_label", &utils::outcome_label(&result.outcome));
    context.insert("completion_percent", &result.completion_percent);
    context.insert("blockers", &result.blockers);
    context.insert("fired_rules", &result.fired_rules);
    context.insert("additional_flags", &result.additional_flags);
    context.insert("mandatory_total", &result.mandatory_total);
    context.insert("mandatory_satisfied", &result.mandatory_satisfied);
