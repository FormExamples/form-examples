...
impl ActiveModel {
    /// Create a new in-progress checklist with empty data.
...
impl Model {
    ...
    /// Deserialize the JSONB `result` column into ValidationResult.
    pub fn validation_result(&self) -> Result<Option<ValidationResult>, serde_json::Error> {
        match &self.result {
...
/// Find a checklist by its UUID.
pub async fn find_by_id(db: &DatabaseConnection, id: Uuid) -> Result<Option<Model>, DbErr> {
    Entity::find_by_id(id).one(db).await
}

/// List all completed checklists (those with a validation result).
...
/// List checklists with pagination (page is 0-indexed, per_page items each).
