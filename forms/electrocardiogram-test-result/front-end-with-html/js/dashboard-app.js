...
import { abnormalitySeverityClass, abnormalitySeverityLabel, ecgTypeLabel, followUpUrgencyClass, followUpUrgencyLabel, reportStatusLabel, resultClassificationClass, resultClassificationLabel } from './types.js';

// Electrocardiogram Test Result — clinician dashboard (vanilla classic-script app).
//
...
// box + ECG-type dropdown + classification dropdown + follow-up-urgency
// dropdown). Columns mirror the SvelteKit dashboard route
// (`front-end-with-svelte/src/routes/+page.svelte`).
//
// Sibling modules loaded as plain `<script>` tags (in dependency order) attach
// their exports to `window.ElectrocardiogramTestResultDashboard`; display
// labels come from the form namespace `window.ElectrocardiogramTestResult`
...
function matchesFilters(row) {
  ...
  if (filters.ecgType && row.ecgType !== filters.ecgType) return false;
...
function compareRows(a, b) {
  ...
  // Default: string compare (id, patientName, ecgType, reportStatus,
...
function renderTableBody() {
  ...
  for (const row of rows) {
    ...
    tr.innerHTML = `
      ...
      <td>${esc(ecgTypeLabel(row.ecgType))}</td>
...
function bindFilterInputs() {
  ...
  const ecgType = document.getElementById('filter-ecg-type');
  ...
  if (ecgType) {
    ecgType.addEventListener('change', () => {
      filters.ecgType = ecgType.value;
      renderAll();
    });
  }
  if (classification) {
