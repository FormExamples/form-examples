# Electrocardiogram Test Result — agent instructions

ECG (electrocardiogram) test result (report). A reporting clinician completes a
single-page wizard recording the recorded examination's **findings** (rate,
rhythm, intervals, structured electrophysiological findings) and a structured
...
This is the **result/report counterpart** to `electrocardiogram-test-request`
(a referral).

## Result semantics

A result form records *what the test found and what it means* — it is **not** a
referral. The source-of-truth record is `electrocardiogram_test_result`; the
...
- **Axis C — report completeness:** 0–100 % of mandatory report sections
  (history, rate/rhythm, intervals, interpretation, impression).
- **Axis D — follow-up urgency:** routine / recommended / urgent / critical-alert,
  with a target timeframe and a recommended action.

A **critical finding** — ST-segment elevation (STEMI), ventricular tachycardia,
complete heart block, or markedly prolonged QTc (≥ 500 ms) — **auto-escalates**
Axis D to *critical-alert* and raises the `critical-result-alert` flag
regardless of the other axes, implying an urgent same-hour communication. Choose
