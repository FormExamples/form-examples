import type { ElectrocardiogramResult, Flag, FlagPriority } from './types';
import { hasCriticalFinding, hasAbnormalRhythm, QTC_PROLONGED_MS } from './utils';

/**
 * Detects safety-critical flags independently of the four axes. Flag
 * categories mirror
 * sql/07_create_table_electrocardiogram_test_result_grade_flag.sql.
 * Flags are returned sorted high → medium → low priority.
 */
export function detectFlags(r: ElectrocardiogramResult): Flag[] {
	const flags: Flag[] = [];
