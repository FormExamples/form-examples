import type { ElectrocardiogramResult, GradingResult, Recommendation, FiredRule } from './types';
...
/**
 * Pure four-axis interpretation engine for an ECG result.
 *
 * Computes:
 ...
 *
 * Invariant: a critical finding (ST elevation / STEMI, ventricular
 * tachycardia, complete heart block, or markedly prolonged QTc) auto-escalates
 * Axis D to critical-alert and raises the critical-result-alert flag,
 * regardless of the other axes. The least-urgent band is only chosen when no
 * rule fires.
 *
 * No side effects, no network calls, no I/O.
 */
export function calculateGrade(result: ElectrocardiogramResult): GradingResult {
