<script lang="ts">
	...
	import Step3RateRhythmIntervals from '#lib/components/steps/Step3RateRhythmIntervals.svelte';
	...
	function validate(): boolean {
		...
		if (d.ecgType === '') {
			found.push({ id: 'ecgType', message: 'ECG type is required.' });
		}
		if (d.reportStatus === '') {
	...
	function submit() {
		...
		goto('/electrocardiogram-test-result/report/view');
	}
...
<main class="mx-16 px-4 py-6">
	<h1 class="text-2xl font-bold text-gray-900">New ECG report</h1>
	...
	<Form label="Electrocardiogram test result" onsubmit={submit}>
		<Step1ReportIdentification />
		<Step2ClinicalHistory />
		<Step3RateRhythmIntervals />
		<Step4Findings />
		<Step5Interpretation />
		<Step6Impression />
