...
import type { BiopsyResult, GradingResult } from '$lib/engine/types';
import {
...
/** Builds the pdfmake document definition for the biopsy histopathology report. */
export function buildPdfDocument(
	...
	return {
		...
		header: {
			text: 'BIOPSY HISTOPATHOLOGY REPORT',
			alignment: 'center',
			margin: [0, 20, 0, 0],
			fontSize: 10,
			color: '#6b7280',
		...
		footer: (currentPage: number, pageCount: number) => ({
			text: `Page ${currentPage} of ${pageCount} | Generated ${new Date(result.gradedAt).toLocaleString()} | RCPath / TNM8 / ICCR`,
			alignment: 'center',
			margin: [0, 20, 0, 0],
			fontSize: 8,
			color: '#9ca3af'
		...
		content: [
			...
			{
				text: `${biopsySiteLabel(data.biopsySite)} — ${reportStatusLabel(data.reportStatus)}`,
				fontSize: 12,
				alignment: 'center',
				color: '#4b5563',
				margin: [0, 0, 0, 4] as Margin
			...
			sectionHeader('Specimen & procedure'),
			{
				ul: [
					`Biopsy site: ${biopsySiteLabel(data.biopsySite)}`,
					`Biopsy method: ${biopsyMethodLabel(data.biopsyMethod)}`,
					`Specimen adequacy: ${data.specimenAdequacy || 'N/A'}`
				],
				margin: [0, 0, 0, 16] as Margin
			},
			...
			// Description
			sectionHeader('Macroscopic description'),
			{ text: data.macroscopicDescription || 'Not specified', margin: [0, 0, 0, 8] as Margin },
			sectionHeader('Microscopic description'),
			{ text: data.microscopicDescription || 'Not specified', margin: [0, 0, 0, 16] as Margin },

			// Diagnosis and grading
			sectionHeader('Diagnosis & grading'),
			{ text: data.diagnosis || 'Not specified', margin: [0, 0, 0, 8] as Margin },
			{
				ul: [
					`Malignancy present: ${data.malignancyPresent ? 'Yes' : 'No'}`,
					`Tumour type: ${data.tumourType || 'N/A'}`,
					`Histological grade: ${histologicalGradeLabel(data.histologicalGrade)}`,
					`Resection margins: ${resectionMarginsLabel(data.resectionMargins)}`,
					`Lymphovascular invasion: ${data.lymphovascularInvasion ? 'Yes' : 'No'}`,
					`Immunohistochemistry: ${data.immunohistochemistry || 'N/A'}`,
					`Molecular results: ${data.molecularResults || 'N/A'}`,
					`SNOMED CT code: ${data.snomedCode || 'N/A'}`
				],
				margin: [0, 0, 0, 16] as Margin
			},
