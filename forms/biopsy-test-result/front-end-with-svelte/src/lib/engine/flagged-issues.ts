import type { BiopsyResult, Flag, FlagPriority } from './types';
import { hasCriticalFinding, hasInvolvedMargin } from './utils';
 ...
 */
export function detectFlags(r: BiopsyResult): Flag[] {
	const flags: Flag[] = [];
	...
	if (hasInvolvedMargin(r)) {
		flags.push({
			...
			suggestedAction: 'Ensure the referrer is alerted and discuss re-excision or further management at MDT.'
		});
	}
	...
	if (r.malignancyPresent && r.lymphovascularInvasion) {
		flags.push({
			...
			priority: 'medium',
			description: 'Malignancy with lymphovascular invasion is present and may warrant urgent referral.',
			suggestedAction: 'Consider urgent referral to the appropriate cancer specialist team.'
		});
	}
	...
	if (r.specimenAdequacy === 'inadequate' || r.specimenAdequacy === 'suboptimal') {
		flags.push({
			...
			description: `Specimen adequacy is ${r.specimenAdequacy}; diagnostic confidence may be reduced.`,
			suggestedAction: 'Consider repeat sampling or ancillary testing to reach a diagnostic standard.'
		});
	}
	...
	if (r.malignancyPresent && (r.histologicalGrade === '' || r.histologicalGrade === 'not-applicable')) {
		flags.push({
			...
			suggestedAction: 'Record the histological differentiation grade for the cancer dataset.'
		});
	}
