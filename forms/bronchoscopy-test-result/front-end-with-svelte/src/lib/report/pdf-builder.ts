...
/** Builds the pdfmake document definition for the bronchoscopy report. */
export function buildPdfDocument(
	data: BronchoscopyResult,
	...
	return {
		...
		header: {
			text: 'BRONCHOSCOPY PROCEDURE REPORT',
			alignment: 'center',
			margin: [0, 20, 0, 0],
			fontSize: 10,
			color: '#6b7280',
		...
		footer: (currentPage: number, pageCount: number) => ({
			text: `Page ${currentPage} of ${pageCount} | Generated ${new Date(result.gradedAt).toLocaleString()} | BTS Quality Standards for Flexible Bronchoscopy`,
			alignment: 'center',
			margin: [0, 20, 0, 0],
			fontSize: 8,
			color: '#9ca3af'
		...
		content: [
			...
			sectionHeader('Procedure'),
			{
				ul: [
					...
					`Sedation: ${data.sedationUsed || 'N/A'}`,
					`Extent examined: ${data.extentExamined || 'N/A'}`
				],
				margin: [0, 0, 0, 16] as Margin
			},
			...
			// Samples and complication
			sectionHeader('Samples & complication'),
			{
				ul: [
					`Samples taken: ${data.samplesTaken || 'None'}`,
...
function structuredFindingList(data: BronchoscopyResult): string[] {
	const list: string[] = [];
	if (data.endobronchialLesion)
		list.push(
			`Endobronchial lesion${data.lesionLocation ? ` (${data.lesionLocation})` : ''}`
		);
