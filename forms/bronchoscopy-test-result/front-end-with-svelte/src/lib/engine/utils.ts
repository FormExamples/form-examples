 ...
 */
export function hasCriticalFinding(r: BronchoscopyResult): boolean {
	return r.endobronchialLesion || r.bleeding || r.complication === 'pneumothorax';
}

/** Whether any structured abnormal finding is present. */
export function hasAnyAbnormalFinding(r: BronchoscopyResult): boolean {
	return (
...
/** Whether the report describes purulent secretions only (no other abnormal finding). */
export function hasOnlyIncidentalFinding(r: BronchoscopyResult): boolean {
	return (
		r.secretionsPurulent &&
		!r.endobronchialLesion &&
		!r.mucosalAbnormality &&
