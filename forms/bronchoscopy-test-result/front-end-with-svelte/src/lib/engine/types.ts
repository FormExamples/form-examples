// ──────────────────────────────────────────────
// Bronchoscopy Test Result — core data types
//
// Field names are camelCase (front-end serde); they mirror the snake_case
// columns in sql/04_create_table_bronchoscopy_test_result.sql and
...
export type SedationUsed = 'none' | 'local' | 'conscious' | 'deep' | 'general' | '';

/** Procedural complication. */
export type Complication = 'none' | 'bleeding' | 'pneumothorax' | 'hypoxia' | 'other' | '';
...
export interface StructuredFindings {
	endobronchialLesion: boolean;
	mucosalAbnormality: boolean;
	extrinsicCompression: boolean;
	bleeding: boolean;
	foreignBody: boolean;
	secretionsPurulent: boolean;
...
/**
 * The bronchoscopy result (report) — the source-of-truth record the four-axis
 * interpretation grade is computed from.
 */
export interface BronchoscopyResult {
	...
	// Procedure
	procedure: Procedure;
	sedationUsed: SedationUsed;
	extentExamined: string;
