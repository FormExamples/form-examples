...
// `src/lib/engine/types.ts` data model for the Bronchoscopy Test Result form.
//
// The camelCase property names mirror the snake_case SQL columns in
// `sql/04_create_table_bronchoscopy_test_result.sql` and
// `sql/05_create_table_bronchoscopy_test_result_grade.sql`. This file builds
// and exports the canonical empty BronchoscopyResult shape used by the wizard,
...
/**
 * Performed bronchoscopy procedure.
 * @typedef {'flexible-bronchoscopy' | 'ebus' | 'rigid-bronchoscopy' |
 *           'bronchoalveolar-lavage' | 'other' | ''} Procedure
 */

/**
 * Sedation actually used.
 * @typedef {'none' | 'local' | 'conscious' | 'deep' | 'general' | ''} SedationUsed
 */

/**
 * Procedural complication.
 * @typedef {'none' | 'bleeding' | 'pneumothorax' | 'hypoxia' | 'other' | ''} Complication
 */
...
/**
 * The bronchoscopy result (report) — the source-of-truth record the
 * four-axis interpretation grade is computed from. Mirrors
 * `BronchoscopyResult` in `front-end-with-svelte/src/lib/engine/types.ts`.
 *
 * @typedef {Object} BronchoscopyResult
 ...
 * @property {SedationUsed} sedationUsed
 * @property {string} extentExamined
 * @property {string} clinicalHistory
 * @property {string} findingsNarrative
 * @property {boolean} endobronchialLesion
 * @property {boolean} mucosalAbnormality
 * @property {boolean} extrinsicCompression
 * @property {boolean} bleeding
 * @property {boolean} foreignBody
 * @property {boolean} secretionsPurulent
 * @property {boolean} normalExamination
 * @property {string} lesionLocation
 * @property {string} samplesTaken
 ...
 * `sql/05_create_table_bronchoscopy_test_result_grade.sql`.
 *
...
function procedureLabel(value) {
  switch (value) {
    case 'flexible-bronchoscopy': return 'Flexible bronchoscopy';
    case 'ebus': return 'EBUS';
    case 'rigid-bronchoscopy': return 'Rigid bronchoscopy';
    case 'bronchoalveolar-lavage': return 'Bronchoalveolar lavage';
    case 'other': return 'Other';
    default: return 'Unspecified';
  }
}

/** Human-readable sedation label. */
function sedationUsedLabel(value) {
  switch (value) {
    case 'none': return 'None';
...
function complicationLabel(value) {
  switch (value) {
    ...
    case 'pneumothorax': return 'Pneumothorax';
