import type { FourATRule } from './types';

/**
 * Declarative 4AT scoring rules.
 *
 * The 4AT has four items. Items 1 and 4 contribute 0 or 4 points; items 2 and 3
 * contribute 0, 1, or 2 points. Each rule evaluates the patient data and
 * returns true when its scoring response was selected; the grader
 * (`fourat-grader.ts`) groups fired rules by item, sums the points into the
 * total 4AT score (0-12), and derives the interpretation band. Only the
 * non-zero responses have rules — a "normal" / "no mistakes" answer fires
 * nothing and contributes 0. Because each item's responses are mutually
 * exclusive, at most one rule fires per item. Rows mirror the
 * `four_a_test_for_delirium_grade_rule` SQL table.
 */
export const fouratRules: FourATRule[] = [
	...
	// ─── ITEM 3: ATTENTION — MONTHS BACKWARDS (0, 1, or 2) ─────────
	{
		...
		points: 1,
		category: 'four-at-item',
		description: 'Attention — starts months backwards but scores under 7, or refuses to start',
	...
	// ─── ITEM 4: ACUTE CHANGE OR FLUCTUATING COURSE (0 or 4) ───────
	{
		...
		category: 'four-at-item',
		description:
			'Acute change or fluctuating course present over the last 2 weeks, still evident in the last 24 hours',
