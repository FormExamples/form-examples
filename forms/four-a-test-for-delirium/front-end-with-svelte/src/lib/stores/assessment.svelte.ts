...
function storageKey(id: string): string {
	return `four-a-test-for-delirium.front-end-with-svelte.${id || 'new'}.v1`;
}

/** A blank 4AT assessment with all fields at their unanswered defaults. */
...
/**
 * Svelte 5 reactive store for the 4AT assessment, with localStorage persistence
...
class AssessmentStore {
	 ...
	 * localStorage) wins; otherwise the `seed` assessment is used (e.g. a sample
	 * for an existing id), falling back to a blank draft.
	 *
