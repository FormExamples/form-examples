# Mobility Assessment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Mobility Assessment form, built with axum + Loco +
...
- [`mobility_assessment/`](./mobility_assessment/) — the Loco crate: `src/mobility_assessment/` holds `app.rs`
...
`src/mobility_assessment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd mobility_assessment && cargo check --all-targets && cargo test
```
