<script lang="ts">
	import type { Allergy, AllergySeverity } from '#lib/engine/types.js';
...
<div class="allergy-list">
	{#each allergies as allergy, i (i)}
		<div class="allergy-row">
			...
			<button type="button" class="button" data-variant="danger" onclick={() => removeAllergy(i)} aria-label="Remove allergy">&times;</button>
		</div>
	{/each}
	<button type="button" class="button" onclick={addAllergy}>+ Add Allergy</button>
</div>

<style>
	.allergy-list { display: flex; flex-direction: column; gap: 0.5rem; }
	.allergy-row { display: grid; grid-template-columns: repeat(3, 1fr) auto; gap: 0.5rem; align-items: center; }
	@media (max-width: 640px) { .allergy-row { grid-template-columns: 1fr; } }
</style>
