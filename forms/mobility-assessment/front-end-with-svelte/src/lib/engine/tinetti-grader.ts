...
/**
 * Full mobility grade: combines the Tinetti score with the additional
 * clinician flags into a single `GradingResult`. Shared entry point used by
 * both the wizard (on submit) and the dashboard sample rows so the two stay
 * aligned.
 */
export function gradeMobility(data: AssessmentData): GradingResult {
	const { tinettiTotal, balanceScore, gaitScore, tinettiCategoryLabel, firedRules } =
		calculateTinetti(data);
