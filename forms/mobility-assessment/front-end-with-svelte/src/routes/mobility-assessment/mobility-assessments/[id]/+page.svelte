<script lang="ts">
	...
	import { gradeMobility } from '#lib/engine/tinetti-grader.js';
	...
	function submit() {
		...
		assessment.result = gradeMobility(assessment.data);
		goto(`/mobility-assessment/mobility-assessments/${id}/report`);
	}
...
<main class="mx-16 px-4 py-6">
	<h1 class="text-2xl font-bold text-base-content">
		{isNew ? 'New mobility assessment' : `Mobility assessment ${id}`}
	</h1>
	<p class="mt-1 text-sm text-base-content/70">
		Complete the ten sections; the Tinetti score and fall-risk category are computed on submit.
	</p>
	...
	<Form label="Mobility assessment" onsubmit={submit}>
		<Step1Demographics />
		<Step2ReferralInfo />
		<Step3FallHistory />
