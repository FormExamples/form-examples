// Flagged-issue detection. Independent of the Tinetti score (which the
// grader computes), this module raises clinician-facing flags for high
// fall risk, recurrent falls, abnormal TUG time, balance/gait deficits,
// polypharmacy, sedating medications, severe fear of falling, fall
// injuries, severe ROM limitations, and assistive-device gaps.
...
function detectAdditionalFlags(data) {
  ...
  const balanceTotal = balanceScores
    .filter((s) => s !== null && s !== undefined)
    .reduce((sum, s) => sum + s, 0);
  if (balanceTotal <= 10 && balanceScores.some((s) => s !== null && s !== undefined)) {
    flags.push({
      id: 'FLAG-BALANCE-001',
  ...
  const gaitTotal = gaitScores
    .filter((s) => s !== null && s !== undefined)
    .reduce((sum, s) => sum + s, 0);
  if (gaitTotal <= 8 && gaitScores.some((s) => s !== null && s !== undefined)) {
    flags.push({
      id: 'FLAG-GAIT-001',
