# Architecture Decision Record (Tyree & Akerman) — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Architecture Decision Record (Tyree & Akerman) form, built with axum + Loco +
...
- [`architecture_decision_record/`](./architecture_decision_record/) — the Loco crate: `src/architecture_decision_record/` holds `app.rs`
...
`src/architecture_decision_record/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd architecture_decision_record && cargo check --all-targets && cargo test
```
