 ...
 * @param {{ seizureControl?: string, reviewStatus?: string }} [grade]
 *        the grader's seizure-control class and review status; drives the
 *        specialist-review and incomplete flags.
...
function detectFlaggedIssues(data, grade) {
  ...
  if (woman && cb.folicAcid === 'no') {
    flags.push({
      ...
      suggestedAction:
        'Offer high-dose folic acid (5 mg) and discuss pre-conception planning.'
    });
  }

  // ─── Review incomplete / overdue (LOW) ─────────────────────
  const overdue =
    present(data.context.monthsSinceLastReview) &&
    data.context.monthsSinceLastReview > 12;
