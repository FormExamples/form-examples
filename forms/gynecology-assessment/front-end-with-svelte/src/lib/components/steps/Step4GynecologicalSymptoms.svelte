<script lang="ts">
	...
	import type { SymptomScore } from '#lib/engine/types.js';
...
<Fieldset title="Gynaecological Symptoms" description="Rate the severity of your current gynaecological symptoms">
	{#each symptomQuestions as question, i}
		<div class="mb-6 border-b border-base-200 pb-4 last:border-0 last:pb-0">
				...
				{#each symptomResponseOptions as opt}
					<label
						class="flex cursor-pointer items-center gap-2 rounded-lg border px-3 py-1.5 text-sm transition-colors
							{gs[question.key] === opt.value ? 'border-primary bg-primary/10 font-medium' : 'border-base-300 bg-base-100 hover:bg-base-200'}"
					>
						<input
							type="radio"
