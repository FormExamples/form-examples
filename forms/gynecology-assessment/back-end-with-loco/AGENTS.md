# Gynecology Assessment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Gynecology Assessment form, built with axum + Loco +
...
- [`gynecology_assessment/`](./gynecology_assessment/) — the Loco crate: `src/gynecology_assessment/` holds `app.rs`
...
`src/gynecology_assessment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd gynecology_assessment && cargo check --all-targets && cargo test
```
