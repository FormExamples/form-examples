# Fluid Balance Chart: Back-end with Rust Axum Loco (JSON API)

Server-side Rust JSON API for the Fluid Balance Chart form. Pure back-end: no HTML
rendering, no Tera templates, no HTMX, no Alpine.js, no CSS, no Lily
Design System.

The Loco crate lives in [`fluid_balance_chart/`](fluid_balance_chart/) — one migration, SeaORM
