import type { ChartData, FlaggedIssue, GradingResult } from './types';
import {
	ANURIA_ABSOLUTE_ML,
	ANURIA_MIN_HOURS,
...
/**
 * The reconciliation outputs the flag detector needs — the core grade computed
 * by the grader, before flags and timestamp are attached.
 */
export type GradingCore = Omit<GradingResult, 'flaggedIssues' | 'timestamp'>;

/**
 * Safety-flag detection. Emitted INDEPENDENTLY of the fluid-status
 * classification (spec §5). Each flag mirrors a category in the
 * `fluid_balance_chart_grade_flag` SQL table, and carries a priority:
 *
 ...
 *   - incomplete-recording      (low/medium) — missing weight (medium), no
 *                                          entries (medium), or missing volume (low)
 */
export function detectFlaggedIssues(data: ChartData, grade: GradingCore): FlaggedIssue[] {
	const flags: FlaggedIssue[] = [];
	const entries = [...(data.intake || []), ...(data.output || [])];
	...
	// ─── Oliguria (HIGH) ─────────────────────────────────────────
	if (rate !== null && grade.hoursObserved >= OLIGURIA_MIN_HOURS && rate < OLIGURIA_RATE_ML_PER_KG_PER_HOUR) {
		flags.push({
			id: 'F-OLIGURIA-001',
			category: 'oliguria',
			priority: 'high',
	...
	if (anuriaByRate || anuriaByAbsolute) {
		const parts: string[] = [];
		if (anuriaByRate && rate !== null) {
			parts.push(`urine output rate ${rate.toFixed(2)} mL/kg/h (< ${ANURIA_RATE_ML_PER_KG_PER_HOUR})`);
		}
		if (anuriaByAbsolute) {
			parts.push(`only ${grade.urineOutputMl} mL of urine over ${grade.hoursObserved} h`);
		}
		flags.push({
			id: 'F-ANURIA-001',
			category: 'anuria',
	...
	if (num(data.patient.weightKg) === null) {
		flags.push({
			...
			priority: 'medium',
			description:
				'Patient weight is missing — the urine output rate in mL/kg/h cannot be computed.',
			suggestedAction:
	...
	if (entries.length === 0) {
		flags.push({
			...
			suggestedAction: 'Record the timed intake and output volumes for the charting period.'
		});
	} else {
