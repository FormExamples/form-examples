import type { AllergySkinResult, GradingResult, Recommendation, FiredRule } from './types';
...
/**
 * Pure four-axis interpretation engine for an allergy skin test result.
 *
 * Computes:
 ...
 *
 * Invariant: anaphylaxis during the test auto-escalates Axis D to
 * critical-alert and raises the critical-result-alert flag, regardless of the
 * other axes. An invalid test classifies as inconclusive. The least-urgent band
 * is only chosen when no rule fires.
 *
 * No side effects, no network calls, no I/O.
 */
export function calculateGrade(result: AllergySkinResult): GradingResult {
