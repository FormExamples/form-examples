...
import type { AllergySkinResult, GradingResult } from '$lib/engine/types';
import {
...
/** Builds the pdfmake document definition for the allergy skin test report. */
export function buildPdfDocument(
	data: AllergySkinResult,
	...
	return {
		...
		footer: (currentPage: number, pageCount: number) => ({
			text: `Page ${currentPage} of ${pageCount} | Generated ${new Date(result.gradedAt).toLocaleString()} | BSACI / EAACI`,
			alignment: 'center',
			margin: [0, 20, 0, 0],
			fontSize: 8,
			color: '#9ca3af'
		...
		content: [
			...
			// Test and validity controls
			sectionHeader('Test and validity controls'),
			{
				ul: [
					`Test type: ${testTypeLabel(data.testType)}`,
					`Antihistamines withheld: ${data.antihistaminesWithheld ? 'Yes' : 'No'}`,
					`Positive control valid: ${data.positiveControlValid ? 'Yes' : 'No'}`
				],
				margin: [0, 0, 0, 16] as Margin
			},
			...
			sectionHeader('Allergens and measured reactions'),
			{
				ul: [
					`Allergens tested: ${data.allergensTested || 'N/A'}`,
					`Weal sizes: ${data.whealSizes || 'N/A'}`,
					`Specific-IgE results: ${data.specificIgeResults || 'N/A'}`,
					`Sensitised allergens: ${data.sensitisedAllergens || 'None'}`
				],
				margin: [0, 0, 0, 8] as Margin
			},
			{
			...
			sectionHeader('Interpretation and impression'),
			{ text: `Interpretation: ${data.interpretation || 'Not specified'}`, margin: [0, 0, 0, 4] as Margin },
...
function structuredFindingList(data: AllergySkinResult): string[] {
	const list: string[] = [];
