# Allergy Skin Test Result — agent instructions
...
This is the **result/report counterpart** to `allergy-skin-test-request` (a
referral). It mirrors the `ct-scan-test-result` gold template.

## Result semantics

A result form records *what the test found and what it means* — it is **not** a
referral. The source-of-truth record is `allergy_skin_test_result`; the
clinician is the report **author/signer** (allergist-immunologist, dermatologist,
nurse), not a referrer. The engine **interprets findings** rather than vetting a
request.
