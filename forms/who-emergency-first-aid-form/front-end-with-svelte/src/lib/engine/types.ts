...
/**
 * The combined output of the WHO Emergency First Aid engine: the completeness
 * validation plus the clinically significant flagged issues, with a few
 * derived summary counts. This is a *status / classification* form, not a
 * numeric-score instrument, so there is no overall grade — the record is
 * summarised by its completeness status and its highest outstanding flag.
 */
