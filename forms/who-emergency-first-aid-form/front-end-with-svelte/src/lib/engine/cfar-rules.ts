 ...
 * "Other" precaution details, etc.) are gated by the `applies` predicate so
 * the validator only counts a rule if the conditional branch is active.
 *
 * Rule IDs follow the pattern <SECTION>-<NN>; the prefix groups fired rules
 * by section in the report.
 */
export const cfarRules: ValidationRule[] = [
	...
	// ─── Step 5 — Major Bleeding (C) ─────────────────────
	{
		...
		description:
			'Major bleeding assessment: tick "Normal" or describe findings.',
		applies: () => true,
