...
import type { AssessmentData, CfarResult } from '#lib/engine/types.js';
import { calculateAge, priorityLabel, sectionLabel } from '#lib/engine/utils.js';

/**
 * Build the pdfmake document definition for a WHO Emergency First Aid report.
 *
 ...
 * by the flagged issues, any outstanding fields, and the patient summary.
 */
export function buildPdfDocument(data: AssessmentData, result: CfarResult): TDocumentDefinitions {
	const p = data.patientIdentification;
	const age = calculateAge(p.dateOfBirth);

	const problemParts: string[] = [];
	if (data.situation.medical) problemParts.push('Medical');
	if (data.situation.trauma) problemParts.push('Trauma');
	const problem = problemParts.join(' + ') || 'N/A';
	...
	const content: Content[] = [
		...
		sectionHeader('Patient Details'),
		{
			table: {
				widths: ['*', '*'],
				body: [
					[field('Name', p.patientName || 'N/A'), field('DOB', `${p.dateOfBirth || 'N/A'}${age ? ` (Age ${age})` : ''}`)],
					[field('Sex', p.sex || 'N/A'), field('Problem', problem)],
					[field('Pregnant', data.situation.pregnant || 'N/A'), field('Recorded by', data.responderDetails.name || 'N/A')]
				]
			},
	...
	if (data.situation.whatHappened) {
		content.push(sectionHeader('What Happened'));
		content.push({ text: data.situation.whatHappened, fontSize: 10, margin: [0, 0, 0, 16] });
	}
	...
	return {
		...
		header: {
			text: 'WHO EMERGENCY FIRST AID FORM',
			alignment: 'center',
			margin: [0, 20, 0, 0],
			fontSize: 10,
			color: '#6b7280',
