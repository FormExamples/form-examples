# WHO Emergency First Aid Form — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the WHO Emergency First Aid Form form, built with axum + Loco +
...
- [`who_emergency_first_aid_form/`](./who_emergency_first_aid_form/) — the Loco crate: `src/who_emergency_first_aid_form/` holds `app.rs`
...
`src/who_emergency_first_aid_form/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd who_emergency_first_aid_form && cargo check --all-targets && cargo test
```
