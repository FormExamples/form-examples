...
// pathway. This module exposes two pure pieces the grader (`grader.js`)
// combines:
//
 ...
 * @typedef {import('./types.js').ScreeningOutcome} ScreeningOutcome
 * @typedef {import('./types.js').OutcomeBand} OutcomeBand
 *
 * @typedef {Object} OutcomeRule
 * @property {string} id
...
const cls = (d) => d.assessment.imagingClassification;
const recalledAndAssessed = (d) =>
...
const outcomeRules = [
  ...
  // ─── Recalled, not yet assessed ───────────────────────────────
  {
    ...
    description: 'Recall for assessment before an assessment result is recorded — book assessment clinic',
    evaluate: (d) =>
      d.reading.readingOutcome === 'recall-for-assessment' &&
  ...
  // ─── Assessed: imaging classification 1–2 → routine ───────────
  {
    ...
    screeningOutcome: 'routine-recall',
    outcomeBand: 'routine',
    description: 'Assessment imaging classification 1–2 (normal/benign) — return to routine recall',
  ...
  // ─── Assessed: imaging classification 3 → short interval ──────
  {
    ...
    outcomeBand: 'assessment',
    description: 'Assessment imaging classification 3 (indeterminate) — short-interval follow-up or biopsy',
  ...
  // ─── Assessed: imaging classification 4–5 → urgent ────────────
  {
    ...
    outcomeBand: 'urgent',
    description: 'Assessment imaging classification 4–5 (suspicious/malignant) — urgent breast-clinic referral',
