import type { ScreeningData, FlaggedIssue } from './types';
import { isComplete } from './breast-screening-grader';
...
/** Detect clinician-facing safety flags, computed independently of the outcome. */
export function detectFlaggedIssues(data: ScreeningData): FlaggedIssue[] {
	...
	if (readingOutcome === 'recall-for-assessment' && assessed !== 'yes') {
		flags.push({
			...
			priority: 'medium',
			description:
				'Reading outcome is recall for assessment and the assessment clinic is not yet attended',
			suggestedAction:
				'Book the woman into an assessment clinic and record the assessment result.'
		});
	}
	...
	if (consent !== '' && consent !== 'yes') {
		flags.push({
			...
			suggestedAction:
				'Do not proceed; record the declination and offer information and a further appointment.'
		});
	}
	...
	if (!isComplete(data)) {
		flags.push({
			...
			suggestedAction:
				'Complete eligibility, consent, views, image adequacy, the reading outcome, and (after a recall) the imaging classification.'
		});
	}
