import { fetchPerformanceIndicators } from './api.js';

const TOTAL_INDICATORS = 50;

const COMPLETENESS = ['complete', 'partial'];
const COMPLETENESS_LABEL = {
  complete: 'All recorded (50/50)',
  partial: 'Partial',
};
...
function completeness(recordedCount) {
  return recordedCount >= TOTAL_INDICATORS ? 'complete' : 'partial';
}
...
function renderTiles() {
  ...
  [
    ...
    { label: 'Avg. indicators recorded', value: String(avgRecorded) + ' / ' + TOTAL_INDICATORS },
  ].forEach(function (t) {
...
function renderTable() {
  ...
  filtered.forEach(function (r) {
    ...
    tbody.appendChild(el('tr', { class: 'data-table-row band-' + (band === 'complete' ? 'clear' : 'medium') }, [
      ...
      el('td', { class: 'data-table-td' }, [r.recordedCount + ' / ' + TOTAL_INDICATORS]),
...
function renderDetail(id) {
  ...
  detail.appendChild(el('div', { class: 'detail-banner band-' + (band === 'complete' ? 'clear' : 'medium') }, [
    ...
    el('p', null, [
      el('strong', null, ['Indicators recorded: ']),
      row.recordedCount + ' / ' + TOTAL_INDICATORS,
    ]),
  ]));
  ...
  detail.appendChild(el('h3', null, ['Perspectives with unrecorded indicators']));
  if (row.categoryGaps.length === 0) {
    detail.appendChild(el('p', { class: 'detail-empty' }, ['None — every indicator was recorded for this reporting period.']));
  } else {
...
document.addEventListener('DOMContentLoaded', function () {
  fetchPerformanceIndicators().then(applyFetchResult);
