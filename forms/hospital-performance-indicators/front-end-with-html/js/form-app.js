...
//
// 6-step continuous wizard rendered in document order:
//   1      Reporting period
//   2..5   One step per Balanced Scorecard perspective (4 perspectives, 50 indicators)
//   6      Summary & sign-off
//
// Each of the 50 indicators is recorded independently as a decimal value (a
...
const STORAGE_KEY = 'hospital-performance-indicators:v1';
...
const TOTAL_STEPS = STEP_DEFINITIONS.length; // 6
...
// ----------------------------------------------------------------------
// Indicator row (numeric value input + notes input for one indicator)
// ----------------------------------------------------------------------
...
function itemNotesInput(id) {
  ...
  wrapper.innerHTML =
    '<label class="label visually-hidden" for="' + fieldId + '">Notes for indicator ' + esc(id) + '</label>' +
...
function renderItemRow(item) {
  ...
  controls.appendChild(itemValueInput(item.id, 'Value for indicator ' + item.id + ' (' + item.text + ')'));
...
function renderStep1() {
  const card = sectionCard({
    ...
    description: 'Record the hospital/site and the reporting period this Balanced Scorecard covers.',
  });
...
function renderCategoryStep(stepNumber, categoryNumber, categoryTitle) {
  const items = PERFORMANCE_INDICATORS.filter(function (it) { return it.category === categoryNumber; });
  const card = sectionCard({
    stepNumber: stepNumber,
    title: categoryNumber + '. ' + categoryTitle,
    description: items.length + (items.length === 1 ? ' indicator' : ' indicators') +
      ' — record each indicator’s numeric value, with an optional note.',
  });
...
function renderStepSummary() {
  ...
  const card = sectionCard({
    ...
    description: 'Review how many indicators have been recorded by perspective, capture overall notes, then sign off.',
  });
...
function updateProgress() {
  ...
  CATEGORIES.forEach(function (c) {
    ...
    const items = PERFORMANCE_INDICATORS.filter(function (it) { return it.category === c.number; });
  ...
  const total = PERFORMANCE_INDICATORS.length; // 50
  const percent = Math.round((answered / total) * 100);
  const bar = document.getElementById('progress');
  if (bar) bar.value = percent;
  const text = document.getElementById('progress-text');
  if (text) {
    text.textContent = answered + ' of ' + total + ' indicators recorded (' + percent + '%)';
  }
...
function updateRecordedPreview() {
  ...
  el.innerHTML =
    '<div class="alert" data-type="' + (result.recordedCount === PERFORMANCE_INDICATORS.length ? 'success' : 'info') + '">' +
      '<p><strong>' + result.recordedCount + ' of ' + PERFORMANCE_INDICATORS.length +
      ' indicators recorded</strong> across ' + CATEGORIES.length + ' perspectives.</p>' +
    '</div>' +
...
function buildCsv(result) {
  ...
  lines.push('');
  lines.push('indicatorId,category,categoryTitle,text,value,notes');
  PERFORMANCE_INDICATORS.forEach(function (item) {
...
function renderReport() {
  ...
  const bannerType = result.recordedCount === PERFORMANCE_INDICATORS.length ? 'success' : 'info';
  const bannerHtml =
    '<div class="alert" data-type="' + bannerType + '">' +
      '<p><strong>Indicators recorded: </strong>' + result.recordedCount + ' / ' + PERFORMANCE_INDICATORS.length + '</p>' +
    '</div>';
  ...
  const itemRowsHtml = PERFORMANCE_INDICATORS.map(function (item) {
    const resp = state.items[item.id];
  ...
  reportEl.innerHTML =
    '<div class="report-header">' +
      '<h2>Balanced Scorecard report</h2>' +
    ...
    bannerHtml +
    '<h3>Indicators recorded by perspective</h3>' +
    '<table class="item-table">' +
      '<thead><tr><th>#</th><th>Perspective</th><th>Recorded</th></tr></thead>' +
      '<tbody>' + categoryRowsHtml + '</tbody>' +
    '</table>' +
    '<h3>Per-indicator values</h3>' +
    '<table class="item-table">' +
      '<thead><tr>' +
        '<th>ID</th><th>Perspective</th><th>Indicator</th><th>Value</th><th>Notes</th>' +
      '</tr></thead>' +
  ...
  document.getElementById('download-csv').addEventListener('click', function () {
    downloadCsv('hospital-performance-indicators.csv', buildCsv(result));
  });
