...
/** Builds the pdfmake document definition for the CT scan radiology report. */
		...
		content: [
			...
			sectionHeader('Examination'),
			{
				ul: [
					...
					`Radiation dose (DLP): ${data.radiationDoseDlp !== null ? `${data.radiationDoseDlp} mGy.cm` : 'N/A'}`
				],
				margin: [0, 0, 0, 16] as Margin
			},
...
function structuredFindingList(data: CtScanResult): string[] {
	const list: string[] = [];
