# CT Scan Test Result — agent instructions

CT (computed tomography) scan result (report). A reporting clinician completes a
...
This is the **result/report counterpart** to `ct-scan-test-request` (a
referral), and the **gold template** for the sibling `*-test-result` forms.
Keep it clean, exemplary, and faithful to the conventions below.

## Result semantics

A result form records *what the test found and what it means* — it is **not** a
referral. The source-of-truth record is `ct_scan_test_result`; the clinician is
...
- **Axis B — severity & structured reporting:** abnormalitySeverity
  ...
  (e.g. an ACR Lung-RADS or incidental-findings category).
...
A **critical finding** (haemorrhage, large new infarct, unexpected significant
abnormality) **auto-escalates** Axis D to *critical-alert* and raises the
