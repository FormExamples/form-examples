...
// `src/lib/engine/types.ts` data model for the CT Scan Test Result form.
//
// The camelCase property names mirror the snake_case SQL columns in
// `sql/04_create_table_ct_scan_test_result.sql` and
// `sql/05_create_table_ct_scan_test_result_grade.sql`. This file builds
// and exports the canonical empty CtScanResult shape used by the wizard,
...
/**
 * Examined CT body region.
 * @typedef {'head' | 'neck' | 'chest' | 'abdomen' | 'pelvis' |
 *           'abdomen-pelvis' | 'spine' | 'ct-angiogram' | 'ct-colonography' |
 *           'whole-body' | 'extremity' | 'other' | ''} BodyRegion
 */

/**
 * Contrast actually administered.
 * @typedef {'none' | 'iv-iodinated' | 'oral' | 'both' | ''} ContrastUsed
 */
...
/**
 * The CT scan result (report) — the source-of-truth record the four-axis
 * interpretation grade is computed from. Mirrors `CtScanResult` in
 * `front-end-with-svelte/src/lib/engine/types.ts`.
 *
 * @typedef {Object} CtScanResult
 ...
 * @property {number | null} radiationDoseDlp     - mGy.cm; null when unset
 ...
 * `sql/05_create_table_ct_scan_test_result_grade.sql`.
 *
 ...
 * @type {string[]}
 */
const NUMERIC_FIELDS = ['largestLesionSizeMm', 'radiationDoseDlp'];

/**
 * Build a fresh, fully-blank CT scan test result.
 * Strings default to `''`; structured-findings booleans default to `false`;
 * numeric measurements default to `null`.
 * @returns {CtScanResult}
 */
...
function bodyRegionLabel(value) {
  switch (value) {
    ...
    case 'abdomen-pelvis': return 'Abdomen / pelvis';
    case 'spine': return 'Spine';
    case 'ct-angiogram': return 'CT angiogram';
    case 'ct-colonography': return 'CT colonography';
    case 'whole-body': return 'Whole body';
    case 'extremity': return 'Extremity';
...
function contrastUsedLabel(value) {
  switch (value) {
    case 'none': return 'None';
    case 'iv-iodinated': return 'IV iodinated';
