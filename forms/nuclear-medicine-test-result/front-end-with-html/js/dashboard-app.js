...
import { abnormalitySeverityClass, abnormalitySeverityLabel, followUpUrgencyClass, followUpUrgencyLabel, reportStatusLabel, resultClassificationClass, resultClassificationLabel, scanTypeLabel } from './types.js';

// Nuclear Medicine Test Result — clinician dashboard (vanilla classic-script app).
//
...
// box + scan-type dropdown + classification dropdown + follow-up-urgency
// dropdown). Columns mirror the SvelteKit dashboard's `ReportRow`
// (`front-end-with-svelte/src/lib/engine/types.ts`).
//
// Sibling modules loaded as plain `<script>` tags (in dependency order) attach
// their exports to `window.NuclearMedicineTestResultDashboard`; display labels
// come from the form namespace `window.NuclearMedicineTestResult`
...
function matchesFilters(row) {
  ...
  if (filters.scanType && row.scanType !== filters.scanType) return false;
...
function compareRows(a, b) {
  ...
  // Default: string compare (id, patientName, scanType, reportStatus,
...
function renderTableBody() {
  ...
  for (const row of rows) {
    ...
    tr.innerHTML = `
      ...
      <td>${esc(scanTypeLabel(row.scanType))}</td>
...
function bindFilterInputs() {
  ...
  const scanType = document.getElementById('filter-scan-type');
  ...
  if (scanType) {
    scanType.addEventListener('change', () => {
      filters.scanType = scanType.value;
      renderAll();
    });
  }
  if (classification) {
