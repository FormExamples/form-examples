...
/** Builds the pdfmake document definition for the nuclear medicine report. */
export function buildPdfDocument(
	data: NuclearMedicineResult,
	...
	return {
		...
		footer: (currentPage: number, pageCount: number) => ({
			text: `Page ${currentPage} of ${pageCount} | Generated ${new Date(result.gradedAt).toLocaleString()} | RCR / IR(ME)R 2017`,
			alignment: 'center',
			margin: [0, 20, 0, 0],
			fontSize: 8,
			color: '#9ca3af'
		...
		content: [
			{
				text: 'NUCLEAR MEDICINE TEST RESULT',
				fontSize: 18,
				bold: true,
				alignment: 'center',
			...
			{
				text: `${scanTypeLabel(data.scanType)} — ${reportStatusLabel(data.reportStatus)}`,
				fontSize: 12,
				alignment: 'center',
				color: '#4b5563',
				margin: [0, 0, 0, 4] as Margin
			...
			sectionHeader('Examination'),
			{
				ul: [
					...
					`Radiopharmaceutical: ${data.radiopharmaceutical || 'N/A'}`,
					`Injected activity: ${data.injectedActivityMbq !== null ? `${data.injectedActivityMbq} MBq` : 'N/A'}`,
...
function structuredFindingList(data: NuclearMedicineResult): string[] {
	const list: string[] = [];
...
function measurementList(data: NuclearMedicineResult): string[] {
	const list: string[] = [];
	if (data.ejectionFractionPercent !== null)
		list.push(`Ejection fraction: ${data.ejectionFractionPercent}%`);
	if (data.splitFunctionLeftPercent !== null)
		list.push(`Split function (left): ${data.splitFunctionLeftPercent}%`);
	if (data.splitFunctionRightPercent !== null)
		list.push(`Split function (right): ${data.splitFunctionRightPercent}%`);
	if (list.length === 0) list.push('No quantitative measurements recorded');
	return list;
}
