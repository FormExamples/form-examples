# Orthopaedics Waiting List Card — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Orthopaedics Waiting List Card form, built with axum + Loco +
...
- [`orthopaedics_waiting_list_card/`](./orthopaedics_waiting_list_card/) — the Loco crate: `src/orthopaedics_waiting_list_card/` holds `app.rs`
...
`/metrics`. The registered domain controllers are:
  ...
  - `orthopaedics_waiting_list_card_appointment`
  - `orthopaedics_waiting_list_card_grade`
  - `orthopaedics_waiting_list_card_grade_flag`
  - `orthopaedics_waiting_list_card_grade_rule`
  - `patient`
  - `practitioner`

## Engine

`src/orthopaedics_waiting_list_card/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd orthopaedics_waiting_list_card && cargo check --all-targets && cargo test
```
