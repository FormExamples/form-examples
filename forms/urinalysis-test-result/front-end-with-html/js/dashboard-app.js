...
import { abnormalitySeverityClass, abnormalitySeverityLabel, followUpUrgencyClass, followUpUrgencyLabel, reportStatusLabel, resultClassificationClass, resultClassificationLabel, specimenTypeLabel } from './types.js';

// Urinalysis Test Result — clinician dashboard (vanilla classic-script app).
//
...
// box + specimen-type dropdown + classification dropdown + follow-up-urgency
// dropdown). Columns mirror the SvelteKit dashboard route
// (`front-end-with-svelte/src/routes/+page.svelte`).
//
// Sibling modules loaded as plain `<script>` tags (in dependency order) attach
// their exports to `window.UrinalysisTestResultDashboard`; display labels come
// from the form namespace `window.UrinalysisTestResult` (js/types.js). The
...
function matchesFilters(row) {
  ...
  if (filters.specimenType && row.specimenType !== filters.specimenType) {
    return false;
  }
...
function compareRows(a, b) {
  ...
  // Default: string compare (id, patientName, specimenType, reportStatus,
...
function renderTableBody() {
  ...
  for (const row of rows) {
    ...
    tr.innerHTML = `
      ...
      <td>${esc(specimenTypeLabel(row.specimenType))}</td>
...
function bindFilterInputs() {
  ...
  const specimenType = document.getElementById('filter-specimen-type');
  ...
  if (specimenType) {
    specimenType.addEventListener('change', () => {
      filters.specimenType = specimenType.value;
      renderAll();
    });
  }
  if (classification) {
  ...
  if (clearBtn) {
    clearBtn.addEventListener('click', () => {
      ...
      if (specimenType) specimenType.value = '';
