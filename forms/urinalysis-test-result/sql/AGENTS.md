...
4. `03_create_table_clinician.sql` — reporting clinician (biomedical scientist / microbiologist / reporting clinician / GP).
5. `04_create_table_urinalysis_test_result.sql` — main result/report record (specimen, dipstick, microscopy, culture, interpretation).
6. `05_create_table_urinalysis_test_result_grade.sql` — four-axis interpretation grade.
7. `06_create_table_urinalysis_test_result_grade_rule.sql` — fired-rule audit trail.
8. `07_create_table_urinalysis_test_result_grade_flag.sql` — safety-critical flags.
...
date / time (e.g. `ph`, `specific_gravity`, `performed_date`, `reported_date`).
