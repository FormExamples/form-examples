<script lang="ts">
	...
	import Step3RequestedStudy from '#lib/components/steps/Step3RequestedStudy.svelte';
	import Step4IndicationAndQuestion from '#lib/components/steps/Step4IndicationAndQuestion.svelte';
	import Step5Symptoms from '#lib/components/steps/Step5Symptoms.svelte';
	import Step6Safety from '#lib/components/steps/Step6Safety.svelte';
	...
	function validate(): boolean {
		...
		if (d.study.studyType === '') {
			found.push({ id: 'studyType', message: 'Requested study type is required.' });
		}
	...
	function submit() {
		...
		goto(`/nerve-conduction-study-test-request/nerve-conduction-study-test-requests/${id}/report`);
	}
...
<main class="mx-16 px-4 py-6">
	<h1 class="text-2xl font-bold text-base-content">
		{isNew ? 'New nerve conduction study request' : `Nerve conduction study request ${id}`}
	</h1>
	...
	<Form label="Nerve conduction study test request" onsubmit={submit}>
		<Step1RequestingClinician />
		<Step2PatientIdentification />
		<Step3RequestedStudy />
		<Step4IndicationAndQuestion />
		<Step5Symptoms />
		<Step6Safety />
