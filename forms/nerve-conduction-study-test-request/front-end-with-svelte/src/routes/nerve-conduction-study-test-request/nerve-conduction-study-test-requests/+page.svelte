<script lang="ts">
	...
	const columns = [
		...
		{ id: 'region', header: 'Region', width: 120, template: (v: string) => regionLabel(v as never) },
		{
			id: 'indication',
			header: 'Indication',
			flexgrow: 2,
		...
		{
			id: 'proceduralRiskBand',
			header: 'Risk',
			width: 110,
			template: (v: string) => proceduralRiskLabel(v as never)
		},
	...
	function init(api: any) {
		...
		api.on('select-row', (ev: { id?: string | number }) => {
			if (ev?.id != null) goto(`/nerve-conduction-study-test-request/nerve-conduction-study-test-requests/${ev.id}`);
		});
	}
</script>
...
<main class="mx-auto max-w-6xl px-4 py-6">
	<div class="mb-6 flex flex-wrap items-end justify-between gap-4">
		<div>
			<h1 class="text-2xl font-bold text-base-content">Neurophysiology vetting dashboard</h1>
			<p class="mt-1 text-sm text-base-content/70">
				Four-axis vetting grades for incoming nerve conduction study / EMG requests, computed by the
				shared engine. Select a row to open the request.
			</p>
		</div>
		<a href="/nerve-conduction-study-test-request/nerve-conduction-study-test-requests/new" class="button" data-variant="primary">New request</a>
	</div>
