...
// Re-export so consumers can import the blank-record factory from the store.
export { createDefaultRequest };

/** localStorage draft key for a given request id (defaults to `new`). */
function storageKey(id: string): string {
	return `nerve-conduction-study-test-request.front-end-with-svelte.${id || 'new'}.v1`;
}
...
/**
 * Svelte 5 reactive store for the nerve conduction study request, with
 ...
 */
class RequestStore {
	data = $state<NerveConductionStudyRequest>(createDefaultRequest());
	 ...
	 * `const c = request.data.clinician`) stay bound to live state.
	 */
	loadForId(id: string, seed?: NerveConductionStudyRequest) {
		const key = id || 'new';
		this.id = key;
		this.result = null;
		this.currentStep = 1;

		let draft: NerveConductionStudyRequest | null = null;
		if (browser) {
			const raw = localStorage.getItem(storageKey(key));
			if (raw) {
				try {
					draft = JSON.parse(raw) as NerveConductionStudyRequest;
				} catch {
