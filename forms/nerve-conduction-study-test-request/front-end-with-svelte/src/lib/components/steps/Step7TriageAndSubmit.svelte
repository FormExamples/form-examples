...
<Fieldset legend="7. Triage and Submit">
	<p class="hint">
		Requested urgency, setting, and notes. The live four-axis vetting grade, flags, and overall
		recommendation update as you complete the request.
	</p>
	...
	{#if preview.triageTier === 'urgent'}
		<Alert type="warning" heading="Urgent triage">
			<p>
				...
				weakness); this request is triaged urgent.
			</p>
		</Alert>
	{/if}
	...
	<div class="my-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
		...
		<div class="rounded-lg border border-base-300 p-3">
			<div class="mb-1 text-xs font-semibold text-base-content/60">Axis B — Procedural risk</div>
			<Badge
				label={proceduralRiskLabel(preview.proceduralRiskBand)}
				color={proceduralRiskColor(preview.proceduralRiskBand)}
			/>
		</div>
		<div class="rounded-lg border border-base-300 p-3">
