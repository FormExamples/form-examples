...
impl MigrationTrait for Migration {
    async fn up(&self, m: &SchemaManager) -> Result<(), DbErr> {
        create_table(m, "nerve_conduction_study_test_request_grades",
            &[
            ...
            ("procedural_risk_band", ColType::StringWithDefault(String::new())),
            ...
            &[
            ("nerve_conduction_study_test_request", ""),
            ]
        ).await
    }

    async fn down(&self, m: &SchemaManager) -> Result<(), DbErr> {
        drop_table(m, "nerve_conduction_study_test_request_grades").await
    }
}
