...
// Nerve Conduction Study Test Request — vetting dashboard
...
// procedural-risk dropdown + safety-flags dropdown).
//
// Sibling modules loaded as plain `<script>` tags (in dependency order)
// attach their exports to `window.NerveConductionStudyTestRequestDashboard`.
...
const columns = [
  ...
  { key: 'proceduralRiskBand',  label: 'Risk' },
...
function matchesFilters(row) {
  ...
  if (filters.risk && row.proceduralRiskBand !== filters.risk) return false;
...
function compareRows(a, b) {
  ...
  // Default: string compare (nhs, patient, studyType, region, indication,
...
function renderTableBody() {
  ...
  for (const row of rows) {
    ...
    if (row.triageTier === 'urgent' || row.proceduralRiskBand === 'high') {
    ...
    tr.innerHTML = `
      ...
      <td class="data-table-td">${esc(titleCase(row.region))}</td>
      ...
      <td class="data-table-td"><span class="risk-badge ${riskClass(row.proceduralRiskBand)}">${esc(titleCase(row.proceduralRiskBand))}</span></td>
