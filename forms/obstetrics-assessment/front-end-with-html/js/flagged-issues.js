// Flagged-issue detection. Independent of the NG201 risk stratification,
// this module raises clinician-facing flags for safeguarding, mental
// health, screening abnormalities, fetal concerns, prophylaxis indications,
// and care-pathway escalations.
...
function detectAdditionalFlags(data) {
  ...
  if (data.medicalHistory.previousVte === 'yes' ||
      data.medicalHistory.thrombophilia === 'yes') {
    flags.push({
      id: 'FLAG-VTE-001',
  ...
  if (data.maternalDemographics.bmi != null && data.maternalDemographics.bmi >= 35) {
    flags.push({
      id: 'FLAG-BMI-001',
      category: 'Body Mass Index',
      message: `BMI ${data.maternalDemographics.bmi} - obstetric anaesthetic review and VTE / GDM screening.`,
      priority: 'high'
    });
  } else if (data.maternalDemographics.bmi != null && data.maternalDemographics.bmi >= 30) {
    flags.push({
      id: 'FLAG-BMI-002',
      category: 'Body Mass Index',
      message: `BMI ${data.maternalDemographics.bmi} - schedule GDM screening and discuss healthy weight.`,
      priority: 'medium'
    });
  } else if (data.maternalDemographics.bmi != null && data.maternalDemographics.bmi < 18.5) {
    flags.push({
      id: 'FLAG-BMI-003',
      category: 'Body Mass Index',
      message: `BMI ${data.maternalDemographics.bmi} - dietitian referral and growth surveillance.`,
      priority: 'medium'
    });
  }

  if (data.maternalDemographics.ageAtBooking != null &&
      data.maternalDemographics.ageAtBooking >= 40) {
    flags.push({
      id: 'FLAG-AGE-001',
  ...
  if (data.maternalDemographics.ageAtBooking != null &&
      data.maternalDemographics.ageAtBooking < 18) {
    flags.push({
      id: 'FLAG-AGE-002',
      category: 'Maternal Age',
  ...
  if (data.lifestyleSocialFactors.substanceUse === 'occasional' ||
  ...
  if (data.mentalHealthAssessment.whooley1 === 'yes' &&
