# Obstetrics Assessment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Obstetrics Assessment form, built with axum + Loco +
...
- [`obstetrics_assessment/`](./obstetrics_assessment/) — the Loco crate: `src/obstetrics_assessment/` holds `app.rs`
...
`src/obstetrics_assessment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd obstetrics_assessment && cargo check --all-targets && cargo test
```
