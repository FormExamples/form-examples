<script lang="ts">
	...
	const columns = [
		...
		{ id: 'carePathway', header: 'Care pathway', width: 150, sort: true },
		{
			id: 'safeguardingFlag',
			header: 'Safeguarding',
			width: 120,
	...
	function init(api: any) {
		...
		api.on('select-row', (ev: { id?: string | number }) => {
			if (ev?.id != null) goto(`/obstetrics-assessments/${ev.id}`);
		});
	}
</script>
...
<main class="mx-auto max-w-6xl px-4 py-6">
	<div class="mb-6 flex flex-wrap items-end justify-between gap-4">
		...
		<a href="/obstetrics-assessments/new" class="button" data-variant="primary">New assessment</a>
	</div>
