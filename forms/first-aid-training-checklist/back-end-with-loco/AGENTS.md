# First Aid Training Checklist — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the First Aid Training Checklist form, built with axum + Loco +
...
- [`first_aid_training_checklist/`](./first_aid_training_checklist/) — the Loco crate: `src/first_aid_training_checklist/` holds `app.rs`
...
`src/first_aid_training_checklist/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd first_aid_training_checklist && cargo check --all-targets && cargo test
```
