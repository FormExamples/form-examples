...
/// Convert form section name (snake_case) to JSON key (camelCase) for the
/// AssessmentData JSON blob.
...
/// Convert form field name (snake_case) to JSON key (camelCase). Handles
/// the one quirk where `recognised_stroke_fast` becomes `recognisedStrokeFAST`.
fn field_to_json(field: &str) -> String {
    // Special-case the FAST acronym that the schema persists upper-cased.
    if field == "recognised_stroke_fast" {
        return "recognisedStrokeFAST".to_string();
    }
...
async fn show_report(
    ...
    context.insert(
        "outcome_label",
        &crate::engine::utils::outcome_label(&grade.outcome),
    );
    context.insert(
        "outcome_class",
        &crate::engine::utils::outcome_class(&grade.outcome),
    );
