...
async fn dashboard(
    ...
    // Server-side search across trainee name, ID, examiner.
    if let Some(ref term) = params.search {
        let term = term.to_lowercase();
        items.retain(|row| {
            row.trainee_name.to_lowercase().contains(&term)
                || row.trainee_id.to_lowercase().contains(&term)
                || row.examiner_name.to_lowercase().contains(&term)
                || row.venue.to_lowercase().contains(&term)
        });
    }
    ...
    if let Some(ref role) = params.role {
        if !role.is_empty() {
            items.retain(|row| row.role == *role);
        }
    }
