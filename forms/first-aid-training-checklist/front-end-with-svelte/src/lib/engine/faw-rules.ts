import type { AssessmentData, FawRule, TriState } from './types';

/**
 * First Aid at Work (FAW) competency rules (declarative).
 *
 ...
 * `critical: true` forces an overall Fail. Non-critical 'no' answers count as
 * deficiencies; 1-2 deficiencies → Needs Development; 3 or more → Fail.
 */
...
function read(d: AssessmentData, section: keyof AssessmentData, field: string): TriState {
	const sec = d[section] as unknown as Record<string, unknown>;
	const v = sec && sec[field];
	if (v === 'yes' || v === 'no' || v === 'na') return v;
	return '';
}

export const fawRules: FawRule[] = [
	...
	// Step 10 (Recording, Reporting & Handover) items are documentation rather
	// than life-saving competencies, so — matching the HTML reference engine —
	// they are collected but excluded from the pass/fail grading. They are
	// surfaced in the report and as debrief flags via flagged-issues.
];

/** Registry of critical-skill rule IDs (any 'no' → overall Fail). */
export const CRITICAL_RULE_IDS: string[] = fawRules.filter((r) => r.critical).map((r) => r.id);
