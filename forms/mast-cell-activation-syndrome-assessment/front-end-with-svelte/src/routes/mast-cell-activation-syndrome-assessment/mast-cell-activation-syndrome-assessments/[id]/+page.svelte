<script lang="ts">
	...
	import { calculateMCASGrade } from '#lib/engine/symptom-grader.js';
	...
	function submit() {
		...
		assessment.result = calculateMCASGrade(assessment.data);
		goto(`/mast-cell-activation-syndrome-assessment/mast-cell-activation-syndrome-assessments/${id}/report`);
	}
...
<main class="mx-16 px-4 py-6">
	<h1 class="text-2xl font-bold text-base-content">
		{isNew ? 'New MCAS assessment' : `MCAS assessment ${id}`}
	</h1>
	<p class="mt-1 text-sm text-base-content/70">
		Complete the ten sections; the MCAS Symptom Score and organ-system burden are computed on
		submit.
	</p>
	...
	<Form label="MCAS assessment" onsubmit={submit}>
		<Step1Demographics />
