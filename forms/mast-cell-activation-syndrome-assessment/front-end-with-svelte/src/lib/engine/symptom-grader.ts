import type { AssessmentData, FiredRule, GradingResult, SymptomDetail } from './types';
...
/**
 * Full MCAS grading entry point: runs the symptom score and the safety-flag
 * detector and assembles the complete {@link GradingResult} the report and the
 * dashboard consume.
 */
export function calculateMCASGrade(data: AssessmentData): GradingResult {
	const { symptomScore, mcasCategoryLabel, organSystemsAffected, firedRules } =
		calculateMCASScore(data);
	return {
		symptomScore,
