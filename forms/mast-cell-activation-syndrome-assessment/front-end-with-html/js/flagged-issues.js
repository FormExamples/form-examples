// Flagged-issue detection. Mirrors the SvelteKit `flagged-issues.ts`
// reference. Independent of the MCAS symptom score, this module raises
// safety-critical or clinically significant alerts: anaphylaxis history,
// elevated tryptase / histamine / prostaglandin D2, cardiovascular
// instability, respiratory compromise, severe GI involvement, multi-system
// involvement, and missing epinephrine prescription.
...
function detectAdditionalFlags(data) {
  ...
  if (
    ...
    flags.push({
      ...
      message:
        'History of syncope with hypotension - assess for anaphylaxis risk and ensure emergency action plan',
      priority: 'high'
    });
  }
  ...
  if (
    ...
    flags.push({
      ...
      message:
        'Significant tachycardia reported - monitor for cardiovascular compromise during mast cell episodes',
      priority: 'high'
    });
  }
  ...
  if (
    ...
    flags.push({
      ...
      category: 'Cardiovascular Instability',
      message:
        'Significant hypotension reported - ensure patient has access to emergency interventions',
      priority: 'high'
    });
  }
  ...
  if (
    ...
    flags.push({
      ...
      message:
        'Significant throat tightening reported - assess for laryngeal oedema risk',
      priority: 'high'
    });
  }
  ...
  if (
    ...
    flags.push({
      ...
      category: 'Respiratory Compromise',
      message:
        'Combined wheezing and dyspnea - evaluate for bronchospasm management plan',
      priority: 'high'
    });
  }
  ...
  if (data.currentTreatment.epinephrine === 'no') {
    const hasSevereSymptoms =
      ...
      (data.respiratorySymptoms.throatTightening.severity !== null &&
        data.respiratorySymptoms.throatTightening.severity >= 1) ||
      (data.cardiovascularSymptoms.hypotension.severity !== null &&
        data.cardiovascularSymptoms.hypotension.severity >= 2);
    ...
    if (hasSevereSymptoms) {
      flags.push({
        ...
        message:
          'Patient has cardiovascular or respiratory symptoms but no epinephrine auto-injector prescribed - consider prescribing',
        priority: 'high'
      });
    }
  }
