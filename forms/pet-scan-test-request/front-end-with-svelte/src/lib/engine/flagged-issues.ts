import type { PetScanRequest, Flag, FlagPriority, RadiationDoseBand } from './types';
import { isFdgStudy, GLUCOSE_UNCONTROLLED_THRESHOLD } from './safety-rules';

/**
 * Detects safety-critical flags independently of the four axes. Flag categories
 * mirror the sql grade_flag CHECK constraint: pregnancy, breastfeeding,
 * uncontrolled-glucose, high-radiation-dose, missing-indication,
 * missing-clinical-question, missing-glucose, other. Flags are returned sorted
 * high → medium → low priority. Flag IDs are stable across every front-end and
 * the back-end.
 */
export function detectFlags(
	...
	if (prep.pregnancyStatus === 'pregnant') {
		flags.push({
			...
			suggestedAction:
				'Do not expose unless justified by exception; discuss alternatives with the nuclear-medicine physician and ARSAC holder.'
		});
	...
	if (prep.breastfeeding === true) {
		flags.push({
			...
			suggestedAction:
				'Advise interruption of breastfeeding and close-contact precautions per local radiation-protection protocol.'
		});
	}
	...
	if (fdg) {
		...
		if (g === null || g === undefined) {
			flags.push({
				...
				suggestedAction:
					'Measure and document blood glucose before tracer injection; FDG uptake depends on glucose control.'
			});
			...
			flags.push({
				...
				suggestedAction:
					'Recheck glucose and reschedule if it remains above ~11 mmol/L; impaired FDG uptake degrades the study.'
			});
		}
	}
	...
	if (ctx.radiationDoseBand === 'high') {
		flags.push({
			...
			suggestedAction:
				'Confirm the IR(ME)R justification documents that the benefit outweighs the dose.'
		});
	}
	...
	if (data.justification.irMeRJustification.trim() === '') {
		flags.push({
			...
			suggestedAction:
				'Record the IR(ME)R justification supporting the radiation exposure before booking.'
		});
	}
