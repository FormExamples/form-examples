...
function storageKey(id: string): string {
	return `pet-scan-test-request.front-end-with-svelte.${id || 'new'}.v1`;
}

/**
 * Svelte 5 reactive store for the PET-CT scan request, with localStorage
 ...
 */
class RequestStore {
	data = $state<PetScanRequest>(createDefault());
	 ...
	 * (e.g. `const d = request.data.preparation`) stay bound to live state.
	 */
	loadForId(id: string, seed?: PetScanRequest) {
		const key = id || 'new';
		this.id = key;
		this.result = null;
		this.currentStep = 1;

		let draft: PetScanRequest | null = null;
		if (browser) {
			const raw = localStorage.getItem(storageKey(key));
			if (raw) {
				try {
					draft = JSON.parse(raw) as PetScanRequest;
				} catch {
