# PET Scan Test Request — agent instructions

PET-CT scan request (referral). A clinician completes a single-page wizard; the
engine computes a **four-axis grading** (appropriateness, preparation safety &
radiation dose, request completeness, triage priority) plus safety flags, and
...
`{ appropriatenessScore (1–9) + appropriatenessBand, prepSafetyBand,
...
front-end and the back-end. Pregnancy and uncontrolled glucose (blood glucose
above ~11 mmol/L for an FDG study) force the caution / contraindicated safety
band and raise a flag. Choose the least-alarming band only when no rule fires.
