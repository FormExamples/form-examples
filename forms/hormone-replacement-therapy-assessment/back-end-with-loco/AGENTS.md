# Hormone Replacement Therapy Assessment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Hormone Replacement Therapy Assessment form, built with axum + Loco +
...
- [`hormone_replacement_therapy_assessment/`](./hormone_replacement_therapy_assessment/) — the Loco crate: `src/hormone_replacement_therapy_assessment/` holds `app.rs`
...
`src/hormone_replacement_therapy_assessment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd hormone_replacement_therapy_assessment && cargo check --all-targets && cargo test
```
