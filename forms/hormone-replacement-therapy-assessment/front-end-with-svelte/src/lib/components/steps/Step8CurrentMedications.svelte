...
<Fieldset title="Current Medications" description="HRT history and other medications">
	...
	<h3 class="mb-3 mt-6 font-semibold text-base-content/80">Other Medications</h3>
	<MedicationEntry bind:medications={m.otherMedications} />
	{#if m.otherMedications.length === 0}
		<p class="mt-3 text-sm text-base-content/60">No other medications added. Click the button above to add one, or proceed to next step if you take none.</p>
	{/if}
