<script lang="ts">
	...
	const t = assessment.data.arrivalTriage;
</script>
...
<Fieldset title="Arrival & Triage" description="Attendance details and Manchester Triage System assessment">
	...
	<RadioGroup
		...
		bind:value={t.attendanceCategory}
		options={[
	...
	{#if t.arrivalMode === 'ambulance'}
		<TextInput label="Ambulance Incident Number" name="ambulanceIncidentNumber" bind:value={t.ambulanceIncidentNumber} />
	{/if}

	<hr class="my-6 border-base-300" />

	<h3 class="mb-3 text-lg font-semibold text-base-content">Triage</h3>
	<div class="grid grid-cols-1 gap-x-4 sm:grid-cols-2">
