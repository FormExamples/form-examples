...
<Fieldset title="Investigations" description="Blood tests, imaging, ECG, and other investigations">
	...
	<h3 class="mb-3 text-lg font-semibold text-base-content">Imaging</h3>
	<div class="space-y-3">
		{#each inv.imaging as img, i (i)}
			<div class="flex items-start gap-2 rounded-lg border border-base-300 bg-base-200 p-3">
				<div class="grid flex-1 grid-cols-1 gap-2 sm:grid-cols-3">
					<input type="text" placeholder="Type (e.g. X-ray, CT)" bind:value={img.type} class="rounded border border-base-300 px-2 py-1.5 text-sm focus:border-primary focus:outline-none" />
					<input type="text" placeholder="Site" bind:value={img.site} class="rounded border border-base-300 px-2 py-1.5 text-sm focus:border-primary focus:outline-none" />
					<input type="text" placeholder="Findings" bind:value={img.findings} class="rounded border border-base-300 px-2 py-1.5 text-sm focus:border-primary focus:outline-none" />
				</div>
				<button type="button" onclick={() => removeImaging(i)} class="mt-1 text-error hover:text-error" aria-label="Remove imaging">&times;</button>
			</div>
		{/each}
		<button type="button" onclick={addImaging} class="rounded-lg border-2 border-dashed border-base-300 px-4 py-2 text-sm text-base-content/70 transition-colors hover:border-primary hover:text-primary">
			+ Add Imaging
		</button>
	</div>
