...
/**
 * Flagged-issue detection. Independent of the MUST score, this raises
 * clinician-facing flags for severe underweight, significant weight loss,
 * swallowing difficulty / choking, poor intake, GI symptoms, food allergies /
 * anaphylaxis, dehydration, dependence on parenteral nutrition, and missing
 * dietician review.
 */
export function detectAdditionalFlags(data: AssessmentData): AdditionalFlag[] {
	...
	if (diet.foodIntakeReduced === 'yes' && diet.reducedIntakeDays !== null && diet.reducedIntakeDays > 5) {
		flags.push({
			id: 'FLAG-INTAKE-002',
			category: 'Dietary Intake',
	...
	// ─── Alcohol alerts ───────────────────────────────────────
	if (diet.alcoholUse === 'yes' && diet.alcoholUnitsPerWeek !== null && diet.alcoholUnitsPerWeek > 14) {
		flags.push({
			id: 'FLAG-ALC-001',
			category: 'Lifestyle',
