# Nutrition Assessment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Nutrition Assessment form, built with axum + Loco +
...
- [`nutrition_assessment/`](./nutrition_assessment/) — the Loco crate: `src/nutrition_assessment/` holds `app.rs`
...
`src/nutrition_assessment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd nutrition_assessment && cargo check --all-targets && cargo test
```
