<script lang="ts">
	...
	$effect(() => {
		if (!store.result) {
			goto(`/craniofacial-surgery-waiting-list-card/craniofacial-surgery-waiting-list-cards/${id}`);
		}
	});
	...
	async function downloadPDF() {
		pdfError = '';
		try {
			const res = await fetch(`/craniofacial-surgery-waiting-list-cards/${id}/report/pdf`, {
				method: 'POST',
			...
			if (res.ok) {
				...
				a.href = url;
				a.download = `craniofacial-surgery-waiting-list-card-${data.patient.name || id}.pdf`;
				a.click();
	...
	<header class="border-b border-base-300 bg-base-100 shadow-sm no-print">
		<div class="mx-auto flex max-w-4xl items-center justify-between px-4 py-4">
			...
			<div class="flex items-center gap-3">
				...
				<Button data-variant="secondary" onclick={() => goto(`/craniofacial-surgery-waiting-list-card/craniofacial-surgery-waiting-list-cards/${id}`)}>
					Edit
				</Button>
			</div>
		</div>
