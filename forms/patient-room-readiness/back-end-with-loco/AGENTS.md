...
Pure JSON API back-end for the Patient Room Readiness form, built with axum + Loco +
...
- [`patient_room_readiness/`](./patient_room_readiness/) — the Loco crate: `src/patient_room_readiness/` holds `app.rs`
...
`/metrics`. The registered domain controllers are:
  ...
  - `patient_room_readiness_checklist`

## Engine

`src/patient_room_readiness/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd patient_room_readiness && cargo check --all-targets && cargo test
```
