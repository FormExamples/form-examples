...
// independent of the PSQI total score (e.g. drowsy driving, suspected
// sleep apnoea, severe insomnia, polypharmacy). Mirrors the SvelteKit
// reference engine in `src/lib/engine/flagged-issues.ts`.
...
function detectAdditionalFlags(data) {
  ...
  if (
    ...
    flags.push({
      ...
      message:
        'Frequent breathing difficulty combined with frequent coughing/snoring' +
  ...
  if (frequencyToScore(data.sleepDisturbances.breathingDifficulty) >= 3) {
    flags.push({
      ...
      category: 'Sleep Apnea Suspicion',
      message:
        'Breathing difficulty reported 3+ times per week' +
  ...
  if (minutesToFall !== null && minutesToFall !== undefined && minutesToFall > 60) {
    flags.push({
      id: 'FLAG-INSOMNIA-001',
      category: 'Severe Insomnia',
      message:
        `Takes over 60 minutes to fall asleep (${minutesToFall} min)` +
        ' - evaluate for chronic insomnia disorder',
      priority: 'high'
    });
  }

  const sleepHours = data.sleepDuration.actualSleepHours;
  if (sleepHours !== null && sleepHours !== undefined && sleepHours < 4) {
    flags.push({
      id: 'FLAG-INSOMNIA-002',
      category: 'Severe Insomnia',
      message:
        `Extremely low sleep duration (${sleepHours} hours)` +
  ...
  if (frequencyToScore(data.daytimeDysfunction.troubleStayingAwake) >= 3) {
    flags.push({
      ...
      message:
        'Trouble staying awake 3+ times per week' +
        ' - assess for narcolepsy or underlying sleep disorder',
      priority: 'high'
    });
  }
  if (data.daytimeDysfunction.drivingDrowsiness === 'yes') {
    flags.push({
      ...
      category: 'Excessive Daytime Sleepiness',
      message:
        'Reports drowsiness while driving - immediate safety concern,' +
        ' advise against driving until assessed',
      priority: 'high'
    });
  }
  ...
  if (
    ...
    flags.push({
      ...
      message:
        'Using both prescription and OTC sleep aids' +
        ' - review for polypharmacy and dependence risk',
      priority: 'high'
    });
  }
  if (frequencyToScore(data.sleepMedicationUse.frequency) >= 3) {
    flags.push({
      id: 'FLAG-MEDS-002',
      category: 'High Medication Use',
      message:
        'Sleep medication used 3+ times per week' +
        ' - assess for medication dependence',
      priority: 'medium'
    });
  }
  ...
  if (data.medicalLifestyle.shiftWork === 'yes') {
    flags.push({
      ...
      category: 'Shift Work',
      message:
        'Patient works shifts - consider shift work sleep disorder' +
  ...
  if (data.medicalLifestyle.caffeineIntake === 'high-5-plus') {
    flags.push({
      ...
      message:
        'High caffeine intake (5+ beverages/day)' +
        ' - likely contributing to sleep difficulties',
      priority: 'medium'
    });
  }
  if (data.medicalLifestyle.alcoholUse === 'heavy') {
    flags.push({
      ...
      category: 'Substance Interference',
      message:
        'Heavy alcohol use reported - alcohol disrupts sleep architecture' +
        ' and worsens sleep quality',
      priority: 'medium'
    });
  }
  ...
  if (data.medicalLifestyle.screenTimeBeforeBed === 'more-than-60-min') {
    flags.push({
      ...
      message:
        'More than 60 minutes of screen time before bed' +
  ...
  if (
    ...
    sleepHours !== null && sleepHours !== undefined && sleepHours < 5
  ) {
    flags.push({
      id: 'FLAG-MENTAL-001',
      category: 'Mental Health Concern',
      message:
        'Combination of low enthusiasm and poor sleep' +
  ...
  if (
    hoursAsleep !== null && hoursAsleep !== undefined &&
    hoursInBed !== null && hoursInBed !== undefined &&
    hoursInBed > 0
  ) {
    ...
    if (efficiency < 65) {
      flags.push({
        ...
        message:
          `Sleep efficiency is ${efficiency.toFixed(1)}% (below 65%)` +
  ...
  if (frequencyToScore(data.sleepDisturbances.pain) >= 3) {
    flags.push({
      ...
      message:
        'Pain disrupts sleep 3+ times per week' +
