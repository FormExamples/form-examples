# Sleep Quality Assessment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Sleep Quality Assessment form, built with axum + Loco +
...
- [`sleep_quality_assessment/`](./sleep_quality_assessment/) — the Loco crate: `src/sleep_quality_assessment/` holds `app.rs`
...
`src/sleep_quality_assessment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd sleep_quality_assessment && cargo check --all-targets && cargo test
```
