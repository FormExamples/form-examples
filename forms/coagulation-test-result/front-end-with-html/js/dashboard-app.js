...
import { abnormalitySeverityClass, abnormalitySeverityLabel, followUpUrgencyClass, followUpUrgencyLabel, overallResultStatusClass, overallResultStatusLabel, reportStatusLabel, resultClassificationClass, resultClassificationLabel } from './types.js';

// Coagulation Test Result — clinician dashboard (vanilla classic-script app).
//
...
// box + result-status dropdown + classification dropdown + follow-up-urgency
// dropdown). Columns mirror the SvelteKit dashboard's `ReportRow` shape
// (`front-end-with-svelte/src/lib/engine/types.ts`).
//
// Sibling modules loaded as plain `<script>` tags (in dependency order) attach
// their exports to `window.CoagulationTestResultDashboard`; display labels come
// from the form namespace `window.CoagulationTestResult` (js/types.js). The
...
const columns = [
  ...
  { key: 'overallResultStatus',       label: 'Result status' },
...
function matchesFilters(row) {
  ...
  if (filters.resultStatus && row.overallResultStatus !== filters.resultStatus) {
    return false;
  }
...
function compareRows(a, b) {
  ...
  if (key === 'overallResultStatus') {
    av = resultStatusRank[av] ?? -1;
    bv = resultStatusRank[bv] ?? -1;
    return (av - bv) * dir;
  }
  ...
  // Default: string compare (id, patientName, reportStatus,
...
function renderTableBody() {
  ...
  for (const row of rows) {
    ...
    tr.innerHTML = `
      ...
      <td>${badgeCell(overallResultStatusClass(row.overallResultStatus), overallResultStatusLabel(row.overallResultStatus))}</td>
...
function bindFilterInputs() {
  ...
  const resultStatus = document.getElementById('filter-result-status');
  ...
  if (resultStatus) {
    resultStatus.addEventListener('change', () => {
      filters.resultStatus = resultStatus.value;
      renderAll();
    });
  }
  if (classification) {
  ...
  if (clearBtn) {
    clearBtn.addEventListener('click', () => {
      ...
      if (resultStatus) resultStatus.value = '';
