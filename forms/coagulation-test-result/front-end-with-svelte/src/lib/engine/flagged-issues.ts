import type { CoagulationResult, Flag, FlagPriority } from './types';
import {
	hasCriticalValue,
	hasDicPicture,
 ...
 * categories mirror sql/07_create_table_coagulation_test_result_grade_flag.sql.
 * Flags are returned sorted high → medium → low priority.
 */
export function detectFlags(r: CoagulationResult): Flag[] {
	const flags: Flag[] = [];
