...
4. `03_create_table_clinician.sql` — reporting clinician (consultant haematologist / biomedical scientist).
5. `04_create_table_coagulation_test_result.sql` — main result/report record (PT/INR, APTT/ratio, fibrinogen, D-dimer, thrombin time, factor assays).
6. `05_create_table_coagulation_test_result_grade.sql` — four-axis interpretation grade.
7. `06_create_table_coagulation_test_result_grade_rule.sql` — fired-rule audit trail.
8. `07_create_table_coagulation_test_result_grade_flag.sql` — safety-critical flags.
...
date / time. Result-value columns are `NUMERIC` with range `CHECK`s.
