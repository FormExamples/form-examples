import type { AssessmentData, FlaggedIssue, GradedTimepoint } from './types';

/** Format a timepoint's minutes for display, e.g. "5-minute". */
function tpLabel(minutes: number | null): string {
	return minutes == null ? 'a' : `${minutes}-minute`;
}

/**
 * Detect clinician-facing safety flags (red flags), independent of the
 * per-timepoint totals the grader produces (spec §5):
 *
 ...
 *                                      scored timepoint
 *
 * Rows mirror the `apgar_score_grade_flag` SQL table (flag_id, category,
 * priority, description, suggested_action).
 */
export function detectFlaggedIssues(
	_data: AssessmentData,
	gradedTimepoints: GradedTimepoint[]
): FlaggedIssue[] {
	...
	if (fiveMinute && fiveMinute.total < 7 && !hasTenPlus) {
		flags.push({
			...
			priority: 'medium',
			description:
				'The 5-minute total is below 7 but no 10-minute (or later) timepoint has been scored',
			suggestedAction:
