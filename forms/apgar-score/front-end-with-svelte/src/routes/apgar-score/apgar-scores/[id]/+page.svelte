<script lang="ts">
	...
	function validate(): boolean {
		...
		if (d.context.clinicianName.trim() === '') {
			found.push({ id: 'context-clinicianName', message: 'Attending clinician name is required.' });
		}
		if (d.context.careSetting === '') {
			found.push({ id: 'context-careSetting', message: 'Care setting is required.' });
		}
		if (d.identification.newbornIdentifier.trim() === '') {
			found.push({
				id: 'identification-newbornIdentifier',
				message: 'Newborn identifier is required.'
			});
		}
		errors = found;
	...
	function submit() {
		...
		assessment.result = calculateApgarGrade(assessment.data);
		goto(`/apgar-score/apgar-scores/${id}/report`);
	}
...
<main class="mx-auto max-w-3xl px-4 py-6">
	<header class="mb-6 no-print">
		<h1 class="text-2xl font-bold text-base-content">
			{isNew ? 'New Apgar assessment' : `Apgar assessment ${id}`}
		</h1>
		<p class="mt-1 text-sm text-base-content/70">
			Complete the four sections; the per-timepoint Apgar totals, bands, and trend are computed on
			submit.
		</p>
	...
	<Form label="Apgar assessment" onsubmit={submit}>
		<Step1BirthContext />
		<Step2NewbornIdentification />
		<Step3TimepointAssessments />
		<Step4Summary />
