...
// `DashboardExaminationsResponse`. When the fetch fails (CORS, network, server
...
/**
 * Fetch the examination list from the backend.
 *
 ...
 *
 * @returns {Promise<import('./dashboard-types.js').ExaminationRow[]>}
 */
async function fetchExaminations() {
  const res = await fetch(`${API_BASE}${EXAMINATIONS_PATH}`);
  if (!res.ok) {
    throw new Error(
      `Failed to fetch examinations: ${res.status} ${res.statusText}`
    );
  }
  /** @type {import('./dashboard-types.js').DashboardExaminationsResponse} */
