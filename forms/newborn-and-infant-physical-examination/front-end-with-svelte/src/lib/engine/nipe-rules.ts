import type { NipeRule } from './types';

/** Oxygen saturation (percent) below this threshold is a heart refer trigger. */
export const LOW_SAT = 95;
 ...
 * (`nipe-grader.ts`) evaluates them, then applies the not-examined /
 * satisfactory fallbacks and the overall outcome roll-up. Rows mirror the
 * `newborn_and_infant_physical_examination_grade_rule` SQL table.
 */
export const nipeReferRules: NipeRule[] = [
	// ─── EYES ──────────────────────────────────────────────────────
	{
		id: 'R-EYES-REFER-01',
		component: 'eyes',
