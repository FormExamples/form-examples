import type { SleepStudyResult, GradingResult, Recommendation, FiredRule } from './types';
...
/**
 * Pure four-axis interpretation engine for a sleep study result.
 *
 * Computes:
 * - Axis A: result classification (normal / abnormal / critical / inconclusive).
 * - Axis B: abnormality severity + structured reporting category (AHI band).
 ...
 *
 * Invariant: a critical finding (severe OSA with significant desaturation, or
 * nocturnal hypoventilation) auto-escalates Axis D to critical-alert, triggers
 * an urgent CPAP / ventilation review, and raises the critical-result-alert
 * flag, regardless of the other axes. The least-urgent band is only chosen when
 * no rule fires.
 *
 * No side effects, no network calls, no I/O.
 */
export function calculateGrade(result: SleepStudyResult): GradingResult {
