# Sleep Study Test Result: Back-end with Rust Axum Loco (JSON API)

Server-side Rust JSON API for the Sleep Study Test Result form. Pure back-end: no HTML
rendering, no Tera templates, no HTMX, no Alpine.js, no CSS, no Lily
Design System.

The Loco crate lives in [`sleep_study_test_result/`](sleep_study_test_result/) — one migration, SeaORM
