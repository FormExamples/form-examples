# Sleep Study Test Result — PostgreSQL migrations (source of truth)
...
3. `02_create_table_patient.sql` — patient demographics (with BMI and neck circumference).
4. `03_create_table_clinician.sql` — reporting clinician (respiratory / sleep physician or clinical physiologist).
5. `04_create_table_sleep_study_test_result.sql` — main result/report record.
6. `05_create_table_sleep_study_test_result_grade.sql` — four-axis interpretation grade.
7. `06_create_table_sleep_study_test_result_grade_rule.sql` — fired-rule audit trail.
8. `07_create_table_sleep_study_test_result_grade_flag.sql` — safety-critical flags.
