# Pre-Anaesthesia Assessment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Pre-Anaesthesia Assessment form, built with axum + Loco +
...
- [`pre_anaesthesia_assessment/`](./pre_anaesthesia_assessment/) — the Loco crate: `src/pre_anaesthesia_assessment/` holds `app.rs`
...
`/metrics`. The registered domain controllers are:
  ...
  - `pre_anaesthesia_assessment_grade`

## Engine

`src/pre_anaesthesia_assessment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd pre_anaesthesia_assessment && cargo check --all-targets && cargo test
```
