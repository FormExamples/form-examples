...
// ──────────────────────────────────────────────
// Detailed pre-anaesthesia proforma (additional to the 16-step ASA
// assessment above). Captures a denser clinical pro-forma checklist —
// most items are boolean "present/checked" flags rather than the
// conversational Yes/No fields used elsewhere in this engine.
// ──────────────────────────────────────────────
...
export interface PreviousAnaesthesiaChecklist {
  anaestheticDifficulty: boolean;
  abnormalReaction: boolean;
  ponv: boolean;
  malignantHyperpyrexia: boolean;
  difficultIntubation: boolean;
  difficultSpinalOrEpidural: boolean;
}

export interface AddictionSubstanceAbuse {
  alcohol: boolean;
  smoking: boolean;
  fastScore3OrMore: boolean;
  betel: boolean;
  drugs: boolean;
  other: boolean;
  otherDetails: string;
...
export interface PastMedicalHistoryProforma {
  hypertension: boolean;
  mi: boolean;
  miWithinPast6Months: boolean;
  anginaOrChestPain: boolean;
  anginaTriggers: (
    | "vigorous-exercise"
  ...
  dyspnoea: boolean;
  dyspnoeaSubtypes: ("at-rest" | "on-lying-flat" | "wakes-at-night-gasping")[];
  palpitation: boolean;
  arrhythmia: boolean;
  ...
  pacemaker: boolean;
  pacemakerLastCheckWithin6Months: boolean;
  implantedIcdCrtd: boolean;
  angioplasty: boolean;
  stenting: boolean;
  stentingWithinPast6Months: boolean;
  stentType: "drug-eluting" | "bare-metal" | "unknown" | "";
  valveDisease: boolean;
  cardiacSurgery: boolean;
  cardiacSurgeryWithinPastYear: boolean;
  ...
  previousHospitalAdmission: boolean;
  icuAdmission: boolean;
  onHomeOxygenOrNebulizers: boolean;
  usesInhalerDailyOrMore: boolean;
  fluCoughSputum: boolean;
  haemoptysis: boolean;
  sleepApnoea: boolean;
  sleepApnoeaUsingCpap: boolean;
  stopBangRisk: "high" | "medium" | "low" | "";
  otherRespiratoryDisease: boolean;
  diabetesMellitus: boolean;
  diabetesDietControlled: boolean;
  diabetesDrugControlled: boolean;
  diabetesInsulinControlled: boolean;
  diabetesHba1cOver69: boolean;
  hypothyroid: boolean;
  hyperthyroid: boolean;
  strokeOrTia: boolean;
  strokeWithin3Months: boolean;
  strokeResidualDisability: string;
  epilepsyOrSeizures: boolean;
  epilepsyControl:
    "well-controlled" | "3-12-months-ago" | "poorly-controlled" | "";
  ms: boolean;
  muscularDystrophy: boolean;
  otherNeurologicalDisease: boolean;
  otherNeurologicalDiseaseDetails: string;
  dementia: boolean;
  creutzfeldtJakobDisease: boolean;
  growthHormoneOrGonadotrophin: boolean;
  brainOrSpinalCordSurgery: boolean;
  fourAtScore: number | null;
  complexNeeds: boolean;
  renalImpairment: boolean;
  ckdStage3: boolean;
  ckdStage4OrHemodialysis: boolean;
  liverDisease: boolean;
  clottingDisordersHaemophilia: boolean;
  easyBruisingProlongedBleeding: boolean;
  dvtOrPe: boolean;
  dvtOrPeWithinPast3Months: boolean;
  gord: boolean;
  anyOtherDisease: boolean;
  chronicPain: boolean;
  dukeActivityStatusIndex: number | null;
  metsScore: number | null;
  ongoingMedications: string;
  cortisonePrednisoneSteroid: boolean;
  bloodTransfusionHistory: boolean;
  chemotherapyDrugs: boolean;
  radiotherapy: boolean;
}

export interface AirwayExamProforma {
  mouthOpening: boolean;
  looseTeeth: boolean;
  dentureMissing: boolean;
  mpGrade: "1" | "2" | "3" | "4" | "";
  micrognathia: boolean;
  tmDistanceOver65: boolean;
  moDistance: boolean;
  tmJoint: boolean;
  shortNeckRom: boolean;
  difficultAirway: boolean;
  obesityScoring: boolean;
  spineBack: boolean;
  scoliosis: boolean;
  vitalsExamination: boolean;
}

export interface VitalsProforma {
...
export interface GeneralExaminationProforma {
  pallor: boolean;
  icterus: boolean;
  cyanosis: boolean;
  clubbing: boolean;
...
export interface InvestigationsProforma {
  bloodGroupHaemoglobin: string;
  differentialN: string;
  differentialL: string;
  differentialE: string;
  differentialM: string;
  ...
  abg: string;
  twoDEcho: string;
  lvefPercent: number | null;
  tmt: string;
  ctScan: string;
  mri: string;
}

export interface RiskFactorsChecklist {
  cardiac: boolean;
  respiratory: boolean;
  diabetes: boolean;
  insulin: boolean;
  ...
  complexNeeds: boolean;
  anaemia: boolean;
  neuromuscularDisorders: boolean;
  others: boolean;
  othersDetails: string;
}

export interface AnaestheticPlanProforma {
  anaestheticConcerns: string;
  planTiva: boolean;
  planRa: boolean;
  planOther: boolean;
  planOtherDetails: string;
  listForOtDeferNotFit: "list-for-ot" | "defer" | "not-fit" | "";
  nilOrallyAfter: string;
  informedWrittenConsent: boolean;
  risksBenefitsAlternativesDiscussed: boolean;
  arrangeUnitsOfBlood: string;
  arrangePostOpIcu: boolean;
  backup: boolean;
  doInvestigations: string;
  doSpecialOrders: string;
  reviewedHighRiskAnaesthesia: boolean;
  reviewedConsent: boolean;
  consultantAnaesthesiologistName: string;
  consultantAnaesthesiologistSignature: string;
}
...
export interface Airway {
  ...
  dentition:
    "good" | "loose-teeth" | "caps-crowns" | "edentulous" | "dentures" | "";
  beard: YesNo;
...
export interface RenalHepatic {
  ...
  dialysisStatus:
    "none" | "peritoneal" | "haemodialysis" | "haemofiltration" | "";
...
export interface Gastrointestinal {
  abdominalExam:
    "normal" | "distended" | "tender" | "organomegaly" | "other" | "";
...
export interface AnaesthesiaPlan {
  ...
  rsiPlanned: YesNo;
  monitoringLevel:
    "standard" | "invasive-arterial" | "invasive-cvc" | "cardiac-output" | "";
...
export interface ClinicianAssessment {
  ...
  previousAnaesthesiaChecklist: PreviousAnaesthesiaChecklist;
  addictionSubstanceAbuse: AddictionSubstanceAbuse;
  pastMedicalHistoryProforma: PastMedicalHistoryProforma;
  airwayExamProforma: AirwayExamProforma;
  vitalsProforma: VitalsProforma;
  generalExaminationProforma: GeneralExaminationProforma;
  investigationsProforma: InvestigationsProforma;
  riskFactorsChecklist: RiskFactorsChecklist;
  anaestheticPlanProforma: AnaestheticPlanProforma;
}
