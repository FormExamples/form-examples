// Flagged-issue detection. Independent of AQ-10 score, this module raises
// clinician-facing flags for high AQ-10 totals, severe sensory profiles,
// social communication difficulties, repetitive-behavior patterns,
// developmental delays, family history of neurodevelopmental conditions,
// and previous assessments.
//
// Mirrors `forms/autism-assessment/front-end-form-with-svelte/src/lib/engine/flagged-issues.ts`.
...
function detectAdditionalFlags(data) {
  ...
  const langText = (data.developmentalHistory.languageMilestones || '').toLowerCase();
  if (langText.includes('delay') || langText.includes('late')) {
    flags.push({
      id: 'FLAG-DEV-001',
  ...
  const motorText = (data.developmentalHistory.motorMilestones || '').toLowerCase();
  if (motorText.includes('delay') || motorText.includes('late')) {
    flags.push({
      id: 'FLAG-DEV-002',
      category: 'Developmental History',
