# Autism Assessment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Autism Assessment form, built with axum + Loco +
...
- [`autism_assessment/`](./autism_assessment/) — the Loco crate: `src/autism_assessment/` holds `app.rs`
...
`src/autism_assessment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd autism_assessment && cargo check --all-targets && cargo test
```
