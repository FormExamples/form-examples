...
impl MigrationTrait for Migration {
    async fn up(&self, m: &SchemaManager) -> Result<(), DbErr> {
        create_table(m, "pulmonary_function_test_request_grades",
            &[
            ...
            ("contraindication_band", ColType::StringWithDefault(String::new())),
            ...
            &[
            ("pulmonary_function_test_request", ""),
            ]
        ).await
    }

    async fn down(&self, m: &SchemaManager) -> Result<(), DbErr> {
        drop_table(m, "pulmonary_function_test_request_grades").await
    }
}
