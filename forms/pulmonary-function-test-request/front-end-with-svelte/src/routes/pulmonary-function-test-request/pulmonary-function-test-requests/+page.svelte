<script lang="ts">
	...
	const columns = [
		...
		{ id: 'patientName', header: 'Patient', flexgrow: 2, sort: true },
		{
			...
			header: 'Test type',
			flexgrow: 2,
			template: (v: string) => testTypeLabel(v as never)
		},
		{
		...
		{ id: 'appropriatenessScore', header: 'Score', width: 80, sort: true, template: (v: number) => `${v}/9` },
		{
			id: 'contraindicationBand',
			header: 'Safety',
			width: 130,
			template: (v: string) => contraindicationLabel(v as never)
		},
	...
	function init(api: any) {
		...
		api.on('select-row', (ev: { id?: string | number }) => {
			if (ev?.id != null) goto(`/pulmonary-function-test-request/pulmonary-function-test-requests/${ev.id}`);
		});
	}
</script>
...
<main class="mx-auto max-w-6xl px-4 py-6">
	<div class="mb-6 flex flex-wrap items-end justify-between gap-4">
		<div>
			<h1 class="text-2xl font-bold text-base-content">Lung-function vetting dashboard</h1>
			<p class="mt-1 text-sm text-base-content/70">
				Four-axis vetting grades for incoming pulmonary function test requests, computed by the
				shared engine. Select a row to open the request.
			</p>
		</div>
		<a href="/pulmonary-function-test-request/pulmonary-function-test-requests/new" class="button" data-variant="primary">New request</a>
	</div>
	...
	<div class="mb-4 flex flex-wrap gap-4">
		...
		<label class="text-sm">
			...
			<select class="select inline-block w-auto" bind:value={recommendationFilter}>
				...
				<option value="redirect">Defer / redirect</option>
