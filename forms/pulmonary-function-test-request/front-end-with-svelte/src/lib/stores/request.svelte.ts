...
// Re-export so consumers can import the factory from the store as well as the
// engine (the engine's defaults.ts is the single source of truth).
export { createDefaultRequest };

/** localStorage draft key for a given request id (defaults to `new`). */
function storageKey(id: string): string {
	return `pulmonary-function-test-request.front-end-with-svelte.${id || 'new'}.v1`;
}

/**
 * Svelte 5 reactive store for the pulmonary function test request, with
 ...
 */
class RequestStore {
	data = $state<PulmonaryFunctionTestRequest>(createDefaultRequest());
	 ...
	 */
	loadForId(id: string, seed?: PulmonaryFunctionTestRequest) {
		const key = id || 'new';
		this.id = key;
		this.result = null;
		this.currentStep = 1;

		let draft: PulmonaryFunctionTestRequest | null = null;
		if (browser) {
			const raw = localStorage.getItem(storageKey(key));
			if (raw) {
				try {
					draft = JSON.parse(raw) as PulmonaryFunctionTestRequest;
				} catch {
