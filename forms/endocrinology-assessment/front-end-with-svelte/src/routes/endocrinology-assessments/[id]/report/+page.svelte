<script lang="ts">
	...
	import { axisStatusLabel, axisStatusColor, bmiCategory, calculateAge } from '$lib/engine/utils';
	...
	$effect(() => {
		if (!assessment.result) {
			goto(`/endocrinology-assessments/${id}`);
		}
	});
	...
	<header class="border-b border-base-300 bg-base-100 shadow-sm no-print">
		<div class="mx-auto flex max-w-4xl items-center justify-between px-4 py-4">
			...
			<div class="flex items-center gap-3">
				...
				<Button data-variant="secondary" onclick={() => goto(`/endocrinology-assessments/${id}`)}>Edit</Button>
			</div>
		</div>
	</header>
