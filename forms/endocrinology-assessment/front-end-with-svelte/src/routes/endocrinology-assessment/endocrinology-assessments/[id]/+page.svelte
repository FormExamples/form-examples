<script lang="ts">
	...
	function submit() {
		...
		assessment.result = calculateGrades(assessment.data);
		goto(`/endocrinology-assessment/endocrinology-assessments/${id}/report`);
	}
...
<main class="mx-auto max-w-3xl px-4 py-6">
	<header class="mb-6 no-print">
		<h1 class="text-2xl font-bold text-base-content">
			{isNew ? 'New endocrinology assessment' : `Endocrinology assessment ${id}`}
		</h1>
		<p class="mt-1 text-sm text-base-content/70">
			Complete the ten sections; the per-axis grades and overall endocrine status are computed on
			submit.
		</p>
	...
	<Form label="Endocrinology assessment" onsubmit={submit}>
		<Step1Demographics />
		<Step2PresentingSymptoms />
		<Step3ThyroidAxisReview />
		<Step4AdrenalAxisReview />
		<Step5GlucoseMetabolism />
		<Step6ReproductiveAxis />
		<Step7PituitaryFunction />
		<Step8BoneCalcium />
		<Step9MedicationsLifestyleReview />
