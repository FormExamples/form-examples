import type { AssessmentData, AxisRule, AxisStatus } from './types';
...
export function anyAnswered(section: Record<string, unknown>): boolean {
...
export const axisRules: AxisRule[] = [
	{
		...
		evaluate: (d: AssessmentData) => {
			...
			const symptomatic =
				sx.heatIntolerance === 'yes' ||
				sx.coldIntolerance === 'yes' ||
				sx.palpitations === 'yes' ||
				sx.tremor === 'yes';
			...
			if (!status) {
				status = anyAnswered(t as unknown as Record<string, unknown>) ? 'normal' : '';
			}
	...
	},
	{
		...
		evaluate: (d: AssessmentData) => {
			...
			if (!status) status = anyAnswered(a as unknown as Record<string, unknown>) ? 'normal' : '';
			return { status, findings };
		}
	},
	{
		...
		evaluate: (d: AssessmentData) => {
			...
			if (!status) status = anyAnswered(g as unknown as Record<string, unknown>) ? 'normal' : '';
			return { status, findings };
		}
	},
	{
		...
		evaluate: (d: AssessmentData) => {
			...
			if (!status) status = anyAnswered(r as unknown as Record<string, unknown>) ? 'normal' : '';
			return { status, findings };
		}
	},
	{
		...
		evaluate: (d: AssessmentData) => {
			...
			if (!status) status = anyAnswered(p as unknown as Record<string, unknown>) ? 'normal' : '';
			return { status, findings };
		}
	},
	{
		...
		evaluate: (d: AssessmentData) => {
			...
			if (!status) status = anyAnswered(b as unknown as Record<string, unknown>) ? 'normal' : '';
			return { status, findings };
		}
	}
];
