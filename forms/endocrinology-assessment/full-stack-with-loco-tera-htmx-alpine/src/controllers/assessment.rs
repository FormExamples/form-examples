...
use crate::engine::{endocrine_grader, types::AssessmentData, utils::axis_status_label};
...
async fn submit_assessment(
    ...
    if let Some(obj) = data_value.as_object_mut() {
        if let Some(form_obj) = form_data.as_object() {
            // Collect dynamic current_medications[idx].field entries.
            let mut meds: std::collections::BTreeMap<
                usize,
                serde_json::Map<String, serde_json::Value>,
            > = std::collections::BTreeMap::new();

            for (key, value) in form_obj {
                // current_medications[0].name -- dynamic medication rows.
                    ...
                    if let Some(bracket_end) = key.find(']') {
                            ...
                            key[key.find('[').unwrap() + 1..bracket_end].parse::<usize>()
                        {
                            if let Some(dot_pos) = key.find("].") {
                                ...
                                meds.entry(idx).or_default().insert(json_field, parsed);
                            }
                        }
                    }
                    continue;
            ...
            // Materialize current_medications as an array on medicationsLifestyle.
            if !meds.is_empty() {
                let items: Vec<serde_json::Value> = meds
                    .values()
                    .filter(|entry| {
                        entry
                            .get("name")
...
async fn show_report(
    ...
    let grade = endocrine_grader::calculate_grades(&assessment_data);
    ...
    context.insert("overall_label", &axis_status_label(&grade.overall_status));
    context.insert("answered_count", &grade.answered_count);
    context.insert("axis_grades", &grade.axis_grades);
