...
/// GET /dashboard -- render the endocrinology dashboard with filters.
#[debug_handler]
async fn dashboard(
    ...
    if let Some(ref term) = params.search {
        ...
        items.retain(|row| row.patient_name.to_lowercase().contains(&term));
    }
    ...
    context.insert(
        "overall_status",
        &params.overall_status.unwrap_or_default(),
    );
