# Endocrinology Assessment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Endocrinology Assessment form, built with axum + Loco +
...
- [`endocrinology_assessment/`](./endocrinology_assessment/) — the Loco crate: `src/endocrinology_assessment/` holds `app.rs`
...
`src/endocrinology_assessment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd endocrinology_assessment && cargo check --all-targets && cargo test
```
