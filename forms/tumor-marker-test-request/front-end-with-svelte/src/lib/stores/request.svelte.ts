...
function storageKey(id: string): string {
	return `tumor-marker-test-request.front-end-with-svelte.${id || 'new'}.v1`;
}

/**
 * Svelte 5 reactive store for the serum tumour-marker request, with
 ...
 */
class RequestStore {
	data = $state<TumorMarkerRequest>(createDefaultRequest());
	 ...
	 */
	loadForId(id: string, seed?: TumorMarkerRequest) {
		const key = id || 'new';
		this.id = key;
		this.result = null;
		this.currentStep = 1;

		let draft: TumorMarkerRequest | null = null;
		if (browser) {
			const raw = localStorage.getItem(storageKey(key));
			if (raw) {
				try {
					draft = JSON.parse(raw) as TumorMarkerRequest;
				} catch {
