<script lang="ts">
	...
	import Step3RequestedMarkers from '#lib/components/steps/Step3RequestedMarkers.svelte';
	import Step4ClinicalContext from '#lib/components/steps/Step4ClinicalContext.svelte';
	import Step5TriageAndSubmit from '#lib/components/steps/Step5TriageAndSubmit.svelte';
	...
	function validate(): boolean {
		...
		if (countSelectedMarkers(d.markers) === 0) {
			found.push({ id: 'markers', message: 'Select at least one tumour marker.' });
		}
		if (d.context.primaryIndication === '') {
			found.push({ id: 'primaryIndication', message: 'Primary indication is required.' });
		}
		if (d.context.clinicalDetails.trim() === '') {
	...
	function submit() {
		...
		goto(`/tumor-marker-test-request/tumor-marker-test-requests/${id}/report`);
	}
...
<main class="mx-16 px-4 py-6">
	<h1 class="text-2xl font-bold text-base-content">
		{isNew ? 'New tumour-marker request' : `Tumour-marker request ${id}`}
	</h1>
	<p class="mt-1 text-sm text-base-content/70">
		Complete the five sections; the four-axis vetting grade is computed on submit.
	</p>
	...
	<Form label="Tumor marker test request" onsubmit={submit}>
		<Step1RequestingClinician />
		<Step2PatientIdentification />
		<Step3RequestedMarkers />
		<Step4ClinicalContext />
		<Step5TriageAndSubmit />
