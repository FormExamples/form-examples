...
impl MigrationTrait for Migration {
    async fn up(&self, m: &SchemaManager) -> Result<(), DbErr> {
        create_table(m, "tumor_marker_test_request_grades",
            &[
            ...
            ("interpretation_band", ColType::StringWithDefault(String::new())),
            ...
            &[
            ("tumor_marker_test_request", ""),
            ]
        ).await
    }

    async fn down(&self, m: &SchemaManager) -> Result<(), DbErr> {
        drop_table(m, "tumor_marker_test_request_grades").await
    }
}
