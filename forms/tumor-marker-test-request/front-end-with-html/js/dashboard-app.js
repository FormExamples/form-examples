...
// Tumor Marker Test Request — vetting dashboard (vanilla classic-script app).
//
...
// interpretation-band dropdown + safety-flags dropdown).
//
// Sibling modules loaded as plain `<script>` tags (in dependency order)
// attach their exports to `window.TumorMarkerTestRequestDashboard`. The whole
...
function interpClass(b) {
  return b ? 'interp-' + String(b) : '';
}
...
function markersText(markers) {
  return Array.isArray(markers) ? markers.join(', ') : '';
}
...
function matchesFilters(row) {
  if (filters.search) {
    ...
    const matches =
      ...
      markersText(row.markers).toLowerCase().includes(term) ||
      String(row.indication || '').toLowerCase().includes(term) ||
      String(row.clinician || '').toLowerCase().includes(term);
    if (!matches) return false;
  }
  if (filters.urgency && row.triageTier !== filters.urgency) return false;
  if (filters.band && row.appropriatenessBand !== filters.band) return false;
  if (filters.interp && row.interpretationBand !== filters.interp) return false;
...
function compareRows(a, b) {
  ...
  if (key === 'interpretationBand') {
    return ((interpRank[av] ?? -1) - (interpRank[bv] ?? -1)) * dir;
  }
  if (key === 'triageTier') {
...
function renderMarkersCell(markers) {
  if (!Array.isArray(markers) || markers.length === 0) {
    return '<span class="flag-empty">— none —</span>';
  }
  const chips = markers
    .map((m) => `<span class="marker-chip">${esc(m)}</span>`)
    .join('');
  return `<div class="marker-list">${chips}</div>`;
}
...
function renderTableBody() {
  ...
  for (const row of rows) {
    ...
    tr.innerHTML = `
      ...
      <td class="data-table-td">${renderMarkersCell(row.markers)}</td>
      ...
      <td class="data-table-td"><span class="interp-badge ${interpClass(row.interpretationBand)}">${esc(titleCase(row.interpretationBand))}</span></td>
...
function bindFilterInputs() {
  ...
  const interp = document.getElementById('filter-interp');
