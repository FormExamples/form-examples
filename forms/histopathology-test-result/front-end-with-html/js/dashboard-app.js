...
import { abnormalitySeverityClass, abnormalitySeverityLabel, followUpUrgencyClass, followUpUrgencyLabel, reportStatusLabel, resultClassificationClass, resultClassificationLabel } from './types.js';

// Histopathology Test Result — clinician dashboard (vanilla classic-script app).
//
...
// box + report-status dropdown + classification dropdown + follow-up-urgency
// dropdown). Columns mirror the SvelteKit dashboard route
// (`front-end-with-svelte/src/routes/+page.svelte`).
//
// Sibling modules loaded as plain `<script>` tags (in dependency order) attach
// their exports to `window.HistopathologyTestResultDashboard`; display labels
// come from the form namespace `window.HistopathologyTestResult` (js/types.js).
...
const filters = {
  search: '',          // matches report id, patient name, or specimen site
  status: '',          // '' | ReportStatus
...
const columns = [
  ...
  { key: 'specimenSite',              label: 'Specimen site' },
...
function matchesFilters(row) {
  if (filters.search) {
    ...
    const matches =
      ...
      row.specimenSite.toLowerCase().includes(term);
    if (!matches) return false;
  }
  if (filters.status && row.reportStatus !== filters.status) return false;
...
function compareRows(a, b) {
  ...
  // Default: string compare (id, patientName, specimenSite, reportStatus,
