<script lang="ts">
	...
	import Step5MalignancyStaging from '$lib/components/steps/Step5MalignancyStaging.svelte';
	...
	function validate(): boolean {
		...
		if (d.specimenType.trim() === '') {
			found.push({ id: 'specimenType', message: 'Specimen type is required.' });
		}
		if (d.diagnosis.trim() === '') {
			found.push({ id: 'diagnosis', message: 'Diagnosis is required.' });
		}
	...
	function submit() {
		...
		goto('/histopathology-test-result/report/view');
	}
...
<main class="mx-auto max-w-3xl px-4 py-6">
	<header class="mb-6 no-print">
		<h1 class="text-2xl font-bold text-gray-900">New histopathology report</h1>
	...
	<Form label="Histopathology test result" onsubmit={submit}>
		<Step1ReportIdentification />
		<Step2Specimen />
		<Step3ClinicalHistory />
		<Step4Examination />
		<Step5MalignancyStaging />
		<Step6Impression />
