...
/** Builds the pdfmake document definition for the histopathology report. */
export function buildPdfDocument(
	data: HistopathologyResult,
	...
	return {
		...
		footer: (currentPage: number, pageCount: number) => ({
			text: `Page ${currentPage} of ${pageCount} | Generated ${new Date(result.gradedAt).toLocaleString()} | RCPath cancer datasets / TNM 8`,
			alignment: 'center',
			margin: [0, 20, 0, 0],
			fontSize: 8,
			color: '#9ca3af'
		...
		content: [
			{
				text: 'HISTOPATHOLOGY TEST RESULT',
				fontSize: 18,
				bold: true,
				alignment: 'center',
			...
			{
				text: `${data.specimenSite || 'Specimen'} — ${reportStatusLabel(data.reportStatus)}`,
				fontSize: 12,
				alignment: 'center',
				color: '#4b5563',
				margin: [0, 0, 0, 4] as Margin
			...
			{
				ul: [
					`Specimen type: ${data.specimenType || 'N/A'}`,
					`Specimen site: ${data.specimenSite || 'N/A'}`,
					`Adequacy: ${data.specimenAdequacy || 'N/A'}`
				],
				margin: [0, 0, 0, 16] as Margin
			},
			...
			{ text: { text: 'Macroscopic: ', bold: true }, margin: [0, 0, 0, 2] as Margin },
			{ text: data.macroscopicDescription || 'Not specified', margin: [0, 0, 0, 6] as Margin },
			{ text: { text: 'Microscopic: ', bold: true }, margin: [0, 0, 0, 2] as Margin },
			{ text: data.microscopicDescription || 'Not specified', margin: [0, 0, 0, 6] as Margin },
			{ text: { text: 'Diagnosis: ', bold: true }, margin: [0, 0, 0, 2] as Margin },
			{ text: data.diagnosis || 'Not specified', margin: [0, 0, 0, 16] as Margin },

			// Malignancy & staging
			sectionHeader('Malignancy & staging'),
			{
				ul: malignancyList(data),
				margin: [0, 0, 0, 16] as Margin
			},
			...
			{
				text: `Reporting category (grade/stage): ${data.reportingCategory || 'None'}`,
				margin: [0, 0, 0, 4] as Margin
			},
			{
...
function malignancyList(data: HistopathologyResult): string[] {
	const list: string[] = [];
	list.push(`Malignancy present: ${data.malignancyPresent ? 'Yes' : 'No'}`);
	if (data.tumourType) list.push(`Tumour type: ${data.tumourType}`);
	if (data.histologicalGrade) list.push(`Histological grade: ${histologicalGradeLabel(data.histologicalGrade)}`);
	const tnm = [data.tnmPt, data.tnmPn, data.tnmPm].filter((t) => t.trim() !== '').join(' ');
	if (tnm) list.push(`Pathological TNM: ${tnm}`);
	list.push(`Resection margins: ${resectionMarginsLabel(data.resectionMargins)}`);
	list.push(`Lymphovascular invasion: ${data.lymphovascularInvasion ? 'Yes' : 'No'}`);
	if (data.immunohistochemistry) list.push(`Immunohistochemistry: ${data.immunohistochemistry}`);
	if (data.snomedCode) list.push(`SNOMED CT code: ${data.snomedCode}`);
	return list;
}
