-- Oral and Maxillofacial Surgery waiting list card: the top-level record placing a patient on a list.

CREATE TABLE omfs_waiting_list_card (
...
CREATE TRIGGER trigger_omfs_waiting_list_card_updated_at
    BEFORE UPDATE ON omfs_waiting_list_card
    FOR EACH ROW
    EXECUTE FUNCTION set_updated_at();

CREATE INDEX index_omfs_waiting_list_card_patient_id
    ON omfs_waiting_list_card(patient_id);

CREATE INDEX index_omfs_waiting_list_card_practitioner_id
    ON omfs_waiting_list_card(practitioner_id);

CREATE INDEX index_omfs_waiting_list_card_clock_start
    ON omfs_waiting_list_card(rtt_clock_start_date);

COMMENT ON TABLE omfs_waiting_list_card IS
    'Top-level record placing a patient on a oral and maxillofacial surgery waiting list. Parent of appointment(s) and grading result.';
COMMENT ON COLUMN omfs_waiting_list_card.id IS
    'Primary key UUID, auto-generated.';
COMMENT ON COLUMN omfs_waiting_list_card.created_at IS
    'Timestamp when the record was created.';
COMMENT ON COLUMN omfs_waiting_list_card.updated_at IS
    'Timestamp when the record was updated most-recently.';
COMMENT ON COLUMN omfs_waiting_list_card.deleted_at IS
    'Timestamp when the record was deleted a.k.a. soft-removed.';
COMMENT ON COLUMN omfs_waiting_list_card.patient_id IS
    'Foreign key to the patient placed on the list.';
COMMENT ON COLUMN omfs_waiting_list_card.practitioner_id IS
    'Foreign key to the practitioner who placed the patient on the list.';
COMMENT ON COLUMN omfs_waiting_list_card.status IS
    'Lifecycle status: draft, submitted, reviewed, or closed.';
COMMENT ON COLUMN omfs_waiting_list_card.entry_date IS
    'Date the practitioner entered the card.';
COMMENT ON COLUMN omfs_waiting_list_card.entry_time IS
    'Time the practitioner entered the card.';
COMMENT ON COLUMN omfs_waiting_list_card.referral_source IS
    'Where the referral originated: gp, consultant, a-and-e, self, community, screening, other.';
COMMENT ON COLUMN omfs_waiting_list_card.referral_date IS
    'Date the referral was made.';
COMMENT ON COLUMN omfs_waiting_list_card.referral_letter_reference IS
    'Reference number / identifier for the originating referral letter.';
COMMENT ON COLUMN omfs_waiting_list_card.reason_for_referral IS
    'Free-text reason for referral.';
COMMENT ON COLUMN omfs_waiting_list_card.presenting_condition IS
    'Free-text presenting condition / clinical summary.';
COMMENT ON COLUMN omfs_waiting_list_card.icd_10_code IS
    'ICD-10 diagnostic code for the presenting condition.';
COMMENT ON COLUMN omfs_waiting_list_card.snomed_ct_code IS
    'SNOMED CT clinical concept code for the presenting condition.';
COMMENT ON COLUMN omfs_waiting_list_card.suspected_cancer IS
    'Whether this is a suspected-cancer (two-week wait) referral.';
COMMENT ON COLUMN omfs_waiting_list_card.list_name IS
    'Display name for the waiting list (e.g. "Orthopaedic — knee replacement").';
COMMENT ON COLUMN omfs_waiting_list_card.specialty IS
    'NHS specialty (e.g. "Trauma & Orthopaedics").';
COMMENT ON COLUMN omfs_waiting_list_card.sub_specialty IS
    'NHS sub-specialty (e.g. "Knee surgery").';
COMMENT ON COLUMN omfs_waiting_list_card.procedure_description IS
    'Plain-language description of what the patient is waiting to do.';
COMMENT ON COLUMN omfs_waiting_list_card.opcs_4_code IS
    'OPCS-4 procedure code if a specific procedure is planned.';
COMMENT ON COLUMN omfs_waiting_list_card.clinical_priority IS
    'NHS England clinical priority: P1a, P1b, P2, P3, P4, P5, P6.';
COMMENT ON COLUMN omfs_waiting_list_card.rtt_clock_start_date IS
    'Referral to Treatment (RTT) clock-start date. The date the patient joined the list.';
COMMENT ON COLUMN omfs_waiting_list_card.expected_procedure_type IS
    'Expected next event for the patient.';
COMMENT ON COLUMN omfs_waiting_list_card.expected_wait_weeks IS
    'Expected wait, in weeks, communicated to the patient at entry.';
COMMENT ON COLUMN omfs_waiting_list_card.consent_to_reminders IS
    'Whether the patient has consented to appointment reminders.';
COMMENT ON COLUMN omfs_waiting_list_card.communication_notes IS
    'Free-text communication preferences and notes.';
COMMENT ON COLUMN omfs_waiting_list_card.additional_notes IS
    'Free-text practitioner sign-off notes.';
COMMENT ON COLUMN omfs_waiting_list_card.signed_at IS
