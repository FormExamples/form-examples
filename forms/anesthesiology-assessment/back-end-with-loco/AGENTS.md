# Anesthesiology Assessment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Anesthesiology Assessment form, built with axum + Loco +
...
- [`anesthesiology_assessment/`](./anesthesiology_assessment/) — the Loco crate: `src/anesthesiology_assessment/` holds `app.rs`
...
`src/anesthesiology_assessment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd anesthesiology_assessment && cargo check --all-targets && cargo test
```
