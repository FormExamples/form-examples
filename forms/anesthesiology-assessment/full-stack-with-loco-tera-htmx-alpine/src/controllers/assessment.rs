...
/// GET /assessment/{id} -- render the single-page anesthesiology assessment wizard.
#[debug_handler]
...
fn section_to_json(section: &str) -> &str {
    match section {
        ...
        "investigations_and_plan" => "investigationsAndPlan",
        other => other,
    }
}
...
async fn show_report(
    ...
    context.insert("asa", &grade.asa);
    context.insert("airway", &grade.airway);
    context.insert("rcri", &grade.rcri);
    context.insert("stopbang", &grade.stopbang);
