...
use crate::engine::{survey_grader, types::AssessmentData};
...
/// POST /assessment/new -- create a new draft survey, redirect to the wizard.
#[debug_handler]
async fn create_new(State(ctx): State<AppContext>) -> Result<Response> {
    let item = ActiveModel::new_draft()
        .map_err(|e| Error::BadRequest(format!("Failed to create survey: {e}")))?;
    let item = item.insert(&ctx.db).await?;
    let id = item.id;
    Ok(Redirect::to(&format!("/assessment/{id}")).into_response())
}

/// GET /assessment/{id} -- render the single-page survey wizard.
#[debug_handler]
...
/// Numeric Likert / eNPS field IDs that must be coerced to a JSON number
/// (or null) regardless of how the browser submits them.
const NUMERIC_FIELDS: &[&str] = &[
    "rt1", "rt2", "wl1", "wl2", "wl3", "wl4", "wl5", "mg1", "mg2", "mg3", "mg4", "mg5", "gr1",
    "gr2", "gr3", "gr4", "cb1", "cb2", "cb3", "cb4", "cu1", "cu2", "cu3", "cu4", "cu5", "en1",
    "en2", "en3", "en4", "rc1", "rc2", "rc3", "rc4", "ov1", "ov2", "ov3", "ov4", "recommendScore",
];

/// Parse a form value string. Empty strings stay as `""` for text/enum
/// fields; for Likert / eNPS numeric fields, empty strings become `null`
/// and non-empty strings become JSON numbers.
fn parse_form_value(field_camel: &str, value: &serde_json::Value) -> serde_json::Value {
    let is_numeric = NUMERIC_FIELDS.contains(&field_camel);
        ...
        if let Some(form_obj) = form_data.as_object() {
            for (key, value) in form_obj {
                ...
                if parts.len() == 2 {
                    ...
                    {
                        let parsed = parse_form_value(&field, value);
...
async fn show_report(
    ...
    let assessment_data: AssessmentData = item
        .assessment_data()
        .map_err(|e| Error::BadRequest(format!("Invalid survey data: {e}")))?;

    let grade = survey_grader::grade(&assessment_data);
    ...
    context.insert("composite_score", &grade.composite_score);
    context.insert("category", &grade.category);
    context.insert(
        "category_label",
        &crate::engine::utils::category_label(&grade.category),
    );
    context.insert("domain_scores", &grade.domain_scores);
    context.insert("enps", &grade.enps);
    context.insert("answered_count", &grade.answered_count);
    context.insert("total_count", &grade.total_count);
