# Employee Satisfaction Survey — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Employee Satisfaction Survey form, built with axum + Loco +
...
- [`employee_satisfaction_survey/`](./employee_satisfaction_survey/) — the Loco crate: `src/employee_satisfaction_survey/` holds `app.rs`
...
`src/employee_satisfaction_survey/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd employee_satisfaction_survey && cargo check --all-targets && cargo test
```
