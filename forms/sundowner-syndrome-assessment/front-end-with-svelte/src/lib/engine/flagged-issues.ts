...
//            medication change, moderate carer strain, infection or pain triggers
...
import { cmaiItems } from './cmai-rules';
...
export function detectAdditionalFlags(data: AssessmentData): AdditionalFlag[] {
	...
	const cmaiAt = (n: number): number => {
		const id = `cmai${String(n).padStart(2, '0')}`;
		const v = cmai[id];
		return typeof v === 'number' ? v : 0;
	};
	const npiScore = (key: string): number => {
	...
	const aggressiveActive = AGGRESSIVE_ITEM_NUMBERS.map((n) => ({ n, v: cmaiAt(n) })).filter(
		(x) => x.v >= 4
	);
	...
	const selfHarmActive = SELF_HARM_ITEM_NUMBERS.map((n) => ({ n, v: cmaiAt(n) })).filter(
		(x) => x.v >= 3
	);
	...
	const fallActive = FALL_RISK_ITEM_NUMBERS.map((n) => ({ n, v: cmaiAt(n) })).filter((x) => x.v >= 4);
	...
	if (
		data.sleepWakeCycle.nighttimeWandering === 'yes' ||
	...
	if (
		data.medicationReview.sedativeUse === 'yes' &&
	...
	if (
		data.carerImpact.carerStrainLevel === 'moderate' &&
	...
	const priorityOrder: Record<AdditionalFlag['priority'], number> = { high: 0, medium: 1, low: 2 };
