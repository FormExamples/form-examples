# Sundowner Syndrome Assessment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Sundowner Syndrome Assessment form, built with axum + Loco +
...
- [`sundowner_syndrome_assessment/`](./sundowner_syndrome_assessment/) — the Loco crate: `src/sundowner_syndrome_assessment/` holds `app.rs`
...
`src/sundowner_syndrome_assessment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd sundowner_syndrome_assessment && cargo check --all-targets && cargo test
```
