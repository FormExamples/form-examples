...
import type { PetScanResult, GradingResult } from '#lib/engine/types.js';
import {
...
/** Builds the pdfmake document definition for the PET scan molecular-imaging report. */
export function buildPdfDocument(
	...
	return {
		...
		header: {
			text: 'PET-CT MOLECULAR-IMAGING REPORT',
			alignment: 'center',
			margin: [0, 20, 0, 0],
			fontSize: 10,
			color: '#6b7280',
		...
		footer: (currentPage: number, pageCount: number) => ({
			text: `Page ${currentPage} of ${pageCount} | Generated ${new Date(result.gradedAt).toLocaleString()} | RCR / Lugano-Deauville / PERCIST / IR(ME)R 2017`,
			alignment: 'center',
			margin: [0, 20, 0, 0],
			fontSize: 8,
			color: '#9ca3af'
		...
		content: [
			...
			sectionHeader('Examination & acquisition'),
			{
				ul: [
					`Scan type: ${scanTypeLabel(data.scanType)}`,
					`Adequacy: ${data.examinationAdequacy || 'N/A'}`,
					`Pre-injection blood glucose: ${data.bloodGlucoseMmolL !== null ? `${data.bloodGlucoseMmolL} mmol/L` : 'N/A'}`,
			...
			// Measurements
			sectionHeader('Measurements & response'),
			{
				ul: [
					`SUVmax: ${data.suvMax !== null ? data.suvMax : 'N/A'}`,
					`Largest lesion: ${data.largestLesionSizeMm !== null ? `${data.largestLesionSizeMm} mm` : 'N/A'}`,
					`Treatment response: ${treatmentResponseLabel(data.treatmentResponse)}`,
...
function structuredFindingList(data: PetScanResult): string[] {
	const list: string[] = [];
	if (data.hypermetabolicLesion)
		list.push(
			`Hypermetabolic lesion${data.suvMax !== null ? ` (SUVmax ${data.suvMax})` : ''}`
		);
