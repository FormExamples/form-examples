# Columbia Suicide Severity Rating Scale (C-SSRS) — tasks
...
- [x] Classification engine (`types.ts`, `utils.ts`, `cssrs-rules.ts`,
      `cssrs-grader.ts`, `flagged-issues.ts`) + Vitest tests.
...
- [ ] `bin/test-form columbia-suicide-severity-rating-scale` passes.
