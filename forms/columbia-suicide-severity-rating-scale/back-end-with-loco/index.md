...
Server-side Rust JSON API for the Columbia Suicide Severity Rating Scale (C-SSRS) form. Pure back-end: no HTML
rendering, no Tera templates, no HTMX, no Alpine.js, no CSS, no Lily
Design System.

The Loco crate lives in [`columbia_suicide_severity_rating_scale/`](columbia_suicide_severity_rating_scale/) — one migration, SeaORM
