...
// and the lethality thresholds. The grader (`grader.js`) evaluates them to
...
// tier. Rows here mirror the
// `columbia_suicide_severity_rating_scale_grade_rule` SQL table
// (rule_id, criterion, level, category, description).

/**
 * @typedef {import('./types.js').AssessmentData} AssessmentData
 *
 * @typedef {Object} CssrsRule
 * @property {string} id
 * @property {string} criterion   - ideation | behaviour | lethality
 * @property {number} level       - ordinal ideation level (1-5), or 0 for non-ideation rows
...
const cssrsRules = [
  ...
  },
  {
    ...
    level: 2,
    category: 'suicidal-ideation',
    description: 'Level 2 — non-specific active suicidal thoughts (no method, intent, or plan)',
  ...
  },
  {
    ...
    level: 3,
    category: 'suicidal-ideation',
    description: 'Level 3 — active ideation with any methods, no specific plan or intent',
  ...
  },
  {
    ...
    level: 4,
    category: 'suicidal-ideation',
    description: 'Level 4 — active ideation with some intent to act, without a fully worked-out plan',
  ...
  // ─── SUICIDAL BEHAVIOUR (each counts as suicidal behaviour) ──────────
  {
    ...
    description: 'Actual attempt — a potentially self-injurious act with at least some intent to die',
  ...
  },
  {
    ...
    level: 0,
    category: 'suicidal-behaviour',
    description: 'Interrupted attempt — stopped by an outside circumstance before self-harm begins',
  ...
  },
  {
    ...
    level: 0,
    category: 'suicidal-behaviour',
    description: 'Aborted / self-interrupted attempt — the person stops before beginning the act',
  ...
  },
  {
    ...
    level: 0,
    category: 'suicidal-behaviour',
    description: 'Preparatory acts or behaviour — steps taken to prepare, e.g. acquiring means or writing a note',
  ...
  },
  {
    ...
    description: 'Non-suicidal self-injury (NSSI) — self-injury without intent to die; tracked separately',
  ...
  // ─── LETHALITY (of the most recent actual attempt) ───────────────────
  {
    ...
    level: 0,
    category: 'lethality',
    description: 'High actual lethality — medical damage of the most recent attempt is 3 or greater',
    evaluate: (d) =>
  ...
  },
  {
    ...
    level: 0,
    category: 'lethality',
    description: 'High potential lethality — potential lethality is 2 (likely to have caused death)',
    evaluate: (d) =>
