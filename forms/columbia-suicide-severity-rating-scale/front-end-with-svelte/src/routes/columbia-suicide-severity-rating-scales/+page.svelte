<script lang="ts">
	...
	import { riskTierLabel, careSettingLabel } from '$lib/engine/utils';
	...
	const columns = [
		...
		{ id: 'ideationLevel', header: 'Ideation', width: 100, sort: true },
		{
			id: 'riskTier',
			header: 'Risk tier',
			width: 140,
	...
	function init(api: any) {
		...
		api.on('select-row', (ev: { id?: string | number }) => {
			if (ev?.id != null) goto(`/columbia-suicide-severity-rating-scales/${ev.id}`);
		});
	}
</script>
...
<main class="mx-auto max-w-6xl px-4 py-6">
	<div class="mb-6 flex flex-wrap items-end justify-between gap-4">
		...
		<a href="/columbia-suicide-severity-rating-scales/new" class="button" data-variant="primary"
			>New assessment</a
		>
	</div>
