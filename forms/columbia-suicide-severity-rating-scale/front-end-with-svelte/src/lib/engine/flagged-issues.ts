import type { AssessmentData, FlaggedIssue, GradeSummary } from './types';

/**
 * Detect clinician-facing safety flags (red flags), computed INDEPENDENTLY of
 * the risk tier the grader derives (spec §5):
 *
 ...
 *
 * Rows mirror the `columbia_suicide_severity_rating_scale_grade_flag` SQL table
...
export function detectFlaggedIssues(data: AssessmentData, grade: GradeSummary): FlaggedIssue[] {
	...
	if (grade.highLethality) {
		const detail =
			leth.actualLethality !== null && leth.actualLethality >= 3
				? `actual lethality ${leth.actualLethality} (significant medical damage)`
