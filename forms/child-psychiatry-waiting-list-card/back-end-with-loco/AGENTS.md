# Child Psychiatry Waiting List Card — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Child Psychiatry Waiting List Card form, built with axum + Loco +
...
- [`child_psych_waiting_list_card/`](./child_psych_waiting_list_card/) — the Loco crate: `src/child_psychiatry_waiting_list_card/` holds `app.rs`
...
**Table/database identifier note:** the full slug (`child_psychiatry_waiting_list_card`) exceeds PostgreSQL's 63-byte `NAMEDATALEN` limit once combined with index/trigger suffixes (e.g. `trigger_..._appointment_updated_at`), so table names, the database name, and FK/column names use the shortened form `child_psych_waiting_list_card` instead. The crate name, the `src/` module directory, and every human-facing name keep the full specialty name — only the low-level SQL/Rust identifier layer is shortened.
...
`/metrics`. The registered domain controllers are:
  ...
  - `child_psych_waiting_list_card_appointment`
  - `child_psych_waiting_list_card_grade`
  - `child_psych_waiting_list_card_grade_flag`
  - `child_psych_waiting_list_card_grade_rule`
  - `patient`
  - `practitioner`

## Engine

`src/child_psychiatry_waiting_list_card/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd child_psych_waiting_list_card && cargo check --all-targets && cargo test
```
