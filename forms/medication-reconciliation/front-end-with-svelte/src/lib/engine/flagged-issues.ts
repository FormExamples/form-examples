import type { FlaggedIssue, GradingResult, Priority, ReconciliationData } from './types';
...
const INTERACTION_PAIRS: { a: string; b: string; priority: Priority; note: string }[] = [
...
export function detectFlaggedIssues(data: ReconciliationData, grade: Grade): FlaggedIssue[] {
	...
	let interactionPriority: Priority = 'medium';
	...
	if (allergyStatus === 'not-documented' || allergyStatus === '') {
		flags.push({
			...
			priority: 'low',
			description: 'Neither drug allergies nor "no known drug allergies" have been recorded',
			suggestedAction:
