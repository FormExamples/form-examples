// Backend API client for the medication-reconciliation clinician dashboard.
//
...
// `DashboardReconciliationsResponse`. When the fetch fails (CORS, network,
...
/**
 * Fetch the reconciliation list from the backend.
 *
 ...
 *
 * @returns {Promise<import('./dashboard-types.js').ReconciliationRow[]>}
 */
async function fetchReconciliations() {
  const res = await fetch(`${API_BASE}${RECONCILIATIONS_PATH}`);
  if (!res.ok) {
    throw new Error(
      `Failed to fetch reconciliations: ${res.status} ${res.statusText}`
    );
  }
  /** @type {import('./dashboard-types.js').DashboardReconciliationsResponse} */
