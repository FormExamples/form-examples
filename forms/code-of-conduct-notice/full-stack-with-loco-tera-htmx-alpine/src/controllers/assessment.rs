...
use crate::engine::{
    ...
    utils::{acknowledgement_status, acknowledgement_status_label, completeness_label},
};
...
/// POST /assessment/new -- create a new draft acknowledgement, redirect to
/// the wizard.
#[debug_handler]
async fn create_new(State(ctx): State<AppContext>) -> Result<Response> {
    let item = ActiveModel::new_draft()
        .map_err(|e| Error::BadRequest(format!("Failed to create acknowledgement: {e}")))?;
    let item = item.insert(&ctx.db).await?;
    let id = item.id;
    Ok(Redirect::to(&format!("/assessment/{id}")).into_response())
}

/// GET /assessment/{id} -- render the single-page acknowledgement wizard.
#[debug_handler]
...
/// Parse a form value string. Empty strings stay as `""`; the `agreed`
/// boolean is mapped from the checkbox `"on"` / absent.
fn parse_form_value(field: &str, value: &serde_json::Value) -> serde_json::Value {
    // `agreed` is a checkbox -- present="on"/"true" → true, absent handled
    // separately at section-level by initializing to false.
    if field == "agreed" {
        if let Some(s) = value.as_str() {
            let lc = s.to_ascii_lowercase();
            return serde_json::Value::Bool(
                lc == "on" || lc == "true" || lc == "1" || lc == "yes",
            );
        }
...
async fn submit_assessment(
    ...
    if let Some(obj) = data_value.as_object_mut() {
        // Reset `agreed` to false before applying form values; an unchecked
        // checkbox sends no key/value pair, so we must default it.
        if let Some(ack) = obj
            .get_mut("acknowledgementSignature")
            .and_then(|s| s.as_object_mut())
        {
            ack.insert("agreed".to_string(), serde_json::Value::Bool(false));
        }
...
async fn show_report(
    ...
    let assessment_data: AssessmentData = item
        .assessment_data()
        .map_err(|e| Error::BadRequest(format!("Invalid acknowledgement data: {e}")))?;

    let grade = form_validator::grade(&assessment_data);
    ...
    let ack_status = acknowledgement_status(&grade.status, &assessment_data);

    let mut context = Context::new();
    context.insert("id", &id.to_string());
    context.insert("data", &assessment_data);
    context.insert("status", &grade.status);
    context.insert("completeness_percent", &grade.completeness_percent);
    context.insert(
        "completeness_label",
        &completeness_label(grade.completeness_percent),
    );
    context.insert("acknowledgement_status", &ack_status);
    context.insert(
        "acknowledgement_status_label",
        &acknowledgement_status_label(&ack_status),
    );
