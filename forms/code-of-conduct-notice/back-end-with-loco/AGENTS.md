# Code of Conduct Notice — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Code of Conduct Notice form, built with axum + Loco +
...
- [`code_of_conduct_notice/`](./code_of_conduct_notice/) — the Loco crate: `src/code_of_conduct_notice/` holds `app.rs`
...
`src/code_of_conduct_notice/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd code_of_conduct_notice && cargo check --all-targets && cargo test
```
