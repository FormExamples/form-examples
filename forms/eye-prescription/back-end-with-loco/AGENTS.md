...
- [`eye_prescription/`](./eye_prescription/) — the Loco crate: `src/eye_prescription/` holds `app.rs`
...
`src/eye_prescription/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd eye_prescription && cargo check --all-targets && cargo test
```
