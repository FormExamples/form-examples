<script lang="ts">
	...
	import { complexityLabel, lensTypeLabel } from '$lib/engine/utils';
	...
	const columns = [
		...
		{ id: 'prismFlag', header: 'Prism', width: 90, template: (v: boolean) => (v ? 'Yes' : 'No') },
		{
			id: 'referralFlag',
			header: 'Referral',
			width: 100,
	...
	function init(api: any) {
		...
		api.on('select-row', (ev: { id?: string | number }) => {
			if (ev?.id != null) goto(`/eye-prescriptions/${ev.id}`);
		});
	}
</script>
...
<main class="mx-auto max-w-6xl px-4 py-6">
	<div class="mb-6 flex flex-wrap items-end justify-between gap-4">
		...
		<a href="/eye-prescriptions/new" class="button" data-variant="primary">New prescription</a>
	</div>
