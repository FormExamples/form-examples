...
import type { EyeVisionRequest, GradingResult } from '#lib/engine/types.js';
import {
...
/** Builds the pdfmake document definition for the eye vision vetting report. */
export function buildPdfDocument(
	data: EyeVisionRequest,
	...
	return {
		...
		header: {
			text: 'EYE VISION TEST REQUEST — VETTING REPORT',
			alignment: 'center',
			margin: [0, 20, 0, 0],
			fontSize: 10,
			color: '#6b7280',
		...
		footer: (currentPage: number, pageCount: number) => ({
			text: `Page ${currentPage} of ${pageCount} | Generated ${new Date(result.gradedAt).toLocaleString()} | RCOphth / NICE NG81 / QS180`,
			alignment: 'center',
			margin: [0, 20, 0, 0],
			fontSize: 8,
			color: '#9ca3af'
		...
		content: [
			...
			{
				ul: [
					`Test type: ${testTypeLabel(data.request.testType)}`,
					`Laterality: ${lateralityLabel(data.request.laterality)}`,
					`Primary indication: ${indicationLabel(data.request.primaryIndication)}`,
			...
			// Reason and question
			sectionHeader('Clinical question and history'),
			...
			{ ul: riskFactorList(data), margin: [0, 0, 0, 16] as Margin },
...
function symptomList(data: EyeVisionRequest): string[] {
	const list: string[] = [];
...
function riskFactorList(data: EyeVisionRequest): string[] {
	const list: string[] = [];
