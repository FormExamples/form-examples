...
// Re-export the default factory so consumers can build blank / seed records.
export { createDefaultRequest };

/** localStorage draft key for a given request id (defaults to `new`). */
function storageKey(id: string): string {
	return `eye-vision-test-request.front-end-with-svelte.${id || 'new'}.v1`;
}

/**
 * Svelte 5 reactive store for the eye vision test request, with localStorage
 ...
 */
class RequestStore {
	data = $state<EyeVisionRequest>(createDefaultRequest());
	 ...
	 */
	loadForId(id: string, seed?: EyeVisionRequest) {
		const key = id || 'new';
		this.id = key;
		this.result = null;
		this.currentStep = 1;

		let draft: EyeVisionRequest | null = null;
		if (browser) {
			const raw = localStorage.getItem(storageKey(key));
			if (raw) {
				try {
					draft = JSON.parse(raw) as EyeVisionRequest;
				} catch {
