...
import type { HerniaDiagnosticEvaluation, GradingResult } from '#lib/engine/types.js';
import { calculateHerniaEvaluation } from '#lib/engine/grader.js';
...
function storageKey(id: string): string {
	return `hernia-diagnostic-evaluation.front-end-with-svelte.${id || 'new'}.v1`;
}
...
class EvaluationStore {
	...
	load(id: string): void {
		...
		if (browser) {
			try {
				...
				if (raw) {
					const parsed = JSON.parse(raw) as Partial<HerniaDiagnosticEvaluation>;
					// Merge stored values over a fresh shape so fields added in a later
					// version do not orphan an existing draft.
					for (const key of Object.keys(fresh) as Array<keyof HerniaDiagnosticEvaluation>) {
						const stored = parsed[key];
						if (stored && typeof stored === 'object') {
							Object.assign(fresh[key] as object, stored);
						} else if (typeof stored === 'string' && key === 'status') {
							fresh.status = stored as HerniaDiagnosticEvaluation['status'];
						}
					}
				}
			} catch {
	...
	private assign(next: HerniaDiagnosticEvaluation): void {
		this.data.status = next.status;
		for (const key of Object.keys(next) as Array<keyof HerniaDiagnosticEvaluation>) {
