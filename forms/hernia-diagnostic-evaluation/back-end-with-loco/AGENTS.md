# Hernia Diagnostic Evaluation — back-end-with-loco/

Rust axum + Loco JSON API for the Hernia Diagnostic Evaluation form. Crate
source under `src/hernia_diagnostic_evaluation/`; one migration and one
...
- `seed()` in `app.rs` resolves fixtures from `CARGO_MANIFEST_DIR` because the
  canonical route layout moved them to `src/hernia_diagnostic_evaluation/fixtures/`;
  do not revert it to the scaffold's `base.join(...)` form.
