<script lang="ts">
	...
	import { classificationLabel, careSettingLabel, formatGap } from '#lib/engine/utils.js';
	...
	function init(api: any) {
		...
		api.on('select-row', (ev: { id?: string | number }) => {
			if (ev?.id != null) goto(`/anion-gap-calculator/anion-gap-calculators/${ev.id}`);
		});
	}
</script>
...
<main class="mx-16 px-4 py-6">
	<div class="mb-6 flex flex-wrap items-end justify-between gap-4">
		...
		<a href="/anion-gap-calculator/anion-gap-calculators/new" class="button" data-variant="primary">New calculation</a>
	</div>
