...
function storageKey(id: string): string {
	return `anion-gap-calculator.front-end-with-svelte.${id || 'new'}.v1`;
}

/** A blank anion-gap assessment with all fields at their unanswered defaults. */
...
/**
 * Svelte 5 reactive store for the anion-gap assessment, with localStorage
 * persistence so an in-progress calculation survives a page reload. Drafts are
...
class AssessmentStore {
	 ...
	 * (e.g. `const d = assessment.data.electrolytes`) stay bound to live state.
	 */
