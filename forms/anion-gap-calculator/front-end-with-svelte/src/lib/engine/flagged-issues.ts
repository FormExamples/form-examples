import type { AssessmentData, FlaggedIssue, GradeSnapshot } from './types';
import { VERY_HIGH } from './anion-gap-rules';
...
/**
 * Detect clinician-facing safety flags (red flags), using the unrounded grader
 * snapshot (spec §5). Independent of the classification band:
 *
 ...
 *
 * Rows mirror the `anion_gap_calculator_grade_flag` SQL table
...
export function detectFlaggedIssues(
	...
	const { anionGapRaw, correctedAnionGapRaw, classificationValue, normalLow, normalHigh } = grade;

	// ─── Incomplete calculation (LOW) ───────────────────────────
	if (anionGapRaw === null || classificationValue === null) {
		...
		flags.push({
			...
			category: 'incomplete',
			priority: 'low',
			description: `Missing input(s): ${missing.join(', ')} — the anion gap cannot be computed`,
			suggestedAction:
	...
	if (classificationValue >= VERY_HIGH) {
		flags.push({
			...
			category: 'very-high',
			priority: 'urgent',
			description: `The ${basis} ${shown} is at or above the ${VERY_HIGH} mmol/L very-high threshold — marked elevation`,
			suggestedAction:
	...
	if (classificationValue > normalHigh && classificationValue < VERY_HIGH) {
		flags.push({
			...
			category: 'high-hagma',
			priority: 'high',
			description: `The ${basis} ${shown} is above the ${normalHigh} mmol/L upper reference limit — suggests a high anion gap metabolic acidosis`,
			suggestedAction:
	...
	if (usingCorrected && anionGapRaw <= normalHigh && correctedAnionGapRaw > normalHigh) {
		flags.push({
			id: 'F-HYPOALBUMINAEMIA-MASKING-001',
			category: 'hypoalbuminaemia-masking',
			priority: 'high',
			description: `The uncorrected anion gap ${roundOne(anionGapRaw).toFixed(1)} mmol/L looks normal, but the albumin-corrected gap ${roundOne(correctedAnionGapRaw).toFixed(1)} mmol/L is raised`,
			suggestedAction:
	...
	if (classificationValue < normalLow) {
		flags.push({
			...
			category: 'low',
			priority: 'medium',
			description: `The ${basis} ${shown} is below the ${normalLow} mmol/L lower reference limit`,
			suggestedAction:
