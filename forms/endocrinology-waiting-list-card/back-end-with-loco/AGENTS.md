# Endocrinology Waiting List Card — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Endocrinology Waiting List Card form, built with axum + Loco +
...
- [`endocrinology_waiting_list_card/`](./endocrinology_waiting_list_card/) — the Loco crate: `src/endocrinology_waiting_list_card/` holds `app.rs`
...
`/metrics`. The registered domain controllers are:
  ...
  - `endocrinology_waiting_list_card`
  - `endocrinology_waiting_list_card_appointment`
  - `endocrinology_waiting_list_card_grade`
  - `endocrinology_waiting_list_card_grade_flag`
  - `endocrinology_waiting_list_card_grade_rule`
  - `patient`
  - `practitioner`

## Engine

`src/endocrinology_waiting_list_card/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd endocrinology_waiting_list_card && cargo check --all-targets && cargo test
```
