<script lang="ts">
	...
	import Step4ClinicalDetail from '#lib/components/steps/Step4ClinicalDetail.svelte';
	import Step5RadiationSafety from '#lib/components/steps/Step5RadiationSafety.svelte';
	import Step6Practicalities from '#lib/components/steps/Step6Practicalities.svelte';
	...
	function validate(): boolean {
		...
		if (d.request.laterality === '') {
			found.push({ id: 'laterality', message: 'Laterality is required.' });
		}
		if (d.request.primaryIndication === '') {
			found.push({ id: 'primaryIndication', message: 'Primary indication is required.' });
		}
		if (d.detail.clinicalQuestion.trim() === '') {
	...
	function submit() {
		...
		goto(`/x-ray-test-request/x-ray-test-requests/${id}/report`);
	}
...
<main class="mx-16 px-4 py-6">
	<h1 class="text-2xl font-bold text-base-content">
		{isNew ? 'New X-ray request' : `X-ray request ${id}`}
	</h1>
