import type { XRayRequest, Flag, RadiationDoseBand } from './types';
...
const LATERALISED_REGIONS = ['hip', 'knee', 'ankle-foot', 'shoulder', 'wrist-hand'];

/**
 * Detect safety flags for a plain-radiograph (X-ray) request. Flag categories
 * mirror the sql grade_flag CHECK constraint. Flag IDs are stable and identical
 * across every front-end and the back-end.
 */
export function detectFlags(data: XRayRequest, context?: { doseBand?: RadiationDoseBand }): Flag[] {
	const flags: Flag[] = [];
	const ctx = context ?? {};
