...
// radiation-safety-band dropdown + safety-flags dropdown).
...
const columns = [
  ...
  { key: 'radiationSafetyBand', label: 'Radiation Safety' },
  { key: 'radiationDoseBand',   label: 'Dose' },
...
function safetyClass(safety) {
  return safety ? 'safety-' + String(safety) : '';
}

function doseClass(dose) {
  return dose ? 'dose-' + String(dose) : '';
}
...
function matchesFilters(row) {
  ...
  if (filters.safety && row.radiationSafetyBand !== filters.safety) return false;
...
function compareRows(a, b) {
  ...
  if (key === 'radiationSafetyBand') {
    return ((safetyRank[av] ?? -1) - (safetyRank[bv] ?? -1)) * dir;
  }
  if (key === 'radiationDoseBand') {
  ...
  // Default: string compare (nhs, patient, bodyRegion, laterality, indication,
...
function renderTableBody() {
  ...
  for (const row of rows) {
    ...
    if (row.triageTier === 'emergency' || row.radiationSafetyBand === 'contraindicated') {
    ...
    tr.innerHTML = `
      ...
      <td class="data-table-td">${esc(titleCase(row.laterality))}</td>
      ...
      <td class="data-table-td"><span class="safety-badge ${safetyClass(row.radiationSafetyBand)}">${esc(titleCase(row.radiationSafetyBand))}</span></td>
      <td class="data-table-td"><span class="dose-badge ${doseClass(row.radiationDoseBand)}">${esc(titleCase(row.radiationDoseBand))}</span></td>
...
function bindFilterInputs() {
  ...
  const safety = document.getElementById('filter-safety');
