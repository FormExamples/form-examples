# Echocardiogram Test Result — agent instructions

Echocardiogram (echo) result (report). A reporting clinician completes a
single-page wizard recording the performed study's **findings and measurements**
...
produces a structured echocardiography report.

This is the **result/report counterpart** to `echocardiogram-test-request` (a
referral).

## Result semantics

A result form records *what the test found and what it means* — it is **not** a
referral. The source-of-truth record is `echocardiogram_test_result`; the
clinician is the report **author/signer** (cardiologist, cardiac physiologist,
sonographer), not a referrer. The engine **interprets findings** rather than
vetting a request.
...
- **Axis C — report completeness:** 0–100 % of mandatory report sections
  (history, LV function, valves, pulmonary pressure, findings, impression).
- **Axis D — follow-up urgency:** routine / recommended / urgent / critical-alert,
  with a target timeframe and a recommended action.

A **critical finding** (severe valve disease, vegetation / suspected
endocarditis, large pericardial effusion or tamponade, severe LV impairment,
intracardiac thrombus) **auto-escalates** Axis D to *critical-alert* and raises
