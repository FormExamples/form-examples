<script lang="ts">
	...
	import Step3LvFunction from '$lib/components/steps/Step3LvFunction.svelte';
	import Step4ValvesPressures from '$lib/components/steps/Step4ValvesPressures.svelte';
	import Step5StructuredFindings from '$lib/components/steps/Step5StructuredFindings.svelte';
	import Step6FindingsImpression from '$lib/components/steps/Step6FindingsImpression.svelte';
	...
	function validate(): boolean {
		...
		if (d.echoType === '') {
			found.push({ id: 'echoType', message: 'Echo type is required.' });
		}
		if (d.reportStatus === '') {
	...
	function submit() {
		...
		goto('/echocardiogram-test-result/report/view');
	}
...
<main class="mx-auto max-w-3xl px-4 py-6">
	<header class="mb-6 no-print">
		<h1 class="text-2xl font-bold text-gray-900">New echocardiogram report</h1>
	...
	<Form label="Echocardiogram test result" onsubmit={submit}>
		<Step1ReportIdentification />
		<Step2ClinicalHistory />
		<Step3LvFunction />
		<Step4ValvesPressures />
		<Step5StructuredFindings />
		<Step6FindingsImpression />
