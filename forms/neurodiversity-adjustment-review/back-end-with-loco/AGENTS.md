...
workplace **reasonable-adjustments effectiveness review** for neurodiversity —
not a clinical form. No HTML, Tera, HTMX, Alpine.js, or CSS.
...
- The four-axis engine (`src/engine/`) is a pure, side-effect-free port of the
  SvelteKit / HTML engines with **identical** rule IDs (`R-EFFECT-*`,
  `R-WELL-*`, `R-COMPLETE-*`, `R-NEXT-*`), flag IDs (`F-*`), bands, and fire
  conditions — see the canonical engine spec and the sibling
  `../../neurodiversity-adjustment-response`. Do not diverge the IDs between
  stacks.
