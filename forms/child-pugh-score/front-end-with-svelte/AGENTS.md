...
for the Child-Pugh specification and scoring engine. Lily Svelte headless
conventions: [`../../AGENTS-front-end-svelte.md`](../../AGENTS-front-end-svelte.md).

## Layout

- `src/lib/engine/` — pure scoring engine (`types.ts`, `utils.ts`,
  `child-pugh-rules.ts`, `child-pugh-grader.ts`, `flagged-issues.ts`) +
  `child-pugh-grader.test.ts`.
...
- `src/routes/child-pugh-scores/` — RESTful routes:
  `/<plural>/` (dashboard, `ssr = false`) + `/<plural>/[id]` (wizard) +
  `/<plural>/[id]/report` (+ `report/pdf` server endpoint).

## Scoring engine

Additive class-band score: five parameters — total bilirubin, serum albumin,
coagulation (INR preferred; prothrombin-time prolongation fallback), ascites, and
hepatic encephalopathy — each award 1, 2, or 3 points against the thresholds in
spec §4. The total 5-15 bands into Class A (5-6), B (7-9), or C (10-15), each
carrying fixed survival and surgical-risk estimates. A missing parameter is
excluded from a partial total and raises a data-completeness flag; the total is
only valid once all five parameters are answered.
