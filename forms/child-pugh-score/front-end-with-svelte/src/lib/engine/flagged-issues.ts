 ...
 *
 * Rows mirror the `child_pugh_score_grade_flag` SQL table (flag_id, category,
 * priority, description, suggested_action). The surgical-risk and
 * decompensation flags require a complete assessment so that a partial total is
 * never mistaken for a final class.
 */
export function detectFlaggedIssues(_data: AssessmentData, grade: GradingResult): FlaggedIssue[] {
	...
	if (childPughClass === 'C' && complete) {
		...
		flags.push({
			...
			priority: 'high',
			description:
				'Class C signals advanced liver disease — liver-transplant assessment may be appropriate.',
			suggestedAction:
	...
	if (childPughClass === 'C' && complete) {
		flags.push({
			...
			priority: 'high',
			description:
				'High peri-operative mortality (~80%) — elective surgery carries prohibitive risk in Class C.',
			suggestedAction:
		...
		flags.push({
			...
			priority: 'medium',
			description:
				'Moderate peri-operative mortality (~30%) — elective surgery carries raised risk in Class B.',
			suggestedAction:
