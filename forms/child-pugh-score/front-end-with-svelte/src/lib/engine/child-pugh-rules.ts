...
/**
 * Declarative Child-Pugh scoring helpers, class banding, and rule table.
 *
 ...
 * (spec §4). The grader (`child-pugh-grader.ts`) sums the five points into a
 * total of 5-15 and bands it into a class (A/B/C) with survival and
 ...
 * separately in `flagged-issues.ts`.
 */

/** Parameter 1 — total bilirubin (µmol/L). Returns 1-3, or null when unmeasured. */
export function bilirubinPoints(d: AssessmentData): ParameterPoint {
...
export function albuminPoints(d: AssessmentData): ParameterPoint {
 ...
 */
export function coagulationPoints(d: AssessmentData): ParameterPoint {
...
export function ascitesPoints(d: AssessmentData): ParameterPoint {
...
export function encephalopathyPoints(d: AssessmentData): ParameterPoint {
...
/** The prognostic estimates fixed by a Child-Pugh class band. */
 ...
 */
export function classBand(score: number): ClassBand {
	if (score <= 6) {
		return {
			childPughClass: 'A',
			oneYearSurvival: '~100%',
 ...
 */
export const childPughRules: ChildPughRule[] = [
