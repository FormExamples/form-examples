# Hematology Waiting List Card — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Hematology Waiting List Card form, built with axum + Loco +
...
- [`hematology_waiting_list_card/`](./hematology_waiting_list_card/) — the Loco crate: `src/hematology_waiting_list_card/` holds `app.rs`
...
`/metrics`. The registered domain controllers are:
  ...
  - `hematology_waiting_list_card`
  - `hematology_waiting_list_card_appointment`
  - `hematology_waiting_list_card_grade`
  - `hematology_waiting_list_card_grade_flag`
  - `hematology_waiting_list_card_grade_rule`
  - `patient`
  - `practitioner`

## Engine

`src/hematology_waiting_list_card/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd hematology_waiting_list_card && cargo check --all-targets && cargo test
```
