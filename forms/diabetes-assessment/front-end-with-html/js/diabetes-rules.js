...
//
// Concern levels: high > medium > low (low rules are positive indicators).

/**
 * @typedef {import('./types.js').AssessmentData} AssessmentData
 * @typedef {import('./types.js').FiredRule} FiredRule
 *
 * @typedef {Object} DiabetesRule
 * @property {string} id
...
/**
 * Evaluate every rule and return the list that fired.
...
function evaluateRules(data) {
  ...
  for (const rule of allRules) {
    try {
      ...
      console.warn(`Diabetes rule ${rule.id} evaluation failed:`, e);
    }
  }
  return fired;
}
