...
//      (`ed-triage-grader.ts`) sums these into a supporting NEWS2 aggregate.
//
//   2. Manchester Triage System (MTS) general discriminators — each maps a
//      recorded finding to a MINIMUM MTS priority level (1 = most urgent). This
//      is a CLASSIFICATION, not an additive score: the grader selects the most
//      urgent level any discriminator forces.
//
// The band tables here mirror the SQL `..._grade_rule` instrument rows.

import type { AirOrOxygen, Acvpu, Discriminators, PriorityLevel } from './types';
 ...
 */
export function scoreSpo2(spo2: number | null): 0 | 1 | 2 | 3 | null {
...
// evaluates derived discriminators from the vital signs and pain score (ACVPU
// U/V/P, SpO2, pain bands) — see `deriveDiscriminators` in the grader.

/** A boolean MTS discriminator flag definition. */
export interface DiscriminatorFlag {
	id: string;
	field: keyof Discriminators;
	level: PriorityLevel;
	category: string;
	description: string;
}
...
export const DISCRIMINATOR_FLAGS: DiscriminatorFlag[] = [
	{
		id: 'D-AIRWAY-THREAT',
		field: 'airwayThreat',
		level: 1,
		category: 'airway',
		description: 'Threatened or compromised airway'
	},
	{
		id: 'D-BREATHING-INADEQUATE',
		field: 'breathingInadequate',
		level: 1,
		category: 'breathing',
		description: 'Severely inadequate breathing / respiratory failure'
	},
	{
		id: 'D-CIRCULATION-SHOCK',
		field: 'circulationShock',
		level: 1,
		category: 'circulation',
		description: 'Shock or circulatory compromise'
	},
	{
		id: 'D-HAEMORRHAGE-MAJOR',
		field: 'haemorrhageMajor',
		level: 1,
		category: 'circulation',
		description: 'Major or catastrophic haemorrhage'
	},
	{
		id: 'D-SEIZURE-ACTIVE',
		field: 'seizureActive',
	...
	{
		id: 'D-CONSCIOUSNESS-REDUCED',
		field: 'consciousnessReduced',
		level: 2,
		category: 'disability',
		description: 'Reduced consciousness (responds to voice or pain)'
	},
	{
		id: 'D-FOCAL-NEUROLOGY',
		field: 'focalNeurology',
		level: 2,
		category: 'disability',
		description: 'Acute focal neurological deficit'
	},
	{
		id: 'D-STROKE-FEATURES',
	...
	{
		id: 'D-SEPSIS-FEATURES',
		field: 'sepsisFeatures',
		level: 2,
		category: 'temperature',
		description: 'Features suggestive of sepsis (sepsis screen)'
	},
	{
		id: 'D-PAEDIATRIC-RED-FLAG',
		field: 'paediatricRedFlag',
		level: 2,
		category: 'disability',
		description: 'Paediatric red flag (time-critical)'
	}
];
