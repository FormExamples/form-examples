...
// `AssessmentData` object, compute the supporting NEWS2 aggregate from the vital
// signs, evaluate the Manchester Triage System (MTS) discriminators, and assign
// the MTS priority level — the MOST URGENT (lowest-numbered) level any finding
// justifies.
//
...
import { priorityColour, priorityName, targetMinutes } from './utils';
import { detectFlaggedIssues } from './flagged-issues';

/** The six scored physiological parameters (oxygen weighting excluded). */
const RED_SCORE_KEYS: (keyof Subscores)[] = [
	'respiratoryRate',
	'spo2',
	'systolicBp',
	'pulse',
...
export function news2Aggregate(s: Subscores): number {
	return (
		(s.respiratoryRate || 0) +
		(s.spo2 || 0) +
		(s.oxygen || 0) +
 ...
 * forces.
 */
export function deriveDiscriminators(data: AssessmentData): FiredDiscriminator[] {
	const fired: FiredDiscriminator[] = [];
	...
	if (acvpu === 'U') {
		fired.push({
			id: 'D-ACVPU-UNRESPONSIVE',
			category: 'disability',
			level: 1,
			description: 'Unresponsive on ACVPU'
		});
		...
		fired.push({
			...
			category: 'disability',
			level: 2,
			description: `Reduced consciousness on ACVPU (${acvpu})`
		});
	}
	...
	if (spo2 !== null && spo2 !== undefined && spo2 < 92) {
		fired.push({
			...
			category: 'breathing',
			level: 2,
			description: `Oxygen saturation ${spo2}% (< 92%)`
		});
	}
	...
	if (pain !== null && pain !== undefined) {
		if (pain >= 7) {
			...
			fired.push({
				...
				category: 'pain',
				level: 3,
				description: `Moderate pain (score ${pain}/10)`
			});
		}
	}
 ...
 */
export function news2Escalation(
	aggregate: number,
	anyParameterThree: boolean
): FiredDiscriminator[] {
	const fired: FiredDiscriminator[] = [];
...
export function triage(data: AssessmentData): TriageResult {
	...
	// the category.
	let priorityLevel: PriorityLevel = 4;
	if (firedDiscriminators.length > 0) {
		priorityLevel = firedDiscriminators.reduce<PriorityLevel>(
			(min, d) => (d.level < min ? d.level : min),
			5
		);
