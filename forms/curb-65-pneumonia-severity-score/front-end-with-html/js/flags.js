// Flagged-issue detection (red flags). Independent of the numeric band (which
...
//
// Rows here mirror the `curb_65_pneumonia_severity_score_grade_flag` SQL table
...
function detectFlaggedIssues(data, grade) {
  ...
  const totalScore = grade && typeof grade.totalScore === 'number'
    ? grade.totalScore
    : 0;
  const variantLabel = grade && grade.scoreVariant === 'crb-65'
    ? 'CRB-65' : 'CURB-65';
  ...
  if (missing.length > 0) {
    flags.push({
      ...
      suggestedAction:
        'Record the missing input(s) and re-score.'
    });
  }
