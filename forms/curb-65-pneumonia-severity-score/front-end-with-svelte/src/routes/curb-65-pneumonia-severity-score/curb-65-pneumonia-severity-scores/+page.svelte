<script lang="ts">
	...
	import { riskBandLabel, scoreVariantLabel, careSettingLabel } from '#lib/engine/utils.js';
	...
	// SVAR DataGrid columns. Score, variant, risk band, and admit render through
	...
	const columns = [
		...
		{ id: 'totalScore', header: 'Score', width: 80, sort: true },
		{
			id: 'scoreVariant',
			header: 'Variant',
			width: 100,
			sort: true,
			template: (v: string) => scoreVariantLabel(v as never)
		},
		{
			id: 'riskBand',
			header: 'Risk band',
	...
	function init(api: any) {
		...
		api.on('select-row', (ev: { id?: string | number }) => {
			if (ev?.id != null) goto(`/curb-65-pneumonia-severity-score/curb-65-pneumonia-severity-scores/${ev.id}`);
		});
	}
</script>
...
<main class="mx-16 px-4 py-6">
	<div class="mb-6 flex flex-wrap items-end justify-between gap-4">
		<div>
			<h1 class="text-2xl font-bold text-base-content">CURB-65 clinician dashboard</h1>
			<p class="mt-1 text-sm text-base-content/70">
				Severity score, risk band, and disposition for assessed patients, computed by the shared
				engine. Select a row to open the assessment.
			</p>
		</div>
		<a href="/curb-65-pneumonia-severity-score/curb-65-pneumonia-severity-scores/new" class="button" data-variant="primary"
			>New assessment</a
		>
	</div>
