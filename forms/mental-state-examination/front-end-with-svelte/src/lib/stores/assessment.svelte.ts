...
function storageKey(id: string): string {
	return `mental-state-examination.front-end-with-svelte.${id || 'new'}.v1`;
}

/** A blank MSE assessment with all fields at their unanswered defaults. */
...
/**
 * Svelte 5 reactive store for the MSE assessment, with localStorage persistence
 * so an in-progress examination survives a page reload. Drafts are keyed by
...
class AssessmentStore {
	 ...
	 * (e.g. `const d = assessment.data.appearance`) stay bound to live state.
	 */
