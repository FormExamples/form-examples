import type { AssessmentData, CompletenessStatus, FlaggedIssue } from './types';

/**
 * Detect the clinician-facing safety flags raised by the examination findings.
 * Computed INDEPENDENTLY of the completeness status (which the grader
 * produces), each flag carries a priority; the highest priority present
 * determines the risk level in `mse-grader.ts` (spec §5).
 *
 ...
 *   - Lack of insight with risk (mod.)  — insight none + any flag, or refuses + a mod/high flag
 ...
 *
 * Rows mirror the `mental_state_examination_grade_flag` SQL table
...
export function detectFlaggedIssues(
	...
	const activeSuicidal =
		t.suicidalIdeation === 'active-no-plan' || t.suicidalIdeation === 'active-with-plan';
	const passiveSuicidal = t.suicidalIdeation === 'passive';
	const homicidal = t.homicidalIdeation === 'thoughts' || t.homicidalIdeation === 'intent';
	...
	const anyHighIdeation = activeSuicidal || homicidal || command || recentSelfHarm;
	...
	const anyModerateOrHigh = flags.some((f) => f.priority === 'high' || f.priority === 'moderate');
	if (
	...
	// ─── Low mood (LOW) ────────────────────────────────────────
	const anyIdeation =
		activeSuicidal || homicidal || passiveSuicidal || thoughtsSelfHarm || recentSelfHarm;
	if (e.moodDescriptor === 'low' && !anyIdeation) {
		flags.push({
			...
			suggestedAction: 'Screen for depression severity and safety; arrange appropriate follow-up.'
		});
	}
	...
	if (status === 'partial') {
		flags.push({
			...
			suggestedAction: 'Complete the outstanding domains so the mental state record is whole.'
		});
	}

	// Sort: high > moderate > low.
	const priorityOrder: Record<string, number> = { high: 0, moderate: 1, low: 2 };
