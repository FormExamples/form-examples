<script lang="ts">
	...
	function submit() {
		...
		assessment.result = calculateMseGrade(assessment.data);
		goto(`/mental-state-examination/mental-state-examinations/${id}/report`);
	}
...
<main class="mx-16 px-4 py-6">
	<h1 class="text-2xl font-bold text-base-content">
		{isNew ? 'New MSE examination' : `MSE examination ${id}`}
	</h1>
	<p class="mt-1 text-sm text-base-content/70">
		Complete the ten sections; completeness and risk are computed on submit. This is a
		documentation instrument — there is no numeric score.
	</p>
	...
	<Form label="MSE examination" onsubmit={submit}>
		<Step1Context />
		<Step2Identification />
		<Step3Appearance />
		<Step4Speech />
