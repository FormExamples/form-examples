...
/// GET /dashboard -- render the BLS training dashboard with filters.
#[debug_handler]
async fn dashboard(
    ...
    if let Some(ref term) = params.search {
        ...
        items.retain(|row| {
            row.trainee_first_name.to_lowercase().contains(&term)
                || row.trainee_last_name.to_lowercase().contains(&term)
                || row.trainee_id.to_lowercase().contains(&term)
    ...
    if let Some(ref role) = params.role {
        if !role.is_empty() {
            items.retain(|row| row.trainee_role == *role);
        }
    }
