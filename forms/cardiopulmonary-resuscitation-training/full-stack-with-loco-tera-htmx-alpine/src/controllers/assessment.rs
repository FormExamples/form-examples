...
use crate::engine::{bls_grader, types::AssessmentData, utils};
...
/// GET /assessment/{id} -- render the single-page BLS wizard.
#[debug_handler]
...
/// JSON numbers so the typed engine deserializes them as `Option<f64>`.
...
async fn show_report(
    ...
    let grade = bls_grader::grade(&assessment_data);
    ...
    context.insert("non_critical_deficiencies", &grade.non_critical_deficiencies);
