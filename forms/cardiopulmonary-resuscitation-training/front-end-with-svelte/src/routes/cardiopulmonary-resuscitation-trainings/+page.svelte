<script lang="ts">
	...
	import { outcomeLabel, roleLabel } from '$lib/engine/utils';
	...
	function init(api: any) {
		...
		api.on('select-row', (ev: { id?: string | number }) => {
			if (ev?.id != null) goto(`/cardiopulmonary-resuscitation-trainings/${ev.id}`);
		});
	}
</script>
...
<main class="mx-auto max-w-6xl px-4 py-6">
	<div class="mb-6 flex flex-wrap items-end justify-between gap-4">
		...
		<a href="/cardiopulmonary-resuscitation-trainings/new" class="button" data-variant="primary">New assessment</a>
	</div>
