# Cardiopulmonary Resuscitation Training — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Cardiopulmonary Resuscitation Training form, built with axum + Loco +
...
- [`cardiopulmonary_resuscitation_training/`](./cardiopulmonary_resuscitation_training/) — the Loco crate: `src/cardiopulmonary_resuscitation_training/` holds `app.rs`
...
`src/cardiopulmonary_resuscitation_training/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd cardiopulmonary_resuscitation_training && cargo check --all-targets && cargo test
```
