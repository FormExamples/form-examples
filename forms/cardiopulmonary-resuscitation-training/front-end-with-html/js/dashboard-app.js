...
// dropdown + instructor dropdown).
//
// Sibling modules loaded as plain `<script>` tags (in dependency order)
// attach their exports to
// `window.CardiopulmonaryResuscitationTrainingDashboard`. Pulling them off
...
// Default sort: failed first, then by certification expiry ascending so
// the most-overdue / soonest-to-expire records are at the top of the list.
// Implemented as a virtual `default` sort key (see `compareRows`).
const sortState = {
  key: 'default',
...
// the row-cell renderer below. `sortable: false` means the header text
// renders without a sort button (e.g. for free-text columns where ranking
// adds no value).
const columns = [
  ...
  { key: 'compressionRate',       label: 'Comp. Rate (cpm)' },
  { key: 'compressionDepthCm',    label: 'Depth (cm)' },
...
// Stable, alphabetical role ordering. Defined explicitly so the order is
...
function matchesFilters(row) {
  if (filters.search) {
    ...
    const matches =
      ...
      row.instructorName.toLowerCase().includes(term);
 ...
 * then expired-soonest" ordering.
 */
function compareRows(a, b) {
  const key = sortState.key;
  const dir = sortState.direction === 'asc' ? 1 : -1;

  // Default sort: Fail outcomes first, then ascending by days-until-expiry
  // (so the soonest-to-expire / already-expired records bubble up).
  if (key === 'default') {
  ...
  if (key === 'compressionRate' || key === 'compressionDepthCm') {
  ...
  // Default: string compare (traineeId, traineeName, instructorName,
  // trainingCentre).
...
function renderTableBody() {
  ...
  for (const row of rows) {
    ...
    const criticalFlag = row.criticalActionFailure
      ? '<span class="critical-flag" title="Critical action failure (compressions, ventilations, or AED safety)">Critical</span>'
      : '';
...
/**
 * Populate the instructor <select> with the unique set of instructors
 * present in the loaded data. Preserves the user's current selection if
 * the instructor still exists; otherwise resets to "All instructors".
 */
function renderInstructorOptions() {
  const sel = /** @type {HTMLSelectElement | null} */ (
    document.getElementById('filter-instructor')
  );
  if (!sel) return;

  const previous = filters.instructor;
  const names = Array.from(new Set(trainees.map((t) => t.instructorName)))
    .filter(Boolean)
...
function bindFilterInputs() {
  ...
  const instructor = document.getElementById('filter-instructor');
  ...
  if (instructor) {
    instructor.addEventListener('change', () => {
      filters.instructor = instructor.value;
      renderAll();
    });
  }
  if (clearBtn) {
