...
//
// Critical actions (per AHA): any 'no' on a rule with critical=true forces
// an overall Fail. Non-critical rules contribute to the deficiency count
// (>2 deficiencies → Fail).
//
// Numeric measurements (compression rate and depth) are also surfaced as
// rules: the rule fires 'no' when a numeric value is recorded outside the
// AHA range, 'yes' when it's in range, and 'na' otherwise.

/**
 * @typedef {import('./types.js').AssessmentData} AssessmentData
 * @typedef {import('./types.js').TriState} TriState
 *
 * @typedef {Object} BLSRule
 * @property {string} id
...
const blsRules = [
  ...
  },
  {
    ...
    defaultExpectation: 'yes',
    evaluate: (d) => {
      // If examiner has explicitly recorded the tri-state field, use it.
