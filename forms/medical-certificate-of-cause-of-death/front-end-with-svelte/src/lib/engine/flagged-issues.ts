import type { DeathCertificate, FlaggedIssue } from './types';
import {
...
import { coronerReasonLabel } from './utils';

/**
 * Detect certifier- and examiner-facing statutory, safety, and governance flags,
 * computed INDEPENDENTLY of the validity class (spec §5):
 *
 ...
 *
 * Rows mirror the `medical_certificate_of_cause_of_death_grade_flag` SQL table.
 * NOTHING here diagnoses or discharges a statutory duty; the flags support the
 * certifying doctor's and medical examiner's judgement.
 */
export function detectFlaggedIssues(d: DeathCertificate): FlaggedIssue[] {
	...
	if (coronerReferralIndicated(d)) {
		...
		const reasonText =
			reason && reason !== 'none' ? ` (${coronerReasonLabel(reason)})` : '';
		flags.push({
			id: 'F-CORONER-REFERRAL-REQUIRED-001',
			category: 'coroner-referral-required',
			priority: 'high',
			description: `A coroner-referral criterion is asserted${reasonText}. The MCCD should not be issued until the coroner has considered the case.`,
			suggestedAction:
	...
	if (unacceptableSoleCause(d)) {
		flags.push({
			...
			priority: 'high',
			description: `The only cause given ("${soleCause(d).trim()}") is a recognised mode of death, not a disease or injury. A mode of death is unacceptable as the sole cause.`,
			suggestedAction:
	...
	if (missingPartIa(d)) {
		flags.push({
			...
			suggestedAction: 'Record the disease or condition directly leading to death on Part I(a).'
		});
	}
	...
	if (missingIntervalLines.length > 0) {
		flags.push({
			...
			priority: 'low',
			description: `A completed cause line has no onset-to-death interval: ${missingIntervalLines.join(', ')}. Each cause line should carry an approximate interval.`,
			suggestedAction:
	...
	if (missingCertifier.length > 0) {
		flags.push({
			...
			priority: 'low',
			description: `Certifying-doctor details are incomplete: ${missingCertifier.join(', ')} not recorded.`,
			suggestedAction:
