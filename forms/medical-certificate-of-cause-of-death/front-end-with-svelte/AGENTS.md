...
the derived underlying cause, a coroner-referral-indicated flag, and a set of
flagged issues. There is **no numeric score**, and it does **not diagnose** or
replace statutory judgement.
...
`validateCertificate(data)` evaluates the coroner-referral criteria, Part I
completeness, and cause acceptability, assigns one validity class (precedence
`refer-to-coroner` > `incomplete` > `valid`), derives the underlying cause, and
calls `detectFlaggedIssues`. All functions are pure — no I/O, no diagnosis.
