# Paediatrics Waiting List Card — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Paediatrics Waiting List Card form, built with axum + Loco +
...
- [`paediatrics_waiting_list_card/`](./paediatrics_waiting_list_card/) — the Loco crate: `src/paediatrics_waiting_list_card/` holds `app.rs`
...
`/metrics`. The registered domain controllers are:
  ...
  - `paediatrics_waiting_list_card`
  - `paediatrics_waiting_list_card_appointment`
  - `paediatrics_waiting_list_card_grade`
  - `paediatrics_waiting_list_card_grade_flag`
  - `paediatrics_waiting_list_card_grade_rule`
  - `patient`
  - `practitioner`

## Engine

`src/paediatrics_waiting_list_card/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd paediatrics_waiting_list_card && cargo check --all-targets && cargo test
```
