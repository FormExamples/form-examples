// The 97-checkpoint catalogue for the hospital daily monitoring checklist,
// transcribed verbatim from ../../../../spec/index.md (the canonical source).
// Item catalogue is data, not TypeScript fields — see AGENTS.md — because 97
// items is well past the point where per-item TypeScript fields stay
// maintainable. Mirrors the agile-checklist convention (57 items, same
// generic-map pattern).

export interface ChecklistItemDef {
  /** Stable dotted identifier from spec/index.md, e.g. '1.1', '6.1.1', '20.10', or a bare section number such as '15'. */
  id: string;
  /** Top-level area number, 1-22. */
  section: number;
  /** Top-level area title, e.g. 'OPD', 'Causality', 'Diagnostic Facility'. */
  sectionTitle: string;
  /** Sub-heading within a section, when the source proforma nests one, e.g. '6.1 Pathology Lab'. */
  subsection?: string;
...
export const TOTAL_ITEMS = CHECKLIST_ITEMS.length; // 97

/** The 22 top-level areas, in catalogue order, derived from CHECKLIST_ITEMS. */
export const SECTIONS: { number: number; title: string }[] = (() => {
  const seen = new Map<number, string>();
  for (const item of CHECKLIST_ITEMS) {
    if (!seen.has(item.section)) seen.set(item.section, item.sectionTitle);
  }
...
export const TOTAL_SECTIONS = SECTIONS.length; // 22

export function itemsForSection(section: number): ChecklistItemDef[] {
  return CHECKLIST_ITEMS.filter((i) => i.section === section);
}

export function itemById(id: string): ChecklistItemDef | undefined {
  return CHECKLIST_ITEMS.find((i) => i.id === id);
}

export function sectionTitleFor(section: number): string {
  return SECTIONS.find((s) => s.number === section)?.title ?? '';
}
