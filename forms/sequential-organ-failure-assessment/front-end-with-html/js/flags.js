...
//
// Rows here mirror the `sequential_organ_failure_assessment_grade_flag` SQL
...
function detectFlaggedIssues(data, grade) {
  ...
  const failing3plus = systems.filter((s) => subScores[s] !== null && subScores[s] >= 3);
  const missing = systems.filter((s) => subScores[s] === null);

  // ─── Severe single-organ failure (HIGH) ─────────────────────
  if (failing4.length > 0) {
    const names = failing4.map(systemLabel).join(', ');
    flags.push({
  ...
  if (failing3plus.length >= 2) {
    const names = failing3plus.map(systemLabel).join(', ');
    flags.push({
  ...
  if (missing.length > 0) {
    const names = missing.map(systemLabel).join(', ');
    flags.push({
      ...
      suggestedAction:
        'Record the missing physiology / laboratory value(s) and re-score.'
    });
  }
