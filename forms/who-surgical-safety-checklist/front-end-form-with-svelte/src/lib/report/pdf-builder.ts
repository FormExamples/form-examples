...
/**
 * Build a pdfmake document definition for a completed WHO Surgical Safety
 * Checklist. The caller is responsible for handing this to `pdfMake.createPdf`
 * in the browser.
 */
export function buildPdfDoc(
  ...
  const rows = (title: string, lines: string[]) => [
    { text: title, style: 'sectionHeader', margin: [0, 10, 0, 4] },
    ...lines
      .filter((l) => l && l.length > 0)
      .map((l) => ({ text: l, margin: [0, 0, 0, 2] })),
  ];
  ...
  return {
    content: [
      ...
      ...rows('Operating-team roster', [
        ...(data.teamMembers.length === 0
          ? ['No team members recorded.']
          : data.teamMembers.map(
              (m) =>
                `${m.name || '—'} (${m.role || '—'}) — introduced: ${m.introducedDuringTimeOut || '—'}`,
            )),
      ]),
