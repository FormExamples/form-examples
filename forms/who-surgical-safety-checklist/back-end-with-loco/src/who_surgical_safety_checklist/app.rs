...
impl Hooks for App {
    ...
    async fn seed(ctx: &AppContext, _base: &Path) -> Result<()> {
        ...
        loco_rs::db::seed::<crate::models::_entities::clinicians::ActiveModel>(
            &ctx.db,
            &format!("{}/src/who_surgical_safety_checklist/fixtures/clinicians.yaml", env!("CARGO_MANIFEST_DIR")),
        )
        .await?;
        loco_rs::db::seed::<crate::models::_entities::patients::ActiveModel>(
            &ctx.db,
            &format!("{}/src/who_surgical_safety_checklist/fixtures/patients.yaml", env!("CARGO_MANIFEST_DIR")),
        )
        .await?;
