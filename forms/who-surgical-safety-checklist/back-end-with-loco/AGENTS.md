# WHO Surgical Safety Checklist — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the WHO Surgical Safety Checklist form, built with axum + Loco +
...
- [`who_surgical_safety_checklist/`](./who_surgical_safety_checklist/) — the Loco crate: `src/who_surgical_safety_checklist/` holds `app.rs`
...
`src/who_surgical_safety_checklist/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd who_surgical_safety_checklist && cargo check --all-targets && cargo test
```
