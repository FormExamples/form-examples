...
use crate::engine::{checklist_grader, types::AssessmentData};
...
/// POST /assessment/new -- create a new draft checklist, redirect to the wizard.
#[debug_handler]
...
async fn submit_assessment(
    ...
    if let Some(obj) = data_value.as_object_mut() {
        if let Some(form_obj) = form_data.as_object() {
            // Collect dynamic team_member[idx].field entries.
            let mut team_members: std::collections::BTreeMap<
                usize,
                serde_json::Map<String, serde_json::Value>,
            > = std::collections::BTreeMap::new();

            for (key, value) in form_obj {
                // team_members[0].name -- dynamic team-member roster rows.
                if key.starts_with("team_members[") {
            ...
            // Materialize team_members as an array (drop empty rows).
            if !team_members.is_empty() {
                let items: Vec<serde_json::Value> = team_members
                    .values()
                    .filter(|entry| {
                        entry
                            .get("name")
                    ...
                    .collect();
                obj.insert("teamMembers".to_string(), serde_json::Value::Array(items));
            }
        }
    }
...
async fn show_report(
    ...
    let grade = checklist_grader::grade(&assessment_data);
    ...
    context.insert("checklist_status", &grade.checklist_status);
    context.insert(
        "checklist_label",
        &crate::engine::utils::checklist_status_label(&grade.checklist_status),
    );
    context.insert("sign_in_progress", &grade.sign_in_progress);
    context.insert("time_out_progress", &grade.time_out_progress);
    context.insert("sign_out_progress", &grade.sign_out_progress);
