# First Responder Assessment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the First Responder Assessment form, built with axum + Loco +
...
- [`first_responder_assessment/`](./first_responder_assessment/) — the Loco crate: `src/first_responder_assessment/` holds `app.rs`
...
`src/first_responder_assessment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd first_responder_assessment && cargo check --all-targets && cargo test
```
