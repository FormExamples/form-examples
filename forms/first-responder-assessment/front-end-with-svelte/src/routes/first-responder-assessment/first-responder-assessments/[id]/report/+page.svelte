<script lang="ts">
	...
	$effect(() => {
		if (!assessment.result) {
			goto(`/first-responder-assessment/first-responder-assessments/${id}`);
		}
	});
	...
	async function downloadPDF() {
		pdfError = '';
		try {
			const res = await fetch(`/first-responder-assessments/${id}/report/pdf`, {
				method: 'POST',
			...
			if (res.ok) {
				...
				a.href = url;
				a.download = `first-responder-assessment-${data.demographics.lastName || id}.pdf`;
				a.click();
...
{#if result}
	<header class="border-b border-base-300 bg-base-100 shadow-sm no-print">
		<div class="mx-auto flex max-w-4xl items-center justify-between px-4 py-4">
			<h1 class="text-lg font-bold text-base-content">First responder assessment report</h1>
			<div class="flex items-center gap-3">
				...
				<Button data-variant="secondary" onclick={() => goto(`/first-responder-assessment/first-responder-assessments/${id}`)}>Edit</Button>
			</div>
		</div>
	</header>
	...
	<main class="mx-auto max-w-4xl px-4 py-6">
		...
		<div class="mb-6 rounded-xl border-2 p-6 text-center {fitnessDecisionColor(result.overallFitness)}">
			...
			<div class="mt-2 flex justify-center gap-6 text-sm">
				<span>Overall competency: {competencyLabel(result.overallCompetency)}</span>
				<span>{riskLevelLabel(result.overallRisk)}</span>
			</div>
		...
		<div class="mb-6 rounded-xl border border-base-300 bg-base-100 p-6">
			<h2 class="mb-4 text-lg font-bold text-base-content">Domain competency levels</h2>
			<div class="grid grid-cols-1 gap-3 text-sm sm:grid-cols-2">
				{#each Object.entries(result.domainLevels) as [domain, level] (domain)}
					<div class="flex items-center justify-between rounded-lg border border-base-300 px-3 py-2">
						<span class="font-medium text-base-content/80">{domainNames[domain] ?? domain}</span>
						<span class="font-semibold text-base-content">{competencyLabel(level)}</span>
					</div>
				{/each}
			</div>
		</div>
		...
		{#if result.firedRules.length > 0}
			<div class="mb-6 rounded-xl border border-base-300 bg-base-100 p-6">
				<h2 class="mb-4 text-lg font-bold text-base-content">Competency assessment justification</h2>
		...
		<!-- Responder summary -->
		<div class="mb-6 rounded-xl border border-base-300 bg-base-100 p-6">
			<h2 class="mb-4 text-lg font-bold text-base-content">Responder summary</h2>
			<div class="grid grid-cols-1 gap-4 text-sm sm:grid-cols-2">
				...
				<div><span class="font-medium text-base-content/70">Role:</span> {data.roleQualifications.roleType || 'N/A'}</div>
				<div><span class="font-medium text-base-content/70">Years of service:</span> {data.roleQualifications.yearsOfService ?? 'N/A'}</div>
			</div>
		</div>
	</main>
{/if}
