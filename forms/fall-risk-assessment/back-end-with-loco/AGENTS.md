# Fall Risk Assessment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Fall Risk Assessment form, built with axum + Loco +
...
- [`fall_risk_assessment/`](./fall_risk_assessment/) — the Loco crate: `src/fall_risk_assessment/` holds `app.rs`
...
`src/fall_risk_assessment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd fall_risk_assessment && cargo check --all-targets && cargo test
```
