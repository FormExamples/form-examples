...
use crate::engine::{fall_risk_grader, types::AssessmentData, utils};
...
fn section_to_json(section: &str) -> &str {
    match section {
        ...
        "mfs" => "mfs",
        "mobility_gait" => "mobilityGait",
        "medication_review" => "medicationReview",
        "vision_sensory" => "visionSensory",
    ...
    if let Some(obj) = data_value.as_object_mut() {
        if let Some(form_obj) = form_data.as_object() {
            ...
            for (key, value) in form_obj {
                // medications[0].name -- dynamic medication-review rows.
            ...
            // Materialize medications inside the medicationReview section.
            if !medications.is_empty() {
                ...
                {
                    med_review
                        .insert("medications".to_string(), serde_json::Value::Array(items));
                }
            }
        }
    }
...
async fn show_report(
    ...
    let grade = fall_risk_grader::grade(&assessment_data);
    ...
    context.insert("mfs_score", &grade.mfs_score);
    context.insert("severity", &grade.severity);
    context.insert("severity_label", &utils::severity_label(&grade.severity));
    context.insert("severity_class", &utils::severity_class(&grade.severity));
    context.insert("critical_override", &grade.critical_override);
    context.insert("critical_reasons", &grade.critical_reasons);
