...
use crate::engine::{fertility_grader, types::AssessmentData};
...
async fn submit_assessment(
    ...
    if let Some(obj) = data_value.as_object_mut() {
        if let Some(form_obj) = form_data.as_object() {
            ...
            // Materialize current_medications as an array (drop empty rows).
            if !medications.is_empty() {
                ...
                if let Some(meds_section) = obj
                    .get_mut("medicationsSupplements")
...
async fn show_report(
    ...
    let grade = fertility_grader::grade(&assessment_data);
    ...
    context.insert("concern_level", &grade.concern_level);
    context.insert(
        "concern_label",
        &crate::engine::utils::concern_level_label(&grade.concern_level),
    );
    context.insert(
        "concern_class",
        &crate::engine::utils::concern_level_class(&grade.concern_level),
    );
    context.insert("concern_score", &grade.concern_score);
