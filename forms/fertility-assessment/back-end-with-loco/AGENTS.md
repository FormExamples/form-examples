# Fertility Assessment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Fertility Assessment form, built with axum + Loco +
...
- [`fertility_assessment/`](./fertility_assessment/) — the Loco crate: `src/fertility_assessment/` holds `app.rs`
...
`src/fertility_assessment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd fertility_assessment && cargo check --all-targets && cargo test
```
