# Cardiac Stress Test Request — agent instructions

Cardiac stress / exercise test request (referral). A clinician completes a
...
back-end. Safety contraindications (recent acute coronary syndrome, severe
symptomatic aortic stenosis, uncontrolled hypertension, inability to exercise
for an exercise test) drive the safety axis and auto-escalate or block the
request. Choose the least-alarming band only when no rule fires.
