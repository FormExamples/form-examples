// Cardiac Stress Test Request — vetting dashboard
...
// attach their exports to `window.CardiacStressTestRequestDashboard`.
...
const columns = [
  ...
  { key: 'contraindicationBand', label: 'Safety' },
...
function matchesFilters(row) {
  ...
  if (filters.safety && row.contraindicationBand !== filters.safety) return false;
...
function renderTableBody() {
  ...
  for (const row of rows) {
    ...
    tr.innerHTML = `
      ...
      <td class="data-table-td"><span class="safety-badge ${safetyClass(row.contraindicationBand)}">${esc(titleCase(row.contraindicationBand))}</span></td>
