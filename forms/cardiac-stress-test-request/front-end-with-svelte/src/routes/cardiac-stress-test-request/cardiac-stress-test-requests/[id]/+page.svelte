<script lang="ts">
	...
	import Step4SymptomsAndExerciseCapacity from '#lib/components/steps/Step4SymptomsAndExerciseCapacity.svelte';
	import Step5CardiacSafetyScreen from '#lib/components/steps/Step5CardiacSafetyScreen.svelte';
	...
	function submit() {
		...
		goto(`/cardiac-stress-test-request/cardiac-stress-test-requests/${id}/report`);
	}
...
<main class="mx-16 px-4 py-6">
	<h1 class="text-2xl font-bold text-base-content">
		{isNew ? 'New cardiac stress test request' : `Cardiac stress test request ${id}`}
	</h1>
	...
	<Form label="Cardiac stress test request" onsubmit={submit}>
		<Step1RequestingClinician />
		<Step2PatientIdentification />
		<Step3RequestedExamination />
		<Step4SymptomsAndExerciseCapacity />
		<Step5CardiacSafetyScreen />
		<Step6Triage />
