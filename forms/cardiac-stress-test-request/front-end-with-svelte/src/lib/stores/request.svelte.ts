...
import type { StressTestRequest, GradingResult } from '$lib/engine/types';
import { createDefaultRequest } from '$lib/engine/defaults';

// Re-export the default factory so callers can import it from the store too.
export { createDefaultRequest };

/** localStorage draft key for a given request id (defaults to `new`). */
function storageKey(id: string): string {
	return `cardiac-stress-test-request.front-end-with-svelte.${id || 'new'}.v1`;
}
...
/**
 * Svelte 5 reactive store for the cardiac stress test request, with localStorage
 ...
 */
class RequestStore {
	data = $state<StressTestRequest>(createDefaultRequest());
	 ...
	 * `const d = request.data.clinician`) stay bound to live state.
	 */
	loadForId(id: string, seed?: StressTestRequest) {
		const key = id || 'new';
		this.id = key;
		this.result = null;
		this.currentStep = 1;

		let draft: StressTestRequest | null = null;
		if (browser) {
			const raw = localStorage.getItem(storageKey(key));
			if (raw) {
				try {
					draft = JSON.parse(raw) as StressTestRequest;
				} catch {
