# Asthma Assessment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Asthma Assessment form, built with axum + Loco +
...
- [`asthma_assessment/`](./asthma_assessment/) — the Loco crate: `src/asthma_assessment/` holds `app.rs`
...
`src/asthma_assessment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd asthma_assessment && cargo check --all-targets && cargo test
```
