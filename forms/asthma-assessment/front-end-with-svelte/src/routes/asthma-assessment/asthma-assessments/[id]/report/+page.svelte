<script lang="ts">
	...
	import { actScoreLabel, actScoreColor, controlLevelLabel, calculateAge } from '#lib/engine/utils.js';
	...
	$effect(() => {
		if (!assessment.result) goto(`/asthma-assessment/asthma-assessments/${id}`);
	});
	...
	async function downloadPDF() {
		pdfError = '';
		try {
			const res = await fetch(`/asthma-assessments/${id}/report/pdf`, {
				method: 'POST',
			...
			if (res.ok) {
				...
				a.href = url;
				a.download = `asthma-assessment-${data.demographics.lastName || id}.pdf`;
				a.click();
...
{#if result}
	<header class="border-b border-base-300 bg-base-100 shadow-sm no-print">
		<div class="mx-auto flex max-w-4xl items-center justify-between px-4 py-4">
			<h1 class="text-lg font-bold text-base-content">Asthma assessment report</h1>
			<div class="flex items-center gap-3">
				{#if pdfError}<span class="text-sm text-error">{pdfError}</span>{/if}
				<Button data-variant="primary" onclick={downloadPDF}>Download PDF</Button>
				<Button data-variant="secondary" onclick={() => window.print()}>Print</Button>
				<Button data-variant="secondary" onclick={() => goto(`/asthma-assessment/asthma-assessments/${id}`)}>Edit</Button>
			</div>
		</div>
	</header>
	...
	<main class="mx-16 px-4 py-6">
		<div class="mb-6 rounded-xl border-2 p-6 text-center {actScoreColor(result.actScore)}">
			<div class="text-3xl font-bold">{result.actScore} / 25</div>
			<div class="mt-1 text-lg font-semibold">{actScoreLabel(result.actScore)}</div>
			<div class="mt-2 text-sm">Control level: {controlLevelLabel(result.controlLevel)}</div>
		...
		{#if result.firedRules.length > 0}
			<div class="mb-6 rounded-xl border border-base-300 bg-base-100 p-6">
				<h2 class="mb-4 text-lg font-bold text-base-content">ACT scoring detail</h2>
