# International Patient Summary — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the International Patient Summary form, built with axum + Loco +
...
- [`international_patient_summary/`](./international_patient_summary/) — the Loco crate: `src/international_patient_summary/` holds `app.rs`
...
`src/international_patient_summary/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd international_patient_summary && cargo check --all-targets && cargo test
```
