<script lang="ts">
	...
	const columns = [
		...
		{ id: 'assessedDate', header: 'Updated', width: 120, sort: true },
		{
			id: 'completeness',
			header: 'Completeness',
			width: 140,
	...
	function init(api: any) {
		...
		api.on('select-row', (ev: { id?: string | number }) => {
			if (ev?.id != null) goto(`/international-patient-summaries/${ev.id}`);
		});
	}
</script>
...
<main class="mx-auto max-w-6xl px-4 py-6">
	<div class="mb-6 flex flex-wrap items-end justify-between gap-4">
		...
		<a href="/international-patient-summaries/new" class="button" data-variant="primary">New IPS</a>
	</div>
