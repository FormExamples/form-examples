...
/**
 * Flagged-issue detection for the International Patient Summary. Independent of
 * completeness grading (which `ips-grader.ts` computes), this raises
 * clinician-facing flags for high-severity allergies, missing identifiers,
 * draft signoff, consent issues, and other cross-border-care risks.
 */
export function detectAdditionalFlags(data: AssessmentData): AdditionalFlag[] {
	...
	if (c.authoringStatus === 'final' && c.signoffDate.trim() === '') {
		flags.push({
			id: 'FLAG-AUTH-003',
			category: 'Authoring',
	...
	if (data.allergiesIntolerances.length === 0) {
		flags.push({
			...
			category: 'Allergies',
			message:
				'No allergies recorded — IPS requires either entries or an explicit "no known allergies" marker.',
			priority: 'high'
		});
	}
	...
	for (let i = 0; i < data.problemList.length; i++) {
		...
		if (!pr) continue;
		if (pr.description.trim() !== '' && pr.icd10Code.trim() === '') {
			flags.push({
				id: `FLAG-DQ-PROB-${i}`,
				category: 'Data Quality',
