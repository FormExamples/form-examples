# Cardiology Waiting List Card — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Cardiology Waiting List Card form, built with axum + Loco +
...
- [`cardiology_waiting_list_card/`](./cardiology_waiting_list_card/) — the Loco crate: `src/cardiology_waiting_list_card/` holds `app.rs`
...
`/metrics`. The registered domain controllers are:
  ...
  - `cardiology_waiting_list_card`
  - `cardiology_waiting_list_card_appointment`
  - `cardiology_waiting_list_card_grade`
  - `cardiology_waiting_list_card_grade_flag`
  - `cardiology_waiting_list_card_grade_rule`
  - `patient`
  - `practitioner`

## Engine

`src/cardiology_waiting_list_card/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd cardiology_waiting_list_card && cargo check --all-targets && cargo test
```
