# Meeting — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Meeting form, built with axum + Loco +
...
- [`meeting/`](./meeting/) — the Loco crate: `src/meeting/` holds `app.rs`
...
`src/meeting/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd meeting && cargo check --all-targets && cargo test
```
