...
// `src/lib/engine/types.ts` data model for the Cystoscopy Test Result form.
//
// The camelCase property names mirror the snake_case SQL columns in
// `sql/04_create_table_cystoscopy_test_result.sql` and
// `sql/05_create_table_cystoscopy_test_result_grade.sql`. This file builds
// and exports the canonical empty CystoscopyResult shape used by the wizard,
 ...
 * @typedef {'flexible-cystoscopy' | 'rigid-cystoscopy' | 'other' | ''} Procedure
 */

/**
 * Anaesthesia used for the procedure.
 * @typedef {'local' | 'sedation' | 'general' | ''} Anaesthesia
 */

/**
 * Tumour / lesion appearance.
 * @typedef {'papillary' | 'solid' | 'flat' | 'not-applicable' | ''} TumourAppearance
 */

/**
 * Procedure complication.
 * @typedef {'none' | 'bleeding' | 'perforation' | 'uti' | 'other' | ''} Complication
 */
 ...
 * interpretation grade is computed from. Mirrors `CystoscopyResult` in
 * `front-end-with-svelte/src/lib/engine/types.ts`.
 *
 * @typedef {Object} CystoscopyResult
 ...
 * @property {Anaesthesia} anaesthesia
 * @property {string} clinicalHistory
 * @property {string} findingsNarrative
 * @property {boolean} bladderTumour
 * @property {boolean} inflammationCystitis
 * @property {boolean} bladderStones
 * @property {boolean} urethralStricture
 * @property {boolean} trabeculation
 * @property {boolean} prostaticEnlargement
 * @property {boolean} normalExamination
 * @property {number | null} tumourSizeMm      - largest tumour / lesion size; null when unset
 * @property {TumourAppearance} tumourAppearance
 ...
 * the numeric tumour size defaults to `null`.
 * @returns {CystoscopyResult}
 */
...
function procedureLabel(value) {
  switch (value) {
    case 'flexible-cystoscopy': return 'Flexible cystoscopy';
    case 'rigid-cystoscopy': return 'Rigid cystoscopy';
    case 'other': return 'Other';
    default: return 'Unspecified';
  }
}

/** Human-readable anaesthesia label. */
function anaesthesiaLabel(value) {
  switch (value) {
...
/** Human-readable tumour-appearance label. */
function tumourAppearanceLabel(value) {
  switch (value) {
    case 'papillary': return 'Papillary';
