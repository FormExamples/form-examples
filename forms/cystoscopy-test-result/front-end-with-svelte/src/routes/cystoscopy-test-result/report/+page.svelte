<script lang="ts">
	...
	import Step2ProcedureDetails from '$lib/components/steps/Step2ExaminationDetails.svelte';
	...
	import Step5TumourComplication from '$lib/components/steps/Step5Measurements.svelte';
	...
	function validate(): boolean {
		...
		if (d.procedure === '') {
			found.push({ id: 'procedure', message: 'Procedure is required.' });
		}
	...
	function submit() {
		...
		goto('/cystoscopy-test-result/report/view');
	}
...
<main class="mx-auto max-w-3xl px-4 py-6">
	<header class="mb-6 no-print">
		<h1 class="text-2xl font-bold text-gray-900">New cystoscopy report</h1>
	...
	<Form label="Cystoscopy test result" onsubmit={submit}>
		<Step1ReportIdentification />
		<Step2ProcedureDetails />
		<Step3ClinicalHistory />
		<Step4Findings />
		<Step5TumourComplication />
		<Step6Impression />
