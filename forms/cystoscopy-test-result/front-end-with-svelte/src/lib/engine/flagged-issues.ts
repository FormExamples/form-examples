import type { CystoscopyResult, Flag, FlagPriority } from './types';
import { hasCriticalFinding } from './utils';

/**
 * Detects safety-critical flags independently of the four axes. Flag
 * categories mirror sql/07_create_table_cystoscopy_test_result_grade_flag.sql.
 * Flags are returned sorted high → medium → low priority.
 */
export function detectFlags(r: CystoscopyResult): Flag[] {
	const flags: Flag[] = [];
	...
	if (hasCriticalFinding(r)) {
		flags.push({
			...
			priority: 'high',
			description: 'A bladder tumour or suspicious lesion is present.',
			suggestedAction:
				'Communicate the critical result to the referrer immediately and book urgent TURBT / MDT referral.'
		});
	...
	// ─── abnormal-requiring-action (procedure complication) ───
	if (
		r.complication === 'perforation' ||
		r.complication === 'bleeding' ||
		r.complication === 'uti'
	) {
		flags.push({
			flagId: 'F-ABNORMAL-ACTION-001',
			category: 'abnormal-requiring-action',
			priority: r.complication === 'perforation' ? 'high' : 'medium',
			description: `A procedure complication (${r.complication}) requiring timely action is present.`,
			suggestedAction: 'Ensure the referrer is alerted and a clear action plan is documented.'
		});
	}

	// ─── urgent-referral ───
	if (r.urethralStricture || r.bladderStones) {
		flags.push({
			flagId: 'F-URGENT-REFERRAL-001',
			category: 'urgent-referral',
			priority: 'medium',
			description:
				'A urethral stricture or bladder stones are present and may warrant urgent referral.',
			suggestedAction: 'Consider urgent referral to the appropriate urology team.'
		});
	}

	// ─── inadequate-technique (limiting complication) ───
	if (r.complication === 'perforation') {
		flags.push({
			flagId: 'F-INADEQUATE-TECHNIQUE-001',
			category: 'inadequate-technique',
			priority: 'high',
			description: 'A perforation occurred; the examination may be incomplete or non-diagnostic.',
			suggestedAction: 'Consider repeating or supplementing the examination once safe to do so.'
		});
	}

	// ─── incidental-finding ───
	if (r.prostaticEnlargement || r.trabeculation) {
		flags.push({
			flagId: 'F-INCIDENTAL-FINDING-001',
			category: 'incidental-finding',
			priority: 'low',
			description: 'One or more incidental benign structural findings are documented.',
	...
	// ─── missing-measurement ───
	if (r.bladderTumour && r.tumourSizeMm === null) {
		flags.push({
			flagId: 'F-MISSING-MEASUREMENT-001',
			category: 'missing-measurement',
			priority: 'low',
			description: 'A bladder tumour is reported but no measurement was recorded.',
			suggestedAction: 'Record the largest tumour size in millimetres for surveillance.'
		});
	}

	// ─── unexpected-finding (abnormal but no originating request linked) ───
	if (r.bladderTumour && r.originatingRequestReference.trim() === '') {
		flags.push({
