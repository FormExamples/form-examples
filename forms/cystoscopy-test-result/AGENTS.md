# Cystoscopy Test Result — agent instructions

Cystoscopy (bladder endoscopy) result (report). An operating clinician completes
a single-page wizard recording the performed procedure's **findings** and a
...
structured cystoscopy report.

This is the **result/report counterpart** to `cystoscopy-test-request` (a
referral). It mirrors the gold template `ct-scan-test-result`. Keep it faithful
to the conventions below.

## Result semantics

A result form records *what the test found and what it means* — it is **not** a
referral. The source-of-truth record is `cystoscopy_test_result`; the clinician
is the report **author/signer** (urologist, nurse cystoscopist, other operator),
not a referrer. The engine **interprets findings** rather than vetting a request.
...
- **Axis B — severity & structured reporting:** abnormalitySeverity
  ...
  (e.g. an EAU NMIBC risk group or a suspected-tumour category).
- **Axis C — report completeness:** 0–100 % of mandatory report sections
  (history, procedure, findings, impression, follow-up).
- **Axis D — follow-up urgency:** routine / recommended / urgent / critical-alert,
  with a target timeframe and a recommended action.

A **bladder tumour or suspicious lesion** (or another unexpected significant
abnormality) **auto-escalates** Axis D to *critical-alert*, drives a
`specialist-referral` / `urgent-review` recommendation, and raises the
