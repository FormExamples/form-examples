# Stomatology Waiting List Card — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Stomatology Waiting List Card form, built with axum + Loco +
...
- [`stomatology_waiting_list_card/`](./stomatology_waiting_list_card/) — the Loco crate: `src/stomatology_waiting_list_card/` holds `app.rs`
...
`/metrics`. The registered domain controllers are:
  ...
  - `stomatology_waiting_list_card`
  - `stomatology_waiting_list_card_appointment`
  - `stomatology_waiting_list_card_grade`
  - `stomatology_waiting_list_card_grade_flag`
  - `stomatology_waiting_list_card_grade_rule`
  - `patient`
  - `practitioner`

## Engine

`src/stomatology_waiting_list_card/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd stomatology_waiting_list_card && cargo check --all-targets && cargo test
```
