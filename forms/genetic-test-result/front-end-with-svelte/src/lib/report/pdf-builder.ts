...
import type { GeneticResult, GradingResult } from '#lib/engine/types.js';
import {
...
/** Builds the pdfmake document definition for the genomic report. */
export function buildPdfDocument(
	...
	return {
		...
		footer: (currentPage: number, pageCount: number) => ({
			text: `Page ${currentPage} of ${pageCount} | Generated ${new Date(result.gradedAt).toLocaleString()} | ACMG/AMP (ACGS)`,
			alignment: 'center',
			margin: [0, 20, 0, 0],
			fontSize: 8,
			color: '#9ca3af'
		...
		content: [
			...
			// Test details
			sectionHeader('Test details'),
			{
				ul: [
					`Test type: ${testTypeLabel(data.testType)}`,
					`Sample type: ${data.sampleType || 'N/A'}`,
					`Genes tested: ${data.genesTested || 'N/A'}`,
					`Inheritance pattern: ${data.inheritancePattern || 'N/A'}`
				],
				margin: [0, 0, 0, 16] as Margin
			},
			...
			{ text: data.variantsDetected || 'Not specified', margin: [0, 0, 0, 8] as Margin },
			{
				ul: [
					`Variant classification: ${variantClassificationLabel(data.variantClassification)}`,
					`Zygosity: ${data.zygosity || 'N/A'}`,
					...structuredFindingList(data)
				],
				margin: [0, 0, 0, 16] as Margin
			},
			...
			{
				text: `Reporting category: ${data.reportingCategory || result.reportingCategory || 'N/A'}`,
				margin: [0, 0, 0, 16] as Margin
			},
			...
			sectionHeader('Follow-up'),
			{
				ul: [
					...
					`Recommended follow-up: ${data.recommendedFollowUp || 'None'}`
				],
				margin: [0, 0, 0, 16] as Margin
			},
...
function structuredFindingList(data: GeneticResult): string[] {
	const list: string[] = [];
