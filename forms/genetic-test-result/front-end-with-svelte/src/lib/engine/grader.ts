import type { GeneticResult, GradingResult, Recommendation, FiredRule } from './types';
...
/**
 * Pure four-axis interpretation engine for a genetic / genomic test result.
 *
 * Computes:
 * - Axis A: result classification (normal / abnormal / critical / inconclusive).
 * - Axis B: abnormality severity + structured-reporting (ACMG class) category.
 ...
 *
 * Invariant: a pathogenic / likely-pathogenic actionable variant auto-escalates
 * Axis D to critical-alert and raises the critical-result-alert /
 * pathogenic-variant-found flags, regardless of the other axes. The
 * least-urgent band is only chosen when no rule fires.
 *
 * No side effects, no network calls, no I/O.
 */
export function calculateGrade(result: GeneticResult): GradingResult {
