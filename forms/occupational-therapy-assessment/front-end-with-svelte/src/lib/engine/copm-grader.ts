...
/**
 * Full grading entry point: runs the COPM score calculation plus the
 * additional-flag detection and stamps the result, producing the complete
 * `GradingResult` consumed by the wizard, report, and dashboard.
 */
export function gradeAssessment(data: AssessmentData): GradingResult {
	const { performanceScore, satisfactionScore, performanceCategoryLabel, satisfactionCategoryLabel, firedRules } =
		calculateCOPM(data);
	return {
		performanceScore,
		satisfactionScore,
