# Occupational Therapy Assessment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Occupational Therapy Assessment form, built with axum + Loco +
...
- [`occupational_therapy_assessment/`](./occupational_therapy_assessment/) — the Loco crate: `src/occupational_therapy_assessment/` holds `app.rs`
...
`src/occupational_therapy_assessment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd occupational_therapy_assessment && cargo check --all-targets && cargo test
```
