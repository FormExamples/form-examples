# Cognitive Assessment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Cognitive Assessment form, built with axum + Loco +
...
- [`cognitive_assessment/`](./cognitive_assessment/) — the Loco crate: `src/cognitive_assessment/` holds `app.rs`
...
`src/cognitive_assessment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd cognitive_assessment && cargo check --all-targets && cargo test
```
