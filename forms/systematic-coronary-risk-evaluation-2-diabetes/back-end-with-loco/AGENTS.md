# SCORE2-Diabetes — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the SCORE2-Diabetes form, built with axum + Loco +
...
- [`systematic_coronary_risk_evaluation_2_diabetes/`](./systematic_coronary_risk_evaluation_2_diabetes/) — the Loco crate: `src/systematic_coronary_risk_evaluation_2_diabetes/` holds `app.rs`
...
`src/systematic_coronary_risk_evaluation_2_diabetes/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd systematic_coronary_risk_evaluation_2_diabetes && cargo check --all-targets && cargo test
```
