...
function detectAdditionalFlags(data) {
  ...
  if (
    ...
    flags.push({
      id: 'FLAG-MED-003',
      category: 'Medications',
      message: 'Albuminuria without ACE inhibitor or ARB - consider renoprotective therapy',
      priority: 'medium'
    });
  }
  ...
  if (bmi != null && bmi >= 40) {
    flags.push({
      id: 'FLAG-BMI-001',
      category: 'Lifestyle',
      message: 'Severe obesity (BMI >= 40) - consider bariatric/metabolic surgery referral',
      priority: 'medium'
    });
  }
  ...
  flags.sort((a, b) => {
    const order = (p) => (p === 'high' ? 0 : p === 'medium' ? 1 : 2);
