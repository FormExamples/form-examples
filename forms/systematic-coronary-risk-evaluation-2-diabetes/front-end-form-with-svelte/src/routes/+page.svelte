<script lang="ts">
  ...
  import { steps as STEPS } from '$lib/config/steps.js';
  ...
  const title = 'SCORE2-Diabetes';
  const subtitle = '10-year cardiovascular risk for adults with type 2 diabetes (ESC 2023).';
  ...
  const percentComplete = $derived.by(() => {
    const totals = STEPS.reduce(
      (acc, s) => {
        acc.filled += sectionFilled(s.sectionKey);
        acc.total += sectionTotal(s.sectionKey);
        return acc;
      },
      { filled: 0, total: 0 }
    );
  ...
  function stepStatus(n: number): 'waiting' | 'in-progress' | 'finished' | 'error' {
    ...
    const filled = sectionFilled(cfg.sectionKey);
    const total = sectionTotal(cfg.sectionKey);
...
<main>
  <div class="intro">
    <p>
      SCORE2-Diabetes predicts 10-year risk of fatal and non-fatal cardiovascular
      disease in individuals with type 2 diabetes without prior CVD, aged
      40\u201369 years. Complete the 10 sections below, then submit to generate
      the risk report.
    </p>
  </div>
  ...
  <footer class="page-footer">
    <p>
      SCORE2-Diabetes is a screening tool for clinical decision support. Final
      risk assessment and management remain the responsibility of the treating
      clinician.
    </p>
  </footer>
</main>
