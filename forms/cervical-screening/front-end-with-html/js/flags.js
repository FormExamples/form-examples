...
//
// Rows here mirror the `cervical_screening_grade_flag` SQL table
 ...
 * @param {{ resultClass: string, managementAction: string, status: string }} grade
...
function detectFlaggedIssues(data, grade) {
  ...
  if (data.consent.consentGiven !== 'yes') {
    flags.push({
      ...
      suggestedAction:
        'Do not report the result until informed consent is recorded.'
    });
  }
  ...
  if (data.eligibility.overdue === 'yes' || isPast(data.eligibility.screenDueDate)) {
    flags.push({
      ...
      suggestedAction:
        'Prioritise recall and follow up any non-attendance.'
    });
  }
  ...
  if (age !== null && (age < MIN_AGE || age > MAX_AGE)) {
    flags.push({
      ...
      priority: 'medium',
      description:
        `Patient age ${age} is outside the routine ${MIN_AGE}-${MAX_AGE} screening range.`,
      suggestedAction:
