import type { AssessmentData, ClassificationRule } from './types';

/** Youngest routine-programme age (inclusive). */
export const MIN_AGE = 25;
/** Oldest routine-programme age (inclusive). */
export const MAX_AGE = 64;

/** Cytology grades that are a *graded* (reportable) value. */
export const GRADED_CYTOLOGY = ['negative', 'borderline', 'low-grade', 'high-grade'];
 ...
 * guard against here).
 */
export function notEligible(d: AssessmentData): boolean {
 ...
 * the grader (`cervical-screening-grader.ts`); the first whose `evaluate`
 * returns true wins. Rows mirror the `cervical_screening_grade_rule` SQL table
...
export const classificationRules: ClassificationRule[] = [
	...
	// ─── 5. hrHPV POSITIVE + LOW-GRADE / BORDERLINE ────────────────
	{
		...
		evaluate: (d) =>
			...
			(d.cytology.cytologyGrade === 'borderline' || d.cytology.cytologyGrade === 'low-grade')
	},
	...
	// ─── 7. hrHPV POSITIVE + CYTOLOGY OUTSTANDING ──────────────────
	{
		...
		evaluate: (d) =>
			d.hpv.hpvResult === 'positive' && GRADED_CYTOLOGY.indexOf(d.cytology.cytologyGrade) === -1
	},
