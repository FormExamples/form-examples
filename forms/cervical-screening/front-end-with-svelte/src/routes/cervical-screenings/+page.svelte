<script lang="ts">
	...
	import { resultClassLabel, managementActionLabel, careSettingLabel } from '$lib/engine/utils';
	...
	function init(api: any) {
		...
		api.on('select-row', (ev: { id?: string | number }) => {
			if (ev?.id != null) goto(`/cervical-screenings/${ev.id}`);
		});
	}
</script>
...
<main class="mx-auto max-w-6xl px-4 py-6">
	<div class="mb-6 flex flex-wrap items-end justify-between gap-4">
		...
		<a href="/cervical-screenings/new" class="button" data-variant="primary">New screening</a>
	</div>
