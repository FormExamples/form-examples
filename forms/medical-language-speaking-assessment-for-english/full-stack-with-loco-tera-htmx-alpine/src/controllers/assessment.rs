...
use crate::engine::{oet_grader, types::AssessmentData, utils::grade_label};
...
fn section_to_json(section: &str) -> &str {
    match section {
        "candidate" => "candidate",
        "role_play1" => "rolePlay1",
        "role_play2" => "rolePlay2",
        "linguistic_role_play1" => "linguisticRolePlay1",
        "linguistic_role_play2" => "linguisticRolePlay2",
...
/// Which (section, field) combinations are numeric ratings. Empty values
/// for these become JSON `null`; numeric strings become JSON numbers.
fn is_numeric_field(section: &str, field: &str) -> bool {
    match (section, field) {
        ("linguisticRolePlay1" | "linguisticRolePlay2", _) => matches!(
            field,
            "intelligibility"
                | "fluency"
...
/// Parse a form value string respecting the numeric-vs-text contract:
/// empty rating fields become `null`; numeric rating fields become JSON
/// numbers; everything else is left as a string.
...
async fn show_report(
    ...
    let grading = oet_grader::grade(&assessment_data);
    ...
    context.insert("candidate", &assessment_data.candidate);
    context.insert("grading", &grading);
    context.insert("grade", &grading.grade);
    context.insert("grade_label", &grade_label(&grading.grade));
    context.insert("scaled_score", &grading.scaled_score);
    context.insert("linguistic_total", &grading.linguistic_total);
    context.insert("clinical_total", &grading.clinical_total);
    context.insert("raw_total", &grading.raw_total);
    context.insert("per_criterion_scores", &grading.per_criterion_scores);
