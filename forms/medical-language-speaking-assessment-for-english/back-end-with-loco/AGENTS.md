# Medical Language Speaking Assessment for English — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Medical Language Speaking Assessment for English form, built with axum + Loco +
...
- [`medical_language_speaking_assessment_for_english/`](./medical_language_speaking_assessment_for_english/) — the Loco crate: `src/medical_language_speaking_assessment_for_english/` holds `app.rs`
...
`src/medical_language_speaking_assessment_for_english/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd medical_language_speaking_assessment_for_english && cargo check --all-targets && cargo test
```
