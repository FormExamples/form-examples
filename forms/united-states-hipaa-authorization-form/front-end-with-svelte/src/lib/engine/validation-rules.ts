...
// Ported from `../front-end-form-with-html/js/validation-rules.js`.
// Each rule is a pure predicate that returns true when the rule fires
// (i.e. the authorization is defective in that respect).
...
export const coreElementRules: ValidationRule[] = [
	{
		...
		test: (a) => {
			...
			].some((k) => r[k as keyof typeof r] === 'yes');
	...
	},
	{
		...
		priority: 'medium',
		description:
			'Vague phrases such as "all my records" do not satisfy the specificity requirement.',
	...
	},
	{
		...
		test: (a) =>
			a.authorizedRecipient.recipientName === '' &&
			a.authorizedRecipient.recipientOrganization === ''
	},
	{
		id: 'purpose-stated',
		...
		test: (a) =>
			a.purposeOfDisclosure.primaryPurpose === '' && a.purposeOfDisclosure.purposes.length === 0
	},
	{
		...
		test: (a) =>
			a.expiration.expirationDate === null &&
			a.expiration.expirationEvent === '' &&
			a.expiration.durationMonths === null
	},
	{
	...
	},
	{
		...
		test: (a) =>
			a.signatureWitness.individualSignatureConfirmed !== 'yes' ||
			a.signatureWitness.signatureDate === null
	},
	{
		...
		test: (a) =>
			a.signer.relationship !== '' &&
			a.signer.relationship !== 'self' &&
			a.signer.representativeAuthorityDescription === ''
	}
];

export const requiredStatementRules: ValidationRule[] = [
	{
...
export const allRules: ValidationRule[] = [...coreElementRules, ...requiredStatementRules];

/** Run every core-element and required-statement rule, returning those that fired. */
export function runRules(authorization: HipaaAuthorization): FiredRule[] {
