<script lang="ts">
	...
	import { statusLabel, urgencyLabel, primaryCategoryLabel } from '#lib/engine/utils.js';
	...
	const columns = [
		...
		{ id: 'childReference', header: 'Child ref', width: 130, sort: true },
		{ id: 'childName', header: 'Child', flexgrow: 2, sort: true },
		...
		},
		{
			...
			width: 130,
			sort: true,
			template: (v: string) => primaryCategoryLabel(v as never)
		},
	...
	function init(api: any) {
		...
		api.on('select-row', (ev: { id?: string | number }) => {
			if (ev?.id != null) goto(`/child-safeguarding-referral/child-safeguarding-referrals/${ev.id}`);
		});
	}
</script>
...
<main class="mx-16 px-4 py-6">
	<div class="mb-6 flex flex-wrap items-end justify-between gap-4">
		<div>
			<h1 class="text-2xl font-bold text-base-content">Child safeguarding referral dashboard</h1>
			<p class="mt-1 text-sm text-base-content/70">
				Urgency, completeness status, category, and flag count for each referral, computed by the
				shared engine. Select a row to open the referral.
			</p>
		</div>
		<a href="/child-safeguarding-referral/child-safeguarding-referrals/new" class="button" data-variant="primary">New referral</a>
	</div>
	...
	<div class="mb-4 flex flex-wrap gap-4">
		<label class="text-sm">
			...
			<select class="select inline-block w-auto" bind:value={urgencyFilter}>
				...
				<option value="urgent">Urgent (s47)</option>
				<option value="standard">Standard (s17)</option>
			</select>
		</label>
		<label class="text-sm">
