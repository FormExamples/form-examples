...
//                                       non-empty.
//
// Rows here mirror the `child_safeguarding_referral_grade_flag` SQL table
 ...
 * `rules.js`. Used by the mandatory-field-missing flag.
...
function missingMandatoryFields(r) {
  ...
  if (!hasDate(r.child.childDateOfBirth) && !hasNumber(r.child.childAge)) {
...
function detectFlaggedIssues(referral) {
  ...
  if (referral.category.primaryCategory === 'sexual' || additionalSexual) {
    flags.push({
  ...
  if (missing.length > 0) {
    flags.push({
      ...
      priority: 'medium',
      description:
        `The referral is incomplete — required field(s) missing: ${missing.join(', ')}.`,
      suggestedAction:
