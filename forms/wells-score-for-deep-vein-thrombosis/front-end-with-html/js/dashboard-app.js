...
// Wells DVT — clinician dashboard (vanilla classic-script app).
//
...
// box + care-setting dropdown + two-level-band dropdown + investigation
// dropdown).
//
// Sibling modules loaded as plain `<script>` tags (in dependency order) attach
// their exports to `window.WellsScoreForDeepVeinThrombosisDashboard`. The whole
...
const filters = {
  ...
  band: '',          // '' | 'likely' | 'unlikely'
  investigation: ''  // '' | 'proximal-leg-vein-ultrasound' | 'd-dimer'
};
...
const columns = [
  ...
  { key: 'wellsScore',               label: 'Wells' },
  { key: 'twoLevelBand',             label: 'DVT Likelihood' },
  { key: 'recommendedInvestigation', label: 'First Test' }
];

// Rank used when sorting the twoLevelBand column so 'unlikely' is always less
// than 'likely' regardless of locale.
const bandRank = {
  'unlikely': 0,
  'likely': 1
};
...
const settingLabels = {
  ...
  'dvt-clinic': 'DVT clinic',
  'other': 'Other'
};

const investigationLabels = {
  'proximal-leg-vein-ultrasound': 'Ultrasound',
  'd-dimer': 'D-dimer'
};
...
function bandClass(band) {
  if (!band) return '';
  return band === 'likely' ? 'risk-high' : 'risk-low';
}

function bandLabel(band) {
  if (band === 'likely') return 'Likely';
  if (band === 'unlikely') return 'Unlikely';
  return 'N/A';
}
...
function investigationLabel(inv) {
  return investigationLabels[inv] || inv || 'N/A';
}
...
function matchesFilters(row) {
  ...
  if (filters.band && row.twoLevelBand !== filters.band) return false;
  if (
    filters.investigation &&
    row.recommendedInvestigation !== filters.investigation
  ) {
    return false;
  }
  return true;
 ...
 * table; the nullable Wells score sorts nulls last; everything else uses a
...
function compareRows(a, b) {
  ...
  // (patientIdentifier, patientName, careSetting, recommendedInvestigation)
...
function renderTableBody() {
  ...
  for (const row of rows) {
    ...
    if (row.twoLevelBand === 'likely') {
      tr.classList.add('row-critical');
    }

    const scoreClassName = (row.wellsScore === null || row.wellsScore === undefined)
    ...
    tr.innerHTML = `
      ...
      <td><span class="${scoreClassName}">${esc(scoreLabel(row.wellsScore))}</span></td>
      <td><span class="risk-badge ${bandClass(row.twoLevelBand)}">${esc(bandLabel(row.twoLevelBand))}</span></td>
      <td>${esc(investigationLabel(row.recommendedInvestigation))}</td>
    `;
    body.appendChild(tr);
  }
}
...
function bindFilterInputs() {
  ...
  const investigation = document.getElementById('filter-investigation');
  ...
  if (investigation) {
    investigation.addEventListener('change', () => {
      filters.investigation = investigation.value;
      renderAll();
    });
  }
  if (clearBtn) {
    clearBtn.addEventListener('click', () => {
      ...
      if (band) band.value = '';
      if (investigation) investigation.value = '';
      renderAll();
    });
  }
}
