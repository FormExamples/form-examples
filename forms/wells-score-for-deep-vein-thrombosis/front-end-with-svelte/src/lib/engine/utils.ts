...
export function twoLevelBandLabel(band: TwoLevelBand): string {
	switch (band) {
		case 'likely':
			return 'DVT likely (Wells >= 2)';
		case 'unlikely':
			return 'DVT unlikely (Wells <= 1)';
		default:
			return '';
	}
}
 ...
 * DVT likely → error; DVT unlikely → success.
 */
...
export function recommendedInvestigationLabel(inv: RecommendedInvestigation): string {
	switch (inv) {
		case 'proximal-leg-vein-ultrasound':
			return 'Proximal leg vein ultrasound';
		case 'd-dimer':
			return 'D-dimer';
		default:
			return '';
...
export function careSettingLabel(setting: CareSetting): string {
	switch (setting) {
		...
		case 'dvt-clinic':
			return 'DVT / anticoagulation clinic';
		case 'other':
			return 'Other';
		default:
			return '';
...
export function symptomaticLegLabel(leg: SymptomaticLeg): string {
	switch (leg) {
		case 'left':
			return 'Left';
		case 'right':
