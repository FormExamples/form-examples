...
Server-side Rust JSON API for the Wells Score for Deep Vein Thrombosis (DVT) form. Pure back-end: no HTML
rendering, no Tera templates, no HTMX, no Alpine.js, no CSS, no Lily
Design System.

The Loco crate lives in [`wells_score_for_deep_vein_thrombosis/`](wells_score_for_deep_vein_thrombosis/) — one migration, SeaORM
