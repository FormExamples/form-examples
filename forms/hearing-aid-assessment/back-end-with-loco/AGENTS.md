# Hearing Aid Assessment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Hearing Aid Assessment form, built with axum + Loco +
...
- [`hearing_aid_assessment/`](./hearing_aid_assessment/) — the Loco crate: `src/hearing_aid_assessment/` holds `app.rs`
...
`src/hearing_aid_assessment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd hearing_aid_assessment && cargo check --all-targets && cargo test
```
