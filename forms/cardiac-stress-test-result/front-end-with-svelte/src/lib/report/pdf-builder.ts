...
import type { CardiacStressResult, GradingResult } from '#lib/engine/types.js';
import {
...
/** Builds the pdfmake document definition for the cardiac stress test report. */
export function buildPdfDocument(
	data: CardiacStressResult,
	...
	return {
		...
		header: {
			text: 'CARDIAC STRESS TEST REPORT',
			alignment: 'center',
			margin: [0, 20, 0, 0],
			fontSize: 10,
			color: '#6b7280',
		...
		footer: (currentPage: number, pageCount: number) => ({
			text: `Page ${currentPage} of ${pageCount} | Generated ${new Date(result.gradedAt).toLocaleString()} | ACC/AHA exercise-testing guidance`,
			alignment: 'center',
			margin: [0, 20, 0, 0],
			fontSize: 8,
			color: '#9ca3af'
		...
		content: [
			{
				text: 'CARDIAC STRESS TEST RESULT',
				fontSize: 18,
				bold: true,
				alignment: 'center',
			...
			sectionHeader('Haemodynamic & exercise response'),
			{
				ul: [
					`Maximum heart rate: ${data.maximumHeartRateBpm !== null ? `${data.maximumHeartRateBpm} bpm` : 'N/A'}`,
					`Percent-predicted heart rate: ${data.percentPredictedHeartRate !== null ? `${data.percentPredictedHeartRate}%` : 'N/A'}`,
					`Exercise duration: ${data.exerciseDurationMinutes !== null ? `${data.exerciseDurationMinutes} min` : 'N/A'}`,
					`METs achieved: ${data.metsAchieved !== null ? data.metsAchieved : 'N/A'}`,
					`Peak blood pressure: ${data.peakBloodPressure || 'N/A'}`,
					`Blood-pressure response: ${bloodPressureResponseLabel(data.bloodPressureResponse)}`
				],
				margin: [0, 0, 0, 16] as Margin
			},
			...
			{
				text: `Reason for termination: ${data.reasonForTermination || 'N/A'}`,
				margin: [0, 0, 0, 16] as Margin
			},

			// Prognostic score
			sectionHeader('Prognostic score'),
			{
				ul: [
					`Duke treadmill score: ${data.dukeTreadmillScore !== null ? data.dukeTreadmillScore : 'N/A'}`
				],
				margin: [0, 0, 0, 16] as Margin
			},
...
function structuredFindingList(data: CardiacStressResult): string[] {
	const list: string[] = [];
