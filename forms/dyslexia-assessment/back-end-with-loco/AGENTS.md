# Dyslexia Assessment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Dyslexia Assessment form, built with axum + Loco +
...
- [`dyslexia_assessment/`](./dyslexia_assessment/) — the Loco crate: `src/dyslexia_assessment/` holds `app.rs`
...
`src/dyslexia_assessment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd dyslexia_assessment && cargo check --all-targets && cargo test
```
