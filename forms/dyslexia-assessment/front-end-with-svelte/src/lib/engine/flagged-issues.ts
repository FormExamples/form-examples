...
//
// Independent of the per-domain grader, this module raises clinician-facing
// flags for severe domain scores, hearing/vision concerns, developmental
// history red flags, family history, ESL learners, emotional/behavioural
// impact, mental-health concerns, and missing supports/recommendations.

/** Push a flag for a numeric standardised score below a dyslexia-band threshold. */
function pushScoreFlag(
...
export function detectAdditionalFlags(data: AssessmentData): AdditionalFlag[] {
	...
	// ── Domain score alerts ───────────────────────────────────
	pushScoreFlag(flags, 'FLAG-READ-001', 'Reading', 'Reading fluency', data.readingAssessment.readingFluencyScore);
	pushScoreFlag(flags, 'FLAG-READ-002', 'Reading', 'Reading comprehension', data.readingAssessment.readingComprehensionScore);
	pushScoreFlag(flags, 'FLAG-WRIT-001', 'Writing & Spelling', 'Spelling accuracy', data.writingSpelling.spellingAccuracyScore);
	pushScoreFlag(flags, 'FLAG-WRIT-002', 'Writing & Spelling', 'Written expression', data.writingSpelling.writtenExpressionScore);
	pushScoreFlag(flags, 'FLAG-PHON-001', 'Phonological Processing', 'Phonological awareness', data.phonologicalProcessing.phonologicalAwarenessScore);
	pushScoreFlag(flags, 'FLAG-PHON-002', 'Phonological Processing', 'Phonological memory', data.phonologicalProcessing.phonologicalMemoryScore);
	pushScoreFlag(flags, 'FLAG-PHON-003', 'Phonological Processing', 'Rapid naming', data.phonologicalProcessing.rapidNamingScore);
	pushScoreFlag(flags, 'FLAG-WMPS-001', 'Working Memory & Processing Speed', 'Working memory', data.workingMemoryProcessingSpeed.workingMemoryScore);
	pushScoreFlag(flags, 'FLAG-WMPS-002', 'Working Memory & Processing Speed', 'Processing speed', data.workingMemoryProcessingSpeed.processingSpeedScore);
