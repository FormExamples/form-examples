...
use crate::engine::utils::{score_band_label, severity_label};
use crate::engine::{dyslexia_grader};
...
async fn show_report(
    ...
    let grade = dyslexia_grader::grade(&assessment_data);
    ...
    // Map per-domain score band labels for the template.
    let domain_score_bands: Vec<String> = grade
        .domain_scores
        .iter()
        .map(|d| score_band_label(d.score))
        .collect();
    ...
    context.insert(
        ...
        &severity_label(&grade.overall_severity),
    );
    context.insert("lowest_score", &grade.lowest_score);
    context.insert("answered_count", &grade.answered_count);
    context.insert("domain_scores", &grade.domain_scores);
    context.insert("domain_score_bands", &domain_score_bands);
