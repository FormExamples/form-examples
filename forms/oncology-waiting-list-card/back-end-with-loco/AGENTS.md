# Oncology Waiting List Card — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Oncology Waiting List Card form, built with axum + Loco +
...
- [`oncology_waiting_list_card/`](./oncology_waiting_list_card/) — the Loco crate: `src/oncology_waiting_list_card/` holds `app.rs`
...
`/metrics`. The registered domain controllers are:
  ...
  - `oncology_waiting_list_card`
  - `oncology_waiting_list_card_appointment`
  - `oncology_waiting_list_card_grade`
  - `oncology_waiting_list_card_grade_flag`
  - `oncology_waiting_list_card_grade_rule`
  - `patient`
  - `practitioner`

## Engine

`src/oncology_waiting_list_card/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd oncology_waiting_list_card && cargo check --all-targets && cargo test
```
