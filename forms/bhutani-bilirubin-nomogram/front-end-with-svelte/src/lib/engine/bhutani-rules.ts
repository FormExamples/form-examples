...
// gestation-band selector used by the classifier in `bhutani-grader.ts`.
//
...
export const AGE_MIN_HOURS = 0;
/** Highest defined age on the nomogram (hours, ≈ 7 days). */
export const AGE_MAX_HOURS = 168;

/** [ageHours, p40, p75, p95] in µmol/L; monotonic in age. */
type PercentileAnchor = [number, number, number, number];
/** [ageHours, thresholdUmolL]. */
type ThresholdAnchor = [number, number];

// ─── Bhutani percentile tracks (prediction) ─────────────────────
// The engine interpolates each track at the measured age, then bands the
// measured TSB.
export const PERCENTILE_ANCHORS: PercentileAnchor[] = [
	//  age   p40   p75   p95
	[0, 34, 60, 86],
	[12, 74, 101, 121],
	[24, 101, 135, 169],
...
/** A gestation band's phototherapy and exchange-transfusion threshold curves. */
export interface ThresholdCurves {
	phototherapy: ThresholdAnchor[];
	exchange: ThresholdAnchor[];
}

// ─── NICE treatment-threshold curves (treatment signal) ─────────
// Each gestation band maps to a phototherapy curve and an exchange curve.
// Thresholds rise with postnatal age then plateau, and are lower for lower
// gestation.
export const THRESHOLD_CURVES: Record<string, ThresholdCurves> = {
...
/**
 * Select the gestation band whose NICE threshold curve applies. The term curve
 * is the safe default when gestational age is not recorded.
 */
export function gestationBand(weeks: number | null): string {
 ...
 */
export function interpolate(anchors: ThresholdAnchor[], x: number): number {
...
export function percentileTracks(ageHours: number): { p40: number; p75: number; p95: number } {
	const p40Anchors: ThresholdAnchor[] = PERCENTILE_ANCHORS.map((r) => [r[0], r[1]]);
	const p75Anchors: ThresholdAnchor[] = PERCENTILE_ANCHORS.map((r) => [r[0], r[2]]);
	const p95Anchors: ThresholdAnchor[] = PERCENTILE_ANCHORS.map((r) => [r[0], r[3]]);
	return {
 ...
 */
export function thresholds(band: string, ageHours: number): { phototherapy: number; exchange: number } {
...
/** Human-readable label for each risk factor (keys mirror RiskFactors). */
