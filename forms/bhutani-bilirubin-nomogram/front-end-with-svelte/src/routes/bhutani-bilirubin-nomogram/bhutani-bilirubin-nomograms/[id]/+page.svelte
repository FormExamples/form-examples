<script lang="ts">
	...
	import Step3Measurement from '$lib/components/steps/Step3Measurement.svelte';
	...
	function validate(): boolean {
		...
		if (d.identification.infantIdentifier.trim() === '') {
			found.push({
				id: 'identification-infantIdentifier',
				message: 'Infant identifier is required.'
			});
		}
		if (d.measurement.ageHours === null) {
			found.push({ id: 'measurement-ageHours', message: 'Age at measurement is required.' });
		}
		if (d.measurement.totalSerumBilirubinUmolL === null) {
			found.push({
				id: 'measurement-totalSerumBilirubinUmolL',
				message: 'Total serum bilirubin is required.'
			});
		}
		errors = found;
	...
	function submit() {
		...
		assessment.result = gradeBhutani(assessment.data);
		goto(`/bhutani-bilirubin-nomogram/bhutani-bilirubin-nomograms/${id}/report`);
	}
...
<main class="mx-16 px-4 py-6">
	<h1 class="text-2xl font-bold text-base-content">
		{isNew ? 'New Bhutani nomogram assessment' : `Bhutani assessment ${id}`}
	</h1>
	<p class="mt-1 text-sm text-base-content/70">
		Complete the five sections; the risk zone and treatment-threshold signals are computed on
		submit.
	</p>
