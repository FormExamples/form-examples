<script lang="ts">
	...
	function init(api: any) {
		...
		api.on('select-row', (ev: { id?: string | number }) => {
			if (ev?.id != null) goto(`/general-practice-waiting-list-card/general-practice-waiting-list-cards/${ev.id}`);
		});
	}
</script>
...
<main class="mx-auto max-w-6xl px-4 py-6">
	<div class="mb-6 flex flex-wrap items-end justify-between gap-4">
		...
		<a href="/general-practice-waiting-list-card/general-practice-waiting-list-cards/new" class="button" data-variant="primary">New card</a>
	</div>
