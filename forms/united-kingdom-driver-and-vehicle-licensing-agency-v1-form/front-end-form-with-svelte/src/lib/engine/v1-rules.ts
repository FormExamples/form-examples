...
/**
 * DVLA V1 (Vision) — branch-aware completeness validation rules.
 *
 * The V1 form is a data-collection instrument, not a scored assessment. Each
 * rule asserts that a required field on the *active* branch has been answered.
 * Rules whose branch is inactive (because of a "No" answer earlier in the
 * conditional flow) trivially pass — they do not produce a fired validation.
 *
 * Severity levels:
 *   error   — required field on active branch is missing → form is incomplete
 *   warning — clinically concerning combination of answers
 *   info    — non-blocking observation for the reviewer
 */
export const v1Rules: ValidationRule[] = [
	...
	},
	{
		...
		evaluate: (d) =>
			!isMonocularBranchActive(d) || d.visionInBothEyes.monocularDeclarationConfirmed,
	...
	},
	{
		...
		message:
			'Please answer whether the double vision has been the same for 6 months or more.'
	},
	{
		id: 'V1-Q9-004',
		section: 'doubleVision',
