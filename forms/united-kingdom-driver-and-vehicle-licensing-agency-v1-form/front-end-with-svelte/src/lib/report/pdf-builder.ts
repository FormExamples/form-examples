...
/**
 * Build the printable DVLA V1 report definition: a completeness summary, the
 * outstanding (missing) fields, and the flagged issues for clinician review.
 */
export function buildPdfDocument(
	...
	const completeness = Math.round(validation.completionRatio * 100);

	return {
		pageSize: 'A4',
		pageMargins: [40, 60, 40, 60],
		header: {
			text: 'DVLA V1 — CONFIDENTIAL MEDICAL INFORMATION (VISION)',
			alignment: 'center',
			margin: [0, 20, 0, 0],
			fontSize: 9,
			color: '#6b7280',
		...
		content: [
			{
				text: `Status: ${statusLabel(validation.isComplete)} (${completeness}% complete)`,
				fontSize: 22,
				bold: true,
				alignment: 'center',
				margin: [0, 0, 0, 4]
			...
			{
				text: `${completeness}% of required fields completed`,
				fontSize: 11,
				alignment: 'center',
				color: '#4b5563',
				margin: [0, 0, 0, 20]
				...
				table: {
					...
					body: [
						...
						[
							...
							field('Vision conditions declared', String(countVisionConditionsDeclared(data)))
						]
					]
				},
			...
			...(validation.firedValidations.length > 0
				? [
						sectionHeader('Outstanding Items'),
						{
							table: {
								...
								body: [
									...
									...validation.firedValidations.map((m) => [
										{ text: m.id, fontSize: 8, color: '#6b7280' },
										{ text: sectionLabels.get(m.section) ?? m.section, fontSize: 9 },
										{ text: m.message, fontSize: 9 }
									])
								]
							},
							layout: 'lightHorizontalLines',
