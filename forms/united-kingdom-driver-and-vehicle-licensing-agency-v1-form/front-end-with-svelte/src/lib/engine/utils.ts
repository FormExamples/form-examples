...
/** Lily token colour classes for a validation severity badge. */
...
/** Whether the applicant self-declares they meet the eyesight standard. */
export function meetsEyesightStandard(data: AssessmentData): boolean {
	const v = data.eyesightStandards.meetsStandard;
	return v === 'yes-without-correction' || v === 'yes-with-correction';
}

/** Count of "Yes"-declared vision conditions across the V1 questionnaire. */
export function countVisionConditionsDeclared(data: AssessmentData): number {
	let n = 0;
	if (data.visionInBothEyes.hasVisionInBothEyes === 'no') n++; // monocular
	if (data.fieldOfVision.hasProblem === 'yes') n++;
	if (data.glaucoma.hasCondition === 'yes') n++;
	if (data.retinitisPigmentosa.hasCondition === 'yes') n++;
	if (data.laserTreatment.hasHadTreatment === 'yes') n++;
	if (data.blepharospasm.hasCondition === 'yes') n++;
	if (data.nightBlindness.hasCondition === 'yes') n++;
	if (data.doubleVision.hasCondition === 'yes') n++;
	if (data.otherVisionConditions.hasOther === 'yes') n++;
	return n;
}
