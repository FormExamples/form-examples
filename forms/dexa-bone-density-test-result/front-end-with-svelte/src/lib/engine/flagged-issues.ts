import type { DexaBoneDensityResult, Flag, FlagPriority } from './types';
import { effectiveWhoClassification, hasCriticalFinding, hasOsteoporosis } from './utils';

/**
 * Detects safety-critical flags independently of the four axes. Flag
 * categories mirror sql/07_create_table_dexa_bone_density_test_result_grade_flag.sql.
 * Flags are returned sorted high → medium → low priority.
 */
export function detectFlags(r: DexaBoneDensityResult): Flag[] {
	const flags: Flag[] = [];
