...
4. `03_create_table_clinician.sql` — reporting clinician (radiologist / consultant / reporting radiographer / rheumatologist / endocrinologist).
5. `04_create_table_dexa_bone_density_test_result.sql` — main result/report record.
6. `05_create_table_dexa_bone_density_test_result_grade.sql` — four-axis interpretation grade.
7. `06_create_table_dexa_bone_density_test_result_grade_rule.sql` — fired-rule audit trail.
8. `07_create_table_dexa_bone_density_test_result_grade_flag.sql` — safety-critical flags.
...
The quantitative findings (T-scores, Z-scores, BMD) are `NUMERIC` and nullable;
the WHO densitometric classification (`who_classification`) is derived from the
lowest (most negative) T-score and is carried into the grade's
`reporting_category` (Axis B). See [`../index.md`](../index.md) for the
T-score → WHO classification → grade-axis mapping.
