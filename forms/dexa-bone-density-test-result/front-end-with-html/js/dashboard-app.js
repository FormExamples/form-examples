...
import { abnormalitySeverityClass, abnormalitySeverityLabel, followUpUrgencyClass, followUpUrgencyLabel, reportStatusLabel, resultClassificationClass, resultClassificationLabel, scanRegionLabel, scoreLabel, whoClassificationClass, whoClassificationLabel } from './types.js';

// DEXA Bone Density Test Result — clinician dashboard (vanilla classic-script app).
//
...
// box + classification dropdown + WHO-classification dropdown +
// follow-up-urgency dropdown). Columns mirror the SvelteKit dashboard route
// (`front-end-with-svelte/src/routes/+page.svelte`).
//
// Sibling modules loaded as plain `<script>` tags (in dependency order) attach
// their exports to `window.DexaBoneDensityTestResultDashboard`; display labels
// come from the form namespace `window.DexaBoneDensityTestResult`
...
// row-cell renderer below. Mirrors the SvelteKit dashboard table.
const columns = [
  { key: 'id',                        label: 'Report' },
  { key: 'patientName',               label: 'Patient' },
  { key: 'scanRegion',                label: 'Region' },
  { key: 'reportStatus',              label: 'Status' },
  { key: 'reportedDate',              label: 'Reported' },
  { key: 'lowestTScore',              label: 'Lowest T' },
  { key: 'whoClassification',         label: 'WHO class' },
...
function matchesFilters(row) {
  ...
  if (filters.who && row.whoClassification !== filters.who) return false;
 ...
 * their rank tables; the numeric columns compare numerically with `null`
 * (unrecorded lowest T-score) always ordered last; everything else uses a
...
function compareRows(a, b) {
  ...
  if (key === 'lowestTScore') {
    // Null (unrecorded) always sorts after any measured score.
  ...
  // Default: string compare (id, patientName, scanRegion, reportStatus,
...
function renderTableBody() {
  ...
  for (const row of rows) {
    ...
    tr.innerHTML = `
      ...
      <td>${esc(scanRegionLabel(row.scanRegion))}</td>
      <td>${esc(reportStatusLabel(row.reportStatus))}</td>
      <td>${esc(row.reportedDate)}</td>
      <td><span class="class-cell">${esc(scoreLabel(row.lowestTScore))}</span></td>
      <td>${badgeCell(whoClassificationClass(row.whoClassification), whoClassificationLabel(row.whoClassification))}</td>
...
function bindFilterInputs() {
  ...
  const who = document.getElementById('filter-who');
