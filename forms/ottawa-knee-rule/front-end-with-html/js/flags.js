// Flagged-issue detection (red flags). Independent of the imaging decision
...
//
// Rows here mirror the `ottawa_knee_rule_grade_flag` SQL table
...
// incomplete-assessment flag. `ageYears` is numeric (null when unanswered);
// the tenderness / flexion / weight-bearing inputs are yes/no enums ('' blank).
 ...
 * @param {boolean} xrayIndicated  - from the grader
 * @returns {FlaggedIssue[]}
 */
function detectFlaggedIssues(data, xrayIndicated) {
  /** @type {FlaggedIssue[]} */
  const flags = [];
  ...
  if (hours == null) {
    ...
    flags.push({
      ...
      priority: 'medium',
      description:
        `Injury is not acute (${hours} hours since injury, over 7 days) — the rule is validated for acute injury; interpret with care`,
      suggestedAction:
  ...
  if (missing.length > 0) {
    flags.push({
      ...
      suggestedAction:
        'Record the missing criterion answer(s) and re-evaluate.'
    });
  }
