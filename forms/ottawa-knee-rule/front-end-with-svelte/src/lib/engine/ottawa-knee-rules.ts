import type { OttawaRule } from './types';
 ...
 * This is a DECISION RULE, not a score: the grader (`ottawa-knee-grader.ts`)
 * applies ANY-of (logical OR) logic — a knee radiograph is indicated when at
 * least one criterion fires. There are no points and no total.
 *
 ...
 * tenderness is surfaced as a flagged issue by `flagged-issues.ts`).
 *
 * Rows mirror the `ottawa_knee_rule_grade_rule` SQL table.
 */
export const ottawaRules: OttawaRule[] = [
	...
	// Fires only when patellar tenderness is present AND there is no other bony
	// tenderness (isolation). Both inputs must be answered for it to fire.
	{
		...
		evaluate: (d) =>
			d.tenderness.patellarTenderness === 'yes' && d.tenderness.otherBonyTenderness === 'no'
	},
