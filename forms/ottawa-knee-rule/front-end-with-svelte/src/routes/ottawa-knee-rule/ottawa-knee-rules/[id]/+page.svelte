<script lang="ts">
	...
	import Step3Age from '$lib/components/steps/Step3Age.svelte';
	import Step4Tenderness from '$lib/components/steps/Step4Tenderness.svelte';
	import Step5Flexion from '$lib/components/steps/Step5Flexion.svelte';
	import Step6WeightBearing from '$lib/components/steps/Step6WeightBearing.svelte';
	...
	function submit() {
		...
		assessment.result = gradeOttawaKnee(assessment.data);
		goto(`/ottawa-knee-rule/ottawa-knee-rules/${id}/report`);
	}
...
<main class="mx-16 px-4 py-6">
	<h1 class="text-2xl font-bold text-base-content">
		{isNew ? 'New Ottawa Knee Rule assessment' : `Ottawa Knee Rule assessment ${id}`}
	</h1>
	<p class="mt-1 text-sm text-base-content/70">
		Complete the seven sections; the imaging decision is computed on submit.
	</p>
	...
	<Form label="Ottawa Knee Rule assessment" onsubmit={submit}>
		<Step1Context />
		<Step2Identification />
		<Step3Age />
		<Step4Tenderness />
		...
		<div class="button-group">
			<Button type="submit" data-variant="primary">Evaluate decision &amp; view report</Button>
