...
//
// Critical criteria (per NREMT Medical Patient station): any 'no' on a rule
// with `critical: true` forces an automatic overall Fail, regardless of how
// many points the candidate has accumulated.
//
// Steps with checklist scoring: 2 Scene Size-Up, 3 Primary Survey, 4 History &
// Secondary Assessment, 5 Reassessment. Step 6 (Critical Criteria Review)
// carries two additional critical-only checks. Step 1 (Candidate, Examiner &
// Scenario) is purely demographic and has no scored rules.

import type { AssessmentData, PsychomotorRule, TriState } from './types';

/** Read a tri-state field, normalising missing/unknown values to ''. */
function read<S extends keyof AssessmentData>(
	d: AssessmentData,
	section: S,
	field: keyof AssessmentData[S]
): TriState {
	const v = d[section][field] as unknown;
	if (v === 'yes' || v === 'no' || v === 'na') return v;
	return '';
}

/** Registry of critical-criterion rule IDs (any 'no' forces an overall Fail). */
...
/** Pass threshold: candidate must achieve at least this percentage of points. */
export const PASS_PERCENT_THRESHOLD = 80;

export const psychomotorRules: PsychomotorRule[] = [
