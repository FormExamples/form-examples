// Prioritised flag detection for the NREMT EMT Psychomotor Skills Examination
// report. Independent of the pass/fail outcome (which the grader computes),
// this module surfaces flags for examiner attention:
//
//   high   - any critical-criterion failure (with reason),
//            domain weakness (>=2 non-critical deficiencies in one category),
//            primary survey skipped (general impression / mental status)
//   medium - primary survey slow / incomplete,
//            missed reassessment block,
//            several non-critical deficiencies overall,
//            point total below pass threshold even with no critical fail
//   low    - single / paired improvement areas,
//            examiner / debrief notes captured

import type { AssessmentData, AdditionalFlag, FiredRule, GradingResult } from './types';
import { PASS_PERCENT_THRESHOLD } from './rules';

/** Threshold for "multiple non-critical deficiencies overall". */
export const MULTI_DEF_THRESHOLD = 3;

/** Threshold for "primary survey appears slow / incomplete". */
export const PRIMARY_SURVEY_INCOMPLETE_THRESHOLD = 2;
...
export function detectAdditionalFlags(
	data: AssessmentData,
	grading: Pick<GradingResult, 'criticalFailures' | 'firedRules' | 'percent' | 'points' | 'maxPoints'>
): AdditionalFlag[] {
	...
	if (psImpression && psImpression.status === 'no') {
		flags.push({
			...
			message:
				'No general impression verbalized — coach candidate to "size up" the patient before approaching.',
			priority: 'high'
		});
	}
	...
	if (
		repeatVitals &&
		repeatVitals.status === 'no' &&
		repeatFocused &&
		repeatFocused.status === 'no'
	) {
		flags.push({
			id: 'FLAG-RA-MISSED',
			category: 'Reassessment',
			message:
				'Both reassessment of vitals and focused exam were missed — practice ongoing reassessment cadence.',
			priority: 'medium'
		});
	...
	// ─── Low: paired non-critical deficiencies (2 items) ──────────────────
