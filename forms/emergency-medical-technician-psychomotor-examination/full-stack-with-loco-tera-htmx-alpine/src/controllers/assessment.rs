...
use crate::engine::{psychomotor_grader, types::AssessmentData, utils};
...
/// Parse a form value string. Empty stays empty; numeric strings become JSON
/// numbers; otherwise pass through.
fn parse_form_value(value: &serde_json::Value) -> serde_json::Value {
    if let Some(s) = value.as_str() {
        if s.is_empty() {
            serde_json::Value::String(String::new())
        } else {
            // Tri-state and free-text fields stay as strings.
            value.clone()
        }
    } else {
        value.clone()
...
/// POST /assessment/{id}/submit -- merge form data into JSONB blob, redirect to report.
#[debug_handler]
...
/// GET /assessment/{id}/report -- run grader, persist, render report.
#[debug_handler]
async fn show_report(
    ...
    let grade = psychomotor_grader::grade(&assessment_data);
    ...
    context.insert("points", &grade.points);
    context.insert("max_points", &grade.max_points);
    context.insert("percent", &grade.percent);
