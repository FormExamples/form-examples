(function (root) {
  ...
  ns.detectAdditionalFlags = function detectAdditionalFlags(card, todayIso) {
    ...
    if (
      (wl.clinicalPriority === 'P1a' || wl.clinicalPriority === 'P1b') &&
    ...
    if (
      ...
      flags.push({
        ...
        priority: 'high',
        description:
          'Suspected-cancer referral has no appointment within ' +
          TWO_WEEK_WAIT_DAYS +
          ' days.',
        suggestedAction:
    ...
    if (
      ...
      flags.push({
        ...
        priority: 'medium',
        description:
          'No appointment scheduled more than ' +
          MISSING_APPOINTMENT_DAYS +
          ' days after RTT clock-start.',
        suggestedAction: 'Contact the patient and book the next appropriate appointment.'
      });
    }

    if (pt.interpreterRequired === 'yes' && String(appt.accessNotes || '').trim() === '') {
      flags.push({
    ...
    if (
      String(pt.accessibilityNeeds || '').trim() !== '' &&
      appt.appointmentDate !== null &&
      String(appt.accessNotes || '').trim() === ''
    ) {
      flags.push({
    ...
    const hasContact =
      String(pt.email || '').trim() !== '' ||
      String(pt.phone || '').trim() !== '' ||
      String(pt.postalAddressAsFullText || '').trim() !== '';
    if (!hasContact) {
      flags.push({
