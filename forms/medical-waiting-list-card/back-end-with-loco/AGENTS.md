# Medical Waiting List Card — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Medical Waiting List Card form, built with axum + Loco +
...
- [`medical_waiting_list_card/`](./medical_waiting_list_card/) — the Loco crate: `src/medical_waiting_list_card/` holds `app.rs`
...
`/metrics`. The registered domain controllers are:
  ...
  - `medical_waiting_list_card`
  - `medical_waiting_list_card_appointment`
  - `medical_waiting_list_card_grade`
  - `medical_waiting_list_card_grade_flag`
  - `medical_waiting_list_card_grade_rule`
  - `patient`
  - `practitioner`

## Engine

`src/medical_waiting_list_card/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd medical_waiting_list_card && cargo check --all-targets && cargo test
```
