...
for the CAGE specification and scoring engine. Lily Svelte headless
conventions: [`../../AGENTS-front-end-svelte.md`](../../AGENTS-front-end-svelte.md).

## Layout

- `src/lib/engine/` — pure scoring engine (`types.ts`, `utils.ts`,
  `cage-rules.ts`, `cage-grader.ts`, `flagged-issues.ts`) + `cage-grader.test.ts`.
...
- `src/routes/cage-alcohol-questionnaires/` — RESTful routes:
