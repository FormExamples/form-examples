<script lang="ts">
	...
	import Step3CutDown from '$lib/components/steps/Step3CutDown.svelte';
	import Step4Annoyed from '$lib/components/steps/Step4Annoyed.svelte';
	import Step5Guilty from '$lib/components/steps/Step5Guilty.svelte';
	import Step6EyeOpener from '$lib/components/steps/Step6EyeOpener.svelte';
	...
	function submit() {
		...
		assessment.result = calculateCageGrade(assessment.data);
		goto(`/cage-alcohol-questionnaire/cage-alcohol-questionnaires/${id}/report`);
	}
...
<main class="mx-16 px-4 py-6">
	<h1 class="text-2xl font-bold text-base-content">
		{isNew ? 'New CAGE assessment' : `CAGE assessment ${id}`}
	</h1>
	<p class="mt-1 text-sm text-base-content/70">
		Complete the seven sections; the CAGE score and result band are computed on submit.
	</p>
	...
	<Form label="CAGE assessment" onsubmit={submit}>
		<Step1Context />
		<Step2Identification />
		<Step3CutDown />
		<Step4Annoyed />
