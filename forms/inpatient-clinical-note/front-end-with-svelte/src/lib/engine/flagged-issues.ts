...
import { atLeast } from './acuity';
import { has } from './note-rules';
import type { AcuityBand, AssessmentData, FlaggedIssue } from './types';

/** Length of stay in whole days, or null when either timestamp is missing. */
export function lengthOfStayDays(data: AssessmentData): number | null {
...
export function detectFlaggedIssues(
	...
	if (atLeast(acuityBand, 'escalate') && !has(pl.escalationAction)) {
		flags.push({
			...
			priority: 'high',
			description: `The acuity band is ${acuityBand}, but no escalation action is recorded on this note.`,
			suggestedAction:
	...
	if (risks.vteStatus === 'not-done') {
		flags.push({
			...
			suggestedAction: 'Complete the VTE risk assessment and prescribe or document prophylaxis.'
		});
	}
	...
	if (unactioned.length > 0) {
		...
		flags.push({
			...
			suggestedAction: 'Review each abnormal result, act on it, and record the action taken.'
		});
	}
	...
	if (med.rows.length > 0 && med.allergyChecked !== 'yes') {
		flags.push({
			...
			suggestedAction: 'Check and record the allergy status before prescribing.'
		});
	}
	...
	if (has(pl.escalationStatus) && !has(pl.ceilingOfCare)) {
		flags.push({
			...
			priority: 'medium',
			description: 'An escalation status is recorded without a corresponding ceiling of care.',
			suggestedAction:
	...
	if (documentedRequired < totalRequired) {
		...
		flags.push({
			...
			suggestedAction: 'Complete the missing components before signing the entry.'
		});
	}
