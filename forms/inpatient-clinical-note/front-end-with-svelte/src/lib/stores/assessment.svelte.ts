...
import { emptyAssessment, type AssessmentData, type GradingResult } from '#lib/engine/types.js';

/** localStorage draft key for a given note id (defaults to `new`). */
function storageKey(id: string): string {
	return `inpatient-clinical-note.front-end-with-svelte.${id || 'new'}.v1`;
}

/**
 * A blank inpatient clinical note with all fields at their unanswered defaults.
...
/**
 * Svelte 5 reactive store for the inpatient clinical note, with localStorage persistence
