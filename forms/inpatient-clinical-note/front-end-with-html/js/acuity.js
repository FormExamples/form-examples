...
import { ACUITY_ORDER } from './types.js';
import { effectiveNews2 } from './news2.js';
 ...
 * @returns {{band: AcuityBand, firedRules: FiredRule[], news2: object}}
 */
function evaluateAcuity(data) {
  ...
  const fire = (id, proposed, description, category) => {
    firedRules.push({
      id,
      engine: 'acuity',
      component: 'acuity',
  ...
  if (total !== null) {
    if (total >= 9) {
      ...
      fire('A-NEWS2-HIGH', 'escalate', `NEWS2 ${total} — RCP high-risk band, emergency assessment by a critical-care-competent team`, 'news2');
    } else if (total >= 5) {
      fire('A-NEWS2-MEDIUM', 'watch', `NEWS2 ${total} — RCP medium-risk band, urgent review by a clinician competent in acute illness`, 'news2');
    } else if (!news2.anyParameterScoresThree) {
      fire('A-NEWS2-LOW', 'stable', `NEWS2 ${total} — RCP low-risk band, no single parameter scoring 3`, 'news2');
    }

    if (news2.anyParameterScoresThree && total < 7) {
      fire('A-NEWS2-SINGLE-3', 'watch', 'A single NEWS2 parameter scores 3 — RCP low-medium band, review regardless of the aggregate', 'news2');
    }
  }

  if (obs.news2Trend === 'worsening') {
    fire('A-NEWS2-TREND', 'watch', 'NEWS2 trend is worsening — a rising score predicts deterioration better than a single reading', 'news2');
  }
  ...
  if (a.newConfusion === 'yes' && obs.acvpu && obs.acvpu !== 'alert') {
    fire('A-NEW-CONFUSION', 'escalate', 'New confusion with an ACVPU below Alert — a core delirium and sepsis marker', 'deterioration-marker');
  }

  if (a.sepsisScreen === 'positive') {
    fire('A-SEPSIS', 'escalate', 'Sepsis screen positive — NICE NG51 requires senior review and the sepsis pathway', 'deterioration-marker');
  }

  const unresolvedAbnormal = inv.rows.some((r) => r.abnormal === 'yes' && r.actioned !== 'yes');
  if (unresolvedAbnormal) {
    fire('A-ABNORMAL-UNRESOLVED', 'escalate', 'An abnormal investigation result has not been actioned', 'deterioration-marker');
  }
  ...
  if (a.criticalCareReferral === 'yes') {
    fire('A-CRITICAL-CARE', 'critical', 'Critical-care outreach or ICU referral made', 'deterioration-marker');
  }

  if (a.newOrganSupport && a.newOrganSupport !== 'none') {
    fire('A-ORGAN-SUPPORT', 'critical', `New organ support started (${a.newOrganSupport}) — level 2 or 3 care by definition`, 'deterioration-marker');
  }

  return { band, firedRules, news2 };
}

export { maxBand, atLeast, evaluateAcuity };
