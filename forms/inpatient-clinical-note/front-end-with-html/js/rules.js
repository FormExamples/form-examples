...
//
// The note is a documentation-and-completeness instrument, not a numeric score.
...
//
//   componentPresence(data)      -> per-component `documented` booleans
//   requiredComponentKeys(type)  -> the effective required set for a note type
//   componentRules(data)         -> every component as a rule row, with
//                                   `required` resolved for this note's type
//   requiredComponents(data)     -> just the required rows
//   recommendedComponents(data)  -> just the non-required rows

import {
  COMPONENTS,
  NOTE_TYPE_EXTRA_REQUIRED
} from './types.js';
import { hasFullObservationSet } from './news2.js';
 ...
 * @property {boolean} required   - required for THIS note's type
...
function componentPresence(data) {
  ...
  return {
    'header':
      has(h.noteType) && has(h.noteAt) && has(h.authorName) && has(h.authorGrade),
    'interval-history':
      has(iv.intervalHistory) || iv.noIntervalEvents === 'yes',
    'observations':
      obs.news2Total !== null || hasFullObservationSet(obs),
    'examination': hasAny(
      ex.general,
      ex.cardiovascular,
      ex.respiratory,
    ...
    'investigations':
      inv.rows.length > 0 || inv.noInvestigationsReviewed === 'yes',
    'problems': data.problems.rows.length > 0,
    'medications':
      med.rows.length > 0 || med.noMedicationChanges === 'yes',
    ...
    'communication': hasAny(
      so.familyCommunication,
      so.patientCommunication,
      so.teamHandover
    )
  };
}
 ...
 * @returns {string[]}
 */
function requiredComponentKeys(noteType) {
...
const COMPONENT_DESCRIPTIONS = {
  'header':
    'Header: note type, date and time, author name and grade recorded',
  'interval-history':
  ...
  'examination':
    'Examination: at least one system examined',
  'investigations':
    'Investigations: at least one result reviewed, or an explicit "none reviewed"',
  'problems':
    'Problems: at least one problem on the list',
  'medications':
    'Medications: at least one prescribing change, or an explicit "no changes"',
  'risk-assessments':
    'Risk assessments: VTE status recorded (NICE NG89)',
  'impression':
    'Impression: a clinical impression recorded',
  'plan':
    'Plan: a narrative plan, or at least one job',
  'escalation':
...
function componentRules(data) {
  ...
  return COMPONENTS.map((c) => {
    ...
    return {
      ...
      category: required ? 'required-component' : 'recommended-component',
      label: c.label,
...
/**
 * The recommended components. They do not affect the status but are surfaced
 * in `documentedComponents[]`.
 * @param {AssessmentData} data
 * @returns {RuleComponent[]}
 */
