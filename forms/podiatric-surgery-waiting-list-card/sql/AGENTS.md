# Podiatric Surgery Waiting List Card — PostgreSQL migrations
...
Child `podiatric_surg_waiting_list_card_*` tables reference
`podiatric_surg_waiting_list_card(id) ON DELETE CASCADE`. The grade child uses a
