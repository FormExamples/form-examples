# Dermatology Assessment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Dermatology Assessment form, built with axum + Loco +
...
- [`dermatology_assessment/`](./dermatology_assessment/) — the Loco crate: `src/dermatology_assessment/` holds `app.rs`
...
`src/dermatology_assessment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd dermatology_assessment && cargo check --all-targets && cargo test
```
