// Flagged-issue detection for the Dermatology Assessment.
//
// Independent of the DLQI score (which the grader computes), this module
// raises clinician-facing flags for melanoma suspicion (ABCDE-style criteria),
// rapidly changing lesions, immunosuppression, cancer history, autoimmune
// disease, latex allergy, drug-allergy anaphylaxis, family history of
// melanoma / skin cancer, UV exposure risk, ulcerated or large lesions, and
// biologic therapy.
...
function detectAdditionalFlags(data) {
  ...
  if (data.currentMedications.biologics.length > 0) {
    const names = data.currentMedications.biologics
      .map((b) => b.name)
      .filter(Boolean)
      .join(', ');
    flags.push({
      ...
      category: 'Treatment',
      message: `On biologic therapy${names ? ` (${names})` : ''} - monitor for infections and malignancy.`,
      priority: 'medium'
    });
  }
