...
// runs the pure Braden Scale scoring engine and renders an inline report.
// State is persisted to localStorage so a partial fill survives a reload.
//
// Sibling files loaded as plain `<script>` tags (in order) attach their
// exports to `window.IntegumentaryAssessment`. Pulling them off here keeps
 ...
 * Re-runs derived values (BMI, wound area), progress, and conditional
...
function bradenRadioGroup(opts) {
  ...
  const list = document.createElement('div');
  list.className = 'radio-options stacked';
  for (const option of opts.options) {
    const radioId = `${groupId}-${option.value}`;
    const label = document.createElement('label');
    label.className = 'radio-option';
...
function checkboxGroup(opts) {
  ...
  const current = state[opts.section][opts.field] || [];
  const wrapper = document.createElement('fieldset');
  wrapper.className = 'field check-group';
  ...
  for (const option of opts.options) {
    ...
    label.innerHTML = `
      <input type="checkbox" id="${checkId}" name="${groupId}" value="${esc(option.value)}"${checked}>
      <span>${esc(option.label)}</span>
    `;
    const input = label.querySelector('input');
    input.addEventListener('change', () => {
...
/** Editor for an array of {site, type, size, description} lesion rows. */
...
/** Editor for an array of {site, date, reference} photo rows. */
