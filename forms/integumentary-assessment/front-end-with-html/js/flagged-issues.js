...
// (e.g. necrotic tissue, sepsis signs), undernutrition, immobility,
// incontinence-driven moisture, photographic-consent gaps, and missing
// referrals.
...
function detectAdditionalFlags(data) {
  ...
  if (data.woundAssessment.woundPresent === 'yes') {
    if (data.woundAssessment.woundStage === 'stage-iv' ||
    ...
    if (data.woundAssessment.tissueType === 'necrotic' ||
    ...
    if (data.woundAssessment.woundOdour === 'strong' ||
        data.woundAssessment.woundOdour === 'foul') {
      flags.push({
        id: 'FLAG-WOUND-006',
        category: 'Wound',
  ...
  if (data.skinInspection.integrity === 'breakdown' ||
  ...
  if (data.skinInspection.turgor === 'tenting' ||
      data.skinInspection.turgor === 'poor') {
    flags.push({
      id: 'FLAG-SKIN-002',
  ...
  if (data.skinInspection.lesionTypes && data.skinInspection.lesionTypes.includes('ulcer')) {
    flags.push({
      id: 'FLAG-SKIN-005',
      category: 'Lesions',
  ...
  if (data.skinInspection.lesionTypes && data.skinInspection.lesionTypes.includes('suspicious-pigmented')) {
    flags.push({
      id: 'FLAG-SKIN-006',
      category: 'Lesions',
  ...
  if (data.hairScalpExamination.scalpFindings && data.hairScalpExamination.scalpFindings.includes('lice')) {
    flags.push({
      id: 'FLAG-HAIR-002',
      category: 'Scalp',
  ...
  // ─── Nail alerts ──────────────────────────────────────────
  if (data.nailExamination.nailFindings && data.nailExamination.nailFindings.includes('clubbing')) {
    flags.push({
      id: 'FLAG-NAIL-001',
      category: 'Nails',
  ...
  if (data.nailExamination.nailFindings && data.nailExamination.nailFindings.includes('koilonychia')) {
    flags.push({
      id: 'FLAG-NAIL-002',
      category: 'Nails',
  ...
  if (data.nailExamination.nailCapillaryRefill === 'sluggish' ||
  ...
  if (data.presentingSkinConcern.painScore !== null &&
      data.presentingSkinConcern.painScore >= 7) {
    flags.push({
      id: 'FLAG-CONCERN-003',
      category: 'Pain',
  ...
  if (data.photographyDocumentation.photosTaken === 'yes' &&
  ...
  // ─── Care-plan alerts ────────────────────────────────────
  if (data.clinicalImpressionCarePlan.referralRequired === 'yes' &&
