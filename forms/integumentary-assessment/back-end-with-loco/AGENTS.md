# Integumentary Assessment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Integumentary Assessment form, built with axum + Loco +
...
- [`integumentary_assessment/`](./integumentary_assessment/) — the Loco crate: `src/integumentary_assessment/` holds `app.rs`
...
`src/integumentary_assessment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd integumentary_assessment && cargo check --all-targets && cargo test
```
