// BMI and BSA Calculator — clinician dashboard (vanilla classic-script app).
//
...
// box + care-setting dropdown + category dropdown + severe dropdown).
//
// Sibling modules loaded as plain `<script>` tags (in dependency order) attach
// their exports to `window.BmiBsaCalculatorDashboard`. The whole file is wrapped
...
const filters = {
  ...
  category: '',  // '' | 'underweight' | 'normal' | ... | 'unknown'
  severe: ''     // '' | 'yes' | 'no'
};
...
const columns = [
  ...
  { key: 'bmiCategory',       label: 'WHO category' },
  { key: 'bsaMosteller',      label: 'BSA (m²)' },
  { key: 'severeFlag',        label: 'Severe' }
];

// Rank used when sorting the category column so the bands order from lowest to
// highest weight status, with unknown last, regardless of locale.
const categoryRank = {
  'underweight': 0,
  'normal': 1,
  'overweight': 2,
...
const categoryLabels = {
  ...
  'obese-class-1': 'Obese class I',
  'obese-class-2': 'Obese class II',
  'obese-class-3': 'Obese class III',
  'unknown': 'Unknown'
};
...
function categoryLabel(category) {
  return categoryLabels[category] || 'N/A';
}
...
function matchesFilters(row) {
  ...
  if (filters.category && row.bmiCategory !== filters.category) return false;
 ...
 * rank table; the nullable BMI and BSA sort nulls last; the severe boolean
...
function compareRows(a, b) {
  ...
  if (key === 'bmiCategory') {
    av = categoryRank[av] ?? 99;
    bv = categoryRank[bv] ?? 99;
    return (av - bv) * dir;
  }

  if (key === 'bmi' || key === 'bsaMosteller') {
...
function renderTableBody() {
  ...
  for (const row of rows) {
    ...
    const bmiClassName = (row.bmi === null || row.bmi === undefined)
    ...
    tr.innerHTML = `
      ...
      <td><span class="${bmiClassName}">${esc(bmiLabel(row.bmi))}</span></td>
      <td><span class="risk-badge ${categoryClass(row.bmiCategory)}">${esc(categoryLabel(row.bmiCategory))}</span></td>
      <td>${esc(bsaLabel(row.bsaMosteller))}</td>
      <td>
