...
/**
 * Detect clinician-facing safety flags (red flags), independent of the WHO
 * category, using the unrounded BMI and the raw height/weight inputs (spec §5):
 *
 *   - Severe obesity (high)         — bmi >= 40
 ...
 */
export function detectFlaggedIssues(data: AssessmentData, bmi: number | null): FlaggedIssue[] {
	...
	const heightMissing = heightCm === null || heightCm === undefined || !(heightCm > 0);
	const weightMissing = weightKg === null || weightKg === undefined || !(weightKg > 0);
	...
	if (bmi === null || heightMissing || weightMissing) {
		...
		flags.push({
			...
			suggestedAction: 'Record the measured height and weight, then re-calculate.'
		});
		return flags;
	}
