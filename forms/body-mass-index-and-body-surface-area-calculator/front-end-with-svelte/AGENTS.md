...
for the BMI/BSA specification and calculation engine. Lily Svelte headless
...
- `src/lib/engine/` — pure calculation engine (`types.ts`, `utils.ts`,
  `bmi-bsa-rules.ts`, `bmi-bsa-grader.ts`, `flagged-issues.ts`) +
  `bmi-bsa-grader.test.ts`.
...
- `src/routes/body-mass-index-and-body-surface-area-calculators/` — RESTful routes:
  `/<plural>/` (dashboard, `ssr = false`) + `/<plural>/[id]` (wizard) +
  `/<plural>/[id]/report` (+ `report/pdf` server endpoint).

## Calculation engine

Formula-based (not additive): `BMI = weightKg ÷ (heightCm ÷ 100)²`,
`BSA (Mosteller) = √((heightCm × weightKg) ÷ 3600)`, and
`BSA (Du Bois) = 0.007184 × heightCm^0.725 × weightKg^0.425`, computed only when
both inputs are present and positive. The unrounded BMI drives WHO banding and
every flag threshold; the rounded values are display-only (BMI 1 dp, BSA 2 dp).
Severe obesity (≥ 40) and underweight (< 18.5) raise high-priority flags; the
Asian action points (≥ 23, ≥ 27.5) are recorded when ancestry is Asian.
