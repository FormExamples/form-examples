 ...
 * @returns {boolean}
 */
function noProgressOverWindow(observations) {
  const points = observations
    .filter((o) => num(o.cervicalDilatationCm) !== null && o.observedAt)
    .map((o) => ({ d: /** @type {number} */ (num(o.cervicalDilatationCm)), t: Date.parse(o.observedAt) }))
 ...
 * @param {GradingResult} grade  - result from gradePartogram
...
function detectFlaggedIssues(data, grade) {
  ...
  const abnormalFhr = observations
    ...
    .filter((v) => v !== null && (v < FHR_LOW_BPM || v > FHR_HIGH_BPM));
  ...
  const feverTemps = observations
    ...
    .filter((v) => v !== null && v >= TEMPERATURE_FEVER_C);
  ...
  const hypertensive = observations.some(
    (o) =>
      (num(o.systolicBloodPressure) !== null && num(o.systolicBloodPressure) >= SYSTOLIC_HYPERTENSION_MMHG) ||
      (num(o.diastolicBloodPressure) !== null && num(o.diastolicBloodPressure) >= DIASTOLIC_HYPERTENSION_MMHG)
  );
  if (hypertensive) {
    flags.push({
  ...
  const tachy = observations
    .map((o) => num(o.pulse))
    .filter((v) => v !== null && v >= PULSE_TACHYCARDIA_BPM);
  if (tachy.length > 0) {
    flags.push({
  ...
  const hypotension = observations
    ...
    .filter((v) => v !== null && v < SYSTOLIC_HYPOTENSION_MMHG);
  ...
  if (observations.some((o) => o.urineProtein !== '' && o.urineProtein !== 'negative')) {
    flags.push({
      ...
      suggestedAction:
        'Assess for pre-eclampsia alongside blood pressure and symptoms.'
    });
  }
