// Backend API client for the Partogram clinician dashboard.
//
...
// `DashboardPartogramsResponse`. When the fetch fails (CORS, network, server
...
/**
 * Fetch the partogram list from the backend.
 *
 ...
 *
 * @returns {Promise<import('./dashboard-types.js').PartogramRow[]>}
 */
async function fetchPartograms() {
  const res = await fetch(`${API_BASE}${PARTOGRAMS_PATH}`);
  if (!res.ok) {
    throw new Error(
      `Failed to fetch partograms: ${res.status} ${res.statusText}`
    );
  }
  /** @type {import('./dashboard-types.js').DashboardPartogramsResponse} */
