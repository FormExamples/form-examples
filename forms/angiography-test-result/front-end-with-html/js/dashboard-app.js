...
// Angiography Test Result — clinician dashboard (vanilla classic-script app).
//
...
// (search box + classification dropdown + urgency dropdown + status
...
// Ranks used when sorting the graded columns so clinical ordering wins over
...
/**
 * Compare two rows for the active sort column. The graded columns use their
 * clinical rank tables; the numeric columns compare numerically; everything
...
function compareRows(a, b) {
  ...
  // reportedDate — ISO dates sort correctly as strings)
...
function renderTableBody() {
  ...
  for (const row of rows) {
    ...
    tr.innerHTML = `
      <td><span class="class-cell">${esc(row.id)}</span></td>
