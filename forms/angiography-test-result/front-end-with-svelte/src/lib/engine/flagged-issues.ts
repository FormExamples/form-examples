import type { AngiographyResult, Flag, FlagPriority } from './types';
 ...
 */
export function detectFlags(r: AngiographyResult): Flag[] {
	const flags: Flag[] = [];
	...
	// ─── urgent-referral ───
	if (r.maxStenosisPercent !== null && r.maxStenosisPercent >= 70 && !hasCriticalFinding(r)) {
		flags.push({
			flagId: 'F-URGENT-REFERRAL-001',
			category: 'urgent-referral',
			priority: 'medium',
			description: 'High-grade stenosis (>= 70 %) is present and may warrant urgent referral.',
			suggestedAction: 'Consider urgent referral to the appropriate vascular / interventional team.'
		});
	}
	...
	if (r.significantStenosis && r.maxStenosisPercent === null) {
		flags.push({
			...
			priority: 'low',
			description: 'A significant stenosis is reported but no maximum stenosis percentage was recorded.',
			suggestedAction: 'Record the maximum stenosis percentage (e.g. NASCET-style) for grading.'
		});
	}

	// ─── unexpected-finding (abnormal but no originating request linked) ───
	if (
		(r.significantStenosis || r.aneurysm || r.dissection) &&
