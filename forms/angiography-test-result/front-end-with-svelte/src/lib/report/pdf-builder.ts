...
import type { AngiographyResult, GradingResult } from '#lib/engine/types.js';
import {
...
/** Builds the pdfmake document definition for the angiography report. */
export function buildPdfDocument(
	data: AngiographyResult,
	...
	return {
		...
		header: {
			text: 'ANGIOGRAPHY VASCULAR-IMAGING REPORT',
			alignment: 'center',
			margin: [0, 20, 0, 0],
			fontSize: 10,
			color: '#6b7280',
		...
		footer: (currentPage: number, pageCount: number) => ({
			text: `Page ${currentPage} of ${pageCount} | Generated ${new Date(result.gradedAt).toLocaleString()} | RCR / NASCET-ECST / IR(ME)R 2017`,
			alignment: 'center',
			margin: [0, 20, 0, 0],
			fontSize: 8,
			color: '#9ca3af'
		...
		content: [
			...
			{
				text: `${angiographyTypeLabel(data.angiographyType)} — ${bodyRegionLabel(data.bodyRegion)} — ${reportStatusLabel(data.reportStatus)}`,
				fontSize: 12,
				alignment: 'center',
				color: '#4b5563',
				margin: [0, 0, 0, 4] as Margin
			...
			{
				ul: [
					`Angiography type: ${angiographyTypeLabel(data.angiographyType)}`,
					`Body region: ${bodyRegionLabel(data.bodyRegion)}`,
					`Contrast: ${data.contrastUsed || 'N/A'}`,
					`Adequacy: ${data.examinationAdequacy || 'N/A'}`,
					`Maximum stenosis: ${data.maxStenosisPercent !== null ? `${data.maxStenosisPercent}%` : 'N/A'}`,
					`Intervention performed: ${data.interventionPerformed ? 'Yes' : 'No'}`
				],
				margin: [0, 0, 0, 16] as Margin
			},
...
function structuredFindingList(data: AngiographyResult): string[] {
	const list: string[] = [];
	if (data.significantStenosis)
		list.push(
			`Significant stenosis${data.maxStenosisPercent !== null ? ` (${data.maxStenosisPercent}%)` : ''}`
		);
