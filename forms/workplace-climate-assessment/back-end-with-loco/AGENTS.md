# Workplace Climate Assessment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Workplace Climate Assessment form, built with axum + Loco +
...
- [`workplace_climate_assessment/`](./workplace_climate_assessment/) — the Loco crate: `src/workplace_climate_assessment/` holds `app.rs`
...
`src/workplace_climate_assessment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd workplace_climate_assessment && cargo check --all-targets && cargo test
```
