<script lang="ts">
	...
	// 'psychSafety' domain, bound to the live store section.
	...
	const meta = DOMAINS.find((dm) => dm.key === 'psychSafety')!;
	const items = surveyItems.filter((it) => it.domain === 'psychSafety');
	const d = assessment.data.psychSafety as unknown as Record<string, LikertValue>;
</script>
