<script lang="ts">
	...
	// 'inclusion' domain, bound to the live store section.
	...
	const meta = DOMAINS.find((dm) => dm.key === 'inclusion')!;
	const items = surveyItems.filter((it) => it.domain === 'inclusion');
	const d = assessment.data.inclusion as unknown as Record<string, LikertValue>;
</script>
