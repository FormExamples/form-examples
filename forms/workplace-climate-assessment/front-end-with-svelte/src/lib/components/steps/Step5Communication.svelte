<script lang="ts">
	...
	// 'communication' domain, bound to the live store section.
	...
	const meta = DOMAINS.find((dm) => dm.key === 'communication')!;
	const items = surveyItems.filter((it) => it.domain === 'communication');
	const d = assessment.data.communication as unknown as Record<string, LikertValue>;
</script>
