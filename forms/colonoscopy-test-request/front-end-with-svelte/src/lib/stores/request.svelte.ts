...
import type { ColonoscopyRequest, GradingResult } from '#lib/engine/types.js';
import { createDefaultRequest } from '#lib/engine/defaults.js';

// Re-export the default factory so existing imports of the store can reach it.
export { createDefaultRequest };

/** localStorage draft key for a given request id (defaults to `new`). */
function storageKey(id: string): string {
	return `colonoscopy-test-request.front-end-with-svelte.${id || 'new'}.v1`;
}

/**
 * Svelte 5 reactive store for the colonoscopy request, with localStorage
 ...
 */
class RequestStore {
	data = $state<ColonoscopyRequest>(createDefaultRequest());
	 ...
	 * `const d = request.data; d.clinician`) stay bound to live state.
	 */
	loadForId(id: string, seed?: ColonoscopyRequest) {
		const key = id || 'new';
		this.id = key;
		this.result = null;
		this.currentStep = 1;

		let draft: ColonoscopyRequest | null = null;
		if (browser) {
			const raw = localStorage.getItem(storageKey(key));
			if (raw) {
				try {
					draft = JSON.parse(raw) as ColonoscopyRequest;
				} catch {
