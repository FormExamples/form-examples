...
impl MigrationTrait for Migration {
    async fn up(&self, m: &SchemaManager) -> Result<(), DbErr> {
        create_table(m, "colonoscopy_test_request_grades",
            &[
            ...
            ("two_week_wait_rationale", ColType::StringWithDefault(String::new())),
            ...
            &[
            ("colonoscopy_test_request", ""),
            ]
        ).await
    }

    async fn down(&self, m: &SchemaManager) -> Result<(), DbErr> {
        drop_table(m, "colonoscopy_test_request_grades").await
    }
}
