...
use crate::engine::utils::{
    acknowledgement_status, acknowledgement_status_label, completeness_label,
};
use crate::engine::{form_validator, types::AssessmentData};
...
/// Parse a form value. Empty strings stay as `""`; the special checkbox
/// value `"on"` (HTML default) becomes JSON `true`. Numbers are coerced
/// so future numeric fields would deserialize correctly.
fn parse_form_value(field: &str, value: &serde_json::Value) -> serde_json::Value {
    if let Some(s) = value.as_str() {
        // Acknowledgement boolean checkbox.
        if field == "agreed" {
            return serde_json::Value::Bool(!s.is_empty() && s != "false");
        }
        if s.is_empty() {
...
async fn submit_assessment(
    ...
    // Ensure the boolean acknowledgement checkbox is recorded as `false`
    // when the form omits it (unchecked checkboxes are not submitted by
    // browsers).
...
async fn show_report(
    ...
    context.insert("ack_status", &ack_status);
    context.insert(
        "ack_status_label",
