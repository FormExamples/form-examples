<script lang="ts">
  ...
  const title = 'Research & Planning Privacy Notice';
  const subtitle =
...
<main>
  {#if submitted && assessment.result}
    ...
    <Panel label="Preferences & Signature">
      <h3>Preferences & Signature</h3>
      <dl class="summary-grid">
        <dt>Acknowledged</dt>
        <dd>{assessment.data.acknowledgementSignature.agreed ? 'Yes' : 'No'}</dd>
        <dt>Type 1 opt-out</dt>
        <dd>{assessment.data.acknowledgementSignature.type1OptOut || 'N/A'}</dd>
        <dt>National Data Opt-Out</dt>
        <dd>{assessment.data.acknowledgementSignature.nationalDataOptOut || 'N/A'}</dd>
  ...
  {:else}
    <div class="intro">
      <p>
        Read the three sections below — recipient details, the research and
        planning privacy notice, and the opt-out preference and
        acknowledgement — then submit to record your acknowledgement.
      </p>
    </div>
    ...
    <footer class="page-footer">
      <p>
        For information-governance records. Aligned with UK GDPR and the
