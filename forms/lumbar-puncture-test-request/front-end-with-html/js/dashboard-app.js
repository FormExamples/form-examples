...
// Lumbar Puncture Test Request — vetting dashboard
...
// contraindication-band dropdown + safety-flags dropdown).
//
// Sibling modules loaded as plain `<script>` tags (in dependency order)
// attach their exports to `window.LumbarPunctureTestRequestDashboard`.
...
const columns = [
  ...
  { key: 'procedureIntent',     label: 'Intent' },
  { key: 'indication',          label: 'Indication' },
  { key: 'appropriatenessBand', label: 'Appropriateness' },
  { key: 'contraindicationBand', label: 'Contraindication' },
...
function contraClass(band) {
  return band ? 'contra-' + String(band) : '';
}
...
function matchesFilters(row) {
  if (filters.search) {
    ...
    const matches =
      ...
      String(row.procedureIntent || '').toLowerCase().includes(term) ||
  ...
  if (filters.contra && row.contraindicationBand !== filters.contra) return false;
...
function compareRows(a, b) {
  ...
  if (key === 'contraindicationBand') {
    return ((contraRank[av] ?? -1) - (contraRank[bv] ?? -1)) * dir;
  }
  if (key === 'triageTier') {
  ...
  // Default: string compare (nhs, patient, procedureIntent, indication,
...
function renderTableBody() {
  ...
  for (const row of rows) {
    ...
    if (row.triageTier === 'emergency' || row.contraindicationBand === 'contraindicated') {
    ...
    tr.innerHTML = `
      ...
      <td class="data-table-td">${esc(titleCase(row.procedureIntent))}</td>
      ...
      <td class="data-table-td"><span class="contra-badge ${contraClass(row.contraindicationBand)}">${esc(titleCase(row.contraindicationBand))}</span></td>
...
function bindFilterInputs() {
  ...
  const contra = document.getElementById('filter-contra');
