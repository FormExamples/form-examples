...
// SQL migration 07: suspected-raised-icp-needs-imaging,
// suspected-meningitis-emergency, coagulopathy, high-bleeding-risk-anticoag,
// thrombocytopenia, local-infection, missing-indication,
...
function detectFlags(data, context) {
  ...
  if (
    ...
    flags.push({
      ...
      suggestedAction: 'Obtain CT head and stabilise before LP; LP risks cerebral herniation if ICP is raised.'
    });
  }
  ...
  if (data.procedure.primaryIndication === 'suspected-meningitis') {
    flags.push({
      ...
      suggestedAction: 'Do not delay antibiotics for LP; give empirical treatment and arrange emergency LP per NICE NG240.'
    });
  }
  ...
  if (data.bleeding.takingAnticoagulant === true) {
    flags.push({
      ...
      priority: 'high',
      description: 'Patient is taking an anticoagulant' +
        (data.bleeding.anticoagulantAgent ? ` (${data.bleeding.anticoagulantAgent})` : '') + '.',
      suggestedAction: 'Hold / reverse anticoagulation and discuss timing with haematology before LP.'
    });
  }
  if (data.bleeding.takingAntiplatelet === true) {
    flags.push({
      ...
      category: 'high-bleeding-risk-anticoag',
      priority: 'medium',
      description: 'Patient is taking an antiplatelet agent' +
        (data.bleeding.antiplateletAgent ? ` (${data.bleeding.antiplateletAgent})` : '') + '.',
      suggestedAction: 'Assess bleeding risk; consider holding the antiplatelet per local policy before LP.'
    });
  }
  if (
    data.bleeding.bleedingDisorder === true ||
    (data.bleeding.inr !== null && data.bleeding.inr !== undefined && data.bleeding.inr !== '' && Number(data.bleeding.inr) > 1.5)
  ) {
    flags.push({
      ...
      suggestedAction: 'Correct coagulopathy (target INR ≤ 1.5) and discuss with haematology before LP.'
    });
  }
  if (
    data.bleeding.plateletCount !== null &&
    data.bleeding.plateletCount !== undefined &&
    data.bleeding.plateletCount !== '' &&
    Number(data.bleeding.plateletCount) < 50
  ) {
  ...
  if (data.bleeding.localSkinInfection === true) {
    flags.push({
      ...
      suggestedAction: 'Do not perform LP through infected skin; choose an alternative site or defer until resolved.'
    });
  }
