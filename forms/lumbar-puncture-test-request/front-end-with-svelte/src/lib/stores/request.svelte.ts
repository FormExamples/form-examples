...
import type { LumbarPunctureRequest, GradingResult } from '$lib/engine/types';
import { createDefaultRequest } from '$lib/engine/defaults';

// Re-export so callers can import the blank-request factory from the store.
export { createDefaultRequest };

/** localStorage draft key for a given request id (defaults to `new`). */
function storageKey(id: string): string {
	return `lumbar-puncture-test-request.front-end-with-svelte.${id || 'new'}.v1`;
}

/**
 * Svelte 5 reactive store for the lumbar puncture request, with localStorage
 ...
 */
class RequestStore {
	data = $state<LumbarPunctureRequest>(createDefaultRequest());
	 ...
	 * (e.g. `const c = request.data.clinician`) stay bound to live state.
	 */
	loadForId(id: string, seed?: LumbarPunctureRequest) {
		const key = id || 'new';
		this.id = key;
		this.result = null;
		this.currentStep = 1;

		let draft: LumbarPunctureRequest | null = null;
		if (browser) {
			const raw = localStorage.getItem(storageKey(key));
			if (raw) {
				try {
					draft = JSON.parse(raw) as LumbarPunctureRequest;
				} catch {
