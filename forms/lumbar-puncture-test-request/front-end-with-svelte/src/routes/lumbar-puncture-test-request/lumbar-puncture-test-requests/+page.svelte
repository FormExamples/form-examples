<script lang="ts">
	...
	const columns = [
		...
		},
		{
			...
			header: 'Intent',
			width: 120,
			template: (v: string) => procedureIntentLabel(v as never)
		},
		{
	...
	function init(api: any) {
		...
		api.on('select-row', (ev: { id?: string | number }) => {
			if (ev?.id != null) goto(`/lumbar-puncture-test-request/lumbar-puncture-test-requests/${ev.id}`);
		});
	}
</script>
...
<main class="mx-auto max-w-6xl px-4 py-6">
	<div class="mb-6 flex flex-wrap items-end justify-between gap-4">
		<div>
			<h1 class="text-2xl font-bold text-base-content">Lumbar puncture vetting dashboard</h1>
			<p class="mt-1 text-sm text-base-content/70">
				Four-axis vetting grades for incoming lumbar puncture requests, computed by the shared
				engine. Select a row to open the request.
			</p>
		</div>
		<a href="/lumbar-puncture-test-request/lumbar-puncture-test-requests/new" class="button" data-variant="primary">New request</a>
	</div>
	...
	<div class="mb-4 flex flex-wrap gap-4">
		...
		<label class="text-sm">
			...
			<select class="select inline-block w-auto" bind:value={recommendationFilter}>
				<option value="">All</option>
				<option value="accept">Accept and schedule</option>
				<option value="query-referrer">Query the referrer</option>
				<option value="redirect">Redirect</option>
				<option value="reject">Reject — contraindicated</option>
			</select>
		</label>
	</div>
