...
// Fluoroscopy Test Request — vetting dashboard (vanilla classic-script app).
//
...
// safety-band dropdown + safety-flags dropdown).
//
// Sibling modules loaded as plain `<script>` tags (in dependency order)
// attach their exports to `window.FluoroscopyTestRequestDashboard`.
...
function matchesFilters(row) {
  if (filters.search) {
    ...
    const matches =
      ...
      String(row.studyType || '').toLowerCase().includes(term) ||
  ...
  if (filters.safety && row.safetyBand !== filters.safety) return false;
...
function compareRows(a, b) {
  ...
  // Default: string compare (nhs, patient, studyType, indication, clinician,
  // referralDate).
...
function renderTableBody() {
  ...
  for (const row of rows) {
    ...
    if (row.triageTier === 'emergency' || row.safetyBand === 'contraindicated') {
    ...
    tr.innerHTML = `
      ...
      <td class="data-table-td">${esc(titleCase(row.studyType))}</td>
      ...
      <td class="data-table-td"><span class="safety-badge ${safetyClass(row.safetyBand)}">${esc(titleCase(row.safetyBand))}</span></td>
