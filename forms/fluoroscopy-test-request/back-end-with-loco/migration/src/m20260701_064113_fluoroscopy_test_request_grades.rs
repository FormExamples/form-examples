...
impl MigrationTrait for Migration {
    async fn up(&self, m: &SchemaManager) -> Result<(), DbErr> {
        create_table(m, "fluoroscopy_test_request_grades",
            &[
            ...
            ("safety_band", ColType::StringWithDefault(String::new())),
            ...
            &[
            ("fluoroscopy_test_request", ""),
            ]
        ).await
    }

    async fn down(&self, m: &SchemaManager) -> Result<(), DbErr> {
        drop_table(m, "fluoroscopy_test_request_grades").await
    }
}
