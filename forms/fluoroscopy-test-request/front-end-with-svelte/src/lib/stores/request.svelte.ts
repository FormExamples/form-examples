...
import type { FluoroscopyRequest, GradingResult } from '#lib/engine/types.js';
...
function storageKey(id: string): string {
	return `fluoroscopy-test-request.front-end-with-svelte.${id || 'new'}.v1`;
}

/**
 * Svelte 5 reactive store for the fluoroscopy / contrast-study request, with
 ...
 */
class RequestStore {
	data = $state<FluoroscopyRequest>(createDefaultRequest());
	 ...
	 */
	loadForId(id: string, seed?: FluoroscopyRequest) {
		const key = id || 'new';
		this.id = key;
		this.result = null;
		this.currentStep = 1;

		let draft: FluoroscopyRequest | null = null;
		if (browser) {
			const raw = localStorage.getItem(storageKey(key));
			if (raw) {
				try {
					draft = JSON.parse(raw) as FluoroscopyRequest;
				} catch {
