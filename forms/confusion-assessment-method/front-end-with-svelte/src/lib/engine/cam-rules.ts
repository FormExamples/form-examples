import type { CamRule } from './types';

/**
 * Declarative CAM feature rules.
 *
 * The Confusion Assessment Method has exactly four observational features. Each
 * rule evaluates the patient data and returns true when its feature is
 * positive; the grader (`cam-grader.ts`) combines them with the fixed boolean
 ...
 * negative. Rows mirror the `confusion_assessment_method_grade_rule` SQL table.
 *
 * A feature left unset ('') is treated as ABSENT for evaluation (spec §3), but
 * the raw tri-state is preserved in storage so an incomplete assessment can
 * still be detected by `flagged-issues.ts`.
 */
export const camRules: CamRule[] = [
	// ─── FEATURE 1: ACUTE ONSET AND FLUCTUATING COURSE ────────────
	{
		...
		description:
			'Feature 1 — acute change in mental status from baseline with a fluctuating course',
	...
	// ─── FEATURE 2: INATTENTION ───────────────────────────────────
	{
		...
		category: 'feature-evaluation',
		description:
			'Feature 2 — difficulty focusing attention, confirmed by a formal attention test',
	...
	// ─── FEATURE 3: DISORGANISED THINKING ─────────────────────────
	{
		...
		category: 'feature-evaluation',
		description:
			'Feature 3 — disorganised or incoherent thinking (rambling, illogical, unpredictable switching)',
