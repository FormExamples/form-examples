# Confusion Assessment Method (CAM): Back-end with Rust Axum Loco (JSON API)

Server-side Rust JSON API for the Confusion Assessment Method (CAM) form. Pure back-end: no HTML
rendering, no Tera templates, no HTMX, no Alpine.js, no CSS, no Lily
Design System.

The Loco crate lives in [`confusion_assessment_method/`](confusion_assessment_method/) — one migration, SeaORM
