# Prescription Request — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Prescription Request form, built with axum + Loco +
...
- [`prescription_request/`](./prescription_request/) — the Loco crate: `src/prescription_request/` holds `app.rs`
...
`src/prescription_request/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd prescription_request && cargo check --all-targets && cargo test
```
