...
impl ActiveModel {
    /// Create a new in-progress prescription request with empty data.
...
/// Find a prescription request by its UUID.
pub async fn find_by_id(db: &DatabaseConnection, id: Uuid) -> Result<Option<Model>, DbErr> {
    Entity::find_by_id(id).one(db).await
}

/// List all completed prescription requests (those with a grading result).
...
/// List prescription requests with pagination (page is 0-indexed).
