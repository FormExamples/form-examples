...
use crate::engine::{prescription_grader, types::AssessmentData};
...
/// POST /assessment/new -- create a new draft prescription request, redirect
/// to the wizard.
#[debug_handler]
async fn create_new(State(ctx): State<AppContext>) -> Result<Response> {
    let item = ActiveModel::new_draft()
        .map_err(|e| Error::BadRequest(format!("Failed to create prescription request: {e}")))?;
...
/// Parse a form value. Empty strings stay as empty strings; otherwise the
/// value is preserved as a JSON string (all prescription fields are
/// strings in the engine type model).
...
async fn show_report(
    ...
    let assessment_data: AssessmentData = item
        .assessment_data()
        .map_err(|e| Error::BadRequest(format!("Invalid request data: {e}")))?;

    let grade = prescription_grader::grade(&assessment_data);
    ...
    context.insert("priority_level", &grade.priority_level);
    context.insert(
        "priority_label",
        &crate::engine::utils::priority_level_label(&grade.priority_level),
    );
    context.insert(
        "priority_class",
        &crate::engine::utils::priority_level_class(&grade.priority_level),
    );
