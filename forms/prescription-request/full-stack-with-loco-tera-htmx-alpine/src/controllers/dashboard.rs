...
/// GET /dashboard -- render the prescription-request dashboard with filters.
#[debug_handler]
async fn dashboard(
    ...
    // Server-side search across patient name, NHS number, and medication.
    if let Some(ref term) = params.search {
        ...
        items.retain(|row| {
            row.patient_first_name.to_lowercase().contains(&term)
                ...
                || row.medication_name.to_lowercase().contains(&term)
        });
    }

    if let Some(ref level) = params.priority_level {
        if !level.is_empty() {
            items.retain(|row| row.priority_level == *level);
        }
    }
    ...
    items.sort_by(|a, b| b.request_date.cmp(&a.request_date));
    ...
    context.insert("priority_level", &params.priority_level.unwrap_or_default());
