# UK NHS England Medical Exemption Certificate (FP92A) — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the UK NHS England Medical Exemption Certificate (FP92A) form, built with axum + Loco +
...
- [`united_kingdom_nhs_england_medical_exemption_certificate/`](./united_kingdom_nhs_england_medical_exemption_certificate/) — the Loco crate: `src/united_kingdom_nhs_england_medical_exemption_certificate/` holds `app.rs`
...
`src/united_kingdom_nhs_england_medical_exemption_certificate/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd united_kingdom_nhs_england_medical_exemption_certificate && cargo check --all-targets && cargo test
```
