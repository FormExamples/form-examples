...
function storageKey(id: string): string {
	return `endoscopy-test-request.front-end-with-svelte.${id || 'new'}.v1`;
}

/**
 * Svelte 5 reactive store for the GI endoscopy request, with localStorage
 ...
 */
class RequestStore {
	data = $state<EndoscopyRequest>(createDefaultRequest());
	 ...
	 */
	loadForId(id: string, seed?: EndoscopyRequest) {
		const key = id || 'new';
		this.id = key;
		this.result = null;
		this.currentStep = 1;

		let draft: EndoscopyRequest | null = null;
		if (browser) {
			const raw = localStorage.getItem(storageKey(key));
			if (raw) {
				try {
					draft = JSON.parse(raw) as EndoscopyRequest;
				} catch {
