<script lang="ts">
	...
	const rows = $derived(
		sampleRequestRows.filter(
			(r) =>
				...
				(riskFilter === '' || r.riskBand === riskFilter)
		)
	);
	...
	function init(api: any) {
		...
		api.on('select-row', (ev: { id?: string | number }) => {
			if (ev?.id != null) goto(`/endoscopy-test-request/endoscopy-test-requests/${ev.id}`);
		});
	}
</script>
...
<main class="mx-16 px-4 py-6">
	<div class="mb-6 flex flex-wrap items-end justify-between gap-4">
		<div>
			<h1 class="text-2xl font-bold text-base-content">Endoscopy vetting dashboard</h1>
			<p class="mt-1 text-sm text-base-content/70">
				Four-axis vetting grades for incoming GI endoscopy requests, computed by the shared engine.
				Select a row to open the request.
			</p>
		</div>
		<a href="/endoscopy-test-request/endoscopy-test-requests/new" class="button" data-variant="primary">New request</a>
	</div>
	...
	<div class="mb-4 flex flex-wrap gap-4">
		...
		<label class="text-sm">
			<span class="mr-2 font-medium text-base-content/80">Pre-procedure risk</span>
