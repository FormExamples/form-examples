<script lang="ts">
	...
	import Step6ComorbiditiesAndFitness from '#lib/components/steps/Step6ComorbiditiesAndFitness.svelte';
	import Step7InfectionAndPreparation from '#lib/components/steps/Step7InfectionAndPreparation.svelte';
	...
	function validate(): boolean {
		...
		if (d.request.requestedProcedure === '') {
			found.push({ id: 'requestedProcedure', message: 'Requested procedure is required.' });
		}
	...
	function submit() {
		...
		goto(`/endoscopy-test-request/endoscopy-test-requests/${id}/report`);
	}
...
<main class="mx-16 px-4 py-6">
	<h1 class="text-2xl font-bold text-base-content">
		{isNew ? 'New endoscopy request' : `Endoscopy request ${id}`}
	</h1>
	...
	<Form label="Endoscopy test request" onsubmit={submit}>
		<Step1RequestingClinician />
		<Step2PatientIdentification />
		<Step3RequestedProcedure />
		<Step4RedFlagsAndTriageLabs />
		<Step5Medication />
		<Step6ComorbiditiesAndFitness />
		<Step7InfectionAndPreparation />
