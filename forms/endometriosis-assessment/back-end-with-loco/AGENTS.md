# Endometriosis Assessment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Endometriosis Assessment form, built with axum + Loco +
...
- [`endometriosis_assessment/`](./endometriosis_assessment/) — the Loco crate: `src/endometriosis_assessment/` holds `app.rs`
...
`src/endometriosis_assessment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd endometriosis_assessment && cargo check --all-targets && cargo test
```
