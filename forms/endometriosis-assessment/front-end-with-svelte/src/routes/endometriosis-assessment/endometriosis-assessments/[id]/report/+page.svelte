<script lang="ts">
	...
	$effect(() => {
		if (!assessment.result) {
			goto(`/endometriosis-assessment/endometriosis-assessments/${id}`);
		}
	});
	...
	async function downloadPDF() {
		pdfError = '';
		try {
			const res = await fetch(`/endometriosis-assessments/${id}/report/pdf`, {
				method: 'POST',
			...
			if (res.ok) {
				...
				a.href = url;
				a.download = `endometriosis-assessment-${data.demographics.lastName || id}.pdf`;
				a.click();
...
{#if result}
	<header class="border-b border-base-300 bg-base-100 shadow-sm no-print">
		<div class="mx-auto flex max-w-4xl items-center justify-between px-4 py-4">
			<h1 class="text-lg font-bold text-base-content">Endometriosis assessment report</h1>
			<div class="flex items-center gap-3">
				...
				<Button data-variant="secondary" onclick={() => goto(`/endometriosis-assessment/endometriosis-assessments/${id}`)}>Edit</Button>
			</div>
		</div>
	</header>
	...
	<main class="mx-16 px-4 py-6">
		...
		<div class="mb-6 rounded-xl border-2 p-6 text-center {severityColor(result.overallSeverity)}">
			<div class="text-3xl font-bold">{severityLabel(result.overallSeverity)} severity</div>
			<div class="mt-2 flex flex-wrap justify-center gap-6 text-sm">
				<span>{asrmStageLabel(result.asrmStage)}</span>
				<span>ASRM points: {result.asrmPoints}</span>
				{#if result.ehp30Score !== null}<span>EHP-30: {result.ehp30Score} ({ehp30Label(result.ehp30Score)})</span>{/if}
			</div>
