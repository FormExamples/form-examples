<script lang="ts">
	...
	import Step6FertilityAssessment from '#lib/components/steps/Step6FertilityAssessment.svelte';
	import Step7PreviousTreatments from '#lib/components/steps/Step7PreviousTreatments.svelte';
	import Step8SurgicalHistory from '#lib/components/steps/Step8SurgicalHistory.svelte';
	import Step9QualityOfLife from '#lib/components/steps/Step9QualityOfLife.svelte';
	import Step10TreatmentPlanning from '#lib/components/steps/Step10TreatmentPlanning.svelte';
	...
	function submit() {
		...
		assessment.result = calculateEndoGrade(assessment.data);
		goto(`/endometriosis-assessment/endometriosis-assessments/${id}/report`);
	}
...
<main class="mx-16 px-4 py-6">
	<h1 class="text-2xl font-bold text-base-content">
		{isNew ? 'New endometriosis assessment' : `Endometriosis assessment ${id}`}
	</h1>
	<p class="mt-1 text-sm text-base-content/70">
		Complete the ten sections; the ASRM stage, EHP-30 score, and overall severity are computed on
		submit.
	</p>
	...
	<Form label="Endometriosis assessment" onsubmit={submit}>
		<Step1Demographics />
		<Step2MenstrualHistory />
		<Step3PainAssessment />
		<Step4GastrointestinalSymptoms />
		<Step5UrinarySymptoms />
		<Step6FertilityAssessment />
		<Step7PreviousTreatments />
		<Step8SurgicalHistory />
		<Step9QualityOfLife />
		<Step10TreatmentPlanning />
