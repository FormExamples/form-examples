...
import { asrmStageLabel, ehp30Label, severityLabel, bmiCategory, calculateAge } from '#lib/engine/utils.js';
...
export function buildPdfDocument(
	...
	return {
		...
		header: {
			text: 'ENDOMETRIOSIS ASSESSMENT REPORT',
			alignment: 'center',
			margin: [0, 20, 0, 0],
			fontSize: 10,
			color: '#6b7280',
		...
		content: [
			// Title & Severity
			{
				text: `Overall Severity: ${severityLabel(result.overallSeverity)}`,
				fontSize: 24,
				bold: true,
				alignment: 'center',
				margin: [0, 0, 0, 4]
			...
			{
				text: asrmStageLabel(result.asrmStage),
				fontSize: 12,
			...
			{
				text:
					result.ehp30Score !== null
						? `EHP-30 score: ${result.ehp30Score} (${ehp30Label(result.ehp30Score)})  ·  ASRM points: ${result.asrmPoints}`
						: `ASRM points: ${result.asrmPoints}`,
				fontSize: 12,
				alignment: 'center' as const,
				color: '#4b5563',
