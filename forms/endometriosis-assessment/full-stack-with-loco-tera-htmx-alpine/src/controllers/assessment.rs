...
use crate::engine::{
    ...
    utils::{asrm_stage_label, ehp30_label, severity_label},
};
...
/// Parse a form value string. Empty strings stay as `""`; numeric
/// strings are converted to JSON numbers so the engine deserializes
/// them as `Option<i32>` / `Option<f64>`.
...
async fn show_report(
    ...
    let grade = endo_grader::grade(&assessment_data);
    ...
    context.insert("asrm_stage", &grade.asrm_stage);
    context.insert("asrm_stage_label", &asrm_stage_label(grade.asrm_stage));
    context.insert("asrm_points", &grade.asrm_points);
    context.insert("ehp30_score", &grade.ehp30_score);
    context.insert("ehp30_label", &ehp30_label(grade.ehp30_score));
    context.insert("overall_severity", &grade.overall_severity);
    context.insert("severity_label", &severity_label(&grade.overall_severity));
