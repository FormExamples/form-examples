...
// Mirrored from front-end-form-with-svelte/src/lib/types.ts so the two
...
export interface CertificateProvider {
  ...
  eligibilityConfirmationAt: string;
}
...
// Flattened row used by the SVAR Grid on the dashboard. One row per LPA.
...
export function lpaToRow(lpa: Lpa): LpaRow {
  const donor = lpa.donor;
  const donorName = [donor.title, donor.firstNames, donor.lastName]
  ...
  return {
    ...
    attorneyCount: lpa.attorneys.length,
    decisionMode: lpa.decisionMode,
    whenAttorneysCanAct: lpa.whenAttorneysCanAct,
    replacementAttorneyCount: lpa.replacementAttorneys.length,
    peopleToNotifyCount: lpa.peopleToNotify.length,
    validityBand: lpa.validation.validityBand,
    compositeRisk: lpa.validation.compositeRisk,
    opgStatus: lpa.status,
    opgReferenceNumber: lpa.opgReferenceNumber,
