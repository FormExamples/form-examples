...
<section>
  ...
  {#if store.data.registrationApplication.applicantKind === 'attorneys'}
    <div class="mt-4 space-y-3">
      <h3 class="text-sm font-semibold text-slate-700">Which attorneys are applying?</h3>
      ...
      {#each store.data.attorneys as a (a.person.id)}
        <div class="border border-slate-200 rounded p-3 bg-slate-50/50 space-y-2">
          ...
          {#if selectedAttorneyIds.includes(a.person.id)}
            <div class="grid grid-cols-1 md:grid-cols-2 gap-3 ml-6 text-sm text-slate-700">
              <p>
