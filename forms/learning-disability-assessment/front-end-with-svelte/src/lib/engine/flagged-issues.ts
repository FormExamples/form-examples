...
/**
 * Flagged-issue detection. Independent of the adaptive-functioning score (which
 * the grader computes), this raises clinician-facing flags for epilepsy /
 * poorly-controlled seizures, psychotropic prescribing, dysphagia, missed
 * national screening, mental-capacity concerns, significant behavioural risk,
 * and required reasonable adjustments.
 */
export function detectAdditionalFlags(data: AssessmentData): AdditionalFlag[] {
	...
	// ─── BMI extremes ─────────────────────────────────────────
	if (data.physicalExamination.bmi !== null && data.physicalExamination.bmi < 18.5) {
		flags.push({
			id: 'FLAG-BMI-001',
	...
	} else if (data.physicalExamination.bmi !== null && data.physicalExamination.bmi >= 30) {
		flags.push({
			id: 'FLAG-BMI-002',
			category: 'Physical Examination',
