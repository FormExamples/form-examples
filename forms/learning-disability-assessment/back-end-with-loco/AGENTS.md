# Learning Disability Assessment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Learning Disability Assessment form, built with axum + Loco +
...
- [`learning_disability_assessment/`](./learning_disability_assessment/) — the Loco crate: `src/learning_disability_assessment/` holds `app.rs`
...
`src/learning_disability_assessment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd learning_disability_assessment && cargo check --all-targets && cargo test
```
