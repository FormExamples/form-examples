# Consent to Treatment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Consent to Treatment form, built with axum + Loco +
...
- [`consent_to_treatment/`](./consent_to_treatment/) — the Loco crate: `src/consent_to_treatment/` holds `app.rs`
...
`src/consent_to_treatment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd consent_to_treatment && cargo check --all-targets && cargo test
```
