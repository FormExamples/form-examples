# Prenatal Assessment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Prenatal Assessment form, built with axum + Loco +
...
- [`prenatal_assessment/`](./prenatal_assessment/) — the Loco crate: `src/prenatal_assessment/` holds `app.rs`
...
`src/prenatal_assessment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd prenatal_assessment && cargo check --all-targets && cargo test
```
