<script lang="ts">
	...
	const columns = [
		...
		{ id: 'riskScore', header: 'Risk score', width: 110, sort: true },
		{
			id: 'riskLevel',
			header: 'Risk level',
			width: 140,
		...
		{ id: 'bpFlag', header: 'High BP', width: 90, template: (v: boolean) => (v ? 'Yes' : 'No') },
		{
			id: 'multipleFlag',
			header: 'Multiple',
			width: 100,
	...
	function init(api: any) {
		...
		api.on('select-row', (ev: { id?: string | number }) => {
			if (ev?.id != null) goto(`/prenatal-assessments/${ev.id}`);
		});
	}
</script>
...
<main class="mx-auto max-w-6xl px-4 py-6">
	<div class="mb-6 flex flex-wrap items-end justify-between gap-4">
		...
		<a href="/prenatal-assessments/new" class="button" data-variant="primary">New assessment</a>
	</div>
