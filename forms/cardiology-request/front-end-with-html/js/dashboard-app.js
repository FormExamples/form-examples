...
// Cardiology Request — vetting dashboard (vanilla classic-script app).
//
...
// safety-band dropdown + recommendation dropdown + safety-flags dropdown).
//
// Sibling modules loaded as plain `<script>` tags (in dependency order)
// attach their exports to `window.CardiologyRequestDashboard`. The whole file
...
function matchesFilters(row) {
  if (filters.search) {
    ...
    const matches =
      ...
      String(row.service || '').toLowerCase().includes(term) ||
...
function compareRows(a, b) {
  ...
  // Default: string compare (nhs, patient, service, reason, clinician,
  // referralDate).
...
function renderTableBody() {
  ...
  for (const row of rows) {
    ...
    if (row.triageTier === 'emergency' || row.safetyBand === 'red-flag') {
    ...
    tr.innerHTML = `
      ...
      <td class="data-table-td">${esc(titleCase(row.service))}</td>
      <td class="data-table-td">${esc(titleCase(row.reason))}</td>
