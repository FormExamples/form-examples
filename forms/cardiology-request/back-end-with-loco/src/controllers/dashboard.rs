...
//! still listed, with a null grade) for the cardiology vetting worklist.
...
use crate::models::_entities::{cardiology_request_grades, cardiology_requests};
...
async fn dashboard(
    ...
    let mut query = cardiology_requests::Entity::find()
        .order_by_desc(cardiology_requests::Column::CreatedAt)
        .limit(limit);
    if let Some(status) = params.status {
        query = query.filter(cardiology_requests::Column::Status.eq(status));
    }
    ...
    for request in &requests {
        let grade = cardiology_request_grades::Entity::find()
            .filter(cardiology_request_grades::Column::CardiologyRequestId.eq(request.id))
            .one(&ctx.db)
            .await?;
        items.push(json!({
            "request": request,
