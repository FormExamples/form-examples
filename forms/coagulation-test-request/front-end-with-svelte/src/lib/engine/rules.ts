...
//
// (A) appropriateness 1-9 + band by indication x selected-tests match (BSH
// indication / retest-interval anchor); (B) pre-analytical / specimen safety
// ok / caution / reject-risk (sodium-citrate tube fill, 9:1 ratio, analysis
// timing); (C) request completeness over mandatory fields; (D) triage tier
// routine / urgent / stat with active-bleeding and suspected-DIC auto-escalation
// to stat. Rule IDs are stable and identical across every front-end and the
// back-end (R-APPROP-*, R-PREANALYTICAL-*, R-COMPLETE-*, R-TRIAGE-*).
...
import { TESTS, countSelectedTests } from './defaults';

// ----------------------------------------------------------------------
// Axis A — Appropriateness (BSH indication / retest-interval anchor, 1-9)
// ----------------------------------------------------------------------

/** Map of indication -> ideal / plausible test fields. */
...
function selectedFields(tests: TestsSection): string[] {
	const out: string[] = [];
...
export function evaluatePreanalytical(
	specimen: SpecimenSection
): { band: PreanalyticalBand; firedRule: FiredRule } {
	const collected = specimen.specimenCollected;
	const fill = specimen.citrateTubeFill;
	const ratio = specimen.citrateRatioCorrect;
...
const COMPLETENESS_FIELDS: {
	weight: number;
	present: (d: CoagulationTestRequest) => boolean;
	ruleId: string;
	label: string;
}[] = [
...
export function scoreCompleteness(data: CoagulationTestRequest): {
	percent: number;
	missing: FiredRule[];
} {
	let totalWeight = 0;
...
export const TRIAGE_ORDER: TriageTier[] = ['routine', 'urgent', 'stat'];
...
const TRIAGE_RULES: {
	ruleId: string;
	tier: TriageTier;
	fires: (d: CoagulationTestRequest) => boolean;
	description: string;
}[] = [
	{
...
export function scoreTriage(data: CoagulationTestRequest): {
	tier: TriageTier;
