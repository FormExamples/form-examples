...
// active-bleeding-stat, suspected-dic, d-dimer-low-pretest-caution,
// specimen-underfilled-risk, missing-clinical-details, no-test-selected, other.
// Each flag is { flagId, category, priority, description, suggestedAction }.
// Flag IDs are stable and identical across every front-end and the back-end.

import type { CoagulationTestRequest, Flag, PreanalyticalBand } from './types';
import { countSelectedTests } from './defaults';

const PRIORITY_ORDER = { high: 0, medium: 1, low: 2 } as const;

/**
 * Detect safety flags for a coagulation test request, sorted high → medium →
 * low. The optional context lets the grader share the already-computed
 * pre-analytical band.
 */
export function detectFlags(
	data: CoagulationTestRequest,
	context?: { preanalyticalBand?: PreanalyticalBand }
): Flag[] {
	const flags: Flag[] = [];
	const ctx = context || {};
	...
	if (data.clinical.activeBleeding === true) {
		flags.push({
			...
			suggestedAction:
				'Discuss with the on-call haematologist; process as STAT and consider fibrinogen / major-haemorrhage pathway.'
		});
	}
	...
	if (
		...
		flags.push({
			...
			suggestedAction:
				'Process PT, APTT, fibrinogen, and D-dimer as STAT; alert the haematology team and treat the underlying cause.'
		});
	}
	...
	if (
		...
		flags.push({
			...
			priority: 'medium',
			description:
				'D-dimer ordered for suspected DVT / PE without an "unlikely" 2-level Wells pre-test probability.',
			suggestedAction:
				'Confirm a 2-level Wells "unlikely" score before D-dimer (NICE NG158); a likely score warrants imaging instead.'
		});
	}
	...
	if (
		...
		flags.push({
			...
			suggestedAction:
				'Reject and recollect; coagulation results from a mis-filled citrate tube are unreliable.'
		});
	}
