...
function storageKey(id: string): string {
	return `coagulation-test-request.front-end-with-svelte.${id || 'new'}.v1`;
}

/**
 * Svelte 5 reactive store for the coagulation test request, with localStorage
 ...
 */
class RequestStore {
	data = $state<CoagulationTestRequest>(createDefaultRequest());
	 ...
	 * `const c = request.data.clinical`) stay bound to live state.
	 */
	loadForId(id: string, seed?: CoagulationTestRequest) {
		const key = id || 'new';
		this.id = key;
		this.result = null;
		this.currentStep = 1;

		let draft: CoagulationTestRequest | null = null;
		if (browser) {
			const raw = localStorage.getItem(storageKey(key));
			if (raw) {
				try {
					draft = JSON.parse(raw) as CoagulationTestRequest;
				} catch {
