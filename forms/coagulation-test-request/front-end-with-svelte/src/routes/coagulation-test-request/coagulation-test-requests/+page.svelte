<script lang="ts">
	...
	import { indicationLabel } from '#lib/engine/defaults.js';
	import {
	...
	const columns = [
		...
		{ id: 'testCount', header: 'Tests', width: 80, sort: true },
		{
			id: 'appropriatenessScore',
			header: 'Appropriateness',
			width: 160,
			sort: true,
			template: (v: number, row: any) => `${v}/9 · ${appropriatenessLabel(row.appropriatenessBand)}`
		},
		{
	...
	function init(api: any) {
		...
		api.on('select-row', (ev: { id?: string | number }) => {
			if (ev?.id != null) goto(`/coagulation-test-request/coagulation-test-requests/${ev.id}`);
		});
	}
</script>
...
<main class="mx-16 px-4 py-6">
	<div class="mb-6 flex flex-wrap items-end justify-between gap-4">
		<div>
			<h1 class="text-2xl font-bold text-base-content">Coagulation vetting dashboard</h1>
			<p class="mt-1 text-sm text-base-content/70">
				Four-axis vetting grades for incoming coagulation requests, computed by the shared engine.
				Select a row to open the request.
			</p>
		</div>
		<a href="/coagulation-test-request/coagulation-test-requests/new" class="button" data-variant="primary">New request</a>
	</div>
