<script lang="ts">
	...
	import Step5Specimen from '#lib/components/steps/Step5Specimen.svelte';
	...
	function validate(): boolean {
		...
		if (countSelectedTests(d.tests) === 0) {
			found.push({ id: 'tests', message: 'Select at least one coagulation test.' });
		}
	...
	function submit() {
		...
		goto(`/coagulation-test-request/coagulation-test-requests/${id}/report`);
	}
...
<main class="mx-16 px-4 py-6">
	<h1 class="text-2xl font-bold text-base-content">
		{isNew ? 'New coagulation test request' : `Coagulation test request ${id}`}
	</h1>
	...
	<Form label="Coagulation test request" onsubmit={submit}>
