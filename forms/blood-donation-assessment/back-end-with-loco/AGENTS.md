# Blood Donation Assessment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Blood Donation Assessment form, built with axum + Loco +
...
- [`blood_donation_assessment/`](./blood_donation_assessment/) — the Loco crate: `src/blood_donation_assessment/` holds `app.rs`
...
`src/blood_donation_assessment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd blood_donation_assessment && cargo check --all-targets && cargo test
```
