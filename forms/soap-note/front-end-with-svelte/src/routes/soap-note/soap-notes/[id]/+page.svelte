<script lang="ts">
	...
	import Step3Subjective from '#lib/components/steps/Step3Subjective.svelte';
	import Step4Objective from '#lib/components/steps/Step4Objective.svelte';
	import Step5Assessment from '#lib/components/steps/Step5Assessment.svelte';
	import Step6Plan from '#lib/components/steps/Step6Plan.svelte';
	...
	function validate(): boolean {
		...
		if (d.context.clinicianName.trim() === '') {
			found.push({ id: 'context-clinicianName', message: 'Authoring clinician name is required.' });
		}
	...
	function submit() {
		...
		assessment.result = calculateSoapGrade(assessment.data);
		goto(`/soap-note/soap-notes/${id}/report`);
	}
...
<main class="mx-16 px-4 py-6">
	<h1 class="text-2xl font-bold text-base-content">
		{isNew ? 'New SOAP note' : `SOAP note ${id}`}
	</h1>
	<p class="mt-1 text-sm text-base-content/70">
		Complete the seven sections; completeness is computed on submit. This is a documentation
	...
	<Form label="SOAP note" onsubmit={submit}>
		<Step1Context />
		<Step2Identification />
		<Step3Subjective />
		<Step4Objective />
