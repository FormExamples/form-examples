...
function storageKey(id: string): string {
	return `soap-note.front-end-with-svelte.${id || 'new'}.v1`;
}

/** A blank SOAP note with all fields at their unanswered defaults. */
...
/**
 * Svelte 5 reactive store for the SOAP note, with localStorage persistence so an
 * in-progress note survives a page reload. Drafts are keyed by note id so each
...
class AssessmentStore {
	 ...
	 * (e.g. `const s = assessment.data.subjective`) stay bound to live state.
	 */
