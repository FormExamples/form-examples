# Ophthalmology Waiting List Card — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Ophthalmology Waiting List Card form, built with axum + Loco +
...
- [`ophthalmology_waiting_list_card/`](./ophthalmology_waiting_list_card/) — the Loco crate: `src/ophthalmology_waiting_list_card/` holds `app.rs`
...
`/metrics`. The registered domain controllers are:
  ...
  - `ophthalmology_waiting_list_card`
  - `ophthalmology_waiting_list_card_appointment`
  - `ophthalmology_waiting_list_card_grade`
  - `ophthalmology_waiting_list_card_grade_flag`
  - `ophthalmology_waiting_list_card_grade_rule`
  - `patient`
  - `practitioner`

## Engine

`src/ophthalmology_waiting_list_card/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd ophthalmology_waiting_list_card && cargo check --all-targets && cargo test
```
