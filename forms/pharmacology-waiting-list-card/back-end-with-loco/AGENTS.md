# Pharmacology Waiting List Card — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Pharmacology Waiting List Card form, built with axum + Loco +
...
- [`pharmacology_waiting_list_card/`](./pharmacology_waiting_list_card/) — the Loco crate: `src/pharmacology_waiting_list_card/` holds `app.rs`
...
`/metrics`. The registered domain controllers are:
  ...
  - `pharmacology_waiting_list_card`
  - `pharmacology_waiting_list_card_appointment`
  - `pharmacology_waiting_list_card_grade`
  - `pharmacology_waiting_list_card_grade_flag`
  - `pharmacology_waiting_list_card_grade_rule`
  - `patient`
  - `practitioner`

## Engine

`src/pharmacology_waiting_list_card/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd pharmacology_waiting_list_card && cargo check --all-targets && cargo test
```
