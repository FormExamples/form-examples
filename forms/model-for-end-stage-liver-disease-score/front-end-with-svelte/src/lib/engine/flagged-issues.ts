...
import { isNum, missingInputs } from './meld-grader';

/**
 * Detect clinician-facing safety flags (red flags). Independent of the mortality
 * band (which the grader produces), this uses the computed score and the raw
 * laboratory inputs (spec §5):
 *
 ...
 *
 * Rows mirror the `model_for_end_stage_liver_disease_score_grade_flag` SQL
 * table (flag_id, category, priority, description, suggested_action).
 */
export function detectFlaggedIssues(data: AssessmentData, result: GradingResult): FlaggedIssue[] {
	...
	if (meldScore === null) {
		...
		flags.push({
			...
			priority: 'low',
			description: `Missing input(s): ${missing.join(', ')} — no MELD score can be computed`,
			suggestedAction: 'Record the missing laboratory value(s) and re-calculate.'
		});
		return flags;
	}
	...
	if (meldScore >= 15) {
		flags.push({
			...
			priority: 'high',
			description: `MELD ${meldScore} at or above the conventional transplant-benefit threshold of 15`,
			suggestedAction: 'Refer to, or discuss with, a liver transplant centre.'
		});
	}
	...
	if (meldScore >= 30) {
		flags.push({
			...
			priority: 'high',
			description: `MELD ${meldScore} indicates very high short-term mortality`,
			suggestedAction: 'Arrange urgent hepatology / critical-care review.'
		});
	}
	...
	if (result.dialysisRuleApplied) {
		flags.push({
			...
			suggestedAction: 'Review renal-replacement status and involve nephrology as indicated.'
		});
	}
	...
	if (isNum(sodium) && Number(sodium) < 130) {
		flags.push({
			...
			priority: 'medium',
			description: `Serum sodium ${sodium} mEq/L below 130 — raises mortality risk and increases MELD-Na`,
			suggestedAction:
	...
	if (isNum(inr) && Number(inr) >= 2.5) {
		flags.push({
			...
			priority: 'medium',
			description: `INR ${inr} at or above 2.5 — marked derangement of clotting`,
			suggestedAction: 'Assess bleeding risk; correct only for active bleeding or procedures.'
		});
	}
