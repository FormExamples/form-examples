...
Server-side Rust JSON API for the Model for End-Stage Liver Disease (MELD) Score form. Pure back-end: no HTML
rendering, no Tera templates, no HTMX, no Alpine.js, no CSS, no Lily
Design System.

The Loco crate lives in [`model_for_end_stage_liver_disease_score/`](model_for_end_stage_liver_disease_score/) — one migration, SeaORM
