...
function storageKey(id: string): string {
	return `echocardiogram-test-request.front-end-with-svelte.${id || 'new'}.v1`;
}

/**
 * Svelte 5 reactive store for the echocardiogram test request, with
 ...
 */
class RequestStore {
	data = $state<EchoRequest>(createDefaultRequest());
	 ...
	 * (e.g. `const d = request.data.patient`) stay bound to live state.
	 */
	loadForId(id: string, seed?: EchoRequest) {
		const key = id || 'new';
		this.id = key;
		this.result = null;
		this.currentStep = 1;

		let draft: EchoRequest | null = null;
		if (browser) {
			const raw = localStorage.getItem(storageKey(key));
			if (raw) {
				try {
					draft = JSON.parse(raw) as EchoRequest;
				} catch {
