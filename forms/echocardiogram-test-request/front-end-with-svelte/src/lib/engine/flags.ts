...
//
// Ported verbatim (logic + flag IDs) from the HTML front-end's flags.js. Pure
// function returning safety flags using the grade_flag categories:
// suspected-endocarditis, severe-symptomatic-valve, acute-heart-failure,
// raised-bnp, rarely-appropriate-indication, missing-indication,
// missing-clinical-question, duplicate-recent-echo, other.
//
// Each flag is { flagId, category, priority, description, suggestedAction }.
// Flag IDs are stable and identical across every front-end and the back-end.

import type { EchoRequest, Flag, AppropriatenessBand } from './types';
...
export function detectFlags(
	data: EchoRequest,
	context?: { appropriatenessBand?: AppropriatenessBand }
): Flag[] {
	const flags: Flag[] = [];
	const ctx = context || {};
	...
	if (data.redFlags.suspectedEndocarditis === true) {
		flags.push({
			...
			suggestedAction:
				'Arrange urgent TTE then TOE; discuss with cardiology and follow ESC 2023 endocarditis pathway.'
		});
	}
	if (data.redFlags.severeSymptomaticValve === true) {
		flags.push({
			...
			suggestedAction:
				'Prioritise urgent echo; refer to valve / heart-failure service for consideration of intervention.'
		});
	}
	...
	if (ctx.appropriatenessBand === 'rarely-appropriate') {
		flags.push({
			...
			suggestedAction:
				'Query the referrer; confirm the requested study is the right examination for the indication.'
		});
	}
	...
	if (duplicateRecentEcho(data)) {
		flags.push({
			...
			suggestedAction:
				'Confirm a repeat study is clinically indicated; review the prior report before booking.'
		});
	}
 ...
 */
export function duplicateRecentEcho(d: EchoRequest): boolean {
	const hasPrior = !!d.request.previousEcho && d.request.previousEcho !== 'none';
