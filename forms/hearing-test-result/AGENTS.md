# Hearing Test Result — agent instructions

Hearing test result (report). A reporting clinician completes a single-page
wizard recording the performed test's **findings** and a structured
...
This is the **result/report counterpart** to `hearing-test-request` (a
...
referral. The source-of-truth record is `hearing_test_result`; the clinician is
the report **author/signer** (audiologist, ENT surgeon, hearing therapist,
...
- **Axis B — severity & structured reporting:** abnormalitySeverity
  ...
  (e.g. a BSA audiometric descriptor or configuration).
- **Axis C — report completeness:** 0–100 % of mandatory report sections
  (history, test reliability, measurements, findings, impression).
- **Axis D — follow-up urgency:** routine / recommended / urgent / critical-alert,
  with a target timeframe and a recommended action.

A **critical finding** — **sudden sensorineural hearing loss** or a **marked
asymmetry** (retrocochlear red flag) — **auto-escalates** Axis D to
