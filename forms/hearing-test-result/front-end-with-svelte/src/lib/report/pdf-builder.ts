...
import type { HearingResult, GradingResult } from '$lib/engine/types';
import {
...
/** Builds the pdfmake document definition for the hearing test audiology report. */
export function buildPdfDocument(
	...
	return {
		...
		header: {
			text: 'HEARING TEST AUDIOLOGY REPORT',
			alignment: 'center',
			margin: [0, 20, 0, 0],
			fontSize: 10,
			color: '#6b7280',
		...
		footer: (currentPage: number, pageCount: number) => ({
			text: `Page ${currentPage} of ${pageCount} | Generated ${new Date(result.gradedAt).toLocaleString()} | BSA / NICE NG98`,
			alignment: 'center',
			margin: [0, 20, 0, 0],
			fontSize: 8,
			color: '#9ca3af'
		...
		content: [
			...
			sectionHeader('Examination'),
			{
				ul: [
					...
					`Test reliability: ${data.testReliability || 'N/A'}`
				],
				margin: [0, 0, 0, 16] as Margin
			},
			...
			sectionHeader('Audiometry & interpretation'),
			{
				table: {
					widths: ['*', '*', '*'],
					body: [
						[
							{ text: 'Ear', bold: true, color: '#6b7280' },
							{ text: 'Right', bold: true, color: '#6b7280' },
							{ text: 'Left', bold: true, color: '#6b7280' }
						],
						[
							'Pure-tone average (dB HL)',
							data.pureToneAverageRightDb !== null ? `${data.pureToneAverageRightDb}` : 'N/A',
							data.pureToneAverageLeftDb !== null ? `${data.pureToneAverageLeftDb}` : 'N/A'
						],
						[
							'Hearing-loss type',
							hearingLossTypeLabel(data.hearingLossTypeRight),
							hearingLossTypeLabel(data.hearingLossTypeLeft)
						],
						[
							'Hearing-loss severity',
							hearingLossSeverityLabel(data.hearingLossSeverityRight),
							hearingLossSeverityLabel(data.hearingLossSeverityLeft)
						],
						[
							'Tympanogram (Jerger)',
							data.tympanometryTypeRight || 'N/A',
							data.tympanometryTypeLeft || 'N/A'
						]
					]
				},
				layout: 'lightHorizontalLines',
...
function structuredFindingList(data: HearingResult): string[] {
	const list: string[] = [];
