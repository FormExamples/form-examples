# Respirology Assessment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Respirology Assessment form, built with axum + Loco +
...
- [`respirology_assessment/`](./respirology_assessment/) — the Loco crate: `src/respirology_assessment/` holds `app.rs`
...
`src/respirology_assessment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd respirology_assessment && cargo check --all-targets && cargo test
```
