<script lang="ts">
	...
	function submit() {
		...
		assessment.result = calculateRespirologyGrade(assessment.data);
		goto(`/respirology-assessment/respirology-assessments/${id}/report`);
	}
...
<main class="mx-auto max-w-3xl px-4 py-6">
	<header class="mb-6 no-print">
		<h1 class="text-2xl font-bold text-base-content">
			{isNew ? 'New respirology assessment' : `Respirology assessment ${id}`}
		</h1>
		<p class="mt-1 text-sm text-base-content/70">
			Complete the ten sections; the MRC dyspnoea grade and flagged issues are computed on submit.
		</p>
	...
	<Form label="Respirology assessment" onsubmit={submit}>
		<Step1Demographics />
		<Step2ChiefComplaint />
