// Additional flag detection for the Respirology Assessment.
//
// Detects safety-critical alerts that should be highlighted to the
...
function detectAdditionalFlags(data) {
  ...
  if (data.coughAssessment.haemoptysis === 'yes') {
    flags.push({
      ...
      message:
        ...
        (data.coughAssessment.haemoptysisDetails
          ? ': ' + data.coughAssessment.haemoptysisDetails
          : '') +
  ...
  if (
    ...
    flags.push({
      ...
      message:
        'SpO2 ' +
        data.pulmonaryFunction.oxygenSaturation +
  ...
  if (data.pulmonaryFunction.fev1 !== null && data.pulmonaryFunction.fev1 < 30) {
    flags.push({
      ...
      category: 'Respiratory Failure',
      message:
        'FEV1 ' + data.pulmonaryFunction.fev1 + '% predicted - very severe obstruction',
      priority: 'high'
    });
  }
  ...
  if (data.respiratoryHistory.pulmonaryEmbolism === 'yes') {
    flags.push({
      ...
      message:
        'Previous PE' +
        (data.respiratoryHistory.peDate ? ' (' + data.respiratoryHistory.peDate + ')' : '') +
        ' - assess anticoagulation and recurrence risk',
      priority: 'high'
    });
  }
  ...
  if (
    ...
    flags.push({
      ...
      message:
        'STOP-BANG score ' +
        data.sleepFunctional.stopBangScore +
        '/7 - high risk of OSA',
      priority: 'medium'
    });
  }
  ...
  if (
    ...
    flags.push({
      ...
      message:
        'STOP-BANG score ' +
        data.sleepFunctional.stopBangScore +
        '/7 - intermediate risk of OSA',
      priority: 'medium'
    });
  }

  // ─── Tuberculosis (MEDIUM/HIGH) ─────────────────────────
  ...
  if (data.respiratoryHistory.interstitialLungDisease === 'yes') {
    flags.push({
      ...
      message:
        'ILD' +
        (data.respiratoryHistory.ildType ? ' (' + data.respiratoryHistory.ildType + ')' : '') +
        ' - monitor progression',
      priority: 'medium'
    });
  }
  ...
  if (data.smokingExposures.asbestosExposure === 'yes') {
    flags.push({
      ...
      message:
        'Asbestos exposure' +
        (data.smokingExposures.asbestosDetails
          ? ': ' + data.smokingExposures.asbestosDetails
          : '') +
  ...
  if (data.currentMedications.oralSteroids === 'yes') {
    flags.push({
      ...
      message:
        ...
        (data.currentMedications.oralSteroidDetails
          ? ': ' + data.currentMedications.oralSteroidDetails
          : '') +
  ...
  // ─── Drug allergies ─────────────────────────────────────
  data.allergies.drugAllergies.forEach((allergy, i) => {
  ...
  if (data.sleepFunctional.daytimeSomnolence === 'yes') {
    flags.push({
      ...
      category: 'Sleep',
      message:
        'Daytime somnolence reported' +
        (data.sleepFunctional.epworthScore !== null
          ? ' (Epworth score: ' + data.sleepFunctional.epworthScore + ')'
          : ''),
      priority: 'low'
    });
  }
  ...
  if (data.smokingExposures.occupationalExposure === 'yes') {
    flags.push({
      ...
      message:
        ...
        (data.smokingExposures.occupationalDetails
          ? ': ' + data.smokingExposures.occupationalDetails
          : ''),
      priority: 'low'
    });
