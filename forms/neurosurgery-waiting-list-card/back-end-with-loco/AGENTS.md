# Neurosurgery Waiting List Card — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Neurosurgery Waiting List Card form, built with axum + Loco +
...
- [`neurosurgery_waiting_list_card/`](./neurosurgery_waiting_list_card/) — the Loco crate: `src/neurosurgery_waiting_list_card/` holds `app.rs`
...
`/metrics`. The registered domain controllers are:
  ...
  - `neurosurgery_waiting_list_card`
  - `neurosurgery_waiting_list_card_appointment`
  - `neurosurgery_waiting_list_card_grade`
  - `neurosurgery_waiting_list_card_grade_flag`
  - `neurosurgery_waiting_list_card_grade_rule`
  - `patient`
  - `practitioner`

## Engine

`src/neurosurgery_waiting_list_card/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd neurosurgery_waiting_list_card && cargo check --all-targets && cargo test
```
