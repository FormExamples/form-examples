# Post-Traumatic Stress Assessment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Post-Traumatic Stress Assessment form, built with axum + Loco +
...
- [`post_traumatic_stress_assessment/`](./post_traumatic_stress_assessment/) — the Loco crate: `src/post_traumatic_stress_assessment/` holds `app.rs`
...
`src/post_traumatic_stress_assessment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd post_traumatic_stress_assessment && cargo check --all-targets && cargo test
```
