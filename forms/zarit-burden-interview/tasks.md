# Zarit Burden Interview (ZBI) — tasks
...
- [x] Author `spec/index.md` (data model, grading algorithm incl. short form,
      flagged issues, I/O shapes, acceptance criteria).
- [x] Author `AGENTS.md` (directory map, scoring engine shape and files,
      conventions, compliance).
- [x] Author `plan.md` and `tasks.md`.
- [x] SQL migrations in `sql/` (assessment table, 22 item ratings,
      instrument-form selector, UUIDv4 PK, timestamps).
- [x] Generated XML, FHIR R5, protobuf, OpenAPI, Loco setup.
- [x] Scoring engine (`types.ts`, `utils.ts`, `zarit-rules.ts`,
      `zarit-grader.ts`, `flagged-issues.ts`) + Vitest tests.
...
- [ ] `bin/test-form zarit-burden-interview` passes.
