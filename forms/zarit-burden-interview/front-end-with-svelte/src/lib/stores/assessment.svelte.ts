...
import type { AssessmentData, GradingResult, Items } from '#lib/engine/types.js';

/** localStorage draft key for a given assessment id (defaults to `new`). */
function storageKey(id: string): string {
	return `zarit-burden-interview.front-end-with-svelte.${id || 'new'}.v1`;
}

/** A blank ZBI assessment with all fields at their unanswered defaults. */
export function createDefaultAssessment(): AssessmentData {
	const items = {} as Items;
	for (let i = 1; i <= 22; i++) {
		items[`item${i}` as keyof Items] = null;
	}
	return {
		context: {
			practitionerName: '',
...
/**
 * Svelte 5 reactive store for the ZBI assessment, with localStorage persistence
...
class AssessmentStore {
	 ...
	 * (e.g. `const d = assessment.data.items`) stay bound to live state.
	 */
