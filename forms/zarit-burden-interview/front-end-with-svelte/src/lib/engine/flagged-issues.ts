import type { AssessmentData, FlaggedIssue, InstrumentForm } from './types';
import { activeItemNumbers, ratingValue } from './zarit-rules';

/**
 * Detect clinician-facing flags (red flags), independent of the burden band
 * (spec §5):
 *
 ...
 *
 * Flags are computed from the grade (total, instrument form) and the raw
 * item-22 rating; the incomplete-assessment flag inspects only the active item
 * set for the selected instrument form. Rows mirror the
 * `zarit_burden_interview_grade_flag` SQL table.
 */
export function detectFlaggedIssues(
	data: AssessmentData,
	grade: { totalScore: number; instrumentForm: InstrumentForm }
): FlaggedIssue[] {
	const flags: FlaggedIssue[] = [];

	const { totalScore, instrumentForm } = grade;
	const item22 = ratingValue(data.items.item22);

	// Severity thresholds by instrument form.
	const severeBurden = instrumentForm === 'zbi12' ? totalScore >= 17 : totalScore >= 61;
	const moderateToSevere =
		instrumentForm === 'zbi22' && totalScore >= 41 && totalScore <= 60;
	...
	if (moderateToSevere) {
		flags.push({
			...
			priority: 'high',
			description: `ZBI-22 total ${totalScore} of 88 is in the moderate-to-severe-burden band (41-60)`,
			suggestedAction:
	...
	if (item22 !== null && item22 >= 3) {
		flags.push({
			...
			priority: 'medium',
			description: `The carer rates their overall (global) burden as ${item22 === 4 ? '"Nearly always"' : '"Quite frequently"'} (item 22 = ${item22} of 4)`,
			suggestedAction:
	...
	for (const n of active) {
		const v = ratingValue(data.items[`item${n}` as keyof AssessmentData['items']]);
	...
	if (missing.length > 0) {
		flags.push({
			...
			category: 'incomplete',
			priority: 'low',
			description: `Missing rating(s) for active item(s): ${missing.join(', ')} — the total may understate burden`,
			suggestedAction: 'Re-administer the outstanding item(s) and re-score.'
		});
	}
