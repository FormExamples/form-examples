import type { AssessmentData, InstrumentForm, Items, ZaritItem } from './types';
 ...
 * frequency scale (0 = Never … 4 = Nearly always); a higher rating always means
 * greater perceived burden, so there is NO reverse-scoring. The wizard stores
 * the raw 0..4 rating; the grader (`zarit-grader.ts`) sums the answered ratings
 ...
 * `zarit_burden_interview_grade_rule` SQL table. Item wording is abridged from
 * the standard Zarit, Reeves & Bach-Peterson (1980) instrument.
 */

/** The 12-item short-form subset (spec §3 / §4). */
export const SHORT_FORM_ITEMS = [1, 2, 3, 6, 9, 10, 11, 12, 17, 20, 21, 22];
...
const itemStatements: { number: number; field: keyof Items; statement: string }[] = [
...
export const zaritItems: ZaritItem[] = itemStatements.map((it) => ({
	...it,
...
export function activeItemNumbers(instrumentForm: InstrumentForm): number[] {
...
export function maxScoreFor(instrumentForm: InstrumentForm): 88 | 48 {
	return instrumentForm === 'zbi12' ? 48 : 88;
}

/** Coerce a raw stored rating to a 0..4 integer, or null when unanswered / invalid. */
export function ratingValue(value: number | null | undefined | string): number | null {
...
export function itemByNumber(n: number): ZaritItem | undefined {
	return zaritItems.find((it) => it.number === n);
}

/** Normalise the stored instrument-form selector to a known value (ZBI-22 default). */
export function normalizeInstrumentForm(data: AssessmentData): InstrumentForm {
	return data.context.instrumentForm === 'zbi12' ? 'zbi12' : 'zbi22';
}
