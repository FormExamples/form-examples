# Padua Venous Thromboembolism Risk Assessment — tasks
...
- [x] Scoring engine (`types.ts`, `utils.ts`, `padua-rules.ts`,
      `padua-grader.ts`, `flagged-issues.ts`) + Vitest tests.
...
- [ ] `bin/test-form padua-venous-thromboembolism-risk-assessment` passes.
