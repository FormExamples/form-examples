// Padua VTE — clinician dashboard (vanilla classic-script app).
//
...
// box + care-setting dropdown + risk-band dropdown + prophylaxis dropdown).
//
// Sibling modules loaded as plain `<script>` tags (in dependency order) attach
// their exports to `window.PaduaVenousThromboembolismRiskAssessmentDashboard`.
...
const filters = {
  search: '',
  setting: '',      // '' | 'acute-medical' | 'general-medical' | 'admissions-unit' | 'other'
  band: '',         // '' | 'low' | 'high'
  prophylaxis: ''   // '' | 'pharmacological' | 'mechanical' | 'none'
};
...
const columns = [
  ...
  { key: 'paduaScore',                label: 'Padua' },
  { key: 'riskBand',                  label: 'Risk Band' },
  { key: 'prophylaxisRecommendation', label: 'Prophylaxis' }
];

// Rank used when sorting the riskBand column so 'low' is always less than
...
function bandClass(band) {
  if (!band) return '';
  return band === 'high' ? 'risk-high' : 'risk-low';
}
...
function matchesFilters(row) {
  ...
  if (filters.prophylaxis && row.prophylaxisRecommendation !== filters.prophylaxis) {
    return false;
  }
  return true;
}
 ...
 * rank table; the nullable Padua score sorts nulls last; everything else uses
...
function renderTableBody() {
  ...
  for (const row of rows) {
    ...
    const scoreClassName = row.paduaScore === null
    ...
    tr.innerHTML = `
      ...
      <td><span class="${scoreClassName}">${esc(scoreLabel(row.paduaScore))}</span></td>
      <td><span class="risk-badge ${bandClass(row.riskBand)}">${esc(bandLabel(row.riskBand))}</span></td>
      <td>${esc(prophylaxisLabel(row.prophylaxisRecommendation))}</td>
    `;
    body.appendChild(tr);
  }
}
...
function bindFilterInputs() {
  ...
  const prophylaxis = document.getElementById('filter-prophylaxis');
  ...
  if (prophylaxis) {
    prophylaxis.addEventListener('change', () => {
      filters.prophylaxis = prophylaxis.value;
      renderAll();
    });
  }
  if (clearBtn) {
    clearBtn.addEventListener('click', () => {
      ...
      if (band) band.value = '';
      if (prophylaxis) prophylaxis.value = '';
      renderAll();
    });
  }
}
