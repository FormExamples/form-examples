<script lang="ts">
	...
	import { riskBandLabel, careSettingLabel, prophylaxisLabel } from '$lib/engine/utils';
	...
	// SVAR DataGrid columns. Padua score, risk band, and prophylaxis render
	...
	const columns = [
		...
		{ id: 'paduaScore', header: 'Padua', width: 90, sort: true },
		{
			id: 'riskBand',
			header: 'Risk band',
			width: 160,
		...
		},
		{
			...
			header: 'Prophylaxis',
			width: 260,
			template: (v: string) => prophylaxisLabel(v as never)
		},
	...
	function init(api: any) {
		...
		api.on('select-row', (ev: { id?: string | number }) => {
			if (ev?.id != null) goto(`/padua-venous-thromboembolism-risk-assessment/padua-venous-thromboembolism-risk-assessments/${ev.id}`);
		});
	}
</script>
...
<main class="mx-16 px-4 py-6">
	<div class="mb-6 flex flex-wrap items-end justify-between gap-4">
		<div>
			<h1 class="text-2xl font-bold text-base-content">Padua VTE clinician dashboard</h1>
			<p class="mt-1 text-sm text-base-content/70">
				Padua score, risk band, and prophylaxis recommendation for assessed patients, computed by
				the shared engine. Select a row to open the assessment.
			</p>
		</div>
		<a
			href="/padua-venous-thromboembolism-risk-assessment/padua-venous-thromboembolism-risk-assessments/new"
			class="button"
	...
	<div class="mb-4 flex flex-wrap gap-4">
		<label class="text-sm">
			...
			<select class="select inline-block w-auto" bind:value={settingFilter}>
				...
				<option value="admissions-unit">Admissions unit</option>
		...
		<label class="text-sm">
			...
			<select class="select inline-block w-auto" bind:value={bandFilter}>
				...
				<option value="high">High (&ge; 4)</option>
			</select>
		</label>
	</div>
