 ...
 *
 * Rows mirror the `padua_venous_thromboembolism_risk_assessment_grade_flag` SQL
 * table (flag_id, category, priority, description, suggested_action).
 */
export function detectFlaggedIssues(data: AssessmentData, paduaScore: number): FlaggedIssue[] {
	...
	if (activeBleeding === 'yes' || highBleedingRisk === 'yes') {
		const detail =
			activeBleeding === 'yes' ? 'Active bleeding present' : 'High bleeding-risk factors present';
		flags.push({
	...
	if (data.history.activeCancer === 'yes') {
		flags.push({
			...
			priority: 'medium',
			description:
				'Active cancer — the highest single-factor contribution (3 points) to the Padua score',
			suggestedAction:
	...
	if (data.history.previousVte === 'yes') {
		flags.push({
			...
			priority: 'medium',
			description:
				'Previous venous thromboembolism — a strong independent predictor of recurrence',
			suggestedAction:
	...
	if (missing.length > 0) {
		flags.push({
			...
			suggestedAction: 'Record the missing value(s) and re-score.'
		});
	}
