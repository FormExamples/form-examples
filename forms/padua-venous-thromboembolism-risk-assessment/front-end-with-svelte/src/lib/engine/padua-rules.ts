import type { PaduaRule } from './types';

/**
 * Declarative Padua Prediction Score grading rules.
 *
 * The Padua instrument has eleven weighted risk factors. Each rule evaluates the
 * patient data and returns true when its factor is present; the grader
 * (`padua-grader.ts`) sums the weighted points into the total Padua score
 * (0-20) and derives the risk band (>= 4 -> high). Rows mirror the
 * `padua_venous_thromboembolism_risk_assessment_grade_rule` SQL table
 * (rule_id, factor, points, category, description).
 */
export const paduaRules: PaduaRule[] = [
	// ─── FACTOR 1: ACTIVE CANCER (3) ──────────────────────────────
	{
		...
		description:
			'Active cancer — metastatic and/or chemo/radiotherapy in the previous 6 months',
	...
	// ─── FACTOR 2: PREVIOUS VTE (3) ───────────────────────────────
	{
		...
		category: 'padua-factor',
		description:
			'Previous VTE — history of deep-vein thrombosis or pulmonary embolism (excluding superficial vein thrombosis)',
	...
	// ─── FACTOR 4: KNOWN THROMBOPHILIA (3) ────────────────────────
	{
		...
		category: 'padua-factor',
		description:
			'Known thrombophilia — e.g. antithrombin, protein C or S defect, factor V Leiden, prothrombin G20210A, antiphospholipid syndrome',
	...
	// ─── FACTOR 6: ELDERLY AGE >= 70 (1) ──────────────────────────
	{
		...
		evaluate: (d) => d.identification.ageYears !== null && d.identification.ageYears >= 70
	},
	...
	// ─── FACTOR 10: OBESITY BMI >= 30 (1) ─────────────────────────
	{
		...
		evaluate: (d) => d.metabolic.bodyMassIndex !== null && d.metabolic.bodyMassIndex >= 30
	},
