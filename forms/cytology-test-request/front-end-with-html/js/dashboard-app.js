...
// Cytology Test Request — vetting dashboard (vanilla classic-script app).
//
// On boot we fetch the request list from the backend; on any failure (or
// empty response) we fall back to sample data and show a small banner. The
// rendered table is sortable (click any column header) and filterable
// (search box + specimen-type dropdown + triage-tier dropdown +
// appropriateness-band dropdown + pre-analytical-adequacy dropdown).
//
// Sibling modules loaded as plain `<script>` tags (in dependency order)
// attach their exports to `window.CytologyTestRequestDashboard`. The whole
...
function preanalyticalClass(adequacy) {
  return adequacy ? 'preanalytical-' + String(adequacy) : '';
}
...
function matchesFilters(row) {
  if (filters.search) {
    ...
    const matches =
      ...
      String(row.specimenType || '').toLowerCase().includes(term) ||
      String(row.clinician || '').toLowerCase().includes(term);
    if (!matches) return false;
  }
  if (filters.specimen && row.specimenType !== filters.specimen) return false;
...
function compareRows(a, b) {
  ...
  // Default: string compare (nhs, patient, specimenType, indication,
...
function renderTableBody() {
  ...
  for (const row of rows) {
    ...
    if (row.triageTier === 'two-week-wait') {
    ...
    tr.innerHTML = `
      ...
      <td class="data-table-td">${esc(titleCase(row.specimenType))}</td>
      ...
      <td class="data-table-td"><span class="preanalytical-badge ${preanalyticalClass(row.preanalyticalBand)}">${esc(titleCase(row.preanalyticalBand))}</span></td>
...
function bindFilterInputs() {
  ...
  const specimen = document.getElementById('filter-specimen');
  const triage = document.getElementById('filter-triage');
  const band = document.getElementById('filter-band');
  const preanalytical = document.getElementById('filter-preanalytical');
  ...
  if (specimen) {
    specimen.addEventListener('change', () => {
      filters.specimen = specimen.value;
      renderAll();
    });
  }
  if (triage) {
  ...
  if (preanalytical) {
    preanalytical.addEventListener('change', () => {
      filters.preanalytical = preanalytical.value;
      renderAll();
    });
  }
  if (clearBtn) {
    clearBtn.addEventListener('click', () => {
      ...
      if (specimen) specimen.value = '';
      if (triage) triage.value = '';
      if (band) band.value = '';
      if (preanalytical) preanalytical.value = '';
      renderAll();
    });
  }
}
