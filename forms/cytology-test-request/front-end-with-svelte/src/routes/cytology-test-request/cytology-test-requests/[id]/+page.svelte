<script lang="ts">
	...
	import Step4CytologyContext from '#lib/components/steps/Step4CytologyContext.svelte';
	import Step5SpecimenCollection from '#lib/components/steps/Step5SpecimenCollection.svelte';
	...
	function validate(): boolean {
		...
		if (d.request.specimenType === '') {
			found.push({ id: 'specimenType', message: 'Requested specimen type is required.' });
		}
	...
	function submit() {
		...
		goto(`/cytology-test-request/cytology-test-requests/${id}/report`);
	}
...
<main class="mx-16 px-4 py-6">
	<h1 class="text-2xl font-bold text-base-content">
		{isNew ? 'New cytology request' : `Cytology request ${id}`}
	</h1>
	...
	<Form label="Cytology test request" onsubmit={submit}>
		<Step1RequestingClinician />
		<Step2PatientIdentification />
		<Step3RequestedExamination />
		<Step4CytologyContext />
		<Step5SpecimenCollection />
