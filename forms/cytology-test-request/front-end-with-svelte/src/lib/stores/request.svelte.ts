...
import type { CytologyRequest, GradingResult } from '$lib/engine/types';
import { createDefaultRequest } from '$lib/engine/defaults';

// Re-export the canonical factory so consumers can import it from the store.
export { createDefaultRequest };

/** localStorage draft key for a given request id (defaults to `new`). */
function storageKey(id: string): string {
	return `cytology-test-request.front-end-with-svelte.${id || 'new'}.v1`;
}

/**
 * Svelte 5 reactive store for the cytology specimen request, with localStorage
 ...
 */
class RequestStore {
	data = $state<CytologyRequest>(createDefaultRequest());
	 ...
	 */
	loadForId(id: string, seed?: CytologyRequest) {
		const key = id || 'new';
		this.id = key;
		this.result = null;
		this.currentStep = 1;

		let draft: CytologyRequest | null = null;
		if (browser) {
			const raw = localStorage.getItem(storageKey(key));
			if (raw) {
				try {
					draft = JSON.parse(raw) as CytologyRequest;
				} catch {
