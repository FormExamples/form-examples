# Cytology Test Request — agent instructions

Cytology specimen request (referral). A clinician completes a single-page
wizard; the engine computes a **four-axis grading** (appropriateness,
pre-analytical specimen adequacy, request completeness, triage priority) plus
...
back-end. A suspected-cancer indication or a previous high-grade cytology
result auto-escalates the triage tier toward the two-week-wait pathway. Choose
the least-alarming band only when no rule fires.
