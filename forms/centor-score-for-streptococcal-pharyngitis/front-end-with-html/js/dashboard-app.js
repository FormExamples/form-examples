// Centor Score for Streptococcal Pharyngitis — clinician dashboard
...
// box + care-setting dropdown + risk-band dropdown + red-flag dropdown).
//
// Sibling modules loaded as plain `<script>` tags (in dependency order) attach
// their exports to `window.CentorScoreForStreptococcalPharyngitisDashboard`.
...
const filters = {
  search: '',
  setting: '',   // '' | 'general-practice' | 'urgent-care' | 'pharmacy' | 'emergency-department' | 'other'
...
const columns = [
  ...
  { key: 'centorScore',       label: 'Centor' },
  { key: 'mcIsaacScore',      label: 'McIsaac' },
  { key: 'riskBand',          label: 'Risk Band' },
  { key: 'redFlag',           label: 'Red Flag' }
];

// Rank used when sorting the riskBand column so bands order low < moderate <
 ...
 * rank table; the nullable Centor / McIsaac scores sort nulls last; the
 * red-flag boolean sorts false<true; everything else uses a locale-aware
 * string compare.
 */
function compareRows(a, b) {
  ...
  if (key === 'centorScore' || key === 'mcIsaacScore') {
...
function renderTableBody() {
  ...
  for (const row of rows) {
    ...
    if (row.riskBand === 'high' || row.redFlag) {
      tr.classList.add('row-critical');
    }

    const centorClassName = row.centorScore === null
      ? 'class-cell class-cell-na'
      : 'class-cell';
    const mcIsaacClassName = row.mcIsaacScore === null
      ? 'class-cell class-cell-na'
      : 'class-cell';
    ...
    tr.innerHTML = `
      ...
      <td><span class="${centorClassName}">${esc(scoreLabel(row.centorScore))}</span></td>
      <td><span class="${mcIsaacClassName}">${esc(scoreLabel(row.mcIsaacScore))}</span></td>
      <td><span class="risk-badge ${bandClass(row.riskBand)}">${esc(bandLabel(row.riskBand))}</span></td>
      <td>
        <span class="flag-badge ${row.redFlag ? 'flag-yes' : 'flag-no'}">
          ${row.redFlag ? 'Yes' : 'No'}
        </span>
      </td>
    `;
