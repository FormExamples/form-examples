// Flagged-issue detection (red flags). Independent of the Centor total (which
// the grader produces), this module raises clinician-facing safety and
// management flags per spec §5:
//
...
// `centor_score_for_streptococcal_pharyngitis_grade_flag` SQL table
 ...
 */
function detectFlaggedIssues(data, mcIsaacScore) {
  /** @type {FlaggedIssue[]} */
  const flags = [];
  ...
  if (data.fever.feverOver38 === '' && data.fever.measuredTemperatureCelsius === null) missing.push('fever (history or measured temperature)');
