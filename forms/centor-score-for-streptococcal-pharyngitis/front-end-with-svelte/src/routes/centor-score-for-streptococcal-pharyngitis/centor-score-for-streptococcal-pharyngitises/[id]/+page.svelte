<script lang="ts">
	...
	import Step3TonsillarExudate from '$lib/components/steps/Step3TonsillarExudate.svelte';
	import Step4CervicalLymphadenopathy from '$lib/components/steps/Step4CervicalLymphadenopathy.svelte';
	import Step5Fever from '$lib/components/steps/Step5Fever.svelte';
	import Step6Cough from '$lib/components/steps/Step6Cough.svelte';
	import Step7RedFlagReview from '$lib/components/steps/Step7RedFlagReview.svelte';
	...
	function submit() {
		...
		assessment.result = calculateCentorGrade(assessment.data);
		goto(`/centor-score-for-streptococcal-pharyngitis/${plural}/${id}/report`);
	}
...
<main class="mx-16 px-4 py-6">
	<h1 class="text-2xl font-bold text-base-content">
		{isNew ? 'New Centor / McIsaac assessment' : `Centor / McIsaac assessment ${id}`}
	</h1>
	<p class="mt-1 text-sm text-base-content/70">
		Complete the eight sections; the Centor total, McIsaac score, and risk band are computed on
		submit.
	</p>
	...
	<Form label="Centor / McIsaac assessment" onsubmit={submit}>
		<Step1Context />
		<Step2Identification />
		<Step3TonsillarExudate />
		<Step4CervicalLymphadenopathy />
		<Step5Fever />
		<Step6Cough />
		<Step7RedFlagReview />
		<Step8Summary />
