# Centor Score for Streptococcal Pharyngitis — tasks

Task tracking. See [`plan.md`](./plan.md) for the roadmap.

## Done

- [x] Author `index.md` (title, scope, scoring system with Centor criteria +
      McIsaac age modifier + bands, assessment steps, conventions, compliance,
      references).
- [x] Author `spec/index.md` (data model, grading algorithm incl. age modifier,
...
- [x] Scoring engine (`types.ts`, `utils.ts`, `centor-rules.ts`,
      `centor-grader.ts`, `flagged-issues.ts`) + Vitest tests.
...
- [ ] `bin/test-form centor-score-for-streptococcal-pharyngitis` passes
