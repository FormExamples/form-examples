...
impl ActiveModel {
    /// Create a new in-progress acknowledgment with empty data.
...
/// Find an acknowledgment by its UUID.
pub async fn find_by_id(db: &DatabaseConnection, id: Uuid) -> Result<Option<Model>, DbErr> {
    Entity::find_by_id(id).one(db).await
}

/// List all completed acknowledgments (those with a grading result).
...
/// List acknowledgments with pagination (page is 0-indexed, per_page items each).
