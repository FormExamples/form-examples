# Legal Requirements Privacy Notice — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Legal Requirements Privacy Notice form, built with axum + Loco +
...
- [`legal_requirements_privacy_notice/`](./legal_requirements_privacy_notice/) — the Loco crate: `src/legal_requirements_privacy_notice/` holds `app.rs`
...
`src/legal_requirements_privacy_notice/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd legal_requirements_privacy_notice && cargo check --all-targets && cargo test
```
