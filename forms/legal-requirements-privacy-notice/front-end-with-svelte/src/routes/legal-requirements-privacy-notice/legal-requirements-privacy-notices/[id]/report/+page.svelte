<script lang="ts">
	...
	$effect(() => {
		if (!assessment.result) {
			goto(`/legal-requirements-privacy-notice/legal-requirements-privacy-notices/${id}`);
		}
	});
	...
	async function downloadPDF() {
		pdfError = '';
		try {
			const res = await fetch(`/legal-requirements-privacy-notices/${id}/report/pdf`, {
				method: 'POST',
			...
			if (res.ok) {
				...
				a.href = url;
				a.download = `legal-requirements-privacy-notice-${data.acknowledgment.patientTypedFullName || id}.pdf`;
				a.click();
	...
	<header class="border-b border-base-300 bg-base-100 shadow-sm no-print">
		<div class="mx-auto flex max-w-4xl items-center justify-between px-4 py-4">
			<h1 class="text-lg font-bold text-base-content">
				Legal requirements privacy notice — acknowledgment summary
			</h1>
			<div class="flex items-center gap-3">
				...
				<Button
					...
					onclick={() => goto(`/legal-requirements-privacy-notice/legal-requirements-privacy-notices/${id}`)}>Edit</Button
				>
			</div>
		</div>
	</header>
