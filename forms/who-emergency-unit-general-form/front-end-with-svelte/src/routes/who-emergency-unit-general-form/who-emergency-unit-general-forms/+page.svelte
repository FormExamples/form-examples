<script lang="ts">
			...
			? 'Orange'
				: v === 'yellow'
					...
					: v === 'green'
						? 'Green'
						: '—';
	const dispositionLabel = (v: string) =>
		v === 'admit'
		? 'Admit'
		: v === 'transfer'
			? 'Transfer'
	...
	const columns = [
		...
		{ id: 'avpu', header: 'AVPU', width: 80, sort: true },
		{
			id: 'disposition',
			header: 'Disposition',
			width: 120,
			sort: true,
			template: (v: string) => dispositionLabel(v)
		},
		{
			id: 'complete',
			header: 'Status',
	...
	function init(api: any) {
		...
		api.on('select-row', (ev: { id?: string | number }) => {
			if (ev?.id != null) goto(`/who-emergency-unit-general-form/who-emergency-unit-general-forms/${ev.id}`);
		});
	}
</script>
...
<main class="mx-16 px-4 py-6">
	<div class="mb-6 flex flex-wrap items-end justify-between gap-4">
		<div>
			<h1 class="text-2xl font-bold text-base-content">Emergency unit clinician dashboard</h1>
			<p class="mt-1 text-sm text-base-content/70">
				Completeness status and flagged issues for recorded emergency unit encounters, computed by
				the shared engine. Select a row to open the encounter.
			</p>
		</div>
		<a href="/who-emergency-unit-general-form/who-emergency-unit-general-forms/new" class="button" data-variant="primary"
			>New encounter</a
		>
	</div>
	...
	<div class="mb-4 flex flex-wrap gap-4">
		<label class="text-sm">
			<span class="mr-2 font-medium text-base-content/80">Triage category</span>
