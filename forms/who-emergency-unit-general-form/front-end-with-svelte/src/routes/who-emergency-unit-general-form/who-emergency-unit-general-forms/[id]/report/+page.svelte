<script lang="ts">
	...
	import { calculateAge, priorityColor, priorityLabel, sectionLabel } from '#lib/engine/utils.js';
	...
	$effect(() => {
		if (!assessment.result) {
			goto(`/who-emergency-unit-general-form/who-emergency-unit-general-forms/${id}`);
		}
	});
	...
	async function downloadPDF() {
		pdfError = '';
		try {
			const res = await fetch(`/who-emergency-unit-general-forms/${id}/report/pdf`, {
				method: 'POST',
			...
			if (res.ok) {
				...
				a.href = url;
				a.download = `who-emergency-unit-general-${data.patientRegistration.surname || id}.pdf`;
				a.click();
	...
	// Status banner colour: success when complete with no flags, error/warning otherwise.
...
{#if result}
	<header class="border-b border-base-300 bg-base-100 shadow-sm no-print">
		<div class="mx-auto flex max-w-4xl items-center justify-between px-4 py-4">
			<h1 class="text-lg font-bold text-base-content">Emergency unit report</h1>
			<div class="flex items-center gap-3">
				...
				<Button
					...
					onclick={() => goto(`/who-emergency-unit-general-form/who-emergency-unit-general-forms/${id}`)}>Edit</Button
				>
			</div>
		</div>
	</header>
	...
	<main class="mx-16 px-4 py-6">
		...
		<div class="mb-6 rounded-xl border border-base-300 bg-base-100 p-6">
			...
			<div class="grid grid-cols-1 gap-4 text-sm sm:grid-cols-2">
				<div><span class="font-medium text-base-content/70">Name:</span> {patientName}</div>
				<div>
				...
				<div>
					...
					{data.chiefComplaintAndVitals.triageCategory || '—'}
				</div>
				<div>
					<span class="font-medium text-base-content/70">Disposition:</span>
			...
			{#if data.historyOfPresentIllness.narrative}
				<p class="mt-4 text-sm text-base-content/80">
					<span class="font-medium text-base-content/70">History of present illness:</span>
					{data.historyOfPresentIllness.narrative}
				</p>
			{/if}
