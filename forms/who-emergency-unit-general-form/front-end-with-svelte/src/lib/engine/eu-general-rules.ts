...
/**
 * WHO Emergency Unit Form: General — completeness rules.
 *
 ...
 * with `applies()` so the validator only counts a rule when its branch is
 * active for the patient's answers.
 *
 * Rule IDs follow the pattern <SECTION>-<NN>; the prefix lets the report
 * group fired rules by section.
 */
export const euGeneralRules: ValidationRule[] = [
	...
	},
	{
		...
		applies: () => true,
		isSatisfied: (d) =>
			d.pastMedicalHistory.medicationsUnknown || hasText(d.pastMedicalHistory.medications)
	},
	{
		id: 'PMH-03',
		section: 'pastMedicalHistory',
		...
		applies: () => true,
		isSatisfied: (d) =>
			d.pastMedicalHistory.allergiesUnknown || hasText(d.pastMedicalHistory.allergies)
	},
