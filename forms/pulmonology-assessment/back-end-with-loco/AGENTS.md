# Pulmonology Assessment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Pulmonology Assessment form, built with axum + Loco +
...
- [`pulmonology_assessment/`](./pulmonology_assessment/) — the Loco crate: `src/pulmonology_assessment/` holds `app.rs`
...
`src/pulmonology_assessment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd pulmonology_assessment && cargo check --all-targets && cargo test
```
