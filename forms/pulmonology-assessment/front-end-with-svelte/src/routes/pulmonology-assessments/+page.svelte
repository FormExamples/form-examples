<script lang="ts">
	...
	import { goldStageLabel, abcdGroupLabel } from '$lib/engine/utils';
	...
	const columns = [
		...
		{ id: 'oxygenFlag', header: 'O₂ therapy', width: 110, template: (v: boolean) => (v ? 'Yes' : 'No') },
	...
	function init(api: any) {
		...
		api.on('select-row', (ev: { id?: string | number }) => {
			if (ev?.id != null) goto(`/pulmonology-assessments/${ev.id}`);
		});
	}
</script>
...
<main class="mx-auto max-w-6xl px-4 py-6">
	<div class="mb-6 flex flex-wrap items-end justify-between gap-4">
		...
		<a href="/pulmonology-assessments/new" class="button" data-variant="primary">New assessment</a>
	</div>
