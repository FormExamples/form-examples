<script lang="ts">
	...
	let { class: className = '', label, description = undefined, error = undefined, required = false, inputId = undefined, children, ...restProps }: { label: string; description?: string; error?: string; required?: boolean; inputId?: string; children: Snippet; [key: string]: unknown } = $props();
	const uid = $props.id();
