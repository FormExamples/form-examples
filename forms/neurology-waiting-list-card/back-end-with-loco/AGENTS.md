# Neurology Waiting List Card — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Neurology Waiting List Card form, built with axum + Loco +
...
- [`neurology_waiting_list_card/`](./neurology_waiting_list_card/) — the Loco crate: `src/neurology_waiting_list_card/` holds `app.rs`
...
`/metrics`. The registered domain controllers are:
  ...
  - `neurology_waiting_list_card`
  - `neurology_waiting_list_card_appointment`
  - `neurology_waiting_list_card_grade`
  - `neurology_waiting_list_card_grade_flag`
  - `neurology_waiting_list_card_grade_rule`
  - `patient`
  - `practitioner`

## Engine

`src/neurology_waiting_list_card/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd neurology_waiting_list_card && cargo check --all-targets && cargo test
```
