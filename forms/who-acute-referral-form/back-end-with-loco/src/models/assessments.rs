...
impl ActiveModel {
    /// Create a new in-progress referral with empty data.
...
impl Model {
    ...
    /// Deserialize the JSONB `result` column into ReportResult.
    pub fn report_result(&self) -> Result<Option<ReportResult>, serde_json::Error> {
        match &self.result {
...
/// List referrals with pagination (page is 0-indexed, per_page items each).
