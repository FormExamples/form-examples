# WHO Acute Referral Form — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the WHO Acute Referral Form form, built with axum + Loco +
...
- [`who_acute_referral_form/`](./who_acute_referral_form/) — the Loco crate: `src/who_acute_referral_form/` holds `app.rs`
...
`src/who_acute_referral_form/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd who_acute_referral_form && cargo check --all-targets && cargo test
```
