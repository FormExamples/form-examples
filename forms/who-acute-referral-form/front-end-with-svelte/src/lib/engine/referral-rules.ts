 ...
 * the "Other" box is ticked, "Receiving provider" details only required when
 * the patient has arrived at the referral facility).
 *
 * Rule IDs follow the pattern <SECTION>-<NN>; the prefix lets the report group
 * fired rules by section.
 */
export const referralRules: ValidationRule[] = [
	...
	// These rules only apply once the receiving facility has begun its part of
	// the form. We trigger them as soon as any receipt-side field has been
	// touched (arrival time recorded, provider name entered, etc.).
	{
		id: 'RFR-01',
