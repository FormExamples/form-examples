...
import type { UltrasoundRequest, GradingResult } from '#lib/engine/types.js';
import { createDefaultRequest } from '#lib/engine/defaults.js';

// Re-export so callers can reach the canonical blank-request factory through the store.
export { createDefaultRequest };

/** localStorage draft key for a given request id (defaults to `new`). */
function storageKey(id: string): string {
	return `pregnancy-ultrasound-test-request.front-end-with-svelte.${id || 'new'}.v1`;
}

/**
 * Svelte 5 reactive store for the obstetric ultrasound request, with
 * localStorage persistence so an in-progress request survives a page reload.
 * Drafts are keyed by request id so each record edits independently.
 */
class RequestStore {
	data = $state<UltrasoundRequest>(createDefaultRequest());
	 ...
	 * reassigned, so step components that captured a section reference
	 * (e.g. `const d = request.data.clinician`) stay bound to live state.
	 */
	loadForId(id: string, seed?: UltrasoundRequest) {
		const key = id || 'new';
		this.id = key;
		this.result = null;
		this.currentStep = 1;

		let draft: UltrasoundRequest | null = null;
		if (browser) {
			const raw = localStorage.getItem(storageKey(key));
			if (raw) {
				try {
					draft = JSON.parse(raw) as UltrasoundRequest;
				} catch {
