...
// suspected-ectopic, heavy-bleeding, severe-pain, haemodynamic-instability,
// reduced-fetal-movements, suspected-praevia, suspected-severe-fgr,
// pre-eclampsia, missing-indication, missing-clinical-question,
// window-mismatch, incomplete-dating. Each flag is
// { flagId, category, priority, description, suggestedAction }. Flag IDs are
// stable and identical across every front-end and the back-end.

import type { UltrasoundRequest, Flag, WindowFit, ScanType } from './types';
...
export function detectFlags(data: UltrasoundRequest, context: FlagContext = {}): Flag[] {
	...
	if (data.symptoms.suspectedEctopic === true) {
		flags.push({
			...
			suggestedAction:
				'Arrange same-day emergency assessment in EPAU; do not delay for routine booking.'
		});
	}
	if (data.symptoms.haemodynamicallyUnstable === true) {
		flags.push({
			...
			suggestedAction:
				'Immediate clinical review and resuscitation; emergency scan alongside obstetric / gynaecology team.'
		});
	}
	...
	if (data.symptoms.reducedFetalMovements === true) {
		flags.push({
			...
			suggestedAction:
				'Urgent assessment per RCOG GTG 57; CTG if viable gestation and growth / wellbeing scan.'
		});
	}
	...
	if (
		...
		flags.push({
			...
			suggestedAction:
				'Avoid digital vaginal examination until praevia excluded; confirm placental site.'
		});
	}
	if (
		...
		flags.push({
			...
			suggestedAction:
				'Prioritise growth + umbilical-artery Doppler; consider fetal-medicine referral.'
		});
	}
	if (
	...
	if (context.windowFit === 'outside-window') {
		flags.push({
			...
			suggestedAction: context.recommendedScanType
				? `Consider redirecting to a ${context.recommendedScanType} scan for this gestation.`
...
function datingIncomplete(d: UltrasoundRequest): boolean {
	const hasGa = d.dating.gestationalAgeWeeks !== null && d.dating.gestationalAgeWeeks !== undefined;
