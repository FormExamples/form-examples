<script lang="ts">
	...
	const columns = [
		...
		{ id: 'gestationalAge', header: 'GA', width: 80 },
		{
		...
		},
		{
			...
			header: 'Window',
			width: 130,
			template: (v: string) => windowFitLabel(v as never)
		},
	...
	function init(api: any) {
		...
		api.on('select-row', (ev: { id?: string | number }) => {
			if (ev?.id != null) goto(`/pregnancy-ultrasound-test-request/pregnancy-ultrasound-test-requests/${ev.id}`);
		});
	}
</script>
...
<main class="mx-16 px-4 py-6">
	<div class="mb-6 flex flex-wrap items-end justify-between gap-4">
		<div>
			<h1 class="text-2xl font-bold text-base-content">Ultrasound request vetting dashboard</h1>
			<p class="mt-1 text-sm text-base-content/70">
				Four-axis vetting grades for incoming obstetric ultrasound requests, computed by the shared
				engine. Select a row to open the request.
			</p>
		</div>
		<a href="/pregnancy-ultrasound-test-request/pregnancy-ultrasound-test-requests/new" class="button" data-variant="primary"
			>New request</a
		>
	</div>
