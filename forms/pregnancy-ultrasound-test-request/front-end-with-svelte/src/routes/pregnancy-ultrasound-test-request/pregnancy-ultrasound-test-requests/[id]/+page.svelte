<script lang="ts">
	...
	import Step3PregnancyDating from '#lib/components/steps/Step3PregnancyDating.svelte';
	import Step4ObstetricHistory from '#lib/components/steps/Step4ObstetricHistory.svelte';
	import Step5RequestedExamination from '#lib/components/steps/Step5RequestedExamination.svelte';
	import Step6SymptomsAndRedFlags from '#lib/components/steps/Step6SymptomsAndRedFlags.svelte';
	import Step7RiskFactors from '#lib/components/steps/Step7RiskFactors.svelte';
	import Step8TriageAndSubmit from '#lib/components/steps/Step8TriageAndSubmit.svelte';
	...
	function validate(): boolean {
		...
		if (d.request.requestedScanType === '') {
			found.push({ id: 'requestedScanType', message: 'Requested scan type is required.' });
		}
	...
	function submit() {
		...
		goto(`/pregnancy-ultrasound-test-request/pregnancy-ultrasound-test-requests/${id}/report`);
	}
...
<main class="mx-16 px-4 py-6">
	<h1 class="text-2xl font-bold text-base-content">
		{isNew ? 'New ultrasound request' : `Ultrasound request ${id}`}
	</h1>
	...
	<RestoreBanner show={request.hadDraftAtLoad} onDiscard={startOver} />

	<Form label="Pregnancy ultrasound request" onsubmit={submit}>
		<Step1RequestingClinician />
		<Step2PatientIdentification />
		<Step3PregnancyDating />
		<Step4ObstetricHistory />
		<Step5RequestedExamination />
		<Step6SymptomsAndRedFlags />
		<Step7RiskFactors />
		<Step8TriageAndSubmit />
