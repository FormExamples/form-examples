	...
	if (data.certificateType === 'pre') {
		...
		if (weeks !== null && weeks > 20) {
			flags.push({
				...
				category: 'Timing',
				message: `Certificate is being issued ${weeks} weeks before the expected date of confinement. DWP guidance prohibits issuing a MAT B1 more than 20 weeks before the EWC.`,
				priority: 'high'
			});
		}
	}
