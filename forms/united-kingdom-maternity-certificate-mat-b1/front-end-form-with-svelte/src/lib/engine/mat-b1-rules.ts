...
import { isFilled, weeksBetween } from './utils';
 ...
 *      `isDuplicate = yes`.
 *   3. **Timing** — DWP guidance: the certificate must not be issued more
 *      than 20 weeks before the expected week of confinement (EWC).
 *   4. **Declaration** — completed-in-ink confirmation; valid NMC registration
 *      for midwives.
 *
 * The rules are pure: no side effects, no network calls.
 */
export const matB1Rules: ValidationRule[] = [
	...
	// ─── Certificate type selection ──────────────────────────
	{
		...
		category: 'completeness',
		priority: 'high',
		description: 'Either Part A (pre-confinement) or Part B (post-confinement) must be selected.',
	...
	// ─── Part A — Pre-confinement ────────────────────────────
	{
		...
		evaluate: (d) =>
			d.certificateType === 'pre' && !isFilled(d.preConfinement.expectedDateOfConfinement),
		message:
