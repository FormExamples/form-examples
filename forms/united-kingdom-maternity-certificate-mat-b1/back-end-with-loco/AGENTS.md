# United Kingdom Maternity Certificate MAT B1 — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the United Kingdom Maternity Certificate MAT B1 form, built with axum + Loco +
...
- [`united_kingdom_maternity_certificate_mat_b1/`](./united_kingdom_maternity_certificate_mat_b1/) — the Loco crate: `src/united_kingdom_maternity_certificate_mat_b1/` holds `app.rs`
...
`src/united_kingdom_maternity_certificate_mat_b1/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd united_kingdom_maternity_certificate_mat_b1 && cargo check --all-targets && cargo test
```
