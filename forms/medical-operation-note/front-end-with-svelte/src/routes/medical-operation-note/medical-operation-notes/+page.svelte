<script lang="ts">
	...
	import { sampleOperationNoteRows } from '#lib/data/sample-reports.js';
	import { compositeRiskLabel, clavienDindoLabel } from '#lib/engine/utils.js';
	...
	const columns = [
...

		{
			...
			header: 'Counts',
			width: 90,
			template: (v: boolean) => v ? 'Agreed' : 'Discrepancy'
		},

		{
			id: 'neverEventFlagged',
			header: 'Never event',
			width: 110,
	...
	function init(api: any) {
		...
		api.on('select-row', (ev: { id?: string | number }) => {
			if (ev?.id != null) goto(`/medical-operation-note/medical-operation-notes/${ev.id}`);
		});
	}
</script>
...
<main class="mx-16 px-4 py-6">
	<div class="mb-6 flex flex-wrap items-end justify-between gap-4">
		...
		<a href="/medical-operation-note/medical-operation-notes/new" class="button" data-variant="primary">New note</a>
	</div>
