# Medical Records Release Permission — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Medical Records Release Permission form, built with axum + Loco +
...
- [`medical_records_release_permission/`](./medical_records_release_permission/) — the Loco crate: `src/medical_records_release_permission/` holds `app.rs`
...
`src/medical_records_release_permission/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd medical_records_release_permission && cargo check --all-targets && cargo test
```
