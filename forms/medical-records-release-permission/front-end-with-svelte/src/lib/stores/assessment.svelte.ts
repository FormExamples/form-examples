...
function storageKey(id: string): string {
	return `medical-records-release-permission.front-end-with-svelte.${id || 'new'}.v1`;
}

/** A blank Medical Records Release Permission form with all fields at their unanswered defaults. */
...
/**
 * Svelte 5 reactive store for the Medical Records Release Permission form, with
 * localStorage persistence so an in-progress form survives a page reload.
 * Drafts are keyed by record id so each record edits independently.
 */
