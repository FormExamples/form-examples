...
import type { FluoroscopyResult, GradingResult } from '$lib/engine/types';
import {
...
/** Builds the pdfmake document definition for the fluoroscopy radiology report. */
export function buildPdfDocument(
	data: FluoroscopyResult,
	...
	return {
		...
		header: {
			text: 'FLUOROSCOPY RADIOLOGY REPORT',
			alignment: 'center',
			margin: [0, 20, 0, 0],
			fontSize: 10,
			color: '#6b7280',
		...
		content: [
			...
			sectionHeader('Examination'),
			{
				ul: [
					...
					`Screening time: ${data.screeningTimeMinutes !== null ? `${data.screeningTimeMinutes} min` : 'N/A'}`
				],
				margin: [0, 0, 0, 16] as Margin
			},
...
function structuredFindingList(data: FluoroscopyResult): string[] {
	const list: string[] = [];
