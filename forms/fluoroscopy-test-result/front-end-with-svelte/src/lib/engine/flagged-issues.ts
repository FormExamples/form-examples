import type { FluoroscopyResult, Flag, FlagPriority } from './types';
import { hasCriticalFinding } from './utils';

/**
 * Detects safety-critical flags independently of the four axes. Flag
 * categories mirror sql/07_create_table_fluoroscopy_test_result_grade_flag.sql.
 * Flags are returned sorted high → medium → low priority.
 */
export function detectFlags(r: FluoroscopyResult): Flag[] {
	const flags: Flag[] = [];
	...
	if (hasCriticalFinding(r)) {
		flags.push({
			...
			priority: 'high',
			description: 'A critical finding (perforation / leak or obstruction) is present.',
			suggestedAction:
	...
	if (r.fistula && !hasCriticalFinding(r)) {
		flags.push({
			flagId: 'F-ABNORMAL-ACTION-001',
			category: 'abnormal-requiring-action',
			priority: 'high',
			description: 'A fistulous tract requiring timely action is present.',
			suggestedAction: 'Ensure the referrer is alerted and a clear action plan is documented.'
		});
	}

	// ─── urgent-referral ───
	if (r.stricture || r.fillingDefect) {
		flags.push({
			flagId: 'F-URGENT-REFERRAL-001',
			category: 'urgent-referral',
			priority: 'medium',
			description: 'A stricture or filling defect is present and may warrant urgent referral.',
	...
	// ─── missing-measurement ───
	if (r.screeningTimeMinutes === null) {
		flags.push({
			flagId: 'F-MISSING-MEASUREMENT-001',
			category: 'missing-measurement',
			priority: 'low',
			description: 'No fluoroscopy screening time was recorded for dose audit.',
			suggestedAction: 'Record the total fluoroscopy screening time in minutes for IR(ME)R dose audit.'
		});
	}

	// ─── unexpected-finding (abnormal but no originating request linked) ───
	if (
		(r.stricture || r.fillingDefect || r.fistula) &&
