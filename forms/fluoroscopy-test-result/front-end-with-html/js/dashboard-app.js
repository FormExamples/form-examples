...
import { abnormalitySeverityClass, abnormalitySeverityLabel, followUpUrgencyClass, followUpUrgencyLabel, reportStatusLabel, resultClassificationClass, resultClassificationLabel, studyTypeLabel } from './types.js';

// Fluoroscopy Test Result — clinician dashboard (vanilla classic-script app).
//
...
// box + study-type dropdown + classification dropdown + follow-up-urgency
// dropdown). Columns mirror the SvelteKit dashboard route
// (`front-end-with-svelte/src/routes/+page.svelte`).
//
// Sibling modules loaded as plain `<script>` tags (in dependency order) attach
// their exports to `window.FluoroscopyTestResultDashboard`; display labels come
// from the form namespace `window.FluoroscopyTestResult` (js/types.js). The
...
function matchesFilters(row) {
  ...
  if (filters.studyType && row.studyType !== filters.studyType) return false;
...
function compareRows(a, b) {
  ...
  // Default: string compare (id, patientName, studyType, reportStatus,
...
function renderTableBody() {
  ...
  for (const row of rows) {
    ...
    tr.innerHTML = `
      ...
      <td>${esc(studyTypeLabel(row.studyType))}</td>
...
function bindFilterInputs() {
  ...
  const studyType = document.getElementById('filter-study-type');
  ...
  if (studyType) {
    studyType.addEventListener('change', () => {
      filters.studyType = studyType.value;
      renderAll();
    });
  }
  if (classification) {
