# Sports Medicine Assessment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Sports Medicine Assessment form, built with axum + Loco +
...
- [`sports_medicine_assessment/`](./sports_medicine_assessment/) — the Loco crate: `src/sports_medicine_assessment/` holds `app.rs`
...
`src/sports_medicine_assessment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd sports_medicine_assessment && cargo check --all-targets && cargo test
```
