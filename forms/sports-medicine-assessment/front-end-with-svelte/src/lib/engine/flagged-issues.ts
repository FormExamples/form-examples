...
// computes), this module raises clinician-facing flags grouped by priority:
//
//   high   - cardiovascular red flags, recent or multiple concussions, RED-S
//            triad indicators, infectious skin lesions in contact sport,
//            monocular athlete in high-risk sport without eyewear
//   medium - uncorrected MSK injury, monocular athlete (general), suspected
//            eating disorder, hypertension diagnosis, previous clearance issue
...
/** Detect additional clinician-facing flags, sorted high → medium → low. */
export function detectAdditionalFlags(data: AssessmentData): AdditionalFlag[] {
	...
	if (isYes(data.familyHistory.suddenCardiacDeathUnder50)) {
		flags.push({
			id: 'FLAG-CV-003',
			category: 'Cardiovascular',
			message: `Family sudden cardiac death under 50${
	...
	if (isYes(data.neurologicalConcussionBaseline.concussionLastSixMonths)) {
		flags.push({
			...
			message:
				'Concussion within the last 6 months — confirm symptom resolution and graduated return.',
			priority: 'high'
		});
	}
	if (
		...
		flags.push({
			id: 'FLAG-NEURO-002',
			category: 'Neurological',
			message: `${data.neurologicalConcussionBaseline.totalConcussions} lifetime concussions — neurology review for return-to-play.`,
			priority: 'high'
		});
	}
	...
	if (
		...
		flags.push({
			id: 'FLAG-REDS-001',
			category: 'RED-S',
			message: `Probable RED-S triad: BMI ${bmi}, amenorrhoea > 6 months, prior stress fracture — urgent multidisciplinary review.`,
			priority: 'high'
		});
	} else if (
		reds.applicable &&
	...
	if (isYes(data.musculoskeletalScreening.uncorrectedMajorInjury)) {
		flags.push({
			...
			message: `Uncorrected major injury${
	...
	if (isYes(data.visionSkin.monocularAthlete) && !isHigh) {
		flags.push({
			id: 'FLAG-VIS-002',
			category: 'Vision',
	...
	if (
		...
		flags.push({
			...
			category: 'Nutrition',
			message:
				'Suspected eating disorder / restrictive eating — refer to sports nutrition / mental health.',
			priority: 'medium'
		});
	}
	...
	if (isYes(data.sportPositionDetails.previousClearanceIssue)) {
		flags.push({
			...
			message: `Previous clearance issue${
	...
	if (
		...
		flags.push({
			...
			message: `${data.sportPositionDetails.hoursPerWeek} h / week training — monitor for overtraining.`,
			priority: 'low'
		});
	}
	if (lowBmi) {
	...
	if (isYes(data.medicalHistory.allergiesKnown) && !data.medicalHistory.allergyDetails.trim()) {
		flags.push({
			id: 'FLAG-ALG-001',
			category: 'Allergies',
