import type { AssessmentData, PPERule } from './types';
...
//
// The grader (`gradePPE`) then aggregates the highest-grade rule that fired
// into a single clearance level. Rule IDs follow PPE-NNN ordering by section.

const isYes = (v: string) => v === 'yes';
const isHighContact = (d: AssessmentData) => d.sportPositionDetails.contactLevel === 'high';
const isContactSport = (d: AssessmentData) =>
	d.sportPositionDetails.contactLevel === 'high' ||
	d.sportPositionDetails.contactLevel === 'moderate';

export const ppeRules: PPERule[] = [
	// ─── Medical history ───────────────────────────────────────
	{
		id: 'PPE-001',
	...
	// ─── RED-S / menstrual ────────────────────────────────────
	{
		...
		grade: 3,
		fires: (d) =>
			d.menstrualHistoryREDS.applicable && isYes(d.menstrualHistoryREDS.amenorrhoeaSixMonths)
	},
	{
		id: 'PPE-021',
		category: 'RED-S',
		...
		grade: 3,
		fires: (d) =>
			d.menstrualHistoryREDS.applicable && isYes(d.menstrualHistoryREDS.restrictiveEatingPattern)
	},
	{
		id: 'PPE-022',
		category: 'RED-S',
		...
		grade: 2,
		fires: (d) =>
			d.menstrualHistoryREDS.applicable && isYes(d.menstrualHistoryREDS.stressFractureHistory)
	},
	{
		id: 'PPE-023',
		category: 'RED-S',
	...
	},
	{
		...
		grade: 2,
		fires: (d) => isYes(d.visionSkin.monocularAthlete) && !isHighContact(d)
	},
	{
		id: 'PPE-070',
		category: 'Skin',
