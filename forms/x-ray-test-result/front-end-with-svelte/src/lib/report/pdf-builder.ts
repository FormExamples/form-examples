...
import type { XRayResult, GradingResult } from '#lib/engine/types.js';
import {
...
/** Builds the pdfmake document definition for the X-ray radiology report. */
		...
		content: [
			...
			sectionHeader('Examination'),
			{
				ul: [
					...
					`Projections / technique: ${data.projections || 'N/A'}`
				],
				margin: [0, 0, 0, 16] as Margin
			},
...
function structuredFindingList(data: XRayResult): string[] {
	const list: string[] = [];
