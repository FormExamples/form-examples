// X-Ray Test Result — clinician dashboard (vanilla classic-script app).
//
...
// box + body-region dropdown + classification dropdown + follow-up-urgency
// dropdown). Columns mirror the SvelteKit `ReportRow` shape
// (`front-end-with-svelte/src/lib/engine/types.ts`).
//
// Sibling modules loaded as plain `<script>` tags (in dependency order) attach
// their exports to `window.XRayTestResultDashboard`; display labels come
// from the form namespace `window.XRayTestResult` (js/types.js). The
...
const {
  fetchReports,
  sampleReports
} = window.XRayTestResultDashboard;
const {
  bodyRegionLabel,
  reportStatusLabel,
...
function matchesFilters(row) {
  ...
  if (filters.bodyRegion && row.bodyRegion !== filters.bodyRegion) return false;
...
function compareRows(a, b) {
  ...
  // Default: string compare (id, patientName, bodyRegion, reportStatus,
...
function renderTableBody() {
  ...
  for (const row of rows) {
    ...
    tr.innerHTML = `
      ...
      <td>${esc(bodyRegionLabel(row.bodyRegion))}</td>
...
function bindFilterInputs() {
  ...
  const bodyRegion = document.getElementById('filter-body-region');
  ...
  if (bodyRegion) {
    bodyRegion.addEventListener('change', () => {
      filters.bodyRegion = bodyRegion.value;
      renderAll();
    });
  }
  if (classification) {
  ...
  if (clearBtn) {
    clearBtn.addEventListener('click', () => {
      ...
      if (bodyRegion) bodyRegion.value = '';
