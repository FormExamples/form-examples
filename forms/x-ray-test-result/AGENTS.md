# X-Ray Test Result — agent instructions

Plain-radiograph (X-ray) result (report). A reporting clinician completes a
...
This is the **result/report counterpart** to `x-ray-test-request` (a referral).
It mirrors the `ct-scan-test-result` gold template.

## Result semantics

A result form records *what the test found and what it means* — it is **not** a
referral. The source-of-truth record is `x_ray_test_result`; the clinician is the
report **author/signer** (radiologist, reporting radiographer, consultant,
...
- **Axis B — severity & structured reporting:** abnormalitySeverity
  ...
  (e.g. normal / abnormal-acute / abnormal-chronic).
- **Axis C — report completeness:** 0–100 % of mandatory report sections
  (history, projections/technique, comparison, findings, impression).
- **Axis D — follow-up urgency:** routine / recommended / urgent / critical-alert,
  with a target timeframe and a recommended action.

A **critical finding** (pneumothorax, free intraperitoneal air, unstable
fracture) **auto-escalates** Axis D to *critical-alert* and raises the
