# X-Ray Test Result — living spec

Living domain spec for the plain-radiograph (X-ray) result (report) form. The
...
| `x_ray_test_result` | Main result/report record (source of truth). |
| `x_ray_test_result_grade` | Computed four-axis interpretation grade (1:1 with result). |
| `x_ray_test_result_grade_rule` | Audit trail of fired scoring rules. |
| `x_ray_test_result_grade_flag` | Safety-critical flags. |

## Result record (`x_ray_test_result`)
...
- **Examination:** `body_region` (chest / abdomen / spine-cervical /
  spine-thoracic / spine-lumbar / pelvis / hip / knee / ankle-foot / shoulder /
  wrist-hand / skull / dental / other), `laterality` (left / right / bilateral /
  not-applicable), `projections`, `examination_adequacy` (adequate / limited /
  non-diagnostic).
...
- **Conclusion:** `impression` (≤2000), `reporting_category` (free-text label),
  `recommended_follow_up`.
- **Critical communication:** `critical_result_communicated`, `reported_to`.

## Grade contract (`x_ray_test_result_grade`)
...
If the result describes a critical finding (pneumothorax, free intraperitoneal
air, or an unstable fracture), Axis D **must** be `critical-alert` and a
`critical-result-alert` flag **must** be present, irrespective of the other axes.

## Rules and flags

- `x_ray_test_result_grade_rule`: one row per fired rule with `rule_id`, `axis`
  (classification / severity / completeness / follow-up), `category`,
  `description`. Rule IDs are stable and shared across implementations.
- `x_ray_test_result_grade_flag`: `flag_id`, `category`
