import type { AssessmentData, FlaggedIssue, GfrCategory, KdigoRiskZone, ReviewStatus } from './types';
import { present } from './utils';
import { isRapidDecline } from './ckd-review-rules';

/**
 * Detect the clinician-facing flags raised by the review findings. Computed
 * INDEPENDENTLY of the review-completeness status (which the grader produces),
 * each flag carries a priority (spec §5). Categories mirror the
 * `chronic_kidney_disease_review_grade_flag` SQL CHECK constraint:
 * very-high-risk-referral, egfr-referral, acr-referral, rapid-decline,
 * hyperkalaemia, anaemia, uncontrolled-bp, nephrotoxic-drug, missing-acr,
 * incomplete, other.
 *
 ...
 *   - Rapid eGFR decline (high)                   — decline rule in rules.ts
...
export function detectFlaggedIssues(
	...
	if (g.gfrCategory === 'G4' || g.gfrCategory === 'G5') {
		flags.push({
			...
			priority: 'high',
			description: `eGFR ${present(renal.egfr) ? renal.egfr : ''} corresponds to ${g.gfrCategory} (eGFR < 30 mL/min/1.73 m²)`,
			suggestedAction: 'Refer to nephrology (eGFR < 30) unless already under specialist care.'
		});
	}
	...
	if (present(alb.acr) && alb.acr >= 70) {
		flags.push({
			...
			suggestedAction: 'Refer to nephrology for severe albuminuria (ACR ≥ 70) per NICE NG203.'
		});
	}
	...
	if (isRapidDecline(data)) {
		flags.push({
			...
			priority: 'high',
			description: `Sustained eGFR fall from ${renal.previousEgfr} to ${renal.egfr} mL/min/1.73 m²`,
			suggestedAction:
	...
	if (present(bloods.potassium) && bloods.potassium >= 6.0) {
		...
		flags.push({
			...
			suggestedAction: 'Recheck potassium and review ACEi/ARB dosing and dietary potassium.'
		});
	}
	...
	// ─── Nephrotoxic drug without dose adjustment (HIGH) ───────
	if (med.nephrotoxicDrugPresent === 'yes' && med.nephrotoxicDoseAdjusted !== 'yes') {
		flags.push({
