# Heart Health Check — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Heart Health Check form, built with axum + Loco +
...
- [`heart_health_check/`](./heart_health_check/) — the Loco crate: `src/heart_health_check/` holds `app.rs`
...
`src/heart_health_check/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd heart_health_check && cargo check --all-targets && cargo test
```
