...
import { createDefaultAssessmentData } from '#lib/engine/utils.js';

/** localStorage draft key for a given assessment id (defaults to `new`). */
function storageKey(id: string): string {
	return `heart-health-check.front-end-with-svelte.${id || 'new'}.v1`;
}

/** A blank heart health check with all fields at their unanswered defaults. */
export function createDefaultAssessment(): AssessmentData {
	return createDefaultAssessmentData();
}

/**
 * Svelte 5 reactive store for the heart health check, with localStorage
...
class AssessmentStore {
	...
	currentStep = $state(1);
	/** Submit-time validation errors keyed by input id (for per-field display). */
	 ...
	 * (e.g. `const d = assessment.data.bloodPressure`) stay bound to live state.
	 */
