# Heart Health Check — specification
...
**Inputs.** A typed assessment object whose shape mirrors the SQL schema in `sql-migrations/` (21 migration files). Unanswered text and enum fields default to `''`; unanswered numeric, date, and time fields default to `null`.
...
- `bin/test-form heart-health-check` exits cleanly.
...
- `bin/lily-html-refactor --check heart-health-check` reports no drift.
- LocalStorage keys preserve draft state across reloads:
  - `heart-health-check.front-end-form-with-html.v1` (HTML)
  - `heart-health-check.front-end-form-with-svelte.v1` (SvelteKit)
