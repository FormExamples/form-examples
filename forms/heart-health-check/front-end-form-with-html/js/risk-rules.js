...
// `src/lib/engine/risk-rules.ts` from the SvelteKit reference.
//
//   HHC-001 to HHC-005 — high-risk findings
//   HHC-006 to HHC-015 — medium-risk findings
//   HHC-016 to HHC-020 — low-risk / positive lifestyle indicators
...
/**
 * Run all rules against the supplied data and return only those that fire.
 * @param {AssessmentData} data
 * @param {number} tenYearRisk
 * @param {number | null} heartAge
 * @returns {FiredRule[]}
 */
function evaluateRules(data, tenYearRisk, heartAge) {
  /** @type {FiredRule[]} */
  const fired = [];
  for (const rule of allRules()) {
    try {
      ...
      console.warn(`HHC rule ${rule.id} evaluation failed:`, e);
    }
  }
  return fired;
}
