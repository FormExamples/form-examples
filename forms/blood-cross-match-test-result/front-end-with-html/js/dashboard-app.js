...
// box + request-type dropdown + classification dropdown + follow-up-urgency
...
const filters = {
  search: '',
  requestType: '',     // '' | 'group-and-save' | 'crossmatch' | 'antibody-screen' | 'emergency-issue'
...
function compareRows(a, b) {
  ...
  // Default: string compare (id, patientName, requestType, reportStatus, reportedDate)
...
function renderTableBody() {
  ...
  for (const row of rows) {
    ...
    if (row.resultClassification === 'critical' || row.followUpUrgency === 'critical-alert') {
    ...
    tr.innerHTML = `
      ...
      <td>${esc(requestTypeLabel(row.requestType))}</td>
...
function bindFilterInputs() {
  ...
  if (requestType) {
    requestType.addEventListener('change', () => {
      filters.requestType = requestType.value;
      renderAll();
    });
  }
  if (classification) {
