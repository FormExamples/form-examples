# Blood Cross-Match Test Result — PostgreSQL migrations (source of truth)
...
4. `03_create_table_clinician.sql` — reporting clinician (biomedical scientist / transfusion practitioner / consultant haematologist).
5. `04_create_table_blood_cross_match_test_result.sql` — main result/report record.
6. `05_create_table_blood_cross_match_test_result_grade.sql` — four-axis interpretation grade.
7. `06_create_table_blood_cross_match_test_result_grade_rule.sql` — fired-rule audit trail.
8. `07_create_table_blood_cross_match_test_result_grade_flag.sql` — safety-critical flags.
