...
<Fieldset legend="7. Interpretation & Sign-off">
	...
	{#if preview.followUpUrgency === 'critical-alert'}
		<Alert type="error" heading="Critical-result alert">
			<p>
				This report contains a critical result (incompatible crossmatch, clinically-significant
				antibodies, ABO discrepancy, or unmet two-sample rule). Communicate the result directly to
				the requester and record it below before signing.
			</p>
		</Alert>
	{/if}
	...
	<Field label="Critical-result communication">
		<CheckboxGroup label="Critical-result communication">
			<label>
				...
				/> Critical / urgent result communicated to requester
			</label>
		</CheckboxGroup>
	</Field>
