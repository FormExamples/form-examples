...
import type { BloodCrossMatchResult, GradingResult } from '$lib/engine/types';
import {
...
/** Builds the pdfmake document definition for the blood cross-match compatibility report. */
export function buildPdfDocument(
	data: BloodCrossMatchResult,
	...
	return {
		...
		header: {
			text: 'BLOOD CROSS-MATCH COMPATIBILITY REPORT',
			alignment: 'center',
			margin: [0, 20, 0, 0],
			fontSize: 10,
			color: '#6b7280',
		...
		footer: (currentPage: number, pageCount: number) => ({
			text: `Page ${currentPage} of ${pageCount} | Generated ${new Date(result.gradedAt).toLocaleString()} | BSH / SHOT / NICE NG24`,
			alignment: 'center',
			margin: [0, 20, 0, 0],
			fontSize: 8,
			color: '#9ca3af'
		...
		content: [
			{
				text: 'BLOOD CROSS-MATCH TEST RESULT',
				fontSize: 18,
				bold: true,
				alignment: 'center',
			...
			{
				text: `${requestTypeLabel(data.requestType)} — ${reportStatusLabel(data.reportStatus)}`,
				fontSize: 12,
				alignment: 'center',
				color: '#4b5563',
				margin: [0, 0, 0, 4] as Margin
			...
			sectionHeader('Grouping, screen, and crossmatch'),
			{
				ul: [
					`ABO / RhD group: ${bloodGroupLabel(data.aboGroup, data.rhdGroup)}`,
					`Historical group concordant: ${data.historicalGroupConcordant ? 'Yes' : 'No'}`,
					`Antibody screen: ${data.antibodyScreenResult || 'N/A'}`,
					`Antibodies identified: ${data.antibodiesIdentified || 'None'}`,
					`Crossmatch: ${crossmatchResultLabel(data.crossmatchResult)}`,
					`Component: ${componentLabel(data.component)}`,
					`Units crossmatched: ${data.unitsCrossmatched ?? 'N/A'}`,
					`Units available: ${data.unitsAvailable ?? 'N/A'}`,
					`Special requirements: ${data.specialRequirements || 'None'}`,
					`Two-sample rule met: ${data.twoSampleRuleMet ? 'Yes' : 'No'}`
				],
				margin: [0, 0, 0, 16] as Margin
			},
...
function structuredFindingList(data: BloodCrossMatchResult): string[] {
	const list: string[] = [];
	if (data.crossmatchResult === 'incompatible') list.push('Incompatible crossmatch');
	if (data.antibodyScreenResult === 'positive')
		list.push('Clinically-significant antibodies (positive screen)');
	if (data.aboGroup !== '' && !data.historicalGroupConcordant)
		list.push('ABO discrepancy (historical-group non-concordance)');
	if (!data.twoSampleRuleMet && (data.aboGroup !== '' || data.crossmatchResult !== ''))
		list.push('Two-sample group-check rule not met');
	if (
