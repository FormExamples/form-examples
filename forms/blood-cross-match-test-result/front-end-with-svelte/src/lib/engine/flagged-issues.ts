import type { BloodCrossMatchResult, Flag, FlagPriority } from './types';
import {
	hasCriticalResult,
	isAboDiscrepancy,
 ...
 */
export function detectFlags(r: BloodCrossMatchResult): Flag[] {
	const flags: Flag[] = [];
	...
	if (r.crossmatchResult === 'incompatible' && !isAboDiscrepancy(r) && !isTwoSampleRuleUnmet(r)) {
		flags.push({
			...
			suggestedAction: 'Investigate the incompatibility and arrange antigen-negative / serologically compatible units.'
		});
	}
	...
	if (r.antibodyScreenResult === 'positive') {
		flags.push({
			...
			priority: 'medium',
			description: 'Antibody screen positive; clinically-significant antibodies may require reference-laboratory work-up.',
			suggestedAction: 'Identify the antibody specificity and select antigen-negative units; refer to the reference laboratory if needed.'
		});
	}
	...
	if (
		...
		flags.push({
			...
			priority: 'medium',
			description: 'A crossmatch / compatibility test was not performed for a crossmatch or emergency-issue request.',
			suggestedAction: 'Complete the appropriate compatibility testing or document the emergency-issue justification.'
		});
	}
	...
	if (insufficientUnits(r)) {
		...
		flags.push({
			...
			category: 'missing-measurement',
			priority: 'low',
			description: 'A compatible result is recorded but the number of available units was not entered.',
