...
// list from the backend; on any failure (or empty response) we fall back to
// sample data and show a small banner. The rendered table is sortable (click
// any column header) and filterable (search box + body-region dropdown +
// triage-tier dropdown + appropriateness-band dropdown + suitability dropdown
...
function matchesFilters(row) {
  ...
  if (filters.region && row.bodyRegion !== filters.region) return false;
...
function renderTableBody() {
  ...
  for (const row of rows) {
    ...
    tr.innerHTML = `
      ...
      <td class="data-table-td"><span class="suit-badge ${suitabilityClass(row.suitabilityBand)}">${esc(titleCase(row.suitabilityBand))}</span></td>
