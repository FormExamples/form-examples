<script lang="ts">
	...
	const columns = [
		...
		},
		{
			...
			header: 'Suitability',
			width: 120,
			template: (v: string) => suitabilityLabel(v as never)
		},
	...
	function init(api: any) {
		...
		api.on('select-row', (ev: { id?: string | number }) => {
			if (ev?.id != null) goto(`/ultrasound-test-request/ultrasound-test-requests/${ev.id}`);
		});
	}
</script>
...
<main class="mx-16 px-4 py-6">
	<div class="mb-6 flex flex-wrap items-end justify-between gap-4">
		<div>
			<h1 class="text-2xl font-bold text-base-content">Ultrasound vetting dashboard</h1>
			<p class="mt-1 text-sm text-base-content/70">
				Four-axis vetting grades for incoming ultrasound requests, computed by the shared engine.
				Select a row to open the request.
			</p>
		</div>
		<a href="/ultrasound-test-request/ultrasound-test-requests/new" class="button" data-variant="primary">New request</a>
	</div>
	...
	<div class="mb-4 flex flex-wrap gap-4">
		...
		<label class="text-sm">
			...
			<select class="select inline-block w-auto" bind:value={recommendationFilter}>
				...
				<option value="redirect">Redirect / amend preparation</option>
