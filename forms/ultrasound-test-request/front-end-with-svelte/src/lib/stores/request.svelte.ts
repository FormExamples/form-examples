...
import type { GradingResult, UltrasoundRequest } from '$lib/engine/types';
import { createDefaultRequest } from '$lib/engine/defaults';

// Re-export so callers can build a blank request from the store module.
export { createDefaultRequest };

/** localStorage draft key for a given request id (defaults to `new`). */
function storageKey(id: string): string {
	return `ultrasound-test-request.front-end-with-svelte.${id || 'new'}.v1`;
}

/**
 * Svelte 5 reactive store for the ultrasound request, with localStorage
