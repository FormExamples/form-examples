...
impl MigrationTrait for Migration {
    async fn up(&self, m: &SchemaManager) -> Result<(), DbErr> {
        create_table(m, "ultrasound_test_request_grades",
            &[
            ...
            ("suitability_band", ColType::StringWithDefault(String::new())),
            ("prep_requirements", ColType::TextWithDefault(String::new())),
            ...
            &[
            ("ultrasound_test_request", ""),
            ]
        ).await
    }

    async fn down(&self, m: &SchemaManager) -> Result<(), DbErr> {
        drop_table(m, "ultrasound_test_request_grades").await
    }
}
