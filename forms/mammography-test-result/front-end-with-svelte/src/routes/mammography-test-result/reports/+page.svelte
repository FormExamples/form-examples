...
<main class="mx-16 px-4 py-6">
	<div class="mb-6 flex flex-wrap items-end justify-between gap-4">
		<div>
			<h1 class="text-2xl font-bold text-gray-900">Graded mammography reports</h1>
			<p class="mt-1 text-sm text-gray-600">
				Four-axis interpretation grades for completed mammography reports, centred on the ACR
				BI-RADS final assessment category.
			</p>
		</div>
		<a href="/mammography-test-result/report" class="button" data-variant="primary">New report</a>
	</div>
		...
		<table class="w-full text-left text-sm">
			<thead class="border-b border-gray-200 bg-gray-50 text-gray-600">
				<tr>
					...
					<th class="px-4 py-3 font-semibold">BI-RADS</th>
			...
			<tbody>
				{#each rows as r (r.id)}
					<tr class="border-b border-gray-100 last:border-0 hover:bg-gray-50">
						...
						<td class="px-4 py-3 text-gray-700">{examTypeLabel(r.examType)}</td>
						<td class="px-4 py-3 text-gray-700">{lateralityLabel(r.laterality)}</td>
						<td class="px-4 py-3">
							<Badge label={biRadsShortLabel(r.biRadsCategory)} color={biRadsColor(r.biRadsCategory)} />
						</td>
