<script lang="ts">
	...
	import Step5Measurements from '#lib/components/steps/Step5Measurements.svelte';
	...
	function validate(): boolean {
		...
		if (d.biRadsCategory === '') {
			found.push({ id: 'biRadsCategory', message: 'BI-RADS final assessment category is required.' });
		}
		errors = found;
	...
	function submit() {
		...
		goto('/mammography-test-result/report/view');
	}
...
<main class="mx-16 px-4 py-6">
	<h1 class="text-2xl font-bold text-gray-900">New mammography report</h1>
	<p class="mt-1 text-sm text-gray-600">
		Complete the seven sections; the four-axis interpretation grade is computed on submit from the
	...
	<Form label="Mammography test result" onsubmit={submit}>
