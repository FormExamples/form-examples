import type { MammographyResult, GradingResult, Recommendation, FiredRule } from './types';
...
/**
 * Pure four-axis interpretation engine for a mammography result.
 *
 * Computes:
 * - Axis A: result classification (normal / abnormal / critical / inconclusive).
 * - Axis B: abnormality severity + structured reporting category (carries the
 *   ACR BI-RADS final assessment category).
 ...
 *
 * Invariant: a BI-RADS 4 or 5 final assessment auto-escalates Axis D (4c/5 to
 * critical-alert, 4a/4b to urgent) and raises the abnormal-requiring-action /
 * urgent-referral flags, regardless of the other axes. The least-urgent band is
 * only chosen when no rule fires.
 *
 * No side effects, no network calls, no I/O.
 */
export function calculateGrade(result: MammographyResult): GradingResult {
...
function deriveRecommendation(
	...
	if (urgency === 'urgent') return 'specialist-referral';
