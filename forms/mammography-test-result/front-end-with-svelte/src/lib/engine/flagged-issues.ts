import type { MammographyResult, Flag, FlagPriority } from './types';
import { isBiRadsUrgent, isBiRadsCritical } from './utils';

/**
 * Detects safety-critical flags independently of the four axes. Flag
 * categories mirror sql/07_create_table_mammography_test_result_grade_flag.sql.
 * A BI-RADS 4 or 5 final assessment raises the abnormal-requiring-action and
 * urgent-referral flags. Flags are returned sorted high → medium → low.
 */
export function detectFlags(r: MammographyResult): Flag[] {
	const flags: Flag[] = [];
