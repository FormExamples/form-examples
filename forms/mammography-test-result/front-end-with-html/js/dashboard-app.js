...
import { abnormalitySeverityClass, abnormalitySeverityLabel, biRadsClass, biRadsShortLabel, examTypeLabel, followUpUrgencyClass, followUpUrgencyLabel, lateralityLabel, reportStatusLabel, resultClassificationClass, resultClassificationLabel } from './types.js';

// Mammography Test Result — clinician dashboard (vanilla classic-script app).
//
...
// box + exam-type dropdown + classification dropdown + follow-up-urgency
// dropdown). Columns mirror the SvelteKit dashboard's `ReportRow`
// (`front-end-with-svelte/src/lib/engine/types.ts`).
//
// Sibling modules loaded as plain `<script>` tags (in dependency order) attach
// their exports to `window.MammographyTestResultDashboard`; display labels come
// from the form namespace `window.MammographyTestResult` (js/types.js). The
...
// row-cell renderer below. Mirrors the SvelteKit dashboard `ReportRow`.
const columns = [
  { key: 'id',                        label: 'Report' },
  { key: 'patientName',               label: 'Patient' },
  { key: 'examType',                  label: 'Exam type' },
  { key: 'laterality',                label: 'Laterality' },
  { key: 'biRadsCategory',            label: 'BI-RADS' },
...
// BI-RADS categories in ascending assessment order.
const biRadsRank = {
  '0': 0,
  '1': 1,
  '2': 2,
...
function matchesFilters(row) {
  ...
  if (filters.examType && row.examType !== filters.examType) return false;
...
/**
 * Compare two rows for the active sort column. The graded-axis and BI-RADS
 * columns use their rank tables; the numeric columns compare numerically;
...
function compareRows(a, b) {
  ...
  // Default: string compare (id, patientName, examType, laterality,
  // reportStatus, reportedDate — ISO dates compare correctly as strings).
...
function renderTableBody() {
  ...
  for (const row of rows) {
    ...
    tr.innerHTML = `
      ...
      <td>${esc(examTypeLabel(row.examType))}</td>
      <td>${esc(lateralityLabel(row.laterality))}</td>
      <td>${badgeCell(biRadsClass(row.biRadsCategory), biRadsShortLabel(row.biRadsCategory))}</td>
...
function bindFilterInputs() {
  ...
  const examType = document.getElementById('filter-exam-type');
  ...
  if (examType) {
    examType.addEventListener('change', () => {
      filters.examType = examType.value;
      renderAll();
    });
  }
  if (classification) {
