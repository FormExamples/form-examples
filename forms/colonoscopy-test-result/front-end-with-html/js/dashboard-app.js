...
import { abnormalitySeverityClass, abnormalitySeverityLabel, followUpUrgencyClass, followUpUrgencyLabel, procedureLabel, reportStatusLabel, resultClassificationClass, resultClassificationLabel } from './types.js';

// Colonoscopy Test Result — clinician dashboard (vanilla classic-script app).
//
...
// box + procedure dropdown + classification dropdown + follow-up-urgency
// dropdown). Columns mirror the SvelteKit dashboard route
// (`front-end-with-svelte/src/routes/+page.svelte`).
//
// Sibling modules loaded as plain `<script>` tags (in dependency order) attach
// their exports to `window.ColonoscopyTestResultDashboard`; display labels come
// from the form namespace `window.ColonoscopyTestResult` (js/types.js). The
...
function matchesFilters(row) {
  ...
  if (filters.procedure && row.procedure !== filters.procedure) return false;
...
function compareRows(a, b) {
  ...
  // Default: string compare (id, patientName, procedure, reportStatus,
...
function renderTableBody() {
  ...
  for (const row of rows) {
    ...
    tr.innerHTML = `
      ...
      <td>${esc(procedureLabel(row.procedure))}</td>
...
function bindFilterInputs() {
  ...
  const procedure = document.getElementById('filter-procedure');
  ...
  if (procedure) {
    procedure.addEventListener('change', () => {
      filters.procedure = procedure.value;
      renderAll();
    });
  }
  if (classification) {
