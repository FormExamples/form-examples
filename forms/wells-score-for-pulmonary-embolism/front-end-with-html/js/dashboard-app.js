...
// Wells PE — clinician dashboard (vanilla classic-script app).
//
...
// box + care-setting dropdown + two-level-band dropdown + pathway dropdown).
//
// Sibling modules loaded as plain `<script>` tags (in dependency order) attach
// their exports to `window.WellsScoreForPulmonaryEmbolismDashboard`. The whole
...
const columns = [
  ...
  { key: 'twoLevelBand',        label: 'PE Likelihood' },
  { key: 'recommendedPathway',  label: 'Next Step' }
];
...
function matchesFilters(row) {
  ...
  if (filters.pathway && row.recommendedPathway !== filters.pathway) {
    return false;
  }
  return true;
}
...
function compareRows(a, b) {
  ...
  // (patientIdentifier, patientName, careSetting, recommendedPathway)
...
function renderTableBody() {
  ...
  for (const row of rows) {
    ...
    tr.innerHTML = `
      ...
      <td>${esc(pathwayLabel(row.recommendedPathway))}</td>
    `;
    body.appendChild(tr);
  }
}
