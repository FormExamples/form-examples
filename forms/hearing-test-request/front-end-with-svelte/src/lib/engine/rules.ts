...
//
// Ported verbatim (preserving every rule ID) from the HTML front-end's
// `js/rules.js`: (A) appropriateness 1–9 + band by indication × test type,
// anchored on British Society of Audiology / NICE NG98 indication match;
// (B) urgency triage tier routine / urgent / emergency with red-flag
// auto-escalation; (C) request completeness over mandatory fields; (D) clinical
// priority low / moderate / high composite of acuity and appropriateness.
//
// Rule IDs are stable and identical across every front-end and the back-end
// (R-APPROP-*, R-URGENCY-*, R-COMPLETE-*, R-PRIORITY-*). The grader composes
...
export const INDICATION_TEST_MAP: Record<string, { ideal: string[]; plausible: string[] }> = {
	'hearing-loss': {
		ideal: ['pure-tone-audiometry'],
		plausible: ['speech-audiometry', 'tympanometry', 'otoacoustic-emissions']
	},
	tinnitus: {
		ideal: ['pure-tone-audiometry'],
		plausible: ['tympanometry', 'speech-audiometry']
	},
	vertigo: {
		ideal: ['pure-tone-audiometry', 'auditory-brainstem-response'],
		plausible: ['tympanometry']
	},
	'ear-discharge': { ideal: ['tympanometry'], plausible: ['pure-tone-audiometry'] },
	'suspected-otosclerosis': {
		ideal: ['tympanometry', 'pure-tone-audiometry'],
		plausible: ['speech-audiometry']
	},
	'occupational-noise': {
		ideal: ['pure-tone-audiometry'],
		plausible: ['otoacoustic-emissions']
	},
	'ototoxic-monitoring': {
		ideal: ['otoacoustic-emissions', 'pure-tone-audiometry'],
		plausible: ['auditory-brainstem-response']
	},
	'developmental-delay-child': {
		ideal: ['auditory-brainstem-response', 'otoacoustic-emissions'],
		plausible: ['newborn-hearing-screen', 'tympanometry']
	},
	'hearing-aid-review': {
		ideal: ['hearing-aid-assessment'],
		plausible: ['pure-tone-audiometry', 'speech-audiometry']
	},
	'sudden-hearing-loss': {
		ideal: ['pure-tone-audiometry'],
		plausible: ['auditory-brainstem-response', 'otoacoustic-emissions']
	},
...
export function isUnilateral(d: HearingRequest): boolean {
...
interface TriageRule {
	ruleId: string;
	tier: TriageTier;
	fires: (d: HearingRequest) => boolean;
	description: string;
}
...
const TRIAGE_RULES: TriageRule[] = [
	{
		...
		description:
			'Sudden sensorineural hearing loss within the past 30 days — otological emergency; be seen within 24 hours.'
	},
	{
		...
		fires: (d) => d.symptoms.suddenOnset === true && d.symptoms.onsetWithinDays !== 'within-30-days',
		description:
			'Sudden onset of hearing loss — refer urgently (within days / 2 weeks) per NICE QS185.'
	},
	{
...
export function scoreTriage(data: HearingRequest): {
	tier: TriageTier;
...
interface CompletenessField {
	weight: number;
	present: (d: HearingRequest) => boolean;
	ruleId: string;
	label: string;
}
...
const COMPLETENESS_FIELDS: CompletenessField[] = [
	...
	{
		weight: 2,
		present: (d) => !!d.request.testType,
		ruleId: 'R-COMPLETE-TEST-TYPE',
		label: 'requested test type'
	},
	{
		weight: 1,
		present: (d) => !!d.request.laterality,
		ruleId: 'R-COMPLETE-LATERALITY',
		label: 'laterality'
	},
	{
		weight: 1,
...
export function scoreCompleteness(data: HearingRequest): {
	percent: number;
	missing: FiredRule[];
} {
	let totalWeight = 0;
...
): { band: PriorityBand; firedRule: FiredRule } {
