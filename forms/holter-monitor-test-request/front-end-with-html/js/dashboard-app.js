...
// (search box + monitor-type dropdown + triage-tier dropdown +
// appropriateness-band dropdown + clinical-priority dropdown + safety-flags
// dropdown).
//
...
const columns = [
  ...
  { key: 'monitorType',         label: 'Monitor Type' },
  { key: 'indication',          label: 'Indication' },
  { key: 'symptomFrequency',    label: 'Frequency' },
...
function matchesFilters(row) {
  if (filters.search) {
    ...
    const matches =
      ...
      String(row.monitorType || '').toLowerCase().includes(term) ||
      String(row.clinician || '').toLowerCase().includes(term);
    if (!matches) return false;
  }
  if (filters.monitor && row.monitorType !== filters.monitor) return false;
...
function compareRows(a, b) {
  ...
  // Default: string compare (nhs, patient, monitorType, indication,
  // symptomFrequency, clinician, referralDate).
...
function renderTableBody() {
  ...
  for (const row of rows) {
    ...
    tr.innerHTML = `
      ...
      <td class="data-table-td">${esc(titleCase(row.monitorType))}</td>
      <td class="data-table-td">${esc(titleCase(row.indication))}</td>
      <td class="data-table-td">${esc(titleCase(row.symptomFrequency) || '—')}</td>
...
function bindFilterInputs() {
  ...
  const monitor = document.getElementById('filter-monitor');
