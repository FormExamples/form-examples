...
import type { HolterRequest, GradingResult } from '#lib/engine/types.js';
...
function storageKey(id: string): string {
	return `holter-monitor-test-request.front-end-with-svelte.${id || 'new'}.v1`;
}

/**
 * Svelte 5 reactive store for the Holter monitor test request, with
 ...
 */
class RequestStore {
	data = $state<HolterRequest>(createDefaultRequest());
	 ...
	 */
	loadForId(id: string, seed?: HolterRequest) {
		const key = id || 'new';
		this.id = key;
		this.result = null;
		this.currentStep = 1;

		let draft: HolterRequest | null = null;
		if (browser) {
			const raw = localStorage.getItem(storageKey(key));
			if (raw) {
				try {
					draft = JSON.parse(raw) as HolterRequest;
				} catch {
