import type { HolterRequest, FiredRule } from './types';
...
interface FieldCheck {
	weight: number;
	present: (d: HolterRequest) => boolean;
	ruleId: string;
	label: string;
}
...
const COMPLETENESS_FIELDS: FieldCheck[] = [
	...
	{
		weight: 2,
		present: (d) => !!d.request.monitorType,
		ruleId: 'R-COMPLETE-MONITOR-TYPE',
		label: 'monitor type'
	},
	{
		weight: 2,
		present: (d) => !!d.symptoms.symptomFrequency,
		ruleId: 'R-COMPLETE-SYMPTOM-FREQUENCY',
...
/**
 * Axis C — compute weighted completeness (0–100, rounded) and the
 * missing-field audit rules.
 */
export function scoreCompleteness(data: HolterRequest): {
	percent: number;
