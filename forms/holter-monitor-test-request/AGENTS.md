# Holter Monitor Test Request — agent instructions

Ambulatory ECG (Holter) monitoring request (referral). A clinician completes a
single-page wizard; the engine computes a **four-axis grading**
(appropriateness, urgency/triage, request completeness, clinical priority) plus
...
back-end. Red-flags (syncope, suspected VT, post-stroke AF detection)
auto-escalate the triage tier. Choose the least-alarming band only when no rule
fires.
