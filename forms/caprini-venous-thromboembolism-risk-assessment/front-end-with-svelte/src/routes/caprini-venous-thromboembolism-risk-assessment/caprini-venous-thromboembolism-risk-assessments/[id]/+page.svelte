<script lang="ts">
	...
	function submit() {
		...
		assessment.result = calculateCapriniGrade(assessment.data);
		goto(`/caprini-venous-thromboembolism-risk-assessment/${plural}/${id}/report`);
	}
...
<main class="mx-auto max-w-3xl px-4 py-6">
	<header class="mb-6 no-print">
		<h1 class="text-2xl font-bold text-base-content">
			{isNew ? 'New Caprini VTE assessment' : `Caprini VTE assessment ${id}`}
		</h1>
		<p class="mt-1 text-sm text-base-content/70">
			Complete the eight sections; the Caprini score and risk band are computed on submit.
		</p>
	...
	<Form label="Caprini VTE assessment" onsubmit={submit}>
		<Step1Context />
		<Step2Identification />
		<Step3OnePointFactors />
		<Step4TwoPointFactors />
		<Step5ThreePointFactors />
		<Step6FivePointFactors />
		<Step7BleedingRisk />
		<Step8Summary />
