import type { AssessmentData, CapriniRule, FactorSection } from './types';

/**
 * Declarative Caprini grading rules.
 *
 * The Caprini Risk Assessment Model scores a checklist of individual risk
 * factors, each worth a fixed weight of 1, 2, 3, or 5 points. Every factor
 * below is a yes/no item that fires (contributes its weight) when answered
 * 'yes'. Age contributes through a separate age band handled in
 * `caprini-grader.ts`. Each rule mirrors one row of the
...
type FactorDef = [FactorSection, string, 1 | 2 | 3 | 5, string, string];
...
const factorDefs: FactorDef[] = [
	...
	[
		...
		1,
		'respiratory',
		'Serious lung disease including pneumonia within the past month'
	],
	[
		'onePoint',
	...
	[
		...
		1,
		'hormonal',
		'Pregnancy or postpartum within the past month'
	],
	[
		'onePoint',
	...
	[
		...
		5,
		'neurological',
		'Acute spinal cord injury with paralysis within the past month'
	],
...
export const capriniRules: CapriniRule[] = factorDefs.map(
	([section, field, points, category, description]) => {
		const factor = toSnake(field);
		const weightGroup = `${points}-point` as CapriniRule['weightGroup'];
		...
		return {
			...
			evaluate: (d: AssessmentData) =>
				(d[section] as unknown as Record<string, unknown>)[field] === 'yes'
		};
	}
);
