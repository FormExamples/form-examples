...
// prior-VTE flag).
const THROMBOPHILIA_FIELDS: (keyof AssessmentData['threePoint'])[] = [
 ...
 * Caprini score (spec §5):
 *
 *   - High VTE risk (high)                  — capriniScore >= 5
 *   - Bleeding-risk contraindication (high) — highBleedingRisk == 'yes' with a
 *                                             moderate or high band
 *   - Prior VTE (medium)                    — historyOfVte == 'yes'
 *   - Known thrombophilia (medium)          — any 3-point thrombophilia factor fired
 *   - Incomplete assessment (low)           — any risk-factor / bleeding input unanswered
 *
 * Rows mirror the `caprini_venous_thromboembolism_risk_assessment_grade_flag`
...
export function detectFlaggedIssues(
	...
	if (capriniScore >= 5) {
		flags.push({
			...
			priority: 'high',
			description: `High VTE risk — Caprini score ${capriniScore} (5 or more)`,
			suggestedAction:
	...
	if (highBleeding && (riskBand === 'moderate' || riskBand === 'high')) {
		flags.push({
	...
	const thrombophilia = THROMBOPHILIA_FIELDS.filter((f) => data.threePoint[f] === 'yes');
	if (thrombophilia.length > 0) {
		flags.push({
			...
			suggestedAction: 'Consider haematology input on prophylaxis intensity and duration.'
		});
	}
	...
	for (const section of FACTOR_SECTIONS) {
		const obj = data[section] as unknown as Record<string, string>;
	...
	if (unanswered > 0) {
		flags.push({
			...
			suggestedAction: 'Complete the outstanding items and re-score.'
		});
	}
