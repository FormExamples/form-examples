<script lang="ts">
	...
	function init(api: any) {
		...
		api.on('select-row', (ev: { id?: string | number }) => {
			if (ev?.id != null) goto(`/neurodiversity-adjustment-response/neurodiversity-adjustment-responses/${ev.id}`);
		});
	}
</script>
...
<main class="mx-16 px-4 py-6">
	<div class="mb-6 flex flex-wrap items-end justify-between gap-4">
		...
		<a href="/neurodiversity-adjustment-response/neurodiversity-adjustment-responses/new" class="button" data-variant="primary"
			>New response</a
		>
	</div>
