...
use crate::engine::{sad_grader, types::AssessmentData};
...
fn section_to_json(section: &str) -> &str {
    match section {
        ...
        "seasonal_pattern_history" => "seasonalPatternHistory",
...
/// Parse a form value. Empty strings stay as `""`; numbers are converted to
/// JSON numbers so the typed engine deserializes them as `Option<i32>` or
/// `Option<f64>`.
...
/// Apply a "section.field" or "section.sub.field" dotted key to the data
/// blob, mapping section / sub-section / field names from snake_case to
/// camelCase.
fn apply_field(
    ...
    match parts.len() {
        ...
        3 => {
            let section = section_to_json(parts[0]);
            let sub = field_to_json(parts[1]);
            let field = field_to_json(parts[2]);
            if let Some(section_obj) = obj.get_mut(section).and_then(|s| s.as_object_mut()) {
                if let Some(sub_obj) = section_obj.get_mut(&sub).and_then(|s| s.as_object_mut()) {
...
async fn show_report(
    ...
    let grade = sad_grader::grade(&assessment_data);
    ...
    context.insert("spaq_score", &grade.spaq_score);
    context.insert("spaq_band", &grade.spaq_band);
    context.insert(
        "spaq_label",
        &crate::engine::utils::spaq_label(&grade.spaq_band),
    );
    context.insert("phq9_score", &grade.phq9_score);
    context.insert("phq9_band", &grade.phq9_band);
    context.insert(
        "phq9_label",
        &crate::engine::utils::phq9_label(&grade.phq9_band),
    );
    context.insert("combined_severity", &grade.combined_severity);
    context.insert(
        "severity_label",
        &crate::engine::utils::severity_label(&grade.combined_severity),
    );
