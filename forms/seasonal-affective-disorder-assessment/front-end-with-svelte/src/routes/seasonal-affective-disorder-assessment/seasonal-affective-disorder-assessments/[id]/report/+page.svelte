<script lang="ts">
	...
	import { calculateAge, combinedSeverityColor, spaqBandColor, phq9BandColor } from '$lib/engine/utils';
	...
	$effect(() => {
		if (!assessment.result) {
			goto(`/seasonal-affective-disorder-assessment/seasonal-affective-disorder-assessments/${id}`);
		}
	});
	...
	async function downloadPDF() {
		pdfError = '';
		try {
			const res = await fetch(`/seasonal-affective-disorder-assessments/${id}/report/pdf`, {
				method: 'POST',
			...
			if (res.ok) {
				...
				a.href = url;
				a.download = `seasonal-affective-disorder-assessment-${data.demographics.lastName || id}.pdf`;
				a.click();
...
{#if result}
	<header class="border-b border-base-300 bg-base-100 shadow-sm no-print">
		<div class="mx-auto flex max-w-4xl items-center justify-between px-4 py-4">
			<h1 class="text-lg font-bold text-base-content">SAD assessment report</h1>
			<div class="flex items-center gap-3">
				...
				<Button
					...
					onclick={() => goto(`/seasonal-affective-disorder-assessment/seasonal-affective-disorder-assessments/${id}`)}>Edit</Button
				>
			</div>
		</div>
	</header>
	...
	<main class="mx-auto max-w-4xl px-4 py-6">
		<!-- Combined severity banner -->
		<div class="mb-6 rounded-xl border-2 p-6 text-center {combinedSeverityColor(result.combinedSeverity)}">
			<div class="text-3xl font-bold">{combinedSeverityLabel(result.combinedSeverity)}</div>
			<div class="mt-3 flex flex-wrap justify-center gap-3 text-sm">
				<span class="rounded-full border px-3 py-1 {spaqBandColor(result.spaqBand)}">
					SPAQ GSS {result.spaqScore}/24 — {spaqBandLabel(result.spaqBand)}
				</span>
				<span class="rounded-full border px-3 py-1 {phq9BandColor(result.phq9Band)}">
					PHQ-9 {result.phq9Score}/27 — {phq9BandLabel(result.phq9Band)}
				</span>
			</div>
		...
		<div class="mb-6 rounded-xl border border-base-300 bg-base-100 p-6">
			...
			<div class="grid grid-cols-1 gap-4 text-sm sm:grid-cols-2">
				...
				<div><span class="font-medium text-base-content/70">Latitude:</span> {data.demographics.latitude || 'N/A'}</div>
			</div>
		</div>
	</main>
{/if}
