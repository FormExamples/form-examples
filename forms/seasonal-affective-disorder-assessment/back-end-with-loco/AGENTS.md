# Seasonal Affective Disorder Assessment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Seasonal Affective Disorder Assessment form, built with axum + Loco +
...
- [`seasonal_affective_disorder_assessment/`](./seasonal_affective_disorder_assessment/) — the Loco crate: `src/seasonal_affective_disorder_assessment/` holds `app.rs`
...
`src/seasonal_affective_disorder_assessment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd seasonal_affective_disorder_assessment && cargo check --all-targets && cargo test
```
