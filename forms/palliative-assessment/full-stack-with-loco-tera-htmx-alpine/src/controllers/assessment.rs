...
use crate::engine::{esas_grader, types::AssessmentData, utils::severity_band_label};
...
/// GET /assessment/{id} -- render the single-page palliative wizard.
#[debug_handler]
...
fn section_to_json(section: &str) -> &str {
    match section {
        ...
        "primary_diagnosis_prognosis" => "primaryDiagnosisPrognosis",
        "esasr_symptoms" => "esasrSymptoms",
        "performance_status" => "performanceStatus",
        "goals_of_care_acp" => "goalsOfCareACP",
        "medications_symptom_control" => "medicationsSymptomControl",
        "psychosocial_spiritual_concerns" => "psychosocialSpiritualConcerns",
        "carer_family_support" => "carerFamilySupport",
        "multidisciplinary_plan" => "multidisciplinaryPlan",
        other => other,
    }
}
...
/// Parse a form value. Empty strings stay as `""`; numbers parsed; numeric
/// inputs (e.g. ESAS 0-10) become JSON numbers so the typed engine
/// deserializes them as `Option<i32>`.
...
async fn submit_assessment(
    ...
    if let Some(obj) = data_value.as_object_mut() {
        if let Some(form_obj) = form_data.as_object() {
            // Collect dynamic medication rows: regular_medications[idx].field
            // and as_needed_medications[idx].field.
            let mut reg_meds: std::collections::BTreeMap<
                usize,
                serde_json::Map<String, serde_json::Value>,
            > = std::collections::BTreeMap::new();
            let mut prn_meds: std::collections::BTreeMap<
                usize,
                serde_json::Map<String, serde_json::Value>,
            > = std::collections::BTreeMap::new();

            for (key, value) in form_obj {
                if let Some(rest) = key.strip_prefix("regular_medications[") {
                    if let Some(bracket_end) = rest.find(']') {
                        if let Ok(idx) = rest[..bracket_end].parse::<usize>() {
                            if let Some(dot_pos) = rest.find("].") {
                                ...
                                reg_meds.entry(idx).or_default().insert(json_field, parsed);
                            }
                        }
                    }
                    continue;
                }
                if let Some(rest) = key.strip_prefix("as_needed_medications[") {
                    if let Some(bracket_end) = rest.find(']') {
                        if let Ok(idx) = rest[..bracket_end].parse::<usize>() {
                            if let Some(dot_pos) = rest.find("].") {
                                let field_name = &rest[dot_pos + 2..];
                                let json_field = field_to_json(field_name);
                                let parsed = parse_form_value(value);
                                prn_meds.entry(idx).or_default().insert(json_field, parsed);
                            }
                        }
                    }
                    continue;
            ...
            // Materialize medication arrays inside medicationsSymptomControl.
            let any_meds = !reg_meds.is_empty() || !prn_meds.is_empty();
            if any_meds {
                if let Some(meds_section) = obj
                    .get_mut("medicationsSymptomControl")
                    .and_then(|s| s.as_object_mut())
                {
                    if !reg_meds.is_empty() {
                        let items: Vec<serde_json::Value> = reg_meds
                            .values()
                            .filter(|entry| {
                                entry
                                    .get("name")
                            ...
                            .collect();
                        meds_section.insert(
                            "regularMedications".to_string(),
                            serde_json::Value::Array(items),
                        );
                    }
                    if !prn_meds.is_empty() {
                        let items: Vec<serde_json::Value> = prn_meds
                            .values()
                            .filter(|entry| {
                                entry
                                    .get("name")
...
async fn show_report(
    ...
    let grade = esas_grader::grade(&assessment_data);
    ...
    context.insert("esas_total", &grade.esas_total);
    context.insert("severity_band", &grade.severity_band);
    context.insert("severity_label", &severity_band_label(&grade.severity_band));
    context.insert("answered_count", &grade.answered_count);
    context.insert("fired_rules", &grade.fired_rules);
    context.insert("individual_flags", &grade.individual_flags);
