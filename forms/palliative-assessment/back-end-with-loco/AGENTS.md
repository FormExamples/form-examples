# Palliative Assessment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Palliative Assessment form, built with axum + Loco +
...
- [`palliative_assessment/`](./palliative_assessment/) — the Loco crate: `src/palliative_assessment/` holds `app.rs`
...
`src/palliative_assessment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd palliative_assessment && cargo check --all-targets && cargo test
```
