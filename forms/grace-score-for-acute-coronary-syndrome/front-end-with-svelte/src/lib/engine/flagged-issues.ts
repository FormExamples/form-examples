...
import { normaliseCreatinine } from './utils';

/**
 * Detect clinician-facing safety flags (red flags), independent of the GRACE
 * point total (spec §5):
 *
 ...
 *
 * Rows mirror the `grace_score_for_acute_coronary_syndrome_grade_flag` SQL
 * table (flag_id, category, priority, description, suggested_action).
 */
export function detectFlaggedIssues(data: AssessmentData, riskCategory: RiskBand): FlaggedIssue[] {
	...
	if (stDeviation === 'yes') {
		flags.push({
			...
			suggestedAction: 'Repeat the ECG, expedite review, and treat as high-risk ischaemia.'
		});
	}
	...
	if (missing.length > 0) {
		flags.push({
			...
			suggestedAction: 'Record the missing admission finding(s) and re-score.'
		});
	}
