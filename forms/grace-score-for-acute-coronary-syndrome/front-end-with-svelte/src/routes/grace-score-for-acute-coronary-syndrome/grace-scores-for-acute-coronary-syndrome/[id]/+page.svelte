<script lang="ts">
	...
	import Step3Haemodynamics from '$lib/components/steps/Step3Haemodynamics.svelte';
	import Step4Renal from '$lib/components/steps/Step4Renal.svelte';
	import Step5HeartFailure from '$lib/components/steps/Step5HeartFailure.svelte';
	import Step6HighRiskFeatures from '$lib/components/steps/Step6HighRiskFeatures.svelte';
	...
	function submit() {
		...
		assessment.result = calculateGraceGrade(assessment.data);
		goto(`/grace-score-for-acute-coronary-syndrome/grace-scores-for-acute-coronary-syndrome/${id}/report`);
	}
...
<main class="mx-auto max-w-3xl px-4 py-6">
	<header class="mb-6 no-print">
		<h1 class="text-2xl font-bold text-base-content">
			{isNew ? 'New GRACE assessment' : `GRACE assessment ${id}`}
		</h1>
		<p class="mt-1 text-sm text-base-content/70">
			Complete the seven sections; the GRACE points and risk category are computed on submit.
		</p>
	...
	<Form label="GRACE assessment" onsubmit={submit}>
		<Step1Context />
		<Step2Identification />
		<Step3Haemodynamics />
		<Step4Renal />
		<Step5HeartFailure />
		<Step6HighRiskFeatures />
		<Step7Summary />
