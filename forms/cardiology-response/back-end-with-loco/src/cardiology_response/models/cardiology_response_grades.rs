//! Cardiology-response-grades domain model: transactional persist of the grade.

use chrono::Utc;
use sea_orm::{entity::prelude::*, ActiveValue, TransactionTrait};

pub use super::_entities::cardiology_response_grades::{ActiveModel, Column, Entity, Model};
use super::_entities::{
    cardiology_response_grade_flags, cardiology_response_grade_rules,
};
use crate::engine::types::GradingResult;

/// Cardiology response grades entity alias.
pub type CardiologyResponseGrades = Entity;
...
impl Model {
    ...
    pub async fn persist_grade(
        ...
        Entity::delete_many()
            .filter(Column::CardiologyResponseId.eq(cardiology_response_id))
            .exec(&txn)
            .await?;
        ...
        let grade_row = ActiveModel {
            ...
            cardiology_response_id: ActiveValue::Set(cardiology_response_id),
            response_classification: ActiveValue::Set(grade.response_classification.clone()),
            severity: ActiveValue::Set(grade.severity.clone()),
            severity_category: ActiveValue::Set(grade.severity_category.clone()),
            ...
            recommended_action: ActiveValue::Set(grade.recommended_action.clone()),
        ...
        for rule in &grade.fired_rules {
            cardiology_response_grade_rules::ActiveModel {
                id: ActiveValue::NotSet,
                created_at: ActiveValue::Set(now.into()),
                updated_at: ActiveValue::Set(now.into()),
                deleted_at: ActiveValue::Set(None),
                cardiology_response_grade_id: ActiveValue::Set(grade_id),
        ...
        // One row per safety flag.
        for flag in &grade.flags {
            cardiology_response_grade_flags::ActiveModel {
                id: ActiveValue::NotSet,
                created_at: ActiveValue::Set(now.into()),
                updated_at: ActiveValue::Set(now.into()),
                deleted_at: ActiveValue::Set(None),
