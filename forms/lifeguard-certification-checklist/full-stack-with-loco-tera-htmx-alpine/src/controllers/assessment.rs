...
use crate::engine::{lifeguard_grader, types::AssessmentData, utils};
...
/// GET /assessment/{id} -- render the single-page lifeguard wizard.
#[debug_handler]
...
/// Numeric field paths (camelCase): empty string -> JSON null, numeric string -> JSON number.
...
/// Parse a form value string for the JSONB payload.
...
/// POST /assessment/{id}/submit -- merge form data into the JSONB blob, redirect to the report.
#[debug_handler]
    ...
    if let Some(obj) = data_value.as_object_mut() {
        if let Some(form_obj) = form_data.as_object() {
            for (key, value) in form_obj {
                // section.field -- the dotted name pattern.
...
async fn show_report(
    ...
    let grade = lifeguard_grader::grade(&assessment_data);
    ...
    context.insert("critical_failures", &grade.critical_failures);
    context.insert("deficiencies", &grade.deficiencies);
    ...
    context.insert("total_rules", &grade.total_rules);
