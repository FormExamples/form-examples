...
//
// Numeric measurements (50 m swim time, CPR rate/depth, AED time) are also
// surfaced as rules: the rule fires 'no' when a numeric value is recorded
// outside the NPLQ/AHA range, 'yes' when it's in range, and falls back to
// the explicit tri-state field otherwise.

/**
 * @typedef {import('./types.js').AssessmentData} AssessmentData
 * @typedef {import('./types.js').TriState} TriState
 *
 * @typedef {Object} LifeguardRule
 * @property {string} id
  ...
  // --- Step 7: CPR & AED --------------------------------------------------
  {
    ...
    evaluate: (d) => {
      ...
      if ((rate === null || rate === undefined) &&
          (depth === null || depth === undefined)) return '';
