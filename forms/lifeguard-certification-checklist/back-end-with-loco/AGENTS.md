# Lifeguard Certification Checklist — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Lifeguard Certification Checklist form, built with axum + Loco +
...
- [`lifeguard_certification_checklist/`](./lifeguard_certification_checklist/) — the Loco crate: `src/lifeguard_certification_checklist/` holds `app.rs`
...
`src/lifeguard_certification_checklist/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd lifeguard_certification_checklist && cargo check --all-targets && cargo test
```
