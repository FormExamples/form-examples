...
// Mammography Test Request — vetting dashboard (vanilla classic-script app).
//
...
const columns = [
  ...
  { key: 'twoWeekWaitEligible', label: '2WW' },
...
function matchesFilters(row) {
  if (filters.search) {
    ...
    const matches =
      ...
      String(row.examType || '').toLowerCase().includes(term) ||
      String(row.clinician || '').toLowerCase().includes(term);
    if (!matches) return false;
  }
  if (filters.exam && row.examType !== filters.exam) return false;
  ...
  if (filters.twoww === 'yes' && row.twoWeekWaitEligible !== true) return false;
  if (filters.twoww === 'no' && row.twoWeekWaitEligible === true) return false;
...
function compareRows(a, b) {
  ...
  // Default: string compare (nhs, patient, examType, indication, clinician,
  // referralDate).
...
function renderTwoWWCell(eligible) {
  return eligible
    ? '<span class="twoww-badge twoww-yes">Yes</span>'
    : '<span class="twoww-badge twoww-no">No</span>';
}
...
function renderTableBody() {
  ...
  for (const row of rows) {
    ...
    if (row.triageTier === 'emergency' || row.triageTier === 'two-week-wait') {
    ...
    tr.innerHTML = `
      ...
      <td class="data-table-td">${esc(titleCase(row.examType))}</td>
      ...
      <td class="data-table-td">${renderTwoWWCell(row.twoWeekWaitEligible)}</td>
...
function bindFilterInputs() {
  ...
  const exam = document.getElementById('filter-exam');
  ...
  const twoww = document.getElementById('filter-twoww');
  ...
  if (twoww) {
    twoww.addEventListener('change', () => {
      filters.twoww = twoww.value;
      renderAll();
    });
  }
  if (flags) {
