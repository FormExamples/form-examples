<script lang="ts">
	...
	import Step5BreastHistoryAndRisk from '$lib/components/steps/Step5BreastHistoryAndRisk.svelte';
	...
	function validate(): boolean {
		...
		if (d.request.examType === '') {
			found.push({ id: 'examType', message: 'Exam type is required.' });
		}
	...
	function submit() {
		...
		goto(`/mammography-test-request/mammography-test-requests/${id}/report`);
	}
...
<main class="mx-16 px-4 py-6">
	<h1 class="text-2xl font-bold text-base-content">
		{isNew ? 'New mammography request' : `Mammography request ${id}`}
	</h1>
	<p class="mt-1 text-sm text-base-content/70">
		Complete the seven sections; the four-axis grade and recommendation are computed on submit.
	</p>
	...
	<Form label="Mammography test request" onsubmit={submit}>
		<Step1RequestingClinician />
		<Step2PatientIdentification />
		<Step3RequestedExamination />
		<Step4Symptoms />
		<Step5BreastHistoryAndRisk />
		<Step6Triage />
