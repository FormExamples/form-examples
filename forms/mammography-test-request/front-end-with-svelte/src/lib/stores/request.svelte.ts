...
function storageKey(id: string): string {
	return `mammography-test-request.front-end-with-svelte.${id || 'new'}.v1`;
}

/**
 * Svelte 5 reactive store for the mammography request, with localStorage
 ...
 */
class RequestStore {
	data = $state<MammographyRequest>(createDefaultRequest());
	 ...
	 */
	loadForId(id: string, seed?: MammographyRequest) {
		const key = id || 'new';
		this.id = key;
		this.result = null;
		this.currentStep = 1;

		let draft: MammographyRequest | null = null;
		if (browser) {
			const raw = localStorage.getItem(storageKey(key));
			if (raw) {
				try {
					draft = JSON.parse(raw) as MammographyRequest;
				} catch {
