import type { AbeGroup, AssessmentData, FlaggedIssue, ReviewStatus } from './types';

/**
 * Detect the clinician-facing flags raised by the review findings. Computed
 * INDEPENDENTLY of the grades (which the grader produces), each flag carries a
 * priority (spec §5):
 *
 ...
 *
 * Rows mirror the `..._grade_flag` SQL table (flag_id, category, priority,
 * description, suggested_action).
 */
export function detectFlaggedIssues(
	data: AssessmentData,
	grade: { abeGroup: AbeGroup; reviewStatus: ReviewStatus }
): FlaggedIssue[] {
	...
	if (grade.abeGroup === 'E') {
		...
		flags.push({
			...
			priority: 'high',
			description: `High exacerbation risk (ABE group E${detail.length ? ': ' + detail.join(', ') : ''}) in the past 12 months`,
			suggestedAction:
	...
	if (vax.pneumococcalVaccine !== '' && vax.pneumococcalVaccine !== 'up-to-date')
		dueVax.push('pneumococcal');
	...
	if (mrc !== null && mrc >= 3 && rehab !== 'completed' && rehab !== 'referred') {
		flags.push({
			...
			priority: 'medium',
			description: `MRC dyspnoea grade ${mrc} (≥ 3) without a completed or in-progress pulmonary-rehabilitation referral`,
			suggestedAction:
