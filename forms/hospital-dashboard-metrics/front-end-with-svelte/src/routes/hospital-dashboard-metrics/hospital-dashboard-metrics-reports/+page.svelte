<script lang="ts">
	...
	// SVAR DataGrid columns. Metrics-recorded counts all render through the
	...
	const columns = [
		...
		{ id: 'reportedCount', header: 'Metrics recorded', width: 150, sort: true },
		{ id: 'totalCount', header: 'Total metrics', width: 120, sort: true }
	];
	...
	function init(api: any) {
		...
		api.on('select-row', (ev: { id?: string | number }) => {
			if (ev?.id != null)
				goto(`/hospital-dashboard-metrics/hospital-dashboard-metrics-reports/${ev.id}`);
		});
	}
</script>
	...
	<div class="mb-6 flex flex-wrap items-end justify-between gap-4">
		<div>
			...
			<p class="mt-1 text-sm text-base-content/70">
				Metrics recorded out of 67, for each reporting period, computed by the shared engine.
