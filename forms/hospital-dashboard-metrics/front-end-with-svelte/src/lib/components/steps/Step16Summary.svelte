<script lang="ts">
  ...
  const result = $derived(summariseMetrics(store.data));
</script>

<Fieldset legend="Step 16 — Summary &amp; sign-off">
  <p class="hint">
    A tally of metrics recorded, grouped by category, followed by overall
    notes and sign-off. This is a completeness tally, not a scored grading
    engine.
  </p>

  <div class="summary-box">
    <p><strong>Metrics recorded:</strong> {result.reportedCount} / {TOTAL_METRICS}</p>
  </div>

  <div class="category-tally">
    <h3>Recorded by category</h3>
    <ul>
