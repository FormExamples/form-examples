<script lang="ts">
	...
	function init(api: any) {
		...
		api.on('select-row', (ev: { id?: string | number }) => {
			if (ev?.id != null) goto(`/urology-waiting-list-card/urology-waiting-list-cards/${ev.id}`);
		});
	}
</script>
...
<main class="mx-16 px-4 py-6">
	<div class="mb-6 flex flex-wrap items-end justify-between gap-4">
		...
		<a href="/urology-waiting-list-card/urology-waiting-list-cards/new" class="button" data-variant="primary">New card</a>
	</div>
