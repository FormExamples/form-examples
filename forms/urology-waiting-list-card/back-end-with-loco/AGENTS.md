# Urology Waiting List Card — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Urology Waiting List Card form, built with axum + Loco +
...
- [`urology_waiting_list_card/`](./urology_waiting_list_card/) — the Loco crate: `src/urology_waiting_list_card/` holds `app.rs`
...
`/metrics`. The registered domain controllers are:
  ...
  - `urology_waiting_list_card`
  - `urology_waiting_list_card_appointment`
  - `urology_waiting_list_card_grade`
  - `urology_waiting_list_card_grade_flag`
  - `urology_waiting_list_card_grade_rule`
  - `patient`
  - `practitioner`

## Engine

`src/urology_waiting_list_card/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd urology_waiting_list_card && cargo check --all-targets && cargo test
```
