...
// NEWS2 — clinician dashboard (vanilla classic-script app).
//
...
const filters = {
  search: '',
  setting: '',   // '' | 'emergency-department' | 'acute-medical-unit' | 'ward' | 'pre-hospital' | 'other'
...
const columns = [
  ...
  { key: 'aggregateScore',      label: 'NEWS2' },
  { key: 'riskBand',            label: 'Risk Band' },
  { key: 'redScore',            label: 'Red Score' },
  { key: 'monitoringFrequency', label: 'Monitoring' }
];

// Rank used when sorting the riskBand column so severity orders correctly
...
function matchesFilters(row) {
  ...
  if (filters.red === 'yes' && !row.redScore) return false;
  if (filters.red === 'no' && row.redScore) return false;
  return true;
}

/**
 * Compare two rows for the active sort column. The risk-band column uses its
 * rank table; the nullable aggregate score sorts nulls last; the red-score
...
function renderTableBody() {
  ...
  for (const row of rows) {
    ...
    if (row.riskBand === 'high' || row.redScore) {
    ...
    tr.innerHTML = `
      ...
      <td>
        <span class="flag-badge ${row.redScore ? 'flag-yes' : 'flag-no'}">
          ${row.redScore ? 'Yes' : 'No'}
        </span>
      </td>
...
function bindFilterInputs() {
  ...
  const red = document.getElementById('filter-red');
