...
for the NEWS2 specification and scoring engine. Lily Svelte headless
conventions: [`../../AGENTS-front-end-svelte.md`](../../AGENTS-front-end-svelte.md).

## Layout

- `src/lib/engine/` — pure scoring engine (`types.ts`, `utils.ts`,
  `news2-rules.ts`, `news2-grader.ts`, `flagged-issues.ts`) + `news2-grader.test.ts`.
...
- `src/routes/national-early-warning-score-2s/` — RESTful routes:
...
(`front-end-with-html/js/{types,rules,grader,flags}.js`). Seven subscores
(respiration, SpO2 with Scale 1 / Scale 2 logic, air-or-oxygen +2 weighting,
systolic BP, pulse, ACVPU consciousness, temperature) sum to an aggregate; the
red-score rule (any single parameter = 3) escalates to at least low-medium; the
risk band is the worst of the aggregate band and the red-score band.
