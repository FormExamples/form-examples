...
export function detectFlaggedIssues(
	data: AssessmentData,
	grade: { subscores: Subscores; aggregate: number; redScore: boolean }
): FlaggedIssue[] {
	...
	// ─── Hypoxia (HIGH) — SpO2 below the selected scale's target ─
	if (spo2 !== null) {
		const hypoxic = scale === 'scale-2' ? spo2 <= 87 : spo2 <= 93;
		if (hypoxic) {
			flags.push({
				id: 'F-HYPOXIA-001',
				category: 'hypoxia',
	...
	if (onOxygen === 'oxygen') {
		flags.push({
			...
			priority: 'medium',
			description:
				'Patient receiving supplemental oxygen (scores 2 for the air-or-oxygen item).',
			suggestedAction:
