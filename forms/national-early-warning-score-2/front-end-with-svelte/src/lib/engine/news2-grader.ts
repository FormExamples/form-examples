...
// parameter via the band tables in `news2-rules.ts`, sum the aggregate, apply
// the red-score rule, and derive the clinical-risk band with the RCP-recommended
...
const BAND_SEVERITY: Record<RiskBand, number> = {
	low: 0,
	'low-medium': 1,
	medium: 2,
	high: 3
...
export function aggregateBand(aggregate: number): RiskBand {
...
/**
 * Monitoring frequency + recommendation for the final band. `aggregate` is used
 * only to distinguish the two low sub-cases (0 vs 1-4).
 */
function response(
	band: RiskBand,
	aggregate: number
): { monitoringFrequency: string; recommendation: string } {
	switch (band) {
		case 'high':
			return {
...
function collectFiredRules(
	...
	if (redScore) {
		fired.push({
			...
			description:
				'A single parameter scored 3 (red score) — escalate to at least low-medium.'
		});
	}
	fired.push({
		id: 'R-AGGREGATE-01',
...
export function gradeNews2(data: AssessmentData): GradingResult {
	...
	const redBand: RiskBand = redScore ? 'low-medium' : 'low';
	...
	const flaggedIssues = detectFlaggedIssues(data, { subscores, aggregate, redScore });
