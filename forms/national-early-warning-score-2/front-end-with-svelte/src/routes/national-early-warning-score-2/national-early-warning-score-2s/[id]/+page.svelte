<script lang="ts">
	...
	function validate(): boolean {
		...
		if (d.context.clinicianName.trim() === '') {
			found.push({ id: 'context-clinicianName', message: 'Recording clinician name is required.' });
		}
		if (d.context.clinicianRole === '') {
			found.push({ id: 'context-clinicianRole', message: 'Clinician role is required.' });
		}
		if (d.context.spo2Scale === '') {
			found.push({ id: 'context-spo2Scale', message: 'SpO2 scale in use is required.' });
		}
		if (d.identification.patientName.trim() === '') {
			found.push({ id: 'identification-patientName', message: 'Patient name is required.' });
		}
		errors = found;
	...
	function submit() {
		...
		assessment.result = gradeNews2(assessment.data);
		goto(`/national-early-warning-score-2/${plural}/${id}/report`);
	}
...
<main class="mx-16 px-4 py-6">
	<h1 class="text-2xl font-bold text-base-content">
		{isNew ? 'New NEWS2 assessment' : `NEWS2 assessment ${id}`}
	</h1>
	<p class="mt-1 text-sm text-base-content/70">
		Complete the ten sections; the aggregate NEWS2 score and risk band are computed on submit.
	</p>
	...
	<Form label="NEWS2 assessment" onsubmit={submit}>
		<Step1Context />
		<Step2Identification />
		<Step3Respiration />
		<Step4OxygenSaturation />
		<Step5OxygenSupport />
		<Step6BloodPressure />
		<Step7Pulse />
		<Step8Consciousness />
