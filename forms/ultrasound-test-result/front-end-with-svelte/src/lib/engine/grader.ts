import type { UltrasoundResult, GradingResult, Recommendation, FiredRule } from './types';
...
/**
 * Pure four-axis interpretation engine for a general (non-obstetric)
 * ultrasound result.
 *
 * Computes:
 ...
 *
 * Invariant: a critical finding (DVT present, ruptured / large AAA, sonographic
 * signs of testicular torsion) auto-escalates Axis D to critical-alert and
 * raises the critical-result-alert flag, regardless of the other axes. The
 * least-urgent band is only chosen when no rule fires.
 *
 * No side effects, no network calls, no I/O.
 */
export function calculateGrade(result: UltrasoundResult): GradingResult {
