...
// Blood Test Result — clinician dashboard (vanilla classic-script app).
//
...
// box + overall-status dropdown + classification dropdown + follow-up-urgency
// dropdown). Columns mirror the SvelteKit dashboard route
// (`front-end-with-svelte/src/routes/+page.svelte`).
//
// Sibling modules loaded as plain `<script>` tags (in dependency order) attach
// their exports to `window.BloodTestResultDashboard`; display labels come
// from the form namespace `window.BloodTestResult` (js/types.js). The
...
const filters = {
  ...
  overallStatus: '',   // '' | OverallResultStatus
...
const columns = [
  ...
  { key: 'overallResultStatus',       label: 'Overall status' },
...
function matchesFilters(row) {
  ...
  if (filters.overallStatus && row.overallResultStatus !== filters.overallStatus) {
    return false;
  }
...
function compareRows(a, b) {
  ...
  if (key === 'overallResultStatus') {
    av = overallStatusRank[av] ?? -1;
    bv = overallStatusRank[bv] ?? -1;
    return (av - bv) * dir;
  }
...
function bindFilterInputs() {
  ...
  const overallStatus = document.getElementById('filter-overall-status');
  ...
  if (overallStatus) {
    overallStatus.addEventListener('change', () => {
      filters.overallStatus = overallStatus.value;
      renderAll();
    });
  }
  if (classification) {
  ...
  if (clearBtn) {
    clearBtn.addEventListener('click', () => {
      ...
      if (overallStatus) overallStatus.value = '';
