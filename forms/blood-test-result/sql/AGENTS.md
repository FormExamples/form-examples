...
8. `07_create_table_blood_test_result_grade_flag.sql` — safety-critical flags.
...
date / time. Analyte result values are `NUMERIC` with sanity-bound `CHECK`
constraints and `NULL` when the analyte was not measured.
