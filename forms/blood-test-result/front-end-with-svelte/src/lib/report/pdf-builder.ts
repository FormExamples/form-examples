...
import type { BloodTestResult, GradingResult } from '#lib/engine/types.js';
import {
...
/** Builds the pdfmake document definition for the blood / pathology report. */
export function buildPdfDocument(
	...
	return {
		...
		footer: (currentPage: number, pageCount: number) => ({
			text: `Page ${currentPage} of ${pageCount} | Generated ${new Date(result.gradedAt).toLocaleString()} | RCPath G158 / Pathology Harmony`,
			alignment: 'center',
			margin: [0, 20, 0, 0],
			fontSize: 8,
			color: '#9ca3af'
		...
		content: [
			...
			sectionHeader('Interpretation summary'),
			{
				ul: [
					`Overall status: ${overallResultStatusLabel(data.overallResultStatus)}`,
					`Abnormal results present: ${data.abnormalResultsPresent ? 'Yes' : 'No'}`,
					`Critical value present: ${data.criticalValuePresent ? 'Yes' : 'No'}`,
					`Critical-value detail: ${data.criticalValueDetail || 'N/A'}`
				],
				margin: [0, 0, 0, 8] as Margin
			},
...
function resultValueList(data: BloodTestResult): string[] {
	const list = analyteValues(data)
		.filter((a) => a.value !== null)
		.map((a) => `${a.label}: ${a.value} ${a.units} (${a.panel})`);
	if (list.length === 0) list.push('No analyte result values recorded');
	return list;
}
