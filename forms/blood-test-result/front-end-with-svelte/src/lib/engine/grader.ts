import type { BloodTestResult, GradingResult, Recommendation, FiredRule } from './types';
...
/**
 * Pure four-axis interpretation engine for a blood / pathology test result.
 *
 * Computes:
 ...
 *
 * Invariant: a critical (panic) value auto-escalates Axis A to critical and
 * Axis D to critical-alert and raises the critical-result-alert flag, regardless
 * of the other axes. The least-urgent band is only chosen when no rule fires.
 *
 * No side effects, no network calls, no I/O.
 */
export function calculateGrade(result: BloodTestResult): GradingResult {
...
function deriveRecommendation(
	...
	if (classification === 'inconclusive') return 'repeat-test';
	if (severity === 'moderate') return 'repeat-test';
