...
//
// Independent of the CFS score (which the grader computes), this module
// raises clinician-facing flags for falls, delirium, polypharmacy,
...
function detectAdditionalFlags(data) {
  ...
  if (data.mobilityFalls.fallHistory === 'yes') {
    ...
    flags.push({
      id: 'FLAG-FALLS-001',
      category: 'Falls Risk',
      message: `Fall history reported (${fallCount === null || fallCount === undefined ? '?' : fallCount} falls in past year)`,
      priority: 'high'
    });
  }
  ...
  if (
    data.mobilityFalls.timedUpAndGo !== null &&
    data.mobilityFalls.timedUpAndGo >= 14
  ) {
    flags.push({
      id: 'FLAG-FALLS-003',
      category: 'Falls Risk',
  ...
  if (data.continenceSkin.pressureInjuryPresent === 'yes') {
    flags.push({
      ...
      priority:
        data.continenceSkin.pressureInjuryStage === '3' ||
