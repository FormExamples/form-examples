...
<Fieldset legend="Demographics">
	...
	<div class="field-grid field-grid-3">
		<Field label="Weight (kg)" required inputId="weight"><NumberInput id="weight" label="Weight" min={1} max={400} bind:value={d.weight} required /></Field>
		<Field label="Height (cm)" required inputId="height"><NumberInput id="height" label="Height" min={50} max={250} bind:value={d.height} required /></Field>
		<Field label="BMI" description="Auto-calculated">
			{#if d.bmi}<p class="bmi-value">{d.bmi} <span class="bmi-cat">({bmiCategory(d.bmi)})</span></p>{:else}<p class="bmi-value bmi-empty">—</p>{/if}
		</Field>
	</div>

	<Field label="Living Situation" required inputId="livingSituation">
		<Select id="livingSituation" label="Living Situation" required bind:value={d.livingSituation}>
			<option value="">-- Select --</option>
			<option value="independent">Independent at home</option>
...
<style>
	...
	.bmi-empty { color: var(--color-muted); }
</style>
