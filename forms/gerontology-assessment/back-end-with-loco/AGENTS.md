# Gerontology Assessment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Gerontology Assessment form, built with axum + Loco +
...
- [`gerontology_assessment/`](./gerontology_assessment/) — the Loco crate: `src/gerontology_assessment/` holds `app.rs`
...
`src/gerontology_assessment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd gerontology_assessment && cargo check --all-targets && cargo test
```
