<script lang="ts">
	...
	$effect(() => {
		if (!assessment.result) {
			goto(`/framingham-risk-score-for-hard-coronary-heart-disease/${plural}/${id}`);
		}
	});

	const bmi = $derived(
		data.lifestyleFactors.bmi ?? calculateBmi(data.demographics.heightCm, data.demographics.weightKg)
	);
	...
	async function downloadPDF() {
		...
		try {
			...
			if (res.ok) {
				...
				a.href = url;
				a.download = `framingham-risk-score-${data.patientInformation.fullName || id}.pdf`;
				a.click();
...
{#if result}
	<header class="border-b border-base-300 bg-base-100 shadow-sm no-print">
		<div class="mx-auto flex max-w-4xl items-center justify-between px-4 py-4">
			<h1 class="text-lg font-bold text-base-content">Framingham risk score report</h1>
			<div class="flex items-center gap-3">
				...
				<Button data-variant="secondary" onclick={() => goto(`/framingham-risk-score-for-hard-coronary-heart-disease/${plural}/${id}`)}>Edit</Button>
			</div>
		</div>
	</header>
	...
	<main class="mx-16 px-4 py-6">
		...
		<div class="mb-6 rounded-xl border-2 p-6 text-center {riskLevelColor(result.riskCategory)}">
			<div class="text-4xl font-bold">{result.tenYearRiskPercent.toFixed(1)} %</div>
			<div class="mt-2 text-lg font-semibold">{riskLevelLabel(result.riskCategory)}</div>
			<div class="mt-1 text-sm opacity-75">10-year risk of hard coronary heart disease</div>
					...
					<tbody>
						{#each result.firedRules as rule (rule.id)}
							<tr class="border-b border-base-200">
								...
								<td class="py-2">{riskLevelLabel(rule.riskLevel)}</td>
							</tr>
						{/each}
					</tbody>
				</table>
		...
		<div class="mb-6 rounded-xl border border-base-300 bg-base-100 p-6">
			...
			<div class="grid grid-cols-1 gap-4 text-sm sm:grid-cols-2">
				...
				<div>
					...
					{data.demographics.age ?? 'N/A'}
				</div>
				<div>
					<span class="font-medium text-base-content/70">Sex:</span>
					{data.demographics.sex || 'N/A'}
				</div>
				<div>
					<span class="font-medium text-base-content/70">Systolic BP:</span>
					{data.bloodPressure.systolicBp ?? 'N/A'}
					{data.bloodPressure.systolicBp !== null ? 'mmHg' : ''}
				</div>
				<div>
					<span class="font-medium text-base-content/70">Total / HDL cholesterol:</span>
					{data.cholesterol.totalCholesterol ?? 'N/A'} / {data.cholesterol.hdlCholesterol ?? 'N/A'}
					{data.cholesterol.cholesterolUnit === 'mmolL' ? 'mmol/L' : 'mg/dL'}
				</div>
				<div>
