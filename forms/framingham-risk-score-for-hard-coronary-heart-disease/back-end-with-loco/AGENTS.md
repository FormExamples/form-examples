# Framingham Risk Score for Hard Coronary Heart Disease — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Framingham Risk Score for Hard Coronary Heart Disease form, built with axum + Loco +
...
- [`framingham_risk_score_for_hard_coronary_heart_disease/`](./framingham_risk_score_for_hard_coronary_heart_disease/) — the Loco crate: `src/framingham_risk_score_for_hard_coronary_heart_disease/` holds `app.rs`
...
`src/framingham_risk_score_for_hard_coronary_heart_disease/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd framingham_risk_score_for_hard_coronary_heart_disease && cargo check --all-targets && cargo test
```
