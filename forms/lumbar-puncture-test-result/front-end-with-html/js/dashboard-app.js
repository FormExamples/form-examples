...
import { abnormalitySeverityClass, abnormalitySeverityLabel, followUpUrgencyClass, followUpUrgencyLabel, reportStatusLabel, reportingCategoryLabel, resultClassificationClass, resultClassificationLabel } from './types.js';

// Lumbar Puncture Test Result — clinician dashboard (vanilla classic-script app).
//
...
// box + pattern dropdown + classification dropdown + follow-up-urgency
// dropdown). Columns mirror the SvelteKit dashboard route
// (`front-end-with-svelte/src/routes/+page.svelte`).
//
// Sibling modules loaded as plain `<script>` tags (in dependency order) attach
// their exports to `window.LumbarPunctureTestResultDashboard`; display labels
// come from the form namespace `window.LumbarPunctureTestResult` (js/types.js).
...
const columns = [
  ...
  { key: 'reportingCategory',         label: 'Pattern' },
...
function matchesFilters(row) {
  ...
  if (filters.pattern && row.reportingCategory !== filters.pattern) return false;
...
function compareRows(a, b) {
  ...
  // Default: string compare (id, patientName, reportingCategory, reportStatus,
...
function renderTableBody() {
  ...
  for (const row of rows) {
    ...
    tr.innerHTML = `
      ...
      <td>${esc(reportingCategoryLabel(row.reportingCategory))}</td>
...
function bindFilterInputs() {
  ...
  const pattern = document.getElementById('filter-pattern');
