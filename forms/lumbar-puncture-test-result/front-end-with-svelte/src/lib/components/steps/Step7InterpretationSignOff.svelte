...
<Fieldset legend="7. Interpretation & Sign-off">
	...
	{#if preview.followUpUrgency === 'critical-alert'}
		<Alert type="error" heading="Critical-result alert">
			<p>
				This report contains a critical CSF result. Communicate the result directly to the
				requesting clinician and record it below before signing.
			</p>
		</Alert>
	{/if}
	...
	<div class="my-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
		...
		<div class="rounded-lg border border-gray-200 p-3">
			...
			{#if preview.reportingCategory}
				<span class="ml-2 text-xs text-gray-500">{reportingCategoryLabel(preview.reportingCategory)}</span>
			{/if}
		</div>
		<div class="rounded-lg border border-gray-200 p-3">
	...
	<Field label="Critical-result communication">
		<CheckboxGroup label="Critical-result communication">
			<label>
				...
				/> Critical / urgent result communicated to requesting clinician
			</label>
		</CheckboxGroup>
	</Field>
