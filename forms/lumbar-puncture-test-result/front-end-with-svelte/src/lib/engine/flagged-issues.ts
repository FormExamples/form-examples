import type { LumbarPunctureResult, Flag, FlagPriority } from './types';
import { hasCriticalFinding, culturePositive } from './utils';

/**
 * Detects safety-critical flags independently of the four axes. Flag
 * categories mirror
 * sql/07_create_table_lumbar_puncture_test_result_grade_flag.sql.
 * Flags are returned sorted high → medium → low priority.
 */
export function detectFlags(r: LumbarPunctureResult): Flag[] {
	const flags: Flag[] = [];
