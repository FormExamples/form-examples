...
// ROSIER — clinician dashboard (vanilla classic-script app).
//
...
// box + care-setting dropdown + score dropdown + band dropdown + activate
// dropdown).
//
// Sibling modules loaded as plain `<script>` tags (in dependency order) attach
// their exports to `window.RecognitionOfStrokeInTheEmergencyRoomDashboard`.
...
const filters = {
  search: '',
  setting: '',    // '' | 'emergency-department' | 'acute-medical' | 'other'
  score: '',      // '' | '-2' .. '5'
...
const columns = [
  ...
  { key: 'rosierScore',       label: 'ROSIER' },
  { key: 'band',              label: 'Band' },
  { key: 'activatePathway',   label: 'Activate' }
];

// Rank used when sorting the band column so 'stroke-unlikely' is always less
// than 'stroke-likely' regardless of locale.
const bandRank = {
  'stroke-unlikely': 0,
  'stroke-likely': 1
};
...
function bandClass(band) {
  if (!band) return '';
  return band === 'stroke-likely' ? 'risk-high' : 'risk-low';
}

function bandLabel(band) {
  if (band === 'stroke-likely') return 'Stroke likely';
  if (band === 'stroke-unlikely') return 'Stroke unlikely';
  return 'N/A';
}
...
function scoreLabel(score) {
  return (score === null || score === undefined) ? 'N/A' : signed(score);
}
...
function matchesFilters(row) {
  ...
  if (filters.score) {
    if (row.rosierScore === null || row.rosierScore !== Number(filters.score)) {
      return false;
    }
  }
  if (filters.band && row.band !== filters.band) return false;
  if (filters.activate === 'yes' && !row.activatePathway) return false;
  if (filters.activate === 'no' && row.activatePathway) return false;
  return true;
}

/**
 * Compare two rows for the active sort column. The band column uses its rank
 * table; the nullable ROSIER score sorts nulls last; the activate boolean
...
function renderTableBody() {
  ...
  for (const row of rows) {
    ...
    if (row.band === 'stroke-likely') {
      tr.classList.add('row-critical');
    }

    const scoreClassName = row.rosierScore === null
    ...
    tr.innerHTML = `
      ...
      <td><span class="${scoreClassName}">${esc(scoreLabel(row.rosierScore))}</span></td>
      <td><span class="risk-badge ${bandClass(row.band)}">${esc(bandLabel(row.band))}</span></td>
      <td>
        <span class="flag-badge ${row.activatePathway ? 'flag-yes' : 'flag-no'}">
          ${row.activatePathway ? 'Yes' : 'No'}
        </span>
      </td>
    `;
    body.appendChild(tr);
...
function bindFilterInputs() {
  ...
  const activate = document.getElementById('filter-activate');
  ...
  if (activate) {
    activate.addEventListener('change', () => {
      filters.activate = activate.value;
      renderAll();
    });
  }
  if (clearBtn) {
