import type { RosierRule } from './types';
 ...
 * defect). Each rule evaluates the patient data and returns true when its
 * criterion is positive (answered `yes`); the grader (`rosier-grader.ts`) sums
 * the signed points into the total ROSIER score (-2..+5) and derives the band.
 * Rows mirror the `recognition_of_stroke_in_the_emergency_room_grade_rule` SQL
 * table.
 */
export const rosierRules: RosierRule[] = [
