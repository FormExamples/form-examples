import type { AssessmentData, FlaggedIssue, Signs, Mimics } from './types';

const SIGN_FIELDS: [keyof Signs, string][] = [
...
const MIMIC_FIELDS: [keyof Mimics, string][] = [
	['lossOfConsciousness', 'loss of consciousness / syncope'],
	['seizureActivity', 'seizure activity']
];

/**
 * Detect clinician-facing safety flags (red flags), independent of the signed
 * ROSIER total (spec §5):
 *
 *   - Activate stroke pathway (high)       — rosierScore > 0
 *   - Hypoglycaemia mimic (high)           — bloodGlucose != null && < 3.5
 *   - Seizure / LOC caution (medium)       — either mimic present
 *   - Clinical suspicion override (medium) — rosierScore <= 0 but any focal sign present
 *   - Incomplete assessment (low)          — glucose missing or any criterion unanswered
 *
 * Rows mirror the `recognition_of_stroke_in_the_emergency_room_grade_flag` SQL
 * table (flag_id, category, priority, description, suggested_action).
 */
export function detectFlaggedIssues(data: AssessmentData, rosierScore: number): FlaggedIssue[] {
	...
	const signsPresent = SIGN_FIELDS.filter(([field]) => data.signs[field] === 'yes').map(
		([, label]) => label
	);
	...
	if (seizure === 'yes' || loc === 'yes') {
		const present = MIMIC_FIELDS.filter(([field]) => data.mimics[field] === 'yes').map(
			([, label]) => label
		);
		flags.push({
	...
	if (missing.length > 0) {
		flags.push({
			...
			suggestedAction: 'Record the missing blood glucose and / or criterion answer(s) and re-score.'
		});
	}
