# Bone Marrow Donation Assessment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Bone Marrow Donation Assessment form, built with axum + Loco +
...
- [`bone_marrow_donation_assessment/`](./bone_marrow_donation_assessment/) — the Loco crate: `src/bone_marrow_donation_assessment/` holds `app.rs`
...
`src/bone_marrow_donation_assessment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd bone_marrow_donation_assessment && cargo check --all-targets && cargo test
```
