...
/// POST /assessment/new — create a new draft assessment and redirect to the wizard.
#[debug_handler]
...
/// GET /assessment/{id} — render the single-page donor-assessment wizard.
#[debug_handler]
...
async fn submit_assessment(
    ...
    if let Some(obj) = data_value.as_object_mut() {
        if let Some(form_obj) = form_data.as_object() {
            ...
            // Recompute BMI from weight + height when both are present.
            if let Some(demo) = obj.get_mut("demographics").and_then(|s| s.as_object_mut()) {
                let w = demo.get("weight").and_then(|v| v.as_f64());
                let h = demo.get("height").and_then(|v| v.as_f64());
                let bmi = utils::calculate_bmi(w, h);
                demo.insert(
                    "bmi".to_string(),
                    match bmi {
...
/// GET /assessment/{id}/report — run the grading engine, persist the result,
...
async fn show_report(
    ...
    context.insert("eligibility", &grade.eligibility);
    context.insert("eligibility_label", &utils::eligibility_label(&grade.eligibility));
    context.insert("overall_risk", &grade.overall_risk);
    context.insert("risk_label", &utils::risk_level_label(&grade.overall_risk));
    context.insert("suggested_eligibility", &grade.suggested_eligibility);
    context.insert("hla_match_level", &grade.hla_match_level);
    context.insert("collection_method", &grade.collection_method);
