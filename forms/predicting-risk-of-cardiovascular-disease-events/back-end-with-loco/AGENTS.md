# Predicting Risk of Cardiovascular Disease Events (PREVENT) — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Predicting Risk of Cardiovascular Disease Events (PREVENT) form, built with axum + Loco +
...
- [`predicting_risk_of_cardiovascular_disease_events/`](./predicting_risk_of_cardiovascular_disease_events/) — the Loco crate: `src/predicting_risk_of_cardiovascular_disease_events/` holds `app.rs`
...
`src/predicting_risk_of_cardiovascular_disease_events/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd predicting_risk_of_cardiovascular_disease_events && cargo check --all-targets && cargo test
```
