# Perioperative Optimization — back-end-with-loco/

Rust axum + Loco JSON API. Crate source under `src/perioperative_optimization/`;
one migration and one entity per SQL table.
...
- Relational schema only: one table per entity. A single JSONB blob is wrong.
  In particular `perioperative_optimization_grade_domain` must stay a table —
  the per-domain statuses are the form's primary output, and a dashboard needs
  to query them.
...
- `seed()` in `app.rs` resolves fixtures from `CARGO_MANIFEST_DIR` because the
  ...
  `src/perioperative_optimization/fixtures/`; do not revert it to the scaffold's
  `base.join(...)` form.
