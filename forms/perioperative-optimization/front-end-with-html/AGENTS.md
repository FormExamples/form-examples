...
The scoring engine (`js/types.js`, `js/domain-rules.js`, `js/gating.js`, `js/composite-grader.js`, `js/flagged-issues.js`) is hand-curated: generators must not overwrite it. Thresholds and lead times live only in `js/domain-rules.js`; never inline them elsewhere, and change `../doc/optimization-domains.md` first.
