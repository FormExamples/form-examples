// Time-to-surgery gating: the computation that distinguishes this form from an
// ordinary pre-operative assessment.
//
// A finding on its own is a fact. A finding gated against the weeks remaining
// before surgery is a decision. See ../../doc/time-to-surgery-gating.md.
//
// Every function here is pure: both dates come from the recorded data, never
// from the system clock, so the same assessment always grades the same way.
 ...
 * @returns {number|null}
 */
function weeksBetween(assessmentDate, plannedSurgeryDate) {
 ...
 */
function recommendedEarliestSurgeryDate(domains, plannedSurgeryDate) {
...
export { weeksBetween, gateDomain, recommendedEarliestSurgeryDate, MILLISECONDS_PER_WEEK };
