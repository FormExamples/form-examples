# Nephrology Waiting List Card — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Nephrology Waiting List Card form, built with axum + Loco +
...
- [`nephrology_waiting_list_card/`](./nephrology_waiting_list_card/) — the Loco crate: `src/nephrology_waiting_list_card/` holds `app.rs`
...
`/metrics`. The registered domain controllers are:
  ...
  - `nephrology_waiting_list_card`
  - `nephrology_waiting_list_card_appointment`
  - `nephrology_waiting_list_card_grade`
  - `nephrology_waiting_list_card_grade_flag`
  - `nephrology_waiting_list_card_grade_rule`
  - `patient`
  - `practitioner`

## Engine

`src/nephrology_waiting_list_card/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd nephrology_waiting_list_card && cargo check --all-targets && cargo test
```
