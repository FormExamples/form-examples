<script lang="ts">
	...
	import { riskLevelLabel, riskLevelColor, calculateAge } from '#lib/engine/utils.js';
	...
	$effect(() => {
		if (!assessment.result) {
			goto(`/patient-intake/patient-intakes/${id}`);
		}
	});
	...
	async function downloadPDF() {
		pdfError = '';
		try {
			const res = await fetch(`/patient-intakes/${id}/report/pdf`, {
				method: 'POST',
			...
			if (res.ok) {
				...
				a.href = url;
				a.download = `patient-intake-${data.personalInformation.fullName.replace(/\s+/g, '-') || id}.pdf`;
				a.click();
...
{#if result}
	<header class="border-b border-base-300 bg-base-100 shadow-sm no-print">
		<div class="mx-auto flex max-w-4xl items-center justify-between px-4 py-4">
			<h1 class="text-lg font-bold text-base-content">Patient intake report</h1>
			<div class="flex items-center gap-3">
				...
				<Button data-variant="secondary" onclick={() => goto(`/patient-intake/patient-intakes/${id}`)}>Edit</Button>
			</div>
		</div>
	</header>
		...
		<div class="mb-6 rounded-xl border border-base-300 bg-base-100 p-6">
			...
			<div class="grid grid-cols-1 gap-4 text-sm sm:grid-cols-2">
				...
				<div>
					...
					{#if calculateAge(data.personalInformation.dateOfBirth)}(Age {calculateAge(data.personalInformation.dateOfBirth)}){/if}
				</div>
				<div>
					<span class="font-medium text-base-content/70">Sex:</span>
					{data.personalInformation.sex || 'N/A'}
				</div>
				<div>
				...
				<div class="sm:col-span-2">
					<span class="font-medium text-base-content/70">Reason for visit:</span>
					{data.reasonForVisit.primaryReason}
					({data.reasonForVisit.urgencyLevel || 'N/A'})
				</div>
			</div>
		</div>
