# Patient Intake — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Patient Intake form, built with axum + Loco +
...
- [`patient_intake/`](./patient_intake/) — the Loco crate: `src/patient_intake/` holds `app.rs`
...
`src/patient_intake/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd patient_intake && cargo check --all-targets && cargo test
```
