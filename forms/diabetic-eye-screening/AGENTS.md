# Diabetic Eye Screening record — Agent Instructions
...
- **Input shape:** `DiabeticEyeScreening` TypeScript type — grading context,
  patient identification, and a right-eye and left-eye grading block (R grade,
  M grade, P marker, U marker, visual acuity).
- **Output shape:**
  ```ts
  gradeDiabeticEyeScreening(data: DiabeticEyeScreening): {
