// Declarative diabetic-eye-screening classification rules.
//
// Unlike an additive score, diabetic eye screening is a *classification*
...
// top-to-bottom by the grader (`grader.js`); the first whose `evaluate` returns
// true wins. Rows here mirror the `diabetic_eye_screening_grade_rule` SQL table
// (rule_id, stage, category, description) plus the outcome fields the grader
// records on the `diabetic_eye_screening_grade` row.
 ...
 * @property {6 | 12 | 24 | null} intervalMonths
 ...
 * @returns {'R0' | 'R1' | 'R2' | 'R3S' | 'R3A'}
 */
function worstRetinopathy(d) {
  const grades = [];
  ...
  for (const g of grades) {
    if (RETINOPATHY_SEVERITY[g] > RETINOPATHY_SEVERITY[worst]) worst = g;
  }
  return worst;
}
 ...
 * @returns {'M0' | 'M1'}
 */
function worstMaculopathy(d) {
  return d.rightEye.maculopathy === 'M1' || d.leftEye.maculopathy === 'M1'
    ? 'M1'
    : 'M0';
}
...
const classificationRules = [
  ...
  // ─── 2. REFERABLE — MACULOPATHY OR STABLE PROLIFERATIVE ─────────
  {
    ...
    evaluate: (ctx) =>
      ctx.worstMaculopathy === 'M1' || ctx.worstRetinopathy === 'R3S'
  },
