...
for the HAS-BLED specification and scoring engine. Lily Svelte headless
conventions: [`../../AGENTS-front-end-svelte.md`](../../AGENTS-front-end-svelte.md).

## Layout

- `src/lib/engine/` — pure scoring engine (`types.ts`, `utils.ts`,
  `hasbled-rules.ts`, `hasbled-grader.ts`, `flagged-issues.ts`) +
  `hasbled-grader.test.ts`.
...
- `src/routes/has-bled-score-for-major-bleeding-risks/` — RESTful routes:
