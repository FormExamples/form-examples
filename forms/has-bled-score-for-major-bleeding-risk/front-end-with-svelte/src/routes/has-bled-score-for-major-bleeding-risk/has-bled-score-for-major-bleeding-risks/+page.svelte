<script lang="ts">
	...
	import { riskBandLabel, careSettingLabel, anticoagulationStatusLabel } from '$lib/engine/utils';
	...
	// SVAR DataGrid columns. HAS-BLED score, risk band, and high-bleeding-risk
	...
	const columns = [
		...
		{
			id: 'anticoagulationStatus',
			header: 'Anticoagulation',
			width: 180,
			sort: true,
			template: (v: string) => anticoagulationStatusLabel(v as never) || '—'
		},
		{ id: 'hasBledScore', header: 'HAS-BLED', width: 110, sort: true },
		{
			id: 'riskBand',
			header: 'Risk band',
			width: 210,
	...
	function init(api: any) {
		...
		api.on('select-row', (ev: { id?: string | number }) => {
			if (ev?.id != null) goto(`/has-bled-score-for-major-bleeding-risk/has-bled-score-for-major-bleeding-risks/${ev.id}`);
		});
	}
</script>
...
<main class="mx-16 px-4 py-6">
	<div class="mb-6 flex flex-wrap items-end justify-between gap-4">
		<div>
			<h1 class="text-2xl font-bold text-base-content">HAS-BLED clinician dashboard</h1>
			<p class="mt-1 text-sm text-base-content/70">
				HAS-BLED score, risk band, and high-bleeding-risk flag for assessed patients, computed by the
				shared engine. Select a row to open the assessment.
			</p>
		</div>
		<a href="/has-bled-score-for-major-bleeding-risk/has-bled-score-for-major-bleeding-risks/new" class="button" data-variant="primary"
			>New assessment</a
		>
	</div>
