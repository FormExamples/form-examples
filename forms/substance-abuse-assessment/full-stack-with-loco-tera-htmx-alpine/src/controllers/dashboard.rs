...
struct DashboardParams {
    ...
    audit_risk_category: Option<String>,
    dast_risk_category: Option<String>,
...
async fn dashboard(
    ...
    // Server-side search across patient name and primary substance.
    if let Some(ref term) = params.search {
        let term = term.to_lowercase();
        items.retain(|row| {
            row.first_name.to_lowercase().contains(&term)
                || row.last_name.to_lowercase().contains(&term)
                || row.primary_substance.to_lowercase().contains(&term)
        });
    }
    ...
    if let Some(ref audit) = params.audit_risk_category {
        if !audit.is_empty() {
            items.retain(|row| row.audit_risk_category == *audit);
        }
    }

    if let Some(ref dast) = params.dast_risk_category {
        if !dast.is_empty() {
            items.retain(|row| row.dast_risk_category == *dast);
        }
    }
    ...
    context.insert(
        "audit_risk_category",
        &params.audit_risk_category.unwrap_or_default(),
    );
    context.insert(
        "dast_risk_category",
        &params.dast_risk_category.unwrap_or_default(),
    );
