...
use crate::engine::{substance_grader, types::AssessmentData};
...
fn section_to_json(section: &str) -> &str {
    match section {
        "alcohol_use_audit" => "alcoholUseAudit",
        "drug_use_dast" => "drugUseDast",
        "substance_use_history" => "substanceUseHistory",
        "withdrawal_assessment" => "withdrawalAssessment",
        "mental_health_comorbidities" => "mentalHealthComorbidities",
        "physical_health_impact" => "physicalHealthImpact",
        "social_legal_impact" => "socialLegalImpact",
        "previous_treatment_history" => "previousTreatmentHistory",
        "treatment_planning_goals" => "treatmentPlanningGoals",
        other => other,
    }
}
...
async fn show_report(
    ...
    let grade = substance_grader::calculate_substance_grade(&assessment_data);
    ...
    context.insert("audit_score", &grade.audit_score);
    context.insert("audit_risk_category", &grade.audit_risk_category);
    context.insert(
        "audit_risk_label",
        &crate::engine::utils::audit_risk_label(&grade.audit_risk_category),
    );
    context.insert("dast_score", &grade.dast_score);
    context.insert("dast_risk_category", &grade.dast_risk_category);
    context.insert(
        "dast_risk_label",
        &crate::engine::utils::dast_risk_label(&grade.dast_risk_category),
    );
