...
export function buildPdfDocument(
	...
	return {
		...
		header: {
			text: 'SUBSTANCE ABUSE ASSESSMENT REPORT',
			alignment: 'center',
			margin: [0, 20, 0, 0],
			fontSize: 10,
			color: '#6b7280',
		...
		content: [
			...
			{
				text: `AUDIT ${result.auditScore}/40 — ${auditRiskLabel(result.auditRiskCategory)}`,
				fontSize: 12,
			...
			{
				text: `DAST-10 ${result.dastScore}/10 — ${dastRiskLabel(result.dastRiskCategory)}`,
				fontSize: 12,
				alignment: 'center' as const,
				color: '#4b5563',
			...
			sectionHeader('Patient Details'),
			{
				table: {
					...
					body: [
						...
						[
							field('Primary substance', data.substanceUseHistory.primarySubstance || 'N/A'),
							field('Current status', data.substanceUseHistory.currentUseStatus || 'N/A')
						]
					]
				},
