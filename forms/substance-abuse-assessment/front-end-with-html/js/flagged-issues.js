...
function detectAdditionalFlags(data) {
  ...
  if (data.withdrawalAssessment.currentlyInWithdrawal === 'yes') {
    flags.push({
      id: 'FLAG-WD-001',
      category: 'Withdrawal',
      message: 'Active withdrawal from ' +
        (data.withdrawalAssessment.withdrawalSubstance || 'substance not specified') +
  ...
  if (data.physicalHealthImpact.overdoseHistory === 'yes') {
    flags.push({
      id: 'FLAG-OD-001',
      category: 'Overdose',
      message: 'Previous overdose(s): ' +
        (data.physicalHealthImpact.overdoseCount == null ? '?' : data.physicalHealthImpact.overdoseCount) +
        ' episode(s) - naloxone provision recommended',
      priority: 'high'
    });
  }
  ...
  if (data.socialLegalImpact.domesticViolence === 'yes') {
    flags.push({
      ...
      message: 'Domestic violence: ' +
        (data.socialLegalImpact.domesticViolenceDetails || 'details not specified') +
  ...
  if (data.physicalHealthImpact.liverDisease === 'yes' &&
