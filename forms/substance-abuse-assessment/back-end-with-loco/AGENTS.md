# Substance Abuse Assessment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Substance Abuse Assessment form, built with axum + Loco +
...
- [`substance_abuse_assessment/`](./substance_abuse_assessment/) — the Loco crate: `src/substance_abuse_assessment/` holds `app.rs`
...
`src/substance_abuse_assessment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd substance_abuse_assessment && cargo check --all-targets && cargo test
```
