...
// MRI Scan Test Request — vetting dashboard (vanilla classic-script app).
//
...
// MRI-safety-band dropdown + safety-flags dropdown).
//
// Sibling modules loaded as plain `<script>` tags (in dependency order)
// attach their exports to `window.MriScanTestRequestDashboard`. The whole file
...
const columns = [
  ...
  { key: 'mriSafetyBand',      label: 'MRI Safety' },
...
function safetyClass(band) {
  return band ? 'safety-' + String(band) : '';
}
...
function matchesFilters(row) {
  ...
  if (filters.safety && row.mriSafetyBand !== filters.safety) return false;
...
function renderTableBody() {
  ...
  for (const row of rows) {
    ...
    if (row.triageTier === 'emergency' || row.mriSafetyBand === 'contraindicated') {
    ...
    tr.innerHTML = `
      ...
      <td class="data-table-td"><span class="safety-badge ${safetyClass(row.mriSafetyBand)}">${esc(titleCase(row.mriSafetyBand))}</span></td>
