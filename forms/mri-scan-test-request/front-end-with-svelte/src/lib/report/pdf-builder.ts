...
export function buildPdfDocument(data: MriRequest, result: GradingResult): TDocumentDefinitions {
	...
	return {
		...
		header: {
			text: 'MRI REQUEST — VETTING REPORT',
			alignment: 'center',
			margin: [0, 20, 0, 0],
			fontSize: 10,
			color: '#6b7280',
		...
		content: [
			...
			sectionHeader('Requested examination'),
			{
				table: {
					widths: ['*', '*'],
					body: [
						[field('Body region', bodyRegionLabel(data.request.bodyRegion) || 'N/A'), field('Indication', indicationLabel(data.request.primaryIndication) || 'N/A')],
						[field('Clinical question', data.request.clinicalQuestion || 'N/A'), field('Contrast', data.contrast.contrastRequired || 'N/A')]
					]
				},
			...
			sectionHeader('Four-axis grade'),
			{
				table: {
					widths: ['*', '*'],
					body: [
						[field('A · Appropriateness', `${result.appropriatenessScore}/9 — ${appropriatenessLabel(result.appropriatenessBand)}`), field('B · MRI safety', mriSafetyLabel(result.mriSafetyBand))],
						[field('B · Contrast / renal', contrastRenalLabel(result.contrastRenalFlag)), field('C · Completeness', `${result.completenessPercent}%`)],
