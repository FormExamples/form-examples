...
function storageKey(id: string): string {
	return `mri-scan-test-request.front-end-with-svelte.${id || 'new'}.v1`;
}

/** A blank MRI scan request with all fields at their unanswered defaults. */
export function createDefaultRequest(): MriRequest {
	return createDefault();
}

/**
 * Svelte 5 reactive store for the MRI scan request, with localStorage
 ...
 */
class RequestStore {
	data = $state<MriRequest>(createDefaultRequest());
	 ...
	 */
	loadForId(id: string, seed?: MriRequest) {
		const key = id || 'new';
		this.id = key;
		this.result = null;
		this.currentStep = 1;

		let draft: MriRequest | null = null;
		if (browser) {
			const raw = localStorage.getItem(storageKey(key));
			if (raw) {
				try {
					draft = JSON.parse(raw) as MriRequest;
				} catch {
