...
// ferromagnetic-implant, pacemaker-icd, orbital-foreign-body,
// gadolinium-renal-risk, pregnancy, claustrophobia, missing-safety-screen,
// missing-indication, missing-clinical-question, other. Flag IDs are stable
// and identical across every front-end and the back-end. Ported verbatim from
// the HTML front-end's flags.js.

import type { MriRequest, Flag, ContrastRenalFlag } from './types';

/** Optional engine context threaded into flag detection. */
export interface FlagContext {
	contrastRenalFlag?: ContrastRenalFlag;
}

/** Detect safety flags for an MRI scan request. */
export function detectFlags(data: MriRequest, context: FlagContext = {}): Flag[] {
	const flags: Flag[] = [];
	const ctx = context;
	...
	if (data.safety.pacemakerOrIcd === true) {
		flags.push({
			...
			suggestedAction:
				'Do not scan until device MR-conditional status and programming are confirmed by cardiology and MR physics.'
		});
	}
	...
	if (data.safety.aneurysmClip === true) {
		flags.push({
			...
			suggestedAction:
				'Confirm documented MR-conditional / non-ferromagnetic clip before scanning; otherwise contraindicated.'
		});
	}
	...
	if (
		...
		flags.push({
			...
			suggestedAction:
				'Refer to MR physics to confirm MR-conditional labelling, field strength, and protocol.'
		});
	}
	...
	if (ctx.contrastRenalFlag === 'contraindicated') {
		...
		flags.push({
			...
			suggestedAction:
				'Use a group II / low-risk agent only when clinically necessary; document justification.'
		});
	...
	if (
		data.contrast.previousGadoliniumReaction === 'moderate' ||
		...
		flags.push({
			...
			suggestedAction:
				'Avoid gadolinium or arrange premedication and supervised administration per local policy.'
		});
	}

	// --- Pregnancy -----------------------------------------------------
	if (
		(data.contrast.pregnancyStatus === 'pregnant' || data.contrast.pregnancyStatus === 'possible') &&
		...
		flags.push({
			...
			suggestedAction:
				'Avoid gadolinium in pregnancy unless the benefit clearly outweighs the risk; senior review required.'
		});
	} else if (data.contrast.pregnancyStatus === 'pregnant' || data.contrast.pregnancyStatus === 'possible') {
		flags.push({
	...
	if (data.safety.claustrophobia === true) {
		flags.push({
			...
			suggestedAction:
				'Consider open-bore scanner, sedation, or anaesthetic support; allow extra appointment time.'
		});
	}
