# Genetic Assessment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Genetic Assessment form, built with axum + Loco +
...
- [`genetic_assessment/`](./genetic_assessment/) — the Loco crate: `src/genetic_assessment/` holds `app.rs`
...
`src/genetic_assessment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd genetic_assessment && cargo check --all-targets && cargo test
```
