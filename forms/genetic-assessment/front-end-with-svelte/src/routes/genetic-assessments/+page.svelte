<script lang="ts">
	...
	const columns = [
		...
		{ id: 'riskLevel', header: 'Risk level', width: 120, sort: true },
		{
			id: 'urgency',
			header: 'Urgency',
			width: 110,
	...
	function init(api: any) {
		...
		api.on('select-row', (ev: { id?: string | number }) => {
			if (ev?.id != null) goto(`/genetic-assessments/${ev.id}`);
		});
	}
</script>
...
<main class="mx-auto max-w-6xl px-4 py-6">
	<div class="mb-6 flex flex-wrap items-end justify-between gap-4">
		...
		<a href="/genetic-assessments/new" class="button" data-variant="primary">New assessment</a>
	</div>
