import type { AssessmentData, FiredRule, RiskLevel, GradingResult } from './types';
import { riskRules } from './risk-rules';
import { riskCategory } from './utils';
import { detectAdditionalFlags } from './flagged-issues';

/**
 * Pure function: produces the full grading result for an assessment — the
 * risk score and level, every fired risk rule, the additional clinical flags,
 * and a generation timestamp. This is the single engine entry point used by
 * the wizard, report, and dashboard.
 */
export function calculateGrade(data: AssessmentData): GradingResult {
	const { riskScore, riskLevel, firedRules } = calculateRisk(data);
	return {
		riskScore,
		riskLevel,
		firedRules,
