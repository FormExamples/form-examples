// Flagged-issue detection (red flags). Independent of the total CHA2DS2-VASc
// score (which the grader produces), this module raises clinician-facing safety
// flags per spec §5:
//
//   - Anticoagulation recommended (high)   — riskBand == 'high'
//   - High stroke risk, decision undocumented (high) — riskBand == 'high'
...
//
// Rows here mirror the `cha2ds2_vasc_grade_flag` SQL table (flag_id, category,
...
function detectFlaggedIssues(data, grade) {
  ...
  // ─── High stroke risk, decision undocumented (HIGH) ─────────
  if (riskBand === 'high') {
    flags.push({
      id: 'F-HIGH-RISK-UNTREATED-001',
      category: 'high-risk-untreated',
      priority: 'high',
      description: 'High stroke risk that may be untreated — no anticoagulation decision is recorded on this assessment',
      suggestedAction:
  ...
  if (stroke === 'yes') {
    flags.push({
      ...
      priority: 'high',
      description: 'Prior stroke, TIA, or thromboembolism — the strongest single risk factor (2 points)',
      suggestedAction:
  ...
  if (sex === 'female' && cha2ds2VascScore === 1) {
    flags.push({
      ...
      priority: 'low',
      description: 'Score driven by female sex category alone — female sex is a risk modifier, not an independent risk factor',
      suggestedAction:
        'Manage as low risk: no anticoagulation for the sex category alone.'
    });
  }
  ...
  if (missing.length > 0) {
    flags.push({
      ...
      suggestedAction:
        'Complete every criterion and re-score.'
    });
  }
