// CHA2DS2-VASc — clinician dashboard (vanilla classic-script app).
//
...
// anticoagulation dropdown).
//
// Sibling modules loaded as plain `<script>` tags (in dependency order) attach
// their exports to
// `window.Cha2ds2VascScoreForAtrialFibrillationStrokeRiskDashboard`. The whole
...
const columns = [
  ...
  { key: 'cha2ds2VascScore',         label: 'CHA2DS2-VASc' },
  { key: 'riskBand',                 label: 'Risk Band' },
  { key: 'anticoagulationRecommended', label: 'Anticoag' }
];

// Rank used when sorting the riskBand column so 'low' < 'intermediate' < 'high'
...
const settingLabels = {
  ...
  'anticoagulation-clinic': 'Anticoag clinic',
...
function bandClass(band) {
  ...
  if (band === 'intermediate') return 'risk-moderate';
...
function matchesFilters(row) {
  ...
  if (filters.score) {
    if (row.cha2ds2VascScore === null ||
        row.cha2ds2VascScore !== Number(filters.score)) {
      return false;
    }
  }
  if (filters.band && row.riskBand !== filters.band) return false;
  if (filters.anticoag === 'yes' && !row.anticoagulationRecommended) return false;
  if (filters.anticoag === 'no' && row.anticoagulationRecommended) return false;
  return true;
}

/**
 * Compare two rows for the active sort column. The risk-band column uses its
 * rank table; the nullable score sorts nulls last; the anticoagulation boolean
...
function compareRows(a, b) {
  ...
  if (key === 'anticoagulationRecommended') {
...
function renderTableBody() {
  ...
  for (const row of rows) {
    ...
    const scoreClassName = row.cha2ds2VascScore === null
    ...
    tr.innerHTML = `
      ...
      <td><span class="${scoreClassName}">${esc(scoreLabel(row.cha2ds2VascScore))}</span></td>
      <td><span class="risk-badge ${bandClass(row.riskBand)}">${esc(bandLabel(row.riskBand))}</span></td>
      <td>
        <span class="flag-badge ${row.anticoagulationRecommended ? 'flag-yes' : 'flag-no'}">
          ${row.anticoagulationRecommended ? 'Yes' : 'No'}
        </span>
      </td>
    `;
    body.appendChild(tr);
