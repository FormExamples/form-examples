<script lang="ts">
	...
	// SVAR DataGrid columns. CHA2DS2-VASc score, risk band, and anticoagulation
	...
	const columns = [
		...
		{ id: 'cha2ds2VascScore', header: 'Score', width: 90, sort: true },
		{
			id: 'riskBand',
			header: 'Risk band',
			width: 160,
		...
		{
			id: 'anticoagulationFlag',
			header: 'Anticoag.',
			width: 110,
			template: (v: boolean) => (v ? 'Recommended' : '—')
		},
	...
	function init(api: any) {
		...
		api.on('select-row', (ev: { id?: string | number }) => {
			if (ev?.id != null) goto(`/cha2ds2-vasc-score-for-atrial-fibrillation-stroke-risk/cha2ds2-vasc-assessments/${ev.id}`);
		});
	}
</script>
...
<main class="mx-auto max-w-6xl px-4 py-6">
	<div class="mb-6 flex flex-wrap items-end justify-between gap-4">
		<div>
			<h1 class="text-2xl font-bold text-base-content">CHA2DS2-VASc clinician dashboard</h1>
			<p class="mt-1 text-sm text-base-content/70">
				CHA2DS2-VASc score, risk band, and anticoagulation recommendation for assessed patients,
				computed by the shared engine. Select a row to open the assessment.
			</p>
		</div>
		<a href="/cha2ds2-vasc-score-for-atrial-fibrillation-stroke-risk/cha2ds2-vasc-assessments/new" class="button" data-variant="primary">New assessment</a>
	</div>
	...
	<div class="mb-4 flex flex-wrap gap-4">
		<label class="text-sm">
			...
			<select class="select inline-block w-auto" bind:value={settingFilter}>
				...
				<option value="cardiology">Cardiology / arrhythmia clinic</option>
