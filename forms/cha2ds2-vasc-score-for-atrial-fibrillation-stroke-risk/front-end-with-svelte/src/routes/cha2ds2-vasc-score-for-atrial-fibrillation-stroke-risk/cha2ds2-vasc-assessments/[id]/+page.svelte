<script lang="ts">
	...
	import Step3CardiacHistory from '$lib/components/steps/Step3CardiacHistory.svelte';
	import Step4MetabolicHistory from '$lib/components/steps/Step4MetabolicHistory.svelte';
	import Step5AgeCriterion from '$lib/components/steps/Step5AgeCriterion.svelte';
	...
	function validate(): boolean {
		...
		if (d.identification.sex === '') {
			found.push({ id: 'identification-sex', message: 'Patient sex is required.' });
		}
		errors = found;
	...
	function submit() {
		...
		assessment.result = calculateCha2ds2VascGrade(assessment.data);
		goto(`/cha2ds2-vasc-score-for-atrial-fibrillation-stroke-risk/cha2ds2-vasc-assessments/${id}/report`);
	}
...
<main class="mx-auto max-w-3xl px-4 py-6">
	<header class="mb-6 no-print">
		<h1 class="text-2xl font-bold text-base-content">
			{isNew ? 'New CHA2DS2-VASc assessment' : `CHA2DS2-VASc assessment ${id}`}
		</h1>
		<p class="mt-1 text-sm text-base-content/70">
			Complete the six sections; the CHA2DS2-VASc score and risk band are computed on submit.
		</p>
	...
	<Form label="CHA2DS2-VASc assessment" onsubmit={submit}>
		<Step1Context />
		<Step2Identification />
		<Step3CardiacHistory />
		<Step4MetabolicHistory />
		<Step5AgeCriterion />
		<Step6Summary />
