...
// Urinalysis Test Request — vetting dashboard (vanilla classic-script app).
//
...
// preanalytical-band dropdown + safety-flags dropdown).
//
// Sibling modules loaded as plain `<script>` tags (in dependency order)
// attach their exports to `window.UrinalysisTestRequestDashboard`. The whole
...
const columns = [
  ...
  { key: 'preanalyticalBand',   label: 'Preanalytical' },
...
// Human-readable labels for the camelCase test keys.
const TEST_LABELS = {
  dipstick: 'Dipstick',
  microscopyCultureSensitivity: 'MC&S',
  albuminCreatinineRatio: 'ACR',
  proteinCreatinineRatio: 'PCR',
  pregnancyTest: 'Pregnancy',
  drugScreen: 'Drug screen',
  cytology: 'Cytology',
  twentyFourHourCollection: '24-hour'
};
...
/** Map a camelCase test key to a short display label. */
function testLabel(field) {
  return TEST_LABELS[field] || titleCase(field);
}
...
function testsText(row) {
  ...
  return row.tests.map(testLabel).join(' ');
}
...
function matchesFilters(row) {
  if (filters.search) {
    ...
    const matches =
      ...
      testsText(row).toLowerCase().includes(term) ||
...
function renderTestsCell(tests) {
  ...
  const chips = tests
    .map((t) => `<span class="test-chip">${esc(testLabel(t))}</span>`)
    .join('');
