...
// Re-export the canonical blank-request factory so consumers can import it from
// the store alongside the store instance.
export { createDefaultRequest };

/** localStorage draft key for a given request id (defaults to `new`). */
function storageKey(id: string): string {
	return `urinalysis-test-request.front-end-with-svelte.${id || 'new'}.v1`;
}

/**
 * Svelte 5 reactive store for the urinalysis test request, with localStorage
 ...
 */
class RequestStore {
	data = $state<UrinalysisRequest>(createDefaultRequest());
	 ...
	 * (e.g. `const d = requestStore.data.clinician`) stay bound to live state.
	 */
	loadForId(id: string, seed?: UrinalysisRequest) {
		const key = id || 'new';
		this.id = key;
		this.result = null;
		this.currentStep = 1;

		let draft: UrinalysisRequest | null = null;
		if (browser) {
			const raw = localStorage.getItem(storageKey(key));
			if (raw) {
				try {
					draft = JSON.parse(raw) as UrinalysisRequest;
				} catch {
