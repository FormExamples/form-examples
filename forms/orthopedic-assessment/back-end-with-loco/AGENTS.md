# Orthopaedic Assessment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Orthopaedic Assessment form, built with axum + Loco +
...
- [`orthopedic_assessment/`](./orthopedic_assessment/) — the Loco crate: `src/orthopedic_assessment/` holds `app.rs`
...
`src/orthopedic_assessment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd orthopedic_assessment && cargo check --all-targets && cargo test
```
