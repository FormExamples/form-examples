// Detects additional flags that should be highlighted for the orthopedic
// surgeon, independent of the DASH score. These are safety-critical or
...
function detectAdditionalFlags(data) {
  ...
  // Severe pain (>8/10)
  if (data.painAssessment.currentPainLevel !== null &&
  ...
  if (data.painAssessment.worstPain !== null &&
  ...
  // Joint instability
  if (data.chiefComplaint.onsetType === 'traumatic' &&
  ...
  // Infection signs
  if (data.chiefComplaint.aggravatingFactors.includes('swelling') &&
  ...
  data.currentTreatment.allergies.forEach((allergy, i) => {
  ...
  if (data.functionalLimitations.difficultyWithADLs.length >= 4) {
    flags.push({
  ...
  if (data.currentTreatment.physicalTherapy === 'yes' &&
  ...
  // Sort: urgent > high > medium > low (match asthma pattern, but only
  // priorities used here are high/medium).
