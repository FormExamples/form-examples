# Quick Sequential Organ Failure Assessment (qSOFA) — tasks
...
- [x] Scoring engine (`types.ts`, `utils.ts`, `qsofa-rules.ts`,
      `qsofa-grader.ts`, `flagged-issues.ts`) + Vitest tests.
...
- [ ] `bin/test-form quick-sequential-organ-failure-assessment` passes
