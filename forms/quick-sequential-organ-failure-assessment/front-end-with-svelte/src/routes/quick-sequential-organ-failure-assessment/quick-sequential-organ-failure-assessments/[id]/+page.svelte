<script lang="ts">
	...
	import Step3RespiratoryRate from '#lib/components/steps/Step3RespiratoryRate.svelte';
	import Step4Mentation from '#lib/components/steps/Step4Mentation.svelte';
	import Step5SystolicBloodPressure from '#lib/components/steps/Step5SystolicBloodPressure.svelte';
	...
	function submit() {
		...
		assessment.result = calculateQsofaGrade(assessment.data);
		goto(`/quick-sequential-organ-failure-assessment/quick-sequential-organ-failure-assessments/${id}/report`);
	}
...
<main class="mx-16 px-4 py-6">
	<h1 class="text-2xl font-bold text-base-content">
		{isNew ? 'New qSOFA assessment' : `qSOFA assessment ${id}`}
	</h1>
	<p class="mt-1 text-sm text-base-content/70">
		Complete the six sections; the qSOFA score and risk band are computed on submit.
	</p>
	...
	<Form label="qSOFA assessment" onsubmit={submit}>
		<Step1Context />
		<Step2Identification />
		<Step3RespiratoryRate />
		<Step4Mentation />
		<Step5SystolicBloodPressure />
		<Step6Summary />
