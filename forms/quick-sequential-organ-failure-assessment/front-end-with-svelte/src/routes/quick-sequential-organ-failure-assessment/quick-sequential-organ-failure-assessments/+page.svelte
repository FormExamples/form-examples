<script lang="ts">
	...
	// SVAR DataGrid columns. qSOFA score, risk band, and escalation render
	...
	const columns = [
		...
		{ id: 'qsofaScore', header: 'qSOFA', width: 90, sort: true },
		{
			id: 'riskBand',
			header: 'Risk band',
			width: 170,
	...
	function init(api: any) {
		...
		api.on('select-row', (ev: { id?: string | number }) => {
			if (ev?.id != null) goto(`/quick-sequential-organ-failure-assessment/quick-sequential-organ-failure-assessments/${ev.id}`);
		});
	}
</script>
...
<main class="mx-16 px-4 py-6">
	<div class="mb-6 flex flex-wrap items-end justify-between gap-4">
		<div>
			<h1 class="text-2xl font-bold text-base-content">qSOFA clinician dashboard</h1>
			<p class="mt-1 text-sm text-base-content/70">
				qSOFA score, risk band, and escalation for assessed patients, computed by the shared engine.
				Select a row to open the assessment.
			</p>
		</div>
		<a href="/quick-sequential-organ-failure-assessment/quick-sequential-organ-failure-assessments/new" class="button" data-variant="primary"
			>New assessment</a
		>
	</div>
	...
	<div class="mb-4 flex flex-wrap gap-4">
		...
		<label class="text-sm">
			...
			<select class="select inline-block w-auto" bind:value={bandFilter}>
				<option value="">All</option>
				<option value="lower">Lower (0-1)</option>
				<option value="higher">Higher (2-3)</option>
			</select>
		</label>
	</div>
