...
for the qSOFA specification and scoring engine. Lily Svelte headless
conventions: [`../../AGENTS-front-end-svelte.md`](../../AGENTS-front-end-svelte.md).

## Layout

- `src/lib/engine/` — pure scoring engine (`types.ts`, `utils.ts`,
  `qsofa-rules.ts`, `qsofa-grader.ts`, `flagged-issues.ts`) + `qsofa-grader.test.ts`.
...
- `src/routes/quick-sequential-organ-failure-assessments/` — RESTful routes:
