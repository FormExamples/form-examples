// qSOFA — clinician dashboard (vanilla classic-script app).
//
...
// their exports to `window.QuickSequentialOrganFailureAssessmentDashboard`.
...
const filters = {
  search: '',
  setting: '',   // '' | 'emergency-department' | 'ward' | 'pre-hospital' | 'other'
  score: '',     // '' | '0' | '1' | '2' | '3'
  band: '',      // '' | 'lower' | 'higher'
  escalate: ''   // '' | 'yes' | 'no'
};
...
// Rank used when sorting the riskBand column so 'lower' is always less than
// 'higher' regardless of locale.
const bandRank = {
  'lower': 0,
  'higher': 1
};
...
function bandClass(band) {
  if (!band) return '';
  return band === 'higher' ? 'risk-high' : 'risk-low';
}
...
function matchesFilters(row) {
  ...
  if (filters.score) {
    if (row.qsofaScore === null || row.qsofaScore !== Number(filters.score)) {
      return false;
    }
  }
 ...
 * rank table; the nullable qSOFA score sorts nulls last; the escalation
...
function renderTableBody() {
  ...
  for (const row of rows) {
    ...
    if (row.riskBand === 'higher') {
      tr.classList.add('row-critical');
    }

    const scoreClassName = row.qsofaScore === null
    ...
    tr.innerHTML = `
      ...
      <td><span class="${scoreClassName}">${esc(scoreLabel(row.qsofaScore))}</span></td>
