...
use crate::engine::{privacy_grader, types::AssessmentData};
...
/// POST /assessment/new -- create a new draft acknowledgement, redirect
/// to the wizard.
#[debug_handler]
...
/// Parse a form value string. Empty stays as `""`; the acknowledgment
/// checkbox flag is recognised as boolean true.
fn parse_form_value(field_key: &str, value: &serde_json::Value) -> serde_json::Value {
    if let Some(s) = value.as_str() {
        if field_key == "checked" {
            // HTML checkboxes submit "on" / "true" / "1" when checked.
            let truthy = matches!(s, "on" | "true" | "1" | "yes" | "checked");
...
async fn submit_assessment(
    ...
    // Ensure the acknowledgment.checked field defaults to false when its
    // checkbox is unchecked (browsers omit unchecked checkboxes).
    if let Some(obj) = data_value.as_object_mut() {
        if let Some(ack) = obj.get_mut("acknowledgment").and_then(|s| s.as_object_mut()) {
            ack.insert(
                "checked".to_string(),
...
async fn show_report(
    ...
    let grade = privacy_grader::grade(&assessment_data);
    ...
    context.insert("overall_status", &grade.overall_status);
    context.insert(
        "overall_label",
        &crate::engine::utils::overall_status_label(&grade.overall_status),
    );
