...
impl ActiveModel {
    /// Create a new in-progress acknowledgement with empty data.
...
/// Find an acknowledgement by its UUID.
pub async fn find_by_id(db: &DatabaseConnection, id: Uuid) -> Result<Option<Model>, DbErr> {
    Entity::find_by_id(id).one(db).await
}

/// List all completed acknowledgements (those with a grading result).
...
/// List acknowledgements with pagination (page is 0-indexed, per_page items each).
