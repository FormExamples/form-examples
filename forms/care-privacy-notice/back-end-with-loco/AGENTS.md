# Care Privacy Notice — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Care Privacy Notice form, built with axum + Loco +
...
- [`care_privacy_notice/`](./care_privacy_notice/) — the Loco crate: `src/care_privacy_notice/` holds `app.rs`
...
`src/care_privacy_notice/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd care_privacy_notice && cargo check --all-targets && cargo test
```
