...
// Blood Test Request — vetting dashboard (vanilla classic-script app).
//
...
// attach their exports to `window.BloodTestRequestDashboard`. The whole file
...
const columns = [
  ...
  { key: 'testsSelectedCount',  label: 'Tests' },
...
function compareRows(a, b) {
  ...
  if (key === 'completenessPercent' || key === 'testsSelectedCount') {
...
function renderTableBody() {
  ...
  for (const row of rows) {
    ...
    tr.innerHTML = `
      ...
      <td class="data-table-td"><span class="numeric-cell${row.testsSelectedCount === 0 ? ' numeric-zero' : ''}">${esc(row.testsSelectedCount ?? 0)}</span></td>
      <td class="data-table-td">${esc(titleCase(row.indication)) || '<span class="flag-empty">—</span>'}</td>
