...
for the corrected-calcium specification and calculation engine. Lily Svelte
...
- `src/lib/engine/` — pure calculation engine (`types.ts`, `utils.ts`,
  `calcium-rules.ts`, `calcium-calculator.ts`, `flagged-issues.ts`) +
  `calcium-calculator.test.ts`.
...
- `src/routes/corrected-calcium-calculators/` — RESTful routes:
  `/<plural>/` (dashboard, `ssr = false`) + `/<plural>/[id]` (wizard) +
  `/<plural>/[id]/report` (+ `report/pdf` server endpoint).

## Calculation engine

Formula-based (not additive): `correctedCalcium = totalCalcium + 0.02 × (40 −
albumin)`, computed only when both inputs are present, then classified against
the adult reference range 2.20-2.60 mmol/L. The unrounded value drives
classification and every flag threshold; the rounded value is display-only.
Severe results (≥ 3.0 or < 1.9 mmol/L) raise high-priority flags.
