<script lang="ts">
	...
	import Step3TotalCalcium from '$lib/components/steps/Step3TotalCalcium.svelte';
	...
	function submit() {
		...
		assessment.result = calculateCorrectedCalcium(assessment.data);
		goto(`/corrected-calcium-calculator/corrected-calcium-calculators/${id}/report`);
	}
...
<main class="mx-auto max-w-3xl px-4 py-6">
	<header class="mb-6 no-print">
		<h1 class="text-2xl font-bold text-base-content">
			{isNew ? 'New corrected calcium calculation' : `Corrected calcium ${id}`}
		</h1>
		<p class="mt-1 text-sm text-base-content/70">
			Complete the six sections; the corrected calcium and classification are computed on submit.
		</p>
	...
	<Form label="Corrected calcium calculation" onsubmit={submit}>
		<Step1Context />
		<Step2Identification />
		<Step3TotalCalcium />
		<Step4SerumAlbumin />
