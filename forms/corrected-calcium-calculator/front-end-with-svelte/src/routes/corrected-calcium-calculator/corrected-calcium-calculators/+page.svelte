<script lang="ts">
	...
	import { classificationLabel, careSettingLabel, formatCalcium } from '#lib/engine/utils.js';
	...
	// SVAR DataGrid columns. Corrected calcium, classification, and the severe
	...
	const columns = [
		...
		},
		{
			...
			width: 130,
			sort: true,
			template: (v: number | null) => formatCalcium(v)
		},
		{
			id: 'classification',
	...
	function init(api: any) {
		...
		api.on('select-row', (ev: { id?: string | number }) => {
			if (ev?.id != null) goto(`/corrected-calcium-calculator/corrected-calcium-calculators/${ev.id}`);
		});
	}
</script>
...
<main class="mx-16 px-4 py-6">
	<div class="mb-6 flex flex-wrap items-end justify-between gap-4">
		<div>
			<h1 class="text-2xl font-bold text-base-content">Corrected calcium dashboard</h1>
			<p class="mt-1 text-sm text-base-content/70">
				Corrected calcium and classification for assessed patients, computed by the shared engine.
				Select a row to open the assessment.
			</p>
		</div>
		<a href="/corrected-calcium-calculator/corrected-calcium-calculators/new" class="button" data-variant="primary"
			>New calculation</a
		>
	</div>
	...
	<div class="mb-4 flex flex-wrap gap-4">
		...
		<label class="text-sm">
			...
			<select class="select inline-block w-auto" bind:value={classificationFilter}>
				<option value="">All</option>
				<option value="hypocalcaemia">Hypocalcaemia</option>
				<option value="normal">Normal</option>
				<option value="hypercalcaemia">Hypercalcaemia</option>
