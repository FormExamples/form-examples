...
import type { EegRequest, GradingResult } from '$lib/engine/types';
import { createDefaultRequest } from '$lib/engine/defaults';

// Re-export the default factory so consumers can import it from the store too.
export { createDefaultRequest };

/** localStorage draft key for a given request id (defaults to `new`). */
function storageKey(id: string): string {
	return `electroencephalogram-test-request.front-end-with-svelte.${id || 'new'}.v1`;
}
...
/**
 * Svelte 5 reactive store for the EEG test request, with localStorage
 ...
 */
class RequestStore {
	data = $state<EegRequest>(createDefaultRequest());
	 ...
	 */
	loadForId(id: string, seed?: EegRequest) {
		const key = id || 'new';
		this.id = key;
		this.result = null;
		this.currentStep = 1;

		let draft: EegRequest | null = null;
		if (browser) {
			const raw = localStorage.getItem(storageKey(key));
			if (raw) {
				try {
					draft = JSON.parse(raw) as EegRequest;
				} catch {
