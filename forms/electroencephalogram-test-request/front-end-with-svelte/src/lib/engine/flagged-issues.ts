import type { EegRequest, Flag, FlagPriority } from './types';

// Phrases that imply the EEG is being requested to rule out / exclude epilepsy
// — contrary to NICE NG217.
...
function impliesExcludeEpilepsy(d: EegRequest): boolean {
 ...
 * medium → low priority.
 */
export function detectFlags(data: EegRequest): Flag[] {
	const flags: Flag[] = [];
	...
	if (
		...
		flags.push({
			...
			suggestedAction:
				'Arrange emergency EEG and escalate to the on-call neurology / neurophysiology team; do not delay for routine booking.'
		});
	}
	if (
		...
		flags.push({
			...
			suggestedAction:
				'Expedite EEG via first-seizure pathway; an early EEG increases the diagnostic yield of interictal discharges.'
		});
	}
	if (data.request.primaryIndication === 'encephalopathy') {
		flags.push({
			...
			suggestedAction:
				'Urgent EEG to characterise diffuse cerebral dysfunction and exclude non-convulsive status epilepticus.'
		});
	}
	...
	if (impliesExcludeEpilepsy(data)) {
		flags.push({
			...
			suggestedAction:
				'Per NICE NG217, EEG must not be used to exclude a diagnosis of epilepsy nor in isolation; query the referrer.'
		});
	}
