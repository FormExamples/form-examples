import type { EegRequest, FiredRule } from './types';
...
interface FieldCheck {
	...
	ruleId: string;
	label: string;
	present: (d: EegRequest) => boolean;
}

/**
 * The tracked request fields, with the primary indication and clinical question
 ...
 */
export function scoreCompleteness(data: EegRequest): {
	percent: number;
	missing: FiredRule[];
} {
	let totalWeight = 0;
