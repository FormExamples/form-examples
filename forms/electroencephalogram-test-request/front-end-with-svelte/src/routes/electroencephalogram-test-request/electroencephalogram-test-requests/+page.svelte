<script lang="ts">
	...
	const columns = [
...

		{
			...
			header: 'EEG type',
			flexgrow: 2,
			template: (v: string) => eegTypeLabel(v as never)
		},
		{
	...
	function init(api: any) {
		...
		api.on('select-row', (ev: { id?: string | number }) => {
			if (ev?.id != null) goto(`/electroencephalogram-test-request/electroencephalogram-test-requests/${ev.id}`);
		});
	}
</script>
...
<main class="mx-16 px-4 py-6">
	<div class="mb-6 flex flex-wrap items-end justify-between gap-4">
		<div>
			<h1 class="text-2xl font-bold text-base-content">EEG vetting dashboard</h1>
			<p class="mt-1 text-sm text-base-content/70">
				Four-axis vetting grades for incoming EEG test requests, computed by the shared engine.
				Select a row to open the request.
			</p>
		</div>
		<a href="/electroencephalogram-test-request/electroencephalogram-test-requests/new" class="button" data-variant="primary"
			>New request</a
		>
	</div>
