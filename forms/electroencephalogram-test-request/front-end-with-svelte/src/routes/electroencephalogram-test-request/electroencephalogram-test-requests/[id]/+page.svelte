<script lang="ts">
	...
	import Step4SeizureContext from '#lib/components/steps/Step4SeizureContext.svelte';
	import Step5RedFlags from '#lib/components/steps/Step5RedFlags.svelte';
	...
	function validate(): boolean {
		...
		if (d.request.eegType === '') {
			found.push({ id: 'eegType', message: 'Requested EEG type is required.' });
		}
	...
	function submit() {
		...
		goto(`/electroencephalogram-test-request/electroencephalogram-test-requests/${id}/report`);
	}
...
<main class="mx-16 px-4 py-6">
	<h1 class="text-2xl font-bold text-base-content">
		{isNew ? 'New EEG test request' : `EEG test request ${id}`}
	</h1>
	...
	<Form label="EEG test request" onsubmit={submit}>
