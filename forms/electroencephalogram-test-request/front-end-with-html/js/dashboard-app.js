...
// Electroencephalogram (EEG) Test Request — vetting dashboard
...
// clinical-priority dropdown + safety-flags dropdown).
//
// Sibling modules loaded as plain `<script>` tags (in dependency order)
// attach their exports to `window.ElectroencephalogramTestRequestDashboard`.
...
function matchesFilters(row) {
  if (filters.search) {
    ...
    const matches =
      ...
      String(row.eegType || '').toLowerCase().includes(term) ||
...
function compareRows(a, b) {
  ...
  // Default: string compare (nhs, patient, eegType, indication, clinician,
  // referralDate).
...
function renderTableBody() {
  ...
  for (const row of rows) {
    ...
    tr.innerHTML = `
      ...
      <td class="data-table-td">${esc(titleCase(row.eegType))}</td>
