...
import { WINDOW_MAX_DAYS, WINDOW_MIN_DAYS } from './bloodspot-rules';

/** Context the grader has already computed, used by the flag detectors. */
...
export function detectFlaggedIssues(
	...
	if (
		...
		flags.push({
			...
			message:
				'Inadequate blood spot sample — cannot be reliably screened; a repeat sample is required.',
			priority: 'high'
		});
	}
	...
	if (
		...
		(ageAtSampleDays < WINDOW_MIN_DAYS || ageAtSampleDays > WINDOW_MAX_DAYS)
	) {
		const when = ageAtSampleDays < WINDOW_MIN_DAYS ? 'early' : 'late';
		flags.push({
			id: 'FLAG-WINDOW-001',
			category: 'Timing',
	...
	if (data.conditions.scdResult === 'carrier') {
		flags.push({
			...
			message:
				'Sickle cell carrier detected — communicate carrier status to the family; consider parental testing / genetic counselling.',
			priority: 'low'
		});
	}
