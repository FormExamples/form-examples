...
import type { EyeVisionResult, GradingResult } from '#lib/engine/types.js';
import {
...
/** Builds the pdfmake document definition for the eye vision report. */
export function buildPdfDocument(
	data: EyeVisionResult,
	...
	return {
		...
		header: {
			text: 'OPHTHALMIC EYE EXAMINATION REPORT',
			alignment: 'center',
			margin: [0, 20, 0, 0],
			fontSize: 10,
			color: '#6b7280',
		...
		footer: (currentPage: number, pageCount: number) => ({
			text: `Page ${currentPage} of ${pageCount} | Generated ${new Date(result.gradedAt).toLocaleString()} | RCOphth / NICE NG81 / NHS DESP`,
			alignment: 'center',
			margin: [0, 20, 0, 0],
			fontSize: 8,
			color: '#9ca3af'
		...
		content: [
			...
			sectionHeader('Report identification'),
			{
				table: {
					...
					body: [
						...
						[
							field('Test type', testTypeLabel(data.testType)),
							field('Report status', reportStatusLabel(data.reportStatus))
						],
						[
			...
			{
				ul: [
					`Visual acuity — right: ${data.visualAcuityRight || 'N/A'}`,
					`Visual acuity — left: ${data.visualAcuityLeft || 'N/A'}`,
					`Intraocular pressure — right: ${data.intraocularPressureRightMmhg !== null ? `${data.intraocularPressureRightMmhg} mmHg` : 'N/A'}`,
					`Intraocular pressure — left: ${data.intraocularPressureLeftMmhg !== null ? `${data.intraocularPressureLeftMmhg} mmHg` : 'N/A'}`,
					`Visual fields: ${visualFieldResultLabel(data.visualFieldResult)}`,
					`Retinopathy grade: ${retinopathyGradeLabel(data.retinopathyGrade)}`
				],
				margin: [0, 0, 0, 16] as Margin
			},
...
function structuredFindingList(data: EyeVisionResult): string[] {
	const list: string[] = [];
	if (data.reducedVisualAcuity) list.push('Reduced visual acuity');
	if (data.visualFieldDefect) list.push('Visual-field defect');
	if (data.raisedIntraocularPressure) list.push('Raised intraocular pressure');
	if (data.diabeticRetinopathy)
		list.push(`Diabetic retinopathy (${retinopathyGradeLabel(data.retinopathyGrade)})`);
	if (data.opticDiscAbnormality) list.push('Optic-disc abnormality');
	if (data.macularAbnormality) list.push('Macular abnormality');
