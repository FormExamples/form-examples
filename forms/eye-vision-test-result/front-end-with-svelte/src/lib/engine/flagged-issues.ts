import type { EyeVisionResult, Flag, FlagPriority } from './types';
import { hasCriticalFinding, hasReferableRetinopathy, hasElevatedIop } from './utils';

/**
 * Detects safety-critical flags independently of the four axes. Flag
 * categories mirror sql/07_create_table_eye_vision_test_result_grade_flag.sql.
 * Flags are returned sorted high → medium → low priority.
 */
export function detectFlags(r: EyeVisionResult): Flag[] {
	const flags: Flag[] = [];
	...
	if (hasReferableRetinopathy(r) && !hasCriticalFinding(r)) {
		flags.push({
			...
			priority: 'high',
			description: 'Referable diabetic retinopathy / maculopathy requiring timely action is present.',
			suggestedAction:
	...
	if (
		...
		flags.push({
			...
			priority: 'medium',
			description: 'No measurements were recorded; diagnostic confidence may be reduced.',
			suggestedAction:
	...
	if (
		...
		flags.push({
			...
			priority: 'low',
			description: 'Raised intraocular pressure is reported but no pressure value was recorded.',
