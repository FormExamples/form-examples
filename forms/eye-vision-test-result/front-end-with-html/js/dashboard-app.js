...
// Eye Vision Test Result — clinician dashboard (vanilla classic-script app).
//
...
// their exports to `window.EyeVisionTestResultDashboard`; display labels come
// from the form namespace `window.EyeVisionTestResult` (js/types.js). The
...
function compareRows(a, b) {
  ...
  // Default: string compare (id, patientName, testType, reportStatus,
...
function renderTableBody() {
  ...
  for (const row of rows) {
    ...
    tr.innerHTML = `
      ...
      <td>${esc(testTypeLabel(row.testType))}</td>
...
function bindFilterInputs() {
  ...
  const testType = document.getElementById('filter-test-type');
  ...
  if (testType) {
    testType.addEventListener('change', () => {
      filters.testType = testType.value;
      renderAll();
    });
  }
  if (classification) {
