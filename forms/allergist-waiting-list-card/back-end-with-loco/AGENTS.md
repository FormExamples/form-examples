# Allergist Waiting List Card — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Allergist Waiting List Card form, built with axum + Loco +
...
- [`allergist_waiting_list_card/`](./allergist_waiting_list_card/) — the Loco crate: `src/allergist_waiting_list_card/` holds `app.rs`
...
`/metrics`. The registered domain controllers are:
  ...
  - `allergist_waiting_list_card`
  - `allergist_waiting_list_card_appointment`
  - `allergist_waiting_list_card_grade`
  - `allergist_waiting_list_card_grade_flag`
  - `allergist_waiting_list_card_grade_rule`
  - `patient`
  - `practitioner`

## Engine

`src/allergist_waiting_list_card/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd allergist_waiting_list_card && cargo check --all-targets && cargo test
```
