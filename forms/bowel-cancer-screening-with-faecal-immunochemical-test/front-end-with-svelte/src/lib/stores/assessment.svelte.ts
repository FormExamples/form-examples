...
import { DEFAULT_THRESHOLD } from '#lib/engine/bowel-fit-rules.js';

/** localStorage draft key for a given assessment id (defaults to `new`). */
function storageKey(id: string): string {
	return `bowel-cancer-screening-with-faecal-immunochemical-test.front-end-with-svelte.${id || 'new'}.v1`;
}

/** A blank bowel-cancer-screening FIT assessment with all fields at their unanswered defaults. */
...
/**
 * Svelte 5 reactive store for the FIT assessment, with localStorage persistence
...
class AssessmentStore {
	 ...
	 * (e.g. `const k = assessment.data.kit`) stay bound to live state.
	 */
