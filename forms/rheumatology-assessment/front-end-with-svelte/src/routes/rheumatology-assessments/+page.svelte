<script lang="ts">
	...
	import { diseaseActivityLabel, diagnosisLabel } from '$lib/engine/utils';
	import type { DiseaseActivity, DiseaseHistory } from '$lib/engine/types';
	...
	const columns = [
		...
		{ id: 'biologicFlag', header: 'Biologic', width: 95, template: (v: boolean) => (v ? 'Yes' : 'No') },
	...
	function init(api: any) {
		...
		api.on('select-row', (ev: { id?: string | number }) => {
			if (ev?.id != null) goto(`/rheumatology-assessments/${ev.id}`);
		});
	}
</script>
...
<main class="mx-auto max-w-6xl px-4 py-6">
	<div class="mb-6 flex flex-wrap items-end justify-between gap-4">
		...
		<a href="/rheumatology-assessments/new" class="button" data-variant="primary">New assessment</a>
	</div>
