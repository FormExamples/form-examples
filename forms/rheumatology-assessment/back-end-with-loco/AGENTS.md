# Rheumatology Assessment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Rheumatology Assessment form, built with axum + Loco +
...
- [`rheumatology_assessment/`](./rheumatology_assessment/) — the Loco crate: `src/rheumatology_assessment/` holds `app.rs`
...
`src/rheumatology_assessment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd rheumatology_assessment && cargo check --all-targets && cargo test
```
