import type { HolterMonitorResult, Flag, FlagPriority } from './types';
import { hasCriticalFinding, hasSignificantPause } from './utils';

/**
 * Detects safety-critical flags independently of the four axes. Flag
 * categories mirror sql/07_create_table_holter_monitor_test_result_grade_flag.sql.
 * Flags are returned sorted high → medium → low priority.
 */
export function detectFlags(r: HolterMonitorResult): Flag[] {
	const flags: Flag[] = [];
