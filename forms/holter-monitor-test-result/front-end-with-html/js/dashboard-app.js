...
import { abnormalitySeverityClass, abnormalitySeverityLabel, followUpUrgencyClass, followUpUrgencyLabel, monitorTypeLabel, reportStatusLabel, resultClassificationClass, resultClassificationLabel } from './types.js';

// Holter Monitor Test Result — clinician dashboard (vanilla classic-script app).
//
...
// box + monitor-type dropdown + classification dropdown + follow-up-urgency
// dropdown). Columns mirror the SvelteKit dashboard route
// (`front-end-with-svelte/src/routes/+page.svelte`).
//
// Sibling modules loaded as plain `<script>` tags (in dependency order) attach
// their exports to `window.HolterMonitorTestResultDashboard`; display labels
// come from the form namespace `window.HolterMonitorTestResult` (js/types.js).
...
function matchesFilters(row) {
  ...
  if (filters.monitorType && row.monitorType !== filters.monitorType) return false;
...
function compareRows(a, b) {
  ...
  // Default: string compare (id, patientName, monitorType, reportStatus,
...
function renderTableBody() {
  ...
  for (const row of rows) {
    ...
    tr.innerHTML = `
      ...
      <td>${esc(monitorTypeLabel(row.monitorType))}</td>
...
function bindFilterInputs() {
  ...
  const monitorType = document.getElementById('filter-monitor-type');
  ...
  if (monitorType) {
    monitorType.addEventListener('change', () => {
      filters.monitorType = monitorType.value;
      renderAll();
    });
  }
  if (classification) {
  ...
  if (clearBtn) {
    clearBtn.addEventListener('click', () => {
      ...
      if (monitorType) monitorType.value = '';
