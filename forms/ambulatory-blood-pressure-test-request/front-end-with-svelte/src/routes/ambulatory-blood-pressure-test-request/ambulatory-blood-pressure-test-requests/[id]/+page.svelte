<script lang="ts">
	...
	import Step4ClinicBloodPressure from '#lib/components/steps/Step4ClinicBloodPressure.svelte';
	...
	function submit() {
		...
		goto(`/ambulatory-blood-pressure-test-request/ambulatory-blood-pressure-test-requests/${id}/report`);
	}
...
<main class="mx-16 px-4 py-6">
	<h1 class="text-2xl font-bold text-base-content">
		{isNew ? 'New ABPM request' : `ABPM request ${id}`}
	</h1>
	<p class="mt-1 text-sm text-base-content/70">
		Complete the six sections; the four-axis grade, recommendation, and flags are computed on
		submit.
	</p>
	...
	<Form label="Ambulatory blood pressure test request" onsubmit={submit}>
		<Step1Clinician />
		<Step2Patient />
		<Step3RequestedTest />
		<Step4ClinicBloodPressure />
		<Step5Symptoms />
		<Step6Triage />
