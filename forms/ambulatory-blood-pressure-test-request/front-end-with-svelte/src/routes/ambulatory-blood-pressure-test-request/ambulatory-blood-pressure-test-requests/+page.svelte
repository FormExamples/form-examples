<script lang="ts">
	...
	import { RECOMMENDATION_LABELS } from '$lib/engine/grader';
	...
	// SVAR DataGrid columns. The four axes, the recommendation, and the flag
	// count all render through the shared engine output so the dashboard and the
	// report stay aligned.
	const columns = [
		{ id: 'id', header: 'Request', width: 130 },
		{ id: 'patientName', header: 'Patient', flexgrow: 2, sort: true },
		{ id: 'referralDate', header: 'Referred', width: 110, sort: true },
		{ id: 'clinicBp', header: 'Clinic BP', width: 100 },
		{
		...
		},
		{
			...
			width: 110,
			sort: true,
			template: (v: string) => suitabilityBandLabel(v as never)
		},
		{
		...
		},
		{
			...
			width: 150,
			sort: true,
			template: (v: string) => RECOMMENDATION_LABELS[v as never]
		},
	...
	function init(api: any) {
		...
		api.on('select-row', (ev: { id?: string | number }) => {
			if (ev?.id != null) goto(`/ambulatory-blood-pressure-test-request/ambulatory-blood-pressure-test-requests/${ev.id}`);
		});
	}
</script>
...
<main class="mx-auto max-w-6xl px-4 py-6">
	<div class="mb-6 flex flex-wrap items-end justify-between gap-4">
		<div>
			<h1 class="text-2xl font-bold text-base-content">ABPM request vetting dashboard</h1>
			<p class="mt-1 text-sm text-base-content/70">
				Four-axis grade, recommendation, and safety-flag count for incoming requests, computed by the
				shared engine. Select a row to open the request.
			</p>
		</div>
		<a href="/ambulatory-blood-pressure-test-request/ambulatory-blood-pressure-test-requests/new" class="button" data-variant="primary">New request</a>
	</div>
	...
	<div class="mb-4 flex flex-wrap gap-4">
		...
		<label class="text-sm">
			...
			<select class="select inline-block w-auto" bind:value={recommendationFilter}>
				...
				<option value="redirect">Redirect / alternative</option>
