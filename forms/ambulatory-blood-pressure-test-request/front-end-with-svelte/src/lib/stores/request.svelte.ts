...
/** Re-export the plain default factory so callers can `import` it from the store. */
export { createDefaultRequest };

/** localStorage draft key for a given request id (defaults to `new`). */
function storageKey(id: string): string {
	return `ambulatory-blood-pressure-test-request.front-end-with-svelte.${id || 'new'}.v1`;
}
...
/**
 * Svelte 5 reactive store for the ABPM request, with localStorage persistence
 * so an in-progress request survives a page reload. Drafts are keyed by request
 * id so each record edits independently.
 */
class RequestStore {
	data = $state<AbpmRequest>(createDefaultRequest());
	 ...
	 */
	loadForId(id: string, seed?: AbpmRequest) {
		const key = id || 'new';
		this.id = key;
		this.result = null;
		this.currentStep = 1;

		let draft: AbpmRequest | null = null;
		if (browser) {
			const raw = localStorage.getItem(storageKey(key));
			if (raw) {
				try {
					draft = JSON.parse(raw) as AbpmRequest;
				} catch {
