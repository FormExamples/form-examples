...
// missing-clinical-question, other. Each flag is
...
import type { AbpmRequest, SafetyFlag } from './types';
import { BP_SEVERE_SYSTOLIC, BP_SEVERE_DIASTOLIC } from './rules';
...
function bpPresent(d: AbpmRequest): boolean {
	return (
...
export function detectFlags(data: AbpmRequest): SafetyFlag[] {
	...
	if (accelerated) {
		flags.push({
			...
			suggestedAction:
				'Arrange same-day specialist review; assess for end-organ damage (fundoscopy, urinalysis, ECG, bloods) before routine ABPM.'
		});
	} else if (severe) {
		flags.push({
			...
			suggestedAction:
				'Arrange same-day specialist review (NICE NG136); same-day assessment takes priority over routine ABPM booking.'
		});
	}
	...
	if (data.symptoms.atrialFibrillation === true) {
		flags.push({
			...
			suggestedAction:
				'Use an AF-validated device or confirm readings auscultatorily; interpret ABPM results with caution.'
		});
	}
	...
	if (!bpPresent(data)) {
		flags.push({
			...
			suggestedAction:
				'Record a clinic blood pressure; it drives both the appropriateness and triage axes.'
		});
	}
