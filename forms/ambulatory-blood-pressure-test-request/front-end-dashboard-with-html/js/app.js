// Ambulatory Blood Pressure Test Request — vetting dashboard
...
// suitability-band dropdown + safety-flags dropdown).
//
// Sibling modules loaded as plain `<script>` tags (in dependency order)
// attach their exports to
// `window.AmbulatoryBloodPressureTestRequestDashboard`. The whole file is
// wrapped in an IIFE so its top-level identifiers do not leak to global scope.
(function () {
'use strict';
const {
  fetchRequests,
...
const columns = [
  ...
  { key: 'clinicBp',           label: 'Clinic BP' },
  { key: 'appropriatenessBand', label: 'Appropriateness' },
  { key: 'suitabilityBand',    label: 'Suitability' },
...
// Clinic-BP sort: convert "S/D" into systolic-major ordinal.
function bpToOrdinal(bp) {
  if (!bp) return -1;
  const m = String(bp).match(/^(\d+)(?:\/(\d+))?$/);
  if (!m) return -1;
...
function suitabilityClass(band) {
  return band ? 'suitability-' + String(band) : '';
}
...
function matchesFilters(row) {
  ...
  if (filters.suitability && row.suitabilityBand !== filters.suitability) return false;
...
function compareRows(a, b) {
  ...
  if (key === 'suitabilityBand') {
    return ((suitabilityRank[av] ?? -1) - (suitabilityRank[bv] ?? -1)) * dir;
  }
  if (key === 'triageTier') {
  ...
  if (key === 'clinicBp') {
    return (bpToOrdinal(av) - bpToOrdinal(bv)) * dir;
  }
  if (key === 'flags') {
...
function renderTableBody() {
  ...
  for (const row of rows) {
    ...
    tr.innerHTML = `
      ...
      <td class="data-table-td"><span class="numeric-cell">${esc(row.clinicBp || '—')}</span></td>
      ...
      <td class="data-table-td"><span class="suitability-badge ${suitabilityClass(row.suitabilityBand)}">${esc(titleCase(row.suitabilityBand))}</span></td>
...
function bindFilterInputs() {
  ...
  const suitability = document.getElementById('filter-suitability');
  ...
  if (suitability) {
    suitability.addEventListener('change', () => {
      filters.suitability = suitability.value;
      renderAll();
    });
  }
  if (flags) {
  ...
  if (clearBtn) {
    clearBtn.addEventListener('click', () => {
      ...
      if (band) band.value = '';
      if (suitability) suitability.value = '';
