import type { ClerkingRecord, FlaggedIssue } from './types';
import { allergyDocumented, nonEmpty, missingCoreExam, missingHistory, abnormalVitals } from './history-and-physical-rules';

/**
 * Detect the clinician-facing safety flags raised by the clerking findings.
 * Computed INDEPENDENTLY of the completeness status per spec §5, each flag
 * carries a priority and a suggested action. Two categories are BLOCKING (they
 * force an incomplete status in the grader): allergies-not-documented and
 ...
 *
 * Rows mirror the `history_and_physical_examination_grade_flag` SQL table
 * (flag_id, category, priority, description, suggested_action).
 */
export function detectFlaggedIssues(r: ClerkingRecord): FlaggedIssue[] {
	...
	if (!nonEmpty(r.impression) && !nonEmpty(r.managementPlan)) {
		flags.push({
			...
			priority: 'high',
			blocking: true,
			description: 'Neither an impression / problem list nor a management plan has been recorded.',
			suggestedAction:
