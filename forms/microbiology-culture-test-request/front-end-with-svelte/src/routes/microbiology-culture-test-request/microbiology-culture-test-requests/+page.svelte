<script lang="ts">
	...
	function init(api: any) {
		...
		api.on('select-row', (ev: { id?: string | number }) => {
			if (ev?.id != null) goto(`/microbiology-culture-test-request/microbiology-culture-test-requests/${ev.id}`);
		});
	}
</script>
...
<main class="mx-auto max-w-6xl px-4 py-6">
	<div class="mb-6 flex flex-wrap items-end justify-between gap-4">
		<div>
			<h1 class="text-2xl font-bold text-base-content">Microbiology vetting dashboard</h1>
			<p class="mt-1 text-sm text-base-content/70">
				Four-axis vetting grades for incoming microbiology culture requests, computed by the shared
				engine. Select a row to open the request.
			</p>
		</div>
		<a href="/microbiology-culture-test-request/microbiology-culture-test-requests/new" class="button" data-variant="primary">New request</a>
	</div>
