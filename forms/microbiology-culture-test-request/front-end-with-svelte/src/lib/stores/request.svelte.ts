...
import type { MicrobiologyRequest, GradingResult } from '#lib/engine/types.js';
import { createDefaultRequest } from '#lib/engine/defaults.js';

// Re-export so callers can import the canonical empty request from the store too.
export { createDefaultRequest };

/** localStorage draft key for a given request id (defaults to `new`). */
function storageKey(id: string): string {
	return `microbiology-culture-test-request.front-end-with-svelte.${id || 'new'}.v1`;
}

/**
 * Svelte 5 reactive store for the microbiology culture request, with
 ...
 */
class RequestStore {
	data = $state<MicrobiologyRequest>(createDefaultRequest());
	 ...
	 */
	loadForId(id: string, seed?: MicrobiologyRequest) {
		const key = id || 'new';
		this.id = key;
		this.result = null;
		this.currentStep = 1;

		let draft: MicrobiologyRequest | null = null;
		if (browser) {
			const raw = localStorage.getItem(storageKey(key));
			if (raw) {
				try {
					draft = JSON.parse(raw) as MicrobiologyRequest;
				} catch {
