# Immunology Waiting List Card — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Immunology Waiting List Card form, built with axum + Loco +
...
- [`immunology_waiting_list_card/`](./immunology_waiting_list_card/) — the Loco crate: `src/immunology_waiting_list_card/` holds `app.rs`
...
`/metrics`. The registered domain controllers are:
  ...
  - `immunology_waiting_list_card`
  - `immunology_waiting_list_card_appointment`
  - `immunology_waiting_list_card_grade`
  - `immunology_waiting_list_card_grade_flag`
  - `immunology_waiting_list_card_grade_rule`
  - `patient`
  - `practitioner`

## Engine

`src/immunology_waiting_list_card/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd immunology_waiting_list_card && cargo check --all-targets && cargo test
```
