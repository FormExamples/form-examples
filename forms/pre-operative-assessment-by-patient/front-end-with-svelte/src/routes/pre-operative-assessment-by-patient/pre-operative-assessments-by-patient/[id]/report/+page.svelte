<script lang="ts">
	...
	import { asaGradeLabel, asaGradeColor, bmiCategory, calculateAge } from '#lib/engine/utils.js';
	...
	$effect(() => {
		if (!assessment.result) {
			goto(`/pre-operative-assessment-by-patient/pre-operative-assessments-by-patient/${id}`);
		}
	});
	...
	async function downloadPDF() {
		pdfError = '';
		try {
			const res = await fetch(`/pre-operative-assessments-by-patient/${id}/report/pdf`, {
				method: 'POST',
			...
			if (res.ok) {
				...
				a.href = url;
				a.download = `pre-operative-assessment-${data.demographics.lastName || id}.pdf`;
				a.click();
...
{#if result}
	<header class="border-b border-base-300 bg-base-100 shadow-sm no-print">
		<div class="mx-auto flex max-w-4xl items-center justify-between px-4 py-4">
			<h1 class="text-lg font-bold text-base-content">Pre-operative assessment report</h1>
			<div class="flex items-center gap-3">
				...
				<Button
					...
					onclick={() => goto(`/pre-operative-assessment-by-patient/pre-operative-assessments-by-patient/${id}`)}>Edit</Button
				>
			</div>
		</div>
	</header>
	...
	<main class="mx-16 px-4 py-6">
		...
		{#if result.additionalFlags.length > 0}
			<div class="mb-6 rounded-xl border border-error/40 bg-base-100 p-6">
				<h2 class="mb-4 text-lg font-bold text-error">Flagged issues for anaesthetist</h2>
				<div class="space-y-2">
		...
		{#if result.firedRules.length > 0}
			<div class="mb-6 rounded-xl border border-base-300 bg-base-100 p-6">
				<h2 class="mb-4 text-lg font-bold text-base-content">ASA grade justification</h2>
		...
		<div class="mb-6 rounded-xl border border-base-300 bg-base-100 p-6">
			...
			<div class="grid grid-cols-1 gap-4 text-sm sm:grid-cols-2">
				...
				<div class="sm:col-span-2">
					<span class="font-medium text-base-content/70">Planned procedure:</span>
					{data.demographics.plannedProcedure}
					{#if data.demographics.procedureUrgency}({data.demographics.procedureUrgency}){/if}
				</div>
			</div>
		</div>
				...
				<ul class="list-disc space-y-1 pl-5 text-sm text-base-content/80">
					{#each data.allergies as allergy (allergy.allergen)}
						<li>
							...
							{#if allergy.severity}
								<span
									class="ml-1 rounded px-1.5 py-0.5 text-xs {allergy.severity === 'anaphylaxis'
