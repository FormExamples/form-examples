# Pre-operative Assessment by Patient — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Pre-operative Assessment by Patient form, built with axum + Loco +
...
- [`pre_operative_assessment_by_patient/`](./pre_operative_assessment_by_patient/) — the Loco crate: `src/pre_operative_assessment_by_patient/` holds `app.rs`
...
`src/pre_operative_assessment_by_patient/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd pre_operative_assessment_by_patient && cargo check --all-targets && cargo test
```
