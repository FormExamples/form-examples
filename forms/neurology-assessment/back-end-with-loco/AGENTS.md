# Neurology Assessment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Neurology Assessment form, built with axum + Loco +
...
- [`neurology_assessment/`](./neurology_assessment/) — the Loco crate: `src/neurology_assessment/` holds `app.rs`
...
`src/neurology_assessment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd neurology_assessment && cargo check --all-targets && cargo test
```
