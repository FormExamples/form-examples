// Flagged-issue detection. Independent of NIHSS score (which the grader
// computes), this module raises clinician-facing flags for acute stroke
// indicators, status epilepticus, raised ICP, headache red flags,
// anticoagulant use, cognitive impairment, gait instability, driving
// restrictions and other safety-critical findings.
//
// Mirrors the SvelteKit reference engine
...
function detectAdditionalFlags(data) {
  ...
  if (
    data.headacheAssessment.redFlagFever === 'yes' &&
    data.headacheAssessment.redFlagNeckStiffness === 'yes'
  ) {
    flags.push({
  ...
  if (
    data.cognitiveAssessment.mmseScore !== null &&
    data.cognitiveAssessment.mmseScore < 24
  ) {
    flags.push({
  ...
  if (
    data.motorSensoryExam.gait === 'ataxic' ||
    data.motorSensoryExam.gait === 'unable'
  ) {
    flags.push({
  ...
  if (
    data.functionalSocial.mrsScore !== null &&
    data.functionalSocial.mrsScore >= 4
  ) {
    flags.push({
