...
//
// Rows here mirror the `abdominal_aortic_aneurysm_screening_grade_flag` SQL
...
function detectFlaggedIssues(data, grade) {
  ...
  if (growthCm !== null && growthCm >= RAPID_GROWTH_CM) {
    flags.push({
      ...
      priority: 'high',
      description: `Growth of ${growthCm} cm since the prior scan (at or above the ${RAPID_GROWTH_CM} cm rapid-growth threshold) — accelerated expansion`,
      suggestedAction:
  ...
  if (visualised === 'no' || diameter === null || diameter === undefined) {
    flags.push({
      ...
      priority: 'medium',
      description:
        'Aorta not adequately measured — the result cannot be classified as normal',
      suggestedAction:
  ...
  if (missing.length > 0) {
    flags.push({
      ...
      suggestedAction:
        'Complete the missing field(s) before sign-off.'
    });
  }
