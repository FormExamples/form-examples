<script lang="ts">
	...
	import Step4Measurement from '#lib/components/steps/Step4Measurement.svelte';
	import Step5Observations from '#lib/components/steps/Step5Observations.svelte';
	...
	function validate(): boolean {
		...
		if (d.context.technicianName.trim() === '') {
			found.push({
				id: 'context-technicianName',
				message: 'Screening technician name is required.'
			});
		}
		if (d.context.technicianRole === '') {
			found.push({ id: 'context-technicianRole', message: 'Technician role is required.' });
		}
	...
	function submit() {
		...
		assessment.result = classifyAaa(assessment.data);
		goto(`/abdominal-aortic-aneurysm-screening/abdominal-aortic-aneurysm-screenings/${id}/report`);
	}
...
<main class="mx-16 px-4 py-6">
	<h1 class="text-2xl font-bold text-base-content">
		{isNew ? 'New AAA screening scan' : `AAA screening ${id}`}
	</h1>
	<p class="mt-1 text-sm text-base-content/70">
		Complete the six sections; the category and surveillance band are computed on submit.
	</p>
	...
	<Form label="AAA screening scan" onsubmit={submit}>
		<Step1Context />
		<Step2Identification />
		<Step3Consent />
		<Step4Measurement />
