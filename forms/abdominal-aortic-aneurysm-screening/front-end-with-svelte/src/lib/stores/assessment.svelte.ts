...
function storageKey(id: string): string {
	return `abdominal-aortic-aneurysm-screening.front-end-with-svelte.${id || 'new'}.v1`;
}

/** A blank AAA screening assessment with all fields at their unanswered defaults. */
...
/**
 * Svelte 5 reactive store for the AAA screening assessment, with localStorage
 * persistence so an in-progress scan record survives a page reload. Drafts are
...
class AssessmentStore {
	 ...
	 * (e.g. `const d = assessment.data.measurement`) stay bound to live state.
	 */
