# Provider Transfer Request — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Provider Transfer Request form, built with axum + Loco +
...
- [`provider_transfer_request/`](./provider_transfer_request/) — the Loco crate: `src/provider_transfer_request/` holds `app.rs`
...
`src/provider_transfer_request/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd provider_transfer_request && cargo check --all-targets && cargo test
```
