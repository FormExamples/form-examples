...
impl ActiveModel {
    /// Create a new in-progress transfer request with an empty data blob.
...
/// Find a transfer request by its UUID.
pub async fn find_by_id(db: &DatabaseConnection, id: Uuid) -> Result<Option<Model>, DbErr> {
    Entity::find_by_id(id).one(db).await
}

/// List all completed transfer requests (those with a grading result).
...
/// List transfer requests with pagination (page is 0-indexed).
