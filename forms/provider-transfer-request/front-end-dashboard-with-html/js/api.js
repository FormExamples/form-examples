...
/**
 * Fetch the transfer-request list from the backend.
 *
...
async function fetchPatients() {
  ...
  if (!res.ok) {
    throw new Error(
      `Failed to fetch transfers: ${res.status} ${res.statusText}`
    );
  }
...
window.ProviderTransferRequestDashboard.fetchPatients = fetchPatients;
window.ProviderTransferRequestDashboard.API_BASE = API_BASE;
})();
