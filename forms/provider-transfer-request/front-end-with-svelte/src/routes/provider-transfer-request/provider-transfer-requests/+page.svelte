<script lang="ts">
	...
	import { completenessLabel, urgencyLabel } from '#lib/engine/utils.js';
	...
	function init(api: any) {
		...
		api.on('select-row', (ev: { id?: string | number }) => {
			if (ev?.id != null) goto(`/provider-transfer-request/provider-transfer-requests/${ev.id}`);
		});
	}
</script>
...
<main class="mx-16 px-4 py-6">
	<div class="mb-6 flex flex-wrap items-end justify-between gap-4">
		...
		<a href="/provider-transfer-request/provider-transfer-requests/new" class="button" data-variant="primary">New transfer</a>
	</div>
