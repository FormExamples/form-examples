...
//
// Each rule maps to one tracked field. Rules tagged `mandatory: true` are
// required for a safe handover; a missing mandatory field downgrades
// completeness to "Incomplete", while missing non-mandatory fields downgrade
// to "Partial". Conditional rules (escort details when an escort is required,
// infectious-precaution detail when ticked, receiving-provider acknowledgement
// gated until any acknowledgement field is touched) are guarded by `applies()`.

import type { AssessmentData, ValidationRule } from './types';
import { hasText, isYesNoUnknownAnswered } from './utils';

/** True if any field on the receiving-provider acknowledgement has been touched. */
export function acknowledgementStarted(data: AssessmentData): boolean {
