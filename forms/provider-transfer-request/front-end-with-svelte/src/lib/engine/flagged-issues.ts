...
// completeness check (handled by the validator), this module raises clinically
// significant flags for unstable patients, abnormal vital signs, emergent
// transfer urgency, infectious precautions, and several safety/safeguarding
// concerns. Priorities (urgent -> high -> medium -> low) drive sort order in
// the report.

import type { AssessmentData, FlaggedIssue } from './types';
import { hasNumber, hasText } from './utils';
...
export function detectFlaggedIssues(data: AssessmentData): FlaggedIssue[] {
	...
	if (log.infectiousPrecautions) {
		flags.push({
			...
			message: `Infectious precautions flagged: ${
	...
	if (log.escortRequired) {
		flags.push({
			id: 'FLAG-LOG-ESCORT',
			category: 'Transport requirement',
			message: `Clinical escort required: ${
