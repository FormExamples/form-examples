...
// categories mirror the CHECK constraint in
...
function detectFlags(r) {
  ...
  if (r.hypertensionConfirmed && !hasCriticalFinding(r)) {
    flags.push({
      ...
      priority: 'high',
      description:
        'Hypertension is confirmed on ambulatory averages and requires action.',
      suggestedAction:
  ...
  if (r.maskedHypertension || r.nocturnalHypertension || nighttimeHypertensive(r)) {
    flags.push({
      ...
      suggestedAction:
        'Consider referral to a hypertension specialist for further assessment.'
    });
  }
  ...
  if (recordingIsInadequate(r)) {
    flags.push({
      ...
      suggestedAction:
        'Consider repeating the monitoring period to obtain an adequate study.'
    });
  }
  ...
  if (r.whiteCoatEffect) {
    flags.push({
      ...
      priority: 'low',
      description:
        'A white-coat effect is documented (raised clinic BP but normal ambulatory averages).',
      suggestedAction:
        'Manage per the relevant white-coat-hypertension pathway and document.'
    });
  }
  ...
  if (
    ...
    flags.push({
      ...
      suggestedAction:
        'Record the averaged blood-pressure measurements for interpretation.'
    });
  }
