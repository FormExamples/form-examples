<script lang="ts">
	...
	import Step2RecordingAdequacy from '$lib/components/steps/Step2RecordingAdequacy.svelte';
	import Step3ClinicalHistory from '$lib/components/steps/Step3ClinicalHistory.svelte';
	import Step4AverageBloodPressures from '$lib/components/steps/Step4AverageBloodPressures.svelte';
	import Step5NocturnalDippingFindings from '$lib/components/steps/Step5NocturnalDippingFindings.svelte';
	...
	function validate(): boolean {
		...
		if (d.monitoringType === '') {
			found.push({ id: 'monitoringType', message: 'Monitoring type is required.' });
		}
		if (d.reportStatus === '') {
	...
	function submit() {
		...
		goto('/ambulatory-blood-pressure-test-result/report/view');
	}
...
<main class="mx-16 px-4 py-6">
	<h1 class="text-2xl font-bold text-gray-900">New ABPM report</h1>
	...
	<Form label="Ambulatory blood pressure test result" onsubmit={submit}>
		<Step1ReportIdentification />
		<Step2RecordingAdequacy />
		<Step3ClinicalHistory />
		<Step4AverageBloodPressures />
		<Step5NocturnalDippingFindings />
		<Step6Impression />
