...
import type { AmbulatoryBloodPressureResult, GradingResult } from '#lib/engine/types.js';
import {
...
/** Formats a systolic/diastolic average pair as "120/76 mmHg" or "N/A". */
function bp(systolic: number | null, diastolic: number | null): string {
	if (systolic === null && diastolic === null) return 'N/A';
	return `${systolic ?? '—'}/${diastolic ?? '—'} mmHg`;
}

/** Builds the pdfmake document definition for the ABPM report. */
export function buildPdfDocument(
	data: AmbulatoryBloodPressureResult,
	...
	return {
		...
		header: {
			text: 'AMBULATORY BLOOD PRESSURE (ABPM) REPORT',
			alignment: 'center',
			margin: [0, 20, 0, 0],
			fontSize: 10,
			color: '#6b7280',
		...
		footer: (currentPage: number, pageCount: number) => ({
			text: `Page ${currentPage} of ${pageCount} | Generated ${new Date(result.gradedAt).toLocaleString()} | NICE NG136 / BIHS / ESH ABPM`,
			alignment: 'center',
			margin: [0, 20, 0, 0],
			fontSize: 8,
			color: '#9ca3af'
		...
		content: [
			{
				text: 'AMBULATORY BLOOD PRESSURE TEST RESULT',
				fontSize: 18,
				bold: true,
				alignment: 'center',
			...
			{
				text: `${monitoringTypeLabel(data.monitoringType)} — ${reportStatusLabel(data.reportStatus)}`,
				fontSize: 12,
				alignment: 'center',
				color: '#4b5563',
				margin: [0, 0, 0, 4] as Margin
			...
			// Recording adequacy
			sectionHeader('Recording adequacy'),
			{
				ul: [
					`Monitoring type: ${monitoringTypeLabel(data.monitoringType)}`,
					`Valid readings: ${data.validReadingsPercent !== null ? `${data.validReadingsPercent}%` : 'N/A'}`,
					`Recording adequate: ${data.recordingAdequate ? 'Yes' : 'No'}`
				],
				margin: [0, 0, 0, 16] as Margin
			},
			...
			sectionHeader('Average blood pressures'),
			{
				ul: [
					`Daytime average: ${bp(data.daytimeAverageSystolic, data.daytimeAverageDiastolic)}`,
					`Nighttime average: ${bp(data.nighttimeAverageSystolic, data.nighttimeAverageDiastolic)}`,
					`24-hour average: ${bp(data.twentyFourHourAverageSystolic, data.twentyFourHourAverageDiastolic)}`
				],
				margin: [0, 0, 0, 16] as Margin
			},

			// Nocturnal dipping
			sectionHeader('Nocturnal dipping'),
			{
				ul: [
					`Nocturnal dip: ${data.nocturnalDipPercent !== null ? `${data.nocturnalDipPercent}%` : 'N/A'}`,
					`Dipper status: ${dipperStatusLabel(data.dipperStatus)}`
				],
				margin: [0, 0, 0, 16] as Margin
			},
...
function structuredFindingList(data: AmbulatoryBloodPressureResult): string[] {
	const list: string[] = [];
