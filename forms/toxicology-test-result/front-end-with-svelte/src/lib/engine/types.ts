// ──────────────────────────────────────────────
// Toxicology Test Result — core data types
//
// Field names are camelCase (front-end serde); they mirror the snake_case
// columns in sql/04_create_table_toxicology_test_result.sql and
...
export type SpecimenCondition = 'satisfactory' | 'insufficient' | 'delayed' | '';

/** Paracetamol treatment-nomogram interpretation. */
export type ParacetamolNomogram =
 ...
 */
export interface ToxicologyResult {
	...
	paracetamolLevelMgL: number | null;
	salicylateLevelMgL: number | null;
	ethanolLevel: number | null;
	lithiumLevelMmolL: number | null;
	digoxinLevel: number | null;
	carboxyhaemoglobinPercent: number | null;
	drugsOfAbuseScreen: string;
	specificDrugLevel: string;

	// Interpretation
	paracetamolNomogram: ParacetamolNomogram;
	overallResultStatus: OverallResultStatus;
	toxicLevelPresent: boolean;
