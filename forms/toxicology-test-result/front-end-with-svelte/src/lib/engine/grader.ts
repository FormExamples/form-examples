import type { ToxicologyResult, GradingResult, Recommendation, FiredRule } from './types';
...
/**
 * Pure four-axis interpretation engine for a toxicology test result.
 *
 * Computes:
 ...
 *
 * Invariant: a toxic result (paracetamol above the treatment line, or
 * `toxicLevelPresent`) auto-escalates Axis D to critical-alert and raises the
 * critical-result-alert flag, with an urgent antidote (NAC) action, regardless
 * of the other axes. The least-urgent band is only chosen when no rule fires.
 *
 * No side effects, no network calls, no I/O.
 */
export function calculateGrade(result: ToxicologyResult): GradingResult {
