 ...
 * categories mirror sql/07_create_table_toxicology_test_result_grade_flag.sql.
 * Flags are returned sorted high → medium → low priority.
 */
export function detectFlags(r: ToxicologyResult): Flag[] {
	const flags: Flag[] = [];
	...
	if (r.specimenCondition === 'insufficient' || r.specimenCondition === 'delayed') {
		flags.push({
			...
			description: `Specimen condition is ${r.specimenCondition}; interpretive confidence may be reduced.`,
