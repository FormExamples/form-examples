...
/** A blank toxicology result with all fields at their unanswered defaults. */
export function createDefaultResult(): ToxicologyResult {
	return {
		reportingClinician: '',
...
/**
 * Svelte 5 reactive store for the toxicology result, with localStorage
 * persistence so an in-progress report survives a page reload.
 */
class ResultStore {
	data = $state<ToxicologyResult>(createDefaultResult());
	...
	constructor() {
		if (browser) {
			...
			if (raw) {
				try {
					const parsed = JSON.parse(raw) as Partial<ToxicologyResult>;
