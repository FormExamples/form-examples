...
/** Builds the pdfmake document definition for the toxicology report. */
export function buildPdfDocument(
	data: ToxicologyResult,
	...
	return {
		...
		footer: (currentPage: number, pageCount: number) => ({
			text: `Page ${currentPage} of ${pageCount} | Generated ${new Date(result.gradedAt).toLocaleString()} | TOXBASE / NPIS / MHRA`,
			alignment: 'center',
			margin: [0, 20, 0, 0],
			fontSize: 8,
			color: '#9ca3af'
		...
		content: [
			...
			{
				text: `${suspectedAgentLabel(data.suspectedAgent)} — ${reportStatusLabel(data.reportStatus)}`,
				fontSize: 12,
				alignment: 'center',
				color: '#4b5563',
				margin: [0, 0, 0, 4] as Margin
			...
			// Specimen and history
			sectionHeader('Specimen & history'),
			{
				ul: [
					`Specimen condition: ${specimenConditionLabel(data.specimenCondition)}`,
					`Suspected agent: ${suspectedAgentLabel(data.suspectedAgent)}`,
					`Time since ingestion: ${data.timeSinceIngestionHours !== null ? `${data.timeSinceIngestionHours} h` : 'N/A'}`
				],
				margin: [0, 0, 0, 8] as Margin
			},
			{ text: `Clinical history: ${data.clinicalHistory || 'Not specified'}`, margin: [0, 0, 0, 16] as Margin },
			...
			{
				ul: [
					`Paracetamol nomogram: ${paracetamolNomogramLabel(data.paracetamolNomogram)}`,
					`Overall result status: ${overallResultStatusLabel(data.overallResultStatus)}`,
					`Toxic level present: ${data.toxicLevelPresent ? 'Yes' : 'No'}`
				],
				margin: [0, 0, 0, 8] as Margin
			},
...
function resultValueList(data: ToxicologyResult): string[] {
	const list: string[] = [];
