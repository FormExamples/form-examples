...
import { abnormalitySeverityClass, abnormalitySeverityLabel, followUpUrgencyClass, followUpUrgencyLabel, reportStatusLabel, resultClassificationClass, resultClassificationLabel, suspectedAgentLabel } from './types.js';

// Toxicology Test Result — clinician dashboard (vanilla classic-script app).
//
...
// box + classification dropdown + follow-up-urgency dropdown). Columns mirror
// the SvelteKit dashboard route
// (`front-end-with-svelte/src/routes/+page.svelte`).
//
// Sibling modules loaded as plain `<script>` tags (in dependency order) attach
// their exports to `window.ToxicologyTestResultDashboard`; display labels come
// from the form namespace `window.ToxicologyTestResult` (js/types.js). The
...
const filters = {
  search: '',          // matches report id, patient name, or suspected agent
...
const columns = [
  ...
  { key: 'suspectedAgent',            label: 'Suspected agent' },
...
function matchesFilters(row) {
  if (filters.search) {
    ...
    const matches =
      ...
      row.suspectedAgent.toLowerCase().includes(term);
...
function compareRows(a, b) {
  ...
  // Default: string compare (id, patientName, suspectedAgent, reportStatus,
...
function renderTableBody() {
  ...
  for (const row of rows) {
    ...
    tr.innerHTML = `
      ...
      <td>${esc(suspectedAgentLabel(row.suspectedAgent))}</td>
