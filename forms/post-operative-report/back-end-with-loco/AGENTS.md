# Post-Operative Report — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Post-Operative Report form, built with axum + Loco +
...
- [`post_operative_report/`](./post_operative_report/) — the Loco crate: `src/post_operative_report/` holds `app.rs`
...
`src/post_operative_report/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd post_operative_report && cargo check --all-targets && cargo test
```
