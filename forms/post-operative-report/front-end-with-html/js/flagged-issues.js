// Flagged-issue detection. Independent of the Clavien-Dindo overall grade,
// this module raises clinician-facing flags for high-risk vital signs,
// substantial blood loss, prolonged surgery, ASA class, conversion to open,
// difficult airway, hypothermia, oxygen desaturation, severe pain,
// life-threatening complications, ICU disposition, and missing critical
...
function detectAdditionalFlags(data) {
  ...
  // ─── Mortality (Grade V) ─────────────────────────────────
  for (let i = 0; i < (comps.complications || []).length; i++) {
    const c = comps.complications[i];
    if (c && c.grade === 'grade-v') {
      flags.push({
        id: 'FLAG-CD-V',
        category: 'Complications',
        message: 'Grade V complication (death) recorded — escalate to mortality review and ensure family notification, coroner referral, and clinical governance reporting.',
        priority: 'urgent'
      });
      break;
    }
  ...
  if (proc.priority === 'emergency') {
    flags.push({
      ...
      category: 'Procedure',
      message: 'Emergency procedure — heightened risk of post-operative deterioration; ensure close observation.',
      priority: 'medium'
    });
  }
  ...
  if (anaes.difficultIntubation === 'yes') {
    flags.push({
      ...
      message: 'Difficult airway encountered — document in patient record and alert future anaesthetists.',
      priority: 'high'
    });
  }
