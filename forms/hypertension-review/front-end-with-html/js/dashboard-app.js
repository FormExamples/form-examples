...
// Hypertension Annual Review — clinician dashboard (vanilla classic-script app).
//
...
const filters = {
  search: '',
  control: '',  // '' | 'controlled' | 'uncontrolled' | 'severe-uncontrolled'
  review: '',   // '' | 'complete' | 'partial' | 'incomplete'
  stage: '',    // '' | 'none' | 'stage-1' | 'stage-2' | 'stage-3-severe'
  flag: ''      // '' | 'yes' | 'no'
};
...
const columns = [
  ...
  { key: 'practiceSite',        label: 'Practice' },
  { key: 'controlStatus',       label: 'Control' },
  { key: 'reviewStatus',        label: 'Review' },
  { key: 'hypertensionStage',   label: 'Stage' },
  { key: 'severeFlag',          label: 'Severe Flag' }
];

// Ranks used when sorting the enum columns so order is locale-independent.
const controlRank = { 'controlled': 0, 'uncontrolled': 1, 'severe-uncontrolled': 2 };
const reviewRank = { 'complete': 0, 'partial': 1, 'incomplete': 2 };
const stageRank = { 'none': 0, 'stage-1': 1, 'stage-2': 2, 'stage-3-severe': 3 };
...
function controlClass(status) {
  switch (status) {
    case 'controlled': return 'risk-low';
    case 'uncontrolled': return 'risk-high';
    case 'severe-uncontrolled': return 'risk-critical';
    default: return '';
  }
}
...
function stageLabel(stage) {
  switch (stage) {
    ...
    case 'stage-3-severe': return 'Stage 3';
    default: return 'N/A';
  }
}
...
function matchesFilters(row) {
  if (filters.search) {
    ...
    const matches =
      ...
      row.practiceSite.toLowerCase().includes(term);
    if (!matches) return false;
  }
  if (filters.control && row.controlStatus !== filters.control) return false;
  if (filters.review && row.reviewStatus !== filters.review) return false;
  if (filters.stage && row.hypertensionStage !== filters.stage) return false;
  if (filters.flag === 'yes' && !row.severeFlag) return false;
  if (filters.flag === 'no' && row.severeFlag) return false;
  return true;
}

/**
 * Compare two rows for the active sort column. Enum columns use their rank
 * tables; the severe-flag boolean sorts false<true; everything else uses a
...
function compareRows(a, b) {
  ...
  // Default: string compare (patientIdentifier, patientName, practiceSite)
...
function renderTableBody() {
  ...
  for (const row of rows) {
    ...
    if (row.controlStatus === 'severe-uncontrolled' || row.severeFlag) {
    ...
    tr.innerHTML = `
      ...
      <td>${esc(row.practiceSite)}</td>
      <td><span class="risk-badge ${controlClass(row.controlStatus)}">${esc(controlLabel(row.controlStatus))}</span></td>
      <td><span class="risk-badge ${reviewClass(row.reviewStatus)}">${esc(reviewLabel(row.reviewStatus))}</span></td>
      <td><span class="risk-badge ${stageClass(row.hypertensionStage)}">${esc(stageLabel(row.hypertensionStage))}</span></td>
      <td>
