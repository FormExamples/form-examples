# Hypertension Annual Review: Back-end with Rust Axum Loco (JSON API)

Server-side Rust JSON API for the Hypertension Annual Review form. Pure back-end: no HTML
rendering, no Tera templates, no HTMX, no Alpine.js, no CSS, no Lily
Design System.

The Loco crate lives in [`hypertension_review/`](hypertension_review/) — one migration, SeaORM
