...
impl MigrationTrait for Migration {
    async fn up(&self, m: &SchemaManager) -> Result<(), DbErr> {
        create_table(m, "histopathology_test_request_grades",
            &[
            ...
            ("specimen_quality_band", ColType::StringWithDefault(String::new())),
            ...
            &[
            ("histopathology_test_request", ""),
            ]
        ).await
    }

    async fn down(&self, m: &SchemaManager) -> Result<(), DbErr> {
        drop_table(m, "histopathology_test_request_grades").await
    }
}
