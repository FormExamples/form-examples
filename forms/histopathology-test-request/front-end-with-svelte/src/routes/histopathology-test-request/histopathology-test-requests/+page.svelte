<script lang="ts">
	...
	const columns = [
		...
		{
			id: 'specimenQualityBand',
			header: 'Specimen quality',
			width: 150,
			template: (v: string) => specimenQualityLabel(v as never)
		},
	...
	function init(api: any) {
		...
		api.on('select-row', (ev: { id?: string | number }) => {
			if (ev?.id != null) goto(`/histopathology-test-request/histopathology-test-requests/${ev.id}`);
		});
	}
</script>
...
<main class="mx-16 px-4 py-6">
	<div class="mb-6 flex flex-wrap items-end justify-between gap-4">
		<div>
			<h1 class="text-2xl font-bold text-base-content">Histopathology vetting dashboard</h1>
			<p class="mt-1 text-sm text-base-content/70">
				Four-axis vetting grades for incoming histopathology requests, computed by the shared engine.
				Select a row to open the request.
			</p>
		</div>
		<a href="/histopathology-test-request/histopathology-test-requests/new" class="button" data-variant="primary">New request</a>
	</div>
