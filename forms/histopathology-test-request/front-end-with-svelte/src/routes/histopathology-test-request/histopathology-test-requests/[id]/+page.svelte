<script lang="ts">
	...
	import Step3Specimen from '#lib/components/steps/Step3Specimen.svelte';
	import Step4IndicationAndClinicalContext from '#lib/components/steps/Step4IndicationAndClinicalContext.svelte';
	import Step5UrgencyAndRedFlags from '#lib/components/steps/Step5UrgencyAndRedFlags.svelte';
	import Step6RequesterAndSite from '#lib/components/steps/Step6RequesterAndSite.svelte';
	import Step7ReviewAndSubmit from '#lib/components/steps/Step7ReviewAndSubmit.svelte';
	...
	function validate(): boolean {
		...
		if (d.specimen.specimenSite.trim() === '') {
			found.push({ id: 'specimenSite', message: 'Anatomical site is required.' });
		}
	...
	function submit() {
		...
		goto(`/histopathology-test-request/histopathology-test-requests/${id}/report`);
	}
...
<main class="mx-16 px-4 py-6">
	<h1 class="text-2xl font-bold text-base-content">
		{isNew ? 'New histopathology request' : `Histopathology request ${id}`}
	</h1>
	...
	<Form label="Histopathology test request" onsubmit={submit}>
		<Step1RequestingClinician />
		<Step2PatientIdentification />
		<Step3Specimen />
		<Step4IndicationAndClinicalContext />
		<Step5UrgencyAndRedFlags />
		<Step6RequesterAndSite />
