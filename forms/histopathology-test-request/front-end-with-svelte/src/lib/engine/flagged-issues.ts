import type { HistopathologyRequest, Flag, SpecimenQualityBand } from './types';

/**
 * Detects safety-critical flags independently of the four axes. Flag categories
 * mirror the grade_flag CHECK constraint: suspected-cancer-2ww,
 * frozen-section-urgent, specimen-fixation-issue, mislabel-risk,
 * missing-clinical-details, missing-indication, other. Flag IDs are stable and
 * identical across every front-end and the back-end.
 */
export function detectFlags(
	data: HistopathologyRequest,
	context?: { specimenQualityBand?: SpecimenQualityBand }
): Flag[] {
	const flags: Flag[] = [];
	const ctx = context || {};
	...
	if (
		...
		flags.push({
			...
			suggestedAction:
				'Alert the on-call pathologist immediately; the patient is in theatre awaiting an immediate diagnosis.'
		});
	}
	if (
		...
		flags.push({
			...
			suggestedAction:
				'Expedite accessioning and reporting on the suspected-cancer (NICE NG12) pathway.'
		});
	}
	...
	if (ctx.specimenQualityBand === 'reject-risk') {
		flags.push({
			...
			priority: 'high',
			description:
				'Specimen fixation is inadequate (fresh / unfixed outside a frozen-section pathway, or no fixative recorded).',
			suggestedAction:
				'Contact the requester urgently; the specimen may be unfit for diagnosis without prompt fixation.'
		});
	...
	if (data.specimen.specimenType && data.specimen.specimenLabelled !== true) {
		flags.push({
			...
			suggestedAction:
				'Verify the specimen container is labelled with patient identifiers matching the request before accessioning.'
		});
	}
