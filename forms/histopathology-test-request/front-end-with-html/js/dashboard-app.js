// Histopathology Test Request — vetting dashboard (vanilla classic-script app).
//
...
// specimen-quality dropdown + safety-flags dropdown).
//
// Sibling modules loaded as plain `<script>` tags (in dependency order)
// attach their exports to `window.HistopathologyTestRequestDashboard`.
...
const columns = [
  ...
  { key: 'specimenQualityBand', label: 'Specimen Quality' },
...
function matchesFilters(row) {
  ...
  if (filters.quality && row.specimenQualityBand !== filters.quality) return false;
...
function compareRows(a, b) {
  ...
  if (key === 'specimenQualityBand') {
    return ((qualityRank[av] ?? -1) - (qualityRank[bv] ?? -1)) * dir;
  }
  if (key === 'triageTier') {
...
function renderTableBody() {
  ...
  for (const row of rows) {
    ...
    if (row.triageTier === 'two-week-wait' || row.specimenQualityBand === 'reject-risk') {
    ...
    tr.innerHTML = `
      ...
      <td class="data-table-td"><span class="quality-badge ${qualityClass(row.specimenQualityBand)}">${esc(titleCase(row.specimenQualityBand))}</span></td>
