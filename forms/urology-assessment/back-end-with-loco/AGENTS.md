# Urology Assessment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Urology Assessment form, built with axum + Loco +
...
- [`urology_assessment/`](./urology_assessment/) — the Loco crate: `src/urology_assessment/` holds `app.rs`
...
`src/urology_assessment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd urology_assessment && cargo check --all-targets && cargo test
```
