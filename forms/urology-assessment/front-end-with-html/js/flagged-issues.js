// Flagged-issue detection for the Urology Assessment. Independent of the
// IPSS total (which the grader computes), this module raises clinician-
// facing flags for severe symptoms, hematuria, elevated PSA, renal
// impairment, urinary retention, recurrent UTI, family history of cancers
// or kidney disease, medication interactions, neurogenic bladder, and
// erectile dysfunction with significant LUTS.
//
// Mirrors the SvelteKit reference engine (`flagged-issues.ts`).
...
function detectAdditionalFlags(data) {
  ...
  const totalIPSS = scores.reduce((sum, s) => sum + (s == null ? 0 : s), 0);
  if (totalIPSS >= 20) {
    flags.push({
      id: 'FLAG-SEVERE-001',
