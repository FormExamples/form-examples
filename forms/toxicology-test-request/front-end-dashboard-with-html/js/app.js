...
// timing-band dropdown + safety-flags dropdown).
//
...
/** Human-readable label for an assay field. */
function assayLabel(field) {
  return (ASSAY_LABELS && ASSAY_LABELS[field]) || titleCase(field);
}

/** Comma-joined assay labels for searching / rendering. */
function assayText(assays) {
  if (!Array.isArray(assays) || assays.length === 0) return '';
  return assays.map(assayLabel).join(', ');
}
...
function timingClass(timing) {
  return timing ? 'timing-' + String(timing) : '';
}
...
function matchesFilters(row) {
  if (filters.search) {
    ...
    const matches =
      ...
      assayText(row.assays).toLowerCase().includes(term) ||
  ...
  if (filters.timing && row.timingBand !== filters.timing) return false;
...
function compareRows(a, b) {
  ...
  if (key === 'timingBand') {
    return ((timingRank[av] ?? -1) - (timingRank[bv] ?? -1)) * dir;
  }
  if (key === 'triageTier') {
...
function renderAssaysCell(assays) {
  if (!Array.isArray(assays) || assays.length === 0) {
    return '<span class="flag-empty">none</span>';
  }
  const chips = assays
    .map((f) => `<span class="assay-chip">${esc(assayLabel(f))}</span>`)
    .join('');
  return `<div class="assay-cell-list">${chips}</div>`;
}
...
function renderTableBody() {
  ...
  for (const row of rows) {
    ...
    tr.innerHTML = `
      ...
      <td class="data-table-td">${renderAssaysCell(row.assays)}</td>
      ...
      <td class="data-table-td"><span class="timing-badge ${timingClass(row.timingBand)}">${esc(titleCase(row.timingBand))}</span></td>
