<script lang="ts">
	...
	import Step3RequestedAssays from '#lib/components/steps/Step3RequestedAssays.svelte';
	...
	function submit() {
		...
		goto(`/toxicology-test-request/toxicology-test-requests/${id}/report`);
	}
...
<main class="mx-16 px-4 py-6">
	<h1 class="text-2xl font-bold text-base-content">
		{isNew ? 'New toxicology test request' : `Toxicology test request ${id}`}
	</h1>
	<p class="mt-1 text-sm text-base-content/70">
		Complete the six sections; the four-axis vetting grade and safety flags are computed on submit.
	</p>
	...
	<Form label="Toxicology test request" onsubmit={submit}>
		<Step1RequestingClinician />
		<Step2PatientIdentification />
		<Step3RequestedAssays />
