...
import type { ToxicologyRequest, GradingResult } from '$lib/engine/types';
...
function storageKey(id: string): string {
	return `toxicology-test-request.front-end-with-svelte.${id || 'new'}.v1`;
}

/**
 * Svelte 5 reactive store for the toxicology test request, with localStorage
 ...
 */
class RequestStore {
	data = $state<ToxicologyRequest>(createDefaultRequest());
	 ...
	 * sample request for an existing id), falling back to a blank draft.
	 *
	 * The data is merged in place (nested object identities preserved) rather
	 * than reassigned, so step components that captured a section reference
	 * (e.g. `const d = request.data.clinical`) stay bound to live state.
	 */
	loadForId(id: string, seed?: ToxicologyRequest) {
		const key = id || 'new';
		this.id = key;
		this.result = null;
		this.currentStep = 1;

		let draft: ToxicologyRequest | null = null;
		if (browser) {
			const raw = localStorage.getItem(storageKey(key));
			if (raw) {
				try {
					draft = JSON.parse(raw) as ToxicologyRequest;
				} catch {
