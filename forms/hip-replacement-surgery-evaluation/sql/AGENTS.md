# Hip Replacement Surgery Evaluation — PostgreSQL migrations
...
`hip_replacement_surgery_evaluation(id) ON DELETE CASCADE` with a UNIQUE
constraint on the foreign key to enforce 1:1. The
`hip_replacement_surgery_evaluation_grade_flag` child references the grade and
is 1:many.
