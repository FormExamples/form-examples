// Flagged-issue detection. Independent of the rule-grading engine, this
// module raises safety-critical alerts for the safety team. Mirrors the
// SvelteKit `flagged-issues.ts`.
...
function detectAdditionalFlags(data) {
  ...
  const merp = data.errorClassification.nccMerpCategory;
  if (merp === 'H' || merp === 'I') {
    flags.push({
      id: 'FLAG-MERP-HIGH-001',
      category: 'NCC MERP',
      message: `NCC MERP Category ${merp} - ${merp === 'I' ? 'patient death' : 'intervention to sustain life'}`,
      priority: 'high'
    });
  }
  ...
  if (data.patientOutcome.extendedHospitalStay === 'yes') {
    flags.push({
      id: 'FLAG-STAY-001',
      category: 'Patient Outcome',
      message: `Extended hospital stay: ${data.patientOutcome.extraDays != null ? data.patientOutcome.extraDays : '?'} extra days`,
      priority: 'medium'
    });
  }
