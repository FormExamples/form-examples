...
use crate::engine::{error_grader, types::AssessmentData};
...
/// GET /assessment/{id} -- render the single-page report wizard.
#[debug_handler]
...
fn section_to_json(section: &str) -> &str {
    match section {
        ...
        "patient_involvement" => "patientInvolvement",
        ...
        "reporting_followup" => "reportingFollowup",
        other => other,
    }
}
...
/// Set of numeric field names (camelCase) that should be coerced to JSON
/// numbers rather than strings.
fn is_numeric_field(field: &str) -> bool {
    matches!(field, "patientAgeAtIncident" | "extraDays")
}

/// Parse a form value for a given camelCase field name. Empty strings for
/// numeric fields become JSON null; otherwise empty strings stay as `""`.
...
async fn show_report(
    ...
    let grade = error_grader::calculate_error_grade(&assessment_data);
    ...
    context.insert("who_severity", &grade.who_severity);
    context.insert("ncc_merp_category", &grade.ncc_merp_category);
    context.insert("overall_risk", &grade.overall_risk);
    context.insert(
        "who_severity_label",
        &crate::engine::utils::who_severity_label(&grade.who_severity),
    );
    context.insert(
        "ncc_merp_label",
        &crate::engine::utils::ncc_merp_label(&grade.ncc_merp_category),
    );
    context.insert(
        "risk_level_label",
