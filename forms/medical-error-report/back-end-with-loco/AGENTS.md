# Medical Error Report — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Medical Error Report form, built with axum + Loco +
...
- [`medical_error_report/`](./medical_error_report/) — the Loco crate: `src/medical_error_report/` holds `app.rs`
...
`src/medical_error_report/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd medical_error_report && cargo check --all-targets && cargo test
```
