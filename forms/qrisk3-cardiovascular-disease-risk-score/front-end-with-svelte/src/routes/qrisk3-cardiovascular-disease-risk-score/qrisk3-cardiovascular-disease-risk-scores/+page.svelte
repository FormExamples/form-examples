<script lang="ts">
	...
	// SVAR DataGrid columns. 10-year risk, risk band, and statin offer render
	...
	const columns = [
		...
		{ id: 'heartAge', header: 'Heart age', width: 100, sort: true },
		{
			id: 'statinOffer',
			header: 'Statin',
			width: 90,
	...
	function init(api: any) {
		...
		api.on('select-row', (ev: { id?: string | number }) => {
			if (ev?.id != null) goto(`/qrisk3-cardiovascular-disease-risk-score/${plural}/${ev.id}`);
		});
	}
</script>
...
<main class="mx-auto max-w-6xl px-4 py-6">
	<div class="mb-6 flex flex-wrap items-end justify-between gap-4">
		<div>
			<h1 class="text-2xl font-bold text-base-content">QRISK3 clinician dashboard</h1>
			<p class="mt-1 text-sm text-base-content/70">
				10-year CVD risk, risk band, and statin offer for assessed patients, computed by the shared
				engine. Select a row to open the assessment.
			</p>
		</div>
		<a href="/qrisk3-cardiovascular-disease-risk-score/{plural}/new" class="button" data-variant="primary">New assessment</a>
	</div>
	...
	<div class="mb-4 flex flex-wrap gap-4">
		<label class="text-sm">
			...
			<select class="select inline-block w-auto" bind:value={settingFilter}>
				...
				<option value="nhs-health-check">NHS Health Check</option>
