// Flagged-issue detection (red flags). Independent of the numeric result (which
...
//
// Rows here mirror the `qrisk3_cardiovascular_disease_risk_score_grade_flag`
 ...
 */
function detectFlaggedIssues(data, tenYearRiskPercent) {
  /** @type {FlaggedIssue[]} */
  const flags = [];
  ...
  if (elig.hasFamilialHypercholesterolaemia === 'yes') eligibilityReasons.push('familial hypercholesterolaemia');
  ...
  if (cm.cholesterolHdlRatio === null) {
    flags.push({
      ...
      priority: 'medium',
      description: 'Total-cholesterol : HDL ratio not recorded — the risk estimate cannot be computed accurately',
      suggestedAction:
        'Obtain a lipid profile (non-fasting) and re-calculate the score.'
    });
  }
  ...
  if (missing.length > 0) {
    flags.push({
      ...
      suggestedAction:
        'Record the missing measurement(s) and re-calculate.'
    });
  }
