...
// QRISK3 — clinician dashboard (vanilla classic-script app).
//
...
// box + care-setting dropdown + risk-band dropdown + statin-flag dropdown).
//
// Sibling modules loaded as plain `<script>` tags (in dependency order) attach
// their exports to `window.Qrisk3CardiovascularDiseaseRiskScoreDashboard`. The
...
const filters = {
  search: '',
  setting: '',  // '' | 'general-practice' | 'pharmacy' | 'nhs-health-check' | 'other'
...
const columns = [
  ...
  { key: 'tenYearRiskPercent', label: '10-yr Risk' },
  { key: 'riskBand',           label: 'Risk Band' },
  { key: 'statinFlag',         label: 'Statin' }
];

// Rank used when sorting the riskBand column so 'low' < 'raised' < 'high'
...
const settingLabels = {
  ...
  'pharmacy': 'Pharmacy',
  'nhs-health-check': 'NHS Health Check',
  'other': 'Other'
};
...
function riskLabel(risk) {
  return (risk === null || risk === undefined) ? 'N/A' : `${risk}%`;
}
...
function matchesFilters(row) {
  ...
  if (filters.statin === 'yes' && !row.statinFlag) return false;
  if (filters.statin === 'no' && row.statinFlag) return false;
  return true;
}

/**
 * Compare two rows for the active sort column. The risk-band column uses its
 * rank table; the nullable risk percentage sorts nulls last; the statin boolean
...
function renderTableBody() {
  ...
  for (const row of rows) {
    ...
    const riskClassName = row.tenYearRiskPercent === null
    ...
    tr.innerHTML = `
      ...
      <td><span class="${riskClassName}">${esc(riskLabel(row.tenYearRiskPercent))}</span></td>
      <td><span class="risk-badge ${bandClass(row.riskBand)}">${esc(bandLabel(row.riskBand))}</span></td>
      <td>
        <span class="flag-badge ${row.statinFlag ? 'flag-yes' : 'flag-no'}">
          ${row.statinFlag ? 'Yes' : 'No'}
        </span>
      </td>
    `;
    body.appendChild(tr);
...
function bindFilterInputs() {
  ...
  const statin = document.getElementById('filter-statin');
  ...
  if (statin) {
    statin.addEventListener('change', () => {
      filters.statin = statin.value;
      renderAll();
    });
  }
  if (clearBtn) {
