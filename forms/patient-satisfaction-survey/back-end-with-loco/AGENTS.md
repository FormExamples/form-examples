# Patient Satisfaction Survey — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Patient Satisfaction Survey form, built with axum + Loco +
...
- [`patient_satisfaction_survey/`](./patient_satisfaction_survey/) — the Loco crate: `src/patient_satisfaction_survey/` holds `app.rs`
...
`src/patient_satisfaction_survey/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd patient_satisfaction_survey && cargo check --all-targets && cargo test
```
