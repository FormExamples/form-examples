...
struct DashboardParams {
    ...
    satisfaction_category: Option<String>,
    visit_type: Option<String>,
    has_flags: Option<String>,
}

/// GET /dashboard -- render the patient-satisfaction dashboard with filters.
#[debug_handler]
async fn dashboard(
    ...
    // Server-side search across patient name, department, and hospital site.
    if let Some(ref term) = params.search {
        let term = term.to_lowercase();
        items.retain(|row| {
            row.patient_name.to_lowercase().contains(&term)
                || row.department.to_lowercase().contains(&term)
                || row.hospital_site.to_lowercase().contains(&term)
        });
    }

    if let Some(ref cat) = params.satisfaction_category {
        if !cat.is_empty() {
            items.retain(|row| row.satisfaction_category == *cat);
        }
    }

    if let Some(ref vt) = params.visit_type {
        if !vt.is_empty() {
            items.retain(|row| row.visit_type == *vt);
        }
    }
    ...
    context.insert(
        ...
        &params.satisfaction_category.unwrap_or_default(),
    );
    context.insert("visit_type", &params.visit_type.unwrap_or_default());
