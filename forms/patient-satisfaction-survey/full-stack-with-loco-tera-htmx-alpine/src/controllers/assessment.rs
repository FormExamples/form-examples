...
use crate::engine::{grader, types::AssessmentData, utils};
...
/// Parse a form value string. Empty strings stay as `""`; integer-looking
/// values are converted to JSON numbers so the typed engine deserializes
/// them as `Option<i32>`.
...
async fn show_report(
    ...
    let grade = grader::grade(&assessment_data);
    ...
    context.insert("normalized_score", &grade.normalized_score);
    context.insert("satisfaction_category", &grade.satisfaction_category);
    context.insert(
        "satisfaction_label",
        &utils::satisfaction_category_label(&grade.satisfaction_category),
    );
    context.insert(
        "satisfaction_class",
        &utils::satisfaction_category_class(&grade.satisfaction_category),
    );
