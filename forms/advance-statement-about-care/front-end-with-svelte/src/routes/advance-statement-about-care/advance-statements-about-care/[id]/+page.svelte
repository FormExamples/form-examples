<script lang="ts">
	...
	// wins, otherwise seed from the matching sample statement (existing id) or a
	// blank draft (new).
	$effect(() => {
	...
	function validate(): boolean {
		...
		if (d.personalInformation.lastName.trim() === '') {
	...
	function submit() {
		...
		assessment.result = calculateCompleteness(assessment.data);
		goto(`/advance-statement-about-care/advance-statements-about-care/${id}/report`);
	}
...
<main class="mx-16 px-4 py-6">
	<h1 class="text-2xl font-bold text-base-content">
		{isNew ? 'New advance statement' : `Advance statement ${id}`}
	</h1>
	<p class="mt-1 text-sm text-base-content/70">
		Complete the nine sections; the completeness assessment and flagged issues are computed on
		submit.
	</p>
	<Progress label="Statement sections" value={TOTAL_STEPS} max={TOTAL_STEPS} />
	<StepList label="Statement sections" current={TOTAL_STEPS}>
	...
	<Form label="Advance statement" onsubmit={submit}>
		...
		<div class="button-group">
			<Button type="submit" data-variant="primary">Assess completeness &amp; view report</Button>
