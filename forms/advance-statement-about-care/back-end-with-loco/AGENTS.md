# Advance Statement About Care — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Advance Statement About Care form, built with axum + Loco +
...
- [`advance_statement_about_care/`](./advance_statement_about_care/) — the Loco crate: `src/advance_statement_about_care/` holds `app.rs`
...
`src/advance_statement_about_care/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd advance_statement_about_care && cargo check --all-targets && cargo test
```
