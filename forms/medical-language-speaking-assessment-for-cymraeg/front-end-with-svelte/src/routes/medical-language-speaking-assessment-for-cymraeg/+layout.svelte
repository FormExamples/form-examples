<script lang="ts">
    ...
    // Local mirror of the store locale so LocalePicker binds a plain string; the
    ...
    // e.g. 'en' -> 'en-GB') and message lookup via t(). LocalePicker's own
    ...
    const localeLabels = Object.fromEntries(
        LOCALE_OPTIONS.map((opt) => [opt.value, opt.label]),
    );
...
<div class="min-h-screen bg-base-200 text-base-content">
    <nav class="border-b border-base-300 bg-base-100 shadow-sm no-print">
        ...
        >
            <a
                href="/medical-language-speaking-assessment-for-cymraeg/"
                class="text-lg font-bold text-base-content">{t("brand")}</a
            >
            <div class="flex items-center gap-1">
                <a
                    href="/medical-language-speaking-assessment-for-cymraeg/"
                    class={navClass("/")}>{t("navWelcome")}</a
                >
                <a href={`/${plural}/new`} class={navClass(`/${plural}/new`)}
                    >{t("navNewAssessment")}</a
                >
                <a href={`/${plural}`} class={navClass(`/${plural}`)}
                    >{t("navDashboard")}</a
                >
                <LocalePicker
                    label={t("localeLabel")}
