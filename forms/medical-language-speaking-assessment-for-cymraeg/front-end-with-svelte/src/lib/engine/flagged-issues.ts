// Flagged-issue detection. Independent of the scaled score (which the grader
// computes), this module raises examiner-facing flags that summarise
// improvement areas, clinical-safety concerns, and borderline outcomes for the
// clinical Welsh-language (Cymraeg) speaking assessment.

import type { AssessmentData, AdditionalFlag, GradingResult, LinguisticField } from './types';

/** The grader output the flag detector needs (core result, before flags). */
type CoreGrading = Omit<GradingResult, 'additionalFlags' | 'timestamp'>;
...
/** Lowest non-null value across the two role-plays for a linguistic criterion. */
function lowestLinguistic(data: AssessmentData, field: LinguisticField): number | null {
...
/** Detect examiner-facing flags for the assessment. */
export function detectAdditionalFlags(data: AssessmentData, grading: CoreGrading): AdditionalFlag[] {
	const flags: AdditionalFlag[] = [];

	const safetyCritical =
		data.rolePlay1?.safetyCriticality === 'high' || data.rolePlay2?.safetyCriticality === 'high';
	...
	const ci = data.clinicalIndicators ?? ({} as AssessmentData['clinicalIndicators']);
	const criticalIndicators: { field: keyof AssessmentData['clinicalIndicators']; label: string }[] = [
