# Medical Language Speaking Assessment for Cymraeg — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Medical Language Speaking Assessment for Cymraeg form, built with axum + Loco +
...
- [`medical_language_speaking_assessment_for_cymraeg/`](./medical_language_speaking_assessment_for_cymraeg/) — the Loco crate: `src/medical_language_speaking_assessment_for_cymraeg/` holds `app.rs`
...
`src/medical_language_speaking_assessment_for_cymraeg/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd medical_language_speaking_assessment_for_cymraeg && cargo check --all-targets && cargo test
```
