<script lang="ts">
	...
	$effect(() => {
		if (!assessment.result) {
			goto(`/gastroenterology-assessment/gastroenterology-assessments/${id}`);
		}
	});
	...
	async function downloadPDF() {
		pdfError = '';
		try {
			const res = await fetch(`/gastroenterology-assessments/${id}/report/pdf`, {
				method: 'POST',
			...
			if (res.ok) {
				...
				a.href = url;
				a.download = `gastroenterology-assessment-${data.demographics.lastName || id}.pdf`;
				a.click();
...
{#if result}
	<header class="border-b border-base-300 bg-base-100 shadow-sm no-print">
		<div class="mx-auto flex max-w-4xl items-center justify-between px-4 py-4">
			<h1 class="text-lg font-bold text-base-content">Gastroenterology assessment report</h1>
			<div class="flex items-center gap-3">
				...
				<Button data-variant="secondary" onclick={() => goto(`/gastroenterology-assessment/gastroenterology-assessments/${id}`)}>Edit</Button>
			</div>
		</div>
	</header>
	...
	<main class="mx-16 px-4 py-6">
		...
		{#if result.additionalFlags.length > 0}
			<div class="mb-6 rounded-xl border border-error/40 bg-base-100 p-6">
				<h2 class="mb-4 text-lg font-bold text-error">Flagged issues for gastroenterologist</h2>
				<div class="space-y-2">
		...
		{#if result.firedRules.length > 0}
			<div class="mb-6 rounded-xl border border-base-300 bg-base-100 p-6">
				<h2 class="mb-4 text-lg font-bold text-base-content">Severity score breakdown</h2>
			...
			<div class="grid grid-cols-1 gap-4 text-sm sm:grid-cols-2">
				...
				{#if data.chiefComplaint.primarySymptom}
					<div class="sm:col-span-2">
						<span class="font-medium text-base-content/70">Primary symptom:</span>
		...
		{#if data.allergiesDiet.drugAllergies.length > 0}
			<div class="mb-6 rounded-xl border border-base-300 bg-base-100 p-6">
				...
				<ul class="list-disc space-y-1 pl-5 text-sm text-base-content/80">
					{#each data.allergiesDiet.drugAllergies as allergy (allergy.allergen)}
						<li>
		...
		{#if data.currentMedications.otherMedications.length > 0}
			<div class="mb-6 rounded-xl border border-base-300 bg-base-100 p-6">
				<h2 class="mb-4 text-lg font-bold text-base-content">Other medications</h2>
				<ul class="list-disc space-y-1 pl-5 text-sm text-base-content/80">
