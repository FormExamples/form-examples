# Gastroenterology Assessment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Gastroenterology Assessment form, built with axum + Loco +
...
- [`gastroenterology_assessment/`](./gastroenterology_assessment/) — the Loco crate: `src/gastroenterology_assessment/` holds `app.rs`
...
`src/gastroenterology_assessment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd gastroenterology_assessment && cargo check --all-targets && cargo test
```
