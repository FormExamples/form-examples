# Semaglutide Assessment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Semaglutide Assessment form, built with axum + Loco +
...
- [`semaglutide_assessment/`](./semaglutide_assessment/) — the Loco crate: `src/semaglutide_assessment/` holds `app.rs`
...
`src/semaglutide_assessment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd semaglutide_assessment && cargo check --all-targets && cargo test
```
