...
// DEXA Bone Density Test Request — vetting dashboard
...
// scan-region dropdown + safety-flags dropdown).
//
// Sibling modules loaded as plain `<script>` tags (in dependency order)
// attach their exports to `window.DexaBoneDensityTestRequestDashboard`.
...
const columns = [
  ...
  { key: 'scanRegion',         label: 'Scan Region' },
  { key: 'indication',         label: 'Indication' },
  { key: 'fraxPercent',        label: 'FRAX %' },
...
function matchesFilters(row) {
  if (filters.search) {
    ...
    const matches =
      ...
      String(row.scanRegion || '').toLowerCase().includes(term) ||
  ...
  if (filters.region && row.scanRegion !== filters.region) return false;
...
function compareRows(a, b) {
  ...
  // Default: string compare (nhs, patient, scanRegion, indication, clinician,
  // referralDate).
...
function fraxCell(frax) {
  if (frax === null || frax === undefined || frax === '') {
    return '<span class="numeric-cell numeric-muted">—</span>';
  }
  return `<span class="numeric-cell">${esc(frax)}%</span>`;
}
...
function renderTableBody() {
  ...
  for (const row of rows) {
    ...
    tr.innerHTML = `
      ...
      <td class="data-table-td">${esc(titleCase(row.scanRegion))}</td>
      <td class="data-table-td">${esc(titleCase(row.indication))}</td>
      <td class="data-table-td">${fraxCell(row.fraxPercent)}</td>
