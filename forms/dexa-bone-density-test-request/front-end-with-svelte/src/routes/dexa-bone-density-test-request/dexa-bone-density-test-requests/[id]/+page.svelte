<script lang="ts">
	...
	import Step4FractureRiskFactors from '$lib/components/steps/Step4FractureRiskFactors.svelte';
	import Step5PreviousDexa from '$lib/components/steps/Step5PreviousDexa.svelte';
	...
	function validate(): boolean {
		...
		if (d.request.scanRegion === '') {
			found.push({ id: 'scanRegion', message: 'Requested scan region is required.' });
		}
	...
	function submit() {
		...
		goto(`/dexa-bone-density-test-request/dexa-bone-density-test-requests/${id}/report`);
	}
...
<main class="mx-auto max-w-3xl px-4 py-6">
	<header class="mb-6 no-print">
		<h1 class="text-2xl font-bold text-base-content">
			{isNew ? 'New DEXA bone-density request' : `DEXA request ${id}`}
		</h1>
	...
	<Form label="DEXA bone-density request" onsubmit={submit}>
		<Step1RequestingClinician />
		<Step2PatientIdentification />
		<Step3RequestedExamination />
		<Step4FractureRiskFactors />
		<Step5PreviousDexa />
		<Step6Triage />
