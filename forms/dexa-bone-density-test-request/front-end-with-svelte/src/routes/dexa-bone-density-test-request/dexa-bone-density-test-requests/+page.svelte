<script lang="ts">
	...
	const columns = [
		...
		{ id: 'patientName', header: 'Patient', flexgrow: 2, sort: true },
		{
			...
			header: 'Region',
			width: 130,
			template: (v: string) => scanRegionLabel(v as never)
		},
		{
	...
	function init(api: { exec: (k: string, p: unknown) => void; on: (e: string, cb: (ev: { id?: string | number }) => void) => void }) {
		...
		api.on('select-row', (ev: { id?: string | number }) => {
			if (ev?.id != null) goto(`/dexa-bone-density-test-request/dexa-bone-density-test-requests/${ev.id}`);
		});
	}
</script>
...
<main class="mx-auto max-w-6xl px-4 py-6">
	<div class="mb-6 flex flex-wrap items-end justify-between gap-4">
		<div>
			<h1 class="text-2xl font-bold text-base-content">DEXA vetting dashboard</h1>
			<p class="mt-1 text-sm text-base-content/70">
				Four-axis vetting grades for incoming DEXA bone-density requests, computed by the shared
				engine. Select a row to open the request.
			</p>
		</div>
		<a href="/dexa-bone-density-test-request/dexa-bone-density-test-requests/new" class="button" data-variant="primary">New request</a>
	</div>
