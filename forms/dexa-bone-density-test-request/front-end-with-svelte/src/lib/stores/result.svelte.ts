...
import type { DexaRequest, GradingResult } from '$lib/engine/types';

/** localStorage draft key for a given request id (defaults to `new`). */
function storageKey(id: string): string {
	return `dexa-bone-density-test-request.front-end-with-svelte.${id || 'new'}.v1`;
}

/** A blank DEXA bone-density request with all fields at their unanswered defaults. */
export function createDefaultRequest(): DexaRequest {
	return {
		clinician: {
			clinicianName: '',
			clinicianRole: '',
...
/**
 * Svelte 5 reactive store for the DEXA bone-density request, with localStorage
 * persistence so an in-progress request survives a page reload. Drafts are keyed
 * by request id so each record edits independently.
 */
class RequestStore {
	data = $state<DexaRequest>(createDefaultRequest());
	 ...
	 * `const d = requestStore.data.clinician`) stay bound to live state.
	 */
	loadForId(id: string, seed?: DexaRequest) {
		const key = id || 'new';
		this.id = key;
		this.result = null;
		this.currentStep = 1;

		let draft: DexaRequest | null = null;
		if (browser) {
			const raw = localStorage.getItem(storageKey(key));
			if (raw) {
				try {
					draft = JSON.parse(raw) as DexaRequest;
				} catch {
