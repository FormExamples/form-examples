# DEXA Bone Density Test Request — agent instructions

DEXA / DXA bone-densitometry (osteoporosis) scan request (referral). A clinician
...
`{ appropriatenessScore (1–9) + appropriatenessBand, radiationDoseBand +
safetyNote, completenessPercent, triageTier + targetTimeframe, recommendation,
firedRules[], flags[] }`. Rule IDs are identical across every front-end and the
back-end. High-acuity factors (recent fragility fracture, very high FRAX risk,
long-term high-dose steroids) escalate the triage tier. Choose the
