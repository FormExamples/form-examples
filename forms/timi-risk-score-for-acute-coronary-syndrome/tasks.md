# TIMI Risk Score for Acute Coronary Syndrome (UA/NSTEMI) — tasks
...
- [x] SQL migrations in `sql/` (assessment table, seven criterion inputs,
      UUIDv4 PK, timestamps).
- [x] Generated XML, FHIR R5, protobuf, OpenAPI, Loco setup script,
      `CHANGELOG.md`, and `examples/`.
- [x] Scoring engine (`types.ts`, `utils.ts`, `timi-rules.ts`,
      `timi-grader.ts`, `flagged-issues.ts`) + Vitest tests.
...
- [ ] `bin/test-form timi-risk-score-for-acute-coronary-syndrome` passes
