 ...
 *   - Positive marker with ST deviation (high) — marker == 'yes' && stDeviation == 'yes'
 *   - Positive cardiac marker (high)       — marker == 'yes'
 ...
 */
export function detectFlaggedIssues(data: AssessmentData, timiScore: number): FlaggedIssue[] {
	...
	// ─── Positive marker with ST deviation (HIGH) ───────────────
	...
	if (data.riskProfile.threeOrMoreCadRiskFactors === '')
		missing.push('>= 3 coronary risk factors');
	...
	if (data.presentation.twoOrMoreAnginaEpisodes24h === '')
		missing.push('>= 2 anginal episodes in 24 h');
	...
	if (missing.length > 0) {
		flags.push({
			...
			suggestedAction: 'Record the missing criterion(s) and re-score.'
		});
	}
