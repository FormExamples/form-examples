<script lang="ts">
	...
	function submit() {
		...
		assessment.result = calculateTimiGrade(assessment.data);
		goto(`/timi-risk-score-for-acute-coronary-syndrome/${plural}/${id}/report`);
	}
...
<main class="mx-16 px-4 py-6">
	<h1 class="text-2xl font-bold text-base-content">
		{isNew ? 'New TIMI assessment' : `TIMI assessment ${id}`}
	</h1>
	<p class="mt-1 text-sm text-base-content/70">
		Complete the seven sections; the TIMI score and risk band are computed on submit.
	</p>
	...
	<Form label="TIMI UA/NSTEMI assessment" onsubmit={submit}>
		<Step1Context />
		<Step2Identification />
		<Step3RiskFactors />
		<Step4CardiacHistory />
