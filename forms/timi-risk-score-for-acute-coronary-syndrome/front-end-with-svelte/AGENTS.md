...
for the TIMI specification and scoring engine. Lily Svelte headless
conventions: [`../../AGENTS-front-end-svelte.md`](../../AGENTS-front-end-svelte.md).

## Layout

- `src/lib/engine/` — pure scoring engine (`types.ts`, `utils.ts`,
  `timi-rules.ts`, `timi-grader.ts`, `flagged-issues.ts`) + `timi-grader.test.ts`.
...
- `src/routes/timi-risk-score-for-acute-coronary-syndromes/` — RESTful routes:
