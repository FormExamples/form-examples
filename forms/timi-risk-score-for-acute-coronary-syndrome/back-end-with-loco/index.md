...
Server-side Rust JSON API for the TIMI Risk Score for Acute Coronary Syndrome (UA/NSTEMI) form. Pure back-end: no HTML
rendering, no Tera templates, no HTMX, no Alpine.js, no CSS, no Lily
Design System.

The Loco crate lives in [`timi_risk_score_for_acute_coronary_syndrome/`](timi_risk_score_for_acute_coronary_syndrome/) — one migration, SeaORM
