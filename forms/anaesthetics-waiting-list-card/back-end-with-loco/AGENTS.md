# Anaesthetics Waiting List Card — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Anaesthetics Waiting List Card form, built with axum + Loco +
...
- [`anaesthetics_waiting_list_card/`](./anaesthetics_waiting_list_card/) — the Loco crate: `src/anaesthetics_waiting_list_card/` holds `app.rs`
...
`/metrics`. The registered domain controllers are:
  ...
  - `anaesthetics_waiting_list_card`
  - `anaesthetics_waiting_list_card_appointment`
  - `anaesthetics_waiting_list_card_grade`
  - `anaesthetics_waiting_list_card_grade_flag`
  - `anaesthetics_waiting_list_card_grade_rule`
  - `patient`
  - `practitioner`

## Engine

`src/anaesthetics_waiting_list_card/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd anaesthetics_waiting_list_card && cargo check --all-targets && cargo test
```
