<script lang="ts">
	...
	import Step3FrequencyOfDrinking from '#lib/components/steps/Step3FrequencyOfDrinking.svelte';
	import Step4TypicalQuantity from '#lib/components/steps/Step4TypicalQuantity.svelte';
	import Step5HeavyEpisodeFrequency from '#lib/components/steps/Step5HeavyEpisodeFrequency.svelte';
	...
	function submit() {
		...
		assessment.result = calculateAuditcGrade(assessment.data);
		goto(`/alcohol-use-disorders-identification-test-consumption/${plural}/${id}/report`);
	}
...
<main class="mx-16 px-4 py-6">
	<h1 class="text-2xl font-bold text-base-content">
		{isNew ? 'New AUDIT-C assessment' : `AUDIT-C assessment ${id}`}
	</h1>
	<p class="mt-1 text-sm text-base-content/70">
		Complete the six sections; the AUDIT-C score and risk band are computed on submit.
	</p>
	...
	<Form label="AUDIT-C assessment" onsubmit={submit}>
		<Step1Context />
		<Step2Identification />
		<Step3FrequencyOfDrinking />
		<Step4TypicalQuantity />
		<Step5HeavyEpisodeFrequency />
		<Step6Summary />
