<script lang="ts">
	...
	// SVAR DataGrid columns. AUDIT-C score, risk band, and positive screen render
	...
	const columns = [
		...
		{ id: 'auditcScore', header: 'AUDIT-C', width: 90, sort: true },
		{
			id: 'riskBand',
			header: 'Risk band',
			width: 200,
	...
	function init(api: any) {
		...
		api.on('select-row', (ev: { id?: string | number }) => {
			if (ev?.id != null) goto(`/alcohol-use-disorders-identification-test-consumption/${plural}/${ev.id}`);
		});
	}
</script>
...
<main class="mx-auto max-w-6xl px-4 py-6">
	<div class="mb-6 flex flex-wrap items-end justify-between gap-4">
		<div>
			<h1 class="text-2xl font-bold text-base-content">AUDIT-C clinician dashboard</h1>
			<p class="mt-1 text-sm text-base-content/70">
				AUDIT-C score, risk band, and positive screen for assessed patients, computed by the shared
				engine. Select a row to open the assessment.
			</p>
		</div>
		<a href="/alcohol-use-disorders-identification-test-consumption/{plural}/new" class="button" data-variant="primary">New assessment</a>
	</div>
	...
	<div class="mb-4 flex flex-wrap gap-4">
		...
		<label class="text-sm">
			...
			<select class="select inline-block w-auto" bind:value={bandFilter}>
				<option value="">All</option>
				<option value="lower">Lower (0-4)</option>
				<option value="increasing">Increasing (5-7)</option>
				<option value="higher">Higher (8-10)</option>
				<option value="possible-dependence">Possible dependence (11-12)</option>
			</select>
		</label>
	</div>
