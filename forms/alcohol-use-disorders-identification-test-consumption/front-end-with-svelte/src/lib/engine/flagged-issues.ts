import type { AssessmentData, FlaggedIssue, ItemScore } from './types';

/** Is an item value unanswered? */
function isMissing(v: ItemScore): boolean {
	return v === null || v === undefined;
}

/**
 * Detect clinician-facing safety flags (red flags), independent of the total
 * AUDIT-C score (spec §5):
 *
 *   - Positive screen / brief intervention (high) — auditcScore >= 5
 *   - Escalate to full AUDIT (high)                — auditcScore >= 8
 ...
 *
 * Rows mirror the `audit_c_grade_flag` SQL table (flag_id, category, priority,
 * description, suggested_action).
 */
export function detectFlaggedIssues(data: AssessmentData, auditcScore: number): FlaggedIssue[] {
	...
	const heavyPoint = isMissing(heavyEpisodeFrequency) ? 0 : Number(heavyEpisodeFrequency);
	...
	if (missing.length > 0) {
		flags.push({
			...
			suggestedAction: 'Complete the missing item(s) and re-score before acting on the result.'
		});
	}
