// Flagged-issue detection. Independent of ECOG grade (which the grader
// computes), this module raises clinician-facing safety flags including
// neutropenic fever risk, severe pain, poor performance status, active
// bleeding risk, cord compression risk, hypercalcaemia, distress,
// progressive disease, severe weight loss, malnourishment, organ
// toxicity, severe anaemia, coagulopathy, and advance-care planning.
//
...
function detectAdditionalFlags(data) {
  ...
  if (
    ...
    flags.push({
      ...
      message:
        'Neutropenic fever risk - monitor temperature, consider prophylactic antibiotics',
      priority: 'high'
    });
  }
  ...
  // ─── Cord compression (brain/spinal site with M1) ───────
  const primarySiteLower = (data.cancerDiagnosis.primarySite || '').toLowerCase();
  if (
    (data.cancerDiagnosis.cancerType === 'brain' ||
      primarySiteLower.includes('spine') ||
      primarySiteLower.includes('spinal')) &&
  ...
  if (data.currentTreatment.responseAssessment === 'progressive-disease') {
    flags.push({
      ...
      category: 'Treatment',
      message:
        'Progressive disease on current regimen - consider treatment change or MDT discussion',
      priority: 'medium'
    });
  }
