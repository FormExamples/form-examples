<script lang="ts">
	...
	const sexOptions = [{ value: 'male', label: 'Male' }, { value: 'female', label: 'Female' }, { value: 'other', label: 'Other' }];
</script>

<Fieldset legend="Demographics">
	<p class="hint">Basic patient information and performance status.</p>
	...
	<div class="field-grid field-grid-3">
		<Field label="Weight (kg)" required inputId="weight"><NumberInput id="weight" label="Weight" min={1} max={400} required bind:value={d.weight} /></Field>
		<Field label="Height (cm)" required inputId="height"><NumberInput id="height" label="Height" min={50} max={250} required bind:value={d.height} /></Field>
		<Field label="BMI" description="Auto-calculated">
			{#if d.bmi}<p class="bmi-value">{d.bmi} <span class="bmi-cat">({bmiCategory(d.bmi)})</span></p>{:else}<p class="bmi-empty">—</p>{/if}
		</Field>
	</div>

	<Field label="ECOG Performance Status" required inputId="ecog">
		<Select id="ecog" label="ECOG" required bind:value={d.ecogPerformanceStatus}>
...
<style>
	...
	.bmi-value { margin: 0; font-weight: 500; }
	.bmi-cat { color: var(--color-muted); font-weight: 400; }
	.bmi-empty { color: var(--color-muted); margin: 0; }
</style>
