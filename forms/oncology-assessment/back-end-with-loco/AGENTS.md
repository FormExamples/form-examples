# Oncology Assessment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Oncology Assessment form, built with axum + Loco +
...
- [`oncology_assessment/`](./oncology_assessment/) — the Loco crate: `src/oncology_assessment/` holds `app.rs`
...
`src/oncology_assessment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd oncology_assessment && cargo check --all-targets && cargo test
```
