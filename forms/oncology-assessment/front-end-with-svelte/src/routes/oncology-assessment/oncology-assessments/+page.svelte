<script lang="ts">
	...
	import { cancerTypeLabel, stageLabel, responseAssessmentLabel } from '#lib/engine/utils.js';
	...
	function init(api: any) {
		...
		api.on('select-row', (ev: { id?: string | number }) => {
			if (ev?.id != null) goto(`/oncology-assessment/oncology-assessments/${ev.id}`);
		});
	}
</script>
...
<main class="mx-16 px-4 py-6">
	<div class="mb-6 flex flex-wrap items-end justify-between gap-4">
		...
		<a href="/oncology-assessment/oncology-assessments/new" class="button" data-variant="primary">New assessment</a>
	</div>
