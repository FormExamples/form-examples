...
// Tumor Marker Test Result — clinician dashboard (vanilla classic-script app).
//
...
// the SvelteKit dashboard route (`front-end-with-svelte/src/routes/+page.svelte`).
//
// Sibling modules loaded as plain `<script>` tags (in dependency order) attach
// their exports to `window.TumorMarkerTestResultDashboard`; display labels come
// from the form namespace `window.TumorMarkerTestResult` (js/types.js). The
...
const columns = [
  ...
  { key: 'knownCancerSite',           label: 'Cancer site' },
...
function compareRows(a, b) {
  ...
  // Default: string compare (id, patientName, knownCancerSite, reportStatus,
...
function renderTableBody() {
  ...
  for (const row of rows) {
    ...
    tr.innerHTML = `
      ...
      <td>${esc(row.knownCancerSite || 'N/A')}</td>
