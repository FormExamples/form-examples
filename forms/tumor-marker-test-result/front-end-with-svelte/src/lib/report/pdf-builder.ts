...
import type { TumorMarkerResult, GradingResult } from '$lib/engine/types';
import {
...
/** Builds the pdfmake document definition for the tumour-marker laboratory report. */
export function buildPdfDocument(
	data: TumorMarkerResult,
	...
	return {
		...
		header: {
			text: 'SERUM TUMOUR-MARKER LABORATORY REPORT',
			alignment: 'center',
			margin: [0, 20, 0, 0],
			fontSize: 10,
			color: '#6b7280',
		...
		footer: (currentPage: number, pageCount: number) => ({
			text: `Page ${currentPage} of ${pageCount} | Generated ${new Date(result.gradedAt).toLocaleString()} | ACB / RCPath / ASCO`,
			alignment: 'center',
			margin: [0, 20, 0, 0],
			fontSize: 8,
			color: '#9ca3af'
		...
		content: [
			...
			{
				text: `${data.knownCancerSite || 'Unspecified site'} — ${reportStatusLabel(data.reportStatus)}`,
				fontSize: 12,
				alignment: 'center',
				color: '#4b5563',
				margin: [0, 0, 0, 4] as Margin
			...
			sectionHeader('Specimen & clinical context'),
			{
				ul: [
					`Specimen condition: ${data.specimenCondition || 'N/A'}`,
					`Known cancer site: ${data.knownCancerSite || 'N/A'}`
				],
				margin: [0, 0, 0, 8] as Margin
			},
			...
			// Measured values
			sectionHeader('Measured tumour-marker values'),
			{
				ul: measuredMarkerList(data),
				margin: [0, 0, 0, 16] as Margin
			},

			// Trend
			sectionHeader('Trend & comparison'),
			{
				ul: [
					`Previous value: ${data.previousValue !== null ? data.previousValue : 'N/A'}`,
					`Trend: ${trendLabel(data.trend)}`,
					`Markedly elevated: ${data.markedlyElevated ? 'Yes' : 'No'}`,
					`Overall result status: ${data.overallResultStatus || 'N/A'}`
				],
				margin: [0, 0, 0, 4] as Margin
			},
			{
...
function measuredMarkerList(data: TumorMarkerResult): string[] {
	const list: string[] = [];
	for (const m of MARKERS) {
		const value = data[m.key] as number | null;
		if (value !== null) {
			list.push(`${m.label}: ${value} ${m.unit}`);
		}
	}
	if (list.length === 0) list.push('No markers measured');
	return list;
}
