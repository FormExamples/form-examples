// ──────────────────────────────────────────────
// Tumor Marker Test Result — core data types
//
// Field names are camelCase (front-end serde); they mirror the snake_case
// columns in sql/04_create_table_tumor_marker_test_result.sql and
...
export type SpecimenCondition = 'satisfactory' | 'haemolysed' | 'lipaemic' | 'insufficient' | '';

/** Trend of the principal marker versus the previous value. */
export type Trend = 'rising' | 'stable' | 'falling' | 'not-applicable' | '';
 ...
 * interpretation grade is computed from. Each measured marker is a NUMERIC
 * value, or `null` when not measured.
 */
export interface TumorMarkerResult {
	...
	ca125: number | null;
	ca19_9: number | null;
	carcinoembryonicAntigenCea: number | null;
	alphaFetoproteinAfp: number | null;
	betaHcg: number | null;
	ca15_3: number | null;
	lactateDehydrogenaseLdh: number | null;
