...
4. `03_create_table_clinician.sql` — reporting clinician (clinical biochemist / oncologist / reporting clinician).
5. `04_create_table_tumor_marker_test_result.sql` — main result/report record (measured marker values as NUMERIC columns).
6. `05_create_table_tumor_marker_test_result_grade.sql` — four-axis interpretation grade.
7. `06_create_table_tumor_marker_test_result_grade_rule.sql` — fired-rule audit trail.
8. `07_create_table_tumor_marker_test_result_grade_flag.sql` — safety-critical flags.
...
date / time. Each measured marker value is a `NUMERIC(12,2)` with a
`>= 0` CHECK and is `NULL` when not measured.
