...
// module exposes:
//
//   componentPresence(data)   -> per-component `documented` booleans (spec §4)
...
//
// The grader (`ward-round-grader.ts`) reads these to derive the completeness
// percentage and the complete / partial / incomplete status, and to build the
// fired-rule audit trail that mirrors `ward_round_note_grade_rule`.

import type { AssessmentData, ComponentPresence, RuleComponent } from './types';
...
/**
 * Per-component `documented` predicates (spec §4). An explicit negative flag
 * counts as documented for the components that support one.
 */
export function componentPresence(data: AssessmentData): ComponentPresence {
	const h = data.header;
	...
	return {
		...
		escalation: has(esc.escalationStatus) && esc.escalationStatus !== 'not-recorded',
