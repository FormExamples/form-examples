...
<Fieldset title="PROM — PROMIS Global Health" description="PROMIS Global Health v1.2 (paraphrased). See doc/licensing.md for attribution and scoring notes.">
	...
	<h3 class="mb-3 font-semibold text-base-content">General Health Ratings</h3>

	<div class="mb-4">
		<label for="item1" class="mb-1 block text-sm font-medium text-base-content/80">In general, how would you rate your health?</label>
		<select id="item1" value={d.item1GeneralHealth ?? ''} onchange={(e) => { d.item1GeneralHealth = parseNum((e.target as HTMLSelectElement).value); }}
			class="w-full rounded-lg border border-base-300 px-3 py-2 text-sm focus:border-primary focus:ring-1 focus:ring-primary focus:outline-none">
			{#each levelOptions as o}<option value={o.value}>{o.label}</option>{/each}
		</select>
	</div>

	<div class="mb-4">
		<label for="item2" class="mb-1 block text-sm font-medium text-base-content/80">In general, how would you rate your quality of life?</label>
	...
	<div class="mb-4">
		<label for="item3" class="mb-1 block text-sm font-medium text-base-content/80">In general, how would you rate your physical health?</label>
	...
	<div class="mb-4">
		<label for="item4" class="mb-1 block text-sm font-medium text-base-content/80">In general, how would you rate your mental health, including your mood and ability to think?</label>
	...
	<div class="mb-4">
		<label for="item5" class="mb-1 block text-sm font-medium text-base-content/80">In general, how would you rate your satisfaction with your social activities and relationships?</label>
	...
	<h3 class="mb-3 mt-6 font-semibold text-base-content">Frequency Questions (past 7 days)</h3>

	<div class="mb-4">
		<label for="item6" class="mb-1 block text-sm font-medium text-base-content/80">In general, how often did you feel tired?</label>
	...
	<div class="mb-4">
		<label for="item7" class="mb-1 block text-sm font-medium text-base-content/80">In general, how often did you have emotional problems such as feeling anxious or depressed?</label>
	...
	<div class="mb-4">
		<label for="item8" class="mb-1 block text-sm font-medium text-base-content/80">How often were you able to carry out your social activities and roles?</label>
	...
	<div class="mb-4">
		<label for="item9" class="mb-1 block text-sm font-medium text-base-content/80">
			How would you rate your pain on average?
			<span class="text-xs text-base-content/50 ml-1">(0 = No pain, 10 = Worst pain imaginable)</span>
		</label>
		<input
			id="item9"
			type="number"
	...
	<div class="mb-4">
		<label for="item10" class="mb-1 block text-sm font-medium text-base-content/80">How often were you able to carry out your everyday physical activities such as walking, climbing stairs, carrying groceries?</label>
	...
	{#if gph !== null || gmh !== null}
		<div class="mt-6 rounded-lg border border-base-300 bg-base-200 p-4">
			<h3 class="mb-2 text-sm font-semibold text-base-content/80">Calculated T-scores (linear approximation)</h3>
			<div class="grid grid-cols-2 gap-4 text-sm">
				<div>
					<span class="text-base-content/60">Global Physical Health:</span>
					<span class="ml-2 font-medium">{gph !== null ? gph.toFixed(1) : '—'}</span>
				</div>
				<div>
					<span class="text-base-content/60">Global Mental Health:</span>
					<span class="ml-2 font-medium">{gmh !== null ? gmh.toFixed(1) : '—'}</span>
				</div>
			</div>
			<p class="mt-2 text-xs text-base-content/50">Population norm = 50. Note: these are approximate values only.</p>
		</div>
	{/if}
</Fieldset>
