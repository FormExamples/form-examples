# Outpatient Outcome Report — specification
...
**Inputs.** A typed assessment object whose shape mirrors the SQL schema in `sql-migrations/` (17 migration files). Unanswered text and enum fields default to `''`; unanswered numeric, date, and time fields default to `null`.
...
- `bin/test-form outpatient-outcome` exits cleanly.
...
- `bin/lily-html-refactor --check outpatient-outcome` reports no drift.
- LocalStorage keys preserve draft state across reloads:
  - `outpatient-outcome.front-end-form-with-html.v1` (HTML)
  - `outpatient-outcome.front-end-form-with-svelte.v1` (SvelteKit)
