...
// exactly eight MANDATORY rules (spec §4). Each rule below evaluates the plan
// and returns true when its mandatory content / process requirement is
// satisfied; the grader (`grader.js`) marks the plan `complete` only when every
// rule is satisfied, otherwise `incomplete`. Rows here mirror the
...
// arithmetic: it counts populated mandatory fields (not passed rules) so an
// incomplete plan still shows how close it is to done. The capacity proxy slot
// is conditional — it only enters the denominator when the person is recorded
// as lacking capacity (Mental Capacity Act 2005).
 ...
 * @property {(p: RespectPlan) => boolean} evaluate
 *
 * @typedef {Object} FieldSlot
 * @property {string} key
 * @property {(p: RespectPlan) => boolean} present   - populated?
...
const mandatoryRules = [
  // ─── R1: PERSONAL DETAILS IDENTIFY THE PERSON ─────────────────
  {
    ...
    evaluate: (p) =>
      ...
      p.personal.dateOfBirth !== null && p.personal.dateOfBirth !== '' &&
  ...
  // ─── R3: PREFERENCES / WHAT MATTERS ───────────────────────────
  {
    ...
    description:
      'Personal preferences / what matters to the person are recorded',
    evaluate: (p) =>
      nonEmpty(p.preferences.whatMatters) ||
      nonEmpty(p.preferences.carePreferences)
  },
  ...
  // ─── R5: CPR RECOMMENDATION ───────────────────────────────────
  {
    ...
    description:
      'A CPR recommendation is documented (attempt or do-not-attempt)',
    evaluate: (p) =>
  ...
  // ─── R8: CLINICIAN SIGN-OFF ───────────────────────────────────
  {
    ...
    description:
      'The plan is signed by the completing clinician, with role and date',
    evaluate: (p) =>
      nonEmpty(p.signOff.clinicianName) &&
      nonEmpty(p.signOff.clinicianRole) &&
      nonEmpty(p.signOff.signature) &&
      p.signOff.signedAt !== null && p.signOff.signedAt !== ''
  }
];
...
const completenessSlots = [
  ...
  { key: 'dateOfBirth', present: (p) => p.personal.dateOfBirth !== null && p.personal.dateOfBirth !== '' },
  { key: 'identifier', present: (p) => nonEmpty(p.personal.identifier) },
  { key: 'healthSummary', present: (p) => nonEmpty(p.health.healthSummary) },
  { key: 'preferences', present: (p) => nonEmpty(p.preferences.whatMatters) || nonEmpty(p.preferences.carePreferences) },
  { key: 'priorityBalance', present: (p) => nonEmpty(p.recommendations.priorityBalance) },
  { key: 'interventions', present: (p) => nonEmpty(p.recommendations.recommendedInterventions) || nonEmpty(p.recommendations.notRecommendedInterventions) },
  { key: 'cprRecommendation', present: (p) => p.cpr.cprRecommendation === 'attempt' || p.cpr.cprRecommendation === 'do-not-attempt' },
  { key: 'ceilings', present: (p) => nonEmpty(p.ceilings.hospitalTransfer) || nonEmpty(p.ceilings.criticalCareAdmission) || nonEmpty(p.ceilings.treatmentCeilings) },
