// Flagged-issue detection (safety and governance). Independent of the
// completeness status (which the grader produces), this module raises
// clinician-facing flags per spec §5:
//
...
//
// Rows here mirror the `respect_grade_flag` SQL table
 ...
 * @param {Date} [today]   - injectable "now" for deterministic tests
 * @returns {FlaggedIssue[]}
 */
function detectFlaggedIssues(plan, today) {
  /** @type {FlaggedIssue[]} */
  const flags = [];
  ...
  if (!nonEmpty(plan.signOff.signature) ||
      plan.signOff.signedAt === null || plan.signOff.signedAt === '') {
    flags.push({
  ...
  if (!nonEmpty(plan.health.healthSummary)) {
    flags.push({
      ...
      suggestedAction:
        'Add a brief clinical summary and the relevant diagnoses.'
    });
  }
