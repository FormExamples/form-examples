// Plain-JavaScript / JSDoc type definitions mirroring the SvelteKit
// `src/lib/types.ts` data model for the ReSPECT clinician dashboard.
//
// This file deliberately exports nothing executable; it exists so other modules
// can reference the JSDoc type aliases via `@typedef` imports and so engineers
// can read the canonical shape of the dashboard data in one place.

/**
 * Documentation-completeness status emitted by the engine.
 *
 * @typedef {'complete' | 'incomplete'} Status
 */

/**
 * CPR recommendation recorded on the plan; '' when not yet documented.
 *
 * @typedef {'attempt' | 'do-not-attempt' | ''} CprRecommendation
 */

/**
 * Plan row displayed in the clinician dashboard.
 *
 * Mirrors `PlanRow` in
 * `forms/recommended-summary-plan-for-emergency-care-and-treatment/front-end-with-svelte/src/lib/types.ts`.
 *
 * @typedef {Object} PlanRow
 * @property {string} id                      - UUID of the ReSPECT record
 * @property {string} patientIdentifier       - NHS / CHI number or local identifier
 * @property {string} personName              - "Surname, Given" display name
 * @property {Status} status                  - complete | incomplete
 * @property {number} completenessPercent     - 0..100
 * @property {CprRecommendation} cprRecommendation - attempt | do-not-attempt | ''
 * @property {string} clinicianName           - completing clinician
 * @property {string | null} reviewDate       - ISO date (yyyy-mm-dd) or null
 * @property {string} updatedAt               - ISO date the plan was last updated
 */

/**
 * Response from `GET /api/dashboard/plans`.
 *
 * @typedef {Object} DashboardPlansResponse
 * @property {PlanRow[]} items
 * @property {number} total
 */

// Wrapped in an IIFE so locals stay scoped — this file is loaded as a classic
// <script> (no ES modules) so the page can be opened directly via `file://`.
// The IIFE attaches its public symbols to a single global namespace,
// `window.RecommendedSummaryPlanForEmergencyCareAndTreatmentDashboard`.
(function () {
'use strict';
window.RecommendedSummaryPlanForEmergencyCareAndTreatmentDashboard =
  window.RecommendedSummaryPlanForEmergencyCareAndTreatmentDashboard || {};

// No runtime exports; types are JSDoc-only. Touch the namespace so this file is
// unambiguously side-effecting and other files can rely on it loading before
// they read the dashboard namespace.
})();
