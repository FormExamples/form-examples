// Biopsy Test Request — vetting dashboard (vanilla classic-script app).
//
...
// bleeding-risk dropdown + safety-flags dropdown).
//
// Sibling modules loaded as plain `<script>` tags (in dependency order)
// attach their exports to `window.BiopsyTestRequestDashboard`. The whole file
...
const columns = [
  ...
  { key: 'biopsyMethod',       label: 'Method' },
  { key: 'indication',         label: 'Indication' },
  { key: 'appropriatenessBand', label: 'Appropriateness' },
  { key: 'bleedingRiskBand',   label: 'Bleeding Risk' },
  { key: 'triageTier',         label: 'Triage / 2WW' },
...
function bleedingClass(band) {
  return band ? 'bleed-' + String(band) : '';
}
...
function matchesFilters(row) {
  if (filters.search) {
    ...
    const matches =
      ...
      String(row.biopsySite || '').toLowerCase().includes(term) ||
      String(row.biopsyMethod || '').toLowerCase().includes(term) ||
  ...
  if (filters.bleeding && row.bleedingRiskBand !== filters.bleeding) return false;
...
function compareRows(a, b) {
  ...
  if (key === 'bleedingRiskBand') {
    return ((bleedingRank[av] ?? -1) - (bleedingRank[bv] ?? -1)) * dir;
  }
  if (key === 'triageTier') {
  ...
  // Default: string compare (nhs, patient, biopsySite, biopsyMethod,
  // indication, clinician, referralDate).
...
function renderTriageCell(row) {
  ...
  const tww = row.twoWeekWaitEligible
    ? ` <span class="tww-badge">2WW</span>`
    : '';
  return tier + tww;
}
...
function renderTableBody() {
  ...
  for (const row of rows) {
    ...
    tr.innerHTML = `
      ...
      <td class="data-table-td">${esc(titleCase(row.biopsySite))}</td>
      <td class="data-table-td">${esc(titleCase(row.biopsyMethod))}</td>
      ...
      <td class="data-table-td"><span class="bleed-badge ${bleedingClass(row.bleedingRiskBand)}">${esc(titleCase(row.bleedingRiskBand))}</span></td>
