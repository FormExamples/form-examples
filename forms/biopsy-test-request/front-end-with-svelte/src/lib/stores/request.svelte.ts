...
function storageKey(id: string): string {
	return `biopsy-test-request.front-end-with-svelte.${id || 'new'}.v1`;
}

/**
 * Svelte 5 reactive store for the biopsy test request, with localStorage
 ...
 */
class RequestStore {
	data = $state<BiopsyRequestData>(createDefaultRequest());
	 ...
	 * (e.g. `const d = request.data.bleeding`) stay bound to live state.
	 */
	loadForId(id: string, seed?: BiopsyRequestData) {
		const key = id || 'new';
		this.id = key;
		this.result = null;
		this.currentStep = 1;

		let draft: BiopsyRequestData | null = null;
		if (browser) {
			const raw = localStorage.getItem(storageKey(key));
			if (raw) {
				try {
					draft = JSON.parse(raw) as BiopsyRequestData;
				} catch {
