...
//
// Ported from the HTML reference engine (front-end-form-with-html/js/flags.js).
// Pure function returning safety flags using the grade_flag categories from the
// SQL migrations: suspected-cancer-2ww, high-bleeding-risk-anticoag,
...
import type { BiopsyRequestData, SafetyFlag } from './types';

function hasValue(v: unknown): boolean {
	return v !== null && v !== undefined && v !== '';
}

/** Optional engine context used to widen the suspected-cancer flag. */
...
export function detectFlags(data: BiopsyRequestData, context: FlagContext = {}): SafetyFlag[] {
	...
	if (data.bleeding.takingAnticoagulant === true) {
		flags.push({
			...
			suggestedAction:
				'Withhold / bridge per BSG / ESGE timing; confirm a periprocedural anticoagulant plan before booking.'
		});
	}

	// --- Coagulopathy ---------------------------------------------------
	if (data.bleeding.bleedingDisorder === true || (hasValue(data.bleeding.inr) && Number(data.bleeding.inr) >= 1.5)) {
		flags.push({
	...
	if (data.bleeding.immunosuppressed === true) {
		flags.push({
			...
			suggestedAction:
				'Consider periprocedural infection precautions and antibiotic prophylaxis where indicated.'
		});
	}
