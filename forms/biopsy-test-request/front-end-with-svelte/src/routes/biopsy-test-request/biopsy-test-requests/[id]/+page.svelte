<script lang="ts">
	...
	function submit() {
		...
		goto(`/biopsy-test-request/biopsy-test-requests/${id}/report`);
	}
...
<main class="mx-16 px-4 py-6">
	<h1 class="text-2xl font-bold text-base-content">
		{isNew ? 'New biopsy test request' : `Biopsy test request ${id}`}
	</h1>
	<p class="mt-1 text-sm text-base-content/70">
		Complete the eight sections; the four-axis grade, safety flags, and recommendation are computed
		on submit.
	</p>
	...
	<Form label="Biopsy test request" onsubmit={submit}>
		<Step1Clinician />
		<Step2Patient />
		<Step3Procedure />
		<Step4Indication />
