# Agile Checklist — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Agile Checklist form, built with axum + Loco +
...
- [`agile_checklist/`](./agile_checklist/) — the Loco crate: `src/agile_checklist/` holds `app.rs`
...
`/metrics`. The registered domain controllers are:
  ...
  - `agile_checklist_grade_flag`
  - `agile_checklist_grade_rule`
  - `respondent`

## Engine

`src/agile_checklist/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd agile_checklist && cargo check --all-targets && cargo test
```
