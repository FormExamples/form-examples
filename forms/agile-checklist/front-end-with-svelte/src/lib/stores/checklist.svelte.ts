...
function storageKey(id: string): string {
	return `agile-checklist.front-end-with-svelte.${id || 'new'}.v1`;
}

/** A blank agile checklist with all fields at their unanswered defaults. */
export function createDefaultAssessment(): AgileChecklist {
	return createEmptyChecklist();
}

/**
 * Svelte 5 reactive store for the agile checklist, with localStorage
...
class AssessmentStore {
	data = $state<AgileChecklist>(createDefaultAssessment());
	currentStep = $state(1);
	/** The id of the checklist currently loaded into the store (`new` for a fresh draft). */
	id = $state('new');

	/** The live grading result, recomputed from `data` on every change. */
	 ...
	 * bound to live state.
	 */
	loadForId(id: string, seed?: AgileChecklist) {
		const key = id || 'new';
		this.id = key;
		this.currentStep = 1;

		let draft: AgileChecklist | null = null;
		if (browser) {
			const raw = localStorage.getItem(storageKey(key));
			if (raw) {
				try {
					draft = JSON.parse(raw) as AgileChecklist;
				} catch {
