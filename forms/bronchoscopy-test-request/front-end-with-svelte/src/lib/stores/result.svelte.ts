...
function storageKey(id: string): string {
	return `bronchoscopy-test-request.front-end-with-svelte.${id || 'new'}.v1`;
}

/**
 * Svelte 5 reactive store for the bronchoscopy request, with localStorage
...
class RequestStore {
	 ...
	 * a sample request for an existing id), falling back to a blank draft.
	 *
	 * The data is merged in place (nested object identities preserved) rather
	 * than reassigned, so step components that captured a section reference
	 * (e.g. `const d = requestStore.data.request`) stay bound to live state.
	 */
	loadForId(id: string, seed?: BronchoscopyRequest) {
		const key = id || 'new';
		this.id = key;
		this.result = null;
		this.currentStep = 1;
