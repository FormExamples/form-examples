 ...
 * 24-hour volume is computed once (see `parkland-grader.ts`) from body weight
 * and %TBSA, split 50/50 into the first-8-hour and next-16-hour phases (measured
 * from the time of injury), and an infusion rate is derived for each phase. The
 * constants below carry the coefficient, the mandated phase hours, the
 * urine-output titration factors, and the major-burn referral thresholds. The
 * values mirror the `parkland_formula_for_burns_grade_rule` SQL table.
 */

/** Parkland (Baxter) coefficient in mL per kg per %TBSA. */
export const PARKLAND_COEFFICIENT = 4;
/** Duration in hours of the first (rapid) resuscitation phase, from injury. */
export const FIRST_PHASE_HOURS = 8;
/** Duration in hours of the second resuscitation phase. */
export const SECOND_PHASE_HOURS = 16;

/** Low end of the adult urine-output target, mL/kg/h. */
export const URINE_LOW_FACTOR = 0.5;
/** High end of the adult urine-output target, mL/kg/h. */
export const URINE_HIGH_FACTOR = 1.0;

/** Adult major-burn referral threshold: %TBSA at or above triggers referral. */
export const REFERRAL_TBSA_ADULT = 15;
/** Child major-burn referral threshold: %TBSA at or above triggers referral. */
export const REFERRAL_TBSA_CHILD = 10;
 ...
 */
export function referralThreshold(ageBand: string): number {
