<script lang="ts">
	...
	import Step3Weight from '$lib/components/steps/Step3Weight.svelte';
	import Step4Burn from '$lib/components/steps/Step4Burn.svelte';
	import Step5Injury from '$lib/components/steps/Step5Injury.svelte';
	import Step6Features from '$lib/components/steps/Step6Features.svelte';
	...
	function submit() {
		...
		assessment.result = calculateParkland(assessment.data);
		goto(`/parkland-formula-for-burns/parkland-formula-for-burns-calculations/${id}/report`);
	}
...
<main class="mx-16 px-4 py-6">
	<h1 class="text-2xl font-bold text-base-content">
		{isNew ? 'New Parkland resuscitation calculation' : `Parkland resuscitation ${id}`}
	</h1>
	<p class="mt-1 text-sm text-base-content/70">
		Complete the seven sections; the total volume, phase split, and infusion rates are computed on
		submit.
	</p>
	...
	<Form label="Parkland resuscitation calculation" onsubmit={submit}>
		<Step1Context />
		<Step2Identification />
		<Step3Weight />
		<Step4Burn />
		...
		<div class="button-group">
			<Button type="submit" data-variant="primary">Compute plan &amp; view report</Button>
