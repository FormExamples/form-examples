...
// Parkland Formula for Burns — clinician dashboard (vanilla classic-script app).
//
...
// box + care-setting dropdown + major-burn dropdown).
//
// Sibling modules loaded as plain `<script>` tags (in dependency order) attach
// their exports to `window.ParklandFormulaForBurnsDashboard`. The whole file is
...
const filters = {
  search: '',
  setting: '',     // '' | 'emergency-department' | 'burns-unit' | ...
  majorBurn: ''    // '' | 'yes' | 'no'
};
...
const columns = [
  ...
  { key: 'tbsaPercent',       label: '%TBSA' },
  { key: 'total24hVolumeMl',  label: 'Total 24 h (mL)' },
  { key: 'majorBurnFlag',     label: 'Major burn' }
];
...
function matchesFilters(row) {
  ...
  if (filters.majorBurn === 'yes' && !row.majorBurnFlag) return false;
  if (filters.majorBurn === 'no' && row.majorBurnFlag) return false;
  return true;
}

/**
 * Compare two rows for the active sort column. The nullable numeric columns
 * (%TBSA, total 24 h volume) sort nulls last; the major-burn boolean sorts
...
function compareRows(a, b) {
  ...
  if (key === 'tbsaPercent' || key === 'total24hVolumeMl') {
...
function renderTableBody() {
  ...
  for (const row of rows) {
    ...
    if (row.majorBurnFlag) {
      tr.classList.add('row-critical');
    }

    const tbsaClassName = (row.tbsaPercent === null || row.tbsaPercent === undefined)
      ? 'class-cell class-cell-na'
      : 'class-cell';
    const volumeClassName = (row.total24hVolumeMl === null || row.total24hVolumeMl === undefined)
      ? 'class-cell class-cell-na'
      : 'class-cell';
    ...
    tr.innerHTML = `
      ...
      <td><span class="${tbsaClassName}">${esc(tbsaLabel(row.tbsaPercent))}</span></td>
      <td><span class="${volumeClassName}">${esc(volumeLabel(row.total24hVolumeMl))}</span></td>
      <td>
        <span class="flag-badge ${row.majorBurnFlag ? 'flag-yes' : 'flag-no'}">
          ${row.majorBurnFlag ? 'Yes' : 'No'}
        </span>
      </td>
    `;
    body.appendChild(tr);
...
function bindFilterInputs() {
  ...
  const majorBurn = document.getElementById('filter-major-burn');
  ...
  if (majorBurn) {
    majorBurn.addEventListener('change', () => {
      filters.majorBurn = majorBurn.value;
      renderAll();
    });
  }
  if (clearBtn) {
