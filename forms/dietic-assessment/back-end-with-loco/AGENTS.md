# Dietetic Assessment — back-end-with-loco/

Rust axum + Loco JSON API for the Dietetic Assessment form. Crate source under
`src/dietic_assessment/`; one migration and one entity per SQL table.
...
- `seed()` in `app.rs` resolves fixtures from `CARGO_MANIFEST_DIR` because the
  canonical route layout moved them to `src/dietic_assessment/fixtures/`; do
  not revert it to the scaffold's `base.join(...)` form.
