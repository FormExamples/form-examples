// Safety-flag detection for the Dietetic Assessment engine.
//
// Pure function returning safety flags using the grade_flag categories from
// SQL migration 11. Flags fire independently of the MUST score and are never
// suppressed by a dietitian override of the risk category — see
// ../../doc/safety-case-notes.md hazard H-07.
//
// Each flag is { flagId, category, priority, description, suggestedAction }.
// Flag IDs are stable and identical across every front-end and the back-end.

import { num } from './must-rules.js';

/**
 * Detect safety flags for a dietetic assessment.
 *
 * @param {object} data - the assessment data model
 * @param {object} context - { must, glim, sarcf, scoff, refeedingRisk, age }
...
function detectFlags(data, context) {
  ...
  const { must, glim, sarcf, scoff, refeedingRisk, age } = context;
  ...
  // --- Swallowing --------------------------------------------------------
  if (data.gastrointestinal.dysphagia === 'yes'
  ...
  const lowMicros = [
    num(data.biochemistry.ferritinUgPerL) !== null && num(data.biochemistry.ferritinUgPerL) < 30 ? 'ferritin' : null,
    num(data.biochemistry.vitaminB12NgPerL) !== null && num(data.biochemistry.vitaminB12NgPerL) < 180 ? 'vitamin B12' : null,
    num(data.biochemistry.folateUgPerL) !== null && num(data.biochemistry.folateUgPerL) < 3 ? 'folate' : null,
    num(data.biochemistry.vitaminDNmolPerL) !== null && num(data.biochemistry.vitaminDNmolPerL) < 25 ? 'vitamin D' : null
  ].filter(Boolean);
  ...
  const lowLytes = [
    num(data.biochemistry.potassiumMmolPerL) !== null && num(data.biochemistry.potassiumMmolPerL) < 3.5 ? 'potassium' : null,
    num(data.biochemistry.magnesiumMmolPerL) !== null && num(data.biochemistry.magnesiumMmolPerL) < 0.7 ? 'magnesium' : null,
    num(data.biochemistry.phosphateMmolPerL) !== null && num(data.biochemistry.phosphateMmolPerL) < 0.8 ? 'phosphate' : null
  ].filter(Boolean);
  ...
  const dehydrationSign = ['mild-dehydration', 'moderate-dehydration', 'severe-dehydration']
    .includes(data.hydration.hydrationSigns);
  ...
  // --- Food insecurity ----------------------------------------------------
  if (data.environment.worriedFoodWouldRunOut === 'yes'
  ...
  // --- Skin ---------------------------------------------------------------
  if (data.physicalExam.pressureUlcerPresent === 'yes'
  ...
  const lowGrip = grip !== null && (
    (data.patient.sex === 'male' && grip < 27) ||
    (data.patient.sex === 'female' && grip < 16)
  );
  ...
  const highProteinPlan = ['high-energy-high-protein'].includes(data.preferences.therapeuticDiet)
    || ['oral-nutritional-supplements', 'enteral-nutrition', 'parenteral-nutrition']
      .includes(data.plan.interventionType);
  ...
  if (data.behavioural.capacityConcern === 'yes'
