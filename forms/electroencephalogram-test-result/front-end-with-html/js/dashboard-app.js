...
// box + EEG-type dropdown + classification dropdown + follow-up-urgency
// dropdown). Columns mirror the SvelteKit dashboard route
// (`front-end-with-svelte/src/routes/+page.svelte`).
//
// Sibling modules loaded as plain `<script>` tags (in dependency order) attach
// their exports to `window.ElectroencephalogramTestResultDashboard`; display
...
function matchesFilters(row) {
  ...
  if (filters.eegType && row.eegType !== filters.eegType) return false;
...
function compareRows(a, b) {
  ...
  // Default: string compare (id, patientName, eegType, reportStatus,
...
function renderTableBody() {
  ...
  for (const row of rows) {
    ...
    tr.innerHTML = `
      ...
      <td>${esc(eegTypeLabel(row.eegType))}</td>
...
function bindFilterInputs() {
  ...
  const eegType = document.getElementById('filter-eeg-type');
  ...
  if (eegType) {
    eegType.addEventListener('change', () => {
      filters.eegType = eegType.value;
      renderAll();
    });
  }
  if (classification) {
