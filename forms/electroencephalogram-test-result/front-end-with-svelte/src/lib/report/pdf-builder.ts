...
/** Builds the pdfmake document definition for the EEG neurophysiology report. */
export function buildPdfDocument(
	data: ElectroencephalogramResult,
	...
	return {
		...
		header: {
			text: 'EEG NEUROPHYSIOLOGY REPORT',
			alignment: 'center',
			margin: [0, 20, 0, 0],
			fontSize: 10,
			color: '#6b7280',
		...
		footer: (currentPage: number, pageCount: number) => ({
			text: `Page ${currentPage} of ${pageCount} | Generated ${new Date(result.gradedAt).toLocaleString()} | ILAE / IFCN reporting standards`,
			alignment: 'center',
			margin: [0, 20, 0, 0],
			fontSize: 8,
			color: '#9ca3af'
		...
		content: [
			{
				text: 'ELECTROENCEPHALOGRAM TEST RESULT',
				fontSize: 18,
				bold: true,
				alignment: 'center',
			...
			{
				text: `${eegTypeLabel(data.eegType)} — ${reportStatusLabel(data.reportStatus)}`,
				fontSize: 12,
				alignment: 'center',
				color: '#4b5563',
				margin: [0, 0, 0, 4] as Margin
			...
			{
				ul: [
					`EEG type: ${eegTypeLabel(data.eegType)}`,
					`Recording duration: ${data.recordingDurationMinutes !== null ? `${data.recordingDurationMinutes} min` : 'N/A'}`,
					`Recording quality: ${data.recordingQuality || 'N/A'}`,
					`Background rhythm: ${backgroundRhythmLabel(data.backgroundRhythm)}`
				],
				margin: [0, 0, 0, 16] as Margin
			},
			...
			{ text: data.clinicalCorrelation || 'Not specified', margin: [0, 0, 0, 4] as Margin },
			{
...
function structuredFindingList(data: ElectroencephalogramResult): string[] {
	const list: string[] = [];
