 ...
 */
export function hasCriticalFinding(r: ElectroencephalogramResult): boolean {
	return r.statusEpilepticus || r.seizureRecorded || r.epileptiformDischarges;
}

/** Whether any structured abnormal finding is present. */
export function hasAnyAbnormalFinding(r: ElectroencephalogramResult): boolean {
	return (
...
export function isNormalStudy(r: ElectroencephalogramResult): boolean {
...
export function backgroundRhythmLabel(value: string): string {
	switch (value) {
		case 'normal':
			return 'Normal';
		case 'excess-slow':
