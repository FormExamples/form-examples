# Nursing Care Plan — SvelteKit front-end (wizard + dashboard)
...
for the care-plan specification and completeness engine. Lily Svelte headless
conventions: [`../../AGENTS-front-end-svelte.md`](../../AGENTS-front-end-svelte.md).

This is a MULTI-TABLE relational form: a parent care-plan header plus a repeating
`problems[]` list, each problem carrying nested `goals[]` and `interventions[]`
arrays and an inline evaluation (the ADPIE nursing process).

## Layout

- `src/lib/engine/` — pure engine (`types.ts`, `utils.ts`,
  `nursing-care-plan-rules.ts`, `nursing-care-plan-grader.ts`,
  `flagged-issues.ts`) + `nursing-care-plan-grader.test.ts`.
...
- `src/lib/components/ui/ListEditor.svelte` — generic repeating one-to-many row
  editor driving the problem list and each problem's nested goal / intervention
  lists.
...
- `src/routes/nursing-care-plans/` — RESTful routes:
...
- Empty string `''` for unanswered text / enum fields; `null` for numeric; `[]`
  for the problems list and each problem's goal / intervention lists.
...
- This is a documentation-and-completeness form: the engine derives a completeness
  class / plan STATUS and a percent, not a numeric score.
