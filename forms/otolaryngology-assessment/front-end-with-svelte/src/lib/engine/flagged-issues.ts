...
function sumNumbers(arr: (number | null)[]): number {
	let total = 0;
...
/**
 * Detects red-flag presentations that warrant urgent or specialist review,
 * independent of the SNOT-22 numeric total. These are safety-critical alerts
 * for the ENT clinician.
 */
export function detectAdditionalFlags(data: AssessmentData): AdditionalFlag[] {
	...
	if (data.pastEntHistory.headNeckRadiotherapy === 'yes') {
		flags.push({
			...
			category: 'Past History',
			message:
				'Prior head and neck radiotherapy — consider late effects (xerostomia, fibrosis, hypothyroidism).',
			priority: 'medium'
		});
	}
	...
	if (
		...
		flags.push({
			...
			category: 'Audiology',
			message:
				'Sudden-onset hearing loss reported — urgent ENT/audiology referral recommended within 24-48 hours.',
			priority: 'urgent'
		});
	}
	...
	const sleepItems = [
		snot.difficultyFallingAsleep,
		snot.wakingUpAtNight,
		snot.lackOfGoodNightsSleep,
		snot.wakingUpTired
	];
	...
	const psychItems = [snot.frustratedRestlessIrritable, snot.sad, snot.embarrassed, snot.fatigue];
