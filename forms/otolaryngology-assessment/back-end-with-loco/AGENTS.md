# Otolaryngology Assessment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Otolaryngology Assessment form, built with axum + Loco +
...
- [`otolaryngology_assessment/`](./otolaryngology_assessment/) — the Loco crate: `src/otolaryngology_assessment/` holds `app.rs`
...
`src/otolaryngology_assessment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd otolaryngology_assessment && cargo check --all-targets && cargo test
```
