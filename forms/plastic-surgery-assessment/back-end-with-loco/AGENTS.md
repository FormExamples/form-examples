# Plastic Surgery Assessment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Plastic Surgery Assessment form, built with axum + Loco +
...
- [`plastic_surgery_assessment/`](./plastic_surgery_assessment/) — the Loco crate: `src/plastic_surgery_assessment/` holds `app.rs`
...
`src/plastic_surgery_assessment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd plastic_surgery_assessment && cargo check --all-targets && cargo test
```
