// Flagged-issue detection. Independent of ASA/wound/complexity
// classification, this module raises clinician-facing alerts for
// safety-critical findings (emergency referral, body dysmorphia, difficult
// airway, malignant hyperthermia, dirty wound, anaphylaxis, etc.).
...
function detectAdditionalFlags(data) {
  ...
  for (let i = 0; i < data.medicationsAllergies.allergies.length; i++) {
    const allergy = data.medicationsAllergies.allergies[i];
  ...
  if (
    (data.medicalSurgicalHistory.diabetes === 'type-1' ||
      data.medicalSurgicalHistory.diabetes === 'type-2') &&
