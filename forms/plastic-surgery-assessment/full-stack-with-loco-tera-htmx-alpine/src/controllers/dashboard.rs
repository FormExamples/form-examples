...
/// GET /dashboard -- render the plastic-surgery dashboard with filters.
#[debug_handler]
async fn dashboard(
    ...
    // Server-side search across patient name and procedure.
    if let Some(ref term) = params.search {
        let term = term.to_lowercase();
        items.retain(|row| {
            row.patient_name.to_lowercase().contains(&term)
                || row.proposed_procedure.to_lowercase().contains(&term)
        });
    }
    ...
    if let Some(ref rt) = params.referral_type {
        if !rt.is_empty() {
            items.retain(|row| row.referral_type == *rt);
        }
    }
    ...
    context.insert("referral_type", &params.referral_type.unwrap_or_default());
