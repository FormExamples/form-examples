import type { MriScanResult, Flag, FlagPriority } from './types';
import { hasCriticalFinding } from './utils';

/**
 * Detects safety-critical flags independently of the four axes. Flag
 * categories mirror sql/07_create_table_mri_scan_test_result_grade_flag.sql.
 * Flags are returned sorted high → medium → low priority.
 */
export function detectFlags(r: MriScanResult): Flag[] {
	const flags: Flag[] = [];
	...
	if (hasCriticalFinding(r)) {
		flags.push({
			...
			priority: 'high',
			description: 'A critical finding (cord compression, haemorrhage, or infarct) is present.',
			suggestedAction:
	...
	// ─── abnormal-requiring-action (cord compression: surgical emergency) ───
	if (r.cordCompression) {
		flags.push({
			flagId: 'F-ABNORMAL-ACTION-001',
			category: 'abnormal-requiring-action',
			priority: 'high',
			description: 'Spinal-cord or cauda-equina compression requires emergency action.',
			suggestedAction: 'Alert the referrer urgently and arrange immediate spinal / neurosurgical review.'
		});
	}

	// ─── urgent-referral ───
	if (r.infectionInflammation || r.demyelination) {
		flags.push({
			flagId: 'F-URGENT-REFERRAL-001',
			category: 'urgent-referral',
			priority: 'medium',
			description:
				'Infection / inflammation or demyelination is present and may warrant urgent referral.',
	...
	if (
		(r.massOrLesion || r.demyelination) &&
