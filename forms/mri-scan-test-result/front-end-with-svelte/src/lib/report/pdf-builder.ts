...
import type { MriScanResult, GradingResult } from '#lib/engine/types.js';
import {
...
/** Builds the pdfmake document definition for the MRI scan radiology report. */
export function buildPdfDocument(
	...
	return {
		...
		header: {
			text: 'MRI SCAN RADIOLOGY REPORT',
			alignment: 'center',
			margin: [0, 20, 0, 0],
			fontSize: 10,
			color: '#6b7280',
		...
		footer: (currentPage: number, pageCount: number) => ({
			text: `Page ${currentPage} of ${pageCount} | Generated ${new Date(result.gradedAt).toLocaleString()} | RCR Standards for reporting`,
			alignment: 'center',
			margin: [0, 20, 0, 0],
			fontSize: 8,
			color: '#9ca3af'
		...
		content: [
			...
			sectionHeader('Examination'),
			{
				ul: [
					...
					`Sequences performed: ${data.sequencesPerformed || 'N/A'}`,
					`Structured-reporting category: ${data.reportingCategory || 'N/A'}`
				],
				margin: [0, 0, 0, 16] as Margin
			},
...
function structuredFindingList(data: MriScanResult): string[] {
	const list: string[] = [];
