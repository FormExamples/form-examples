// Declarative four-axis grading rules for the MRI Scan Test Result.
//
...
// `mri_scan_test_result_grade_rule` SQL table.
//
...
/**
 * @typedef {import('./types.js').MriScanResult} MriScanResult
 ...
 * - critical: a critical structured finding (cord / cauda-equina compression,
 *   haemorrhage, or infarct) is present.
...
function classifyResult(r) {
  ...
  if (hasCriticalFinding(r)) {
    firedRules.push({
      ...
      description:
        'A critical structured finding (cord compression, haemorrhage, or infarct) is present; classified as critical.'
    });
 ...
 * actionable-reporting principles and structured MRI reporting systems such as
 * ACR PI-RADS (prostate) and BI-RADS (breast):
 * - major: a critical finding, or a large lesion (>= 30 mm).
 * - moderate: an actionable abnormal finding (mass, disc herniation,
 *   demyelination, infection) or a measurable lesion (10-29 mm).
 * - minor: incidental-only findings.
 * - none: a normal study.
 *
 * The `reportingCategory` carries forward the structured-reporting label the
 * reporter recorded (e.g. a PI-RADS prostate or BI-RADS breast category) when
 * present, otherwise a short label suitable for downstream workflows.
 *
 * @param {MriScanResult} r
...
function gradeSeverity(r, classification) {
  ...
  if (hasCriticalFinding(r)) {
    ...
    return {
      ...
      reportingCategory: recorded || 'critical-actionable',
      firedRules
    };
  }
  ...
  if (size !== null && size >= 30) {
    ...
    return {
      abnormalitySeverity: 'major',
      reportingCategory: recorded || 'large-lesion',
      firedRules
    };
  }
  ...
  if (actionable) {
    firedRules.push({
      ...
      description:
        'An actionable abnormal finding (mass, disc herniation, demyelination, or infection) is present; severity graded moderate.'
    });
    const category =
      recorded || (size !== null && size >= 10 ? 'measurable-lesion' : 'actionable-finding');
  ...
  if (hasOnlyIncidentalFinding(r)) {
    ...
    return { abnormalitySeverity: 'minor', reportingCategory: recorded || 'incidental', firedRules };
  }
  ...
  return { abnormalitySeverity: 'none', reportingCategory: recorded || 'normal', firedRules };
}
 ...
 * clinical history, sequences/technique, comparison, findings, and impression.
 *
 * @type {Array<{ ruleId: string, category: string, label: string,
 *                present: (r: MriScanResult) => boolean }>}
 */
const completenessSections = [
  ...
  },
  {
    ...
    category: 'technique',
    label: 'sequences / technique',
    present: (r) => r.sequencesPerformed.trim() !== ''
  },
  {
 ...
 * critical finding (cord / cauda-equina compression, haemorrhage, or infarct)
