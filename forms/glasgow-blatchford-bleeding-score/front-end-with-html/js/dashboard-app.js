...
// Glasgow-Blatchford Bleeding Score — clinician dashboard (vanilla
// classic-script app).
//
...
// box + care-setting dropdown + risk-band dropdown + admit-flag dropdown).
//
// Sibling modules loaded as plain `<script>` tags (in dependency order) attach
// their exports to `window.GlasgowBlatchfordBleedingScoreDashboard`. The whole
...
const filters = {
  search: '',
  setting: '', // '' | 'emergency-department' | 'acute-medical-unit' | 'ward' | 'other'
...
const columns = [
  ...
  { key: 'gbsScore',          label: 'Score' },
  { key: 'riskBand',          label: 'Risk band' },
  { key: 'admitFlag',         label: 'Admit' }
];

// Rank used when sorting the risk-band column so very-low < low-moderate < high
...
function bandCss(band) {
  if (band === 'high') return 'risk-high';
  if (band === 'low-moderate') return 'risk-moderate';
  if (band === 'very-low') return 'risk-low';
  return '';
}
 ...
 * rank table; the nullable score sorts nulls last; the admit boolean sorts
...
function renderTableBody() {
  ...
  for (const row of rows) {
    ...
    const scoreClassName = row.gbsScore === null
    ...
    tr.innerHTML = `
      ...
      <td><span class="${scoreClassName}">${esc(scoreLabel(row.gbsScore))}</span></td>
      <td><span class="risk-badge ${bandCss(row.riskBand)}">${esc(bandLabel(row.riskBand))}</span></td>
      <td>
