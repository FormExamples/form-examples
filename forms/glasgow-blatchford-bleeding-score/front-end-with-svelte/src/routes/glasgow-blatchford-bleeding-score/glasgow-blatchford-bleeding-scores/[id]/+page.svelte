<script lang="ts">
	...
	import Step3LaboratoryMarkers from '$lib/components/steps/Step3LaboratoryMarkers.svelte';
	import Step4Haemodynamics from '$lib/components/steps/Step4Haemodynamics.svelte';
	import Step5ClinicalMarkers from '$lib/components/steps/Step5ClinicalMarkers.svelte';
	...
	function submit() {
		...
		assessment.result = calculateGbsGrade(assessment.data);
		goto(`/glasgow-blatchford-bleeding-score/glasgow-blatchford-bleeding-scores/${id}/report`);
	}
...
<main class="mx-auto max-w-3xl px-4 py-6">
	<header class="mb-6 no-print">
		<h1 class="text-2xl font-bold text-base-content">
			{isNew ? 'New Glasgow-Blatchford assessment' : `Glasgow-Blatchford assessment ${id}`}
		</h1>
		<p class="mt-1 text-sm text-base-content/70">
			Complete the six sections; the Glasgow-Blatchford score and risk band are computed on submit.
		</p>
	...
	<Form label="Glasgow-Blatchford assessment" onsubmit={submit}>
		<Step1Context />
		<Step2Identification />
		<Step3LaboratoryMarkers />
		<Step4Haemodynamics />
		<Step5ClinicalMarkers />
		<Step6Summary />
