import type { AssessmentData, GbsRule, RiskBand } from './types';

/**
 * Declarative Glasgow-Blatchford scoring helpers, risk banding, and rule table.
 *
 ...
 * (spec §4). The grader (`gbs-grader.ts`) sums the eight point contributions and
 * derives the risk band with its recommended management.
 *
 * Bands (spec §4):
 ...
 * grader raises a data-completeness flag separately in `flagged-issues.ts`.
 */

/**
 * Parameter 1 — blood urea (mmol/L). Returns 0, 2, 3, 4, or 6; 0 when unmeasured
 * (a missing input contributes no points, per spec §4).
 */
export function bloodUreaPoints(d: AssessmentData): 0 | 2 | 3 | 4 | 6 {
 ...
 */
export function haemoglobinPoints(d: AssessmentData): 0 | 1 | 3 | 6 {
...
/** Parameter 4 — systolic blood pressure (mmHg). Returns 0, 1, 2, or 3. */
export function systolicBloodPressurePoints(d: AssessmentData): 0 | 1 | 2 | 3 {
 ...
 */
export function riskBandFor(score: number): RiskBandInfo {
	if (score === 0) {
		return {
			riskBand: 'very-low',
...
function hasUrea(d: AssessmentData): boolean {
	const v = d.labs.bloodUrea;
	return v !== null && v !== undefined;
}
function hasHb(d: AssessmentData): boolean {
	const v = d.labs.haemoglobin;
	return v !== null && v !== undefined;
}
function hasSbp(d: AssessmentData): boolean {
	const v = d.haemodynamics.systolicBloodPressure;
	return v !== null && v !== undefined;
}
 ...
 * numeric parameters (urea, haemoglobin, systolic BP) exactly one row fires when
 * the value is recorded, including the 0-point normal band. The single-/
 * fixed-point markers (pulse, melaena, syncope, hepatic disease, cardiac
 * failure) fire only when they award points.
 */
export const gbsRules: GbsRule[] = [
	// ─── PARAMETER 1: BLOOD UREA ──────────────────────────────────
	{
		id: 'R-BLOOD-UREA-0POINT-01',
		parameter: 'blood-urea',
		points: 0,
		category: 'threshold-band',
		description: 'Blood urea < 6.5 mmol/L',
		evaluate: (d) => hasUrea(d) && bloodUreaPoints(d) === 0
	},
	{
		id: 'R-BLOOD-UREA-2POINT-01',
		parameter: 'blood-urea',
		points: 2,
		category: 'threshold-band',
		description: 'Blood urea 6.5-7.9 mmol/L',
		evaluate: (d) => bloodUreaPoints(d) === 2
	},
	{
		id: 'R-BLOOD-UREA-3POINT-01',
		parameter: 'blood-urea',
		points: 3,
		category: 'threshold-band',
		description: 'Blood urea 8.0-9.9 mmol/L',
		evaluate: (d) => bloodUreaPoints(d) === 3
	},
	{
		id: 'R-BLOOD-UREA-4POINT-01',
		parameter: 'blood-urea',
		points: 4,
		category: 'threshold-band',
		description: 'Blood urea 10.0-24.9 mmol/L',
		evaluate: (d) => bloodUreaPoints(d) === 4
	},
	{
		id: 'R-BLOOD-UREA-6POINT-01',
		parameter: 'blood-urea',
		points: 6,
		category: 'threshold-band',
		description: 'Blood urea >= 25.0 mmol/L',
		evaluate: (d) => bloodUreaPoints(d) === 6
	},

	// ─── PARAMETERS 2/3: HAEMOGLOBIN (SEX-SPECIFIC) ───────────────
	{
		id: 'R-HAEMOGLOBIN-0POINT-01',
	...
	{
		id: 'R-HAEMOGLOBIN-6POINT-01',
		parameter: 'haemoglobin',
		points: 6,
		category: 'threshold-band',
		description: 'Haemoglobin < 100 g/L (either sex)',
		evaluate: (d) => haemoglobinPoints(d) === 6
	},

	// ─── PARAMETER 4: SYSTOLIC BLOOD PRESSURE ─────────────────────
	{
		id: 'R-SYSTOLIC-BLOOD-PRESSURE-0POINT-01',
		parameter: 'systolic-blood-pressure',
		points: 0,
		category: 'threshold-band',
		description: 'Systolic blood pressure >= 110 mmHg',
		evaluate: (d) => hasSbp(d) && systolicBloodPressurePoints(d) === 0
	},
	{
		id: 'R-SYSTOLIC-BLOOD-PRESSURE-1POINT-01',
		parameter: 'systolic-blood-pressure',
		points: 1,
		category: 'threshold-band',
		description: 'Systolic blood pressure 100-109 mmHg',
		evaluate: (d) => systolicBloodPressurePoints(d) === 1
	},
	{
		id: 'R-SYSTOLIC-BLOOD-PRESSURE-2POINT-01',
		parameter: 'systolic-blood-pressure',
		points: 2,
		category: 'threshold-band',
		description: 'Systolic blood pressure 90-99 mmHg',
		evaluate: (d) => systolicBloodPressurePoints(d) === 2
	},
	{
		id: 'R-SYSTOLIC-BLOOD-PRESSURE-3POINT-01',
		parameter: 'systolic-blood-pressure',
		points: 3,
		category: 'threshold-band',
	...
	// ─── PARAMETER 5: PULSE ───────────────────────────────────────
	{
		id: 'R-PULSE-1POINT-01',
		parameter: 'pulse',
		points: 1,
		category: 'marker',
		description: 'Pulse >= 100 beats/min (tachycardia)',
		evaluate: (d) => pulsePoint(d) === 1
	},

	// ─── PARAMETER 6: MELAENA ─────────────────────────────────────
	{
		id: 'R-MELAENA-1POINT-01',
		parameter: 'melaena',
		points: 1,
		category: 'marker',
		description: 'Melaena present',
		evaluate: (d) => melaenaPoint(d) === 1
	},

	// ─── PARAMETER 7: SYNCOPE ─────────────────────────────────────
	{
		id: 'R-SYNCOPE-2POINT-01',
		parameter: 'syncope',
		points: 2,
		category: 'marker',
		description: 'Syncope present',
		evaluate: (d) => syncopePoint(d) === 2
	},

	// ─── PARAMETER 8: HEPATIC DISEASE ─────────────────────────────
	{
		id: 'R-HEPATIC-DISEASE-2POINT-01',
		parameter: 'hepatic-disease',
		points: 2,
		category: 'marker',
	...
	{
		id: 'R-CARDIAC-FAILURE-2POINT-01',
		parameter: 'cardiac-failure',
		points: 2,
		category: 'marker',
		description: 'History of cardiac failure',
