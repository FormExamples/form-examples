...
for the Glasgow-Blatchford specification and scoring engine. Lily Svelte headless
conventions: [`../../AGENTS-front-end-svelte.md`](../../AGENTS-front-end-svelte.md).

## Layout

- `src/lib/engine/` — pure scoring engine (`types.ts`, `utils.ts`,
  `gbs-rules.ts`, `gbs-grader.ts`, `flagged-issues.ts`) + `gbs-grader.test.ts`.
...
- `src/routes/glasgow-blatchford-bleeding-scores/` — RESTful routes:
  `/<plural>/` (dashboard, `ssr = false`) + `/<plural>/[id]` (wizard) +
  `/<plural>/[id]/report` (+ `report/pdf` server endpoint).

## Scoring engine

Weighted additive score: eight admission parameters — blood urea, haemoglobin
(sex-specific bands), systolic blood pressure, pulse, melaena, syncope, hepatic
disease, and cardiac failure — each contribute points by band (spec §4). The
total 0-23 bands into a risk level: 0 → very-low, 1-5 → low-moderate, ≥ 6 →
high. A missing numeric input contributes 0 points; unknown / unset sex falls
back to the female haemoglobin table. Both cases raise a data-completeness flag;
the total is only valid once all eight parameters and sex are recorded.
