<script lang="ts">
	...
	function init(api: { exec: (a: string, b: unknown) => void; on: (e: string, cb: (ev: { id?: string | number }) => void) => void }) {
		...
		api.on('select-row', (ev: { id?: string | number }) => {
			if (ev?.id != null) goto(`/workplace-stress-assessment/workplace-stress-assessments/${ev.id}`);
		});
	}
</script>
...
<main class="mx-auto max-w-6xl px-4 py-6">
	<div class="mb-6 flex flex-wrap items-end justify-between gap-4">
		...
		<a href="/workplace-stress-assessment/workplace-stress-assessments/new" class="button" data-variant="primary">New survey</a>
	</div>
