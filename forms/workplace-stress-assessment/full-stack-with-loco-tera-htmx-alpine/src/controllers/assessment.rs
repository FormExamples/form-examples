...
use crate::engine::{stress_grader, types::AssessmentData};
...
/// Parse a form value. Empty strings stay as `""`; integer-like strings
/// become JSON numbers so Likert fields deserialize as `Option<i32>`.
fn parse_form_value(value: &serde_json::Value, section: &str) -> serde_json::Value {
    if let Some(s) = value.as_str() {
        if s.is_empty() {
            // Likert / numeric domains use null for unanswered. Text-only
            // sections (demographics, additionalComments) use ''.
            return match section {
                "demographics" | "additionalComments" => serde_json::Value::String(String::new()),
    ...
    if let Some(obj) = data_value.as_object_mut() {
        if let Some(form_obj) = form_data.as_object() {
            for (key, value) in form_obj {
                // section.field -- the dotted name pattern used by step partials.
                ...
                if parts.len() == 2 {
                    ...
                    {
                        section_obj.insert(field, parse_form_value(value, section));
                    }
                }
            }
        }
...
async fn show_report(
    ...
    let grade = stress_grader::grade_stress(&assessment_data);
    ...
    context.insert("domains", &grade.domains);
