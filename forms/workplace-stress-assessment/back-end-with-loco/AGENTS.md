# Workplace Stress Assessment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Workplace Stress Assessment form, built with axum + Loco +
...
- [`workplace_stress_assessment/`](./workplace_stress_assessment/) — the Loco crate: `src/workplace_stress_assessment/` holds `app.rs`
...
`src/workplace_stress_assessment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd workplace_stress_assessment && cargo check --all-targets && cargo test
```
