...
use crate::engine::{kdigo_grader, types::AssessmentData};
...
/// GET /assessment/{id} -- render the single-page renal-assessment wizard.
#[debug_handler]
...
fn section_to_json(section: &str) -> &str {
    match section {
        ...
        "ckd_risk_factors" => "ckdRiskFactors",
        "physical_examination" => "physicalExamination",
        "blood_tests" => "bloodTests",
        "urine_tests" => "urineTests",
        "imaging_biopsy" => "imagingBiopsy",
...
/// Parse a form value. Empty strings stay as `""`; numbers convert to JSON
/// numbers so the typed engine deserializes them as `Option<i32>` / `Option<f64>`.
...
async fn submit_assessment(
    ...
    if let Some(obj) = data_value.as_object_mut() {
        if let Some(form_obj) = form_data.as_object() {
            // Collect dynamic medications[idx].field entries.
            ...
            for (key, value) in form_obj {
                // medications[0].name -- dynamic medication-list rows.
                if key.starts_with("medications[") {
            ...
            // Materialize medications under medicationReview.currentMedications
...
async fn show_report(
    ...
    let grade = kdigo_grader::grade(&assessment_data);
    ...
    context.insert("gfr_category", &grade.gfr_category);
    context.insert("albuminuria_category", &grade.albuminuria_category);
    context.insert("risk_level", &grade.risk_level);
    context.insert(
        "risk_label",
        &crate::engine::utils::risk_level_label(&grade.risk_level),
    );
    context.insert(
        "risk_class",
        &crate::engine::utils::risk_level_class(&grade.risk_level),
    );
    context.insert("egfr", &grade.egfr);
    context.insert("acr", &grade.acr);
