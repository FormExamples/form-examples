...
import { resolveEgfr, resolveAlbuminuriaCategory } from './kdigo-rules';

/**
 * Detects additional flags that should be highlighted for the nephrologist,
 * independent of the KDIGO classification: severe biochemical derangements,
 * advanced CKD requiring referral, anemia of CKD, mineral-bone disorder, AKI
 * on CKD, hypertension out of target, nephrotoxic exposure, and major risk
 * factors. These are safety-critical alerts.
 */
export function detectAdditionalFlags(data: AssessmentData): AdditionalFlag[] {
	...
	if (egfr !== null && egfr < 15) {
		flags.push({
			id: 'FLAG-EGFR-001',
			category: 'CKD Stage',
			message: `eGFR ${egfr} mL/min/1.73 m² (G5) - kidney failure; urgent nephrology referral and renal replacement therapy planning.`,
			priority: 'urgent'
		});
		...
		flags.push({
			id: 'FLAG-EGFR-002',
			category: 'CKD Stage',
			message: `eGFR ${egfr} mL/min/1.73 m² (G4) - severely decreased kidney function; nephrology referral required.`,
			priority: 'high'
		});
		...
		flags.push({
			id: 'FLAG-EGFR-003',
			category: 'CKD Stage',
			message: `eGFR ${egfr} mL/min/1.73 m² (G3b) - moderately to severely decreased; consider nephrology input.`,
			priority: 'medium'
		});
	}
