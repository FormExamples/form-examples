<script lang="ts">
	...
	$effect(() => {
		if (!assessment.result) {
			goto(`/renal-assessment/renal-assessments/${id}`);
		}
	});
	...
	async function downloadPDF() {
		pdfError = '';
		try {
			const res = await fetch(`/renal-assessments/${id}/report/pdf`, {
				method: 'POST',
			...
			if (res.ok) {
				...
				a.href = url;
				a.download = `renal-assessment-${data.demographics.lastName || id}.pdf`;
				a.click();
...
{#if result}
	<header class="border-b border-base-300 bg-base-100 shadow-sm no-print">
		<div class="mx-auto flex max-w-4xl items-center justify-between px-4 py-4">
			<h1 class="text-lg font-bold text-base-content">Renal assessment report</h1>
			<div class="flex items-center gap-3">
				...
				<Button data-variant="secondary" onclick={() => goto(`/renal-assessment/renal-assessments/${id}`)}>Edit</Button>
			</div>
		</div>
	</header>

	<main class="mx-16 px-4 py-6">
		<!-- KDIGO composite risk banner -->
		<div class="mb-6 rounded-xl border-2 p-6 text-center {riskLevelColor(result.riskLevel)}">
			...
			<div class="mt-2 flex flex-wrap justify-center gap-6 text-sm">
				{#if result.gfrCategory}<span>{gfrCategoryLabel(result.gfrCategory)}</span>{/if}
				{#if result.albuminuriaCategory}<span>{albuminuriaCategoryLabel(result.albuminuriaCategory)}</span>{/if}
			</div>
			<div class="mt-2 text-sm opacity-75">
				{result.egfr != null ? `eGFR ${result.egfr} mL/min/1.73 m²` : 'eGFR unavailable'} —
				{result.acr != null ? `ACR ${result.acr} mg/mmol` : 'ACR unavailable'}
			</div>
			<div class="mt-2 text-sm opacity-75">
		...
		{#if result.additionalFlags.length > 0}
			<div class="mb-6 rounded-xl border border-error/40 bg-base-100 p-6">
				<h2 class="mb-4 text-lg font-bold text-error">Flagged issues for nephrologist</h2>
				<div class="space-y-2">
		...
		{#if result.firedRules.length > 0}
			<div class="mb-6 rounded-xl border border-base-300 bg-base-100 p-6">
				<h2 class="mb-4 text-lg font-bold text-base-content">KDIGO classification</h2>
					...
					<tbody>
						{#each result.firedRules as rule (rule.id)}
							<tr class="border-b border-base-200">
								...
								<td class="py-2 font-medium">{rule.value}</td>
							</tr>
						{/each}
					</tbody>
				</table>
		...
		{:else}
			<div class="mb-6 rounded-xl border border-base-300 bg-base-100 p-6">
				<p class="text-sm text-base-content/70">
					No KDIGO classification possible — supply eGFR (or creatinine + age + sex) and urine ACR.
				</p>
			</div>
		{/if}
