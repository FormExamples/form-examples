<script lang="ts">
	...
	function submit() {
		...
		assessment.result = calculateKdigo(assessment.data);
		goto(`/renal-assessment/renal-assessments/${id}/report`);
	}
...
<main class="mx-16 px-4 py-6">
	<h1 class="text-2xl font-bold text-base-content">
		{isNew ? 'New renal assessment' : `Renal assessment ${id}`}
	</h1>
	<p class="mt-1 text-sm text-base-content/70">
		Complete the nine sections; the KDIGO GFR / albuminuria categories and composite risk are
		computed on submit.
	</p>
	...
	<Form label="Renal assessment" onsubmit={submit}>
		<Step1Demographics />
		...
		<Step5BloodTests />
		<Step6UrineTests />
		<Step7ImagingBiopsyReview />
		<Step8MedicationReviewDoseAdjustment />
		<Step9ClinicalImpressionKDIGOStage />
