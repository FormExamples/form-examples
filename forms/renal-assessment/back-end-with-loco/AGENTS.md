# Renal Assessment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Renal Assessment form, built with axum + Loco +
...
- [`renal_assessment/`](./renal_assessment/) — the Loco crate: `src/renal_assessment/` holds `app.rs`
...
`src/renal_assessment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd renal_assessment && cargo check --all-targets && cargo test
```
