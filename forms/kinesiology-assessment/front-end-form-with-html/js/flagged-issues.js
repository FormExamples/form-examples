// Flagged-issue detection. Independent of the FMS total score (which the
// grader computes), this module raises clinician-facing flags for:
//   - Pain during any movement (high)
//   - Inability (score 1) on any pattern (medium)
//   - Asymmetry between left/right scores (>1: high; ==1: medium)
//   - Clearing-test failures (shoulder, trunk flexion, trunk extension; high)
//   - Patient-reported severe current pain (high)
//   - Total FMS at or below threshold of 14 (medium)
 ...
 */
function getEffectiveScore(pattern) {
...
function calculateQuickTotal(patterns) {
  let total = 0;
  for (const entry of patterns) {
    const s = getEffectiveScore(entry[1]);
    if (s !== null) total += s;
  }
  return total;
}
