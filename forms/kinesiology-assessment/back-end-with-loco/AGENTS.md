# Kinesiology Assessment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Kinesiology Assessment form, built with axum + Loco +
...
- [`kinesiology_assessment/`](./kinesiology_assessment/) — the Loco crate: `src/kinesiology_assessment/` holds `app.rs`
...
`src/kinesiology_assessment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd kinesiology_assessment && cargo check --all-targets && cargo test
```
