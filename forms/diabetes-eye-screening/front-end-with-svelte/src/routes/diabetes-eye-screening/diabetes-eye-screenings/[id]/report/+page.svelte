<script lang="ts">
	...
	$effect(() => {
		if (!assessment.result) {
			goto(`/diabetes-eye-screening/diabetes-eye-screenings/${id}`);
		}
	});
	...
	async function downloadPDF() {
		pdfError = '';
		try {
			const res = await fetch(`/diabetes-eye-screenings/${id}/report/pdf`, {
				method: 'POST',
			...
			if (res.ok) {
				...
				a.href = url;
				a.download = `diabetes-eye-screening-${data.identification.patientIdentifier || id}.pdf`;
				a.click();
...
{#if result}
	<header class="border-b border-base-300 bg-base-100 shadow-sm no-print">
		<div class="mx-auto flex max-w-4xl items-center justify-between px-4 py-4">
			<h1 class="text-lg font-bold text-base-content">Diabetes eye screening report</h1>
			<div class="flex items-center gap-3">
				...
				<Button data-variant="secondary" onclick={() => goto(`/diabetes-eye-screening/diabetes-eye-screenings/${id}`)}
					>Edit</Button
				>
			</div>
		</div>
