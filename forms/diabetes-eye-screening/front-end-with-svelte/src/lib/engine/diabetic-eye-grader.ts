...
import { classificationRules, deriveContext } from './diabetes-eye-rules';
...
/**
 * Pure function: compute the full diabetes-eye-screening classification for the
...
export function calculateGrade(data: AssessmentData): GradingResult {
	...
	for (const rule of classificationRules) {
		try {
			...
			console.warn(`Diabetes-eye-screening rule ${rule.id} evaluation failed:`, e);
		}
	}
