...
function storageKey(id: string): string {
	return `diabetes-eye-screening.front-end-with-svelte.${id || 'new'}.v1`;
}

/** A blank diabetes-eye-screening record with all fields at their unanswered defaults. */
...
/**
 * Svelte 5 reactive store for the diabetes-eye-screening record, with
 * localStorage persistence so an in-progress record survives a page reload.
 * Drafts are keyed by screening id so each record edits independently.
 */
class AssessmentStore {
	 ...
	 * (e.g. `const d = assessment.data.rightEye`) stay bound to live state.
	 */
