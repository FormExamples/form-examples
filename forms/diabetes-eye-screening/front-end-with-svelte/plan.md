# Plan: Diabetes Eye Screening — SvelteKit front-end (form + dashboard)
...
and porting the diabetes-eye-screening classification engine from the HTML
front-end (`front-end-with-html/js/{types,rules,grader,flags}.js`).

## Done

- Ported the pure classification engine to TypeScript: `types.ts`,
  `diabetes-eye-rules.ts` (the worst-eye derivation helpers — `worstRetinopathy`
  ...
  `classificationRules`), `diabetes-eye-grader.ts` (`calculateGrade` — worst-eye
...
- Engine unit tests (`diabetes-eye-grader.test.ts`) with a local
...
- Id-keyed Svelte 5 store with `deepAssign` in-place deep-merge, localStorage
  key `diabetes-eye-screening.front-end-with-svelte.<id>.v1`, and
...
- Five wizard step components (grading context, patient identification, right
  ...
  readout.
- RESTful routes under `src/routes/diabetes-eye-screenings/`: SVAR dashboard
