...
for the diabetes-eye-screening specification and classification engine. Lily
...
- `src/lib/engine/` — pure classification engine (`types.ts`, `utils.ts`,
  `diabetes-eye-rules.ts`, `diabetes-eye-grader.ts`, `flagged-issues.ts`) +
  `diabetes-eye-grader.test.ts`.
...
- `src/routes/diabetes-eye-screenings/` — RESTful routes: `/<plural>/` (dashboard,
