# Diabetes Eye Screening — SvelteKit front-end (form + dashboard)

Consolidated SvelteKit front-end for the Diabetes Eye Screening record under the
...
- **Wizard** (`/diabetes-eye-screenings/[id]`) — the five-step screening record;
  classifies on submit.
- **Report** (`/diabetes-eye-screenings/[id]/report`) — outcome banner,
  interpretation, per-eye grading, outcome detail, and flagged issues; PDF via
  `report/pdf`.
- **Dashboard** (`/diabetes-eye-screenings`) — SVAR DataGrid of screened patients
