// Flagged-issue detection (safety flags). Independent of the recall / referral
// pathway (which the grader produces), this module raises grader-facing safety
// flags per spec §5:
//
...
//   - Eligibility (low)                         — age band < 12, or diabetes type not diabetic
//
// Rows here mirror the `diabetes_eye_screening_grade_flag` SQL table
...
function detectFlaggedIssues(data, grade) {
  ...
  if (data.rightEye.maculopathy === 'M1' || data.leftEye.maculopathy === 'M1') {
    flags.push({
      ...
      priority: 'high',
      description:
        'Diabetic maculopathy (M1) in at least one eye — referable disease.',
      suggestedAction:
        'Refer to the hospital eye service for assessment of maculopathy.'
    });
  }
  ...
  if (eitherEyeR(data, 'R3S')) {
    flags.push({
      ...
      suggestedAction:
        'Refer to / keep under hospital eye service surveillance.'
    });
  }
  ...
  if (eitherEyeR(data, 'R2')) {
    flags.push({
      ...
      priority: 'medium',
      description:
        'Pre-proliferative diabetic retinopathy (R2) in at least one eye.',
      suggestedAction:
  ...
  if (gapMonths !== null && gapMonths > 12) {
    flags.push({
      ...
      priority: 'medium',
      description:
        `The previous screen was ${gapMonths} months before this grading — beyond the recommended recall interval.`,
      suggestedAction:
