# Psychiatry Waiting List Card — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Psychiatry Waiting List Card form, built with axum + Loco +
...
- [`psychiatry_waiting_list_card/`](./psychiatry_waiting_list_card/) — the Loco crate: `src/psychiatry_waiting_list_card/` holds `app.rs`
...
`/metrics`. The registered domain controllers are:
  ...
  - `psychiatry_waiting_list_card`
  - `psychiatry_waiting_list_card_appointment`
  - `psychiatry_waiting_list_card_grade`
  - `psychiatry_waiting_list_card_grade_flag`
  - `psychiatry_waiting_list_card_grade_rule`
  - `patient`
  - `practitioner`

## Engine

`src/psychiatry_waiting_list_card/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd psychiatry_waiting_list_card && cargo check --all-targets && cargo test
```
