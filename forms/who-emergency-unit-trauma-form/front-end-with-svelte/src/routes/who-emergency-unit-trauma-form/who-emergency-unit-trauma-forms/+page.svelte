<script lang="ts">
	...
	const rows = $derived(
		sampleAssessmentRows.filter(
			(r) =>
				(triageFilter === '' || r.triage === triageFilter) &&
	...
	const dispositionLabel = (v: Disposition) =>
		v === 'admit'
			? 'Admit'
			: v === 'transfer'
				? 'Transfer'
	...
	// SVAR DataGrid columns. Triage classification, completeness status, and flag
	...
	const columns = [
		...
		{ id: 'recordedDate', header: 'Recorded', width: 120, sort: true },
		{
			...
			width: 160,
			sort: true,
			template: (v: TriageCategory) => triageLabel(v)
		},
		{
			id: 'disposition',
			header: 'Disposition',
			width: 120,
			sort: true,
			template: (v: Disposition) => dispositionLabel(v)
		},
		{
			id: 'complete',
			header: 'Status',
	...
	function init(api: any) {
		...
		api.on('select-row', (ev: { id?: string | number }) => {
			if (ev?.id != null) goto(`/who-emergency-unit-trauma-form/who-emergency-unit-trauma-forms/${ev.id}`);
		});
	}
</script>
...
<main class="mx-auto max-w-6xl px-4 py-6">
	<div class="mb-6 flex flex-wrap items-end justify-between gap-4">
		<div>
			<h1 class="text-2xl font-bold text-base-content">Emergency unit trauma dashboard</h1>
			<p class="mt-1 text-sm text-base-content/70">
				Triage category, completeness status, and flagged issues for recorded trauma encounters,
				computed by the shared engine. Select a row to open the encounter.
			</p>
		</div>
		<a href="/who-emergency-unit-trauma-form/who-emergency-unit-trauma-forms/new" class="button" data-variant="primary"
			>New encounter</a
		>
	</div>
