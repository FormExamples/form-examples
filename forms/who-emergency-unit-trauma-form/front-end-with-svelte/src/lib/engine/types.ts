...
/**
 * Combined engine output for a WHO Emergency Unit (Trauma) encounter.
 *
 * This is a status / classification form (no numeric grade): the headline
 * classification is the triage category (RED / YELLOW / GREEN) together with
 * the completeness status and the flag counts by priority. The wizard, report,
 * and dashboard all consume this single result so they stay aligned.
 */
export interface EuTraumaResult {
	validation: ValidationResult;
	flags: FlaggedIssue[];
	/** Assigned triage category (RED / YELLOW / GREEN), or '' when not triaged. */
