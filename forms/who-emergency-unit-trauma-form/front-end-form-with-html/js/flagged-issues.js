...
// AVPU, low GCS, hypoglycaemia, RED triage with missing airway /
// breathing interventions, dead-on-arrival without time of death,
// polytrauma / penetrating / crush, anticoagulation, pregnancy,
// unstable pelvis, FAST positive, high-energy mechanism, prolonged
// loss of consciousness, or other findings that warrant the
...
function detectFlaggedIssues(data) {
  ...
  if (hasNumber(data.disability.gcsTotal)) {
    const gcs = data.disability.gcsTotal;
    if (gcs <= 8) {
      flags.push({
        id: 'FLAG-GCS-LOW',
        category: 'Neurological',
  ...
  if (
    ...
    data.disability.bloodGlucose < 65
  ) {
    flags.push({
      id: 'FLAG-GLUC-LOW',
      category: 'Metabolic',
