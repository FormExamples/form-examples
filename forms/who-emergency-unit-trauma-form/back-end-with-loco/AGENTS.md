# WHO Emergency Unit Form: Trauma — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the WHO Emergency Unit Form: Trauma form, built with axum + Loco +
...
- [`who_emergency_unit_trauma_form/`](./who_emergency_unit_trauma_form/) — the Loco crate: `src/who_emergency_unit_trauma_form/` holds `app.rs`
...
`src/who_emergency_unit_trauma_form/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd who_emergency_unit_trauma_form && cargo check --all-targets && cargo test
```
