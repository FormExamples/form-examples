# Post-Anaesthesia Care Unit (PACU) Record — tasks
...
- [x] Author `spec/index.md` (data model, Aldrete grading algorithm, flagged
      issues, I/O shapes, acceptance criteria).
- [x] Author `AGENTS.md` (directory map, scoring engine shape and files,
      conventions, compliance).
- [x] Author `plan.md` and `tasks.md`.
- [x] SQL migrations in `sql/` (recovery-record table, UUIDv4 PK, timestamps).
- [x] Generated XML, FHIR R5, protobuf, OpenAPI, Loco setup.
- [x] Scoring engine (`types.ts`, `utils.ts`, `aldrete-rules.ts`,
      `aldrete-grader.ts`, `flagged-issues.ts`) + Vitest tests.
...
- [ ] `bin/test-form post-anaesthesia-care-unit-record` passes.
