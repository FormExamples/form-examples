# Ophthalmology Assessment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Ophthalmology Assessment form, built with axum + Loco +
...
- [`ophthalmology_assessment/`](./ophthalmology_assessment/) — the Loco crate: `src/ophthalmology_assessment/` holds `app.rs`
...
`src/ophthalmology_assessment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd ophthalmology_assessment && cargo check --all-targets && cargo test
```
