...
// Independent of the VA grade, this module raises additional safety-critical
// alerts: sudden onset, raised IOP, RAPD, retinal detachment markers,
// diabetic retinopathy stages, optic disc abnormality, visual field defect,
// ophthalmic drug allergies, previous eye surgery, thyroid eye disease,
// falls risk, colour vision defect, anterior chamber abnormality.
...
function detectAdditionalFlags(data) {
  ...
  for (let i = 0; i < data.currentMedications.ophthalmicDrugAllergies.length; i++) {
    const allergy = data.currentMedications.ophthalmicDrugAllergies[i];
    if (allergy && allergy.severity === 'anaphylaxis') {
      flags.push({
        id: `FLAG-ALLERGY-ANAPH-${i}`,
        category: 'Allergy',
        message: `ANAPHYLAXIS history to ophthalmic drug: ${allergy.allergen || '(allergen not specified)'}`,
        priority: 'high'
      });
    }
  }
