...
/** Short Visual Acuity grade label (dashboard column). */
export function vaGradeShortLabel(grade: VAGrade): string {
	switch (grade) {
		case 'normal':
			return 'Normal';
		case 'mild':
...
/** Visual Acuity grade colour class (Lily design tokens). */
...
/** IOP status colour class (Lily design tokens). */
export function iopStatusColor(iop: number | null): string {
	if (iop === null) return 'text-base-content/60';
	if (iop <= 21) return 'text-success';
	if (iop <= 30) return 'text-warning';
	return 'text-error';
}
