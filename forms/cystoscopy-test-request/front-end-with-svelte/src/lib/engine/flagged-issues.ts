...
//
// Ported verbatim (flag IDs preserved) from the HTML front-end's js/flags.js.
...
// other. Each flag is { flagId, category, priority, description,
// suggestedAction }. Flag IDs are identical across every front-end and the
// back-end.

import type { CystoscopyRequest, Flag } from './types';
...
export function detectFlags(data: CystoscopyRequest, context?: FlagContext): Flag[] {
	const flags: Flag[] = [];
	const ctx = context || {};
	...
	if (
		...
		flags.push({
			...
			suggestedAction:
				'Refer on the suspected-cancer two-week-wait pathway; book the cystoscopy within 14 days.'
		});
	}
	...
	if (
		...
		flags.push({
			...
			suggestedAction:
				'Per BAUS, any episode of visible haematuria warrants urological assessment including cystoscopy and upper-tract imaging.'
		});
	}
	...
	if (data.symptoms.currentUti === true) {
		flags.push({
			...
			suggestedAction:
				'Defer instrumentation: treat the active UTI first and reschedule once resolved to avoid urosepsis.'
		});
	}
	...
	if (data.bleeding.takingAnticoagulant === true) {
		...
		flags.push({
			...
			suggestedAction:
				'Confirm the agent and indication; plan peri-procedure hold / bridging per local anticoagulation policy before any biopsy or resection.'
		});
	}
