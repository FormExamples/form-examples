...
import type { CystoscopyRequest, GradingResult } from '#lib/engine/types.js';
import { createDefaultRequest } from '#lib/engine/defaults.js';

// Re-export the default-request factory so callers can import it from the store.
...
function storageKey(id: string): string {
	return `cystoscopy-test-request.front-end-with-svelte.${id || 'new'}.v1`;
}

/**
 * Svelte 5 reactive store for the cystoscopy request, with localStorage
 ...
 */
class RequestStore {
	data = $state<CystoscopyRequest>(createDefaultRequest());
	 ...
	 * (e.g. `const c = requestStore.data.clinician`) stay bound to live state.
	 */
	loadForId(id: string, seed?: CystoscopyRequest) {
		const key = id || 'new';
		this.id = key;
		this.result = null;
		this.currentStep = 1;

		let draft: CystoscopyRequest | null = null;
		if (browser) {
			const raw = localStorage.getItem(storageKey(key));
			if (raw) {
				try {
					draft = JSON.parse(raw) as CystoscopyRequest;
				} catch {
