# Structured Medication Review (SMR): Back-end with Rust Axum Loco (JSON API)

Server-side Rust JSON API for the Structured Medication Review (SMR) form. Pure back-end: no HTML
rendering, no Tera templates, no HTMX, no Alpine.js, no CSS, no Lily
Design System.

The Loco crate lives in [`structured_medication_review/`](structured_medication_review/) — one migration, SeaORM
