// Flagged-issue detection for the hematology assessment. Independent of the
// composite abnormality score, this module raises clinician-facing flags for
// pancytopenia, severe leukopenia, polycythemia, suspected DIC, prolonged
// aPTT, severely elevated D-dimer, iron deficiency / overload patterns,
// abnormal blood film, positive hemoglobinopathy screens, prior transfusion
// reactions, and high clinical urgency.
...
function detectAdditionalFlags(data) {
  ...
  // ─── Severe neutropenia / leukopenia risk ───────────────
  if (
    data.bloodCountAnalysis.whiteBloodCellCount !== null &&
    data.bloodCountAnalysis.whiteBloodCellCount < 1.0
  ) {
    flags.push({
      id: 'FLAG-CBC-002',
      category: 'Blood Count',
  ...
  // ─── Polycythemia ───────────────────────────────────────
  if (
    data.bloodCountAnalysis.hemoglobin !== null &&
    data.bloodCountAnalysis.hemoglobin > 18.5
  ) {
    flags.push({
      id: 'FLAG-CBC-003',
      category: 'Blood Count',
  ...
  // ─── Prolonged aPTT ─────────────────────────────────────
  if (
    data.coagulationStudies.activatedPartialThromboplastinTime !== null &&
    data.coagulationStudies.activatedPartialThromboplastinTime > 60.0
  ) {
    flags.push({
      id: 'FLAG-COAG-002',
      category: 'Coagulation',
  ...
  // ─── Severely elevated D-dimer ──────────────────────────
  if (
    data.coagulationStudies.dDimer !== null &&
    data.coagulationStudies.dDimer > 5.0
  ) {
    flags.push({
      id: 'FLAG-COAG-003',
      category: 'Coagulation',
  ...
  if (
    data.ironStudies.serumFerritin !== null &&
    data.ironStudies.serumFerritin > 1000.0
  ) {
    flags.push({
      id: 'FLAG-IRON-002',
      category: 'Iron Studies',
  ...
  // ─── Abnormal blood film ────────────────────────────────
  const abnormalCells = (data.peripheralBloodFilm.abnormalCellMorphology || '').trim().toLowerCase();
  if (
    abnormalCells !== '' &&
    abnormalCells !== 'none' &&
    abnormalCells !== 'normal'
  ) {
    flags.push({
      id: 'FLAG-FILM-001',
      category: 'Blood Film',
  ...
  const reactions = data.transfusionHistory.transfusionReactions;
  if (
    reactions !== '' &&
    reactions !== 'none' &&
    reactions !== 'no'
  ...
  if (
    data.clinicalReview.urgencyLevel !== null &&
    data.clinicalReview.urgencyLevel >= 4
  ) {
    flags.push({
      id: 'FLAG-CLIN-001',
      category: 'Clinical',
