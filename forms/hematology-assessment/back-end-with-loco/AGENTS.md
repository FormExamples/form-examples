# Hematology Assessment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Hematology Assessment form, built with axum + Loco +
...
- [`hematology_assessment/`](./hematology_assessment/) — the Loco crate: `src/hematology_assessment/` holds `app.rs`
...
`src/hematology_assessment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd hematology_assessment && cargo check --all-targets && cargo test
```
