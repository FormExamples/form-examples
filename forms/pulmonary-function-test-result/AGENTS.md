# Pulmonary Function Test Result — agent instructions

Pulmonary function test (lung-function / spirometry) result (report). A reporting
clinician completes a single-page wizard recording the performed test's
**measured values** and a structured **interpretation**; the engine computes a
...
and produces a structured lung-function report.

This is the **result/report counterpart** to `pulmonary-function-test-request`
(a referral). It mirrors the conventions of the gold template
`ct-scan-test-result`.

## Result semantics

A result form records *what the test found and what it means* — it is **not** a
referral. The source-of-truth record is `pulmonary_function_test_result`; the
clinician is the report **author/signer** (respiratory physician, clinical
physiologist, other), not a referrer. The engine **interprets findings** rather
...
- **Axis B — severity & structured reporting:** abnormalitySeverity
  ...
  (e.g. a GOLD 1–4 or ATS/ERS z-score severity band).
- **Axis C — report completeness:** 0–100 % of mandatory report sections
  (history, measured values, interpretation, findings, impression).
- **Axis D — follow-up urgency:** routine / recommended / urgent / critical-alert,
  with a target timeframe and a recommended action.

A **critical finding** (severe / very-severe obstruction or restriction, marked
decline in FEV1 versus a previous study, or another unexpected significant
