...
import type { PulmonaryFunctionResult, GradingResult } from '#lib/engine/types.js';
import {
...
/** Builds the pdfmake document definition for the pulmonary function report. */
export function buildPdfDocument(
	data: PulmonaryFunctionResult,
	...
	return {
		...
		header: {
			text: 'PULMONARY FUNCTION REPORT',
			alignment: 'center',
			margin: [0, 20, 0, 0],
			fontSize: 10,
			color: '#6b7280',
		...
		footer: (currentPage: number, pageCount: number) => ({
			text: `Page ${currentPage} of ${pageCount} | Generated ${new Date(result.gradedAt).toLocaleString()} | ATS/ERS 2022 / GOLD / NICE NG115`,
			alignment: 'center',
			margin: [0, 20, 0, 0],
			fontSize: 8,
			color: '#9ca3af'
		...
		content: [
			{
				text: 'PULMONARY FUNCTION TEST RESULT',
				fontSize: 18,
				bold: true,
				alignment: 'center',
			...
			{
				text: `${testTypeLabel(data.testType)} — ${reportStatusLabel(data.reportStatus)}`,
				fontSize: 12,
				alignment: 'center',
				color: '#4b5563',
				margin: [0, 0, 0, 4] as Margin
			...
			{
				ul: [
					`Test type: ${testTypeLabel(data.testType)}`,
					`Test quality: ${data.testQuality || 'N/A'}`,
					`Ventilatory pattern: ${ventilatoryPatternLabel(data.ventilatoryPattern)}`,
					`Severity: ${severityLabel(data.severity)}`,
					`Bronchodilator reversibility: ${bronchodilatorReversibilityLabel(data.bronchodilatorReversibility)}`
				],
				margin: [0, 0, 0, 16] as Margin
			},

			// Measured values
			sectionHeader('Measured values'),
			{
				ul: [
					`FEV1: ${data.fev1Litres !== null ? `${data.fev1Litres} L` : 'N/A'}${data.fev1PercentPredicted !== null ? ` (${data.fev1PercentPredicted}% predicted)` : ''}`,
					`FVC: ${data.fvcLitres !== null ? `${data.fvcLitres} L` : 'N/A'}${data.fvcPercentPredicted !== null ? ` (${data.fvcPercentPredicted}% predicted)` : ''}`,
					`FEV1/FVC ratio: ${data.fev1FvcRatio !== null ? data.fev1FvcRatio : 'N/A'}`,
					`Peak expiratory flow: ${data.peakExpiratoryFlow !== null ? `${data.peakExpiratoryFlow} L/min` : 'N/A'}`,
					`DLCO: ${data.dlcoPercentPredicted !== null ? `${data.dlcoPercentPredicted}% predicted` : 'N/A'}`
				],
				margin: [0, 0, 0, 16] as Margin
			},
...
function structuredFindingList(data: PulmonaryFunctionResult): string[] {
	const list: string[] = [];
