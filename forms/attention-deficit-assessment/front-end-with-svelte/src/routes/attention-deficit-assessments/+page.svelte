<script lang="ts">
	...
	import { asrsClassificationLabel, adhdSubtypeLabel } from '$lib/engine/utils';
	...
	function init(api: any) {
		...
		api.on('select-row', (ev: { id?: string | number }) => {
			if (ev?.id != null) goto(`/attention-deficit-assessments/${ev.id}`);
		});
	}
</script>
...
<main class="mx-auto max-w-6xl px-4 py-6">
	<div class="mb-6 flex flex-wrap items-end justify-between gap-4">
		...
		<a href="/attention-deficit-assessments/new" class="button" data-variant="primary"
			>New assessment</a
		>
	</div>
