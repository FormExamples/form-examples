<script lang="ts">
	...
	$effect(() => {
		if (!assessment.result) {
			goto(`/attention-deficit-assessment/attention-deficit-assessments/${id}`);
		}
	});
	...
	async function downloadPDF() {
		pdfError = '';
		try {
			const res = await fetch(`/attention-deficit-assessments/${id}/report/pdf`, {
				method: 'POST',
			...
			if (res.ok) {
				...
				a.href = url;
				a.download = `adhd-assessment-${data.demographics.lastName || id}.pdf`;
				a.click();
...
{#if result}
	<header class="border-b border-base-300 bg-base-100 shadow-sm no-print">
		<div class="mx-auto flex max-w-4xl items-center justify-between px-4 py-4">
			<h1 class="text-lg font-bold text-base-content">ADHD assessment report</h1>
			<div class="flex items-center gap-3">
				...
				<Button
					...
					onclick={() => goto(`/attention-deficit-assessment/attention-deficit-assessments/${id}`)}>Edit</Button
				>
			</div>
		</div>
	</header>
	...
	<main class="mx-auto max-w-4xl px-4 py-6">
		...
		<div class="mb-6 rounded-xl border-2 p-6 text-center {asrsClassificationColor(result.classification)}">
			<div class="text-3xl font-bold">{asrsClassificationLabel(result.classification)}</div>
			<div class="mt-2 flex flex-wrap justify-center gap-6 text-sm">
				<span>ASRS total {result.asrsTotal}/72</span>
				<span>{adhdSubtypeLabel(result.subtype)}</span>
				<span>Part A screener {result.partAScreenerPositive ? 'positive' : 'negative'}</span>
			</div>
		...
		<div class="mb-6 rounded-xl border border-base-300 bg-base-100 p-6">
			<h2 class="mb-4 text-lg font-bold text-base-content">Score breakdown</h2>
			<div class="grid grid-cols-1 gap-4 text-sm sm:grid-cols-2">
				<div><span class="font-medium text-base-content/70">Part A score:</span> {result.partAScore}/24</div>
				<div><span class="font-medium text-base-content/70">Part B score:</span> {result.partBScore}/48</div>
				<div>
					<span class="font-medium text-base-content/70">Inattentive subscore:</span>
					{result.inattentiveSubscore}
				</div>
				<div>
					<span class="font-medium text-base-content/70">Hyperactive-impulsive subscore:</span>
					{result.hyperactiveImpulsiveSubscore}
				</div>
				<div>
					<span class="font-medium text-base-content/70">Part A screener:</span>
					...
					<tbody>
						{#each result.firedRules as rule (rule.id)}
							<tr class="border-b border-base-200">
								...
								<td class="py-2"><Badge classification={rule.classification} /></td>
							</tr>
						{/each}
					</tbody>
				</table>
		...
		<div class="mb-6 rounded-xl border border-base-300 bg-base-100 p-6">
			...
			<div class="grid grid-cols-1 gap-4 text-sm sm:grid-cols-2">
				...
				<div>
					<span class="font-medium text-base-content/70">Occupation:</span>
