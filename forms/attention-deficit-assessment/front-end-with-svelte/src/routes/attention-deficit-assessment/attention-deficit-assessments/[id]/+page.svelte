<script lang="ts">
	...
	function submit() {
		...
		assessment.result = calculateASRS(assessment.data);
		goto(`/attention-deficit-assessment/attention-deficit-assessments/${id}/report`);
	}
...
<main class="mx-16 px-4 py-6">
	<h1 class="text-2xl font-bold text-base-content">
		{isNew ? 'New ADHD assessment' : `ADHD assessment ${id}`}
	</h1>
	<p class="mt-1 text-sm text-base-content/70">
		Complete the ten sections; the ASRS score and ADHD classification are computed on submit.
	</p>
	...
	<Form label="ADHD assessment" onsubmit={submit}>
		<Step1Demographics />
		<Step2ASRSPartA />
		<Step3ASRSPartB />
