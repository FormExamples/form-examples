# Attention Deficit Assessment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Attention Deficit Assessment form, built with axum + Loco +
...
- [`attention_deficit_assessment/`](./attention_deficit_assessment/) — the Loco crate: `src/attention_deficit_assessment/` holds `app.rs`
...
`src/attention_deficit_assessment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd attention_deficit_assessment && cargo check --all-targets && cargo test
```
