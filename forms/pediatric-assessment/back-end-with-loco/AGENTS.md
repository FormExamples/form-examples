# Pediatric Assessment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Pediatric Assessment form, built with axum + Loco +
...
- [`pediatric_assessment/`](./pediatric_assessment/) — the Loco crate: `src/pediatric_assessment/` holds `app.rs`
...
`src/pediatric_assessment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd pediatric_assessment && cargo check --all-targets && cargo test
```
