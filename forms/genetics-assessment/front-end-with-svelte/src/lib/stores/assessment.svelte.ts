...
export { createDefaultAssessment, emptyFixedRelative, emptyRelative };

/** localStorage draft key for a given assessment id (defaults to `new`). */
function storageKey(id: string): string {
	return `genetics-assessment.front-end-with-svelte.${id || 'new'}.v1`;
}

/**
 * Svelte 5 reactive store for the genetics assessment, with localStorage
...
class AssessmentStore {
	 ...
	 * (e.g. `const d = assessment.data.probandDemographics`) stay bound.
	 */
