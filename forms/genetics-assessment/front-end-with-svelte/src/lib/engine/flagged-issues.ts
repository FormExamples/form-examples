...
// Tyrer-Cuzick scoring (which the grader resolves into the overall risk level).
// They surface clinician-facing issues such as testing recommendations,
// single-line family history, paternal-line considerations, modifier risk
// factors and general counselling reminders.

import type { AssessmentData, AdditionalFlag } from './types';
import { calculateManchesterScore, countBethesdaMet } from './genetics-grader';

export function detectAdditionalFlags(data: AssessmentData): AdditionalFlag[] {
	const flags: AdditionalFlag[] = [];

	const manchesterScore = calculateManchesterScore(data);
	const bethesdaMet = countBethesdaMet(data);
	const premm5Raw = data.targetedRiskScoring.premm5.externalPREMM5Percent;
	const premm5Score = premm5Raw === null || premm5Raw === undefined ? null : Number(premm5Raw);
	const tcRaw = data.targetedRiskScoring.tyrerCuzick.externalLifetimeRisk;
	...
	// Multiple early-onset / paediatric cancers
	...
	if (paediatricCount >= 1) {
		flags.push({
			...
			category: 'Cancer Spectrum',
			message:
				'Paediatric cancer (<18) reported — consider Li-Fraumeni and other paediatric tumour syndromes.',
			priority: 'high'
		});
	}
	...
	if (data.personalMedicalHistory.multiplePrimaryCancers === 'yes') {
		flags.push({
			...
			message:
				'Proband has multiple primary cancers — pre-test probability of pathogenic variant elevated.',
			priority: 'high'
		});
	}
	...
	const maternalAffected = [fp.maternalGrandmother, fp.maternalGrandfather, fp.mother]
		.concat(fp.maternalAuntsUncles || [])
		.concat(fp.maternalCousins || [])
		.filter((r) => r.affectedWithCancer === 'yes' && (r.cancers || []).length > 0).length;
	const paternalAffected = [fp.paternalGrandmother, fp.paternalGrandfather, fp.father]
		.concat(fp.paternalAuntsUncles || [])
		.concat(fp.paternalCousins || [])
		.filter((r) => r.affectedWithCancer === 'yes' && (r.cancers || []).length > 0).length;
	...
	if (
		...
		flags.push({
			...
			message:
				'Cancer history concentrated on the maternal line; paternal line under-represented — request additional paternal pedigree where possible.',
			priority: 'medium'
		});
	} else if (
		paternalAffected > 0 &&
		...
		flags.push({
			...
			category: 'Pedigree Limitation',
			message:
				'Cancer history concentrated on the paternal line; maternal line under-represented — request additional maternal pedigree where possible.',
			priority: 'medium'
		});
	}
	...
	if (paternalAffected > 0) {
		flags.push({
			...
			message:
				'Affected paternal-line relatives — remember that BRCA1/2 transmits paternally; consider full paternal pedigree.',
			priority: 'medium'
		});
	}
	...
	if (data.consanguinityAncestry.ashkenaziJewish === 'yes') {
		flags.push({
			...
			category: 'Ancestry',
			message:
				'Ashkenazi Jewish ancestry — three BRCA1/2 founder variants account for the majority of mutations in this population.',
			priority: 'medium'
		});
	}
	if (data.consanguinityAncestry.sephardicJewish === 'yes') {
		flags.push({
			...
			category: 'Ancestry',
			message:
				'Sephardic Jewish ancestry — population-specific founder variants exist; consider extended panel.',
			priority: 'medium'
		});
	}
	...
	if (data.consanguinityAncestry.consanguinity === 'yes') {
		flags.push({
			...
			message:
				'Consanguinity reported — increased recurrence risk for autosomal recessive conditions; offer carrier screening.',
			priority: 'medium'
		});
	}
	...
	if (
		data.targetedRiskScoring.tyrerCuzick.atypicalHyperplasia === 'yes' ||
		...
		flags.push({
			...
			message:
				'Atypical hyperplasia or LCIS reported — strong individual risk modifier; ensure included in Tyrer-Cuzick model run.',
			priority: 'medium'
		});
	}
	if (data.targetedRiskScoring.tyrerCuzick.dense === 'yes') {
		flags.push({
			...
			category: 'Breast Risk Modifier',
			message:
				'Dense mammographic breast tissue — modest independent risk modifier and reduces mammographic sensitivity.',
			priority: 'medium'
		});
	}
	...
	if (data.priorGeneticTesting.variantsOfUncertainSignificance === 'yes') {
		flags.push({
			...
			message:
				'Variant(s) of uncertain significance previously reported — review variant interpretation databases for reclassification.',
			priority: 'medium'
		});
	}
	if (data.priorGeneticTesting.familialVariantKnown === 'yes') {
		flags.push({
			...
			category: 'Prior Testing',
			message:
				'Known familial pathogenic variant — predictive testing for the proband is the appropriate first-line approach.',
			priority: 'high'
		});
	}
	...
	if (data.patientUnderstandingConcerns.insuranceConcerns === 'yes') {
		flags.push({
			...
			message:
				'Patient has insurance concerns — discuss GINA / local protections before testing.',
			priority: 'medium'
		});
	}
	if (data.patientUnderstandingConcerns.confidentialityConcerns === 'yes') {
		flags.push({
			...
			category: 'Counselling',
			message:
				'Patient has confidentiality concerns — review consent and information-sharing arrangements.',
			priority: 'medium'
		});
	}
	...
	if (data.patientUnderstandingConcerns.reproductiveImplications === 'yes') {
		flags.push({
			...
			message:
				'Patient asking about reproductive implications — offer preconception / prenatal genetic counselling.',
			priority: 'low'
		});
	}

	if (
		data.priorGeneticTesting.priorGeneticCounselling === 'no' ||
		...
		flags.push({
			...
			message:
				'No prior formal genetic counselling recorded — pre-test counselling appointment recommended.',
			priority: 'low'
		});
	}
