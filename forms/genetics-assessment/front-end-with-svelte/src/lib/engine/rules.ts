...
//
// Each rule examines the AssessmentData (and derived GraderContext) and either
// fires (returning a severity: 'low' | 'moderate' | 'high') or does not
// (returning ''). The grader collects every fired rule, picks the maximum
// severity for the final risk level, and reports the rule audit trail.
//
// Reference thresholds:
//   - Manchester score >= 15  -> consider BRCA testing (moderate)
...
//   - Tyrer-Cuzick lifetime >= 17% moderate, >= 30% high (NICE FH)
...
import type { GeneticsRule } from './types';

export const rules: GeneticsRule[] = [
	...
	},
	{
		...
		category: 'Manchester Score',
		description:
			'Manchester score >= 30 — high probability of BRCA1/2 mutation; testing strongly indicated.',
	...
	// ─── Bethesda criteria (Lynch syndrome) ───────────────────
	{
		...
		category: 'Bethesda',
		description:
			'At least one Revised Bethesda criterion met — MMR IHC / MSI testing indicated.',
	...
	},
	{
		...
		category: 'Bethesda',
		description:
			'Two or more Revised Bethesda criteria met — strong indication for Lynch syndrome workup.',
	...
	// ─── PREMM5 threshold ─────────────────────────────────────
	{
		...
		evaluate: (_d, ctx) => (ctx.premm5Score !== null && ctx.premm5Score >= 5 ? 'high' : '')
	},
	...
	// ─── Tyrer-Cuzick (IBIS) lifetime risk ────────────────────
	{
		...
		description:
			'Tyrer-Cuzick lifetime risk >= 17% — moderate-risk breast surveillance per NICE FH.',
		evaluate: (_d, ctx) =>
	...
	},
	{
		...
		evaluate: (_d, ctx) => (ctx.affectedFirstDegree === 2 ? 'moderate' : '')
	},
	{
		id: 'GEN-FAM-003',
		category: 'Family History',
		description:
			'Multiple early-onset cancers (<=50) in the family — strongly suggestive of hereditary syndrome.',
	...
	},
	{
		...
		category: 'Family History',
		description:
			'Paediatric cancer cluster — at least one paediatric (<18) cancer in close relatives.',
	...
	},
	{
		...
		description:
			'Multiple primary cancers in proband — pathogenic-variant pre-test probability elevated.',
	...
	// ─── Ancestry-based risk modifiers ────────────────────────
	{
		...
		category: 'Ancestry',
		description:
			'Ashkenazi Jewish ancestry plus breast / ovarian / pancreatic cancer in proband or family — BRCA founder mutation testing recommended.',
		evaluate: (d, ctx) => {
			if (d.consanguinityAncestry.ashkenaziJewish !== 'yes') return '';
			const probandCancers = (d.personalMedicalHistory.cancers || []).map((c) =>
				(c.type || '').toLowerCase()
			);
			const probandHasBOP = probandCancers.some(
				(t) => t.includes('breast') || t.includes('ovar') || t.includes('pancrea')
			);
	...
	},
	{
		...
		category: 'Ancestry',
		description:
			'Consanguinity reported — increased risk of recessive conditions; carrier-screening relevant.',
	...
	},
	{
		...
		category: 'Ancestry',
		description:
			'Founding-population ancestry reported — population-specific founder variants may apply.',
	...
	// ─── Suspected syndrome (clinician-entered) ───────────────
	{
		...
		description:
			'Referring clinician has named a suspected syndrome — formal genetics review indicated.',
	...
	// ─── Lifestyle / general counselling ──────────────────────
	{
		...
		description:
			'Patient has reproductive concerns — preconception / prenatal genetic counselling relevant.',
