...
use crate::engine::{genetics_grader, types::AssessmentData, utils};
...
/// GET /assessment/{id} -- render the single-page genetics-assessment wizard.
#[debug_handler]
...
fn section_to_json(section: &str) -> &str {
    match section {
        "proband_demographics" => "probandDemographics",
        "presenting_concern" => "presentingConcern",
        "personal_medical_history" => "personalMedicalHistory",
        "consanguinity_ancestry" => "consanguinityAncestry",
        "prior_genetic_testing" => "priorGeneticTesting",
        "patient_understanding_concerns" => "patientUnderstandingConcerns",
        "recommendation_referral_plan" => "recommendationReferralPlan",
        other => other,
    }
}

/// Convert a snake_case field to camelCase JSON.
...
/// Parse a form value. Empty strings stay as ""; numeric strings become numbers.
        ...
        if let Some(form_obj) = form_data.as_object() {
            for (key, value) in form_obj {
                ...
                if parts.len() == 2 {
                    ...
                    // section.subsection.field — for nested targetedRiskScoring.
                    let section = section_to_json(parts[0]);
                    let subsection = field_to_json(parts[1]);
...
async fn show_report(
    ...
    let grade = genetics_grader::grade(&assessment_data);
    ...
    context.insert("risk_level", &grade.risk_level);
    context.insert("risk_label", &utils::risk_level_label(&grade.risk_level));
    context.insert("risk_class", &utils::risk_level_class(&grade.risk_level));
    context.insert("manchester_score", &grade.manchester_score);
    context.insert("bethesda_met", &grade.bethesda_met);
    context.insert("premm5_score", &grade.premm5_score);
    context.insert("tyrer_cuzick_lifetime", &grade.tyrer_cuzick_lifetime);
