# Genetics Assessment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Genetics Assessment form, built with axum + Loco +
...
- [`genetics_assessment/`](./genetics_assessment/) — the Loco crate: `src/genetics_assessment/` holds `app.rs`
...
`src/genetics_assessment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd genetics_assessment && cargo check --all-targets && cargo test
```
