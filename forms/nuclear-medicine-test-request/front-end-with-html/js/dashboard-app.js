// Nuclear Medicine Test Request — vetting dashboard
...
// prep-safety-band dropdown + radiation-dose-band dropdown + safety-flags
// dropdown).
//
// Sibling modules loaded as plain `<script>` tags (in dependency order)
// attach their exports to `window.NuclearMedicineTestRequestDashboard`.
...
const columns = [
  ...
  { key: 'prepSafetyBand',      label: 'Prep Safety' },
...
function prepClass(band) {
  return band ? 'prep-' + String(band) : '';
}
...
function matchesFilters(row) {
  ...
  if (filters.prep && row.prepSafetyBand !== filters.prep) return false;
  if (filters.dose && row.radiationDoseBand !== filters.dose) return false;
...
function compareRows(a, b) {
  ...
  if (key === 'prepSafetyBand') {
    return ((prepRank[av] ?? -1) - (prepRank[bv] ?? -1)) * dir;
  }
...
function renderTableBody() {
  ...
  for (const row of rows) {
    ...
    if (row.triageTier === 'emergency' || row.prepSafetyBand === 'contraindicated') {
    ...
    tr.innerHTML = `
      ...
      <td class="data-table-td"><span class="prep-badge ${prepClass(row.prepSafetyBand)}">${esc(titleCase(row.prepSafetyBand))}</span></td>
...
function bindFilterInputs() {
  ...
  const prep = document.getElementById('filter-prep');
  const dose = document.getElementById('filter-dose');
  ...
  if (prep) {
    prep.addEventListener('change', () => {
      filters.prep = prep.value;
      renderAll();
    });
  }
  if (dose) {
    dose.addEventListener('change', () => {
      filters.dose = dose.value;
      renderAll();
    });
  }
  if (flags) {
