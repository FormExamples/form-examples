...
import type { NuclearMedicineRequest, GradingResult } from '#lib/engine/types.js';
import { createDefault } from '#lib/engine/defaults.js';

/** localStorage draft key for a given request id (defaults to `new`). */
function storageKey(id: string): string {
	return `nuclear-medicine-test-request.front-end-with-svelte.${id || 'new'}.v1`;
}

/** A blank nuclear medicine request with all fields at their unanswered defaults. */
export function createDefaultRequest(): NuclearMedicineRequest {
	return createDefault();
}

/**
 * Svelte 5 reactive store for the nuclear medicine request, with localStorage
 ...
 */
class RequestStore {
	data = $state<NuclearMedicineRequest>(createDefaultRequest());
	 ...
	 * (e.g. `const d = request.data.safety`) stay bound to live state.
	 */
	loadForId(id: string, seed?: NuclearMedicineRequest) {
		const key = id || 'new';
		this.id = key;
		this.result = null;
		this.currentStep = 1;

		let draft: NuclearMedicineRequest | null = null;
		if (browser) {
			const raw = localStorage.getItem(storageKey(key));
			if (raw) {
				try {
					draft = JSON.parse(raw) as NuclearMedicineRequest;
				} catch {
