...
// pregnancy, breastfeeding, high-radiation-dose,
...
// every front-end and the back-end. Ported verbatim from the HTML
// front-end's flags.js.

import type { NuclearMedicineRequest, Flag, RadiationDoseBand } from './types';
...
export function detectFlags(data: NuclearMedicineRequest, context: FlagContext = {}): Flag[] {
	const flags: Flag[] = [];
	const ctx = context;
	...
	if (data.safety.pregnancyStatus === 'pregnant') {
		flags.push({
			...
			suggestedAction:
				'Do not administer until justified by a nuclear-medicine physician; consider alternative non-ionising imaging.'
		});
