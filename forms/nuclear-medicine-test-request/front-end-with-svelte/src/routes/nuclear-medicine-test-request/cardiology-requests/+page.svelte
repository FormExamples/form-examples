<script lang="ts">
	...
	import { sampleReferralRows } from '$lib/data/sample-reports';
	import {
	...
	const columns = [
		{ id: 'id', header: 'Referral', width: 90 },
		{ id: 'patientName', header: 'Patient', flexgrow: 2, sort: true },
		{
			id: 'requestedService',
			header: 'Service',
			flexgrow: 2,
			template: (v: string) => requestedServiceLabel(v as never)
		},
		{
			id: 'referralReason',
			header: 'Reason',
			flexgrow: 2,
			template: (v: string) => referralReasonLabel(v as never)
		},
		{ id: 'status', header: 'Status', width: 110, template: (v: string) => statusLabel(v as never) },
		{
		...
		{ id: 'safetyBand', header: 'Safety', width: 120, template: (v: string) => safetyLabel(v as never) },
	...
	function init(api: any) {
		...
		api.on('select-row', (ev: { id?: string | number }) => {
			if (ev?.id != null) goto(`/nuclear-medicine-test-request/cardiology-requests/${ev.id}`);
		});
	}
</script>
...
<main class="mx-auto max-w-6xl px-4 py-6">
	<div class="mb-6 flex flex-wrap items-end justify-between gap-4">
		<div>
			<h1 class="text-2xl font-bold text-base-content">Cardiology vetting dashboard</h1>
			<p class="mt-1 text-sm text-base-content/70">
				Four-axis vetting grades for incoming cardiology referrals, computed by the shared engine.
				Select a row to open the referral.
			</p>
		</div>
		<a href="/nuclear-medicine-test-request/cardiology-requests/new" class="button" data-variant="primary">New referral</a>
	</div>
