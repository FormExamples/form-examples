<script lang="ts">
	...
	function init(api: any) {
		...
		api.on('select-row', (ev: { id?: string | number }) => {
			if (ev?.id != null) goto(`/advance-decision-to-refuse-treatment/advance-decisions-to-refuse-treatment/${ev.id}`);
		});
	}
</script>
...
<main class="mx-auto max-w-6xl px-4 py-6">
	<div class="mb-6 flex flex-wrap items-end justify-between gap-4">
		...
		<a href="/advance-decision-to-refuse-treatment/advance-decisions-to-refuse-treatment/new" class="button" data-variant="primary">New ADRT</a>
	</div>
