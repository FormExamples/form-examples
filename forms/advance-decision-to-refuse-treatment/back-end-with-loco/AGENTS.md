# Advance Decision to Refuse Treatment — Back-end with Rust Axum Loco (JSON API)

Pure JSON API back-end for the Advance Decision to Refuse Treatment form, built with axum + Loco +
...
- [`advance_decision_to_refuse_treatment/`](./advance_decision_to_refuse_treatment/) — the Loco crate: `src/advance_decision_to_refuse_treatment/` holds `app.rs`
...
`src/advance_decision_to_refuse_treatment/` also carries the form-specific scoring engine (types + a grader
...
```sh
cd advance_decision_to_refuse_treatment && cargo check --all-targets && cargo test
```
