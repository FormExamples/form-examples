...
def build_fhir_resource(table_name, columns, form_slug, main_table=None, has_patient=True):
    ...
    # ClinicalImpression.subject is 1..1 (required). A form with no patient
    # table (non-clinical forms: arc42, meeting, issue-tracker, ...) has no
    # valid Patient subject, so emit the neutral Observation (subject 0..1)
    # instead of an invalid ClinicalImpression.
    if classification == 'clinical_impression' and not has_patient:
        classification = 'observation'

    if classification == 'encounter':
        resource = build_encounter_resource(table_name, columns, form_slug)
        ...
        resource = build_fired_rule_resource(table_name, columns, form_slug)
    elif classification == 'additional_flag':
        resource = build_additional_flag_resource(table_name, columns, form_slug)
    else:
    ...
    # When a form has no patient table, every builder's hardcoded
    # `subject: Patient/<uuid>` points at a Patient resource that is never
    # emitted. All remaining resource types (Observation, Encounter,
    # DetectedIssue) have an optional subject, so drop it rather than dangle.
    if not has_patient and isinstance(resource, dict):
...
def process_form(form_dir):
    ...
    has_patient = any(t[0] == "patient" for t in all_tables)

    fhir_dir.mkdir(parents=True, exist_ok=True)

    count = 0
    for table_name, columns in all_tables:
        resource = build_fhir_resource(table_name, columns, form_slug, main_table, has_patient)
    ...
    # Remove stale *.json files that this run no longer produces: renamed
    # tables (e.g. grading_result -> grade), merged per-section assessment_*
