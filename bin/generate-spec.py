...
def render_spec(slug: str, form_dir: Path) -> str:
    ...
    out.append("- The HTML front-ends conform to the Lily Design System contract")
    out.append("  ([`forms/AGENTS-front-end-html.md`](../AGENTS-front-end-html.md)).")
    out.append("- The SvelteKit front-ends pass `pnpm check` and `pnpm test`.")
    out.append("- The Rust crate builds (`cargo build`) and tests pass (`cargo test`).")
    out.append("- LocalStorage key `{}.front-end-form-with-html.v1` (HTML) and".format(slug))
    out.append("  `{}.front-end-form-with-svelte.v1` (SvelteKit) preserve draft".format(slug))
    out.append("  state across reloads.")
    out.append("")
