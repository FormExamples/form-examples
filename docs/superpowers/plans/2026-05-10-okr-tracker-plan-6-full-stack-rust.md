...
2. `bin/test-form objective-and-key-result-tracker` passes (this is the repo-wide gate; passes only when the full-stack scaffold + templates exist).
...
```
forms/objective-and-key-result-tracker/full-stack-with-loco-tera-htmx-alpine/
...
**Files:**
- Modify: `forms/objective-and-key-result-tracker/full-stack-with-loco-tera-htmx-alpine/Cargo.toml`
...
edition = "2024"
publish = false
default-run = "objective-and-key-result-tracker-cli"
...
[[bin]]
name = "objective-and-key-result-tracker-cli"
...
```sh
git add forms/objective-and-key-result-tracker/full-stack-with-loco-tera-htmx-alpine/Cargo.toml
...
- [ ] **Step 1: Write the setup script** at `forms/objective-and-key-result-tracker/full-stack-with-loco-tera-htmx-alpine-setup` (an executable shell script). Pattern from `forms/issue-tracker/full-stack-with-loco-tera-htmx-alpine-setup`:

```sh
#!/bin/sh
set -euf
createuser --host=localhost --port=5432 --username=postgres --login --createdb loco || :
createdb --host=localhost --port=5432 --username=postgres --owner=loco objective_and_key_result_tracker_development || :
createdb --host=localhost --port=5432 --username=postgres --owner=loco objective_and_key_result_tracker_test || :
createdb --host=localhost --port=5432 --username=postgres --owner=loco objective_and_key_result_tracker_production || :
loco new --name objective-and-key-result-tracker --db postgres --bg async --assets serverside
```
...
```sh
chmod +x forms/objective-and-key-result-tracker/full-stack-with-loco-tera-htmx-alpine-setup
forms/objective-and-key-result-tracker/full-stack-with-loco-tera-htmx-alpine-setup
```
...
Cross-check each migration against the equivalent file in `forms/objective-and-key-result-tracker/sql-migrations/`. The schemas must match column-for-column so the existing Plan 1 SQL migrations could (in principle) interoperate with this SeaORM schema.
...
```sh
git add forms/objective-and-key-result-tracker/full-stack-with-loco-tera-htmx-alpine/{templates/base.html.tera,assets/static/style.css}
...
- [ ] **Step 1: Fill in** `forms/objective-and-key-result-tracker/full-stack-with-loco-tera-htmx-alpine/{index.md,AGENTS.md,plan.md,tasks.md}`. Reference Plan 1's scoring engine, this plan, the parent design spec.

- [ ] **Step 2: Run `bin/test-form objective-and-key-result-tracker` and expect it to PASS now** that the full-stack scaffold + base template exist.
...
- [ ] **Step 6:** `bin/test-form objective-and-key-result-tracker` — passes.
- [ ] **Step 7:** Tag
