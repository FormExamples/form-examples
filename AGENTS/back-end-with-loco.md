...
- **OTLP exporter** → ships metrics + traces to an external OpenTelemetry
  collector for centralized aggregation (Tempo, Jaeger, Mimir, …).
...
- `bin/loco-seed-data-rollout [--check] [--dry-run] [--all|<slug>…]` — implements tasks.md's "Loco seed data: per-crate seeder loading `examples/` typical fixture; document `cargo loco db seed`". Every scaffolded crate ships an empty (or `users`-auth-table-only) `App::seed()`; this tool discovers each crate's domain tables from `models/_entities/*.rs`, parses each `Model` struct's fields, detects foreign keys (a column typed to match another discovered table's own PK type and named `<table>_id`), topologically sorts the tables, and emits one deterministic-id YAML fixture per table plus a `db::seed::<T>()` call sequence inserted into the *existing* `seed()` body (never replacing whatever it already seeds). A fleet survey done before generalizing overturned this repo's own prior assumption that the fleet was uniformly UUID-keyed (per the relational-schema convention): only 2/338 crates (`cardiology-request`, `cardiology-response`) use a `Uuid` PK — the other 336 use the scaffold's `i64` auto-increment default, which the source-of-truth `sql/` never specifies (it's always `UUID`) — so both a `det_uuid()` and a `det_int_id()` path are generated depending on each table's actual PK type; a nullable FK column is left `null` rather than guessed at. 355/356 crates changed 2026-09-12 (1 SKIP: `united-kingdom-statement-of-fitness-for-work`'s main-table entity carries a genuinely self-referential `..._id` column with no matching SQL/`is_duplicate_of` column — a real, separate, pre-existing entity-generation bug, flagged not fixed here). A same-day sibling, `bin/generate-api-transcripts`, live-tested every crate's actual seeded server and found 3 more real bugs this tool's own `cargo check` couldn't see (a foreign key resolved by name-guessing instead of the entity's own `belongs_to` relation, breaking on irregular plurals/semantic column names; a hardcoded-nullable `deleted_at` where one crate's is actually required; a name heuristic matching a `bool` column by substring and handing it a string) — all fixed here, re-verified live. `--check` is the CI drift detector
- `bin/generate-api-transcripts [--check] [<slug>…]` — records a real `examples/api-create.http` transcript (a live POST-then-GET against the seeded crate's own real server, not a reimplemented one — the same fidelity rule as `bin/generate-export-samples`) for every crate with the fleet's uniform `controllers/patient.rs` shape (336/356). Reuses `bin/loco-integration-test-rollout`'s field-parsing/value-synthesis so the POSTed body matches exactly what that tool's own generated test already sends. Slow (a real `cargo run ... start` per crate); an occasional generator, not a CI gate. Found and fixed 3 real bugs in `bin/loco-seed-data-rollout` (see above) plus 3 environment-specific findings while proving it out: macOS resolves a bare `localhost` bind to IPv6-only, so probing `127.0.0.1` gets refused even though the server is listening; POST mounts without the prefix's own trailing slash while GET-by-id needs it; the same 9 crates `bin/loco-missing-request-test-stubs-fix` already found were "built via an earlier/different process" default to Postgres port `5433`, not the fleet's usual `5432`. 336/336 eligible crates captured 2026-09-12 (20 SKIP: no uniform patient controller, same set that tool's own docstring already names)
